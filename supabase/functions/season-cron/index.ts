...
Deno.serve(async (req: Request) => {
  ...
  for (const dl of deadlines ?? []) {
    ...
    const { data: room } = await client.from("season_rooms").select("id, phase").eq("id", dl.room_id).single();
    if (!room) continue;

    const { data: members } = await client.from("season_room_members").select("participant_id, franchise_id").eq("room_id", dl.room_id);
    const { data: existingDecisions } = await client.from("season_private_decisions").select("participant_id").eq("room_id", dl.room_id).eq("cursor", dl.cursor);
    const existingSet = new Set((existingDecisions ?? []).map((d: { participant_id: string }) => d.participant_id));
    ...
    for (const m of members ?? []) {
      if (!existingSet.has(m.participant_id)) {
        ...
        const { data: mem } = await client.from("season_room_members").select("miss_streak, control").eq("room_id", dl.room_id).eq("participant_id", m.participant_id).single();
        if (mem) {
          const nextStreak = (mem.miss_streak ?? 0) + 1;
          const nextControl = nextStreak >= 3 ? "ai-takeover" : mem.control;
          await client.from("season_room_members").update({ miss_streak: nextStreak, control: nextControl }).eq("room_id", dl.room_id).eq("participant_id", m.participant_id);
        }
      }
    }

    // mark deadline resolved as timeout-default (or ai-takeover if 3 misses)
    await client.from("season_deadlines").update({ resolution_source: "timeout-default", grace_ends_at: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString() }).eq("room_id", dl.room_id).eq("cursor", dl.cursor);
    resolved++;
  }
  results.resolvedDeadlines = resolved;

  // 2) handle grace expiry -> expired
  const { data: graceRooms } = await client.from("season_deadlines").select("room_id, grace_ends_at").not("grace_ends_at", "is", null).lt("grace_ends_at", nowIso);
  const graceRoomIds = [...new Set((graceRooms ?? []).map((r: { room_id: string }) => r.room_id))];
  for (const rid of graceRoomIds) {
    const { data: dl } = await client.from("season_deadlines").select("resolution_source").eq("room_id", rid).is("resolution_source", null).limit(1);
    if (!dl || dl.length === 0) {
      // all deadlines resolved but grace expired without human action -> mark expired if still not completed
      await client.from("season_rooms").update({ phase: "expired", expires_at: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(), updated_at: nowIso }).eq("id", rid).in("phase", ["market", "private-lock", "simulation", "hash-verification"]);
    }
  }

  // 3) cleanup season_join_attempts >24h
  const dayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
  const { count: deletedAttempts } = await client.from("season_join_attempts").delete({ count: "exact" }).lt("created_at", dayAgo);
  results.deletedAttempts = deletedAttempts;

  // 4) clear expired codes
  await client.from("season_rooms").update({ code: null, code_expires_at: null }).not("code_expires_at", "is", null).lt("code_expires_at", nowIso);
  ...
  const { data: emptyRooms } = await client.from("season_rooms").select("id").lt("created_at", fifteenAgo).in("phase", ["waiting", "drafting"]);
  let closedEmpty = 0;
  for (const r of emptyRooms ?? []) {
    const { count } = await client.from("season_room_members").select("participant_id", { count: "exact", head: true }).eq("room_id", r.id);
    if ((count ?? 0) === 0) {
  ...
  const { data: staleWaiting } = await client.from("season_rooms").select("id, updated_at").eq("phase", "waiting").lt("updated_at", thirtyAgo);
  let closedStale = 0;
  for (const r of staleWaiting ?? []) {
    const { count } = await client.from("season_room_members").select("participant_id", { count: "exact", head: true }).eq("room_id", r.id);
    if ((count ?? 0) === 1) {
      await client.from("season_rooms").update({ phase: "expired", expires_at: new Date(Date.now() + 60 * 60 * 1000).toISOString(), updated_at: nowIso }).eq("id", r.id);
      closedStale++;
    }
  }
