...
Deno.serve(async (req: Request) => {
  ...
  if (member.participant_id !== participantId) return json(403, { code: "authorization", message: "participant mismatch" });
  if (member.control === "surrendered") return json(400, { code: "authorization", message: "already surrendered" });
  const { error } = await sc.from("season_room_members").update({ reclaim_requested: true }).eq("room_id", roomId).eq("participant_id", participantId);
