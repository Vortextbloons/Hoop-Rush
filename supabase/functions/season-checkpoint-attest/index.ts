...
Deno.serve(async (req: Request) => {
  ...
  if (
    ...
    !att.roomId ||
    !att.cursor ||
    typeof att.attempt !== 'number' ||
    !att.participantId ||
    !att.inputDigest ||
    !att.resultDigest
  )
    return json(400, { code: 'phase', message: 'invalid attestation' });
  if (att.attempt < 1 || att.attempt > 3)
    return json(400, { code: 'phase', message: 'invalid attempt' });
  ...
  if (member.participant_id !== att.participantId)
  ...
  const { data: existing } = await serviceClient
    ...
    .maybeSingle();
  if (existing) {
    if (existing.input_digest !== att.inputDigest || existing.result_digest !== att.resultDigest)
      return json(400, { code: 'hash-mismatch', message: 'attestation digest mismatch' });
  } else {
  ...
  if (a.input_digest === b.input_digest && a.result_digest === b.result_digest) {
    await serviceClient
      .from('season_rooms')
      .update({
        phase: 'checkpoint-setup',
        cursor: att.cursor,
        revision: (room.revision ?? 0) + 1,
        digest: a.result_digest,
    ...
    return json(200, {
      ...
      accepted: {
        roomId: att.roomId,
        cursor: att.cursor,
        inputDigest: a.input_digest,
        resultDigest: a.result_digest,
  ...
  return json(200, {
    ...
    failure: {
      roomId: att.roomId,
      cursor: att.cursor,
      expectedInputDigest: a.input_digest,
      expectedResultDigest: a.result_digest,
