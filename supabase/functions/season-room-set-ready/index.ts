...
Deno.serve(async (req: Request) => {
  ...
  if (participantId !== 'p1' && participantId !== 'p2')
    return json(400, { code: 'phase', message: 'invalid participant' });
  ...
  if (!isV2Ready)
    console.warn(
      `set-ready for room ${room.id} with version ${room.multiplayer_version}/${room.room_protocol_version}, allowing`,
    );
  if (room.phase !== 'waiting')
    return json(400, { code: 'phase', message: 'can only set ready in waiting phase' });
  ...
  if (participantId === 'p1')
    return json(403, { code: 'authorization', message: 'only guest can set ready' });
  if (member.participant_id !== participantId)
    return json(403, { code: 'authorization', message: 'cannot set ready for other participant' });
  ...
  const snap = {
    ...
    mode: (updated as unknown as { mode?: string }).mode ?? 'season',
    settingsRevision:
      (updated as unknown as { settings_revision?: number }).settings_revision ?? currentRev,
