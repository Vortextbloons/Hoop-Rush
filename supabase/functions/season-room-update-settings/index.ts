...
Deno.serve(async (req: Request) => {
  ...
  if (!['season', 'classic', 'sandbox'].includes(mode))
    return json(400, { code: 'phase', message: 'invalid mode' });
  if (!['live', 'async'].includes(pace))
  ...
  if (!isV2Update)
    console.warn(
      `update-settings for room ${room.id} with version ${room.multiplayer_version}/${room.room_protocol_version}, allowing`,
    );
  if (room.phase !== 'waiting')
    return json(400, { code: 'phase', message: 'can only change settings in waiting phase' });
  ...
  if (member.participant_id !== 'p1')
    return json(403, { code: 'authorization', message: 'only host can update settings' });
  ...
  const snap = {
    ...
    mode: (updated as unknown as { mode?: string }).mode ?? mode,
    settingsRevision:
      (updated as unknown as { settings_revision?: number }).settings_revision ?? currentRev + 1,
    guestReady: false,
    presence,
