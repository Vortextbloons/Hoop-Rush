...
async function resolveUid(req:Request,url:string,srk:string):Promise<string|null>{
  ...
  if(ah){const ac=createClient(url,Deno.env.get('SUPABASE_ANON_KEY')??srk,{global:{headers:{Authorization:ah}}});const{data:{user}}=await ac.auth.getUser();if(user) return user.id;}
  const du=req.headers.get('x-dev-uid');if(du&&/^[0-9a-f-]{36}$/i.test(du)) return du;return null;
}
Deno.serve(async (req:Request)=>{
  ...
  const targetPid = (participantId==='p1'||participantId==='p2')?participantId: (member.participant_id as 'p1'|'p2');
  if(targetPid!==member.participant_id) return json(403,{code:'authorization',message:'cannot leave as other participant'});
  ...
  if(room && room.phase==='waiting'){
    ...
    if((count??0)===1){
      ...
      await sc.from('season_rooms').update({code, code_expires_at:new Date(Date.now()+15*60*1000).toISOString(), guest_ready:false, updated_at:new Date().toISOString()} as unknown as Record<string,unknown>).eq('id',roomId);
    } else if((count??0)===0){
      await sc.from('season_rooms').update({code:null,code_expires_at:null,guest_ready:false} as unknown as Record<string,unknown>).eq('id',roomId);
    }
  }
