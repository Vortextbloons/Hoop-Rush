...
Deno.serve(async (req: Request) => {
  ...
  if (
    ...
    return json(400, { code: 'phase', message: 'invalid submission' });
  const size = new TextEncoder().encode(JSON.stringify(sub.payload ?? '')).length;
  if (size > CHECKPOINT_MAX)
    return json(400, { code: 'phase', message: 'private decision too large' });

  const serviceClient = createClient(supabaseUrl, serviceRoleKey);
  const { data: member } = await serviceClient
    .from('season_room_members')
    .select('*')
    .eq('room_id', sub.roomId)
    .eq('uid', uid)
    .maybeSingle();
