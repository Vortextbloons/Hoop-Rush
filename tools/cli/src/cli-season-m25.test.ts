...
import { m25ToleranceGate, m25RangeGate, m25LiftGate, m25GapGate, gateSummary, mean, share, rateBasisPoints, seasonCalibrationSeed, seedIndexRange, } from './commands/season-calibration.ts';
import { durabilityRatingFrom, durabilityProfileOf } from './gen-season-draft-catalog.ts';
import { seasonHealthCalibrateReportSchema, seasonInfluenceCalibrateReportSchema, seasonTradeCalibrateReportSchema, seasonFullSimulateReportSchema, seasonBlockSimulateReportSchema, } from './report-schemas.ts';
describe('season m25 gate helpers', () => {
    it('tolerance gates pass inside the envelope and skip below the sample floor', () => {
        expect(m25ToleranceGate('incidence', 85, 80, 15, 100000, 50000).status).toBe('pass');
        expect(m25ToleranceGate('incidence', 96, 80, 15, 100000, 50000).status).toBe('fail');
        const skipped = m25ToleranceGate('incidence', 85, 80, 15, 100, 50000);
    ...
    it('lift gates assert direction (high > low) with a sample floor', () => {
        expect(m25LiftGate('minutes', 106, 94, 400000, 200000).pass).toBe(true);
        expect(m25LiftGate('minutes', 94, 106, 400000, 200000).pass).toBe(false);
        expect(m25LiftGate('minutes', 106, 94, 10, 200000).status).toBe('skippedInsufficientSample');
    });
    it('gap gates require the frozen absolute minimum gap', () => {
        expect(m25GapGate('recurrence', 40, 15, 400000, 200000).pass).toBe(true);
        expect(m25GapGate('recurrence', 10, 15, 400000, 200000).pass).toBe(false);
    });
