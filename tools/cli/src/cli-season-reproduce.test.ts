...
import { buildSeasonRunReplayExport, contentHashSchema, franchiseIdSchema, seasonCommandLogDigest, seasonRunReplayExportSchema, type SeasonAlmanac, type SeasonCommandLogEntry, } from '@hoop-rush/data-contracts';
...
import { buildFreeAgencyReplayedRun, buildReplayedRun, chainLog, freeAgencyReplayDeps, replayDeps, } from './season-reproduce-test-support.ts';
let sharedReplayDeps: ReturnType<typeof replayDeps> | null = null;
function cachedReplayDeps(): ReturnType<typeof replayDeps> {
    if (sharedReplayDeps === null)
        sharedReplayDeps = replayDeps();
...
function cachedFreeAgencyReplayDeps(): ReturnType<typeof freeAgencyReplayDeps> {
    if (sharedFreeAgencyReplayDeps === null)
        sharedFreeAgencyReplayDeps = freeAgencyReplayDeps();
...
describe('season run reproduce (replay-export-v1)', () => {
    ...
    it('fails at the exact ordinal when a command post-state digest diverges', () => {
        ...
        const mutatedEntries = log.entries.map((entry, index) => index === log.entries.length - 1 ? { ...entry, postStateDigest: 'f'.repeat(32) } : entry);
        ...
        expect(divergence?.commandId).toBe(mutatedEntries[mutatedEntries.length - 1]?.command.commandId);
    });
    it('rejects an ordinal gap as a chain-fact divergence', () => {
        const { exportInput } = buildReplayedRun();
        const entries = exportInput.commandLog.entries;
        const gapped = [entries[0], entries[2]].filter((entry): entry is SeasonCommandLogEntry => entry !== undefined);
    ...
    it('reproduces through the CLI end-to-end and fails precisely on a corrupted log', async () => {
        await withTmpDir(async (dir) => {
            ...
            writeFileSync(exportPath, `${JSON.stringify(buildSeasonRunReplayExport(exportInput), null, 2)}\n`);
    ...
    it('reproduces a byte-identical pass on the same file twice', async () => {
        await withTmpDir(async (dir) => {
            ...
            const firstPayload = seasonRunReproduceReportSchema.parse(jsonPayload(first.stdout, first.stderr));
            const secondPayload = seasonRunReproduceReportSchema.parse(jsonPayload(second.stdout, second.stderr));
...
describe('season run reproduce free-agency divergence (M2.6.5)', () => {
    it('reproduces a free-agency replay export with no divergence', () => {
        ...
        const { divergence, divergences } = replaySeasonRunExport(exportArtifact, cachedFreeAgencyReplayDeps());
        expect(divergence).toBeNull();
        expect(divergences).toEqual([]);
    });
    it('reports a free-agency divergence when a recorded canonical candidate is missing', () => {
        const { exportInput } = buildFreeAgencyReplayedRun((freeAgency) => {
            ...
            if (first !== undefined) {
                freeAgency.canonicalCandidates = Object.fromEntries(Object.entries(freeAgency.canonicalCandidates).filter(([key]) => key !== first));
            }
        });
        const { divergence } = replaySeasonRunExport(buildSeasonRunReplayExport(exportInput), cachedFreeAgencyReplayDeps());
        expect(divergence).not.toBeNull();
    ...
    it('fails the CLI command on a tampered free-agency export and passes on the clean file', async () => {
        await withTmpDir(async (dir) => {
            ...
            writeFileSync(exportPath, `${JSON.stringify(buildSeasonRunReplayExport(clean.exportInput), null, 2)}\n`);
            const ok = await runCli([
                'season',
                'run',
                'reproduce',
            ...
            writeFileSync(tamperedPath, `${JSON.stringify(buildSeasonRunReplayExport(tampered.exportInput), null, 2)}\n`);
            const bad = await runCli([
                'season',
                'run',
                'reproduce',
