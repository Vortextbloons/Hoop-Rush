...
import { seasonEffectsSensitivity, seasonEffectsDistribution, seasonEffectsRoles, runSeasonEffectsCohortInProcess, validateSeasonEffectTargets, defaultEffectsEngineDeps, DEFAULT_EFFECT_TARGETS, } from './commands/season-effects';
import { SEASON_EFFECTS_ASSIST_CONVERSION_MAX_PP, SEASON_EFFECTS_DEFENSE_FATIGUE_MAX_PP, SEASON_EFFECTS_HANDLER_FATIGUE_MAX_PP, SEASON_EFFECTS_HELP_DEFENSE_MAX_PP, SEASON_EFFECTS_SHOOTER_FATIGUE_MAX_PP, SEASON_EFFECTS_TURNOVER_SECURITY_MAX_PP, checkSeasonGameResult, createSeasonEffectsState, simulateSeasonGame, simulateSeasonGameWithEffects, } from '@hoop-rush/engine';
import { SEASON_EFFECT_TARGETS_VERSION } from '@hoop-rush/data-contracts';
import { seasonEffectsCalibrateReportSchema, seasonEffectsDistributionReportSchema, seasonEffectsRolesReportSchema, seasonEffectsSensitivityReportSchema, } from './report-schemas';
...
describe('season effects CLI commands', () => {
    ...
    it('distribution stays inside the production envelopes on a small cohort', async () => {
        const report = await seasonEffectsDistribution({ 'seed-from': '0', 'seed-to': '5', workers: '1' }, (request) => runSeasonEffectsCohortInProcess(request, {
            simulateSeasonGame,
    ...
    it('calibrate runs the gates with injected doubles and reports the payload', async () => {
        ...
        const report = await seasonEffectsCalibrate({ 'seed-to': '3', workers: '1' }, {
            simulateSeasonGame,
            checkSeasonGameResult,
            simulateSeasonGameWithEffects,
            createSeasonEffectsState,
        }, (request) => runSeasonEffectsCohortInProcess(request, {
            simulateSeasonGame,
            checkSeasonGameResult,
            simulateSeasonGameWithEffects,
            createSeasonEffectsState,
