...
describe('cli: calibrate commands', () => {
    ...
    it('calibrate run exits 1 when a gate fails', async () => {
        await withTmpDir(async (tmp) => {
            const badProfile = JSON.parse(readFileSync(join(REPO_ROOT, 'apps/web/static/data/era-sim/1990s.json'), 'utf8')) as {
            ...
            if (!pointsTarget)
                throw new Error('profile lacks pointsPerGame target');
