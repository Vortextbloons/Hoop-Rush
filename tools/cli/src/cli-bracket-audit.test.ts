...
import { jsonPayload, REPO_ROOT, runCli, withTmpDir, writeManifestWithBracket, } from './cli-test-helpers.ts';
describe('cli: bracket audit', () => {
    ...
    it('exits 1 when the bracket fails validation', async () => {
        await withTmpDir(async (tmp) => {
            const packaged = JSON.parse(readFileSync(join(REPO_ROOT, 'apps/web/static/data/opponents/bracket.json'), 'utf8')) as {
                schedule: Array<{
                    gameNumber: number;
                    opponentId: string;
                }>;
            };
            const repeated = packaged.schedule.map((entry, index) => index === 1 ? { ...entry, opponentId: packaged.schedule[0]?.opponentId ?? '' } : entry);
            ...
            copyFileSync(join(REPO_ROOT, 'apps/web/static/data/opponents/lakers-1990s-opening.json'), join(opponentsDir, 'lakers-1990s-opening.json'));
