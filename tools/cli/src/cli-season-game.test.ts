...
import { SEASON_ROTATION_PRESET_TARGETS, SEASON_ROTATION_VERSION, seasonGameSimulationResultSchema, seasonGameTargetsSchema, seasonRotationPresetSchema, slotGroupOf, type SeasonGameSimulationInput, type SeasonGameSimulationResult, type SeasonGameTeamInput, type SeasonRotation, type SeasonRotationPreset, } from '@hoop-rush/data-contracts';
import { seasonGameFixtureSchema } from './fixture-schema.ts';
import { SEASON_GAME_CALIBRATION_SEED_COUNT, SEASON_GAME_VALIDATION_SEED_COUNT, seasonGameCalibrationSeed, seasonGameCalibrate, seasonGameSimulate, type SeasonGameGameFacts, type SeasonGameCohortRunner, } from './commands/season-game.ts';
import { UsageError } from './args.ts';
import { seasonGameCalibrateReportSchema, seasonGameSimulateReportSchema, } from './report-schemas.ts';
...
function loadFixture(id: string) {
    return seasonGameFixtureSchema.parse(JSON.parse(readFileSync(join(FIXTURES_DIR, `${id}.json`), 'utf8')));
}
describe('cli: committed season game fixtures', () => {
    it('every fixture is schema-valid with a matching id and legal roster', () => {
        for (const id of ALL_FIXTURE_IDS) {
            ...
            for (const id2 of versionIds)
                expect(availableIds).toContain(id2);
        }
    });
    it('preset fixtures use v2 rotations with the frozen preset targets totaling 240', () => {
        for (const id of PRESET_FIXTURE_IDS) {
            ...
            for (const rotation of [fixture.input.homeRotation, fixture.input.awayRotation]) {
                ...
                const benchRoleByVersion = new Map(rotation.benchOrder.map((playerVersionId, index) => [playerVersionId, index + 1]));
    ...
    it('scenario fixtures carry their intended availability, removals, and foul traits', () => {
        ...
        const homeUnavailable = unavailablePlayers.find((entry) => unavailable.home.players.some((p) => p.playerVersionId === entry.playerVersionId));
        const awayUnavailable = unavailablePlayers.find((entry) => unavailable.away.players.some((p) => p.playerVersionId === entry.playerVersionId));
        ...
        for (const entry of removal.removals) {
            ...
            expect([...removal.home.players, ...removal.away.players].map((p) => p.playerVersionId)).toContain(entry.playerVersionId);
        }
        ...
        const homeAvailable = noLegalFive.availability.filter((entry) => entry.available &&
        ...
        const homeAvailableBoth = both.availability.filter((entry) => entry.available &&
            both.home.players.some((p) => p.playerVersionId === entry.playerVersionId));
        const awayAvailableBoth = both.availability.filter((entry) => entry.available &&
    ...
    it('non-preset fixtures omit the preset field', () => {
        for (const id of ALL_FIXTURE_IDS) {
            if (PRESET_FIXTURE_IDS.includes(id as (typeof PRESET_FIXTURE_IDS)[number]))
                continue;
            const fixture = loadFixture(id);
...
describe('season game simulate (unit, injected doubles)', () => {
    it('completes a game with a validated payload and renders the score and minutes', () => {
        ...
        const check = vi.fn(() => []);
        const report = seasonGameSimulate({ input: 'season-game-balanced', seed: 'ab'.repeat(16) }, { simulateSeasonGame: simulate, checkSeasonGameResult: check });
    ...
    it('omitting --seed uses the fixture embedded seed', () => {
        const simulate = vi.fn((input: SeasonGameSimulationInput) => buildCompletedResult(input));
        seasonGameSimulate({ input: 'season-game-balanced', seed: null }, { simulateSeasonGame: simulate, checkSeasonGameResult: () => [] });
    ...
    it('reports a forfeit with the official 2-0 line (double)', () => {
        ...
        const report = seasonGameSimulate({ input: 'season-game-no-legal-five', seed: 'cd'.repeat(16) }, { simulateSeasonGame: () => forfeit, checkSeasonGameResult: () => [] });
        expect(report.exitCode).toBe(0);
        const payload = seasonGameSimulateReportSchema.parse(report.payload);
    ...
    it('reports the no-legal-five-both variant without scores', () => {
        ...
        const report = seasonGameSimulate({ input: 'season-game-no-legal-five-both', seed: 'ef'.repeat(16) }, {
    ...
    it('surfaces invariant failures with exit 1 and pass false', () => {
        const report = seasonGameSimulate({ input: 'season-game-balanced', seed: 'ab'.repeat(16) }, {
    ...
    it('throws usage errors for missing input, bad seeds, and unknown fixtures', () => {
        ...
        expect(() => seasonGameSimulate({ input: 'season-game-balanced', seed: 'not-hex' })).toThrow(UsageError);
        expect(() => seasonGameSimulate({ input: 'season-game-missing', seed: 'ab'.repeat(16) })).toThrow(UsageError);
    });
    it('resolves a fixture path directly through --input', () => {
        const path = join(FIXTURES_DIR, 'season-game-balanced.json');
        const simulate = vi.fn((input: SeasonGameSimulationInput) => buildCompletedResult(input));
        const report = seasonGameSimulate({ input: path, seed: 'ab'.repeat(16) }, { simulateSeasonGame: simulate, checkSeasonGameResult: () => [] });
        expect(report.exitCode).toBe(0);
        expect(seasonGameSimulateReportSchema.parse(report.payload).fixtureId).toBe('season-game-balanced');
    });
});
const PRESET_SECONDS: Record<SeasonRotationPreset, {
    starter: number;
    bench: number[];
}> = {
...
function presetFact(fixtureId: string, seedIndex: number): SeasonGameGameFacts {
    ...
    for (let i = 0; i < 10; i += 1)
        starterPairs.push(seconds.starter);
    return {
        fixtureId,
        seedIndex,
...
function fakeRunner(factory: (fixtureId: string, seedIndex: number) => SeasonGameGameFacts, options: {
...
describe('season game calibrate (unit, injected doubles)', () => {
    it('passes all gates on the frozen cohort and writes the targets artifact', async () => {
        await withTmpDir(async (tmp) => {
            ...
            const manifestBefore = readFileSync(join(REPO_ROOT, 'apps/web/static/data/manifest.json'), 'utf8');
            const report = await seasonGameCalibrate({ out: targetsPath, workers: '4' }, { runCohort: fakeRunner(presetFact) });
            expect(report.exitCode).toBe(0);
            ...
            const manifestAfter = readFileSync(join(REPO_ROOT, 'apps/web/static/data/manifest.json'), 'utf8');
    ...
    it('worker counts never change aggregates (1 vs 4 workers)', async () => {
        await withTmpDir(async (tmp) => {
            const run = async (workers: string) => {
                const report = await seasonGameCalibrate({ workers, out: join(tmp, `game-targets-workers-${workers}.json`) }, { runCohort: fakeRunner(presetFact) });
    ...
    it('fails the bench ordering gate when the bench-heavy fixture benches little', async () => {
        await withTmpDir(async (tmp) => {
            ...
            const report = await seasonGameCalibrate({ out: join(tmp, 'game-targets-ordering.json') }, { runCohort: fakeRunner(factory) });
            expect(report.exitCode).toBe(1);
            const payload = seasonGameCalibrateReportSchema.parse(report.payload);
            ...
            expect(report.failures.some((failure) => failure.includes('benchOrdering') || failure.includes('bench second medians'))).toBe(true);
        });
    });
    it('fails the held-out gate when validation points leave the envelopes', async () => {
        await withTmpDir(async (tmp) => {
            ...
            const report = await seasonGameCalibrate({ out: join(tmp, 'game-targets-heldout.json') }, { runCohort: fakeRunner(factory) });
            const payload = seasonGameCalibrateReportSchema.parse(report.payload);
    ...
    it('fails the zero-failures gate on check failures and determinism divergences', async () => {
        await withTmpDir(async (tmp) => {
            const factory = (fixtureId: string, seedIndex: number): SeasonGameGameFacts => {
                const fact = presetFact(fixtureId, seedIndex);
                if (seedIndex === 5)
                    return { ...fact, checks: ['legality: unavailable player on court'] };
                if (seedIndex === 7)
                    return { ...fact, deterministic: false };
                return fact;
            };
            const report = await seasonGameCalibrate({ out: join(tmp, 'game-targets-zerofailures.json') }, { runCohort: fakeRunner(factory) });
            const payload = seasonGameCalibrateReportSchema.parse(report.payload);
    ...
    it('detects worker-count dependence through the chunking probe', async () => {
        await withTmpDir(async (tmp) => {
            ...
            const report = await seasonGameCalibrate({ out: join(tmp, 'game-targets-chunking.json') }, { runCohort: fakeRunner(presetFact, { perturbSingleChunk: perturb }) });
            const payload = seasonGameCalibrateReportSchema.parse(report.payload);
    ...
    it('refuses to freeze when a preset fixture is missing from the selection', async () => {
        await withTmpDir(async (tmp) => {
            const report = await seasonGameCalibrate({ fixture: 'season-game-tight', out: join(tmp, 'game-targets-single.json') }, { runCohort: fakeRunner(presetFact) });
            const payload = seasonGameCalibrateReportSchema.parse(report.payload);
    ...
    it('throws usage errors for malformed ranges', async () => {
        await expect(seasonGameCalibrate({ 'seed-from': '5', 'seed-to': '1' }, { runCohort: fakeRunner(presetFact) })).rejects.toThrow(UsageError);
    });
});
