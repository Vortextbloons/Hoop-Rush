...
import { SEASON_EMPTY_COMMAND_LOG_DIGEST, buildSeasonRunReplayExport, contentHashSchema, eraIdSchema, franchiseIdSchema, idSchema, seasonCommandLogDigest, seasonCommandLogEntrySchema, seasonCommandLogSchema, seasonPostseasonSummaryDigest, seasonPostseasonSummarySchema, type FranchiseId, type SeasonPostseasonSummary, } from '@hoop-rush/data-contracts';
...
import { auditSeasonPostseasonFacts, seasonPostseasonAuditFixtureSchema, } from './commands/season-postseason-audit.ts';
...
function baseSummary(facts: {
    ...
    const summary = seasonPostseasonSummarySchema.parse({
        ...
        homePlayers: status === 'forfeit'
            ? []
            : Array.from({ length: 10 }, (_, index) => zeroLine(`pv-${String(index).padStart(32, '0')}`)),
        awayPlayers: status === 'forfeit'
            ? []
            : Array.from({ length: 10 }, (_, index) => zeroLine(`pv-${String(index + 10).padStart(32, '0')}`)),
        rotationEvidence: status === 'forfeit'
...
function sweep(seriesId: string, round: SeasonPostseasonSummary['round'], conference: 'east' | 'west', homeCourt: string, challenger: string, winner: string): SeasonPostseasonSummary[] {
...
function validPostseason(): SeasonPostseasonSummary[] {
    ...
    for (const conference of ['east', 'west'] as const) {
        ...
        firstRound.forEach((pair, index) => {
            ...
            summaries.push(...sweep(seriesIds[index] as string, 'first-round', conference, home, away, home));
        ...
        semifinalPairs.forEach((pair, index) => {
            summaries.push(...sweep(`${conferenceLetter}sf${String(index + 1)}`, 'conference-semifinal', conference, pair.home, pair.away, pair.home));
        });
        summaries.push(...sweep(`${conferenceLetter}cf`, 'conference-final', conference, semifinalPairs[0]?.home ?? 'lakers', semifinalPairs[1]?.home ?? 'knicks', semifinalPairs[0]?.home ?? 'lakers'));
    }
...
function runWithChampion(championRaw: string) {
    ...
    const emptySeries = (seriesIdRaw: string, round: 'first-round' | 'conference-semifinal' | 'conference-final' | 'finals', conference: 'east' | 'west' | null, higherSeed: number | null, lowerSeed: number | null) => ({
    ...
    const conferenceBracket = (conference: 'east' | 'west', seeds: FranchiseId[]) => {
        ...
        return {
            conference,
            seeds,
            firstRound: Array.from({ length: 4 }, (_, index) => emptySeries(`${letter}fr${String(index + 1)}`, 'first-round', conference, 8 - index, index + 1)),
            semifinals: Array.from({ length: 2 }, (_, index) => emptySeries(`${letter}sf${String(index + 1)}`, 'conference-semifinal', conference, 4 - index, index + 1)),
...
describe('season postseason audit (postseason-v2)', () => {
    ...
    it('detects an incomplete bracket (missing teams)', () => {
        ...
        expect(failures.some((failure) => failure.includes('instead of 8') || failure.includes('has no recorded series'))).toBe(true);
    });
    it('detects an invalid play-in final pairing (invalid feeders)', () => {
        ...
        expect(failures.some((failure) => failure.includes('must pair the seven-eight loser'))).toBe(true);
        expect(failures.some((failure) => failure.includes('re-pairs the eliminated nine-ten loser'))).toBe(true);
    });
    it('detects an incorrect play-in final host (home court)', () => {
        ...
        expect(failures.some((failure) => failure.includes('must be hosted by the seven-eight loser'))).toBe(true);
    });
    ...
    it('detects an inconsistent summary digest (inconsistent summaries)', () => {
        ...
        expect(failures.some((failure) => failure.includes('result digest does not recompute'))).toBe(true);
    });
    it('detects a series ending before four wins (inconsistent summaries)', () => {
        ...
        expect(failures.some((failure) => failure.includes('ends before any team reached four wins'))).toBe(true);
    });
    it('detects champion/completion mismatches (fixture and export paths)', () => {
        ...
        expect(fixture.failures.some((failure) => failure.includes('recorded champion warriors does not match the finals winner lakers'))).toBe(true);
        ...
        expect(stateMismatch.failures.some((failure) => failure.includes('completion champion warriors does not match the state machine champion'))).toBe(true);
    });
    it('runs the CLI end-to-end on an audit fixture (exit 0 clean, exit 1 corrupt)', async () => {
        await withTmpDir(async (dir) => {
            ...
            const badPayload = seasonPostseasonAuditReportSchema.parse(jsonPayload(bad.stdout, bad.stderr));
