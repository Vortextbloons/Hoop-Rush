...
import { makeReport, renderJson, renderText, EXIT_OK, EXIT_USAGE_OR_DATA_ERROR, type CliReport, } from './report.ts';
...
const COMMANDS: Record<string, CommandEntry> = {
    ...
    'data overalls-distribution': command(async () => {
        const { dataOverallsDistribution, DATA_OVERALLS_DISTRIBUTION_OPTIONS } = await import('./commands/data-overalls-distribution.ts');
        const { DEFAULT_MANIFEST } = await import('./commands/data-loader.ts');
        return {
            options: DATA_OVERALLS_DISTRIBUTION_OPTIONS,
            run: (args) => dataOverallsDistribution({ input: getOptionString(args, 'input') ?? DEFAULT_MANIFEST }),
        };
    }),
    'data defense-bpm-correlation': command(async () => {
        const { defenseBpmCorrelation, DATA_DEFENSE_BPM_CORRELATION_OPTIONS } = await import('./commands/data-defense-bpm-correlation.ts');
        const { DEFAULT_MANIFEST } = await import('./commands/data-loader.ts');
        return {
            options: DATA_DEFENSE_BPM_CORRELATION_OPTIONS,
            run: (args) => defenseBpmCorrelation({ input: getOptionString(args, 'input') ?? DEFAULT_MANIFEST }),
        };
    }),
    ...
    'data lineage-audit': command(async () => {
        const { dataLineageAudit, DATA_LINEAGE_AUDIT_OPTIONS } = await import('./commands/data-lineage-audit.ts');
        const { DEFAULT_MANIFEST } = await import('./commands/data-loader.ts');
        return {
    ...
    'bracket audit': command(async () => {
        ...
        return {
            ...
            run: (args) => bracketAudit(getOptionString(args, 'input') ?? DEFAULT_MANIFEST, hasOption(args, 'verbose')),
        };
    }),
    'bracket generate': command(async () => {
        const { bracketGenerate, BRACKET_GENERATE_OPTIONS } = await import('./commands/bracket-generate.ts');
        return {
    ...
    'calibrate ratings': command(async () => {
        const { calibrateRatings, CALIBRATE_RATINGS_OPTIONS } = await import('./commands/calibrate-ratings.ts');
        return {
    ...
    'calibrate three-point': command(async () => {
        const { calibrateThreePoint, CALIBRATE_THREE_POINT_OPTIONS } = await import('./commands/calibrate-three-point.ts');
        return {
    ...
    'season schedule generate': command(async () => {
        const { seasonScheduleGenerate, SEASON_SCHEDULE_GENERATE_OPTIONS } = await import('./commands/season-schedule.ts');
        return {
    ...
    'season schedule audit': command(async () => {
        const { seasonScheduleAudit, SEASON_SCHEDULE_AUDIT_OPTIONS } = await import('./commands/season-schedule.ts');
        return {
    ...
    'season draft reproduce': command(async () => {
        const { seasonDraftReproduce, SEASON_DRAFT_REPRODUCE_OPTIONS } = await import('./commands/season-draft.ts');
        return {
    ...
    'season rosters generate': command(async () => {
        const { seasonRostersGenerate, SEASON_ROSTERS_GENERATE_OPTIONS } = await import('./commands/season-rosters.ts');
        return {
    ...
    'season rosters audit': command(async () => {
        const { seasonRostersAudit, SEASON_ROSTERS_AUDIT_OPTIONS } = await import('./commands/season-rosters.ts');
        return {
    ...
    'season rosters calibrate': command(async () => {
        const { seasonRostersCalibrate, SEASON_ROSTERS_CALIBRATE_OPTIONS } = await import('./commands/season-rosters.ts');
        return {
    ...
    'season draft calibrate': command(async () => {
        const { seasonDraftCalibrate, SEASON_DRAFT_CALIBRATE_OPTIONS } = await import('./commands/season-draft-calibrate.ts');
        return {
    ...
    'season game simulate': command(async () => {
        const { seasonGameSimulate, SEASON_GAME_SIMULATE_OPTIONS } = await import('./commands/season-game.ts');
        return {
    ...
    'season game calibrate': command(async () => {
        const { seasonGameCalibrate, SEASON_GAME_CALIBRATE_OPTIONS } = await import('./commands/season-game.ts');
        return {
    ...
    'season block simulate': command(async () => {
        const { seasonBlockSimulate, SEASON_BLOCK_SIMULATE_OPTIONS } = await import('./commands/season-block.ts');
        return {
    ...
    'season block audit': command(async () => {
        const { seasonBlockAudit, SEASON_BLOCK_AUDIT_OPTIONS } = await import('./commands/season-block.ts');
        return {
    ...
    'season full simulate': command(async () => {
        const { seasonFullSimulate, SEASON_FULL_SIMULATE_OPTIONS } = await import('./commands/season-block.ts');
        return {
    ...
    'season home-court calibrate': command(async () => {
        const { seasonHomeCourtCalibrate, SEASON_HOME_COURT_CALIBRATE_OPTIONS } = await import('./commands/season-home-court.ts');
        return {
    ...
    'season effects sensitivity': command(async () => {
        const { seasonEffectsSensitivity, SEASON_EFFECTS_OPTIONS } = await import('./commands/season-effects.ts');
        return {
    ...
    'season effects distribution': command(async () => {
        const { seasonEffectsDistribution, SEASON_EFFECTS_OPTIONS } = await import('./commands/season-effects.ts');
        return {
            options: SEASON_EFFECTS_OPTIONS,
    ...
    'season effects roles': command(async () => {
        const { seasonEffectsRoles, SEASON_EFFECTS_OPTIONS } = await import('./commands/season-effects.ts');
        return {
            options: SEASON_EFFECTS_OPTIONS,
    ...
    'season effects calibrate': command(async () => {
        const { seasonEffectsCalibrate, SEASON_EFFECTS_OPTIONS } = await import('./commands/season-effects.ts');
        return {
            options: SEASON_EFFECTS_OPTIONS,
    ...
    'season health calibrate': command(async () => {
        const { seasonHealthCalibrate, SEASON_HEALTH_CALIBRATE_OPTIONS } = await import('./commands/season-health.ts');
        return {
    ...
    'season campaign audit': command(async () => {
        const { seasonCampaignAudit, SEASON_CAMPAIGN_AUDIT_OPTIONS } = await import('./commands/season-campaign.ts');
        return {
    ...
    'season campaign calibrate': command(async () => {
        const { seasonCampaignCalibrate, SEASON_CAMPAIGN_CALIBRATE_OPTIONS } = await import('./commands/season-campaign.ts');
        return {
    ...
    'season trade audit': command(async () => {
        const { seasonTradeAudit, SEASON_TRADE_AUDIT_OPTIONS } = await import('./commands/season-trade.ts');
        return {
    ...
    'season trade calibrate': command(async () => {
        const { seasonTradeCalibrate, SEASON_TRADE_CALIBRATE_OPTIONS } = await import('./commands/season-trade.ts');
        return {
    ...
    'season influence calibrate': command(async () => {
        const { seasonInfluenceCalibrate, SEASON_INFLUENCE_CALIBRATE_OPTIONS } = await import('./commands/season-influence.ts');
        return {
    ...
    'season free-agency audit': command(async () => {
        const { seasonFreeAgencyAudit, SEASON_FREE_AGENCY_AUDIT_OPTIONS } = await import('./commands/season-free-agency-audit.ts');
        return {
    ...
    'season free-agency calibrate': command(async () => {
        const { seasonFreeAgencyCalibrate, SEASON_FREE_AGENCY_CALIBRATE_OPTIONS } = await import('./commands/season-free-agency-calibrate.ts');
        return {
    ...
    'season run reproduce': command(async () => {
        const { seasonRunReproduce, SEASON_RUN_REPRODUCE_OPTIONS } = await import('./commands/season-reproduce.ts');
        return {
    ...
    'season postseason audit': command(async () => {
        const { seasonPostseasonAudit, SEASON_POSTSEASON_AUDIT_OPTIONS } = await import('./commands/season-postseason-audit.ts');
        return {
    ...
    'season postseason calibrate': command(async () => {
        const { seasonPostseasonCalibrate, SEASON_POSTSEASON_CALIBRATE_OPTIONS } = await import('./commands/season-postseason-calibrate.ts');
        return {
    ...
    'season benchmark block': command(async () => {
        const { seasonBenchmarkBlock, SEASON_BENCHMARK_OPTIONS } = await import('./commands/season-benchmark.ts');
        return {
    ...
    'season benchmark full': command(async () => {
        const { seasonBenchmarkFull, SEASON_BENCHMARK_OPTIONS } = await import('./commands/season-benchmark.ts');
        return {
            options: SEASON_BENCHMARK_OPTIONS,
    ...
    'season benchmark determinism': command(async () => {
        const { seasonBenchmarkDeterminism, SEASON_BENCHMARK_OPTIONS } = await import('./commands/season-benchmark.ts');
        return {
            options: SEASON_BENCHMARK_OPTIONS,
    ...
    'season benchmark persistence': command(async () => {
        const { seasonBenchmarkPersistence, SEASON_BENCHMARK_OPTIONS } = await import('./commands/season-benchmark.ts');
        return {
            options: SEASON_BENCHMARK_OPTIONS,
    ...
    'projection season': command(async () => {
        const { projectionSeason, PROJECTION_SEASON_OPTIONS } = await import('./commands/projection.ts');
        return {
    ...
    'projection calibrate-base': command(async () => {
        const { projectionCalibrateBase, PROJECTION_CALIBRATE_OPTIONS } = await import('./commands/projection.ts');
        return {
    ...
    'projection benchmark': command(async () => {
        const { projectionBenchmark, PROJECTION_BENCHMARK_OPTIONS } = await import('./commands/projection.ts');
        return {
    ...
    'projection ai-shadow': command(async () => {
        const { projectionAiShadow, PROJECTION_AI_SHADOW_OPTIONS } = await import('./commands/projection.ts');
        return {
