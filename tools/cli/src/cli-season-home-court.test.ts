...
import { seasonHomeCourtCalibrate, seasonHomeCourtTargetsSchema, simulateSeasonHomeCourtFacts, validateSeasonHomeCourtTargets, } from './commands/season-home-court.ts';
...
import type { SeasonGameSimulationResult, SeasonGameSimulationInput, SeasonGameSideResult, } from '@hoop-rush/data-contracts';
...
describe('season home-court calibration helpers', () => {
    it('derives facts that flip toward the home team under the tuned profile', () => {
        ...
        const facts = simulateSeasonHomeCourtFacts('balanced', 0, input, SEASON_HOME_COURT_PROFILE, fakeDeps);
    ...
    it('validates the committed artifact against the engine profile', () => {
        ...
        expect(parsed.constants.homeDefensiveCommunication).toBe(SEASON_HOME_COURT_PROFILE.homeDefensiveCommunication);
        expect(parsed.constants.awayTurnoverPressure).toBe(SEASON_HOME_COURT_PROFILE.awayTurnoverPressure);
        expect(seasonHomeCourtProfileSchema.safeParse(SEASON_HOME_COURT_PROFILE).success).toBe(true);
        expect(seasonHomeCourtMechanisms(SEASON_HOME_COURT_PROFILE).homeDefenseShotAdjustment).toBeLessThan(0);
        expect(seasonHomeCourtMechanisms(SEASON_HOME_COURT_PROFILE).awayTurnoverPressureAdjustment).toBeGreaterThan(0);
    });
});
describe('cli: season home-court calibrate (injected doubles)', () => {
    it('writes the evidence artifact when the gates pass', async () => {
        await withTmpDir(async (tmp) => {
            ...
            expect(artifact.gameVersion).toBe('season-game-v4');
    ...
    it('rejects a tampered artifact through --validate', async () => {
        await withTmpDir((tmp) => {
            ...
            const content = JSON.parse(readFileSync(committed, 'utf8')) as {
                constants: {
                    homeDefensiveCommunication: number;
                };
            };
