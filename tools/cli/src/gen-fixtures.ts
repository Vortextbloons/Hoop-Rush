...
import { buildEqualFixture, buildLegalSimulationTeam, buildRolesTeam, buildStrongMediumFixture, buildStrongWeakFixture, } from '@hoop-rush/test-fixtures';
import type { SimFixture } from './fixture-schema.ts';
const FIXTURES_DIR = resolve(dirname(fileURLToPath(import.meta.url)), 'fixtures');
function mutateAll(team: SimulationTeam, mutate: (player: SimulationPlayer) => SimulationPlayer): SimulationTeam {
    return { ...team, players: team.players.map(mutate) };
}
function ratingDelta(team: SimulationTeam, key: keyof SimulationPlayer['ratings'], delta: number): SimulationTeam {
    return mutateAll(team, (p) => ({
        ...p,
        ratings: { ...p.ratings, [key]: Math.min(98, Math.max(20, p.ratings[key] + delta)) },
    }));
}
function tendencyDelta(team: SimulationTeam, key: keyof SimulationPlayer['tendencies'], delta: number): SimulationTeam {
    return mutateAll(team, (p) => ({
        ...p,
...
function main(): void {
    ...
    write('sens-creation.json', {
        ...
        home: base,
        away: base,
        variantHome: mutateAll(base, (p) => p.playerId === 'p-fixture-1'
    ...
    write('roles.json', {
        schemaVersion: 2,
        fixtureId: 'roles',
        description: 'Role-differentiated lineup (creator, spacer, secondary, post, rim runner) against itself; player-role calibration gates.',
        home: roleTeam,
