...
function writeOverallsFixture(dataRoot: string, pools: FixturePool[]): string {
    ...
    const refs = pools.map(({ franchiseId, eraId, players }) => {
        ...
        writeFileSync(join(poolsDir, `${franchiseId}-${eraId}.json`), JSON.stringify(buildPool(players.map(({ overall, externalId, seasonKey }) => buildPlayerSeason({
...
function bandOf(bands: readonly {
    label: string;
}[], label: string) {
...
describe('cli: data overalls-distribution', () => {
    it('reports band counts, percentages, medians, and per-era breakdowns', async () => {
        await withTmpDir(async (tmp) => {
            const manifestPath = writeOverallsFixture(tmp, [
                {
                    franchiseId: 'lakers',
                    eraId: '1990s',
                    players: [
    ...
    it('exits 2 with a clean report on an invalid manifest', async () => {
        await withTmpDir((tmp) => expectExit2CleanManifestReport(['data', 'overalls-distribution'], join(tmp, 'apps/web/static/data')));
    });
});
