...
import { SEASON_FREE_AGENCY_INDEX_VERSION, contentHashSchema, seasonDraftCatalogSchema, seasonFreeAgencyIndexSchema, seasonRosterRoleSchema, type SeasonDraftCandidate, type SeasonDraftCatalog, type SeasonFreeAgencyBand, type SeasonFreeAgencyIndex, type SeasonFreeAgencyIndexEntry, type SeasonRosterRole, } from '@hoop-rush/data-contracts';
import { evaluateSeasonRoster, percentileTierOf, playerPercentileTier, rolePercentileThresholds, type PercentileTier, type RoleThresholds, } from '@hoop-rush/engine';
...
function scoreCandidate(candidate: SeasonDraftCandidate, thresholds: Record<SeasonRosterRole, RoleThresholds>): ScoredCandidate {
...
function bestRoleOf(scored: ScoredCandidate): SeasonRosterRole {
    ...
    for (const role of ROSTER_ROLES) {
        if (scored.roleScores[role] > scored.roleScores[best])
            best = role;
    }
    return best;
}
function rolesAtOrAbove(scored: ScoredCandidate, thresholds: Record<SeasonRosterRole, RoleThresholds>, threshold: 'elite' | 'strong' | 'useful'): SeasonRosterRole[] {
    return ROSTER_ROLES.filter((role) => scored.roleScores[role] >= thresholds[role][threshold]);
}
function bandOf(scored: ScoredCandidate, thresholds: Record<SeasonRosterRole, RoleThresholds>, excluded: ReadonlySet<string>): SeasonFreeAgencyBand | null {
    const { candidate } = scored;
    if (excluded.has(candidate.playerVersionId))
        return null;
    if (scored.playerTier === 'strong')
        return 'featured';
    if (scored.playerTier === 'useful')
        return 'role';
    if (candidate.stamina.rating >= DEVELOPMENT_MIN_STAMINA)
        return 'development';
    return 'emergency';
}
function featuredVersionIdOf(scoredGroup: ScoredCandidate[], thresholds: Record<SeasonRosterRole, RoleThresholds>): string | null {
...
function missingFactReason(candidate: SeasonDraftCandidate, catalogVersion: string): string | null {
    if (candidate.positions.playable.length === 0)
        return 'missing playable positions';
    if (catalogVersion === 'season-draft-catalog-v4' && candidate.anchors === undefined) {
        return 'missing validated anchors (v4)';
    }
    return null;
}
function strengthsOf(scored: ScoredCandidate, thresholds: Record<SeasonRosterRole, RoleThresholds>): string[] {
    const strengths: string[] = [];
    for (const role of rolesAtOrAbove(scored, thresholds, 'strong')) {
        if (strengths.length >= ROLE_FACT_CAP)
            break;
        strengths.push(`${role} ${String(rounded(scored.roleScores[role]))} (p75 ${String(rounded(thresholds[role].strong))})`);
    }
...
function limitationsOf(scored: ScoredCandidate, thresholds: Record<SeasonRosterRole, RoleThresholds>): string[] {
    const limitations: string[] = [];
    for (const role of ROSTER_ROLES) {
        if (scored.roleScores[role] >= LIMITATION_MAX_ROLE_SCORE)
            continue;
        if (limitations.length >= LIMITATION_ROLE_FACT_CAP)
            break;
        limitations.push(`${role} ${String(rounded(scored.roleScores[role]))} (p50 ${String(rounded(thresholds[role].useful))})`);
    }
...
function minimumInfluenceOf(band: SeasonFreeAgencyBand, scored: ScoredCandidate, thresholds: Record<SeasonRosterRole, RoleThresholds>): number {
    if (band === 'emergency' || band === 'development')
        return 1;
    if (band === 'featured') {
...
function supportedRolesOf(band: SeasonFreeAgencyBand): SeasonFreeAgencyIndexEntry['supportedRoles'] {
    ...
    if (band === 'featured' || band === 'role')
        roles.push('rotation');
    roles.push('depth');
    roles.push('emergency');
    return roles;
}
function derivationEvidenceOf(scored: ScoredCandidate, thresholds: Record<SeasonRosterRole, RoleThresholds>, capped: boolean): string {
    const { candidate } = scored;
...
function exclusionEvidenceOf(excludedSiblings: ReadonlyArray<{
    version: SeasonDraftCandidate;
    reason: string;
}>, thresholds: Record<SeasonRosterRole, RoleThresholds>): string {
    if (excludedSiblings.length === 0)
        return '';
...
export function deriveFreeAgencyIndex(catalog: SeasonDraftCatalog, catalogContentHash: string): SeasonFreeAgencyIndex {
    const canonical = [...catalog.candidates].sort((a, b) => a.playerVersionId < b.playerVersionId ? -1 : 1);
    ...
    for (const [playerId, group] of eligibleByIdentity) {
        ...
        if (featured !== null)
            featuredIds.add(`${playerId}::${featured}`);
    }
    ...
    for (const playerId of [...groupedVersions.keys()].sort()) {
        ...
        if (versions !== undefined)
            groupedObject[playerId] = versions;
    }
    ...
    if (!parsed.success) {
        throw new Error(`derived free-agency index fails the schema: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
    }
    return parsed.data;
}
export function freeAgencyIndexContent(index: SeasonFreeAgencyIndex): string {
    ...
    if (content.length > FREE_AGENCY_INDEX_MAX_BYTES) {
        throw new Error(`free-agency index exceeds the ${String(FREE_AGENCY_INDEX_MAX_BYTES)}-byte compactness gate (${String(content.length)} bytes)`);
    }
    return content;
}
...
function main(): void {
    ...
    console.log(`wrote ${target} (${String(stats.bytes)} bytes, ${String(stats.candidateCount)} candidates, ${String(stats.identityCount)} identities)`);
    console.log(`bands: featured ${String(stats.bandCounts.featured)} · role ${String(stats.bandCounts.role)} · development ${String(stats.bandCounts.development)} · emergency ${String(stats.bandCounts.emergency)}`);
    console.log(`manifest freeAgencyIndex hash updated (${manifest.season?.freeAgencyIndex?.contentHash ?? 'missing season section'})`);
}
if (IS_ENTRY) {
    main();
}
