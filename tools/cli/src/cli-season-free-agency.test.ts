...
import { SEASON_FREE_AGENCY_TARGETS_VERSION, type SeasonFreeAgencyState, } from '@hoop-rush/data-contracts';
import { jsonPayload, REPO_ROOT, runCli, withTmpDir } from './cli-test-helpers.ts';
import { seasonFreeAgencyAuditReportSchema, seasonFreeAgencyCalibrateReportSchema, } from './report-schemas.ts';
import { auditSeasonFreeAgencyFacts } from './commands/season-free-agency-audit.ts';
import { SEASON_FREE_AGENCY_ABOVE_DRAFTED_MAX, SEASON_FREE_AGENCY_ABOVE_DRAFTED_MIN, SEASON_FREE_AGENCY_OWNERSHIP_MAX, SEASON_FREE_AGENCY_OWNERSHIP_MIN, SEASON_FREE_AGENCY_SKIP_SHARE_MAX, evaluateFreeAgencyGates, medianOf, seasonFreeAgencyTargetsSchema, type SeasonFreeAgencyMeasuredFacts, } from './commands/season-free-agency-calibrate.ts';
...
describe('season free-agency audit CLI', () => {
    it('passes on the committed run fixture and fails precisely on a tampered copy', async () => {
        await withTmpDir(async (dir) => {
            ...
            const badPayload = seasonFreeAgencyAuditReportSchema.parse(jsonPayload(bad.stdout, bad.stderr));
...
describe('season free-agency calibrate gates (pure)', () => {
    function measuredFixture(overrides: Partial<SeasonFreeAgencyMeasuredFacts> = {}): SeasonFreeAgencyMeasuredFacts {
        return {
            seasonsSimulated: 4,
            windowsOpened: 12,
            windowsComplete: 12,
    ...
    it('fails the determinism probe gate on a diverged window', () => {
        const metrics = evaluateFreeAgencyGates(measuredFixture({ determinismProbe: { probed: true, identical: false } }), heldOut, { summaryIdentity: { probed: false, identical: true } });
    ...
    it('skips gates below the minimum season sample (never passing)', () => {
        const metrics = evaluateFreeAgencyGates(measuredFixture({ seasonsSimulated: 2, windowsOpened: 6 }), measuredFixture({ seasonsSimulated: 0 }), { summaryIdentity: { probed: false, identical: true } });
        expect(metrics.find((metric) => metric.key === 'bandSigningCaps')?.status).toBe('skippedInsufficientSample');
        expect(metrics.find((metric) => metric.key === 'bandSigningCaps')?.pass).toBe(false);
    });
});
...
describe('season free-agency helpers (pure)', () => {
    ...
    it('maps trade offer sizes to the v2 package kinds and bands', () => {
        expect(packageKindOfOffer({ outgoingPlayerVersionIds: [1], incomingPlayerVersionIds: [1] })).toBe('1-1');
        expect(packageKindOfOffer({ outgoingPlayerVersionIds: [1, 2], incomingPlayerVersionIds: [3, 4] })).toBe('2-2');
        expect(packageKindOfOffer({ outgoingPlayerVersionIds: [1], incomingPlayerVersionIds: [2, 3] })).toBe('1-2');
        expect(packageKindOfOffer({ outgoingPlayerVersionIds: [1, 2], incomingPlayerVersionIds: [3] })).toBe('2-1');
...
describe.skipIf(!INTEGRATION_RUNS)('season free-agency calibrate (integration-run)', () => {
    it('freezes free-agency-targets-v1 with all gates passing over the full cohort', async () => {
        const { seasonFreeAgencyCalibrate, DEFAULT_FREE_AGENCY_TARGETS } = await import('./commands/season-free-agency-calibrate.ts');
    ...
    it('validates the committed free-agency targets artifact', async () => {
        const { validateSeasonFreeAgencyTargets, DEFAULT_FREE_AGENCY_TARGETS } = await import('./commands/season-free-agency-calibrate.ts');
