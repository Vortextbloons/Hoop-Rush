...
import { SEASON_FREE_AGENCY_TARGETS_VERSION, franchiseIdSchema, seasonRunSchema, } from '@hoop-rush/data-contracts';
...
import { SEASON_FREE_AGENCY_ABOVE_DRAFTED_MAX, SEASON_FREE_AGENCY_ABOVE_DRAFTED_MIN, SEASON_FREE_AGENCY_OWNERSHIP_MAX, SEASON_FREE_AGENCY_OWNERSHIP_MIN, SEASON_FREE_AGENCY_SKIP_SHARE_MAX, evaluateFreeAgencyGates, seasonFreeAgencyTargetsSchema, type SeasonFreeAgencyMeasuredFacts, } from './commands/season-free-agency-calibrate.ts';
...
describe('season free-agency audit CLI', () => {
    it('passes on the committed run fixture and fails precisely on a tampered copy', async () => {
        await withTmpDir(async (dir) => {
            ...
            const rawParsed = seasonRunSchema.safeParse(JSON.parse(readFileSync(RUN_FIXTURE, 'utf8')) as unknown);
            if (!rawParsed.success)
                throw new Error('fixture run fails validation');
