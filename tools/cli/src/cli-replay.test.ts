...
describe('cli: replay', () => {
    it('reproduces a saved input/expected pair byte-for-byte', async () => {
        await withTmpDir(async (tmp) => {
            ...
            const fixture = JSON.parse(readFileSync(join(REPO_ROOT, 'tools/cli/src/fixtures/equal.json'), 'utf8')) as {
                home: unknown;
                away: unknown;
            };
    ...
    it('reports the first structured difference and exits 1 on mismatch', async () => {
        await withTmpDir(async (tmp) => {
            ...
            const profile = JSON.parse(readFileSync(join(REPO_ROOT, 'apps/web/static/data/era-sim/1990s.json'), 'utf8')) as {
                dataVersion: string;
            };
