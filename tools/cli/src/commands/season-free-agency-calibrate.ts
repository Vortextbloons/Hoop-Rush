...
import { z } from 'zod';
import { SEASON_BLOCK_COUNT, SEASON_FREE_AGENCY_TARGETS_VERSION, SEASON_GAME_TARGETS_VERSION, SEASON_GAME_VERSION, SEASON_RUN_SCHEMA_VERSION, type Position, type SeasonCandidateCheckpoint, type SeasonEffectsState, type SeasonFreeAgencyBand, type SeasonFreeAgencyIndex, type SeasonFreeAgencyState, type SeasonFreeAgencyWindowState, type SeasonGameSummary, type SeasonResolveFreeAgentMarketCommand, type SeasonRun, type SeasonRosterTargets, type SeasonSkipFreeAgentMarketCommand, type SeasonSubmitBlockCommand, } from '@hoop-rush/data-contracts';
...
export function seasonFreeAgencyFactsOf(season: SeasonFreeAgencySeasonFacts, draftedMedianOverall: number): SeasonFreeAgencyMeasuredFacts {
    ...
    for (const window of windows) {
        ...
        for (const candidatePlayerVersionId of candidates) {
            ...
            const influenceOf = (franchiseId: string): number => window.declarations[franchiseId]?.targets.find((target) => target.playerVersionId === candidatePlayerVersionId)?.influence ?? 0;
...
export type SeasonFreeAgencySeasonRunner = (rootSeed: string, options: {
    driveFreeAgency: boolean;
    probeWindow: boolean;
}) => SeasonFreeAgencySeasonFacts;
