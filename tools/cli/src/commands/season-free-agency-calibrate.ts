...
import {
  ...
  type SeasonResolveFreeAgentMarketCommand,
  type SeasonRun,
...
import {
  auditSeasonFreeAgencyFacts,
  type SeasonFreeAgencyAuditCounts,
} from './season-free-agency-audit.ts';
...
export function simulateSeasonFreeAgencyFacts(options: {
  ...
  const draftedVersionIds = run.rosters.flatMap((roster) =>
  ...
  let probed = false;
  let determinismProbe: { probed: boolean; identical: boolean } = {
    probed: false,
    identical: true,
  };
    ...
    if (
      ...
      if (options.probeWindow && !probed) {
        ...
        determinismProbe = {
          probed: true,
          identical:
            JSON.stringify(again.freeAgency) === JSON.stringify(opened.freeAgency) &&
...
): Map<string, import('@hoop-rush/data-contracts').SeasonGamePlayerInput> {
...
function assertBlockPreconditions(
  ...
  if (unresolved !== null) {
    throw new Error(
      `block ${String(blockIndex)} rejected: free-agency window ${String(unresolved)} unresolved`,
    );
  }
  ...
  for (const rotation of state.run.rotations) {
    ...
    const members = [...rotation.starters, ...rotation.benchOrder].map((playerVersionId) => {
      if (!rosterIds.has(playerVersionId)) {
        throw new Error(
          `block ${String(blockIndex)}: rotation of ${rotation.franchiseId} references an unrostered player ${playerVersionId}`,
        );
      }
      return {
        playerVersionId,
    ...
    if (validateSeasonRotation(rotation, memberPlayable).length > 0) {
      throw new Error(
        `block ${String(blockIndex)}: rotation of ${rotation.franchiseId} is illegal`,
      );
    }
  }
}
...
function simulateBlockGames(
  ...
  const retainedDetails: NonNullable<
    Extract<SeasonBlockGameOutcome, { summary: SeasonGameSummary }>['retainedDetail']
  >[] = [];
  ...
  const shared = {
    ...
    rosterByFranchise: new Map(
      input.run.rosters.map((roster) => {
    ...
    staminaByVersion: new Map(
      [...input.expanded.values()].map((player) => {
    ...
    positions: new Map(
      [...input.expanded.values()].map((player) => [player.playerVersionId, player.positions]),
    ),
  };
  for (const game of seasonBlockGamesOf(input.schedule, blockIndex)) {
    ...
    if ('interruption' in outcome) {
      throw new Error(
        `game ${game.gameId} interrupted: the human franchise cannot field a legal five from health availability`,
      );
    }
...
function runFreeAgencyCommands(
  ...
  if (state.humanFranchiseId !== null) {
    ...
    if (skipped.result.result.status === 'rejected') {
      throw new Error(
        `seed ${deps.rootSeed} window ${String(windowIndex)} skip rejected (${skipped.result.result.rejection.code})`,
      );
    }
    state.run = skipped.run;
    state.effects =
      (skipped.run as SeasonRun & { effects?: SeasonEffectsState }).effects ?? state.effects;
  ...
  if (resolved.result.result.status === 'rejected') {
    throw new Error(
      `seed ${deps.rootSeed} window ${String(windowIndex)} resolve rejected (${resolved.result.result.rejection.code})`,
    );
  }
  state.run = resolved.run;
  state.effects =
    (resolved.run as SeasonRun & { effects?: SeasonEffectsState }).effects ?? state.effects;
...
export interface SeasonFreeAgencyMeasuredFacts {
  ...
  determinismProbe: { probed: boolean; identical: boolean };
  summaryIdentityProbe: { probed: boolean; identical: boolean };
}
...
export function seasonFreeAgencyFactsOf(
  ...
  const candidateShortfalls = windows.filter(
    (window) => window.candidates.length < SEASON_FREE_AGENCY_WINDOW_MAX_CANDIDATES,
  ).length;
  const uniqueIdentities = new Set(
    windows.flatMap((window) => window.candidates.map((candidate) => candidate.playerId)),
  ).size;
  ...
  const declarations = windows.reduce(
    (sum, window) => sum + Object.keys(window.declarations).length,
    0,
  );
  ...
  for (const window of windows) {
    for (const declaration of Object.values(window.declarations)) {
      ...
      for (const target of declaration.targets) {
        ...
        if (candidate !== undefined) interestByBand[candidate.band] += 1;
      }
    }
  }
  for (const signing of signings) winsByBand[signing.band] += 1;

  const signingsPerFranchise = run.league.teams.map(
    (team) => freeAgency.signingCounts[team.franchiseId] ?? 0,
  );
  ...
  const bandOf = new Map(
    run.aiAssignments.map((assignment) => [assignment.franchiseId, assignment.band]),
  );
  ...
  for (const signing of signings) {
    ...
    if (overall > 100) {
      outliersPerFranchise.set(
        signing.franchiseId,
        (outliersPerFranchise.get(signing.franchiseId) ?? 0) + 1,
      );
    }
  }
  let oneOutlierFailures = 0;
  for (const count of outliersPerFranchise.values()) {
    if (count > 1) oneOutlierFailures += 1;
  }
  ...
  for (const window of windows) {
    ...
    for (const candidatePlayerVersionId of candidates) {
      const claims = Object.values(window.declarations)
        .filter((declaration) =>
          declaration.targets.some((target) => target.playerVersionId === candidatePlayerVersionId),
        )
      ...
      if (
        (decisiveCriterion === 'influence' || decisiveCriterion === 'draw') &&
        winner !== undefined &&
  ...
  const contenders = run.league.teams.filter(
    (team) => (bandOf.get(team.franchiseId) ?? 'average') === 'contender',
  );
  const weaker = run.league.teams.filter(
    (team) => (bandOf.get(team.franchiseId) ?? 'average') === 'weaker',
  );
  const contenderSignings = contenders.reduce(
    (sum, team) => sum + (freeAgency.signingCounts[team.franchiseId] ?? 0),
    0,
  );
  const weakerSignings = weaker.reduce(
    (sum, team) => sum + (freeAgency.signingCounts[team.franchiseId] ?? 0),
    0,
  );
  ...
  return {
    ...
    linkFailures:
      season.audit.counts.ledgerFailures +
...
function effectsAccountingOf(season: SeasonFreeAgencySeasonFacts): {
  ...
  for (const rotation of run.rotations) {
    ...
    const rotationMembers = [...rotation.starters, ...rotation.benchOrder].map(
      (playerVersionId) => {
        if (!rosterIds.has(playerVersionId)) rotationIllegal += 1;
        return {
          playerVersionId,
...
export function evaluateFreeAgencyGates(
  ...
  options: { summaryIdentity: { probed: boolean; identical: boolean } },
): M25Gate[] {
  const c = calibration;
  const h = heldOut;
  ...
  const metrics: M25Gate[] = [
    ...
    m25ToleranceGate(
      'threeSigningsPerSeason',
      c.signingCapViolations,
      0,
      0,
      c.seasonsSimulated,
      min,
    ...
    m25ToleranceGate(
      'rosterLegality',
      c.rosterIllegal + c.rotationIllegal,
      0,
      0,
      c.seasonsSimulated,
      min,
    ...
    m25RangeGate(
      'ownershipRows',
      c.ownershipRows / Math.max(1, c.seasonsSimulated),
    ...
    m25ToleranceGate(
      'determinismProbe',
      c.determinismProbe.probed && c.determinismProbe.identical ? 1 : 0,
      1,
      0,
      c.seasonsSimulated,
      1,
    ...
    m25RangeGate(
      'candidateQuality',
      c.signedAboveDraftedMedianShare,
    ...
    summaryIdentityGate,
    m25ToleranceGate(
      'heldOut.windowsOpened',
      h.windowsOpened,
      SEASON_FREE_AGENCY_WINDOW_BLOCK_INDEXES.length * h.seasonsSimulated,
      0,
      h.seasonsSimulated,
      min,
    ),
    ...
    m25ToleranceGate(
      'heldOut.rosterLegality',
      h.rosterIllegal + h.rotationIllegal,
      0,
      0,
      h.seasonsSimulated,
      min,
    ...
    m25ToleranceGate(
      'heldOut.determinism',
      h.determinismProbe.probed && h.determinismProbe.identical ? 1 : 0,
      1,
      0,
      h.seasonsSimulated,
      1,
...
export function validateSeasonFreeAgencyTargets(
...
export function seasonFreeAgencyCalibrate(
  ...
  const draftedMedian =
    calibration.length > 0 && calibration[0] !== undefined
  ...
  let summaryIdentity: { probed: boolean; identical: boolean } = {
    probed: false,
    identical: true,
  };
  ...
  const gates = {
    ...
    heldOut:
      gateValue(metrics, 'heldOut.windowsOpened') &&
