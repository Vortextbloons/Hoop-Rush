...
import { createSeasonBlockRunner, rollForwardTo, runBlockThroughHandler, runnerBlockCommand, runnerPipelineInput, type SeasonBlockRunnerState, } from './season-block.ts';
...
function timed(state: SeasonBlockRunnerState, blockIndex: number): {
    digest: string;
    durationMs: number;
} {
...
export function seasonBenchmarkBlock(args: {
    ...
    const details = [
        ...
        ...perBlock.map((entry) => `block ${String(entry.blockIndex)} ${entry.digest} (${entry.durationMs.toFixed(0)}ms)`),
        `report ${outPath ?? 'not written (pass --out <path>)'}`,
    ];
    const failures: string[] = [];
    if (!normalWithinBudget) {
        failures.push(`normal block ${normal.durationMs.toFixed(0)}ms exceeds the ${String(SEASON_BUDGET_NORMAL_BLOCK_MS)}ms budget`);
    }
    if (!finalWithinBudget) {
        failures.push(`final block ${finalDurationMs.toFixed(0)}ms exceeds the ${String(SEASON_BUDGET_FINAL_BLOCK_MS)}ms budget`);
    }
    return makeReport('season benchmark block', { run: state.run.runId }, { details, failures, payload });
}
export function seasonBenchmarkFull(args: {
    ...
    return makeReport('season benchmark full', { run: state.run.runId }, { details, failures, payload });
}
export function seasonBenchmarkDeterminism(args: {
    ...
    const interrupted = (() => {
        ...
        try {
            ...
            if (error instanceof SeasonBlockCancelledError)
                cancelled = true;
            else
                throw error;
        }
    ...
    const identicalInterrupted = JSON.stringify(interrupted) === JSON.stringify(first.digests.slice(0, interrupted.length));
    ...
    const failures: string[] = [];
    if (!identicalDigests)
        failures.push('uninterrupted full-season digests diverged');
    if (!identicalInterrupted)
        failures.push('interrupted-resume digests diverged');
    return makeReport('season benchmark determinism', { run: first.runId }, { details, failures, payload });
}
export async function seasonBenchmarkPersistence(args: {
    ...
    let report: Awaited<ReturnType<typeof import('@hoop-rush/persistence').benchmarkSeasonRunPersistence>>;
    try {
        ...
        return makeReport('season benchmark persistence', { samples: args.samples }, {
    ...
    const failures: string[] = [];
    if (!commitOk)
        failures.push('persistence commit p95 exceeds the budget');
    if (!reloadOk)
        failures.push('persistence reload p95 exceeds the budget');
    if (!storageOk)
        failures.push('persistence storage exceeds the budget');
