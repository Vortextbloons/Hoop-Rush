import { z } from 'zod';
import { seasonDraftCommandSchema, seasonDraftStateSchema, seedSchema, type SeasonDraftState, type SeasonRosterTargets, } from '@hoop-rush/data-contracts';
import { applySeasonDraftCommand, generateAiLeague, seasonDraftStateDigest, } from '@hoop-rush/engine';
import { makeReport, type CliReport } from '../report.ts';
import { seasonDraftReproduceReportSchema } from '../report-schemas.ts';
import { DEFAULT_MANIFEST, loadSeasonDraftCatalog, loadSeasonRosterTargets, readJsonFile, } from './season-data.ts';
...
export function seasonDraftReproduce(args: {
    ...
    if (!parsedInput.success) {
        return makeReport('season draft reproduce', { input: inputPath }, {
            failures: [
    ...
    try {
        ...
        return makeReport('season draft reproduce', { input: inputPath }, { failures: [(error as Error).message], exitCode: 2 });
    }
    ...
    for (const command of input.commands) {
        const result = applySeasonDraftCommand(state, catalog, command, {
            generate: (generationInput) => generateAiLeague({
                ...generationInput,
                targets,
            }),
        });
    ...
    const identical = expectedDigest === null
        ? true
    ...
    if (input.expected !== undefined && state.revision !== input.expected.finalRevision) {
        divergences.push(`final revision ${String(state.revision)} does not match expected ${String(input.expected.finalRevision)}`);
    }
