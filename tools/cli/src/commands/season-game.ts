...
import { checkSeasonGameResult, createEngineContext, simulateSeasonGame, simulateSeasonGameWithEffects, } from '@hoop-rush/engine';
import { SEASON_GAME_TARGETS_VERSION, SEASON_GAME_VERSION, SEASON_ROTATION_PLANNER_VERSION, SEASON_ROTATION_VERSION, seasonGameSimulationInputSchema, seasonGameTargetsSchema, seedSchema, type SeasonGameSimulationInput, type SeasonGameSimulationResult, type SeasonGameTargets, type SeasonRotationPreset, } from '@hoop-rush/data-contracts';
...
import { seasonGameCalibrateReportSchema, seasonGameSimulateReportSchema, type SeasonGameSimulateReport, } from '../report-schemas.ts';
...
export const SEASON_GAME_CALIBRATION_SEED_TOTAL = SEASON_GAME_CALIBRATION_SEED_COUNT + SEASON_GAME_VALIDATION_SEED_COUNT;
...
export function loadSeasonGameFixture(ref: string): SeasonGameFixture {
    ...
    if (!parsed.success) {
        throw new UsageError(`fixture ${ref} fails validation: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
    }
    return parsed.data;
}
...
export async function simulateSeasonGameFacts(fixtureId: string, seedIndex: number, input: SeasonGameSimulationInput, deps: SeasonGameEngineDeps, effects = false): Promise<SeasonGameGameFacts> {
    ...
    for (const side of [input.home, input.away]) {
        ...
        const secondsOf = new Map(resultSide.players.map((player) => [player.playerVersionId, player.seconds]));
        for (const id of rotation.starters)
            facts.starterSeconds.push(secondsOf.get(id) ?? 0);
        rotation.benchOrder.forEach((id, role) => {
            ...
            if (roleSeconds !== undefined)
                roleSeconds.push(seconds);
        });
...
export type SeasonGameCohortRunner = (request: SeasonGameCohortRequest) => Promise<SeasonGameGameFacts[]>;
export async function runSeasonGameCohort(request: SeasonGameCohortRequest): Promise<SeasonGameGameFacts[]> {
    const promises: Array<Promise<SeasonGameGameFacts[]>> = [];
    for (const fixture of request.fixtures) {
        promises.push(runWorkerChunks<number, SeasonGameGameFacts>({
...
export async function runSeasonGameCohortInProcess(request: SeasonGameCohortRequest, deps: SeasonGameEngineDeps): Promise<SeasonGameGameFacts[]> {
    ...
    for (const fixture of request.fixtures) {
        ...
        for (let start = 0; start < request.seedIndices.length; start += chunkSize) {
            for (const index of request.seedIndices.slice(start, start + chunkSize)) {
                ...
                facts.push(await simulateSeasonGameFacts(fixture.fixtureId, index, input, deps, request.effects ?? false));
            }
        }
    }
    return facts;
}
export function seasonGameSimulate(args: {
    input: string | null;
    seed: string | null;
}, deps: Partial<SeasonGameEngineDeps> = {}): CliReport {
    ...
    const payload = seasonGameSimulateReportSchema.parse(buildSimulatePayload(fixture, input, result, invariantFailures));
    return makeReport('season game simulate', { fixture: fixture.fixtureId, seed }, {
        details: renderSimulateDetails(input, result, payload),
        failures: invariantFailures,
        payload,
    });
}
function buildSimulatePayload(fixture: SeasonGameFixture, input: SeasonGameSimulationInput, result: SeasonGameSimulationResult, invariantFailures: string[]): SeasonGameSimulateReport {
    ...
    return {
        ...
        forfeit: result.outcome === 'forfeit'
...
function playerMinutesOf(input: SeasonGameSimulationInput, result: SeasonGameSimulationResult): SeasonGameSimulateReport['playerMinutes'] {
    const reasonOf = new Map((result.outcome === 'completed' ? result.deviations : []).map((deviation) => [
    ...
    for (const side of ['home', 'away'] as const) {
        ...
        const target = new Map(rotation.targetMinutes.map((row) => [row.playerVersionId, row.minutes * 60]));
        const actual = result.outcome === 'completed'
            ? new Map((side === 'home' ? result.home : result.away).players.map((player) => [
...
function renderSimulateDetails(input: SeasonGameSimulationInput, result: SeasonGameSimulationResult, payload: SeasonGameSimulateReport): string[] {
    ...
    if (result.outcome === 'completed') {
        ...
        details.push(`${home.displayName} ${String(home.score)} - ${String(away.score)} ${away.displayName} (winner: ${result.winner}${overtime})`, versions);
        for (const side of [home, away]) {
            const box = side.box;
            details.push(`${side.displayName}: ${String(box.points)} pts · ${String(box.fieldGoals.made)}/${String(box.fieldGoals.attempted)} FG · ${String(box.threes.made)}/${String(box.threes.attempted)} 3P · ${String(box.freeThrows.made)}/${String(box.freeThrows.attempted)} FT · ${String(box.rebounds.offensive)}+${String(box.rebounds.defensive)}+${String(box.rebounds.team)} REB · ${String(box.assists)} AST · ${String(box.turnovers)} TOV · ${String(box.fouls)} PF · ${String(box.possessions)} POSS`);
        }
        const names = new Map([...input.home.players, ...input.away.players].map((player) => [
            player.playerVersionId,
            player.displayName,
        ]));
        for (const row of payload.playerMinutes) {
            const reason = row.reasons.length > 0 ? ` (${row.reasons.join(', ')})` : '';
            details.push(`  ${row.side} ${names.get(row.playerVersionId) ?? row.playerVersionId}: ${minutes(row.actualSeconds)}/${minutes(row.targetSeconds)} min actual/target${reason}`);
        }
        details.push(`substitutions: ${String(result.substitutions.length)}`);
        for (const sub of result.substitutions) {
            details.push(`  sub ${sub.side} p${String(sub.period)} ${String(sub.secondsRemaining)}s in ${sub.playerIn} out ${sub.playerOut} (${sub.reason})`);
        }
        details.push(`foul outs: ${String(result.foulOuts.length)}`);
        for (const foul of result.foulOuts) {
            details.push(`  ${foul.side} ${foul.playerVersionId} (p${String(foul.period)} ${String(foul.secondsRemaining)}s)`);
        }
        ...
        for (const deviation of result.deviations) {
            details.push(`  dev ${deviation.side} ${deviation.playerVersionId}: ${String(deviation.actualSeconds)}s vs ${String(deviation.targetSeconds)}s (${deviation.reasons.join(', ')})`);
        }
    }
    else if (result.outcome === 'forfeit') {
        details.push(`${input.home.displayName} 2 - 0 ${input.away.displayName} (forfeit: ${result.losingFranchiseId}, ${result.trigger})`, versions);
    }
    else {
        details.push('no legal five on either side (no-legal-five-both); no player statistics', versions);
    }
...
export interface SeasonGameFixtureStats {
    ...
    aggregateEnvelopes: Record<string, [
        number,
        number
    ]>;
    failures: {
...
function aggregateFixture(fixtureId: string, facts: readonly SeasonGameGameFacts[], metricKeys: readonly string[]): SeasonGameFixtureStats {
    ...
    for (const fact of facts) {
        if (fact.fixtureId !== fixtureId)
            continue;
        if (fact.checks.length > 0 || !fact.deterministic)
            failureGames += 1;
        ...
        if (metricKeys.includes('points'))
            metrics.points?.push(...fact.points);
        if (metricKeys.includes('possessions'))
            metrics.possessions?.push(...fact.possessions);
    }
    const aggregateEnvelopes: Record<string, [
        number,
        number
    ]> = {};
...
export async function seasonGameCalibrate(args: {
    fixture?: string | null;
    ...
    effects?: string | null;
}, deps: SeasonGameCalibrateDeps = {}): Promise<CliReport> {
    ...
    const { from: seedFrom, to: seedTo } = parseSeedRange(args, SEASON_GAME_CALIBRATION_SEED_TOTAL - 1, {
        requireOrder: true,
    });
    ...
    const presetCount = new Set(fixtures
        .filter((fixture): fixture is typeof fixture & {
        preset: SeasonRotationPreset;
    } => fixture.preset !== null)
        .map((fixture) => fixture.preset)).size;
    const hasAllPresetFixtures = presetCount === 3;
    const calibrationIndices = seedIndexRange(seedFrom, Math.min(seedTo, SEASON_GAME_CALIBRATION_SEED_COUNT - 1));
    const validationIndices = seedIndexRange(Math.max(seedFrom, SEASON_GAME_CALIBRATION_SEED_COUNT), seedTo);
    const simulate = deps.simulateSeasonGame ?? simulateSeasonGame;
    const check = deps.checkSeasonGameResult ?? checkSeasonGameResult;
    ...
    const runCohort = deps.runCohort ??
    ...
    const probeByKey = new Map(probeFacts.map((fact) => [factKey(fact.fixtureId, fact.seedIndex), fact]));
    const chunkingIndependent = [...probeByKey.values()].every((fact) => calibrationFacts.some((main) => factKey(main.fixtureId, main.seedIndex) === factKey(fact.fixtureId, fact.seedIndex) &&
    ...
    const starterOrdering = hasAllPresetFixtures &&
        tight !== undefined &&
        balanced !== undefined &&
    ...
    if (pass) {
        const targets: SeasonGameTargets = {
            ...
            fixtures: ['season-game-balanced', 'season-game-tight', 'season-game-bench-heavy'].map((fixtureId) => {
                const stat = statsByFixture.get(fixtureId);
                if (stat === undefined) {
                    throw new Error(`cannot freeze targets: preset fixture ${fixtureId} was not calibrated`);
                }
                return {
                    fixtureId,
    ...
    return makeReport('season game calibrate', { fixtures: fixtureIds, seedFrom, seedTo, workers }, { details, failures: gateFailures, payload });
}
