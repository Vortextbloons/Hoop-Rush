...
import { z } from 'zod';
import { SEASON_POSTSEASON_TARGETS_VERSION, SEASON_RUN_SCHEMA_VERSION, type SeasonAdvancePostseasonCommand, type SeasonPostseasonSummary, type SeasonRun, type SeasonStartPostseasonCommand, } from '@hoop-rush/data-contracts';
...
export function simulateSeasonPostseasonFacts(rootSeed: string, options: {
    runPath?: string | null;
    ...
    run = start.run;
    effects = (start.run as SeasonRun & {
    ...
    while (run.stage !== 'completed' && guard < 300) {
        ...
        run = output.run;
        effects = (output.run as SeasonRun & {
            effects?: typeof effects;
        }).effects ?? effects;
...
export function seasonPostseasonFactsOf(run: SeasonRun, season: SeasonM25SeasonFacts, postseasonSummaries: readonly SeasonPostseasonSummary[], integrityFailures: number): SeasonPostseasonSeasonFacts {
    ...
    if (bracket !== null) {
        ...
        for (const bracketSeries of allSeries) {
            ...
            const teams = new Set([bracketSeries.homeCourtFranchiseId, bracketSeries.challengerFranchiseId].filter((team): team is string => team !== null));
