...
import { DEFAULT_ERA_ID, eraSimulationProfileSchema, franchiseEraPoolSchema, hoopRushManifestSchema, opponentBracketSchema, playersIndexSchema, type EraSimulationProfile, type FranchiseEraPool, type HoopRushManifest, type OpponentBracket, type PlayersIndex, } from '@hoop-rush/data-contracts';
...
export function loadPackagedData(manifestPath: string = DEFAULT_MANIFEST): {
    ...
    if (!parsed.success) {
        throw new Error(`manifest ${manifestPath} fails validation: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
    }
...
export class PackagedData {
    ...
    constructor(manifest: HoopRushManifest, dir: string) {
        ...
        this.dir = dir;
        this.poolEntries = new Map(manifest.pools.map((entry) => [`${entry.franchiseId}/${entry.eraId}`, entry]));
    }
    private artifact(url: string): {
        path: string;
        bytes: Buffer;
    } {
    ...
    eraProfile(eraId: string = DEFAULT_ERA_ID): EraSimulationProfile {
        ...
        if (!entry)
            throw new Error(`no era simulation profile for ${eraId} in the manifest`);
        ...
        if (!parsed.success)
            throw new Error(`profile ${path} fails validation`);
    ...
    pool(franchiseId: string, eraId: string): FranchiseEraPool {
        ...
        if (!entry)
            throw new Error(`no pool for ${franchiseId}/${eraId} in the manifest`);
        const { path, bytes } = this.artifact(entry.url);
        verifyHash(path, bytes, entry.contentHash);
        const parsed = franchiseEraPoolSchema.safeParse(parseJson(path, bytes));
        if (!parsed.success)
            throw new Error(`pool ${path} fails validation`);
    ...
    playersIndex(): PlayersIndex {
        ...
        if (!entry)
            throw new Error('no players index packaged in the manifest');
        const { path, bytes } = this.artifact(entry.url);
        verifyHash(path, bytes, entry.contentHash);
        const parsed = playersIndexSchema.safeParse(parseJson(path, bytes));
        if (!parsed.success)
            throw new Error(`players index ${path} fails validation`);
    ...
    bracket(): OpponentBracket {
        ...
        if (!entry)
            throw new Error('no bracket packaged in the manifest');
        const { path, bytes } = this.artifact(entry.url);
        verifyHash(path, bytes, entry.contentHash);
        const parsed = opponentBracketSchema.safeParse(parseJson(path, bytes));
        if (!parsed.success)
            throw new Error(`bracket ${path} fails validation`);
        this.bracketCache = parsed.data;
        return parsed.data;
    }
}
export function loadProfileFile(path: string): EraSimulationProfile {
    const parsed = eraSimulationProfileSchema.safeParse(JSON.parse(readFileSync(path, 'utf8')) as unknown);
    if (!parsed.success) {
        throw new UsageError(`profile ${path} fails validation: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
    }
    return parsed.data;
}
export function loadBracketFile(path: string): OpponentBracket {
    ...
    if (!parsed.success) {
        throw new UsageError(`bracket ${path} fails validation: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
    }
    return parsed.data;
}
