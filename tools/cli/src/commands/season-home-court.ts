...
import { z } from 'zod';
import { SEASON_HOME_COURT_VERSION, SEASON_HOME_WIN_RATE_TARGET, SEASON_GAME_VERSION, seasonGameSimulationInputSchema, type SeasonGameSimulationInput, type SeasonGameSimulationResult, type SeasonHomeCourtProfile, } from '@hoop-rush/data-contracts';
import { SEASON_HOME_COURT_PROFILE, checkSeasonGameResult, createEngineContext, simulateSeasonGame, } from '@hoop-rush/engine';
...
import { loadSeasonGameFixture, resolveSeasonGameFixturePath, type SeasonGameEngineDeps, } from './season-game.ts';
...
export const SEASON_HOME_COURT_SEED_TOTAL = SEASON_HOME_COURT_CALIBRATION_SEED_COUNT + SEASON_HOME_COURT_VALIDATION_SEED_COUNT;
...
function homeWon(result: SeasonGameSimulationResult): boolean {
    ...
    if (result.outcome === 'forfeit')
        return result.homeScore === 2;
    return false;
}
export function simulateSeasonHomeCourtFacts(fixtureId: string, seedIndex: number, input: SeasonGameSimulationInput, profile: SeasonHomeCourtProfile, deps: SeasonGameEngineDeps): SeasonHomeCourtGameFacts {
...
export type SeasonHomeCourtCohortRunner = (request: SeasonHomeCourtCohortRequest) => Promise<SeasonHomeCourtGameFacts[]>;
export async function runSeasonHomeCourtCohort(request: SeasonHomeCourtCohortRequest): Promise<SeasonHomeCourtGameFacts[]> {
    const promises: Array<Promise<SeasonHomeCourtGameFacts[]>> = [];
    for (const fixture of request.fixtures) {
        promises.push(runWorkerChunks<number, SeasonHomeCourtGameFacts>({
...
export function runSeasonHomeCourtCohortInProcess(request: SeasonHomeCourtCohortRequest, deps: SeasonGameEngineDeps): Promise<SeasonHomeCourtGameFacts[]> {
    const facts: SeasonHomeCourtGameFacts[] = [];
    for (const fixture of request.fixtures) {
        ...
        for (const index of request.seedIndices) {
            ...
            facts.push(simulateSeasonHomeCourtFacts(fixture.fixtureId, index, input, request.profile, deps));
        }
    }
...
function foldWinRates(facts: readonly SeasonHomeCourtGameFacts[]): {
    ...
    for (const fact of facts) {
        games += 1;
        if (fact.neutralHomeWon)
            neutralWins += 1;
        if (fact.homeProfileHomeWon)
            homeWins += 1;
    }
    return {
...
function profileOfConstants(constants: string | null | undefined): SeasonHomeCourtProfile {
    if (constants === null || constants === undefined)
        return SEASON_HOME_COURT_PROFILE;
...
export function validateSeasonHomeCourtTargets(path: string): string[] {
    ...
    if (targets.constants.homeDefensiveCommunication !==
        SEASON_HOME_COURT_PROFILE.homeDefensiveCommunication) {
        failures.push(`homeDefensiveCommunication ${String(targets.constants.homeDefensiveCommunication)} does not match the engine profile ${String(SEASON_HOME_COURT_PROFILE.homeDefensiveCommunication)}`);
    }
    if (targets.constants.awayTurnoverPressure !== SEASON_HOME_COURT_PROFILE.awayTurnoverPressure) {
        failures.push(`awayTurnoverPressure ${String(targets.constants.awayTurnoverPressure)} does not match the engine profile ${String(SEASON_HOME_COURT_PROFILE.awayTurnoverPressure)}`);
    }
    return failures;
}
...
export async function seasonHomeCourtCalibrate(args: {
    fixture?: string | null;
...
}, deps: SeasonHomeCourtCalibrateDeps = {}): Promise<CliReport> {
    if (args.validate !== null && args.validate !== undefined) {
        const failures = validateSeasonHomeCourtTargets(args.validate);
        return makeReport('season home-court calibrate', { validate: args.validate }, { details: ['validated existing artifact'], failures });
    }
    ...
    const runCohort = deps.runCohort ??
        (deps.simulateSeasonGame !== undefined
            ? (request: SeasonHomeCourtCohortRequest) => runSeasonHomeCourtCohortInProcess(request, engineDeps)
            : runSeasonHomeCourtCohort);
    const calibrationIndices = seedIndexRange(seedFrom, Math.min(seedTo, SEASON_HOME_COURT_CALIBRATION_SEED_COUNT - 1));
    const validationIndices = seedIndexRange(Math.max(seedFrom, SEASON_HOME_COURT_CALIBRATION_SEED_COUNT), seedTo);
    ...
    const withinTolerance = Math.abs(achievedHomeWinRate - SEASON_HOME_WIN_RATE_TARGET) <= targetTolerance;
    ...
    const monotonic = await (async () => {
        if (args.constants !== null && args.constants !== undefined)
            return true;
    ...
    if (!neutralBaseline) {
        gateFailures.push(`neutral home win rate ${(neutralHomeWinRate * 100).toFixed(1)}% is outside the ${(baselineTolerance * 100).toFixed(1)}pp 3σ baseline gate`);
    }
    if (!withinTolerance) {
        gateFailures.push(`held-out home win rate ${(achievedHomeWinRate * 100).toFixed(1)}% is outside ${(targetTolerance * 100).toFixed(1)}pp of ${(SEASON_HOME_WIN_RATE_TARGET * 100).toFixed(1)}%`);
    }
    if (!possessionStable) {
    ...
    return makeReport('season home-court calibrate', { fixtures: fixtureIds, seedFrom, seedTo, workers }, { details, failures: gateFailures, payload });
}
