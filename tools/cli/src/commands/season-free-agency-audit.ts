import { type SeasonFreeAgencyBand, type SeasonFreeAgencyDeclaration, type SeasonFreeAgencySigning, type SeasonFreeAgencyWindowState, type SeasonRun, } from '@hoop-rush/data-contracts';
import { SEASON_FREE_AGENCY_BAND_SIGNING_CAPS, SEASON_FREE_AGENCY_WINDOW_BLOCK_INDEXES, SEASON_FREE_AGENCY_WINDOW_COMPOSITION, SEASON_FREE_AGENCY_WINDOW_MAX_CANDIDATES, } from '@hoop-rush/engine';
...
function declarationTargetOf(declarations: Record<string, SeasonFreeAgencyDeclaration>, franchiseId: string, playerVersionId: string): number | null {
    const target = declarations[franchiseId]?.targets.find((entry) => entry.playerVersionId === playerVersionId);
    return target?.influence ?? null;
}
function auditWindow(window: SeasonFreeAgencyWindowState, windowIndex: number, run: SeasonRun, leagueFranchiseIds: ReadonlySet<string>, failures: string[], counts: SeasonFreeAgencyAuditCounts): void {
    ...
    if (window.blockIndex !== expectedBlock) {
        counts.windowOrderFailures += 1;
        failures.push(`${where} opened by block ${String(window.blockIndex)}, expected block ${String(expectedBlock)}`);
    }
    ...
    for (const candidate of window.candidates) {
        ...
        if (canonical === undefined) {
            ...
            failures.push(`${where} candidate ${candidate.playerVersionId} has no canonical identity record`);
        }
        else {
            if (canonical.playerVersionId !== candidate.playerVersionId) {
                counts.canonicalFailures += 1;
                failures.push(`${where} candidate ${candidate.playerVersionId} is not the canonical version ${canonical.playerVersionId} of identity ${candidate.playerId}`);
            }
            if (canonical.band !== candidate.band) {
                counts.canonicalFailures += 1;
                failures.push(`${where} candidate ${candidate.playerVersionId} records band ${candidate.band}, canonical records ${canonical.band}`);
            }
            if (canonical.seedPath.join('/') !==
                `${String(canonical.admittedWindowIndex)}/canonical/${candidate.playerId}`) {
                counts.canonicalFailures += 1;
                failures.push(`${where} canonical ${candidate.playerId} seed path ${canonical.seedPath.join('/')} does not match its admission window ${String(canonical.admittedWindowIndex)}`);
            }
        }
    }
    if (window.candidates.length > SEASON_FREE_AGENCY_WINDOW_MAX_CANDIDATES) {
        counts.candidateUniquenessFailures += 1;
        failures.push(`${where} carries ${String(window.candidates.length)} candidates (max ${String(SEASON_FREE_AGENCY_WINDOW_MAX_CANDIDATES)})`);
    }
    for (const band of ['featured', 'role', 'development', 'emergency'] as const) {
        if (bandCounts[band] > SEASON_FREE_AGENCY_WINDOW_COMPOSITION[band]) {
            counts.candidateUniquenessFailures += 1;
            failures.push(`${where} carries ${String(bandCounts[band])} ${band} candidates (composition target ${String(SEASON_FREE_AGENCY_WINDOW_COMPOSITION[band])})`);
        }
    }
    if (featured > 1) {
    ...
    for (const earlier of run.freeAgency.windows) {
        if (earlier.windowIndex >= windowIndex)
            continue;
        for (const candidate of earlier.candidates) {
            ...
            if (current !== undefined && current.playerVersionId !== candidate.playerVersionId) {
                counts.canonicalFailures += 1;
                failures.push(`identity ${candidate.playerId} appears as sibling versions ${candidate.playerVersionId} (window ${String(earlier.windowIndex)}) and ${current.playerVersionId} (window ${String(windowIndex)})`);
            }
        }
    }
    if (window.status === 'resolved') {
        ...
        for (const declaration of Object.values(window.declarations)) {
            if (declaration.windowIndex !== window.windowIndex) {
                counts.declarationFailures += 1;
                failures.push(`${where} declaration ${declaration.franchiseId} names window ${String(declaration.windowIndex)}`);
            }
            for (const target of declaration.targets) {
                const candidate = window.candidates.find((entry) => entry.playerVersionId === target.playerVersionId);
                if (candidate === undefined) {
                    counts.declarationFailures += 1;
                    failures.push(`${where} declaration ${declaration.franchiseId} targets a non-candidate ${target.playerVersionId}`);
                }
                else if (!candidate.supportedRoles.includes(target.roleExpectation)) {
                    counts.declarationFailures += 1;
                    failures.push(`${where} declaration ${declaration.franchiseId} uses unsupported role ${target.roleExpectation} for ${target.playerVersionId}`);
                }
            }
        }
    }
    if (window.status === 'resolved') {
        if (window.traces.length !== 1) {
            ...
            failures.push(`${where} resolved with ${String(window.traces.length)} traces (expected exactly one)`);
        }
        for (const trace of window.traces) {
            if (trace.windowIndex !== window.windowIndex) {
                counts.traceFailures += 1;
                failures.push(`${where} trace names window ${String(trace.windowIndex)}, expected ${String(window.windowIndex)}`);
            }
            ...
            if (trace.resolution !== expectedResolution) {
                counts.traceFailures += 1;
                failures.push(`${where} trace resolution ${trace.resolution}, signings reconcile ${expectedResolution}`);
            }
            if (trace.signedPlayerVersionId !== (firstSigning?.playerVersionId ?? null)) {
                counts.traceFailures += 1;
                failures.push(`${where} trace signed player ${String(trace.signedPlayerVersionId)} does not match the signing ${String(firstSigning?.playerVersionId ?? null)}`);
            }
            if (trace.signingFranchiseId !== (firstSigning?.franchiseId ?? null)) {
                counts.traceFailures += 1;
                failures.push(`${where} trace signing franchise ${String(trace.signingFranchiseId)} does not match the signing ${String(firstSigning?.franchiseId ?? null)}`);
            }
            const winnerKeys = new Set([...trace.firstPriorityWinners, ...trace.secondPriorityWinners].map((winner) => `${winner.candidatePlayerVersionId}\u0000${winner.winnerFranchiseId}`));
            for (const signing of window.signings) {
                if (!winnerKeys.has(`${signing.playerVersionId}\u0000${signing.franchiseId}`)) {
                    counts.traceFailures += 1;
                    failures.push(`${where} signing ${signing.signingId} is not recorded among the trace winners`);
                }
            }
            for (const step of trace.steps) {
                ...
                if (step.criterion === 'influence' && windowIndex === 0) {
                    const committed = declarationTargetOf(window.declarations, step.franchiseId, step.candidatePlayerVersionId);
                    if (committed !== null && step.category !== String(committed)) {
                        counts.traceFailures += 1;
                        failures.push(`${where} trace influence step records ${step.category} for ${step.franchiseId}, declaration commits ${String(committed)}`);
                    }
                }
            }
        }
...
function auditSigningLinks(signing: SeasonFreeAgencySigning, run: SeasonRun, failures: string[], counts: SeasonFreeAgencyAuditCounts): void {
    ...
    if (ledger === undefined) {
        ...
        if (ledger.franchiseId !== signing.franchiseId) {
            counts.ledgerFailures += 1;
            failures.push(`${where} ledger entry ${signing.ledgerEntryId} belongs to ${ledger.franchiseId}, not ${signing.franchiseId}`);
        }
        if (ledger.source !== 'free-agent-signing') {
            counts.ledgerFailures += 1;
            failures.push(`${where} ledger entry ${signing.ledgerEntryId} records source ${ledger.source}, expected free-agent-signing`);
        }
        if (ledger.appliedDelta !== -signing.influenceCost) {
            counts.ledgerFailures += 1;
            failures.push(`${where} ledger entry ${signing.ledgerEntryId} applies ${String(ledger.appliedDelta)}, expected ${String(-signing.influenceCost)}`);
        }
        if (ledger.commandId !== signing.commandId) {
            counts.ledgerFailures += 1;
            failures.push(`${where} ledger entry ${signing.ledgerEntryId} records command ${String(ledger.commandId)}, not ${signing.commandId}`);
        }
    }
    const transaction = run.transactions.find((entry) => entry.transactionId === signing.transactionId);
    if (transaction === undefined) {
        ...
        if (transaction.type !== 'free-agent-signing') {
            counts.transactionFailures += 1;
            failures.push(`${where} transaction ${signing.transactionId} records type ${transaction.type}, expected free-agent-signing`);
        }
        ...
        if (payload?.playerVersionId !== signing.playerVersionId) {
            counts.transactionFailures += 1;
            failures.push(`${where} transaction ${signing.transactionId} payload names ${String(payload?.playerVersionId)}, expected ${signing.playerVersionId}`);
        }
        if (payload?.windowIndex !== signing.windowIndex) {
            counts.transactionFailures += 1;
            failures.push(`${where} transaction ${signing.transactionId} payload names window ${String(payload?.windowIndex)}, expected ${String(signing.windowIndex)}`);
        }
        if (payload?.ledgerEntryId !== signing.ledgerEntryId) {
            counts.transactionFailures += 1;
            failures.push(`${where} transaction ${signing.transactionId} payload ledger link ${String(payload?.ledgerEntryId)} does not match ${signing.ledgerEntryId}`);
        }
        if (payload?.signingId !== signing.signingId) {
            counts.transactionFailures += 1;
            failures.push(`${where} transaction ${signing.transactionId} payload signing link ${String(payload?.signingId)} does not match ${signing.signingId}`);
        }
    }
    ...
    if (owner === undefined || owner.ownerFranchiseId !== signing.franchiseId) {
        ...
        failures.push(`${where} does not reconcile with ownership (${signing.playerVersionId} -> ${String(owner?.ownerFranchiseId)})`);
    }
...
export function auditSeasonFreeAgencyFacts(run: SeasonRun): {
    ...
    for (const team of run.league.teams) {
        ...
        const band = run.aiAssignments.find((entry) => entry.franchiseId === franchiseId)?.band ?? 'average';
        const bandCap = SEASON_FREE_AGENCY_BAND_SIGNING_CAPS[band] ?? 3;
        if (signings.length > bandCap) {
            counts.bandCapFailures += 1;
            failures.push(`${franchiseId} (${band}) signed ${String(signings.length)} players, over the ${String(bandCap)} band cap`);
        }
        if (signings.length > 3) {
            counts.signingCapFailures += 1;
            failures.push(`${franchiseId} signed ${String(signings.length)} players, over the three-per-season cap`);
        }
        const recordedSignings = run.freeAgency.signingCounts[franchiseId];
        if (recordedSignings !== signings.length) {
            counts.signingCapFailures += 1;
            failures.push(`${franchiseId} signingCounts records ${String(recordedSignings)}, signings reconcile ${String(signings.length)}`);
        }
        ...
        if (recordedSpend !== spent) {
            ...
            failures.push(`${franchiseId} seasonSpend records ${String(recordedSpend)}, signings reconcile ${String(spent)}`);
        }
        if (spent > 6) {
            counts.spendCapFailures += 1;
            failures.push(`${franchiseId} spent ${String(spent)} Influence on free agency, over the six-point season cap`);
        }
    }
    const versionIds = new Set<string>();
    let activeLoads = 0;
    ...
    for (const roster of run.rosters) {
        if (roster.players.length < 10 || roster.players.length > 15) {
            counts.effectsFailures += 1;
            failures.push(`roster of ${roster.franchiseId} has ${String(roster.players.length)} players (10-15 required)`);
        }
    ...
    if (activeLoads !== 300 || activePairs !== 1350) {
        counts.effectsFailures += 1;
        failures.push(`active effects reconcile ${String(activeLoads)} loads / ${String(activePairs)} pairs, expected 300 / 1,350`);
    }
...
export function seasonFreeAgencyAudit(args: {
    ...
    try {
        ...
        return makeReport('season free-agency audit', { input: inputPath }, { failures: [(error as Error).message], exitCode: 2 });
    }
    ...
    return makeReport('season free-agency audit', { input: inputPath, manifest: args.manifest ?? undefined }, { details, failures, payload });
}
