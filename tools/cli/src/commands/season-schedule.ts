...
import { SEASON_COMMITTED_SCHEDULE_SEED, seasonLeagueSchema, seasonScheduleSchema, seedSchema, type SeasonLeague, type SeasonSchedule, } from '@hoop-rush/data-contracts';
import { auditSeasonSchedule, generateSeasonSchedule } from '@hoop-rush/engine';
import { EXIT_CHECKS_FAILED, EXIT_USAGE_OR_DATA_ERROR, makeReport, type CliReport, } from '../report.ts';
...
export const DEFAULT_SEASON_SCHEDULE = resolve(REPO_ROOT, 'apps/web/static/data/season/schedule.json');
...
function loadLeague(leaguePath: string): SeasonLeague {
    ...
    if (!parsed.success) {
        throw new Error(`league artifact fails the schema: ${parsed.error.issues[0]?.message ?? 'invalid'}`);
    }
    return parsed.data;
}
export function seasonScheduleGenerate(args: {
    ...
    if (!parsedSeed.success) {
        return makeReport('season schedule generate', { out: args.out, league: leaguePath, seed: rawSeed }, {
    ...
    try {
        ...
        return makeReport('season schedule generate', { out: args.out, league: leaguePath, seed }, { failures: [(error as Error).message], exitCode: EXIT_USAGE_OR_DATA_ERROR });
    }
    ...
    try {
        ...
        return makeReport('season schedule generate', { out: args.out, league: leaguePath, seed }, {
    ...
    if (auditFailures.length > 0) {
        return makeReport('season schedule generate', { out: args.out, league: leaguePath, seed }, { failures: auditFailures.map((f) => `audit: ${f}`), exitCode: EXIT_CHECKS_FAILED });
    }
    ...
    details.push(`schedule ${schedule.scheduleVersion} · formula ${schedule.formulaVersion} · league ${schedule.leagueVersion}`, `rounds ${String(schedule.rounds)} · games ${String(schedule.games.length)} · seed ${seed}`, `sha256 ${contentHash}`);
    return makeReport('season schedule generate', { out: args.out, league: leaguePath, seed }, { details, failures, payload });
}
export function seasonScheduleAudit(args: {
    ...
    try {
        ...
        if (!parsedSchedule.success) {
            throw new Error(`schedule artifact fails the schema: ${parsedSchedule.error.issues[0]?.message ?? 'invalid'}`);
        }
    ...
    details.push(`schedule ${schedule.scheduleVersion} · formula ${schedule.formulaVersion} · league ${schedule.leagueVersion}`, `rounds ${String(schedule.rounds)} · games ${String(schedule.games.length)} · seed ${schedule.generationSeed}`, `regeneration ${regenerationIdentical ? 'identical' : 'DIFFERS'} · manifest ${manifestVerified === null ? 'n/a' : manifestVerified ? 'verified' : 'mismatch'}`);
