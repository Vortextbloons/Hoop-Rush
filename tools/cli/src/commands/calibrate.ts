...
import { checkGameResult, createChallenge, createEngineContext, evaluateLineupStrength, simulateChallenge, toSimulationPlayer, } from '@hoop-rush/engine';
import type { SimulationPlayer } from '@hoop-rush/data-contracts';
import { DEFAULT_ERA_ID, POSITION_SLOTS, franchiseIdSchema, idSchema, playerIdSchema, type EraSimulationProfile, type FranchiseEraPool, type GameResult, type GameSimulationInput, type SimulationTeam, playableSlotGroups, slotGroupOf, } from '@hoop-rush/data-contracts';
import { makeReport, type CliReport } from '../report.ts';
import { calibrationMetricSchema, calibrateRunReportSchema, calibrateSensitivityReportSchema, } from '../report-schemas.ts';
...
export function leagueAverageTeam(pool: FranchiseEraPool): SimulationTeam {
    ...
    if (!firstPlayer)
        throw new UsageError('pool is empty');
    ...
    if (allReconstructed) {
        ...
        reconstructedPlayers.forEach((player, i) => {
            ...
            const usageAt = usage[i];
            if (profile === undefined || usageAt === undefined)
                return;
            const weight = usageAt / totalUsage;
    ...
    const averagedAnchors = averageProfile !== undefined
        ? ({
            ...Object.fromEntries(Object.entries(meanAnchors).map(([key, value]) => [key, Number(value.toFixed(4))])),
            threePointPct: null,
...
export function poolStrengthLineups(pool: FranchiseEraPool): {
    ...
    const pick = (players: typeof pool.players): SimulationTeam => {
        ...
        for (const slot of SLOTS) {
            ...
            if (!requirement)
                throw new UsageError('pool cannot form a legal lineup');
            const index = remaining.findIndex((p) => playableSlotGroups(p.positions.playable).includes(slotGroupOf(requirement)));
            if (index < 0)
                throw new UsageError('pool cannot form a legal lineup');
    ...
    const byScoreDesc = [...pool.players].sort((a, b) => b.selectionScore - a.selectionScore || a.playerId.localeCompare(b.playerId));
...
function accumulate(acc: MetricAccumulator, input: GameSimulationInput, trackHomeWin: boolean): number {
    ...
    if (result.overtimePeriods > 0)
        acc.overtime += 1;
    if (trackHomeWin && result.winner === 'home')
        acc.homeWins += 1;
...
export function calibrateRun(args: {
    ...
    const openingOpponent = bracket.opponents.find((o) => o.opponentId === `lakers-${profile.eraId}-opening`);
    ...
    if (!pass) {
        for (const m of allMetrics) {
            if (m.status === 'fail') {
                failures.push(`${m.key}: observed ${m.observed.toFixed(4)} outside ${(m.target - m.tolerance).toFixed(4)}..${(m.target + m.tolerance).toFixed(4)}`);
            }
            else if (m.status === 'skippedInsufficientSample' && !allowSkipped) {
                failures.push(`${m.key}: skipped (sample ${String(m.sample)} below minimum ${String(m.minimumSample)})`);
            }
        }
    }
    if (invariantFailures > 0)
        failures.push(`${String(invariantFailures)} invariant failures`);
    const details = [
        `profile ${profile.profileVersion} · era ${profile.eraId} · ${String(samples)} samples · engine ${payload.engineVersion}`,
        ...metrics.map((m) => `${m.status === 'pass' ? 'pass' : m.status === 'fail' ? 'FAIL' : 'SKIPPED'} ${m.key}: ${m.observed.toFixed(4)} (target ${m.target.toFixed(4)} ± ${m.tolerance.toFixed(4)}, sample ${String(m.sample)}/${String(m.minimumSample)})`),
    ...
    return makeReport('calibrate run', { profile: profile.profileVersion, samples }, { details, failures, payload });
}
function buildMetrics(equal: MetricAccumulator, strongWeak: MetricAccumulator, n: number, profile: EraSimulationProfile): Array<z.infer<typeof calibrationMetricSchema>> {
    ...
    return [
        ...
        metric('zoneMix.cornerThree', (equal.zoneAttempts.cornerThree ?? 0) / fga, t.zoneMix.cornerThree, n),
        metric('zoneMix.aboveBreakThree', (equal.zoneAttempts.aboveBreakThree ?? 0) / fga, t.zoneMix.aboveBreakThree, n),
...
function metric(key: string, observed: number, target: {
    value: number;
    tolerance: number;
    minimumSample: number;
}, sample: number): z.infer<typeof calibrationMetricSchema> {
    ...
    const inRange = observed >= target.value - target.tolerance && observed <= target.value + target.tolerance;
    return {
        key,
        target: target.value,
        tolerance: target.tolerance,
...
function buildRoleMetrics(samples: number, profile: EraSimulationProfile): Array<z.infer<typeof calibrationMetricSchema>> {
    ...
    for (let i = 0; i < roleSamples; i += 1) {
        ...
        for (const side of [result.home, result.away] as const) {
            ...
            const misses = (team: GameResult['home']) => team.box.fieldGoals.attempted -
...
export function calibrateSensitivity(args: {
    ...
    const failures = payload.pass
        ...
        : metrics
            .filter((m) => !m.pass)
            .map((m) => `${m.family}: expected ${m.direction}, got base ${m.baseValue.toFixed(2)} -> changed ${m.changedValue.toFixed(2)}`);
    const details = [
        `engine ${payload.engineVersion} · ${String(samples)} samples per family`,
        ...metrics.map((m) => `${m.pass ? 'pass' : 'FAIL'} ${m.family}: ${m.baseValue.toFixed(2)} -> ${m.changedValue.toFixed(2)} (${(m.relativeShift * 100).toFixed(1)}%, expected ${m.direction})`),
    ];
    return makeReport('calibrate sensitivity', { samples }, { details, failures, payload });
}
function sampleMetric(fixtureId: string, profile: EraSimulationProfile, fixture: ReturnType<typeof loadFixture>, variant: boolean, samples: number): number {
    let total = 0;
...
function metricValue(fixtureId: string, home: GameResult['home'], away: GameResult['away']): number {
    switch (fixtureId) {
        case 'sens-creation': {
