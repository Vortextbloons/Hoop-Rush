...
import { z } from 'zod';
import { SEASON_AI_VERSION, SEASON_DRAFT_SAFE_MINIMUM, SEASON_DRAFT_VERSION, SEASON_OFFER_TARGETS_VERSION, SEASON_ROSTER_GENERATION_VERSION, SEASON_ROTATION_VERSION, type SeasonDraftCatalog, type SeasonDraftCommand, type SeasonDraftState, type SeasonLeague, type SeasonLeagueGenerationResult, type SeasonRosterTargets, } from '@hoop-rush/data-contracts';
...
export function playSeasonDraftCalibrationSeed(seed: string, catalog: SeasonDraftCatalog, league: SeasonLeague, targets: SeasonRosterTargets): SeasonDraftCalibrationRun {
    const empty = {
        variety: 0,
        minSafePerOffer: 0,
...
export function runSeasonDraftCalibrationSeeds(args: {
    ...
    return args.seeds.map((seed) => playSeasonDraftCalibrationSeed(seed, args.catalog, args.league, args.targets));
}
