...
import { z } from 'zod';
import { SEASON_EFFECT_TARGETS_LEGACY_VERSION, SEASON_EFFECT_TARGETS_VERSION, SEASON_GAME_TARGETS_VERSION, SEASON_GAME_VERSION, playerVersionId, seasonGameSimulationInputSchema, type SeasonEffectsState, type SeasonGameSimulationInput, type SeasonGameSimulationResult, type SeasonMechanism, type SeasonStaminaInput, } from '@hoop-rush/data-contracts';
import { SEASON_EFFECTS_ASSIST_CONVERSION_MAX_PP, SEASON_EFFECTS_DEFENSE_FATIGUE_MAX_PP, SEASON_EFFECTS_HANDLER_FATIGUE_MAX_PP, SEASON_EFFECTS_HELP_DEFENSE_MAX_PP, SEASON_EFFECTS_SHOOTER_FATIGUE_MAX_PP, SEASON_EFFECTS_TURNOVER_SECURITY_MAX_PP, applySeasonRecoveryTick, checkSeasonGameResult, createEngineContext, createSeasonEffectsState, simulateSeasonGame, simulateSeasonGameWithEffects, unitChemistryBasisPoints, } from '@hoop-rush/engine';
import { parseCount, UsageError } from '../args.ts';
import { makeReport, type CliReport } from '../report.ts';
import { seasonEffectsCalibrateReportSchema, seasonEffectsDistributionReportSchema, seasonEffectsRolesReportSchema, seasonEffectsSensitivityReportSchema, } from '../report-schemas.ts';
import { seasonGameFixtureSchema } from '../fixture-schema.ts';
import { seasonGameCalibrationSeed, loadSeasonGameFixture, resolveSeasonGameFixturePath, type SeasonGameEngineDeps, } from './season-game.ts';
...
export function withFixtureStamina(input: SeasonGameSimulationInput): SeasonGameSimulationInput {
    const attach = (players: SeasonGameSimulationInput['home']['players']): SeasonGameSimulationInput['home']['players'] => players.map((player, index) => ({
        ...player,
        stamina: {
            schemaVersion: 1,
...
function fillerStamina(roster: number, slot: number): SeasonStaminaInput {
    return {
        schemaVersion: 1,
        playerVersionId: playerVersionId(`p-effects-filler-${String(roster)}-${String(slot)}`, `fx-${String(roster)}`, '1990s', '1995-96'),
        rating: 60 + ((roster + slot) % 20),
        historicalMpg: 12 + ((roster + slot) % 20),
        derivationVersion: 'season-stamina-v1',
    };
}
function staminaInputOf(player: SeasonGameSimulationInput['home']['players'][number], index: number): SeasonStaminaInput {
    return {
        schemaVersion: 1,
        playerVersionId: player.playerVersionId,
...
export function representativeEffectsState(input: SeasonGameSimulationInput): {
    ...
    const gameVersions = new Set([...input.home.players, ...input.away.players].map((player) => player.playerVersionId));
    const state: SeasonEffectsState = {
        schemaVersion: 2,
        inactivePlayerStates: base.inactivePlayerStates,
        playerStates: base.playerStates.map((player) => {
            if (!gameVersions.has(player.playerVersionId))
                return player;
            const rosterIndex = input.home.players.findIndex((p) => p.playerVersionId === player.playerVersionId);
...
function teamStats(result: SeasonGameSimulationResult): {
    ...
    if (result.outcome !== 'completed')
        return { score: 0, turnovers: 0, assists: 0, fga: 0 };
    return {
...
export function simulateSeasonEffectsGameFacts(fixtureId: string, seedIndex: number, input: SeasonGameSimulationInput, deps: SeasonEffectsEngineDeps): SeasonEffectsGameFacts {
...
export type SeasonEffectsCohortRunner = (request: SeasonEffectsCohortRequest) => Promise<SeasonEffectsGameFacts[]>;
export async function runSeasonEffectsCohort(request: SeasonEffectsCohortRequest): Promise<SeasonEffectsGameFacts[]> {
    const promises: Array<Promise<SeasonEffectsGameFacts[]>> = [];
    for (const fixture of request.fixtures) {
        promises.push(runWorkerChunks<number, SeasonEffectsGameFacts>({
...
export function runSeasonEffectsCohortInProcess(request: SeasonEffectsCohortRequest, deps: SeasonEffectsEngineDeps): Promise<SeasonEffectsGameFacts[]> {
...
export function runMiniSeason(input: SeasonGameSimulationInput, deps: SeasonEffectsEngineDeps, unitOverride?: {
    homeUnit: string[];
    awayUnit: string[];
    rollingUnit?: boolean;
}): SeasonEffectsState {
    ...
    for (let round = 1; round <= 10; round += 1) {
        ...
        if (unitOverride !== undefined) {
            ...
            const homeUnit = unitOverride.rollingUnit === true
                ? rolling(homeVersions, round)
                : [...unitOverride.homeUnit];
            const awayUnit = unitOverride.rollingUnit === true
        ...
        const { transition } = deps.simulateSeasonGameWithEffects(gameInput, createEngineContext(), current);
        current = {
            schemaVersion: 2,
            ...
            pairStates: current.pairStates.map((pair) => {
                const increment = transition.pairIncrements.find((inc) => inc.a === pair.a && inc.b === pair.b);
...
export function seasonEffectsSensitivity(args: SeasonEffectsArgs, deps: SeasonEffectsEngineDeps = defaultEffectsEngineDeps): CliReport {
    ...
    for (const fatigueBp of fatigueLevels) {
        const leveled: SeasonEffectsState = {
            ...
            playerStates: state.playerStates.map((player) => ({
                ...player,
                fatigueBasisPoints: player.fatigueBasisPoints > 0 || fatigueBp === 0 ? fatigueBp : player.fatigueBasisPoints,
            })),
            pairStates: state.pairStates,
            archivedPairs: state.archivedPairs,
        };
        const { transition } = deps.simulateSeasonGameWithEffects(input, createEngineContext(), leveled);
        const row = {
            fatigueBp,
            shooterDelta: 0,
            handlerDelta: 0,
    ...
    const details = rows.map((row) => `fatigue ${String(row.fatigueBp).padStart(5)}bp · shooter ${String(row.shooterDelta)} · handler ${String(row.handlerDelta)} · defense ${String(row.defenseDelta)} · security ${String(row.securityDelta)} · assist ${String(row.assistDelta)} · help ${String(row.helpDelta)} (millionths)`);
    return makeReport('season effects sensitivity', { fixture: fixtureId }, { details, payload });
}
export async function seasonEffectsDistribution(args: SeasonEffectsArgs, runner: SeasonEffectsCohortRunner = runSeasonEffectsCohort): Promise<CliReport> {
    const started = Date.now();
    const from = parseCount(args['seed-from'] ?? undefined, '--seed-from', 0);
    const to = parseCount(args['seed-to'] ?? undefined, '--seed-to', SEASON_EFFECTS_CALIBRATION_SEED_COUNT - 1);
    ...
    const scoringDeltas = completed.map((fact) => pct(fact.effectsScore - fact.neutralScore, fact.neutralScore));
    const turnoverDeltas = completed.map((fact) => pct(fact.effectsTurnovers - fact.neutralTurnovers, fact.neutralTurnovers));
    const assistDeltas = completed.map((fact) => pct(fact.effectsAssists - fact.neutralAssists, fact.neutralAssists));
    ...
    const envelope = (deltas: number[], max: number): number => deltas.filter((delta) => Math.abs(delta) <= max * 100).length / Math.max(1, deltas.length);
    ...
    return makeReport('season effects distribution', { fixtureIds, seedFrom: from, seedTo: to }, { details, failures: failuresList, payload });
}
export function seasonEffectsRoles(args: SeasonEffectsArgs, deps: SeasonEffectsEngineDeps = defaultEffectsEngineDeps): CliReport {
    const started = Date.now();
    ...
    const starterOrdering = tight !== undefined &&
        balanced !== undefined &&
    ...
    const benchOrdering = tight !== undefined &&
        balanced !== undefined &&
        benchHeavy !== undefined &&
    ...
    const details = rows.map((row) => `${row.fixtureId}: starter median ${String(row.starterMedianFatigue)}bp · bench median ${String(row.benchMedianFatigue)}bp`);
    const failuresList: string[] = [];
    if (!starterOrdering)
        failuresList.push('starter median fatigue ordering (tight > balanced > bench-heavy) failed');
    if (!benchOrdering)
        failuresList.push('bench median fatigue ordering (inverse) failed');
    return makeReport('season effects roles', { fixtureIds }, { details, failures: failuresList, payload });
}
export function seasonEffectsChemistrySeparation(deps: SeasonEffectsEngineDeps): {
    ...
    const shuffledMedian = unitChemistryBasisPoints(shuffledState.pairStates, homeVersions.slice(0, 5));
    return {
...
export async function seasonEffectsCalibrate(args: SeasonEffectsArgs, deps: SeasonEffectsEngineDeps = defaultEffectsEngineDeps, runner: SeasonEffectsCohortRunner = runSeasonEffectsCohort): Promise<CliReport> {
    const started = Date.now();
    const workers = parseCount(args['workers'] ?? undefined, '--workers', 4);
    ...
    const validationIndices = seedIndexRange(SEASON_EFFECTS_CALIBRATION_SEED_COUNT, SEASON_EFFECTS_CALIBRATION_SEED_COUNT + SEASON_EFFECTS_VALIDATION_SEED_COUNT - 1);
    ...
    const fold = (facts: SeasonEffectsGameFacts[]): {
        ...
        const completed = facts.filter((fact) => fact.completed);
        const scoring = completed.map((fact) => pct(fact.effectsScore - fact.neutralScore, fact.neutralScore));
        const turnovers = completed.map((fact) => pct(fact.effectsTurnovers - fact.neutralTurnovers, fact.neutralTurnovers));
        const assists = completed.map((fact) => pct(fact.effectsAssists - fact.neutralAssists, fact.neutralAssists));
        return {
    ...
    const heldOutCoverage = (deltas: number[], envelope: number[]): number => coverageWithin(deltas, percentile(envelope, 1), percentile(envelope, 99));
    const heldOutWithinEnvelopeShare = Math.min(heldOutCoverage(heldOut.scoringDeltas, calibration.scoringDeltas), heldOutCoverage(heldOut.turnoverDeltas, calibration.turnoverDeltas), heldOutCoverage(heldOut.assistDeltas, calibration.assistDeltas));
    ...
    for (let i = 1; i < rows.length && sensitivityMonotonic; i += 1) {
        ...
        if (prev === undefined || cur === undefined)
            continue;
        if (Math.abs(cur.shooterDelta) < Math.abs(prev.shooterDelta))
            sensitivityMonotonic = false;
        if (Math.abs(cur.handlerDelta) < Math.abs(prev.handlerDelta))
            sensitivityMonotonic = false;
        if (Math.abs(cur.defenseDelta) < Math.abs(prev.defenseDelta))
            sensitivityMonotonic = false;
    }
    ...
    const productionEnvelopes = Math.abs(calibration.scoringMedian) <= SEASON_EFFECTS_SCORING_ENVELOPE * 100 &&
    ...
    if (!zeroFailures)
        gateFailures.push('accounting/invariant failures in the cohort');
    if (!determinism)
        gateFailures.push('determinism failures in the cohort');
    if (!productionEnvelopes)
        gateFailures.push('production envelope gates failed on the calibration cohort');
    if (!heldOutPassShare)
        gateFailures.push(`held-out envelope share below ${String(SEASON_EFFECTS_HELD_OUT_PASS_SHARE * 100)}%`);
    if (!rotationOrdering)
        gateFailures.push('rotation workload ordering gates failed');
    if (!chemistrySeparationPass)
        gateFailures.push('chemistry separation gate failed');
    if (!sensitivityMonotonic)
        gateFailures.push('sensitivity monotonicity gate failed');
    if (pass && !targetsWritten)
        gateFailures.push('targets artifact was not written');
    return makeReport('season effects calibrate', { fixtureIds, workers }, { details, failures: gateFailures, payload });
}
export function validateSeasonEffectTargets(args: SeasonEffectsArgs, outPath: string): CliReport {
    ...
    return validateTargetsArtifact({
        ...
        extraChecks: (parsed) => {
            ...
            const capsMatch = Object.entries(expectedCaps).every(([key, value]) => parsed.mechanismCaps[key as keyof typeof parsed.mechanismCaps] === value);
            return capsMatch
...
export async function seasonEffectsCommand(args: SeasonEffectsArgs, extra: string[], deps: SeasonEffectsEngineDeps = {
    simulateSeasonGame,
    checkSeasonGameResult,
    simulateSeasonGameWithEffects,
    createSeasonEffectsState,
}, runner: SeasonEffectsCohortRunner = runSeasonEffectsCohort): Promise<CliReport> {
    ...
    switch (subcommand) {
        ...
        default:
            throw new UsageError('season effects requires a subcommand: sensitivity | distribution | roles | calibrate');
    }
}
