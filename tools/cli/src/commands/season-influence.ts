...
import { z } from 'zod';
import { SEASON_GAME_TARGETS_VERSION, SEASON_GAME_VERSION, SEASON_INFLUENCE_CAP, SEASON_INFLUENCE_FLOOR, SEASON_INFLUENCE_TARGETS_VERSION, } from '@hoop-rush/data-contracts';
...
import { gateValue, gateSummary, m25RangeGate, m25ToleranceGate, seasonCalibrationSeed, seedIndexRange, share, type M25Gate, } from './season-calibration.ts';
...
export function incomeIdentityFailuresOf(season: SeasonM25SeasonFacts): number {
    ...
    for (const team of run.league.teams) {
        ...
        for (const entry of run.influence.ledger) {
            if (entry.franchiseId !== franchiseId)
                continue;
            if (entry.source === 'initial-grant')
                continue;
        ...
        if (blockGrantEntries !== acceptedBlocks)
            failures += 1;
...
export function reconciliationFailuresOf(season: SeasonM25SeasonFacts): number {
    ...
    for (const entry of run.influence.ledger) {
        sumByFranchise.set(entry.franchiseId, (sumByFranchise.get(entry.franchiseId) ?? 0) + entry.appliedDelta);
    }
    let failures = 0;
    for (const team of run.league.teams) {
        const franchiseId = team.franchiseId;
...
export function seasonInfluenceFactsOf(season: SeasonM25SeasonFacts): SeasonInfluenceFacts {
    ...
    for (const snapshot of season.balanceSnapshots) {
        for (const balance of Object.values(snapshot)) {
            ...
            if (balance < 0)
                debtBoundaries += 1;
            if (balance > SEASON_INFLUENCE_CAP)
                capViolations += 1;
        }
    }
    ...
    const objectiveSuccessRate = evaluated.length === 0
        ? null
...
export function foldInfluenceCohort(seasons: readonly SeasonM25SeasonFacts[]): {
    ...
    for (const fact of facts) {
        ...
        if (fact.objectiveSuccessRate !== null) {
            totals.objectiveSuccesses += Math.round(fact.objectiveSuccessRate * fact.objectiveEvaluations);
        }
    ...
    return {
        ...
        objectiveSuccessRate: totals.objectiveEvaluations === 0
            ? null
...
export function evaluateInfluenceGates(args: {
    ...
    const metrics: M25Gate[] = [
        m25ToleranceGate('ledgerReconciliation', c.reconciliationFailures, 0, 0, c.balanceChecks, SEASON_INFLUENCE_MIN_BOUNDARY_SAMPLE),
        m25ToleranceGate('incomeIdentity', c.incomeIdentityFailures, 0, 0, c.balanceChecks, SEASON_INFLUENCE_MIN_BOUNDARY_SAMPLE),
        m25RangeGate('debtFrequency', c.debtFrequency, 0, SEASON_INFLUENCE_DEBT_FREQUENCY_MAX, c.balanceChecks, SEASON_INFLUENCE_MIN_BOUNDARY_SAMPLE),
        m25ToleranceGate('zeroCapViolations', c.capViolations, 0, 0, c.balanceChecks, SEASON_INFLUENCE_MIN_BOUNDARY_SAMPLE),
        m25RangeGate('objectiveSuccessRate', c.objectiveSuccessRate ?? 0, SEASON_INFLUENCE_OBJECTIVE_SUCCESS_MIN, SEASON_INFLUENCE_OBJECTIVE_SUCCESS_MAX, objectiveSample, SEASON_INFLUENCE_MIN_OBJECTIVE_SAMPLE),
        m25RangeGate('extraOfferSpendRate', c.extraOfferSpendShare, 0, SEASON_INFLUENCE_EXTRA_OFFER_SPEND_MAX, c.extraOfferWindows, SEASON_INFLUENCE_MIN_WINDOW_SAMPLE),
        m25RangeGate('rehabSpendRate', c.rehabSpendShare, 0, SEASON_INFLUENCE_REHAB_SPEND_MAX, c.rehabInjuries, SEASON_INFLUENCE_MIN_INJURY_SAMPLE),
        m25ToleranceGate('heldOut.reconciliation', h.reconciliationFailures, 0, 0, h.balanceChecks, SEASON_INFLUENCE_MIN_BOUNDARY_SAMPLE),
        m25ToleranceGate('heldOut.incomeIdentity', h.incomeIdentityFailures, 0, 0, h.balanceChecks, SEASON_INFLUENCE_MIN_BOUNDARY_SAMPLE),
        m25RangeGate('heldOut.debtFrequency', h.debtFrequency, 0, SEASON_INFLUENCE_DEBT_FREQUENCY_MAX, h.balanceChecks, SEASON_INFLUENCE_MIN_BOUNDARY_SAMPLE),
        m25RangeGate('heldOut.objectiveSuccessRate', h.objectiveSuccessRate ?? 0, SEASON_INFLUENCE_OBJECTIVE_SUCCESS_MIN, SEASON_INFLUENCE_OBJECTIVE_SUCCESS_MAX, h.objectiveEvaluations, SEASON_INFLUENCE_MIN_OBJECTIVE_SAMPLE),
    ];
    return metrics;
}
...
export function validateSeasonInfluenceTargets(args: SeasonInfluenceArgs, outPath: string): CliReport {
    void args;
...
export function seasonInfluenceCalibrate(args: SeasonInfluenceArgs): CliReport {
    ...
    try {
        ...
        return makeReport('season influence calibrate', { seedFrom: from, seedTo: to, workers }, { failures: [`calibration cohort failed: ${(error as Error).message}`], exitCode: 2 });
    }
    ...
    const gates = {
        ...
        heldOut: gateValue(metrics, 'heldOut.reconciliation') &&
    ...
    return makeReport('season influence calibrate', { seedFrom: from, seedTo: to, workers }, { details, failures: gateFailures, payload });
}
