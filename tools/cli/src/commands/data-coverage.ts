...
import { hoopRushManifestSchema, type CoverageSummary, type HoopRushManifest, } from '@hoop-rush/data-contracts';
...
export async function dataCoverage(args: {
    ...
    try {
        ...
        return makeReport('data coverage', { input: inputPath }, {
    ...
    if (args.franchise)
        rows = rows.filter((row) => row.franchiseId === args.franchise);
    if (args.era)
        rows = rows.filter((row) => row.eraId === args.era);
    if (args.status)
    ...
    for (const row of available.slice(0, 10)) {
        ...
        details.push(`${row.franchiseId}/${row.eraId}: ${String(row.playerCount)} players · ${summary?.coverageBand ?? 'n/a'} · lowConfidence ${String(summary?.lowConfidenceShare)}`);
    }
    return makeReport('data coverage', { input: inputPath, franchise: args.franchise ?? null, era: args.era ?? null }, { details, payload });
}
