...
import { eraSimulationProfileSchema, franchiseEraPoolSchema, hoopRushManifestSchema, opponentBracketSchema, playersIndexSchema, rosterDetailsSchema, REQUIRED_RATING_KEYS, SEASON_DRAFT_CATALOG_VERSION, SELECTION_SCORE_VERSION, seasonFreeAgencyIndexSchema, unavailabilityReasonSchema, POSITIONS, POSITION_NORMALIZATION_VERSION, playableSlotGroups, type HoopRushManifest, } from '@hoop-rush/data-contracts';
...
function auditLineage(manifest: HoopRushManifest): AuditResult {
    ...
    for (const [franchiseId, segments] of bySlot) {
        ...
        for (const [i, current] of sorted.entries()) {
            if (current.validThroughSeasonKey !== undefined &&
            ...
            if (next &&
                ...
                failures.push(`lineage: ${franchiseId} overlapping ranges ${current.validThroughSeasonKey} vs ${next.validFromSeasonKey}`);
            }
        }
    }
    details.push(`lineage: ${String(manifest.modernFranchiseSlots.length)} slots · ${String(manifest.franchiseLineage.length)} segments`);
...
function auditEras(manifest: HoopRushManifest): AuditResult {
    ...
    for (const era of manifest.eras) {
        if (ids.has(era.eraId))
            failures.push(`eras: duplicate eraId ${era.eraId}`);
        ids.add(era.eraId);
        if (era.fromSeasonKey > era.toSeasonKey) {
            failures.push(`eras: ${era.eraId} range inverted (${era.fromSeasonKey} > ${era.toSeasonKey})`);
        }
    }
    for (let i = 1; i < sorted.length; i += 1) {
        ...
        if (curr.fromSeasonKey <= prev.toSeasonKey) {
            failures.push(`eras: ranges overlap (${prev.eraId} ends ${prev.toSeasonKey}, ${curr.eraId} starts ${curr.fromSeasonKey})`);
        }
    }
...
async function auditPools(manifest: HoopRushManifest, manifestDir: string, verbose: boolean): Promise<AuditResult> {
    const failures: string[] = [];
    const details: string[] = [];
...
function auditPoolContent(content: Buffer, index: HoopRushManifest['pools'][number], manifest: HoopRushManifest, failures: string[], details: string[], playerSeasons: Map<string, string>): void {
    ...
    if (!parsed.success) {
        failures.push(`pools: ${key} asset fails the pool schema: ${parsed.error.issues[0]?.path.join('.') ?? '(root)'} ${parsed.error.issues[0]?.message ?? ''}`);
        return;
    }
    ...
    for (const player of pool.players) {
        ...
        if (era !== undefined &&
            ...
            failures.push(`pools: ${key} ${player.displayName} season ${player.seasonKey} outside era ${era.fromSeasonKey}-${era.toSeasonKey}`);
        }
        if (player.eligibility.teamGames < player.eligibility.minimumTeamGames) {
            failures.push(`pools: ${key} ${player.displayName} has ${String(player.eligibility.teamGames)} team games (min ${String(player.eligibility.minimumTeamGames)})`);
        }
        ...
        if (player.historicalTeamIdentity.seasonKey !== player.seasonKey ||
        ...
        if (player.seasonKey < '1979-80') {
            if (player.stats.threesAttempted !== null || player.stats.threesMade !== null) {
                failures.push(`pools: ${key} ${player.displayName} pre-1979 season with three-point observations`);
            }
        }
    ...
    const forwards = pool.players.filter((p) => playableSlotGroups(p.positions.playable).includes('F'));
    const centers = pool.players.filter((p) => playableSlotGroups(p.positions.playable).includes('C'));
    if (guards.length < 2 || forwards.length < 2 || centers.length < 1) {
        failures.push(`pools: ${key} cannot form G,G,F,F,C (G ${String(guards.length)}, F ${String(forwards.length)}, C ${String(centers.length)})`);
    }
    for (const player of pool.players) {
        ...
        for (const position of secondary) {
            if (!playable.includes(position)) {
                failures.push(`pools: ${key} ${player.displayName} secondary ${position} missing from playable`);
            }
            if (position === primary) {
                failures.push(`pools: ${key} ${player.displayName} secondary ${position} equals primary ${primary}`);
            }
        }
        ...
        if (normalizationVersion !== POSITION_NORMALIZATION_VERSION) {
            failures.push(`pools: ${key} ${player.displayName} position normalization ${normalizationVersion} != ${POSITION_NORMALIZATION_VERSION}`);
        }
    ...
    for (const player of pool.players) {
        ...
        const recomputed = pools.selectionScore(rawOverall, player.summaryRatings.offenseRating, player.summaryRatings.defenseRating, player.stats.usageRate, player.eligibility.teamMinutes, player.eligibility.teamGames);
        if (Math.abs(recomputed - player.selectionScore) > 1e-9) {
            failures.push(`pools: ${key} ${player.displayName} selectionScore not reproducible (packaged ${String(player.selectionScore)}, recomputed ${String(recomputed)})`);
        }
        if (player.selectionScoreVersion !== SELECTION_SCORE_VERSION) {
            failures.push(`pools: ${key} ${player.displayName} selectionScoreVersion ${player.selectionScoreVersion} != ${SELECTION_SCORE_VERSION}`);
        }
    }
    const withFallback = pool.players.filter((p) => p.altIds?.bbref != null || p.altIds?.photoUrl != null).length;
    details.push(`pools: ${key} fallback coverage ${String(withFallback)}/${String(pool.players.length)} · band ${pool.coverageSummary.coverageBand} · lowConfidence ${String(pool.coverageSummary.lowConfidenceShare)}`);
    if (manifest.assets.headshotUrlTemplate) {
        ...
        if (missingMarker.length > 0) {
            failures.push(`pools: ${key} ${String(missingMarker.length)} players lack nbaHeadshotAvailable while a primary headshot template is configured`);
        }
    }
    details.push(`pools: ${key} ${String(pool.players.length)} players audited`);
}
async function auditEraSimulationProfiles(manifest: HoopRushManifest, manifestDir: string, verbose: boolean): Promise<AuditResult> {
    const failures: string[] = [];
    const details: string[] = [];
    const eraIds = new Set(manifest.eras.map((e) => e.eraId));
    for (const entry of manifest.eraSimulationProfiles) {
        ...
        try {
            ...
            const parsed = eraSimulationProfileSchema.safeParse(JSON.parse(content.toString('utf8')) as unknown);
            if (!parsed.success) {
                ...
                details.push(`era-sim: ${entry.eraId} profile ${parsed.data.profileVersion} (${parsed.data.parameters.source})`);
            }
        }
        catch {
...
async function auditBracket(manifest: HoopRushManifest, manifestDir: string, verbose: boolean): Promise<AuditResult> {
    const failures: string[] = [];
    const details: string[] = [];
    ...
    try {
        ...
        if (!parsed.success) {
            failures.push(`bracket: artifact fails the bracket schema: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
            return { ok: failures.length === 0, details, failures };
        }
        const bracket = parsed.data;
        failures.push(...validateBracketContent(bracket).map((f) => `bracket: ${f}`), ...scheduleInvariants(bracket.schedule).map((f) => `bracket: ${f}`));
        const percentiles = bracket.opponents.map((o) => o.strength.percentile);
        const sorted = [...percentiles].sort((a, b) => a - b);
        const median = sorted[Math.floor(sorted.length / 2)] ?? 0;
        details.push(`bracket: ${String(bracket.opponents.length)} opponents · ${String(bracket.schedule.length)} games · median pct ${median.toFixed(3)} · version ${bracket.bracketVersion}`);
    }
    catch {
...
function auditAssets(manifest: HoopRushManifest): AuditResult {
    ...
    if (manifest.assets.headshotUrlTemplateSecondary &&
    ...
    if (manifest.assets.logoUrlTemplate &&
    ...
    if (manifest.assets.logoUrlTemplateSecondary &&
    ...
    details.push(`assets: source "${manifest.assets.source}", cacheVersion ${manifest.assets.cacheVersion}`);
    return { ok: failures.length === 0, details, failures };
}
async function auditGlobalAssets(manifest: HoopRushManifest, manifestDir: string, verbose: boolean): Promise<AuditResult> {
    const failures: string[] = [];
    const details: string[] = [];
    if (manifest.playersIndex === undefined && manifest.rosterDetails === undefined) {
        return { ok: true, details, failures };
    }
    const loadAsset = async (entry: {
        url: string;
        contentHash: string;
    } | undefined, label: string): Promise<{
        keys: Set<string>;
        parsed: unknown;
    } | null> => {
        if (entry === undefined) {
    ...
    if (indexAsset !== null) {
        ...
        if (!parsed.success) {
            failures.push(`playersIndex: asset fails the schema: ${parsed.error.issues[0]?.path.join('.') ?? '(root)'} ${parsed.error.issues[0]?.message ?? ''}`);
        }
        else {
    ...
    if (detailsAsset !== null) {
        ...
        if (!parsed.success) {
            failures.push(`rosterDetails: asset fails the schema: ${parsed.error.issues[0]?.path.join('.') ?? '(root)'} ${parsed.error.issues[0]?.message ?? ''}`);
        }
        else {
            for (const player of parsed.data.players) {
                const key = keyOf(player);
...
async function auditSeasonFreeAgencyIndex(manifest: HoopRushManifest, manifestDir: string, verbose: boolean): Promise<AuditResult> {
    const failures: string[] = [];
    const details: string[] = [];
    ...
    if (!parsed.success) {
        failures.push(`free-agency-index: artifact fails the index schema: ${parsed.error.issues[0]?.path.join('.') ?? '(root)'} ${parsed.error.issues[0]?.message ?? 'unknown'}`);
        return { ok: failures.length === 0, details, failures };
    }
    ...
    if (draftEntry === undefined) {
        ...
        failures.push(`free-agency-index: catalogRef content hash ${index.catalogRef.contentHash} does not match the packaged draft catalog ${draftEntry.contentHash}`);
    }
    else if (index.catalogRef.catalogVersion !== SEASON_DRAFT_CATALOG_VERSION) {
        failures.push(`free-agency-index: unexpected catalogVersion ${index.catalogRef.catalogVersion}`);
    }
    const bandCounts = { featured: 0, role: 0, development: 0, emergency: 0 } as Record<'featured' | 'role' | 'development' | 'emergency', number>;
    for (const candidate of index.candidates) {
        bandCounts[candidate.band] += 1;
    }
    details.push(`free-agency-index: ${String(index.candidates.length)} candidates · ${String(Object.keys(index.groupedVersions).length)} identities · featured ${String(bandCounts.featured)} / role ${String(bandCounts.role)} / development ${String(bandCounts.development)} / emergency ${String(bandCounts.emergency)} · ${String(content.length)} bytes`);
    return { ok: failures.length === 0, details, failures };
}
export async function dataValidate(inputPath: string, verbose: boolean): Promise<CliReport> {
    ...
    try {
        ...
        return makeReport('data validate', { input: inputPath }, {
    ...
    if (!result.success) {
        const failures = result.error.issues.map((issue) => `manifest: ${issue.path.join('.') || '(root)'} ${issue.message}`);
    ...
    return makeReport('data validate', { input: inputPath, dataVersion: manifest.dataVersion }, {
        details,
        failures,
    });
}
