...
import { threePointReconstructionArtifactSchema, type ReconstructionMetrics, } from '@hoop-rush/data-contracts';
import { fitThreePointReconstruction, loadCohortRows, validateThreePointReconstructionArtifact, writeThreePointReconstructionArtifact, } from '@hoop-rush/importer';
...
export function calibrateThreePoint(args: {
    ...
    if (!gates.meanBiasNonPositiveTranslatedAttemptRate) {
        failures.push('translated attempt-rate mean bias vs the modern cohort is positive (mean bias attempt rate <= 0 gate failed)');
    }
    ...
    return makeReport('calibrate three-point', { write: args.write === true, output }, { details, failures, payload });
}
function modelDetails(model: string, metrics: ReconstructionMetrics): string[] {
    ...
    const evidence = metrics.evidenceBands.map((band) => `${band.band} MAE ${band.mae.toFixed(4)} bias ${band.bias.toFixed(4)} (n=${String(band.count)})`);
    return [
...
function existingArtifactDetail(output: string): string {
    ...
    if (!parsed.success)
        return 'existing artifact: invalid (schema validation failed)';
