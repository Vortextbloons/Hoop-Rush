...
import { humanFranchiseIdOf, SEASON_BLOCK_COUNT, SEASON_BLOCK_VERSION, SEASON_RUN_SCHEMA_VERSION, commandIdSchema, seasonCandidateCheckpointSchema, seasonRunSchema, seasonScheduleSchema, type EraSimulationProfile, type SeasonCandidateCheckpoint, type SeasonCheckpointState, type SeasonDraftCatalog, type SeasonGamePlayerInput, type SeasonGameSummary, type SeasonHealthState, type SeasonObjectiveId, type SeasonRun, type SeasonSchedule, type SeasonSubmitBlockCommand, } from '@hoop-rush/data-contracts';
import { SeasonBlockValidationError, auditSeasonBlock, createSeasonEffectsState, deriveSeasonPostBlockState, expandSeasonRunRosters, handleSubmitSeasonBlockCommand, rosterPlayerIdsOf, seasonCheckpointDigest, seasonNextBlockIndex, seasonRotationSetDigest, type SeasonBlockSimulationInput, } from '@hoop-rush/engine';
...
import { DEFAULT_MANIFEST, DEFAULT_SEASON_DIR, loadSeasonDraftCatalog, readJsonFile, } from './season-data.ts';
...
export const DEFAULT_RUN_FIXTURE = resolve(new URL('../fixtures/', import.meta.url).pathname
    .replace(/^\/([A-Za-z]:)/, '$1')
    .replace(/%20/g, ' '), 'season-run.json');
...
export function loadSeasonRunFixture(path: string): SeasonRun {
    ...
    if (!parsed.success) {
        throw new Error(`season run ${path} fails the schema: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
    }
    return parsed.data;
}
export function createSeasonBlockRunner(options: {
    runPath?: string | null;
...
export function initialEffectsState(expanded: ReadonlyMap<string, SeasonGamePlayerInput>): SeasonEffectsState {
...
export function runnerNextBlockIndex(state: SeasonBlockRunnerState): number {
    ...
    if (next === null) {
        throw new Error(`season run ${state.run.runId} is complete (cursor ${String(state.run.cursor.completedRounds)})`);
    }
    return next;
}
export function runnerBlockCommand(state: SeasonBlockRunnerState, blockIndex: number): SeasonSubmitBlockCommand {
    return {
        schemaVersion: SEASON_RUN_SCHEMA_VERSION,
        blockVersion: SEASON_BLOCK_VERSION,
        command: 'submit-season-block',
        commandId: commandIdSchema.parse(`season-block-${String(blockIndex)}-${String(state.acceptedCommandIds.length)}`),
...
export function runnerPipelineInput(state: SeasonBlockRunnerState, command: SeasonSubmitBlockCommand): SeasonBlockSimulationInput {
    return {
        command,
        run: state.run,
...
export function runBlockThroughHandler(state: SeasonBlockRunnerState, blockIndex: number): SeasonCandidateCheckpoint {
...
function validateBlockOption(block: string | null, state: SeasonBlockRunnerState): number {
    if (block === null)
        return runnerNextBlockIndex(state);
...
export function seasonBlockSimulate(args: {
    ...
    return makeReport('season block simulate', { run: state.run.runId, block: blockIndex }, { details, failures: auditFailures, payload });
}
export function seasonBlockAudit(args: {
    ...
    if (!parsed.success) {
        return makeReport('season block audit', { input: inputPath }, {
            failures: [
    ...
    details.push(...auditFailures.slice(0, 10));
    return makeReport('season block audit', { input: inputPath }, { details, failures: auditFailures, payload });
}
...
export function seasonFullSimulate(args: {
    ...
    for (let blockIndex = 0; blockIndex < SEASON_BLOCK_COUNT; blockIndex += 1) {
        ...
        if (previousPostState !== null &&
            ...
            auditFailures.push(`block ${String(blockIndex)} expected state facts do not match the previous post-block facts (expected r${String(checkpoint.expectedStateRevision)}/d${checkpoint.expectedStateDigest}, previous r${String(previousPostState.stateRevision)}/d${previousPostState.stateDigest})`);
        }
        previousPostState = {
    ...
    const details = [
        ...
        ...blockDigests.map((entry) => `block ${String(entry.blockIndex)} ${entry.digest} (${entry.durationMs.toFixed(0)}ms)`),
    ...
    details.push(...auditFailures.slice(0, 10));
    return makeReport('season full simulate', { run: state.run.runId }, { details, failures: auditFailures, payload });
}
