...
import { z } from 'zod';
import { SEASON_CAMPAIGN_TARGETS_VERSION, SEASON_CAMPAIGN_VERSION, SEASON_GAME_TARGETS_VERSION, SEASON_GAME_VERSION, buildEmptyCampaignState, } from '@hoop-rush/data-contracts';
import { createEngineContext, generateSeasonCampaignOffers, evaluateSeasonCampaignOpportunity, generateSeasonCampaignEvolutionOffers, applySeasonCampaignEvolutionSelection, applySeasonCampaignReward, createInitialSeasonInfluenceState, generateSeasonSchedule, } from '@hoop-rush/engine';
import { makeReport, type CliReport } from '../report.ts';
import { parseSeedRange, parseWorkers } from '../args.ts';
import { DEFAULT_MANIFEST, DEFAULT_SEASON_DIR } from './season-data.ts';
import { gateValue, gateSummary, m25ToleranceGate, m25RangeGate, mean, seasonCalibrationSeed, seedIndexRange, type M25Gate, } from './season-calibration.ts';
...
function auditOffersForSeed(rootSeed: string): {
    ...
    const humanFranchiseId = run.league.teams.find((t) => t.control === 'human')?.franchiseId ??
    ...
    for (let blockIndex = 0; blockIndex <= 7; blockIndex += 1) {
        ...
        if (blockIndex === 5 && campaignState.evolutionSelection === null) {
            try {
                ...
                if (evoOffers.length === 0 || evoOffers.length > 3)
        ...
        try {
            ...
            throw new Error(`campaign generation failed for seed ${rootSeed} block ${String(blockIndex)}: ${(e as Error).message} audit ${JSON.stringify((e as unknown as {
                audit?: unknown;
            }).audit ?? [])}`);
        }
        if (offers.length !== 2) {
        ...
        if (oppIds.size !== 2 || tplIds.size !== 2) {
            throw new Error(`campaign offers not unique for seed ${rootSeed} block ${String(blockIndex)}: oppIds ${JSON.stringify([...oppIds])} tplIds ${JSON.stringify([...tplIds])} offers ${JSON.stringify(offers.map((o) => ({ id: o.opportunityId, tpl: o.templateId })))}`);
        }
        ...
        try {
            ...
            if (JSON.stringify(offers) !== JSON.stringify(second))
        ...
        try {
            ...
            if (JSON.stringify(offers) !== JSON.stringify(shuffledOffers))
        ...
        if (offers[0]) {
            ...
            campaignState = {
                ...campaignState,
                offers: { ...campaignState.offers, [blockIndex]: offers as [
                        typeof first,
                        typeof first
                    ] },
                selections: {
...
export function validateSeasonCampaignTargets(args: SeasonCampaignArgs, outPath: string): CliReport {
    ...
    return validateTargetsArtifact({
        ...
        extraChecks: (parsed) => {
            ...
            if (parsed.policy.offersPerCheckpoint !== 2)
                failures.push('offersPerCheckpoint must be 2');
...
export function seasonCampaignCalibrate(args: SeasonCampaignArgs): CliReport {
    ...
    const writeRequested = (args['write'] as unknown) === true ||
    ...
    try {
        ...
        return makeReport('season campaign calibrate', { seedFrom: from, seedTo: to, workers }, { failures: [`calibration cohort failed: ${(error as Error).message}`], exitCode: 2 });
    }
    ...
    const gates = {
        ...
        heldOut: gateValue(metrics, 'heldOut.offersPerCheckpoint') &&
    ...
    if (shouldWrite && !targetsWritten)
        gateFailures.push('targets artifact was not written');
    return makeReport('season campaign calibrate', { seedFrom: from, seedTo: to, workers }, { details, failures: gateFailures, payload });
}
export function seasonCampaignAudit(args: {
    input: string | null;
    manifest: string | null;
}): CliReport {
    const rootSeed = seasonCalibrationSeed(0);
    const facts = auditOffersForSeed(rootSeed);
    const pass = facts.offerPerCheckpointFailures === 0 &&
    ...
    const failures: string[] = [];
    if (!pass)
        failures.push('campaign hard gates failed');
    if (facts.offerPerCheckpointFailures > 0)
        failures.push(`exactly 2 offers per checkpoint violated: ${String(facts.offerPerCheckpointFailures)}`);
    if (facts.unsupportedFactFailures > 0)
        failures.push(`unsupported fact violations: ${String(facts.unsupportedFactFailures)}`);
    if (facts.duplicateRewardFailures > 0)
        failures.push(`duplicate reward violations: ${String(facts.duplicateRewardFailures)}`);
    if (facts.branchViolations > 0)
        failures.push(`branch violations: ${String(facts.branchViolations)}`);
    if (facts.evolutionViolations > 0)
        failures.push(`evolution violations: ${String(facts.evolutionViolations)}`);
    if (facts.determinismFailures > 0)
        failures.push(`determinism failures: ${String(facts.determinismFailures)}`);
    if (facts.orderInvarianceFailures > 0)
        failures.push(`order invariance failures: ${String(facts.orderInvarianceFailures)}`);
    return makeReport('season campaign audit', { input: args.input ?? 'seed-0' }, {
        details,
