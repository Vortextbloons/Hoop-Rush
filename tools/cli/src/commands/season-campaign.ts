...
import { z } from 'zod';
import { SEASON_CAMPAIGN_TARGETS_VERSION, SEASON_GAME_TARGETS_VERSION, SEASON_GAME_VERSION, buildEmptyCampaignState, commandIdSchema, type Seed, } from '@hoop-rush/data-contracts';
...
import { gateValue, gateSummary, m25ToleranceGate, seasonCalibrationSeed, seedIndexRange, type M25Gate, } from './season-calibration.ts';
...
function auditOffersForSeed(rootSeed: Seed): {
    ...
    for (let blockIndex = 0; blockIndex <= 7; blockIndex += 1) {
        ...
        try {
            ...
            throw new Error(`campaign generation failed for seed ${rootSeed} block ${String(blockIndex)}: ${(e as Error).message} audit ${JSON.stringify((e as {
                audit?: unknown;
            }).audit ?? [])}`);
        }
        if (offers.length !== 2) {
