...
function auditOffersForSeed(rootSeed: Seed): {
  ...
  try {
    ...
    throw new Error(
      `baseline runSeasonM25 failed for seed ${rootSeed}: ${(error as Error).message}`,
    );
  }
  ...
  for (let blockIndex = 0; blockIndex <= 7; blockIndex += 1) {
    ...
    if (offers.length !== 2) {
      throw new Error(
        `campaign generation returned ${String(offers.length)} offers for seed ${rootSeed} block ${String(blockIndex)} (expected 2): ${JSON.stringify(offers.map((o) => o.opportunityId))}`,
      );
    }
    ...
    if (offers[0]) {
      ...
      try {
        ...
        const influence = createInitialSeasonInfluenceState(
          run.league.teams.map((t) => t.franchiseId),
        );
...
export function evaluateCampaignGates(args: {
  ...
  return [
    m25ToleranceGate(
      'offersPerCheckpoint',
      c.offerPerCheckpointFailures,
      0,
      0,
      c.eligibleCheckpoints,
      minCheckpoints,
    ),
    m25ToleranceGate(
      'zeroUnsupportedFact',
      c.unsupportedFactFailures,
      0,
      0,
      c.offersGenerated,
      minCheckpoints * 2,
    ...
    m25ToleranceGate(
      'heldOut.offersPerCheckpoint',
      h.offerPerCheckpointFailures,
      0,
      0,
      h.eligibleCheckpoints,
      minCheckpoints,
    ),
    m25ToleranceGate(
      'heldOut.zeroUnsupportedFact',
      h.unsupportedFactFailures,
      0,
      0,
      h.offersGenerated,
      minCheckpoints * 2,
...
export function validateSeasonCampaignTargets(
  args: SeasonCampaignArgs,
  outPath: string,
): CliReport {
  void args;
...
export function seasonCampaignAudit(args: {
  ...
  if (facts.offerPerCheckpointFailures > 0)
    failures.push(
      `exactly 2 offers per checkpoint violated: ${String(facts.offerPerCheckpointFailures)}`,
    );
