...
import { z } from 'zod';
import { SEASON_GAME_VERSION, SEASON_GAME_TARGETS_VERSION, SEASON_INJURY_TARGETS_VERSION, type FranchiseId, type SeasonInjuryRecord, type SeasonRun, } from '@hoop-rush/data-contracts';
import { createEngineContext, rollSeasonInjuryForPlayer, type SeasonInjuryRollInput, } from '@hoop-rush/engine';
...
export function runSeasonHealthCohort(args: SeasonHealthArgs, seedIndices: number[], probeRootSeed: import('@hoop-rush/data-contracts').Seed): {
    ...
    const low = (input: Omit<SeasonInjuryRollInput, 'rootSeed' | 'gameId' | 'playerVersionId' | 'franchiseId'>, exposures: number): number => runInjuryRollProbe({ rootSeed: probeRootSeed, input, exposures });
