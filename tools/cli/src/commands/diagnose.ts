...
function sideMisses(team: GameResult['home']): number {
    return (team.box.fieldGoals.attempted -
        team.box.fieldGoals.made +
        (team.box.freeThrows.attempted - team.box.freeThrows.made));
}
function foldPlayer(acc: PlayerAccumulator, box: GameResult['home']['players'][number], team: GameResult['home'], opponent: GameResult['away']): void {
    acc.games += 1;
...
export function simDiagnose(args: {
    ...
    for (let i = 0; i < samples; i += 1) {
        ...
        for (const side of ['home', 'away'] as const) {
            ...
            for (const box of team.players) {
                if (box.diagnostics)
                    teamUsage += box.diagnostics.usage;
            }
        }
    }
    ...
    const spread = usagePerGame.length >= 2 && top !== undefined && last !== undefined
...
export function simSeason(args: {
        ...
        for (let g = 0; g < 82; g += 1) {
            ...
            for (const box of result.home.players) {
                const acc = players.get(box.playerId) ?? {
                    ...
                    displayName: fixture.home.players.find((p) => p.playerId === box.playerId)?.displayName ??
                        box.playerId,
                    games: 0,
                    points: 0,
                    fieldGoalMakes: 0,
                ...
                acc.perGameFieldGoalPct.push(box.fieldGoals.attempted > 0 ? box.fieldGoals.made / box.fieldGoals.attempted : 0);
                acc.perGameFreeThrowPct.push(box.freeThrows.attempted > 0 ? box.freeThrows.made / box.freeThrows.attempted : 0);
    ...
    const rows = allSeasons.map(({ season, players }) => ({
        ...
        players: players
            ...
            return {
                ...
                variance: {
                    fieldGoalRatio: round2(varianceRatio(p.perGameFieldGoalPct, p.perGameFieldGoalAttempts, p.fieldGoalMakes / Math.max(1, p.fieldGoalAttempts))),
                    freeThrowRatio: round2(varianceRatio(p.perGameFreeThrowPct, p.perGameFreeThrowAttempts, p.freeThrowMakes / Math.max(1, p.freeThrowAttempts))),
                },
            };
        })
    ...
    for (const { season, players } of allSeasons) {
        for (const p of players) {
            ...
            for (const [label, ratio] of [
                [
                    'fieldGoal',
                    varianceRatio(p.perGameFieldGoalPct, p.perGameFieldGoalAttempts, p.fieldGoalMakes / Math.max(1, p.fieldGoalAttempts)),
                ],
                [
                    'freeThrow',
                    varianceRatio(p.perGameFreeThrowPct, p.perGameFreeThrowAttempts, p.freeThrowMakes / Math.max(1, p.freeThrowAttempts)),
                ],
            ] as const) {
                if (games < 20 || ratio < RATIO_BAND[0] || ratio > RATIO_BAND[1]) {
                    varianceFailures.push(`${p.playerId} season ${String(season)} ${label} variance ratio ${ratio.toFixed(2)} outside ${String(RATIO_BAND[0])}..${String(RATIO_BAND[1])}`);
                }
            }
        }
    }
    ...
    const details = [
        ...
        ...rows.flatMap(({ season, players }) => [
            ...
            ...players.map((p) => `  ${p.playerId}: ${String(p.pointsPerGame)} ppg · FG ${p.fieldGoalPct.toFixed(3)} · 3P ${p.threePointPct.toFixed(3)} · FT ${p.freeThrowPct.toFixed(3)} · var ${p.variance.fieldGoalRatio.toFixed(2)}/${p.variance.freeThrowRatio.toFixed(2)}`),
        ]),
    ];
    return makeReport('sim season', { fixture: fixtureId, seasons: seasonSamples }, { details, failures: varianceFailures, payload });
}
function varianceRatio(perGameRates: readonly number[], perGameAttempts: readonly number[], seasonRate: number): number {
