...
import { POSITION_NORMALIZATION_VERSION, contentHashSchema, eraIdSchema, franchiseIdSchema, playerIdSchema, seasonKeySchema, } from '@hoop-rush/data-contracts';
...
function legalPool(altIds: PeakPlayerSeason['altIds'] = null): ReturnType<typeof buildPool> {
    ...
    return buildPool(playable.map((positions, index) => {
        const primary = positions[0];
        if (primary === undefined)
            throw new Error('fixture position label missing');
...
describe('dataValidate season free-agency index audit', () => {
    ...
    it('flags a free-agency index whose catalogRef pins a different catalog hash', async () => {
        ...
        const seasonDir = join(dir, 'season');
        await mkdir(seasonDir);
        await writeFile(join(seasonDir, 'draft-catalog.json'), JSON.stringify({ catalogVersion: 'season-draft-catalog-v4' }));
        ...
        const report = await dataValidate(path, false);
        expect(report.ok).toBe(false);
        expect(report.failures.some((f) => f.includes('does not match the packaged draft catalog'))).toBe(true);
    });
