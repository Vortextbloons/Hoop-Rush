...
import { checkGameResult, createEngineContext, fnv1a32, hex32, simulateGame, } from '@hoop-rush/engine';
import { gameSimulationInputSchema, seedSchema, type EraSimulationProfile, type GameResult, type GameSimulationInput, } from '@hoop-rush/data-contracts';
...
export function loadFixture(fixtureId: string): SimFixture {
    ...
    if (!parsed.success) {
        throw new UsageError(`fixture ${fixtureId} fails validation: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
    }
    return parsed.data;
}
...
export function chunkRange(from: number, to: number, workers: number): Array<{
    from: number;
    to: number;
}> {
...
export function buildInput(fixture: SimFixture, profile: EraSimulationProfile, seed: string, variant: boolean): GameSimulationInput {
    const variantProfile = variant && fixture.variantParameters
        ? {
            ...profile,
...
export function simGame(args: {
    ...
    if (!fixtureId)
        throw new UsageError('sim game requires --input <fixture-id>');
    const seed = args.seed;
    if (seed === undefined)
        throw new UsageError('sim game requires --seed <hex>');
    ...
    return makeReport('sim game', { fixture: fixtureId, seed, engineVersion: payload.engineVersion }, { details, failures: invariants, payload });
}
export async function simBatch(args: {
    ...
    return makeReport('sim batch', { fixture: fixtureId, seedFrom, seedTo, workers }, { details, failures, payload });
}
function renderBoxLines(result: GameResult): string[] {
    const lines: string[] = [];
    for (const side of [result.home, result.away]) {
        const b = side.box;
        lines.push(`${side.displayName}: ${String(b.points)} pts · ${String(b.fieldGoals.made)}/${String(b.fieldGoals.attempted)} FG · ${String(b.threes.made)}/${String(b.threes.attempted)} 3P · ${String(b.freeThrows.made)}/${String(b.freeThrows.attempted)} FT · ${String(b.rebounds.offensive)}+${String(b.rebounds.defensive)}+${String(b.rebounds.team)} REB · ${String(b.assists)} AST · ${String(b.turnovers)} TOV · ${String(b.fouls)} PF · ${String(b.possessions)} POSS`);
        for (const player of side.players) {
            lines.push(`  ${player.playerId}: ${String(player.points)} pts · ${String(player.fieldGoals.made)}/${String(player.fieldGoals.attempted)} FG · ${String(player.rebounds.total)} REB · ${String(player.assists)} AST · ${String(player.turnovers)} TOV · ${String(player.minutes)} MIN`);
        }
    }
    return lines;
}
