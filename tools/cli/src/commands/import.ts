import { DEFAULT_SEASONS, eraProfile, freeze, manifest, opponent, pools, ratings, runPythonFetch, } from '@hoop-rush/importer';
...
export async function importRatings(args: {
    ...
    try {
        ...
        return usageFailure('import ratings', { seasons }, `ratings failed: ${(error as Error).message}`);
    }
    return makeReport('import ratings', { seasons, force: args.forceRatings ?? false }, {
...
export async function importPools(args: {
    ...
    if (args.all) {
        try {
            ...
            return usageFailure('import pools', {}, `computing --all targets failed: ${(error as Error).message}`);
        }
    }
    else {
    ...
    return makeReport('import pools', { targets: targets ?? [['lakers', '1990s']], all: args.all ?? false }, { details: ['pool build complete'] });
}
...
export function importEraProfile(args: {
    era?: string | null;
}): CliReport {
    ...
    return makeReport('import era-profile', { era: eras ?? 'all' }, {
...
export function importOpponent(): CliReport {
    ...
    return makeReport('import opponent', {}, {
...
export function importFreeze(args: {
    report?: string | null;
    era?: string | null;
}): CliReport {
    ...
    return makeReport('import freeze', { report: reportPath }, {
...
export async function importRunAll(args: {
    ...
    if (args.includeSchedule)
        fetchArgs.push('--include-schedule');
    if (args.forceStints)
        fetchArgs.push('--force-stints');
    if (args.skipBbref)
        fetchArgs.push('--skip-bbref');
    ...
    return makeReport('import run-all', { seasons, workers, includeSchedule: args.includeSchedule ?? false }, { details, failures });
}
