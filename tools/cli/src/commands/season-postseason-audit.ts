import { z } from 'zod';
import { seasonPostseasonSummaryDigest, seasonPostseasonSummarySchema, seasonRunReplayExportSchema, seasonRunSchema, type SeasonPostseasonSummary, type SeasonRun, type SeasonRunReplayExport, } from '@hoop-rush/data-contracts';
...
export function auditSeasonPostseasonFacts(input: {
    ...
    for (const summary of input.summaries) {
        ...
        if (!parsed.success) {
            counts.inconsistentSummaries += 1;
            failures.push(`summary ${summary.gameId} fails the schema: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
            continue;
        }
    ...
    for (const conference of ['east', 'west'] as const) {
        ...
        if (sevenEight === undefined || nineTen === undefined)
            continue;
        const sevenWinner = sevenEight.winnerFranchiseId;
        const sevenLoser = sevenEight.winnerFranchiseId === sevenEight.homeFranchiseId
            ? sevenEight.awayFranchiseId
            : sevenEight.homeFranchiseId;
        const nineWinner = nineTen.winnerFranchiseId;
        const nineLoser = nineTen.winnerFranchiseId === nineTen.homeFranchiseId
        ...
        if (finalGame !== undefined) {
            ...
            if (!finalTeams.has(sevenLoser) || !finalTeams.has(nineWinner)) {
                counts.invalidFeeders += 1;
                failures.push(`${conference} play-in final must pair the seven-eight loser ${sevenLoser} and nine-ten winner ${nineWinner}`);
            }
            if (finalTeams.has(sevenWinner)) {
                counts.invalidFeeders += 1;
                failures.push(`${conference} play-in final re-pairs the seven-eight winner ${sevenWinner} (already seeded)`);
            }
            if (finalTeams.has(nineLoser)) {
                counts.invalidFeeders += 1;
                failures.push(`${conference} play-in final re-pairs the eliminated nine-ten loser ${nineLoser}`);
            }
        }
    }
    ...
    for (const summary of ordered) {
        if (summary.round !== 'first-round')
            continue;
        const set = firstRoundTeams.get(summary.conference) ?? new Set<string>();
        const seriesByTeam = firstRoundSeriesByTeam.get(summary.conference) ?? new Map<string, string>();
        for (const team of teamsOf(summary)) {
            ...
            if (priorSeries !== undefined && priorSeries !== summary.seriesId) {
                ...
                failures.push(`${summary.conference} team ${team} appears in two first-round series (${priorSeries}, ${String(summary.seriesId)})`);
            }
    ...
    for (const conference of ['east', 'west'] as const) {
        ...
        if (set.size !== 8) {
            counts.missingTeams += 1;
            failures.push(`${conference} first round carries ${String(set.size)} distinct teams instead of 8`);
        }
    }
    ...
    for (const summary of playoffSummaries) {
        ...
        if (games === undefined)
            seriesGames.set(summary.seriesId ?? '', [summary]);
        else
            games.push(summary);
    }
    ...
    for (const [seriesId, games] of seriesGames) {
        ...
        for (let number = 1; number <= games.length; number += 1) {
            ...
            if (game.status === 'forfeit')
                continue;
            if (game.winnerFranchiseId === homeCourtSide)
                homeCourtWins += 1;
            else
                challengerWins += 1;
            ...
            if ((game.homeFranchiseId === homeCourtSide) !== shouldBeHomeSide) {
                ...
                failures.push(`series ${seriesId} game ${String(game.gameNumber)} home ${game.homeFranchiseId} breaks the 2-2-1-1-1 pattern (home side ${homeCourtSide})`);
            }
        }
        const last = games[games.length - 1];
        const winner = last?.winnerFranchiseId ?? null;
        const winnerWins = winner === null ? 0 : games.filter((game) => game.winnerFranchiseId === winner).length;
    ...
    for (const conference of ['east', 'west'] as const) {
        ...
        for (const summary of ordered) {
            if (summary.round !== 'first-round' || summary.conference !== conference)
                continue;
            if (gameOrdinalOf(summary) <= lastPlayIn) {
                counts.gamesAfterClinching += 1;
                failures.push(`${conference} first-round game ${summary.gameId} precedes the conference play-in games`);
            }
            break;
        }
    }
    for (const conference of ['east', 'west'] as const) {
        const sevenEight = playIn.get(`${conference}-seven-eight`);
        const finalGame = playIn.get(`${conference}-final`);
        if (sevenEight === undefined || finalGame === undefined)
            continue;
        const sevenLoser = sevenEight.winnerFranchiseId === sevenEight.homeFranchiseId
            ? sevenEight.awayFranchiseId
            : sevenEight.homeFranchiseId;
        if (finalGame.homeFranchiseId !== sevenLoser) {
            counts.incorrectHomeCourt += 1;
            failures.push(`${conference} play-in final must be hosted by the seven-eight loser ${sevenLoser}, not ${finalGame.homeFranchiseId}`);
        }
    }
    const finalsSeries = seriesGames.get('finals');
    const finalsWinner = finalsSeries?.find((game) => game.status === 'final')?.winnerFranchiseId ?? null;
    ...
    if (expectedChampion !== null && finalsWinner !== null && expectedChampion !== finalsWinner) {
        ...
        failures.push(`recorded champion ${expectedChampion} does not match the finals winner ${finalsWinner}`);
    }
    if (input.exportArtifact !== undefined &&
        expectedChampion !== null &&
        input.exportArtifact.almanac.championFranchiseId !== expectedChampion) {
        counts.championCompletionMismatch += 1;
        failures.push(`export champion ${expectedChampion} does not match the almanac champion ${input.exportArtifact.almanac.championFranchiseId}`);
    }
    if (input.run !== undefined) {
        ...
        if (stateChampion !== null && expectedChampion !== null && stateChampion !== expectedChampion) {
            counts.championCompletionMismatch += 1;
            failures.push(`state machine champion ${stateChampion} does not match the recorded champion ${expectedChampion}`);
        }
        if (completionChampion !== null &&
            stateChampion !== null &&
            completionChampion !== stateChampion) {
            counts.championCompletionMismatch += 1;
            failures.push(`completion champion ${completionChampion} does not match the state machine champion ${stateChampion}`);
        }
    }
    return { failures, counts };
}
export function seasonPostseasonAudit(args: {
    input: string | null;
}): CliReport {
    ...
    if (exportParsed.success) {
        ...
        return auditReport(inputPath, exportArtifact.runId, exportArtifact.championFranchiseId, exportArtifact.postseasonSummaries.length, failures, counts);
    }
    const fixtureParsed = seasonPostseasonAuditFixtureSchema.safeParse(raw);
    if (!fixtureParsed.success) {
        return makeReport('season postseason audit', { input: inputPath }, {
            failures: [
    ...
    return auditReport(inputPath, fixture.run.runId, champion, fixture.postseasonSummaries.length, failures, counts);
}
function auditReport(inputPath: string, runId: string, champion: string | null, gameCount: number, failures: string[], counts: SeasonPostseasonAuditCounts): CliReport {
    ...
    return makeReport('season postseason audit', { input: inputPath }, { details, failures, payload });
}
