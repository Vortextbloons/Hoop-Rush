...
import { seasonDraftCatalogSchema, seasonLeagueSchema, seasonRosterTargetsSchema, seedSchema, } from '@hoop-rush/data-contracts';
...
function main(): void {
    ...
    const targets = seasonRosterTargetsSchema.parse(typeof input.targets === 'string' ? JSON.parse(input.targets) : input.targets);
