...
function replayFromInitialRun(
  ...
  if (campOfferFailuresList.length > 0) {
    return {
      divergence: {
        ...
        detail: campOfferFailuresList[0] ?? 'campaign offers diverged',
      },
  ...
  if (campEvalFailuresList.length > 0) {
    return {
      divergence: {
        ...
        detail: campEvalFailuresList[0] ?? 'campaign evaluations diverged',
      },
  ...
  if (boardFailuresList.length > 0) {
    return {
      divergence: {
        ...
        commandId: '',
        kind: 'board',
        detail: boardFailuresList[0] ?? 'board facts diverged',
      },
  ...
  if (inquiryFailuresList.length > 0) {
    return {
      divergence: {
        ...
        commandId: '',
        kind: 'inquiries',
        detail: inquiryFailuresList[0] ?? 'inquiry facts diverged',
      },
  ...
  if (aiResponseFailuresList.length > 0) {
    const kind = aiResponseFailuresList[0]?.includes('counter')
      ? ('ai-counter' as const)
      : ('ai-response' as const);
    return {
      divergence: {
        ordinal: null,
        commandId: '',
        kind,
        detail: aiResponseFailuresList[0] ?? 'AI response diverged',
      },
  ...
  if (aiTransactionFailuresList.length > 0) {
    return {
      divergence: {
        ...
        commandId: '',
        kind: 'ai-transaction',
        detail: aiTransactionFailuresList[0] ?? 'AI transaction diverged',
      },
  ...
  if (rehabFailuresList.length > 0) {
    return {
      divergence: {
        ...
        commandId: '',
        kind: 'rehab-outcome',
        detail: rehabFailuresList[0] ?? 'rehab outcome diverged',
      },
  ...
  if (valueTrendFailuresList.length > 0) {
    return {
      divergence: {
        ...
        commandId: '',
        kind: 'value-trends',
        detail: valueTrendFailuresList[0] ?? 'value trends diverged',
      },
  ...
  if (influenceCashFailuresList.length > 0) {
    return {
      divergence: {
        ...
        commandId: '',
        kind: 'influence-cash',
        detail: influenceCashFailuresList[0] ?? 'Influence cash diverged',
      },
...
export function inquiryFailures(run: SeasonRun): string[] {
  ...
  for (const win of trade.windows) {
    ...
    if (allowance < 3 || allowance > 5)
      failures.push(
        `window ${String(win.windowIndex)} inquiryAllowance ${String(allowance)} out of [3,5]`,
      );
    const negotiations =
      (
        win as unknown as {
    ...
    if (negotiations.length > allowance)
      failures.push(
        `window ${String(win.windowIndex)} negotiations ${String(negotiations.length)} exceeds allowance ${String(allowance)}`,
      );
  }
  return failures;
}
...
export function rehabOutcomeFailures(run: SeasonRun): string[] {
  ...
  for (const injury of run.health.injuries) {
    ...
    if (
      injury.rehabOutcome === 'failure' &&
      injury.missedGamesRemaining !== injury.missedGamesTotal &&
      injury.missedGamesTotal < 10000
    ) {
...
export function influenceCashFailures(run: SeasonRun): string[] {
  ...
  for (const entry of run.influence.ledger) {
    ...
    if (entry.source === 'trade-cash-sent' || entry.source === 'trade-cash-received') {
      if (entry.appliedDelta < -2 || entry.appliedDelta > 2)
        failures.push(
          `cash entry ${entry.entryId} amount ${String(entry.appliedDelta)} out of [-2,2]`,
        );
  ...
  for (const [txn, sent] of sentByTxn) {
    ...
    if (Math.abs(sent) !== Math.abs(recv))
      failures.push(
        `cash reconciliation for txn ${txn} sent ${String(sent)} vs recv ${String(recv)}`,
      );
  }
  return failures;
}
