...
import { franchiseEraPoolSchema, hoopRushManifestSchema, type HoopRushManifest, } from '@hoop-rush/data-contracts';
...
export async function dataLineageAudit(args: {
    ...
    try {
        ...
        return makeReport('data lineage-audit', { input: inputPath }, {
    ...
    for (const segment of manifest.franchiseLineage) {
        ...
        if (candidates.length === 0) {
            logoFailures.push(`logos: ${segment.modernFranchiseId} ${segment.displayName} (${segment.validFromSeasonKey}) has no logo candidates`);
            continue;
        }
        for (const [i, candidate] of candidates.entries()) {
            ...
            try {
                ...
                logoFailures.push(`logos: ${segment.modernFranchiseId} ${segment.displayName} candidate ${String(i)} is not a valid URL: ${candidate.url}`);
                continue;
            }
            if (url.protocol !== 'https:') {
                logoFailures.push(`logos: ${segment.modernFranchiseId} ${segment.displayName} candidate ${String(i)} must be https: ${candidate.url}`);
            }
        }
        if (!candidates[0]?.source) {
            logoFailures.push(`logos: ${segment.modernFranchiseId} ${segment.displayName} candidate 0 has no source host`);
        }
    }
    if (args.verifyLogos) {
        for (const segment of manifest.franchiseLineage) {
            ...
            try {
                ...
                clearTimeout(timer);
                if (!response.ok) {
                    logoVerificationFailures.push(`logos: ${segment.modernFranchiseId} ${segment.displayName} primary candidate returned HTTP ${String(response.status)}: ${primary.url}`);
                    continue;
                }
                const contentType = response.headers.get('content-type') ?? '';
                const bytes = (await response.arrayBuffer()).byteLength;
                if (!IMAGE_CONTENT_TYPES.test(contentType)) {
                    logoVerificationFailures.push(`logos: ${segment.modernFranchiseId} ${segment.displayName} primary candidate is not an image (${contentType}): ${primary.url}`);
                    continue;
                }
                if (bytes < 500) {
                    logoVerificationFailures.push(`logos: ${segment.modernFranchiseId} ${segment.displayName} primary candidate is suspiciously small (${String(bytes)} bytes): ${primary.url}`);
                }
            }
            catch {
                logoVerificationFailures.push(`logos: ${segment.modernFranchiseId} ${segment.displayName} primary candidate could not be fetched: ${primary.url}`);
            }
        }
    }
    ...
    for (const pool of manifest.pools) {
        ...
        for (const player of poolParsed.data.players) {
            ...
            const segment = manifest.franchiseLineage.find((s) => s.modernFranchiseId === pool.franchiseId &&
            ...
            if (!segment) {
                ownershipFailures.push(`${pool.franchiseId}/${pool.eraId}: ${player.displayName} ${player.seasonKey} (${identity.displayName}, ${identity.teamId}) has no owning lineage segment`);
            }
            if (segment && segment.displayName !== identity.displayName) {
                ownershipFailures.push(`${pool.franchiseId}/${pool.eraId}: ${player.displayName} identity "${identity.displayName}" does not match segment "${segment.displayName}"`);
            }
        }
    }
    ...
    details.push(`slots=${String(manifest.modernFranchiseSlots.length)} segments=${String(manifest.franchiseLineage.length)} pools=${String(manifest.pools.length)}`);
    details.push(`unavailable combinations: ${String(unavailableCombinations.length)}`);
    details.push(`logos: segments with candidates=${String(manifest.franchiseLineage.filter((s) => (s.logoCandidates?.length ?? 0) > 0).length)}/${String(manifest.franchiseLineage.length)}`);
