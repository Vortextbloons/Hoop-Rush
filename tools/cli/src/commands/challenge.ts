...
import { BEST_OF_ATTEMPTS, createEngineContext, simulateChallengeBestOf, toSimulationPlayer, type ChallengeCreation, } from '@hoop-rush/engine';
import { FIXED_SANDBOX_ERA, LINEUP_STRUCTURE, playerIdSchema, seedSchema, simulationTeamSchema, type ChallengeRun, type PeakPlayerSeason, type PlayersIndex, type PlayersIndexEntry, type SimulationPlayer, type SimulationTeam, } from '@hoop-rush/data-contracts';
...
function franchiseDisplayName(data: PackagedData, franchiseId: string): string {
    return (data.manifest.modernFranchiseSlots.find((franchise) => franchise.franchiseId === franchiseId)
...
function resolveFranchiseQualifier(data: PackagedData, value: string): string | null {
    ...
    for (const franchise of data.manifest.modernFranchiseSlots) {
        if (franchise.franchiseId === normalized ||
...
function nameMatches(row: PlayersIndexEntry, name: string): boolean {
    ...
    return (normalizeName(row.displayName) === normalized ||
...
export function bestRow(rows: PlayersIndexEntry[]): PlayersIndexEntry {
    const sorted = [...rows].sort((a, b) => {
        ...
        if (a.seasonKey !== b.seasonKey)
            return a.seasonKey < b.seasonKey ? -1 : 1;
        return 0;
    });
    const best = sorted[0];
    if (!best)
        throw new UsageError('--lineup: no matching players');
    return best;
}
const CANDIDATE_CAP = 12;
function resolveName(name: string, franchiseId: string | undefined, eraId: string | undefined, index: PlayersIndex, data: PackagedData): PoolRef {
    const rows = index.players.filter((row) => nameMatches(row, name) &&
    ...
    if (franchiseId === undefined && rows.length > 1) {
        const candidates = rows
            ...
            .map((row) => `${row.displayName}@${franchiseDisplayName(data, row.franchiseId)}/${row.eraId}`)
            .join(', ');
        const suffix = rows.length > CANDIDATE_CAP ? `, and ${String(rows.length - CANDIDATE_CAP)} more` : '';
        throw new UsageError(`--lineup: "${name}" matches multiple peaks; qualify it: ${candidates}${suffix}`);
    }
    if (franchiseId === undefined) {
        const row = rows[0];
        if (!row)
            throw new UsageError(`--lineup: no player named "${name}"`);
        return { playerId: row.playerId, franchiseId: row.franchiseId, eraId: row.eraId };
    }
    return bestRow(rows);
}
function resolveQualifiedId(playerId: string, qualifier: string, token: string, index: PlayersIndex): PoolRef {
    const slash = qualifier.indexOf('/');
    if (slash === -1 || qualifier.indexOf('/', slash + 1) !== -1) {
        throw new UsageError(`--lineup: "${token}" must be a playerId, playerId@franchiseId/eraId, a player name, or name@franchise/era`);
    }
    const franchiseId = qualifier.slice(0, slash);
    const eraId = qualifier.slice(slash + 1);
    const row = index.players.find((candidate) => candidate.playerId === playerId &&
...
function resolveQualifiedName(name: string, qualifier: string, token: string, index: PlayersIndex, data: PackagedData): PoolRef {
    const slash = qualifier.indexOf('/');
    if (slash === -1) {
        const franchiseId = resolveFranchiseQualifier(data, qualifier);
        if (!franchiseId)
            throw new UsageError(`--lineup: unknown franchise "${qualifier}"`);
    ...
    if (!franchiseId)
        throw new UsageError(`--lineup: unknown franchise "${franchisePart}"`);
    const era = data.manifest.eras.find((entry) => entry.eraId === normalizeName(eraPart));
    if (!era)
        throw new UsageError(`--lineup: unknown era "${eraPart}"`);
        ...
        if (playerIdSchema.safeParse(token).success) {
            ...
            if (matches.length > 1) {
                const candidates = matches
                    ...
                    .join(', ');
                throw new UsageError(`--lineup: player id "${token}" matches multiple peaks; qualify it: ${candidates}`);
            }
            const row = matches[0];
            if (!row)
                throw new UsageError(`--lineup: unknown player id "${token}"`);
...
export function resolvePoolLineup(spec: string, data: PackagedData): PeakPlayerSeason[] {
    ...
    return refs.map((ref) => {
        ...
        if (!player) {
            throw new UsageError(`--lineup: ${ref.playerId} is missing from the ${ref.franchiseId}/${ref.eraId} pool`);
        }
        return player;
    });
}
export function simChallenge(args: {
    ...
    if (lineupSpec === undefined) {
        throw new UsageError('sim challenge requires --lineup <playerId[,playerId@franchise/era] or Name[,Name@Franchise/era]>');
    }
    const rawSeed = args.seed;
    if (rawSeed === undefined)
        throw new UsageError('sim challenge requires --seed <hex>');
    ...
    if (reruns < 1)
        throw new UsageError('--reruns must be >= 1');
    ...
    const displayName = new Map<string, string>(run.players.map((player) => [player.playerId, player.displayName]));
    const nameWidth = Math.max(6, ...payload.playerTotals.map((p) => (displayName.get(p.playerId) ?? p.playerId).length));
    ...
    const shots = (made: number, attempted: number, width: number) => `${String(made)}/${String(attempted)}`.padStart(width);
    const avg = (value: number, games: number) => (games > 0 ? value / games : 0);
    const pct = (made: number, attempted: number) => attempted > 0 ? `${((made / attempted) * 100).toFixed(1)}%` : '-';
    const padPct = (made: number, attempted: number, width: number) => pct(made, attempted).padStart(width);
    const totals = payload.playerTotals.reduce((acc, p) => {
