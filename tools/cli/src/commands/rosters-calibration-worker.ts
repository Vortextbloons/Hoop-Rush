...
import { SeasonAiGenerationError, completionTargetsMet, generateAiLeague, validateSeasonRoster, } from '@hoop-rush/engine';
import { seasonDraftCatalogSchema, seasonLeagueSchema, seedSchema, type Seed, type SeasonDraftCatalog, type SeasonLeague, type SeasonRosterTargets, type SeasonStrengthBand, } from '@hoop-rush/data-contracts';
...
export interface RosterCalibrationWorkerRun {
    ...
    tierCounts: Record<SeasonStrengthBand, {
        elite: number;
        strong: number;
        useful: number;
        total: number;
...
function selectionFailuresOf(catalog: SeasonDraftCatalog, league: SeasonLeague, rosterIds: Map<string, string[]>, humanFranchiseIds: readonly string[]): string[] {
    ...
    for (const team of league.teams) {
        ...
        if (legality.length > 0)
            failures.push(`${team.franchiseId}: ${legality.join('; ')}`);
...
function rosterOf(generation: {
    ...
    for (const roster of generation.rosters) {
        rosterIds.set(roster.franchiseId, roster.players.map((player) => player.playerVersionId));
    }
    return rosterIds;
}
    ...
    return input.seeds.map((rawSeed) => {
        ...
        for (const pool of generation.aiPools) {
            ...
            for (const versionId of pool.playerVersionIds) {
                ...
                if (owner !== undefined) {
                    poolFailures.push(`exclusivity: ${versionId} appears in pools ${owner} and ${pool.franchiseId}`);
                }
                else {
...
function main(): void {
    ...
    const output: WorkerOutput = input.variant === 'order-invariance'
