...
import { POSITION_SLOTS, playableSlotGroups, RATINGS_VERSION, RATING_MODEL_VERSION, ratingsModelArtifactSchema, slotGroupOf, } from '@hoop-rush/data-contracts';
...
function pairImpact(candidate: SimulationTeam, replacement: SimulationTeam, benchmark: SimulationTeam, profile: EraSimulationProfile, samples: number, playerId: string, context: string): PairAccumulator {
    ...
    for (const pool of pools) {
        ...
        for (const player of pool.players) {
            ...
            const totals = CONTEXTS.map((context) => {
                ...
                return pairImpact(candidate, replacement, benchmark, data.eraProfile(pool.eraId), samples, player.playerId, context);
            });
