...
function teamMetric(result: ReturnType<typeof simulateGame>, side: 'home' | 'away') {
  ...
  return {
    ...
    shotQuality:
      (team.fieldGoals.made + 0.5 * team.threes.made) / Math.max(1, team.fieldGoals.attempted),
  };
}

function adjustBenchmark(team: SimulationTeam, context: (typeof CONTEXTS)[number]): SimulationTeam {
  if (context !== 'interior-heavy' && context !== 'perimeter-heavy') return team;
  return {
    ...team,
    ...
    const source =
      slotIndex === candidateSlot
        ...
        : pool.players.find((player) => {
            if (used.has(player.playerId) || player.playerId === candidateId) return false;
            const requirement = slot[0];
            return (
              requirement !== undefined &&
...
function replacementLineup(pool: FranchiseEraPool, candidateId: string): SimulationTeam | null {
  ...
  if (!candidateLineup) return null;
  const candidate = pool.players.find((player) => player.playerId === candidateId);
  if (!candidate) return null;
  const candidateSlot = candidateLineup.players.findIndex(
    (player) => player.playerId === candidateId,
  );
  ...
  const replacement = pool.players
    .filter(
      (player) =>
        player.playerId !== candidateId &&
        playableSlotGroups(player.positions.playable).includes(slotGroupOf(requirement)),
    )
    .sort(
      (a, b) =>
        Math.abs(a.selectionScore - candidate.selectionScore) -
  ...
  return {
    ...
    players: candidateLineup.players.map((player, index) => {
      ...
      if (!positions) throw new Error(`replacement slot ${String(index)} is invalid`);
...
function pairImpact(
  ...
  for (let index = 0; index < samples; index += 1) {
    ...
    const candidateResult = simulateGame(
      { ...base, home: homeFirst ? candidate : benchmark, away: homeFirst ? benchmark : candidate },
      engineContext,
    );
    ...
    if (candidateResult.winner === candidateSide) candidateAcc.wins += 1;
    if (replacementResult.winner === replacementSide) replacementAcc.wins += 1;
  ...
  return {
    ...
    netRating:
      candidateAcc.netRating / Math.max(1, samples) -
      replacementAcc.netRating / Math.max(1, samples),
    offensiveEfficiency:
      candidateAcc.offensiveEfficiency / Math.max(1, samples) -
      replacementAcc.offensiveEfficiency / Math.max(1, samples),
    defensiveEfficiency:
      replacementAcc.defensiveEfficiency / Math.max(1, samples) -
      candidateAcc.defensiveEfficiency / Math.max(1, samples),
    turnovers:
      candidateAcc.turnovers / Math.max(1, samples) -
      replacementAcc.turnovers / Math.max(1, samples),
    rebounds:
      candidateAcc.rebounds / Math.max(1, samples) - replacementAcc.rebounds / Math.max(1, samples),
    shotQuality:
      candidateAcc.shotQuality / Math.max(1, samples) -
...
export function calibrateRatings(args: {
  ...
  if (samples < 1)
    return makeReport(
      'calibrate ratings',
      { samples },
      { failures: ['--samples must be positive'] },
    );
  ...
  for (const pool of pools) {
    ...
    for (const player of pool.players) {
      ...
      const totals = CONTEXTS.map((context) => {
        const benchmark = adjustBenchmark(
      ...
      const adjustment = Math.max(
        ...
        Math.min(
          6,
          metrics.netRating * artifact.mapping.impactPerNetRating +
  ...
  return makeReport(
    'calibrate ratings',
    { samples, workers: args.workers ?? '1', output },
    {
      details: [
...
function leagueAverage(pool: FranchiseEraPool): SimulationTeam {
  ...
  return {
    ...
    players: SLOT_POSITIONS.map((positions, index) => {
      ...
      if (!source) throw new Error('cannot build a benchmark from an empty pool');
      return {
...
function poolStrength(pool: FranchiseEraPool): { strong: SimulationTeam; weak: SimulationTeam } {
  const sorted = [...pool.players].sort(
  ...
  const make = (source: typeof sorted): SimulationTeam => ({
    ...
    players: SLOT_POSITIONS.map((positions, index) => {
      ...
      if (!requiredPosition) throw new Error(`benchmark slot ${String(index)} is invalid`);
      const player =
        source.find((candidate) =>
          playableSlotGroups(candidate.positions.playable).includes(slotGroupOf(requiredPosition)),
        ) ??
        source[index] ??
        source.at(0);
