...
import { POSITION_SLOTS, PROJECTION_MODEL_VERSION, PROJECTION_TARGETS_VERSION, fnv1a32, parseProjectionModelArtifact, seedFromString, type BaseFiveProjectionInput, type EraSimulationProfile, type ProjectionMatchupArchetype, type ProjectionModelArtifact, type ProjectionReferenceFive, type ProjectionTargets, type SimulationPlayer, type SimulationTeam, } from '@hoop-rush/data-contracts';
...
function aggregateReferencePlayer(candidates: readonly SimulationPlayer[], slotIndex: number, label: string): SimulationPlayer {
    ...
    const ratings = Object.fromEntries(Object.keys(pool[0]?.ratings ?? {}).map((key) => [
        key,
    ...
    const tendencies = Object.fromEntries(Object.keys(pool[0]?.tendencies ?? {}).map((key) => [
        key,
