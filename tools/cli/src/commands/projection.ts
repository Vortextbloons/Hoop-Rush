...
import { z } from 'zod';
import { POSITION_SLOTS, PROJECTION_MODEL_VERSION, PROJECTION_TARGETS_VERSION, contentHashSchema, eraIdSchema, fnv1a32, franchiseIdSchema, parseProjectionModelArtifact, playerIdSchema, seasonRotationSchema, seedFromString, seedSchema, simulationPlayerSchema, simulationRatingsSchema, simulationTeamSchema, simulationTendenciesSchema, type BaseFiveProjectionInput, type EraSimulationProfile, type ProjectionMatchupArchetype, type ProjectionModelArtifact, type ProjectionReferenceFive, type ProjectionTargets, type SimulationPlayer, type SimulationTeam, } from '@hoop-rush/data-contracts';
import { createEngineContext, generateAiLeague, projectBaseFive, projectSeasonRoster, simulateGame, toSimulationPlayer, type EngineContext, } from '@hoop-rush/engine';
...
import { fixtureHumanRoster, loadSeasonDraftCatalog, loadSeasonLeague, loadSeasonRosterTargets, } from './season-data.ts';
...
function loadModel(manifestPath: string | null | undefined, modelPath: string | null | undefined): ProjectionModelArtifact {
...
function archetypeFilter(archetype: ProjectionMatchupArchetype): {
    ...
    switch (archetype) {
        ...
        case 'perimeter':
            return Object.assign((p: SimulationPlayer) => p.ratings.threePoint >= 55 || p.tendencies.threePointRate >= 25, { kind: 'trait' as const });
        case 'interior':
            return Object.assign((p: SimulationPlayer) => p.ratings.insideScoring >= 65 || p.tendencies.rimFrequency >= 45, { kind: 'trait' as const });
        case 'pressure':
            return Object.assign((p: SimulationPlayer) => p.ratings.perimeterDefense >= 60 || p.ratings.steal >= 60, { kind: 'trait' as const });
        case 'size-switch':
            return Object.assign((p: SimulationPlayer) => (p.heightInches ?? 0) >= 79 &&
                (p.ratings.interiorDefense >= 55 || p.ratings.defensiveIq >= 60), { kind: 'trait' as const });
    }
}
const meanOf = mean;
function aggregateReferencePlayer(candidates: readonly SimulationPlayer[], slotIndex: number, label: string): SimulationPlayer {
    ...
    const ratingsRaw: Record<string, number> = Object.fromEntries(Object.keys(pool[0]?.ratings ?? {}).map((key) => [
        key,
        Math.round(Math.min(100, Math.max(0, meanOf(pool.map((p) => p.ratings[key as keyof SimulationPlayer['ratings']]))))),
    ]));
    const ratingsParsed = simulationRatingsSchema.safeParse(ratingsRaw);
    if (!ratingsParsed.success) {
        throw new Error(`projection: aggregated ratings failed validation: ${ratingsParsed.error.issues[0]?.message ?? 'invalid'}`);
    }
    const ratings: Pick<SimulationPlayer, 'ratings'>['ratings'] = ratingsParsed.data;
    const tendenciesRaw: Record<string, number> = Object.fromEntries(Object.keys(pool[0]?.tendencies ?? {}).map((key) => [
        key,
        Math.round(Math.min(100, Math.max(0, meanOf(pool.map((p) => p.tendencies[key as keyof SimulationPlayer['tendencies']]))))),
    ]));
    const tendenciesParsed = simulationTendenciesSchema.safeParse(tendenciesRaw);
    if (!tendenciesParsed.success) {
        throw new Error(`projection: aggregated tendencies failed validation: ${tendenciesParsed.error.issues[0]?.message ?? 'invalid'}`);
    }
    ...
    const anchors: SimulationPlayer['anchors'] = anchored.length > 0
        ? {
            gamesPlayed: Math.round(meanOf(anchored.map((p) => p.anchors?.gamesPlayed ?? 0))),
            minutesPerGame: Math.round(meanOf(anchored.map((p) => p.anchors?.minutesPerGame ?? 0)) * 10) / 10,
            pointsPerGame: Math.round(meanOf(anchored.map((p) => p.anchors?.pointsPerGame ?? 0)) * 10) / 10,
            reboundsPerGame: Math.round(meanOf(anchored.map((p) => p.anchors?.reboundsPerGame ?? 0)) * 10) / 10,
            offensiveReboundsPerGame: Math.round(meanOf(anchored.map((p) => p.anchors?.offensiveReboundsPerGame ?? 0)) * 10) /
                10,
            defensiveReboundsPerGame: Math.round(meanOf(anchored.map((p) => p.anchors?.defensiveReboundsPerGame ?? 0)) * 10) /
                10,
            assistsPerGame: Math.round(meanOf(anchored.map((p) => p.anchors?.assistsPerGame ?? 0)) * 10) / 10,
            stealsPerGame: Math.round(meanOf(anchored.map((p) => p.anchors?.stealsPerGame ?? 0)) * 10) / 10,
            blocksPerGame: Math.round(meanOf(anchored.map((p) => p.anchors?.blocksPerGame ?? 0)) * 10) / 10,
            turnoversPerGame: Math.round(meanOf(anchored.map((p) => p.anchors?.turnoversPerGame ?? 0)) * 10) / 10,
            fieldGoalPct: Math.round(meanOf(anchored.map((p) => p.anchors?.fieldGoalPct ?? 0)) * 1000) / 1000,
            threePointPct: null,
            freeThrowPct: Math.round(meanOf(anchored.map((p) => p.anchors?.freeThrowPct ?? 0)) * 1000) / 1000,
            threePointAttemptRate: null,
            freeThrowAttemptRate: Math.round(meanOf(anchored.map((p) => p.anchors?.freeThrowAttemptRate ?? 0)) * 1000) /
                1000,
        }
        : undefined;
    ...
    const reconstructedThreePoint: SimulationPlayer['reconstructedThreePoint'] = reconstructed.length >= Math.max(1, pool.length / 2) && firstReconstructed !== undefined
        ? {
            modelVersion: firstReconstructed.modelVersion,
            accuracyConservative: Math.round(meanOf(reconstructed.map((p) => p.reconstructedThreePoint?.accuracyConservative ?? 0)) * 1000) / 1000,
            accuracyMean: Math.round(meanOf(reconstructed.map((p) => p.reconstructedThreePoint?.accuracyMean ?? 0)) * 1000) / 1000,
            accuracyStdDev: Math.round(meanOf(reconstructed.map((p) => p.reconstructedThreePoint?.accuracyStdDev ?? 0)) *
                1000) / 1000,
            attemptRateConservative: Math.round(meanOf(reconstructed.map((p) => p.reconstructedThreePoint?.attemptRateConservative ?? 0)) * 1000) / 1000,
            attemptRateMean: Math.round(meanOf(reconstructed.map((p) => p.reconstructedThreePoint?.attemptRateMean ?? 0)) *
                1000) / 1000,
            attemptRateStdDev: Math.round(meanOf(reconstructed.map((p) => p.reconstructedThreePoint?.attemptRateStdDev ?? 0)) *
                1000) / 1000,
            confidence: 'medium',
            floor: Math.round(meanOf(reconstructed.map((p) => p.reconstructedThreePoint?.floor ?? 0)) * 1000) / 1000,
            zoneFloors: {
                cornerThree: Math.round(meanOf(reconstructed.map((p) => p.reconstructedThreePoint?.zoneFloors.cornerThree ?? 0)) * 1000) / 1000,
                aboveBreakThree: Math.round(meanOf(reconstructed.map((p) => p.reconstructedThreePoint?.zoneFloors.aboveBreakThree ?? 0)) * 1000) / 1000,
            },
            evidence: {
                missingFeatures: 0,
...
function deriveReferenceFive(players: readonly SimulationPlayer[], eraId: string, archetype: ProjectionMatchupArchetype): ProjectionReferenceFive {
    ...
    const five = [0, 1, 2, 3, 4].map((slotIndex) => aggregateReferencePlayer(population, slotIndex, labels[slotIndex] ?? 'Reference')) as [
        SimulationPlayer,
        SimulationPlayer,
        SimulationPlayer,
        SimulationPlayer,
...
export function projectionAiShadow(input: {
    ...
    try {
        ...
        return makeReport('projection ai-shadow', { manifest: manifestPath }, { failures: [(error as Error).message] });
    }
    ...
    try {
        ...
        return makeReport('projection ai-shadow', { seed: runSeed, era: eraId }, { failures: [(error as Error).message] });
    }
    const summaries = generation.evaluations.filter((evaluation) => evaluation.projectionSummary !== undefined);
    const different = summaries.filter((evaluation) => evaluation.projectionSummary?.selectedIsBest === false);
    const gaps = summaries
        .map((evaluation) => {
        const summary = evaluation.projectionSummary;
        if (summary === undefined || summary.bestNetRating === null)
            return null;
    ...
    return makeReport('projection ai-shadow', { seed: runSeed, era: eraId }, {
        details,
        payload: {
...
export function projectionSeason(input: {
    ...
    if (!fixtureParsed.success) {
        return makeReport('projection season', { fixture }, { failures: [`fixture ${fixture} is invalid`] });
    }
    ...
    if (roster === undefined || rotation === undefined) {
        return makeReport('projection season', { fixture }, { failures: [`fixture ${fixture} must carry a roster and a rotation`] });
    }
    ...
    try {
        ...
        return makeReport('projection season', { fixture, era: eraId }, { failures: [(error as Error).message] });
    }
    const details = [
    ...
    if (verbose) {
        for (const row of projection.minutes) {
            details.push(`  ${row.playerVersionId}: target ${String(row.targetMinutes)} / trace ${String(row.traceMinutes)} min (dev ${String(row.deviation)})`);
        }
    }
...
export function projectionBuild(input: {
    ...
    const details = [
        ...
        ...Object.entries(populationSizes).map(([eraId, size]) => `  ${eraId}: ${String(size)} pooled players`),
        ...Object.entries(model.references).map(([eraId, set]) => `  ${eraId}: neutral=${set.neutral.referenceHash.slice(0, 12)} archetypes=${set.archetypes
    ...
    if (write) {
        ...
        if (!manifestParsed.success) {
            return makeReport('projection build', { manifest: manifestPath ?? 'default', write }, { details, failures: ['manifest is invalid'] });
        }
        const manifest = manifestParsed.data;
        const existingProjection = typeof manifest.projection === 'object' && manifest.projection !== null
            ? manifest.projection
            : {};
        manifest.projection = {
            ...existingProjection,
    ...
    return makeReport('projection build', { manifest: manifestPath ?? 'default', write }, { details });
}
export function projectionBase(input: {
    ...
    if (referenceSet === undefined) {
        return makeReport('projection base', { era: eraId }, { failures: [`no reference set for era ${eraId} in the model artifact`] });
    }
    ...
    if (fixture !== null && fixture !== undefined) {
        ...
        if (!fixtureParsed.success) {
            return makeReport('projection base', { fixture }, { failures: [`fixture ${fixture} is invalid`] });
        }
        const fixtureValue = fixtureParsed.data;
        const candidate = fixtureValue.home ?? fixtureValue.lineup ?? fixtureValue.away;
        if (candidate === undefined) {
            return makeReport('projection base', { fixture }, { failures: [`fixture ${fixture} has no home/away/lineup team`] });
        }
        team = candidate;
    }
    else {
        return makeReport('projection base', { era: eraId }, { failures: ['--fixture is required (lineup projection source)'] });
    }
    ...
    if (verbose) {
        for (const player of projection.offense.players) {
            details.push(`  ${player.slot} ${player.displayName}: ${player.expectedShots.toFixed(1)} FGA / ${player.expectedPoints.toFixed(1)} pts / ${player.expectedAssists.toFixed(1)} ast / ${player.expectedTurnovers.toFixed(1)} tov`);
        }
    }
...
export function buildLineupCohort(data: PackagedData, eraId: string, count: number, seedNamespace: string): SimulationTeam[] {
    ...
    const guardIndices = players
        ...
        .filter((entry) => entry.player.positions.includes('PG') || entry.player.positions.includes('SG'))
        ...
        for (let offset = 1; offset < players.length && legal.size < count * 8; offset += 1) {
            ...
            for (const position of neededPositions) {
                ...
                for (let attempt = 0; attempt < players.length && found === undefined; attempt += 1) {
                    ...
                    if (candidate.positions.includes(needed))
                        found = candidate;
                }
                if (found !== undefined)
                    five.push(found);
...
function seededOrder(length: number, namespace: string): number[] {
    ...
    const offsets = order.map((value) => fnv1a32(`${namespace}#${String(value)}`) % Math.max(1, order.length));
    return order
...
export function projectionCalibrateBase(input: {
    ...
    const referenceSet = model.references[eraId];
    if (referenceSet === undefined) {
        return makeReport('projection calibrate-base', { era: eraId }, { failures: [`no reference set for era ${eraId} in the model artifact`] });
    }
    ...
    if (lineups.length === 0) {
        return makeReport('projection calibrate-base', { era: eraId }, { failures: ['could not build a legal lineup cohort from the era pools'] });
    }
    const context = createEngineContext();
    const rows = lineups.map((lineup, index) => projectSimulateLineup({
        lineup,
    ...
    if (!validate) {
        ...
        for (const [label, passed] of gateChecks) {
            if (!passed && enforceGates)
                failures.push(`${label} gate failed`);
        }
        ...
        return makeReport('projection calibrate-base', { era: eraId, samples: gamesPerLineup }, { details, failures, payload: stats });
    }
    const passRate = stats.netRatingMae <= targets.gates.netRatingMaeMax ? 1 : 0;
    details.push(`held-out pass: ${passRate === 1 ? 'PASS' : 'FAIL'}`);
    return makeReport('projection validate', { era: eraId, samples: gamesPerLineup }, { details, payload: stats });
}
function summarize(rows: Array<{
    ...
    const mae = (a: number[], b: number[]) => a.reduce((sum, value, index) => sum + Math.abs(value - (b[index] ?? 0)), 0) /
        Math.max(1, a.length);
...
export function projectionBenchmark(input: {
    ...
    const referenceSet = model.references[eraId];
    if (referenceSet === undefined) {
        return makeReport('projection benchmark', { era: eraId }, { failures: [`no reference set for era ${eraId} in the model artifact`] });
    }
    const lineups = buildLineupCohort(data, eraId, Math.max(8, Math.min(64, samples)), 'projection-benchmark/lineups');
    if (lineups.length === 0) {
        return makeReport('projection benchmark', { era: eraId }, { failures: ['no legal lineups to benchmark'] });
    }
    ...
    return makeReport('projection benchmark', { era: eraId, samples: timings.length }, { details, failures, payload: { median, p95, min, max } });
}
