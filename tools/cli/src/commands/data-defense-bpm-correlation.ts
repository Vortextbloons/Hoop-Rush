...
export function defenseBpmCorrelation(options: DefenseBpmCorrelationOptions): CliReport {
    ...
    if (!parsedManifest.success) {
        ...
        return makeReport('data defense-bpm-correlation', { input: options.input }, {
            failures: [
    ...
    const details = [
        ...
        ...eraCorrelations.map((era) => `${era.eraId}: r=${era.correlation === null ? '—' : String(era.correlation)} (n=${String(era.sample)})`),
    ];
    return makeReport('data defense-bpm-correlation', { input: options.input, dataVersion: manifest.dataVersion }, {
        details,
        failures,
