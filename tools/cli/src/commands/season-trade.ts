...
export function seasonTradeAudit(args: {
    ...
    try {
        ...
        if (!parsed.success) {
            return makeReport('season trade audit', { input: inputPath }, {
                failures: [`run fails schema: ${parsed.error.issues[0]?.message ?? 'unknown'}`],
                exitCode: 2,
            });
        }
        ...
        const dummyFacts: SeasonM25SeasonFacts = {
            ...
            checkpoints: [],
            postBlock: [],
            windows: (run.trade?.windows ?? []).map((w) => ({
                blockIndex: w.blockIndex,
                result: {
                    trade: { windows: [w] },
                } as unknown as SeasonM25SeasonFacts['windows'][number]['result'],
            })),
            ...
            effects: {
                ...
                playerStates: [],
                pairStates: [],
            } as unknown as SeasonM25SeasonFacts['effects'],
    ...
    for (const window of season.windows) {
        ...
        for (const offer of opened.offers) {
            ...
            if (maybeProposal.influenceAmount !== undefined) {
                if (maybeProposal.influenceAmount > 0 &&
                    maybeProposal.outgoingPlayerVersionIds.length === 0 &&
                    maybeProposal.incomingPlayerVersionIds.length === 0) {
                    failures += 1;
                }
...
export function evaluateTradeGates(args: {
    ...
    const cashPerProposalFailures = c.reduce((sum, s) => sum + influenceOnlyFailuresOf(s), 0) * 0 +
