...
import { SEASON_AI_VERSION, SEASON_ROSTER_GENERATION_VERSION, SEASON_ROTATION_VERSION, seasonDraftStateSchema, seasonLeagueGenerationResultSchema, seasonRosterTargetsSchema, seasonRunSchema, seedSchema, type SeasonDraftState, type SeasonLeagueGenerationResult, type SeasonRosterTargets, type SeasonStrengthBand, type Seed, } from '@hoop-rush/data-contracts';
...
export function seasonRostersGenerate(args: {
    ...
    if (!parsedSeed.success) {
        return makeReport('season rosters generate', { seed: rawSeed, draft: args.draft }, { failures: [`--seed must be a hex seed (got "${rawSeed}")`], exitCode: 2 });
    }
