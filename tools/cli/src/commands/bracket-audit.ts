...
import { opponentBracketSchema, opponentTeamSchema, type HoopRushManifest, type OpponentBracket, } from '@hoop-rush/data-contracts';
...
export function bracketAudit(inputPath: string, verbose: boolean, previewPath = resolve(dirname(inputPath), 'opponents/lakers-1990s-opening.json')): CliReport {
    let manifest: HoopRushManifest;
    try {
        manifest = readJson(inputPath) as HoopRushManifest;
    }
    catch (error) {
        return makeReport('bracket audit', { input: inputPath }, { failures: [(error as Error).message], exitCode: EXIT_USAGE_OR_DATA_ERROR });
    }
    const entry = manifest.bracket;
    if (!entry) {
        return makeReport('bracket audit', { input: inputPath }, { failures: ['manifest has no bracket reference'], exitCode: EXIT_USAGE_OR_DATA_ERROR });
    }
    ...
    try {
        ...
        return makeReport('bracket audit', { input: inputPath }, {
    ...
    if (minP < band[0] || maxP > band[1]) {
        failures.push(`generated strength percentiles span ${minP.toFixed(3)}..${maxP.toFixed(3)} outside band ${band[0].toFixed(2)}..${band[1].toFixed(2)}`);
    }
    if (bracketMedian < medianBand[0] || bracketMedian > medianBand[1]) {
        failures.push(`league median percentile ${bracketMedian.toFixed(3)} outside ${medianBand[0].toFixed(2)}..${medianBand[1].toFixed(2)}`);
    }
    if (openingPercentile < band[0] || openingPercentile > band[1]) {
        details.push(`opening opponent percentile ${openingPercentile.toFixed(3)} outside the band (authored fixed entry, informational)`);
    }
    ...
    details.push(`bracket ${bracket.bracketVersion} · schedule ${bracket.scheduleVersion} · generation ${bracket.generation.generationVersion} (seed ${bracket.generation.seed})`, `opponents: ${String(bracket.opponents.length)} · percentile span ${minP.toFixed(3)}..${maxP.toFixed(3)} · median ${bracketMedian.toFixed(3)}`, `difficulty ${difficulty.profileVersion} · bands team ${band[0].toFixed(2)}..${band[1].toFixed(2)} median ${medianBand[0].toFixed(2)}..${medianBand[1].toFixed(2)}`);
    if (verbose) {
        for (const opponent of bracket.opponents) {
            details.push(`  ${opponent.opponentId}: ${String(opponent.strength.sampleCount)} games · winRate ${opponent.strength.winRate.toFixed(3)} · pct ${opponent.strength.percentile.toFixed(3)}`);
        }
    }
    return makeReport('bracket audit', { input: inputPath, dataVersion: manifest.dataVersion }, { details, failures, payload });
}
