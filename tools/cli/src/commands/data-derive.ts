...
import { DERIVATION_METHOD_VERSION, LINEAGE_RULE_VERSION, POSITION_NORMALIZATION_VERSION, RATINGS_VERSION, SELECTION_SCORE_VERSION, SOURCE_VERSION, } from '@hoop-rush/data-contracts';
...
export function dataDerive(args: {
    ...
    if (!player || !season || !franchise) {
        return makeReport('data derive', { player, season, franchise }, {
    ...
    try {
        stints = JSON.parse(readFileSync(join(seasonDir, 'stints.json'), 'utf8')) as Array<Record<string, unknown>>;
        statsRows = JSON.parse(readFileSync(join(seasonDir, 'season-stats.json'), 'utf8')) as Array<Record<string, unknown>>;
    }
    catch (error) {
        return makeReport('data derive', { player, season, franchise }, {
    ...
    const rosterRows = JSON.parse(readFileSync(join(seasonDir, 'roster.json'), 'utf8')) as Array<Record<string, unknown>>;
    const roster = rosterRows.find((row) => String(row.externalId) === player);
    const stint = stints.find((row) => String(row.playerExternalId) === player &&
