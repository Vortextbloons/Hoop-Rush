...
describe('cli: calibrate three-point', () => {
    it('fits the cohort and emits a validated payload with all gates passing', async () => {
    ...
    it('writes the artifact with --write and renders the default text report', async () => {
        await withTmpDir(async (tmp) => {
            ...
            const artifact = threePointReconstructionArtifactSchema.parse(JSON.parse(readFileSync(artifactPath, 'utf8')));
