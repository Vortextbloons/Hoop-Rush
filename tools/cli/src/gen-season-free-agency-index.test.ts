...
import { seasonFreeAgencyIndexSchema, type SeasonDraftCatalog, type SeasonRosterRole, } from '@hoop-rush/data-contracts';
import { evaluateSeasonRoster, playerPercentileTier, percentileTierOf, rolePercentileThresholds, type RoleThresholds, } from '@hoop-rush/engine';
import { DEFAULT_DRAFT_CATALOG, DEFAULT_FREE_AGENCY_INDEX, DEFAULT_MANIFEST, loadSeasonDraftCatalog, } from './commands/season-data.ts';
import { deriveFreeAgencyIndex, FREE_AGENCY_INDEX_MAX_BYTES, freeAgencyIndexContent, type FreeAgencyIndexStats, freeAgencyIndexStats, } from './gen-season-free-agency-index.ts';
import { sha256Hex } from './io.ts';
function roleScoresOf(candidate: SeasonDraftCatalog['candidates'][number]): Record<SeasonRosterRole, number> {
...
describe('free-agency index derivation (committed artifacts)', () => {
    it('regenerates the committed artifact byte-for-byte and validates the schema', () => {
        ...
        expect(seasonFreeAgencyIndexSchema.safeParse(JSON.parse(content) as unknown).success).toBe(true);
    ...
    it('pins the manifest hashes: artifact hash and catalog content hash', () => {
        ...
        expect(manifest.season?.draftCatalog?.contentHash).toBe(sha256Hex(readFileSync(DEFAULT_DRAFT_CATALOG)));
...
describe('free-agency index band sanity', () => {
    ...
    it('cites every excluded sibling in the survivors exclusion evidence', () => {
        ...
        for (const candidate of catalog.candidates) {
            if (eligible.has(candidate.playerVersionId))
                continue;
            const tier = playerPercentileTier(percentileTierOf(roleScoresOf(candidate), thresholds));
        ...
        for (const [playerId, excluded] of excludedByIdentity) {
            if ((indexedByIdentity.get(playerId) ?? 0) === 0)
                continue;
            ...
            const more = Number.parseInt((evidence.match(/\+(\d+) more siblings excluded/) ?? [])[1] ?? '0', 10);
    ...
    it('keeps groupedVersions canonical and identity-consistent', () => {
        ...
        expect(Object.keys(index.groupedVersions).length).toBe(new Set(index.candidates.map((entry) => entry.playerId)).size);
    });
    it('stays inside the compactness gate and reports the measured size', () => {
        const { content } = fixture;
        const stats: FreeAgencyIndexStats = freeAgencyIndexStats(seasonFreeAgencyIndexSchema.parse(JSON.parse(content) as unknown), content.length);
