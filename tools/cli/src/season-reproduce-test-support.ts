...
import { SEASON_EMPTY_COMMAND_LOG_DIGEST, buildSeasonRunReplayExport, commandIdSchema, contentHashSchema, eraIdSchema, franchiseIdSchema, humanFranchiseIdOf, seasonCommandLogDigest, seasonCommandLogEntrySchema, seasonCommandLogSchema, seasonCommandResultDigest, type SeasonAlmanac, type SeasonCommandLog, type SeasonCommandLogEntry, type SeasonFreeAgencyState, type SeasonRunReplayExport, type SeasonRunReplayExportInput, } from '@hoop-rush/data-contracts';
import { createSeasonEffectsState, expandSeasonRunRosters, handleSeasonRunCommand, openSeasonFreeAgencyWindow, openSeasonTradeWindow, seasonObjectiveChoicesForBlock, } from '@hoop-rush/engine';
import { REPO_ROOT } from './cli-test-helpers.ts';
import { loadSeasonRunFixture } from './commands/season-block.ts';
import { loadSeasonDraftCatalog, loadSeasonFreeAgencyIndex, loadSeasonRosterTargets, } from './commands/season-data.ts';
...
export function loadManifestHashes(): SeasonRunReplayExportInput['assetHashes'] {
    ...
    const eraProfile = manifest.eraSimulationProfiles?.find((entry) => entry.eraId === '1990s')?.contentHash ?? '';
    const parseHash = (value: string): import('@hoop-rush/data-contracts').ContentHash => contentHashSchema.parse(value);
    return {
...
export function buildReplayedRun(): ReplayedRun {
    ...
    if (humanFranchiseId === null)
        throw new Error('fixture has no human franchise');
    ...
    const secondOpen = offers.find((offer, index) => offer.status === 'open' && index > offers.indexOf(firstOpen as never));
    if (firstOpen === undefined)
        throw new Error('no open offers to accept');
    const entries: SeasonCommandLogEntry[] = [];
    const objectiveId = seasonObjectiveChoicesForBlock(run.rootSeed, 0)[0];
    if (objectiveId === undefined)
        throw new Error('no objective offered for block 0');
    const record = (command: Parameters<typeof handleSeasonRunCommand>[0], runFor: typeof run, currentEffects: typeof effects): {
        run: typeof run;
        ...
        entries.push(seasonCommandLogEntrySchema.parse({
            runId: run.runId,
...
export function buildFreeAgencyReplayedRun(mutateInitialFreeAgency?: (freeAgency: SeasonFreeAgencyState) => void): ReplayedRun {
    const run = loadSeasonRunFixture(SEASON_RUN);
    const catalog = loadSeasonDraftCatalog(MANIFEST);
    ...
    const initialEffects = createSeasonEffectsState(staminaInputs);
    const opened = openSeasonFreeAgencyWindow({ run, effects: initialEffects, catalog, index, targets, humanFranchiseId }, 0, 2);
    ...
    const entries: SeasonCommandLogEntry[] = [];
    const record = (command: Parameters<typeof handleSeasonRunCommand>[0], runFor: typeof run, currentEffects: typeof initialEffects): {
        run: typeof run;
        ...
        if (output.result.result.status === 'rejected') {
            throw new Error(`free-agency replay fixture command rejected: ${JSON.stringify(output.result.result)}`);
        }
        const accepted = output.result.result;
        const gameIds = 'advancedGameIds' in accepted ? accepted.advancedGameIds : [];
        const summaryDigests = (output.postseasonSummaries ?? []).map((summary) => summary.resultDigest);
    ...
    const resolveOutput = record({
        ...
        windowIndex: 0,
    }, skipOutput.run, skipOutput.effects);
    const commandLog = chainLog(entries);
