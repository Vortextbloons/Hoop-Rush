...
import { POSITION_NORMALIZATION_VERSION, RATINGS_VERSION, SEASON_DRAFT_CATALOG_VERSION, SEASON_DURABILITY_VERSION, SEASON_STAMINA_VERSION, eraIdSchema, franchiseIdSchema, playerIdSchema, playerVersionId, seasonDraftCatalogSchema, seasonKeySchema, type SeasonDraftCandidate, type SeasonDraftCandidateDurability, } from '@hoop-rush/data-contracts';
...
export function durabilityRatingFrom(gamesPlayed: number | null | undefined, teamGames: number | null | undefined): number {
    if (gamesPlayed === null ||
...
export function durabilityProfileOf(gamesPlayed: number | null | undefined, teamGames: number | null | undefined): SeasonDraftCandidateDurability {
    return {
...
function main(): void {
    ...
    for (const entry of manifest.pools) {
        ...
        for (const player of pool.players) {
            const versionId = playerVersionId(player.playerId, player.franchiseId, player.eraId, player.seasonKey);
            ...
            const record: SeasonDraftCandidate = {
                ...
                durability: durabilityProfileOf(player.stats?.gamesPlayed ?? null, player.eligibility?.teamGames ?? null),
                anchors: player.anchors,
                reconstructedThreePoint: player.reconstructedThreePoint,
            };
            if (existing !== undefined) {
                if (JSON.stringify(existing) !== JSON.stringify(record)) {
                    throw new Error(`conflicting records derive the same playerVersionId ${versionId} (${player.playerId} in ${player.franchiseId}/${player.eraId} ${player.seasonKey})`);
                }
                continue;
            }
    ...
    const catalog = {
        ...
        pools,
        candidates: [...candidates.values()].sort((a, b) => a.playerVersionId < b.playerVersionId ? -1 : 1),
    };
    const parsed = seasonDraftCatalogSchema.safeParse(catalog);
    if (!parsed.success) {
        throw new Error(`derived catalog fails the schema: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
    }
