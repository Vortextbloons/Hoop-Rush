...
function fitsSlot(entry: PlayersIndexEntry, group: 'G' | 'F' | 'C'): boolean {
    ...
    if (group === 'G')
        return union.some((p) => G_POSITIONS.has(p));
    if (group === 'F')
        return union.some((p) => F_POSITIONS.has(p));
    return union.some((p) => C_POSITIONS.has(p));
}
let memoizedIndex: {
    data: PackagedData;
    entries: PlayersIndexEntry[];
} | null = null;
function indexData(): {
    data: PackagedData;
    entries: PlayersIndexEntry[];
...
function legalUniqueLineup(): PlayersIndexEntry[] {
    if (memoizedLegalLineup !== null)
        return memoizedLegalLineup;
    ...
    const pick = (group: 'G' | 'F' | 'C', exclude: Set<string>): PlayersIndexEntry => {
        const entry = unique.find((candidate) => !exclude.has(candidate.playerId) && fitsSlot(candidate, group));
        if (!entry)
            throw new Error('test helper could not assemble a legal packaged lineup');
...
function legalUniqueNames(): PlayersIndexEntry[] {
    if (memoizedLegalNames !== null)
        return memoizedLegalNames;
    const { entries } = indexData();
    ...
    const legal = legalUniqueLineup().filter((entry) => counts.get(normalize(entry.displayName)) === 1 &&
        !entry.displayName.includes(',') &&
        !entry.displayName.includes('@'));
    if (legal.length < 5)
        throw new Error('test helper could not assemble a name-unique lineup');
...
function franchiseDisplayName(data: PackagedData, franchiseId: string): string {
    ...
    if (!slot)
        throw new Error(`test helper: no franchise slot for ${franchiseId}`);
    return slot.displayName;
}
let memoizedAmbiguousName: string | null = null;
function commonAmbiguousName(): string {
    if (memoizedAmbiguousName !== null)
    ...
    const sorted: Array<[
        string,
        string[]
    ]> = [...counts.entries()].sort((a, b) => b[1].length - a[1].length);
    const first = sorted[0];
    if (!first)
        throw new Error('test helper: no ambiguous name found');
...
describe('cli: sim challenge', () => {
    ...
    it('resolves player names, with and without franchise/era qualification', async () => {
        ...
        expect(simChallengeReportSchema.parse(jsonPayload(qualified.stdout)).playerTotals).toHaveLength(5);
        ...
        expect(simChallengeReportSchema.parse(jsonPayload(franchiseOnly.stdout)).playerTotals).toHaveLength(5);
    });
