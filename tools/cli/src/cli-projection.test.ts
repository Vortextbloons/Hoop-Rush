...
function payloadOf(stdout: string): unknown {
    ...
    if (start < 0)
        throw new Error(`no JSON payload in output: ${stdout.slice(0, 400)}`);
...
describe('projection season', () => {
    function buildSeasonFixture(): void {
        ...
        const canPlay = (player: SimulationPlayer, group: 'G' | 'F' | 'C') => group === 'G'
        ...
        const rotation = buildSeasonRotation('lakers', [...starters, ...bench].map((player) => player.playerVersionId ?? ''));
        writeFileSync(join(TMP, 'season-project.json'), JSON.stringify({ roster: [...starters, ...bench], rotation }));
    }
...
describe('projection benchmark', () => {
    it('reports base-projection timings within the release gates', async () => {
        ...
        if (code === 0) {
            expect(payload.failures ?? []).toEqual([]);
            expect(payload.details?.some((line) => line.includes('release gates: base median < 40 ms / p95 < 200 ms: PASS'))).toBe(true);
        }
    });
});
