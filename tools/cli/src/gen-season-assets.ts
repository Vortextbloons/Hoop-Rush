...
function playCommittedDraft(catalog: ReturnType<typeof seasonDraftCatalogSchema.parse>, league: SeasonLeague, targets: ReturnType<typeof seasonRosterTargetsSchema.parse>): {
    ...
    const apply = (command: SeasonDraftCommand): SeasonDraftState => {
        ...
        if (result.record.status !== 'accepted' || result.state === null) {
            const message = result.record.status === 'rejected' ? result.record.message : 'no state produced';
    ...
    state = apply(cmd('fixture-create', 0, {
    ...
    while (state.status === 'drafting' && state.currentTurnParticipantId !== null) {
        ...
        state = apply(cmd(`fixture-draw-${String(sequence)}`, state.revision, {
            kind: 'draw-season-offer',
            participantId: pid,
        }));
        const best = pickBestSelectable(state, catalog);
        state = apply(cmd(`fixture-pick-${String(sequence)}`, state.revision, {
...
function buildRun(schedule: SeasonSchedule, league: SeasonLeague, draft: SeasonDraftState, generation: SeasonLeagueGenerationResult, catalog: ReturnType<typeof seasonDraftCatalogSchema.parse>): SeasonRun {
...
function main(): void {
    ...
    if (!existsSync(schedulePath)) {
        console.log(`SKIP fixture regeneration: ${schedulePath} missing (run season schedule generate first)`);
        return;
    }
    ...
    const catalog = seasonDraftCatalogSchema.parse(readJson(resolve(SEASON_DIR, 'draft-catalog.json')));
    const targets = seasonRosterTargetsSchema.parse(readJson(resolve(SEASON_DIR, 'roster-targets.json')));
    ...
    writeFileSync(resolve(FIXTURES_DIR, 'season-draft-commands.json'), `${JSON.stringify(reproduceFixture, null, 2)}\n`);
    console.log(`wrote ${resolve(FIXTURES_DIR, 'season-draft-commands.json')} (${String(commands.length)} commands, digest ${finalDigest})`);
