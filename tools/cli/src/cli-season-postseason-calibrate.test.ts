...
import {
  foldSeasonPostseasonCohortFacts,
  seasonPostseasonCalibrate,
  seasonPostseasonGates,
  seasonPostseasonTargetsSchema,
...
describe('season postseason calibrate fold and gates (postseason-targets-v1)', () => {
  ...
  it('passes every gate for a healthy cohort and fails each violation class', () => {
    ...
    const championGate = seasonPostseasonGates(noChampion).find(
      (gate) => gate.key === 'championPerSeason',
    );
    ...
    const duplicateGate = seasonPostseasonGates(duplicateTeams).find(
      (gate) => gate.key === 'zeroDuplicateTeams',
    );
    ...
    expect(
      seasonPostseasonGates(missingTeams).find((g) => g.key === 'zeroMissingTeams')?.status,
    ).toBe('fail');
    ...
    expect(
      seasonPostseasonGates(bothInvalid).find((g) => g.key === 'zeroBothInvalidGames')?.status,
    ).toBe('fail');
    ...
    expect(
      seasonPostseasonGates(integrity).find((g) => g.key === 'zeroIntegrityFailures')?.status,
    ).toBe('fail');
    ...
    expect(seasonPostseasonGates(forfeits).find((g) => g.key === 'forfeitRate')?.status).toBe(
      'fail',
    );
...
describe('season postseason calibrate command (postseason-targets-v1)', () => {
  it('measures an injected cohort and writes the artifact only with --write', async () => {
    await withTmpDir((dir) => {
      ...
      const runSeason = (rootSeed: string) =>
        cleanSeason(rootSeed, seeds.indexOf(rootSeed) % 2 === 0 ? 5 : 6);
      ...
      const artifact = seasonPostseasonTargetsSchema.parse(
        JSON.parse(readFileSync(join(dir, 'postseason-targets.json'), 'utf8')),
      );
...
describe.skipIf(!INTEGRATION_RUNS)('season postseason calibrate (integration-run)', () => {
  it('freezes postseason-targets-v1 over real seasons with all gates passing', async () => {
    const { seasonPostseasonCalibrate, DEFAULT_POSTSEASON_TARGETS } =
