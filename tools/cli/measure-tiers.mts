...
const candidates = [...catalog.candidates].sort((a, b) => (a.playerVersionId < b.playerVersionId ? -1 : 1));
const scores = candidates.map((c) => evaluateSeasonRoster({ franchiseId: c.playerVersionId, band: 'average', identity: 'continuity', members: [{ detailedRatings: c.detailedRatings, tendencies: c.tendencies }] }).roleScores);
const thresholds = rolePercentileThresholds(scores);
console.log('thresholds:', JSON.stringify(thresholds));

const tiers = candidates.map((c, i) => playerPercentileTier(percentileTierOf(scores[i], thresholds)));
const count = (t: string) => tiers.filter((x) => x === t).length;
console.log('tier counts:', 'elite', count('elite'), 'strong', count('strong'), 'useful', count('useful'), 'depth', count('depth'));
console.log('eligible (non-elite):', count('strong') + count('useful') + count('depth'));

// depth-tier trait analysis
const roles = ['primary-creation', 'secondary-creation', 'perimeter-shooting', 'rim-finishing-interior-scoring', 'perimeter-defense', 'interior-defense', 'offensive-rebounding', 'defensive-rebounding'];
const depthInfo: Array<{ maxScore: number; durability: number; stamina: number; versatile: boolean }> = [];
for (let i = 0; i < candidates.length; i++) {
  if (tiers[i] !== 'depth') continue;
  const c = candidates[i];
  const maxScore = Math.max(...roles.map((r) => scores[i][r]));
  const versatile = new Set(['G', 'F', 'C'].filter((g) => c.positions.playable.includes(g))).size >= 2;
  depthInfo.push({ maxScore, durability: c.durability.rating, stamina: c.stamina.rating, versatile });
}
...
console.log('depth maxScore p10/p25/p50/p75:', pct(maxScores, 0.1), pct(maxScores, 0.25), pct(maxScores, 0.5), pct(maxScores, 0.75));
console.log('depth durability p10/p25/p50/p75:', pct(durs, 0.1), pct(durs, 0.25), pct(durs, 0.5), pct(durs, 0.75));
console.log('depth stamina p10/p25/p50/p75:', pct(stas, 0.1), pct(stas, 0.25), pct(stas, 0.5), pct(stas, 0.75));
...
console.log('depth durability<70:', depthInfo.filter((d) => d.durability < 70).length, 'stamina<55:', depthInfo.filter((d) => d.stamina < 55).length);
...
// role tier useful
let roleUseful = 0; let roleMulti = 0;
for (let i = 0; i < candidates.length; i++) {
...
let sorted = true;
for (let i = 1; i < candidates.length; i++) if (candidates[i].playerVersionId < candidates[i - 1].playerVersionId) sorted = false;
