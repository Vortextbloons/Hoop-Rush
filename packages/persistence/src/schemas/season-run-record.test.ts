...
import { SEASON_GAME_SUMMARY_VERSION, SEASON_RECAP_VERSION, SEASON_BLOCK_VERSION, SEASON_RUN_SAVE_SCHEMA_VERSION, type SeasonPendingBlockCandidate, } from '@hoop-rush/data-contracts';
import { buildFixtureCheckpointRow, buildFixtureEffectsState, buildFixturePendingBlock, buildFixtureRun, buildFixtureStoredDraft, fixtureSeedFromString, } from '../testing/season-run-fixture.ts';
import { SEASON_RUN_RECORD_ID, storedSeasonAcceptedBlockRowSchema, storedSeasonActiveRunIndexSchema, storedSeasonDetailRowSchema, storedSeasonPendingBlockRowSchema, storedSeasonRunRecordSchema, storedSeasonSummaryRowSchema, } from './season-run-record.ts';
...
describe('storedSeasonRunRecordSchema', () => {
    ...
    it('rejects the v1 and v2 development rows outright (never read)', () => {
        expect(storedSeasonRunRecordSchema.safeParse({
    ...
    it('rejects a v4 row missing the M2.5 mutable state and with a wrong save schema', () => {
        ...
        expect(storedSeasonRunRecordSchema.safeParse({ ...row, health: { corrupted: true } }).success).toBe(false);
        expect(storedSeasonRunRecordSchema.safeParse({ ...row, stateDigest: 'not-a-digest' }).success).toBe(false);
        expect(storedSeasonRunRecordSchema.safeParse({ ...row, objectives: { corrupted: true } }).success).toBe(false);
        expect(storedSeasonRunRecordSchema.safeParse({ ...row, trade: { corrupted: true } }).success).toBe(false);
    });
    it('rejects a row with a corrupt snapshot portion or bad cursor facts', () => {
        ...
        expect(storedSeasonRunRecordSchema.safeParse({ ...row, completedRounds: 83 }).success).toBe(false);
        expect(storedSeasonRunRecordSchema.safeParse({ ...row, lastCommandId: 42 }).success).toBe(false);
        expect(storedSeasonRunRecordSchema.safeParse({ ...row, run: { ...row.run, runId: 'BAD ID!' } })
            .success).toBe(false);
        expect(storedSeasonRunRecordSchema.safeParse({ ...row, standings: { corrupted: true } }).success).toBe(false);
        expect(storedSeasonRunRecordSchema.safeParse({ ...row, effects: { corrupted: true } }).success).toBe(false);
    });
    it('rejects wrong-length aggregate arrays', () => {
        const row = checkpointRowFixture();
        expect(storedSeasonRunRecordSchema.safeParse({ ...row, teamAggregates: [{}] }).success).toBe(false);
    });
});
describe('storedSeasonSummaryRowSchema', () => {
    ...
    it('accepts rows whose row facts are consistent; identity is the repository audit', () => {
        ...
        expect(storedSeasonSummaryRowSchema.safeParse({
            ...base,
...
describe('storedSeasonDetailRowSchema', () => {
    it('accepts a valid retained detail row and rejects a corrupt one', () => {
        ...
        expect(storedSeasonDetailRowSchema.safeParse({ ...base, detail: { corrupted: true } }).success).toBe(false);
    });
});
describe('storedSeasonAcceptedBlockRowSchema and index row', () => {
    it('accepts a valid accepted-block row (with M2.5 state chain facts) and rejects a corrupt one', () => {
        ...
        expect(storedSeasonAcceptedBlockRowSchema.safeParse({
            ...block,
    ...
    it('accepts a valid active-run index row', () => {
        ...
        expect(storedSeasonActiveRunIndexSchema.safeParse({
            recordId: SEASON_RUN_RECORD_ID,
            index: { runId: 'run-1' },
        }).success).toBe(false);
    });
...
describe('storedSeasonPendingBlockRowSchema', () => {
    ...
    it('rejects a corrupt candidate or interruption', () => {
        expect(storedSeasonPendingBlockRowSchema.safeParse({
            ...pendingRowFixture(),
