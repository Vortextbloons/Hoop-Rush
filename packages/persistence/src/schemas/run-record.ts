import { z } from 'zod';
import { challengeRunSchema, CHECKPOINT_SAVE_SCHEMA_VERSION, classicVariantSchema, gameResultSchema, playerIdSchema, runModeSchema, runStatusSchema, runOutcomeSchema, runPlayerSelectionSchema, RUN_SCHEMA_VERSION, SAVE_SCHEMA_VERSION, seedSchema, type ChallengeRun, type GameResult, type RunAggregates, } from '@hoop-rush/data-contracts';
...
const CHECKPOINT_CARRIED_FIELD_NAMES: readonly (keyof CheckpointCarriedRunFields)[] = Object.keys(activeRunCheckpointBaseSchema.shape) as readonly (keyof CheckpointCarriedRunFields)[];
...
export function runFromCheckpoint(checkpoint: ActiveRunCheckpoint, results: GameResult[]): ChallengeRun {
    return {
