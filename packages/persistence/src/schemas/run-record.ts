...
export const activeRunCheckpointSchema = z.object({
  ...
  mode: runModeSchema,
  /** The selected modern franchise slot (every sandbox run has one). */
  franchiseId: franchiseIdSchema,
  /** Simulation environment era: the selected decade's profile. */
...
export const completedRunIndexSchema = z.object({
  ...
  /** The selected modern franchise slot. */
  franchiseId: franchiseIdSchema,
  eraId: z.string().min(1).max(24),
