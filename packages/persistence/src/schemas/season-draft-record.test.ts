...
describe('storedSeasonDraftSchema', () => {
    ...
    it('rejects the v1 and v2 development save schemas outright (never read)', () => {
        const record = recordFromState(buildSeasonDraftState());
        expect(storedSeasonDraftSchema.safeParse({ ...record, saveSchemaVersion: 1 }).success).toBe(false);
        expect(storedSeasonDraftSchema.safeParse({ ...record, saveSchemaVersion: 2 }).success).toBe(false);
        expect(storedSeasonDraftSchema.safeParse({
