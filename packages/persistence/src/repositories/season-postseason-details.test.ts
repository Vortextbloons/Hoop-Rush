...
import { commandIdSchema, franchiseIdSchema, idSchema, seasonCommandLogDigest, seasonRunCommandSchema, type SeasonPostseasonSummary, type SeasonRun, type SeasonRunCommand, } from '@hoop-rush/data-contracts';
import { SEASON_RUN_RECORD_ID, seasonPostseasonDetailSchema, type SeasonPostseasonDetail, } from '../schemas/season-run-record.ts';
...
import { buildFixtureEffectsState, buildFixtureRetainedDetail, buildFixtureRun, buildFixtureSchedule, buildFixtureStoredDraft, buildFixtureSummaries, buildStubSeasonEngineSeam, } from '../testing/season-run-fixture.ts';
...
function makeAdapters(seed = 'a1b2c3d4e5f60718293a4b5c6d7e8f9a', runId = 'ps-detail-run'): Adapters {
...
function basePostseasonSummary(adapters: Adapters, options: {
    gameId?: string;
...
function postseasonDetailOf(adapters: Adapters, summary: SeasonPostseasonSummary): SeasonPostseasonDetail {
    ...
    if (regular === undefined)
        throw new Error('no fixture regular summary');
...
function advancementInput(adapters: Adapters, command: SeasonRunCommand, summary: SeasonPostseasonSummary, next: SeasonRun, details: SeasonPostseasonDetail[] = []): CommitPostseasonAdvancementInput {
    return {
        runId: adapters.run.runId,
        run: next,
        summaries: [summary],
...
describe('season postseason details (M2.6)', () => {
    ...
    it('commits details atomically with the advance: run, summary, detail, and log in one transaction', async () => {
        ...
        await adapters.repo.commitPostseasonAdvancement(advancementInput(adapters, command, summary, next, [detail]));
    ...
    it('round-trips details from multiple advances, ordered by gameId ascending', async () => {
        ...
        await adapters.repo.commitPostseasonAdvancement(advancementInput(adapters, firstCommand, first, next, [postseasonDetailOf(adapters, first)]));
        adapters.run = next;
        ...
        await adapters.repo.commitPostseasonAdvancement(advancementInput(adapters, secondCommand, second, next, [
        ...
        next = advancedRun(adapters, 'playoffs');
        await adapters.repo.commitPostseasonAdvancement(advancementInput(adapters, thirdCommand, third, next));
    ...
    it('rejects a stale command with details without writing anything', async () => {
        ...
        await expect(adapters.repo.commitPostseasonAdvancement(advancementInput(adapters, stale, basePostseasonSummary(adapters), next, [
    ...
    it('rejects a detail naming another run without writing anything', async () => {
        ...
        await expect(adapters.repo.commitPostseasonAdvancement(advancementInput(adapters, command, summary, next, [foreign]))).rejects.toBeInstanceOf(SeasonRunCommandRunMismatchError);
        expect(await adapters.repo.loadCommandLog(adapters.run.runId)).toBeNull();
        expect(await adapters.repo.loadPostseasonSummaries(adapters.run.runId)).toEqual([]);
        expect(await adapters.repo.loadPostseasonDetails(adapters.run.runId)).toEqual([]);
        const snapshot = await adapters.repo.loadActiveRun();
    ...
    it('rolls the whole advance back when ANY transaction write fails', async () => {
        const failures: Array<[
            ...
            [
                'run',
                (adapters) => vi.spyOn(adapters.db.seasonRuns, 'put').mockRejectedValueOnce(new Error('x-run')),
            ],
            [
                'log',
                (adapters) => vi.spyOn(adapters.db.seasonCommandLog, 'put').mockRejectedValueOnce(new Error('x-log')),
            ],
            [
                'summary',
                (adapters) => vi
    ...
    it('surfaces corrupt detail rows (row identity disagrees with the detail facts) as SeasonRunLoadError', async () => {
        ...
        const next = advancedRun(adapters, 'play-in');
        const summary = basePostseasonSummary(adapters);
        await adapters.repo.commitPostseasonAdvancement(advancementInput(adapters, command, summary, next, [postseasonDetailOf(adapters, summary)]));
        const row = await adapters.db.seasonPostseasonDetails.get([
            adapters.run.runId,
            'pi-east-seven-eight',
        ]);
        if (row === undefined)
            throw new Error('expected a detail row');
        await adapters.db.seasonPostseasonDetails.put({
            ...row,
            detail: { ...row.detail, gameId: 'pi-east-nine-ten' },
        });
        await expect(adapters.repo.loadPostseasonDetails(adapters.run.runId)).rejects.toBeInstanceOf(SeasonRunLoadError);
    ...
    it('reloads the run through a fresh repository between advances with identical state and digests', async () => {
        ...
        expect(firstReload?.run.postseason.tiebreakResolutions).toEqual(adapters.run.postseason.tiebreakResolutions);
        expect(seasonCommandLogDigest((await reloadedRepo.loadCommandLog(adapters.run.runId))?.entries ?? [])).toBe(seasonCommandLogDigest((await adapters.repo.loadCommandLog(adapters.run.runId))?.entries ?? []));
        expect((await reloadedRepo.loadPostseasonDetails(adapters.run.runId))[0]?.result.outcome).toBe('completed');
        const second = basePostseasonSummary(adapters, {
            gameId: 'po-finals-g1',
            round: 'finals',
            conference: 'west',
        ...
        next = advancedRun(adapters, 'playoffs');
        await reloadedRepo.commitPostseasonAdvancement(advancementInput(adapters, secondCommand, second, next, [
            postseasonDetailOf(adapters, second),
        ]));
        adapters.run = next;
        ...
        expect(log?.entries[1]?.previousLogDigest).toBe(seasonCommandLogDigest(log?.entries.slice(0, 1) ?? []));
        expect((await reloadedRepo.loadPostseasonDetails(adapters.run.runId)).map((detail) => detail.gameId)).toEqual(['pi-east-seven-eight', 'po-finals-g1']);
    });
    it('rejects an advancement against a gapped command log (ordinals not dense from 0)', async () => {
        ...
        let next = advancedRun(adapters, 'play-in');
        await adapters.repo.commitPostseasonAdvancement(advancementInput(adapters, first, basePostseasonSummary(adapters), next));
        adapters.run = next;
        const second = commandOf(adapters.run, 'advance-postseason', 'cmd-gap-2');
        next = advancedRun(adapters, 'playoffs');
        await adapters.repo.commitPostseasonAdvancement(advancementInput(adapters, second, basePostseasonSummary(adapters), next));
        adapters.run = next;
        ...
        await expect(adapters.repo.commitPostseasonAdvancement(advancementInput(adapters, third, basePostseasonSummary(adapters), after))).rejects.toThrow('ordinals are not dense from 0');
