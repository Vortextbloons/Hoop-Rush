...
import { SEASON_AWARDS_VERSION, SEASON_EMPTY_COMMAND_LOG_DIGEST, SEASON_GAME_COUNT, commandIdSchema, franchiseIdSchema, idSchema, seasonAlmanacDigest, seasonAwardsDigest, seasonCommandLogDigest, seasonRunCommandSchema, type PlayoffRound, type PlayoffSeries, type SeasonAlmanac, type SeasonAwards, type SeasonCommandLog, type SeasonPostseasonSummary, type SeasonRun, type SeasonRunCommand, } from '@hoop-rush/data-contracts';
...
function completedRunOf(adapters: Adapters, stateRevision: number, awards: SeasonAwards | null = null): SeasonRun {
...
function buildAlmanac(run: SeasonRun, champion: string, commandLogDigestValue: string, awardsDigest = 'e'.repeat(32)): SeasonAlmanac {
    const parsedChampion = franchiseIdSchema.parse(champion);
    const base = {
    ...
    const recipient = (index: number) => {
        ...
        if (found === undefined) {
            return {
                ...
                franchiseId: lakersFallback,
            };
        }
        return {
            playerVersionId: found.playerVersionId,
...
async function completeChampion(adapters: Adapters, awards: SeasonAwards | null = null): Promise<{
    ...
    const almanac = buildAlmanac(adapters.run, champion, seasonCommandLogDigest(log.entries), awards?.digest ?? 'e'.repeat(32));
...
describe('season completed-season listing and completion transaction (M2.6)', () => {
    ...
    it('persists engine awards through champion promotion via the run row', async () => {
        ...
        expect(completedSeason?.run.completion?.championFranchiseId).toBe(completedSeason?.almanac.championFranchiseId);
    });
    ...
    it('rolls the whole promotion back when ANY transaction write fails', async () => {
        const failures: Array<[
            ...
            [
                'almanac row',
                'x-almanac',
                (adapters) => vi.spyOn(adapters.db.seasonAlmanacs, 'put').mockRejectedValueOnce(new Error('x-almanac')),
            ],
            [
        ...
        for (const [label, errorMessage, inject] of failures) {
            ...
            expect((await adapters.repo.loadCommandLog(adapters.run.runId))?.entries, label).toEqual(entriesBefore);
            expect(await adapters.repo.loadPostseasonSummaries(adapters.run.runId), label).toHaveLength(1);
            vi.restoreAllMocks();
        }
    });
    it('reloads between every postseason stage and completes with identical digests', async () => {
        ...
        await repo1.commitPostseasonAdvancement(advancementInput(adapters, command2, basePostseasonSummary(adapters), next2));
        adapters.run = next2;
        ...
        expect(log2?.entries[1]?.previousLogDigest).toBe(seasonCommandLogDigest(log2?.entries.slice(0, 1) ?? []));
        await completeChampion(adapters);
    ...
    it('produces identical command logs and replay exports for identical inputs', async () => {
        ...
        for (const [commandId, stage] of [
            ...
            const commandA = commandOf(runA.run, commandId === 'cmd-det-1' ? 'start-postseason' : 'advance-postseason', commandId);
            const nextA = advancedRun(runA, stage);
            await runA.repo.commitPostseasonAdvancement(advancementInput(runA, commandA, basePostseasonSummary(runA), nextA));
            runA.run = nextA;
            const commandB = commandOf(runB.run, commandId === 'cmd-det-1' ? 'start-postseason' : 'advance-postseason', commandId);
            const nextB = advancedRun(runB, stage);
            await runB.repo.commitPostseasonAdvancement(advancementInput(runB, commandB, basePostseasonSummary(runB), nextB));
            runB.run = nextB;
        }
        ...
        expect(seasonCommandLogDigest(logA?.entries ?? [])).toBe(seasonCommandLogDigest(logB?.entries ?? []));
