...
import { SEASON_DRAFT_CATALOG_V3, SEASON_DURABILITY_VERSION, SEASON_HEALTH_VERSION, SEASON_ROUND_COUNT, SEASON_RUN_SCHEMA_VERSION, SEASON_STAMINA_VERSION, SIMULATION_RATINGS, SIMULATION_TENDENCIES, PLAYER_VERSION_ID_VERSION, canonicalJson, seasonAlmanacDigest, seasonCommandLogDigest, seasonDigestHex, seasonHealthStateSchema, seasonPostseasonSummarySchema, type Position, type SeasonDraftCandidate, type SeasonDraftCatalog, type SeasonGameSimulationInput, type SeasonGameSimulationResult, type SeasonGameSideResult, type SeasonPostseasonSummary, type SeasonRun, type SeasonRunCommand, type SeasonStandings, } from '@hoop-rush/data-contracts';
...
describe('cross-track postseason integration (M2.6)', () => {
    it('ranks, starts, advances, reloads, and promotes one champion', async () => {
        ...
        const westTie = rankings.west.resolutions.find((entry) => entry.teams.includes('clippers') && entry.teams.includes(HUMAN));
