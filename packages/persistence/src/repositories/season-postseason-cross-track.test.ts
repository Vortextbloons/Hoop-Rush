...
import { SEASON_DRAFT_CATALOG_V3, SEASON_DURABILITY_VERSION, SEASON_HEALTH_VERSION, SEASON_ROUND_COUNT, SEASON_RUN_SCHEMA_VERSION, SEASON_STAMINA_VERSION, SIMULATION_RATINGS, SIMULATION_TENDENCIES, PLAYER_VERSION_ID_VERSION, canonicalJson, commandIdSchema, eraIdSchema, franchiseIdSchema, seasonAlmanacDigest, seasonCommandLogDigest, seasonDigestHex, seasonHealthStateSchema, seasonPostseasonSummarySchema, seasonRunCommandSchema, type Position, type SeasonDraftCandidate, type SeasonDraftCatalog, type SeasonGameSimulationInput, type SeasonGameSimulationResult, type SeasonGameSideResult, type SeasonPostseasonSummary, type SeasonRun, type SeasonRunCommand, type SeasonStandings, } from '@hoop-rush/data-contracts';
import { handleSeasonRunCommand, rankSeasonPostseason, seasonPostseasonHumanPlaysGame, zeroSeasonGameTransition, type SeasonPostseasonGameResolver, } from '@hoop-rush/engine';
...
import { buildFixtureEffectsState, buildFixtureLeague, buildFixtureRosters, buildFixtureRun, buildFixtureSchedule, buildFixtureStoredDraft, buildStubSeasonEngineSeam, } from '../testing/season-run-fixture.ts';
...
function catalogOf(rosters: ReturnType<typeof buildFixtureRosters>): SeasonDraftCatalog {
    const candidates: SeasonDraftCandidate[] = rosters.flatMap((roster) => roster.players.map((player, slot) => {
...
function commandOf(run: SeasonRun, command: 'start-postseason' | 'advance-postseason' | 'submit-postseason-rotation', commandId: string, extra: Record<string, unknown> = {}): SeasonRunCommand {
...
async function makeFlow(): Promise<FlowContext> {
    ...
    const influence = seam.createInitialSeasonInfluenceState(run.league.teams.map((team) => team.franchiseId));
...
async function commit(repo: DexieSeasonRunRepository, nextRun: SeasonRun, command: SeasonRunCommand, summaries: readonly SeasonPostseasonSummary[], relatedGameIds: readonly string[]): Promise<void> {
...
describe('cross-track postseason integration (M2.6)', () => {
    it('ranks, starts, advances, reloads, and promotes one champion', async () => {
        ...
        const westTie = rankings.west.resolutions.find((entry) => entry.teams.includes(clippersId) && entry.teams.includes(humanId));
        ...
        while (run.stage !== 'completed' && guard < 40) {
            ...
            if ('nextDecision' in accepted) {
                if (accepted.nextDecision === 'rotation') {
                    ...
                    if (next === null)
                        throw new Error('missing next human game');
                    ...
                    const submitCommand = commandOf(run, 'submit-postseason-rotation', `xt-sub-${String(guard)}`, {
                        targetGameId: next,
        ...
        expect((await repo.listCompletedSeasonRuns()).some((entry) => entry.runId === RUN_ID)).toBe(true);
