...
import { SEASON_RUN_SAVE_SCHEMA_VERSION, commandIdSchema, franchiseIdSchema, idSchema, seasonGameIdSchema, seasonRunCommandSchema, type SeasonEffectsState, type SeasonPendingBlockCandidate, type SeasonRun, type SeasonRunCommand, } from '@hoop-rush/data-contracts';
...
describe('season run M2.5 pending blocks (v5)', () => {
    ...
    it('rejects a pending save with stale expected state facts or a mismatched run', async () => {
        ...
        await expect(repo.savePendingBlock({ ...pendingFor(adapters), runId: idSchema.parse('other-run') }, interruption)).rejects.toThrow(SeasonPendingBlockRejectedError);
    });
