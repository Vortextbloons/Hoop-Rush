...
import { SEASON_RUN_SAVE_SCHEMA_VERSION, type SeasonEffectsState, type SeasonPendingBlockCandidate, type SeasonRun, type SeasonRunCommand, } from '@hoop-rush/data-contracts';
...
import { DexieSeasonRunRepository, SeasonRunLoadError, loadActiveRunWithSchedule, } from './season-run-dexie.ts';
import { TestDatabase, resetIndexedDb, restoreIndexedDb, testDatabaseName, } from '../testing/repo-test-support.ts';
import { buildFixtureEffectsState, buildFixtureInterruption, buildFixturePendingBlock, buildFixtureRun, buildFixtureSchedule, buildFixtureStateDigest, buildFixtureStoredDraft, buildStubSeasonEngineSeam, } from '../testing/season-run-fixture.ts';
import { buildFullSeasonDataset } from '../benchmark/season-run.ts';
import { SeasonPendingBlockRejectedError, SeasonRunCommandDuplicateError, SeasonRunCommandRunMismatchError, SeasonRunCommandStaleStateError, type CommitSeasonBlockInput, } from './season-run.ts';
interface Adapters {
    db: TestDatabase;
...
function commitInputFor(dataset: Pick<Adapters, 'run' | 'blocks'>, blockIndex: number): CommitSeasonBlockInput {
    const block = dataset.blocks[blockIndex];
    if (block === undefined)
        throw new Error(`no fixture block ${String(blockIndex)}`);
    return {
...
function lockedRotationSet(dataset: Pick<Adapters, 'run'>): SeasonRun['rotations'] {
    return dataset.run.rotations.map((rotation) => rotation.franchiseId === 'lakers'
        ? {
            ...rotation,
...
describe('season run repository (dexie)', () => {
    ...
    it('promotes a draft into an all-zero checkpoint and removes the draft', async () => {
        ...
        expect(snapshot?.run.standings.rows.every((row) => row.wins === 0 && row.losses === 0)).toBe(true);
    ...
    it('rejects promotion when the stored draft runId does not match', async () => {
        ...
        await expect(repo.promoteSeasonDraftToRun(draftForOther, otherRun)).rejects.toThrow(/stored draft runId/);
        expect(await db.seasonDrafts.count()).toBe(1);
        expect(await db.seasonRuns.count()).toBe(0);
    });
    it('commits one block and reloads summaries, details, history, and totals', async () => {
        ...
        expect(snapshot.summaries.map((summary) => summary.gameId)).toEqual(blocks[0]?.summaries.map((summary) => summary.gameId));
    ...
    it('rejects duplicate command ids and revision regressions inside the transaction', async () => {
        ...
        await promote(adapters);
        await repo.commitSeasonBlock(commitInputFor(adapters, 0));
        await expect(repo.commitSeasonBlock({ ...commitInputFor(adapters, 1), commandId: 'command-0' })).rejects.toThrow(/duplicate commandId/);
        await expect(repo.commitSeasonBlock(commitInputFor(adapters, 0))).rejects.toThrow(/revision regression/);
        await expect(repo.commitSeasonBlock({ ...commitInputFor(adapters, 1), revision: 7 })).rejects.toThrow(/revision regression/);
    ...
    it('rejects a mismatched runId, a completedRounds regression, and invalid revisions', async () => {
        ...
        await promote(adapters);
        await repo.commitSeasonBlock(commitInputFor(adapters, 0));
        await expect(repo.commitSeasonBlock({ ...commitInputFor(adapters, 1), runId: 'other-run' })).rejects.toThrow(/runId/);
        await expect(repo.commitSeasonBlock({ ...commitInputFor(adapters, 1), completedRounds: 5 })).rejects.toThrow(/completedRounds regression/);
        await expect(repo.commitSeasonBlock({ ...commitInputFor(adapters, 1), revision: 0 })).rejects.toThrow(/not a valid block boundary/);
        expect(await db.seasonRuns.count()).toBe(1);
        expect(await db.seasonRunSummaries.count()).toBe(150);
    });
    it('rejects commits when no active run checkpoint exists', async () => {
        const adapters = makeAdapters();
        await expect(adapters.repo.commitSeasonBlock(commitInputFor(adapters, 0))).rejects.toThrow(/no active run checkpoint/);
    });
    it('a failure inside the transaction rolls back the entire block', async () => {
        ...
        if (firstSummary === undefined)
            throw new Error('expected a fixture summary');
        const invalid = {
            ...input,
    ...
    it('a failure before the transaction writes nothing', async () => {
        ...
        const { db, repo } = adapters;
        await promote(adapters);
        await expect(repo.commitSeasonBlock({ ...commitInputFor(adapters, 0), revision: 0 })).rejects.toThrow(/not a valid block boundary/);
        await expect(repo.commitSeasonBlock({ ...commitInputFor(adapters, 0), revision: 10 })).rejects.toThrow(/not a valid block boundary/);
        expect(await db.seasonRunSummaries.count()).toBe(0);
        expect(await db.seasonRunBlocks.count()).toBe(0);
        const snapshot = await loadOrThrow(adapters);
        expect(snapshot.acceptedBlocks).toHaveLength(0);
    ...
    it('surfaces a corrupt stored summary row on load', async () => {
        ...
        if (row === undefined)
            throw new Error('expected a stored summary row');
...
describe('season run development-row auto-clear (M2.4)', () => {
    ...
    async function currentCheckpoint(adapters: Adapters): Promise<import('../schemas/season-run-record.ts').StoredSeasonRunRecord> {
    ...
    async function corruptStoredEffects(adapters: Adapters, mutate: (effects: SeasonEffectsState) => SeasonEffectsState): Promise<void> {
    ...
    it('never deletes legacy rows with their summaries, details, and blocks', async () => {
        ...
        if (summary === undefined)
            throw new Error('expected a fixture summary');
        await db.seasonRunSummaries.put(storedSeasonSummaryRowSchema.parse({
            runId: run.runId,
        ...
        await db.seasonRunIndex.put(storedSeasonActiveRunIndexSchema.parse({
            recordId: SEASON_RUN_RECORD_ID,
            index: {
                runId: run.runId,
    ...
    it('commit and promotion reject legacy rows instead of silently rewriting them', async () => {
        ...
        expect(await db.seasonRuns.count()).toBe(1);
        await db.seasonRuns.put(developmentV2Row(adapters) as never);
        await expect(repo.promoteSeasonDraftToRun(buildFixtureStoredDraft(run), run)).rejects.toMatchObject({
            name: 'SeasonRunIncompatibleError',
        });
        expect(await db.seasonRuns.count()).toBe(1);
    });
    ...
    it('surfaces corrupt v2 effects as SeasonRunLoadError on load', async () => {
        ...
        const cases: Array<[
            string,
            (effects: SeasonEffectsState) => SeasonEffectsState
        ]> = [
            [
                'wrong player count',
            ...
            [
                ...
                (effects) => {
                    ...
                    if (first === undefined)
                        throw new Error('expected a pair state');
                    return {
                        ...effects,
                        pairStates: [
            ...
            [
                ...
                (effects) => {
                    ...
                    if (first === undefined)
                        throw new Error('expected a player state');
                    return {
                        ...effects,
                        playerStates: [
            ...
            [
                ...
                (effects) => {
                    ...
                    return {
                        ...effects,
                        pairStates: [
                            { ...first, sharedPossessions: 10000001 },
                            ...effects.pairStates.slice(1),
                        ],
                    };
                },
            ...
            [
                ...
                (effects) => {
                    ...
                    return {
                        ...
                        pairStates: effects.pairStates.map((pair) => pair.a === original
                            ? { ...pair, a: stranger }
                            : pair.b === original
                                ? { ...pair, b: stranger }
                                : pair),
...
describe('season run M2.5 pending blocks (v5)', () => {
    function pendingFor(adapters: Adapters, blockIndex = 0, overrides: Partial<SeasonPendingBlockCandidate> = {}): SeasonPendingBlockCandidate {
    ...
    it('round-trips a pending block through save, load, and discard', async () => {
        ...
        await repo.savePendingBlock(pending, buildFixtureInterruption({
            runId: run.runId,
            blockIndex: 0,
            commandId: 'command-0',
    ...
    it('rejects a pending save when the cursor advanced past the pending block', async () => {
        ...
        await expect(repo.savePendingBlock(pending, buildFixtureInterruption({
            runId: adapters.run.runId,
            blockIndex: 1,
            commandId: 'command-1',
            nextGameId: pending.nextGameId,
        }))).rejects.toThrow(SeasonPendingBlockRejectedError);
        await expect(repo.savePendingBlock(pendingFor(adapters, 0), buildFixtureInterruption({
            runId: adapters.run.runId,
            blockIndex: 0,
            commandId: 'command-0',
            nextGameId: 's000016',
    ...
    it('rejects a pending save with stale expected state facts or a mismatched run', async () => {
        ...
        await expect(repo.savePendingBlock({ ...pendingFor(adapters), expectedStateRevision: 1 }, interruption)).rejects.toThrow(SeasonPendingBlockRejectedError);
        await expect(repo.savePendingBlock({ ...pendingFor(adapters), expectedStateDigest: 'f'.repeat(32) }, interruption)).rejects.toThrow(SeasonPendingBlockRejectedError);
        await expect(repo.savePendingBlock({ ...pendingFor(adapters), runId: 'other-run' }, interruption)).rejects.toThrow(SeasonPendingBlockRejectedError);
    });
    ...
    it('resume commits atomically and deletes the pending row', async () => {
        ...
        if (block === undefined)
            throw new Error('expected fixture block 0');
        ...
        expect(new Set(summaries.map((summary) => summary.gameId))).toEqual(new Set(block.summaries.map((summary) => summary.gameId)));
        expect(summaries.map((summary) => summary.gameId)).toEqual(block.summaries.map((summary) => summary.gameId));
    });
    it('an interrupted-resumed block equals an uninterrupted block with no duplicates', async () => {
        ...
        await resumed.repo.savePendingBlock(pending, buildFixtureInterruption({
        ...
        const a = (await uninterrupted.repo.loadBlockSummaries(uninterrupted.run.runId, 0)).map((summary) => summary.gameId);
        const b = (await resumed.repo.loadBlockSummaries(resumed.run.runId, 0)).map((summary) => summary.gameId);
        expect(a).toEqual(b);
...
describe('season run M2.5 command application (v5)', () => {
    function selectObjectiveCommand(adapters: Adapters, overrides: Partial<SeasonRunCommand> = {}): SeasonRunCommand {
        return {
            schemaVersion: 11,
    ...
    it('rejects a stale command (revision and digest) and a run mismatch', async () => {
        ...
        const { repo, run } = adapters;
        await promote(adapters);
        await expect(repo.applySeasonRunCommand({
            runId: run.runId,
    ...
    it('rejects a command id that collides with the last accepted block command', async () => {
        ...
        if (blockState === undefined)
            throw new Error('expected fixture checkpoint state');
        await expect(repo.applySeasonRunCommand({
            runId: run.runId,
            command: selectObjectiveCommand(adapters, {
                commandId: 'command-0',
...
describe('season run M2.5 reload audit (v5)', () => {
    async function currentRow(adapters: Adapters): Promise<import('../schemas/season-run-record.ts').StoredSeasonRunRecord> {
        const row = await adapters.db.seasonRuns.get(SEASON_RUN_RECORD_ID);
        if (row === undefined ||
            (row as {
                saveSchemaVersion?: unknown;
    ...
    it('repairs the legacy rotation-lock divergence and re-audits the recovered run', async () => {
        ...
        expect(snapshot?.run.stateDigest).toBe(adapters.seam.seasonRunStateDigest({
    ...
    it('accepts a commit whose digest covers the locked rotation set the commit stores', async () => {
        ...
        const base = commitInputFor(adapters, 0);
        const locked = lockedRotationSet(adapters);
        expect(adapters.seam.seasonRotationSetDigest(locked)).not.toBe(adapters.seam.seasonRotationSetDigest(run.rotations));
        const checkpointState = {
            ...base.checkpointState,
            rotationDigest: adapters.seam.seasonRotationSetDigest(locked),
        };
    ...
    it('accepts post-lock rotation repairs when a trade window exists', async () => {
        ...
        expect(snapshot?.acceptedBlocks.at(-1)?.rotationDigest).not.toBe(adapters.seam.seasonRotationSetDigest(rotations));
    });
    ...
    it('rejects a ledger entry whose balanceAfter does not reconcile', async () => {
        ...
        await promote(adapters);
        const row = await currentRow(adapters);
        const ledger = row.influence.ledger.map((entry, index) => index === 0 ? { ...entry, balanceAfter: 5 } : entry);
    ...
    it('commit rejects stale expected state facts and non-advancing stateRevision', async () => {
        ...
        await promote(adapters);
        await repo.commitSeasonBlock(commitInputFor(adapters, 0));
        await expect(repo.commitSeasonBlock({ ...commitInputFor(adapters, 1), expectedStateRevision: 5 })).rejects.toThrow(/stale expectedStateRevision/);
        await expect(repo.commitSeasonBlock({
            ...commitInputFor(adapters, 1),
            expectedStateDigest: 'f'.repeat(32),
        })).rejects.toThrow(/stale expectedStateDigest/);
        await expect(repo.commitSeasonBlock({ ...commitInputFor(adapters, 1), stateRevision: 1 })).rejects.toThrow(/does not advance/);
