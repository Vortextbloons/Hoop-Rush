...
async function advanceN(
  adapters: Adapters,
  count: number,
  summary: SeasonPostseasonSummary = basePostseasonSummary(adapters),
  firstOrdinal = 0,
): Promise<void> {
  for (let index = 0; index < count; index += 1) {
    ...
    const command = commandOf(
      adapters.run,
      ordinal === 0 ? 'start-postseason' : 'advance-postseason',
      `cmd-${String(ordinal + 1)}`,
    );
    const next = advancedRun(adapters, index === 0 ? 'play-in' : 'playoffs');
    await adapters.repo.commitPostseasonAdvancement(
      advancementInput(adapters, command, summary, next),
    );
    adapters.run = next;
  }
}
...
describe('season postseason repository contract extension (M2.6)', () => {
  ...
  it('rejects repeated command ids so a duplicate ordinal cannot be appended through the interface', async () => {
    ...
    await expect(
      adapters.repo.commitPostseasonAdvancement(
        advancementInput(adapters, replay, basePostseasonSummary(adapters), adapters.run),
      ),
    ...
    expect(seasonCommandLogDigest(after?.entries ?? [])).toBe(
      seasonCommandLogDigest(before?.entries ?? []),
    );
  ...
  it('surfaces corrupt postseason summary rows as SeasonRunLoadError from both loaders', async () => {
    ...
    await expect(adapters.repo.loadPostseasonSummaries(adapters.run.runId)).rejects.toBeInstanceOf(
      SeasonRunLoadError,
    );
    await expect(
      adapters.repo.loadPostseasonSummary(adapters.run.runId, 'pi-east-seven-eight'),
    ).rejects.toBeInstanceOf(SeasonRunLoadError);
  });
  it('rejects an advancement whose summary names a different run without writing anything', async () => {
    ...
    await expect(
      adapters.repo.commitPostseasonAdvancement(advancementInput(adapters, command, foreign, next)),
  ...
  it('assembles the full validated completed view and nulls before promotion and after deletion', async () => {
    ...
    expect(completedSeason?.run.games.filter((game) => game.status === 'scheduled')).toHaveLength(
      SEASON_GAME_COUNT - regularSummaryCount,
    );
    expect(completedSeason?.run.stage).toBe('completed');
    expect(completedSeason?.almanac.runId).toBe(adapters.run.runId);
    expect(completedSeason?.almanac.commandLogDigest).toBe(
      seasonCommandLogDigest(completedSeason?.commandLog.entries ?? []),
    );
    ...
    expect(
      await adapters.db.seasonRunSummaries.where('runId').equals(adapters.run.runId).count(),
    ).toBe(0);
    expect(
      await adapters.db.seasonRunDetails.where('runId').equals(adapters.run.runId).count(),
    ).toBe(0);
    expect(
      await adapters.db.seasonRunBlocks.where('runId').equals(adapters.run.runId).count(),
    ).toBe(0);
  });
  ...
  it('finalizes the command log to the provided entries, overwriting stored rows at the same ordinals', async () => {
    ...
    const almanac = buildAlmanac(
      adapters.run,
      champion,
      seasonCommandLogDigest(providedLog.entries),
    );
    await adapters.repo.promoteChampionToCompleted({
      runId: adapters.run.runId,
      run: finalRunWithAlmanac(finalRun, almanac.digest),
