...
function blockFactsFor(
  ...
  summaries: ReturnType<typeof buildFixtureSummaries>,
) {
  const played = seam
...
describe('Season Run performance pass (dexie v9)', () => {
  ...
  it('commits the trade-window health so the reload digest recomputes (regression)', async () => {
    ...
    expect(
      snapshot?.run.health.injuries.some(
        (injury) => injury.injuryId === 'inj-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
      ),
    ).toBe(true);
  ...
  it('every teardown path removes every run-scoped row (centralized lifecycle)', async () => {
    ...
    const postseasonLines = (franchiseId: string) => {
      const roster =
        run.rosters.find((entry) => entry.franchiseId === franchiseId) ?? run.rosters[0];
  ...
  it('writes the compact player slice at promotion and merges upserts', async () => {
    ...
    if (first === undefined) throw new Error('no fixture player');
    const slice = [
      {
    ...
    if (second === undefined) throw new Error('no fixture player');
    const traded = [
      {
    ...
    expect(merged?.map((entry) => entry.playerVersionId).sort()).toEqual(
      [first.playerVersionId, second.playerVersionId].sort(),
    );
    expect(
      merged?.find((entry) => entry.playerVersionId === first.playerVersionId)?.summaryRatings
        .overallRating,
    ).toBe(80);
