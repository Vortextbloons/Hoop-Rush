import { blockIndexForRound, buildEmptyCampaignState, emptySeasonPlayerAggregate, humanTeamOf, seasonAcceptedBlockSchema, seasonAlmanacSchema, seasonCampaignStateSchema, seasonCheckpointDigestSchema, seasonCommandLogDigest, seasonCommandLogEntrySchema, seasonCommandLogSchema, seasonCommandResultDigest, seasonPostseasonSummarySchema, seasonEffectsStateSchema, seasonFreeAgencyStateSchema, seasonReplayExportDigest, seasonReplayExportSchema, seasonRunCommandSchema, seasonRunSchema, seasonScheduleSchema, SEASON_CAMPAIGN_VERSION, SEASON_COMMAND_LOG_VERSION, SEASON_OBJECTIVE_CATALOG, SEASON_OBJECTIVE_VERSION, seasonObjectiveStateSchema, SEASON_HEALTH_VERSION, seasonHealthStateSchema, SEASON_RUN_SAVE_SCHEMA_VERSION, SEASON_RUN_SCHEMA_VERSION, type SeasonAcceptedBlock, type SeasonActiveRunIndex, type SeasonCampaignState, type SeasonCommandLog, type SeasonGameSummary, type SeasonInvalidRosterInterruption, type SeasonPendingBlockCandidate, type SeasonPlayerAggregate, type SeasonPostseasonSummary, type SeasonReplayExport, type SeasonRetainedGameDetail, type SeasonRoster, type SeasonRun, type SeasonSchedule, } from '@hoop-rush/data-contracts';
import { SEASON_RUN_RECORD_ID, seasonRunCheckpointDeltaSchema, seasonRunCursorSchema, seasonRunPlayerSliceEntrySchema, storedSeasonAcceptedBlockRowSchema, storedSeasonActiveRunIndexSchema, storedSeasonAlmanacRowSchema, storedSeasonCommandLogRowSchema, storedSeasonCompletedIndexSchema, storedSeasonCompletedRunRowSchema, storedSeasonDetailRowSchema, storedSeasonPendingBlockRowSchema, storedSeasonPlayerSliceRowSchema, storedSeasonPostseasonDetailRowSchema, storedSeasonPostseasonSummaryRowSchema, storedSeasonRunRecordSchema, storedSeasonSummaryRowSchema, seasonCompletedSeasonSchema, seasonPostseasonDetailSchema, type SeasonCompletedRunIndexEntry, type SeasonCompletedSeason, type SeasonPostseasonDetail, type SeasonRunPlayerSliceEntry, type StoredSeasonRunRecord, } from '../schemas/season-run-record.ts';
import type { Table } from 'dexie';
import { SEASON_DRAFT_RECORD_ID, storedSeasonDraftSchema, type StoredSeasonDraft, } from '../schemas/season-draft-record.ts';
...
import { normalizeSeasonFreeAgencyState, normalizeSeasonInfluenceState, normalizeSeasonRunForPersistence, normalizeSeasonTransactions, } from '../season/normalize-mutable-state.ts';
import { HoopRushDatabase } from './dexie.ts';
import { SeasonPendingBlockRejectedError, SeasonRunCommandDuplicateError, SeasonRunCommandRunMismatchError, SeasonRunCommandStaleStateError, type CommitSeasonBlockInput, type SeasonRunCommandApplication, type SeasonRunIncompatibleInfo, type SeasonRunRepository, type SeasonRunSnapshot, } from './season-run.ts';
import { SeasonPostseasonIntegrityError, type CommitPostseasonAdvancementInput, type PromoteChampionInput, type SeasonPostseasonRepository, } from './season-postseason.ts';
...
export class SeasonRunIncompatibleError extends Error {
    ...
    constructor(info: SeasonRunIncompatibleInfo) {
        super(`stored Season Run was made under older rules (schema ${String(info.storedRunSchemaVersion)}); ` +
...
function incompatibleInfoOf(row: unknown): SeasonRunIncompatibleInfo | null {
    ...
    const runSchemaVersion = typeof record.run?.versions?.runSchemaVersion === 'number'
        ? record.run.versions.runSchemaVersion
        : SEASON_RUN_SCHEMA_VERSION - 1;
    return {
        storedSaveSchemaVersion: typeof saveVersion === 'number' ? saveVersion : SEASON_RUN_SAVE_SCHEMA_VERSION,
...
function byGameId<T extends {
    gameId: string;
}>(rows: readonly T[]): T[] {
    return [...rows].sort((a, b) => (a.gameId < b.gameId ? -1 : 1));
}
function byRevision<T extends {
    revision: number;
}>(rows: readonly T[]): T[] {
    return [...rows].sort((a, b) => a.revision - b.revision);
}
function topUpPlayerAggregates(stored: readonly SeasonPlayerAggregate[], rosters: readonly SeasonRoster[]): SeasonPlayerAggregate[] {
    const byVersionId = new Map(stored.map((row) => [row.playerVersionId, row]));
    for (const roster of rosters) {
        for (const player of roster.players) {
            if (!byVersionId.has(player.playerVersionId)) {
                byVersionId.set(player.playerVersionId, emptySeasonPlayerAggregate(player.playerVersionId, roster.franchiseId));
            }
        }
    }
...
export class DexieSeasonRunRepository implements SeasonRunRepository, SeasonPostseasonRepository {
    ...
    constructor(db: HoopRushDatabase = new HoopRushDatabase(), options: SeasonRunRepositoryOptions = {}) {
        this.db = db;
    ...
    async loadActiveRunWithSchedule(schedule: SeasonSchedule): Promise<SeasonRunSnapshot | null> {
        ...
        if (isDevelopmentRow(checkpoint)) {
            ...
            throw new SeasonRunLoadError(['stored Season Run checkpoint failed schema validation'], 'stored Season Run checkpoint is unidentifiable');
        }
        try {
    ...
    private async repairLegacyRotationLockDivergence(failures: readonly string[]): Promise<boolean> {
        const rotationFailure = failures.some((failure) => /^stored rotations digest [0-9a-f]{32} does not match the last accepted lock [0-9a-f]{32}$/.test(failure));
        const stateDigestFailure = failures.includes('stored stateDigest does not recompute over the stored mutable state');
        const allowed = failures.every((failure) => /^stored rotations digest [0-9a-f]{32} does not match the last accepted lock [0-9a-f]{32}$/.test(failure) ||
        ...
        if (!rotationFailure || !stateDigestFailure || !allowed)
            return false;
        return this.db.transaction('rw', [this.db.seasonRuns, this.db.seasonRunBlocks, this.db.seasonPendingBlocks], async () => {
            ...
            if (stored.revision === 0 || stored.checkpointState === null)
                return false;
            if ((await this.db.seasonPendingBlocks.get(stored.run.runId)) !== undefined)
                return false;
            ...
            if (lockedDigest === oldDigest)
                return false;
            if (stored.lastRotationDigest !== oldDigest ||
    ...
    private async repairLegacyCommittedStateDigest(failures: readonly string[]): Promise<boolean> {
        ...
        const effectsFailure = 'run.effects diverged from the last checkpoint effects without a trade window ' +
            '(last block stateDigest does not recompute over the stored facts)';
        if (!failures.includes(digestFailure) ||
        ...
        return this.db.transaction('rw', [this.db.seasonRuns, this.db.seasonRunBlocks, this.db.seasonPendingBlocks], async () => {
            ...
            if (!parsedBlock.success)
                return false;
            const last = parsedBlock.data.block;
            if (last.stateRevision !== stored.stateRevision ||
            ...
            if (stateDigest === stored.stateDigest)
                return false;
            const updatedAtIso = new Date().toISOString();
            await this.db.seasonRuns.put(storedSeasonRunRecordSchema.parse({ ...stored, stateDigest, updatedAtIso }));
            await this.db.seasonRunBlocks.put(storedSeasonAcceptedBlockRowSchema.parse({
                ...parsedBlock.data,
    ...
    private async loadValidated(checkpoint: unknown, schedule: SeasonSchedule): Promise<SeasonRunSnapshot> {
        ...
        try {
            ...
            throw new SeasonRunLoadError(['stored Season Run checkpoint failed schema validation'], `corrupt Season Run checkpoint: ${errorMessage(error)}`);
        }
        ...
        for (const row of detailRows) {
            try {
                ...
                if (parsed.gameId !== parsed.detail.gameId || parsed.round !== parsed.detail.round) {
                    failures.push(`retained detail row ${row.gameId} identity does not match its detail facts`);
                    continue;
                }
        ...
        for (const row of blockRows) {
            try {
                ...
                if (parsed.blockIndex !== parsed.block.blockIndex ||
                    parsed.block.blockIndex !== parsed.block.revision - 1) {
                    failures.push(`accepted-block row ${String(row.blockIndex)} does not match its block facts`);
                    continue;
                }
                acceptedBlocks.push(parsed.block);
            }
            catch (error) {
                failures.push(`corrupt accepted-block row ${String(row.blockIndex)}: ${errorMessage(error)}`);
            }
        }
        ...
        if (indexRow === undefined) {
            ...
            try {
                ...
                if (activeIndex.runId !== runId) {
                    failures.push(`active-run index runId ${activeIndex.runId} does not match the checkpoint`);
                }
            }
            catch (error) {
        ...
        if (failures.length === 0) {
            ...
            if (humanFranchiseId === undefined) {
                ...
                failures.push(...auditSeasonRunState({
    ...
    async loadBlockSummaries(runId: string, blockIndex: number): Promise<SeasonGameSummary[]> {
        const rows = await this.db.seasonRunSummaries
            ...
            .toArray();
        return byGameId(rows.map((row) => {
            try {
                return storedSeasonSummaryRowSchema.parse(row).summary;
            }
            catch (error) {
                throw new SeasonRunLoadError([`corrupt summary row ${row.gameId}: ${errorMessage(error)}`], 'corrupt stored Season Run summary row');
            }
        }));
    }
    async loadRetainedDetails(runId: string): Promise<SeasonRetainedGameDetail[]> {
        ...
        return byGameId(rows.map((row) => {
            try {
                ...
                throw new SeasonRunLoadError([`corrupt retained detail row ${row.gameId}: ${errorMessage(error)}`], 'corrupt stored Season Run detail row');
            }
        }));
    }
    async loadBlockHistory(runId: string): Promise<SeasonAcceptedBlock[]> {
        ...
        return byRevision(rows.map((row) => {
            try {
                return storedSeasonAcceptedBlockRowSchema.parse(row).block;
            }
            catch (error) {
                throw new SeasonRunLoadError([`corrupt accepted-block row ${String(row.blockIndex)}: ${errorMessage(error)}`], 'corrupt stored Season Run block row');
            }
        }));
    }
    async commitSeasonBlock(input: CommitSeasonBlockInput): Promise<void> {
        ...
        if (blockIndex < 0 || blockIndex > 8) {
            throw new Error(`commitSeasonBlock: revision ${String(input.revision)} is not a valid block boundary`);
        }
        ...
        await this.db.transaction('rw', [
            ...
            if (cursor.revision !== input.revision - 1) {
                throw new Error(`commitSeasonBlock: revision regression (stored ${String(cursor.revision)}, ` +
            ...
            if (cursor.stateRevision !== input.expectedStateRevision) {
                throw new Error(`commitSeasonBlock: stale expectedStateRevision ${String(input.expectedStateRevision)} ` +
            ...
            if (input.stateRevision <= cursor.stateRevision) {
                throw new Error(`commitSeasonBlock: stateRevision does not advance (stored ${String(cursor.stateRevision)}, ` +
            ...
            const updatedAtIso = new Date().toISOString();
            await this.db.seasonRunSummaries.bulkPut(input.summaries.map((summary) => storedSeasonSummaryRowSchema.parse({
                runId: input.runId,
            ...
            await this.db.seasonRunDetails.bulkPut(input.retainedDetails.map((detail) => storedSeasonDetailRowSchema.parse({
                runId: input.runId,
            ...
            const existingCampaign = (checkpoint as {
                campaign?: unknown;
            }).campaign;
            const mutableState = {
                health: window !== null ? window.health : input.health,
                transactions: normalizeSeasonTransactions(window !== null ? window.transactions : input.transactions),
                influence: normalizeSeasonInfluenceState(window !== null ? window.influence : input.influence),
            ...
            const delta = seasonRunCheckpointDeltaSchema.parse({
                ...
                playerAggregates: topUpPlayerAggregates(input.playerAggregates, window !== null ? window.rosters : (cursor.run.rosters as SeasonRoster[])),
                recap: input.recap,
                ...
                run: {
                    ...
                    freeAgency: seasonFreeAgencyStateSchema.parse(normalizeSeasonFreeAgencyState(input.freeAgency)),
                },
            });
    ...
    async savePendingBlock(pending: SeasonPendingBlockCandidate, interruption: SeasonInvalidRosterInterruption): Promise<void> {
        ...
        await this.db.transaction('rw', this.db.seasonRuns, this.db.seasonPendingBlocks, async () => {
            ...
            if (cursor.run.runId !== pending.runId) {
                throw new SeasonPendingBlockRejectedError(`runId ${pending.runId} does not match the active checkpoint`);
            }
            if (cursor.revision !== pending.expectedRevision) {
                throw new SeasonPendingBlockRejectedError(`cursor revision ${String(cursor.revision)} does not match the pending's expectedRevision ${String(pending.expectedRevision)}`);
            }
    ...
    async loadPendingBlock(runId: string): Promise<SeasonPendingBlockCandidate | null> {
        ...
        if (parsed.block.runId !== runId || parsed.interruption.runId !== runId) {
            throw new SeasonRunLoadError(['pending block row runId does not match its key'], 'corrupt stored Season Run pending block row');
        }
        return parsed.block;
    }
    ...
    async applySeasonRunCommand(input: SeasonRunCommandApplication): Promise<void> {
        ...
        await this.db.transaction('rw', this.db.seasonRuns, this.db.seasonPendingBlocks, this.db.seasonCommandLog, async () => {
            const checkpoint = await this.db.seasonRuns.get(SEASON_RUN_RECORD_ID);
            if (checkpoint === undefined) {
            ...
            if (cursor.stateRevision !== command.expectedStateRevision) {
                throw new SeasonRunCommandStaleStateError(command.commandId, command.expectedStateRevision, cursor.stateRevision);
            }
            ...
            if (cursor.lastCommandId !== null)
                recorded.push(cursor.lastCommandId);
            if (cursor.checkpointState !== null)
                recorded.push(cursor.checkpointState.commandId);
            for (const entry of cursor.transactions) {
                if (entry.commandId !== null)
                    recorded.push(entry.commandId);
            }
            ...
            for (let index = 0; index < logEntries.length; index += 1) {
                if (logEntries[index]?.ordinal !== index) {
                    throw new SeasonRunLoadError(['command log ordinals are not dense from 0 (gap at ordinal ' + String(index) + ')'], 'Season Run command log state is inconsistent');
                }
            }
            ...
            if (input.pending === null) {
                ...
                if (existingPending === undefined) {
                    throw new SeasonRunLoadError(['a command produced a pending candidate without a prior pending row'], 'Season Run pending block state is inconsistent');
                }
    ...
    async promoteSeasonDraftToRun(draft: StoredSeasonDraft, run: SeasonRun, playerSlice?: SeasonRunPlayerSliceEntry[]): Promise<void> {
        ...
        const influence = this.seam.createInitialSeasonInfluenceState(validatedRun.league.teams.map((team) => team.franchiseId));
        ...
        const indexRow = storedSeasonActiveRunIndexSchema.parse({
            ...
            index: {
                ...
                humanFranchiseId,
                participantFranchiseIds: participantFranchiseIds.length > 0
                    ? (participantFranchiseIds as [
                        string,
                        ...string[]
                    ])
                    : undefined,
        ...
        await this.db.transaction('rw', [...SEASON_RUN_SCOPED_TABLES(this.db), this.db.seasonDrafts], async () => {
            ...
            if (playerSlice !== undefined && playerSlice.length > 0) {
                await this.db.seasonRunPlayerSlices.put(storedSeasonPlayerSliceRowSchema.parse({
                    runId: validatedRun.runId,
    ...
    async loadSeasonRunPlayerSlice(runId: string): Promise<SeasonRunPlayerSliceEntry[] | null> {
        ...
        if (parsed.runId !== runId) {
            throw new SeasonRunLoadError(['player slice row runId does not match its key'], 'corrupt stored Season Run player slice row');
        }
        return parsed.players;
    }
    async upsertSeasonRunPlayerSlice(runId: string, entries: SeasonRunPlayerSliceEntry[]): Promise<void> {
        if (entries.length === 0)
            return;
        await this.db.transaction('rw', this.db.seasonRunPlayerSlices, async () => {
            ...
            const byVersion = new Map<string, SeasonRunPlayerSliceEntry>((existing?.players ?? []).map((entry) => [entry.playerVersionId, entry]));
    ...
    async commitPostseasonAdvancement(input: CommitPostseasonAdvancementInput): Promise<void> {
        const validatedRun = seasonRunSchema.parse(normalizeSeasonRunForPersistence(input.run, input.effects ?? seasonRunEngineSeam.zeroSeasonEffectsState(input.run.rosters)));
        const command = seasonRunCommandSchema.parse(input.command);
        const summaries = input.summaries.map((summary) => seasonPostseasonSummarySchema.parse(summary));
        const details = (input.details ?? []).map((detail) => seasonPostseasonDetailSchema.parse(detail));
        ...
        await this.db.transaction('rw', [
            ...
            if (cursor.lastCommandId !== null)
                recordedCommandIds.add(cursor.lastCommandId);
            if (cursor.checkpointState !== null) {
                recordedCommandIds.add(cursor.checkpointState.commandId);
            }
            for (const entry of cursor.transactions) {
                if (entry.commandId !== null)
                    recordedCommandIds.add(entry.commandId);
            }
            for (const entry of cursor.influence.ledger) {
                if (entry.commandId !== null)
                    recordedCommandIds.add(entry.commandId);
            ...
            for (const row of existingLogRows) {
                ...
                if (parsed.success)
                    recordedCommandIds.add(parsed.data.entry.command.commandId);
            }
            ...
            if (validatedRun.stateRevision !== command.expectedStateRevision + 1) {
                throw new SeasonPostseasonIntegrityError(`advancement ${command.commandId} must advance the state revision by exactly one`);
            }
            ...
            for (let index = 0; index < entries.length; index += 1) {
                if (entries[index]?.ordinal !== index) {
                    throw new SeasonPostseasonIntegrityError(`command log ordinals are not dense from 0 (gap at ordinal ${String(index)})`);
                }
            }
            ...
            if (summaries.length > 0) {
                await this.db.seasonPostseasonSummaries.bulkPut(summaries.map((summary) => ({
                    runId: input.runId,
                    gameId: summary.gameId,
                    phase: summary.phase,
                    summary,
            ...
            if (details.length > 0) {
                await this.db.seasonPostseasonDetails.bulkPut(details.map((detail) => ({
                    runId: input.runId,
                    gameId: detail.gameId,
                    phase: detail.phase,
                    detail,
    ...
    async loadPostseasonSummaries(runId: string): Promise<SeasonPostseasonSummary[]> {
        ...
        for (const row of rows) {
            ...
            if (parsed.gameId !== parsed.summary.gameId) {
                throw new SeasonRunLoadError([`postseason summary row ${row.gameId} identity does not match its facts`], 'corrupt stored Season Run postseason summary row');
            }
            summaries.push(parsed.summary);
        }
        return summaries.sort((a, b) => (a.gameId < b.gameId ? -1 : 1));
    }
    async loadPostseasonSummary(runId: string, gameId: string): Promise<SeasonPostseasonSummary | null> {
    ...
    async loadPostseasonDetails(runId: string): Promise<SeasonPostseasonDetail[]> {
        ...
        for (const row of rows) {
            ...
            if (parsed.gameId !== parsed.detail.gameId || parsed.phase !== parsed.detail.phase) {
                throw new SeasonRunLoadError([`postseason detail row ${row.gameId} identity does not match its facts`], 'corrupt stored Season Run postseason detail row');
            }
    ...
    async loadCommandLog(runId: string): Promise<SeasonCommandLog | null> {
        ...
        const entries = rows
            ...
            if (parsed.ordinal !== parsed.entry.ordinal) {
                throw new SeasonRunLoadError([`command log row ${String(row.ordinal)} does not match its entry facts`], 'corrupt stored Season Run command log row');
            }
            return parsed.entry;
        })
            .sort((a, b) => a.ordinal - b.ordinal);
    ...
    async promoteChampionToCompleted(input: PromoteChampionInput): Promise<void> {
        ...
        if (validatedRun.stage !== 'completed') {
            throw new SeasonPostseasonIntegrityError(`cannot promote a run in stage ${validatedRun.stage}`);
        }
        ...
        if (completion.championFranchiseId !== validatedRun.postseason.championFranchiseId ||
            completion.championFranchiseId !== almanac.championFranchiseId) {
            throw new SeasonPostseasonIntegrityError('the run, its completion state, and the almanac must name the same champion');
        }
        ...
        if (commandLog.entries.length === 0) {
            throw new SeasonPostseasonIntegrityError('a completed run must finalize a non-empty command log');
        }
        if (completion.almanacDigest !== almanac.digest) {
            throw new SeasonPostseasonIntegrityError('the run completion almanac digest does not match the almanac');
        }
        const humanFranchiseId = humanTeamOf(validatedRun.league)?.franchiseId;
        if (humanFranchiseId === undefined) {
        ...
        await this.db.transaction('rw', [
            ...
            const storedPostseasonRows = await this.db.seasonPostseasonSummaries
                ...
                .equals(input.runId)
                .toArray();
            const storedGameIds = new Set(storedPostseasonRows.map((row) => storedSeasonPostseasonSummaryRowSchema.parse(row).gameId));
            const providedGameIds = new Set(postseasonSummaries.map((summary) => summary.gameId));
            if (storedGameIds.size !== providedGameIds.size) {
                throw new SeasonPostseasonIntegrityError('the frozen postseason summary set does not match the stored summaries');
            }
            for (const gameId of providedGameIds) {
                if (!storedGameIds.has(gameId)) {
                    throw new SeasonPostseasonIntegrityError(`postseason summary ${gameId} is missing from the stored set`);
                }
            }
            const { games: _games, ...runWithoutGames } = validatedRun;
            ...
            if (commandLog.entries.length > 0) {
                await this.db.seasonCommandLog.bulkPut(commandLog.entries.map((entry) => ({
                    runId: input.runId,
    ...
    async loadCompletedSeason(runId: string): Promise<SeasonCompletedSeason | null> {
        ...
        if (index.runId !== runId || almanac.runId !== runId) {
            throw new SeasonRunLoadError(['completed-season rows disagree about the runId'], 'corrupt stored completed Season Run');
        }
        ...
        if (commandLog === null) {
            throw new SeasonRunLoadError(['completed season has no command log'], 'corrupt stored completed Season Run');
        }
        if (commandLog.entries.length === 0 ||
            almanac.commandLogDigest !== seasonCommandLogDigest(commandLog.entries)) {
            throw new SeasonRunLoadError(['completed season command log does not reconcile with the almanac'], 'corrupt stored completed Season Run');
        }
        const games = this.seam.reconstructSeasonGames(schedule, summaries);
...
export function loadActiveRunWithSchedule(schedule: SeasonSchedule, db: HoopRushDatabase = new HoopRushDatabase()): Promise<SeasonRunSnapshot | null> {
