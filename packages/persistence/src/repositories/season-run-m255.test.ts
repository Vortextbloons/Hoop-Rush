...
import { SEASON_RUN_SAVE_SCHEMA_VERSION, SEASON_RUN_SCHEMA_VERSION, SEASON_BLOCK_VERSION, SEASON_CHECKPOINT_VERSION, SEASON_RECAP_VERSION, SEASON_CAMPAIGN_VERSION, SEASON_TRADE_VERSION, SEASON_INFLUENCE_VERSION, SEASON_HEALTH_VERSION, buildEmptyCampaignState, commandIdSchema, franchiseIdSchema, idSchema, seasonCampaignStateSchema, seasonRunCommandSchema, seasonTradeStateSchema, } from '@hoop-rush/data-contracts';
...
import { buildFixtureRun, buildFixtureSchedule, buildFixtureStateDigest, buildFixtureStoredDraft, buildStubSeasonEngineSeam, } from '../testing/season-run-fixture.ts';
...
describe('M2.5.5 persistence — saveSchema 9, atomic commits, replay, incompatibility', () => {
    ...
    it('every accepted Campaign/Trade command commits snapshot + log atomically with stale checks', async () => {
        ...
        await expect(adapters.repo.applySeasonRunCommand({
            runId: adapters.run.runId,
            command: staleCommand,
            run: { ...nextRun, stateRevision: 2, stateDigest: 'f'.repeat(32) },
            pending: null,
        })).rejects.toMatchObject({ name: 'SeasonRunCommandStaleStateError' });
        expect(await adapters.db.seasonCommandLog.where('runId').equals(adapters.run.runId).count()).toBe(1);
    });
    ...
    it('accepted trade commits ownership/rosters/effects/health/influence/transaction atomically; fault injection proves rollback at each boundary', async () => {
        ...
        if (!lakersRoster || !celticsRoster)
            throw new Error('no rosters');
        const lakersFirst = lakersRoster.players[0];
        const celticsFirst = celticsRoster.players[0];
        if (lakersFirst === undefined || celticsFirst === undefined)
        ...
        const nextOwnership = [
            ...adapters.run.ownership.filter((o) => o.playerVersionId !== outId && o.playerVersionId !== inId),
        ...
        expect(afterSuccess?.run.rosters.find((r) => r.franchiseId === 'lakers')?.players[0]
            ?.playerVersionId).toBe(inId);
        expect(afterSuccess?.run.ownership.find((o) => o.playerVersionId === outId)?.ownerFranchiseId).toBe('celtics');
        const boundaries: Array<[
            string,
            (db: TestDatabase) => void
        ]> = [
            [
                'seasonRuns.put',
                (db) => vi.spyOn(db.seasonRuns, 'put').mockRejectedValueOnce(new Error('inject seasonRuns.put')),
            ],
            [
        ...
        for (const [label, inject] of boundaries) {
            ...
            inject(fresh.db);
            await expect(fresh.repo.applySeasonRunCommand({
                runId: fresh.run.runId,
            ...
            expect(after?.run.rosters.find((r) => r.franchiseId === 'lakers')?.players[0]?.playerVersionId, label).not.toBe(inId);
            vi.restoreAllMocks();
        }
    });
    ...
    it('cross-tab reload cancels stale work, reloads snapshot, never repeats exchange/transaction', async () => {
        ...
        await expect(tab2.applySeasonRunCommand({
            runId: adapters.run.runId,
            command,
            run: nextRun,
            pending: null,
