...
import { buildChallengeRun, buildClassicCompletedDraft, buildClassicDraftState, buildClassicPick, buildCompletedDraftState, } from '@hoop-rush/test-fixtures';
...
import type { ActiveGameAppend, ChallengeRepository, CompletedRunIndex, StoredRunRecord, } from '../schemas/run-record.ts';
import { TestDatabase, resetIndexedDb, restoreIndexedDb, testDatabaseName, } from '../testing/repo-test-support.ts';
function draftRecord(draft: StoredClassicDraft['draft'] = buildClassicDraftState()): StoredClassicDraft {
    return {
...
function buildGameResult(gameNumber: number): GameResult {
    const side = (teamId: string, displayName: string): GameResult['home'] => ({
        ...
        shotZones: (['rim', 'shortMid', 'longMid', 'cornerThree', 'aboveBreakThree'] as const).map((zone) => ({
            zone,
            attempts: 17,
            makes: 8,
        })),
...
describe('challenge repository (dexie)', () => {
    ...
    it('rejects appending without an active checkpoint or with a mismatched runId', async () => {
        ...
        await repo.saveActiveRun(finishedRecord());
        await expect(repo.appendActiveGame({ ...append, runId: 'run-other' })).rejects.toThrow(/does not match the active checkpoint/);
    });
    ...
    ])('promotes and lists a completed run with its selected franchise ($franchiseId/$eraId)', async ({ franchiseId, eraId }) => {
        const { repo } = makeAdapter();
        const record = {
    ...
    it('rejects promotion of an unfinished run', async () => {
        ...
        await repo.saveActiveRun(record);
        await expect(repo.promoteActiveToCompleted(record, indexFor('run-y'))).rejects.toThrow(/cannot promote/);
    ...
    it('rejects promotion on draftId mismatch and leaves state unchanged', async () => {
        ...
        await expect(repo.promoteClassicDraftToRun(record, 'draft-stale')).rejects.toThrow('draftId mismatch');
...
describe('dexie active-run migration', () => {
    ...
    it('opens a v3-era save at schema version 4 and keeps the active run intact', async () => {
        ...
        await legacyDb.table('activeGames').bulkPut(run.games.map((result) => ({
            runId: run.runId,
