...
import { CHECKPOINT_SAVE_SCHEMA_VERSION, CLASSIC_DRAFT_SCHEMA_VERSION, SAVE_SCHEMA_VERSION, } from '@hoop-rush/data-contracts';
import { classicDraftRecordSchema, type StoredClassicDraft, } from '../schemas/classic-draft-record.ts';
import { activeGameRowSchema, activeRunCheckpointSchema, checkpointFromRun, completedRunIndexSchema, runFromCheckpoint, storedRunRecordSchema, type ActiveGameAppend, type ActiveRunCheckpoint, type ActiveGameRow, type ChallengeRepository, type CompletedRunIndex, type StoredRunRecord, } from '../schemas/run-record.ts';
...
function hasStaleSaveSchemaVersion(record: unknown, expected: number): boolean {
    if (typeof record !== 'object' || record === null)
        return false;
    if (!Object.hasOwn(record, 'saveSchemaVersion'))
        return false;
    return (record as {
        saveSchemaVersion?: unknown;
    }).saveSchemaVersion !== expected;
}
export class HoopRushDatabase extends Dexie {
    ...
    constructor() {
        ...
        this.version(2)
            ...
            await tx.table('activeGames').bulkPut(validated.run.games.map((result) => ({
...
export class DexieChallengeRepository implements ChallengeRepository {
    ...
    async promoteActiveToCompleted(completed: StoredRunRecord, index: CompletedRunIndex): Promise<void> {
        ...
        await this.db.transaction('rw', this.db.active, this.db.activeGames, this.db.completed, this.db.history, async () => {
            await this.db.active.delete(ACTIVE_RECORD_ID);
            await this.db.activeGames.clear();
    ...
    async loadCompletedRun(runId: string): Promise<StoredRunRecord | null> {
        ...
        if (hasStaleSaveSchemaVersion(record, SAVE_SCHEMA_VERSION)) {
            await this.db.transaction('rw', this.db.completed, this.db.history, async () => {
                ...
                if (!hasStaleSaveSchemaVersion(current, SAVE_SCHEMA_VERSION))
                    return;
    ...
    async promoteClassicDraftToRun(record: StoredRunRecord, draftId: string): Promise<void> {
        ...
        await this.db.transaction('rw', this.db.active, this.db.activeGames, this.db.classicDrafts, async () => {
