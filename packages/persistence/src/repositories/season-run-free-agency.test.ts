...
import { playerVersionId, SEASON_RUN_SAVE_SCHEMA_VERSION, commandIdSchema, eraIdSchema, franchiseIdSchema, idSchema, playerIdSchema, seasonKeySchema, seasonRunCommandSchema, seasonCommandLogDigest, seasonFreeAgencyStateSchema, type SeasonFreeAgencyCandidate, type SeasonFreeAgencyDeclaration, type SeasonFreeAgencySigning, type SeasonFreeAgencyState, type SeasonInfluenceState, type SeasonRun, type SeasonRunCommand, type SeasonTransactionEntry, } from '@hoop-rush/data-contracts';
...
import { SeasonRunCommandDuplicateError, SeasonRunCommandStaleStateError, type CommitSeasonBlockInput, } from './season-run.ts';
import { TestDatabase, restoreIndexedDb, testDatabaseName } from '../testing/repo-test-support.ts';
import { buildFixtureEffectsState, buildFixtureFreeAgencyState, buildFixtureStateDigest, buildFixtureStoredDraft, buildStubSeasonEngineSeam, } from '../testing/season-run-fixture.ts';
const FA_LAKERS = {
...
function faCandidate(versionId: string, playerId: string, band: 'featured' | 'role' | 'development' | 'emergency' = 'role'): SeasonFreeAgencyCandidate {
    return {
...
function declarationsOf(run: SeasonRun, windowIndex: number): Record<string, SeasonFreeAgencyDeclaration> {
    return Object.fromEntries(run.league.teams.map((team) => [
        team.franchiseId,
        {
...
function resolvedWindowState(run: SeasonRun, commandId: string, franchiseIds: readonly string[] = ['lakers', 'celtics']): SeasonFreeAgencyState {
...
function faLedgerEntry(franchiseId: string, entryId: string, commandId: string, balanceAfter: number): SeasonInfluenceState['ledger'][number] {
    return {
...
function faTransaction(franchiseId: string, transactionId: string, commandId: string, playerVersionIdValue: string, appliedAtStateRevision: number): SeasonTransactionEntry {
    return {
...
function resolveCommand(adapters: Adapters, overrides: Partial<SeasonRunCommand> = {}): SeasonRunCommand {
    const block2 = adapters.blocks[2];
    if (block2 === undefined)
        throw new Error('expected fixture block 2');
...
describe('season run free-agency persistence (M2.6.5)', () => {
    ...
    it('reloads an open market that is still waiting on the human declaration', async () => {
        ...
        if (window === undefined)
            throw new Error('expected an open window');
        const lakersDeclKey = franchiseIdSchema.parse('lakers');
        const aiDeclarations: typeof window.declarations = Object.fromEntries(Object.entries(window.declarations).filter(([key]) => key !== lakersDeclKey));
        const pendingHuman = {
            ...opened,
        ...
        expect(snapshot?.run.freeAgency.windows[0]?.declarations[franchiseIdSchema.parse('lakers')]).toBeUndefined();
    });
    it('reports a stored save-schema-6 row as typed incompatible and preserves it', async () => {
        ...
        const row = await db.seasonRuns.get(SEASON_RUN_RECORD_ID);
        if (row === undefined)
            throw new Error('expected the promoted checkpoint row');
    ...
    it('applies a multi-team resolution atomically: signings, ledger, transactions, caps, chain', async () => {
        ...
        const stored = await db.seasonRuns.get(SEASON_RUN_RECORD_ID);
        if (stored === undefined)
            throw new Error('expected the stored checkpoint row');
        ...
        expect(stored.influence.ledger.filter((entry) => entry.source === 'free-agent-signing')).toHaveLength(2);
        expect(stored.transactions.filter((entry) => entry.type === 'free-agent-signing')).toHaveLength(2);
        ...
        expect(snapshot?.run.rosters.find((roster) => roster.franchiseId === 'lakers')?.players).toHaveLength(11);
    });
    ...
    it('rolls back the whole resolution when a transaction write fails mid-apply', async () => {
        ...
        expect(after?.run.rosters.find((roster) => roster.franchiseId === 'lakers')?.players).toHaveLength(10);
        expect(after?.run.ownership).toHaveLength(300);
        expect(after?.run.freeAgency.windows[0]?.status).toBe('open');
        expect(after?.transactions.filter((entry) => entry.type === 'free-agent-signing')).toHaveLength(0);
        expect(after?.influence.balances[franchiseIdSchema.parse('lakers')]).toBe(2);
        expect(after?.influence.ledger.filter((entry) => entry.source === 'free-agent-signing')).toHaveLength(0);
    ...
    it('records free-agency commands with dense command-log ordinals and a chained digest', async () => {
        ...
        const rosters = run.rosters.map((roster) => roster.franchiseId === faLakersKey
            ? {
                ...roster,
                players: [
                    ...roster.players,
        ...
        expect(log.entries[2]?.previousLogDigest).toBe(seasonCommandLogDigest([log.entries[0], log.entries[1]].filter((entry): entry is NonNullable<(typeof log.entries)[number]> => entry !== undefined)));
    ...
    it('tops up factual zero-game aggregate rows for acquired players at signing application', async () => {
        ...
        const lakersRow = stored?.playerAggregates.find((row) => row.playerVersionId === FA_LAKERS.playerVersionId);
        const celticsRow = stored?.playerAggregates.find((row) => row.playerVersionId === FA_CELTICS.playerVersionId);
        ...
        const existing = stored?.playerAggregates.find((row) => row.playerVersionId === run.rosters[0]?.players[0]?.playerVersionId);
    ...
    it('rejects an effects state with active/inactive overlap on load', async () => {
        ...
        if (first === undefined)
            throw new Error('expected an active player state');
    ...
    it('rejects an effects active set that does not match the locked rotations', async () => {
        ...
        if (replaced === undefined)
            throw new Error('expected an active player state');
        const playerStates = row.effects.playerStates.map((player) => player.playerVersionId === replaced.playerVersionId
    ...
    it('rejects active pairs that span rotations (non-per-rotation 1,350)', async () => {
        ...
        const pairStates = row.effects.pairStates.map((pair, index) => index === 0 ? { a: smallest, b: largest, sharedPossessions: 0 } : pair);
        await db.seasonRuns.put({
            ...row,
    ...
    it('rejects a signing that does not reconcile with the recorded transactions', async () => {
        ...
        if (window === undefined)
            throw new Error('expected the resolved window');
