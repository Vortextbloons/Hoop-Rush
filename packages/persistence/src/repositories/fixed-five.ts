...
import {
  storedFixedFiveActiveSchema,
  storedFixedFiveCommandRowSchema,
  storedFixedFiveCompletedSchema,
  storedFixedFiveHistoryIndexSchema,
  storedFixedFivePendingResultSchema,
  type StoredFixedFiveActive,
  type StoredFixedFiveCompleted,
  type StoredFixedFiveHistoryIndex,
  type StoredFixedFivePendingResult,
...
export class DexieFixedFiveRepository {
  ...
  async appendCommand(command: FixedFiveCommand): Promise<void> {
    ...
    await this.db.transaction(
      'rw',
      this.db.fixedFiveActive,
      this.db.fixedFiveCommands,
      async () => {
  ...
  async savePendingResult(
    roomId: string,
    run: FixedFiveCompetitionRun,
    proposer: 'p1' | 'p2',
  ): Promise<void> {
  ...
  async promoteToCompleted(roomId: string, run: FixedFiveCompetitionRun): Promise<void> {
    ...
    const index: StoredFixedFiveHistoryIndex = storedFixedFiveHistoryIndexSchema.parse({
      ...
      winner:
        run.result.competition === 'shared-82'
    ...
    await this.db.transaction(
      ...
      this.db.fixedFiveActive,
      this.db.fixedFiveCommands,
      this.db.fixedFivePendingResults,
      this.db.fixedFiveCompleted,
      this.db.fixedFiveHistory,
      async () => {
