...
import type { createInitialSeasonInfluenceState as engineCreateInitialSeasonInfluenceState, foldSeasonPlayerAggregates as engineFoldSeasonPlayerAggregates, foldSeasonTeamAggregates as engineFoldSeasonTeamAggregates, reconstructSeasonGames as engineReconstructSeasonGames, seasonRotationSetDigest as engineSeasonRotationSetDigest, seasonRunStateDigest as engineSeasonRunStateDigestFn, } from '@hoop-rush/engine';
...
export function buildFixtureEffectsState(rosters: readonly SeasonRoster[], options: {
...
function fixtureRotation(roster: SeasonRoster): SeasonRotation {
    ...
    const starters = [ids[0], ids[1], ids[2], ids[3], ids[4]].filter((id): id is string => id !== undefined);
    const bench = [ids[5], ids[6], ids[7], ids[8], ids[9]].filter((id): id is string => id !== undefined);
    return {
...
function fixtureAiPools(league: SeasonLeague): SeasonRun['aiPools'] {
    return league.teams
        ...
        const playerVersionIds = Array.from({ length: 20 }, (_, slot) => {
            const hex = `${String(poolIndex).padStart(2, '0')}${String(slot).padStart(2, '0')}`.padEnd(32, '0');
            return `pv-${hex}`;
        });
        const selections = playerVersionIds.slice(0, 10);
        return {
            franchiseId: team.franchiseId,
            band: ['contender', 'playoff', 'average', 'weaker'][poolIndex % 4] as SeasonAiAssignment['band'],
            identity: [
                'star-chaser',
                'depth-builder',
                'defense-first',
...
function fixturePlayerLine(playerVersionIdValue: string, seedKey: string, slot: number): SeasonCompactPlayerLine {
...
function boxOfLines(franchiseId: string, lines: readonly SeasonCompactPlayerLine[], possessions: number): SeasonTeamBox {
    const sum = (pick: (line: SeasonCompactPlayerLine) => number) => lines.reduce((total, line) => total + pick(line), 0);
    return {
...
export function buildFixtureSummaries(input: {
    ...
    const linesByFranchise = new Map([...rostersByFranchise].map(([franchiseId, roster]) => [
        franchiseId,
        roster.players.map((player) => player.playerVersionId),
    ]));
    const summaries: SeasonGameSummary[] = [];
    for (const game of input.schedule.games) {
        if (game.round < input.fromRound || game.round > input.toRound)
            continue;
...
export function reconstructSeasonGamesFixture(schedule: SeasonSchedule, summaries: readonly SeasonGameSummary[]): SeasonGame[] {
...
export function foldTeamAggregatesFixture(league: SeasonLeague, summaries: readonly SeasonGameSummary[]): SeasonTeamAggregate[] {
    ...
    const totals = new Map(league.teams.map((team) => [team.franchiseId, zeroRow(team.franchiseId)] as const));
    for (const summary of summaries) {
        for (const side of ['home', 'away'] as const) {
            ...
            if (winnerOf(summary) === franchiseId)
                row.wins += 1;
            else
                row.losses += 1;
        }
...
export function foldPlayerAggregatesFixture(rosters: readonly SeasonRoster[], summaries: readonly SeasonGameSummary[]): SeasonPlayerAggregate[] {
    const ownerOf = new Map(rosters.flatMap((roster) => roster.players.map((player) => [player.playerVersionId, roster.franchiseId] as const)));
    ...
    const totals = new Map(rosters.flatMap((roster) => roster.players.map((player) => [player.playerVersionId, zeroRow(player.playerVersionId)] as const)));
    for (const summary of summaries) {
        if (summary.status === 'forfeit')
            continue;
        for (const side of ['home', 'away'] as const) {
...
export function buildFixtureRetainedDetail(input: {
    ...
    const side = (summary: SeasonGameSummary, which: 'home' | 'away') => {
        ...
        const players = lines.map((line) => {
            const rosterEntry = roster?.players.find((player) => player.playerVersionId === line.playerVersionId);
...
export function buildFixtureInfluenceStateFromIds(franchiseIds: readonly string[]): SeasonInfluenceState {
...
export function seasonRotationSetDigestFixture(rotations: readonly SeasonRotation[]): string {
    const canonical = JSON.stringify([...rotations]
...
const _digestFactsParity: EngineDigestFactsParity extends FixtureDigestFactsParity ? true : never = true;
const _digestReturnParity: FixtureDigestReturnParity extends EngineDigestReturnParity ? true : never = true;
const _reconstructParamsParity: EngineReconstructParamsParity extends FixtureReconstructParamsParity ? true : never = true;
const _rotationDigestParamsParity: EngineRotationDigestParamsParity extends FixtureRotationDigestParamsParity ? true : never = true;
const _createInfluenceParamsParity: EngineCreateInfluenceParamsParity extends FixtureCreateInfluenceParamsParity ? true : never = true;
const _foldTeamReturnParity: FixtureFoldTeamReturnParity extends EngineFoldTeamReturnParity ? true : never = true;
const _foldPlayerReturnParity: FixtureFoldPlayerReturnParity extends EngineFoldPlayerReturnParity ? true : never = true;
...
export function buildFixtureStateDigest(run: SeasonRun, overrides: Partial<SeasonRunStateDigestFacts> = {}): string {
...
export function buildFixturePendingBlock(input: {
    ...
    const played = reconstructSeasonGamesFixture(schedule, summaries).filter((game) => game.status !== 'scheduled');
...
export function buildFixtureCheckpointRow(run: SeasonRun, overrides: Partial<StoredSeasonRunRecord> = {}): StoredSeasonRunRecord {
...
export function buildSeasonDraftState(overrides: Partial<SeasonDraftState> & {
    revision?: number;
} = {}): SeasonDraftState {
    const seedPath = (participantId: string, round: number, pickOrdinal: number): string[] => [
        'draft',
...
export function buildFixtureStoredDraft(run: SeasonRun, generation: SeasonLeagueGenerationResult | null = null): StoredSeasonDraft {
