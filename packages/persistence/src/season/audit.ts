...
export function auditSeasonRunState(facts: SeasonRunAuditFacts, seam: SeasonRunEngineSeam): string[] {
    ...
    for (const injury of health.injuries) {
        ...
        if ((() => {
            ...
            const scheduled = seasonGameParse.success
    ...
    for (const [rawFranchiseId, windowStates] of Object.entries(influence.windows)) {
        ...
        for (const windowState of windowStates) {
            if (seen.has(windowState.windowIndex)) {
                failures.push(`influence window spend ${rawFranchiseId}/${String(windowState.windowIndex)} recorded twice`);
            }
            seen.add(windowState.windowIndex);
            if (windowState.windowIndex < 0 || windowState.windowIndex > 2) {
                failures.push(`influence window spend ${rawFranchiseId} carries out-of-range windowIndex ${String(windowState.windowIndex)}`);
            }
        }
    }
