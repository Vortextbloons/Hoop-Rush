...
import { evaluateContextualPlayerValue, evaluateLineupFit, evaluateLineupMatchup, } from './contextual-value.ts';
describe('contextual lineup value', () => {
    ...
    it('penalizes redundant non-shooting and high-usage roles', () => {
        ...
        const teammates = [0, 1, 2, 3].map((index) => buildSimulationPlayer({
    ...
    it('keeps fit and matchup bounded and leaves simulation player data unchanged', () => {
        ...
        if (!focalPlayer)
            throw new Error('fixture lineup must contain a focal player');
