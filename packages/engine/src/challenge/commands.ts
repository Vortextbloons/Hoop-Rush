import type { ChallengeRun, ClassicCompletedDraft, ClassicVariant, EraSimulationProfile, GameResult, GameSimulationInput, Lineup, OpponentBracket, RunAggregates, Seed, SimulationPlayer, SimulationTeam, } from '@hoop-rush/data-contracts';
import { RUN_SCHEMA_VERSION, SAVE_SCHEMA_VERSION, classicCompletedDraftSchema, eraIdSchema, franchiseIdSchema, seedSchema, type RunPlayerSelection, } from '@hoop-rush/data-contracts';
...
export function validateBracketContent(bracket: OpponentBracket): string[] {
    ...
    for (const opponent of bracket.opponents) {
        ...
        if (!lineupValidation.ok) {
            failures.push(`opponent ${opponent.opponentId} lineup is not legal: ${lineupValidation.issues
                .map((i) => i.message)
                .join('; ')}`);
        }
        ...
        for (const id of assignmentIds) {
            if (!playerIds.includes(id)) {
                failures.push(`opponent ${opponent.opponentId} lineup references ${id} missing from players`);
            }
        }
        ...
        if (opponent.strength.winRate < 0 ||
...
export function validateSchedule(bracket: OpponentBracket): string[] {
    ...
    for (const entry of facts.entries) {
        ...
        if (entry.repeatsPreviousByGameNumber) {
            failures.push(`schedule repeats opponent ${entry.opponentId} at games ${String(entry.gameNumber - 1)} and ${String(entry.gameNumber)}`);
        }
    }
    ...
    if (facts.threeCount !== 22 || facts.twoCount !== 8) {
        failures.push(`schedule counts must be 22 opponents x3 and 8 opponents x2 (got ${String(facts.threeCount)}x3, ${String(facts.twoCount)}x2)`);
    }
    return failures;
}
function validateCreationInput(input: ChallengeCreation): string[] {
    ...
    if (!lineupValidation.ok) {
        failures.push(`lineup is not legal: ${lineupValidation.issues.map((i) => i.message).join('; ')}`);
    }
        ...
        if (!parsedDraft.success) {
            ...
            for (let slotIndex = 0; slotIndex < 5; slotIndex += 1) {
                ...
                if (input.lineup.assignments[slotIndex]?.playerId !== pick.playerId) {
                    failures.push(`classic draft pick for slot ${String(slotIndex)} does not match the lineup`);
                }
                if (input.selections[slotIndex]?.playerId !== pick.playerId) {
                    failures.push(`classic draft pick for slot ${String(slotIndex)} does not match the selections`);
                }
                if (input.players[slotIndex]?.playerId !== pick.playerId) {
                    failures.push(`classic draft pick for slot ${String(slotIndex)} does not match the player snapshot`);
                }
                const selection = input.selections[slotIndex];
                if (selection &&
                    (selection.franchiseId !== pick.franchiseId || selection.eraId !== pick.eraId)) {
                    failures.push(`classic draft pick for slot ${String(slotIndex)} does not match selection provenance`);
                }
            }
        }
    }
...
export function createChallenge(input: ChallengeCreation): ChallengeRun {
    ...
    const run: ChallengeRun = {
        ...
        franchiseId: input.franchiseId === null ? null : franchiseIdSchema.parse(input.franchiseId),
...
export function opponentForGame(run: ChallengeRun, gameNumber: number): ChallengeRun['bracket']['opponents'][number] {
    ...
    if (!opponent) {
        throw new Error(`game ${String(gameNumber)} references unknown opponent ${String(entry?.opponentId)}`);
    }
    return opponent;
}
export function createGameInput(run: ChallengeRun, profile: EraSimulationProfile, gameNumber: number): GameSimulationInput {
...
export function createNextGameInput(run: ChallengeRun, profile: EraSimulationProfile): GameSimulationInput | null {
...
export function acceptGameResult(run: ChallengeRun, result: GameResult): ChallengeRun {
    ...
    if (result.away.teamId !== opponent.teamId || result.away.displayName !== opponent.displayName) {
        failures.push(`game ${String(result.gameNumber)} opponent ${result.away.displayName} does not match scheduled ${opponent.displayName}`);
    }
    if (result.engineVersion !== run.versions.engineVersion) {
        failures.push(`engine version mismatch: run ${run.versions.engineVersion}, result ${result.engineVersion}`);
    }
    if (result.dataVersion !== run.versions.dataVersion) {
        failures.push(`data version mismatch: run ${run.versions.dataVersion}, result ${result.dataVersion}`);
    }
    if (result.profileVersion !== run.eraProfileVersion) {
        failures.push(`era profile version mismatch: run ${run.eraProfileVersion}, result ${result.profileVersion}`);
    }
    ...
    if (failures.length > 0) {
        throw new Error(`acceptGameResult rejected game ${String(result.gameNumber)}: ${failures.join('; ')}`);
    }
...
export function simulateChallenge(run: ChallengeRun, profile: EraSimulationProfile, context: EngineContext): ChallengeRun {
    let current = run;
