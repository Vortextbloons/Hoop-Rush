import type { ChallengeRun, MadeAttempted, ShotZone, ShotZoneSummary, } from '@hoop-rush/data-contracts';
...
export function opponentSeasonTotals(run: ChallengeRun): OpponentSeasonTotals {
    const zoneTotals = new Map<ShotZone, {
        attempts: number;
        makes: number;
    }>();
...
export function explainSeason(run: ChallengeRun): SeasonExplanation {
    ...
    for (const game of run.games) {
        if (game.home.box.turnovers < game.away.box.turnovers)
            turnoverBattleWins += 1;
        if (game.away.box.turnovers < game.home.box.turnovers)
            turnoverBattleLosses += 1;
    }
    const userPossessions = Math.max(1, team.possessions);
    const netRatingPer100 = ((team.points - opponents.points) / userPossessions) * 100;
    const userZoneTotals = new Map<ShotZone, {
        attempts: number;
        makes: number;
    }>();
    for (const game of run.games) {
    ...
    const opponentMisses = Math.max(1, opponents.fieldGoals.attempted -
    ...
    const defensiveReboundPct = pct(team.rebounds.defensive, team.rebounds.defensive + opponents.rebounds.offensive);
    ...
    if (teamUsage > 0) {
        ...
        for (const player of run.aggregates.players) {
            const usage = usageOf(player.fieldGoals.attempted, player.freeThrows.attempted, player.turnovers);
