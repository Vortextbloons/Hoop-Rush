...
function team(teamId: string, overrides: {
    points?: number;
    fieldGoals?: {
        made: number;
        attempted: number;
    ...
    const players = Array.from({ length: 5 }, (_, i) => player({
...
function threeGameRun(): ChallengeRun {
    ...
    const games = [
        ...
        game({ ...homeBase, box: { ...homeBase.box, turnovers: 12 } }, { ...awayBase, box: { ...awayBase.box, turnovers: 8 } }, 'home'),
        game({ ...homeBase, box: { ...homeBase.box, turnovers: 12 } }, { ...awayBase, box: { ...awayBase.box, turnovers: 12 } }, 'away'),
    ];
...
describe('explainSeason', () => {
    ...
    it('reports no zone advantage below the attempt minimum', () => {
        ...
        const explanation = explainSeason({
            ...
            games: [
                game(team('user', { zones: [{ zone: 'rim', attempts: 5, makes: 5 }] }), team('away'), 'home'),
            ],
        });
    ...
    it('flags weak defensive glass from the opponent offensive-rebound rate', () => {
        const explanation = explainSeason(threeGameRun());
        expect(explanation.opponentOffensiveReboundRate).toBeLessThan(EXPLAIN_THRESHOLDS.opponentOffensiveReboundRate);
    ...
    it('names the usage leader from the accepted aggregates', () => {
        ...
        expect(explanation.usageLeader?.usageShare).toBeGreaterThanOrEqual(EXPLAIN_THRESHOLDS.usageShare);
    });
});
