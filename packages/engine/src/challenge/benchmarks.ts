import type { SimulationPlayer, SimulationRatings, SimulationTeam, SimulationTendencies, } from '@hoop-rush/data-contracts';
...
function scaleRatings(targetCenter: number): SimulationRatings {
    ...
    for (const [key, value] of Object.entries(base)) {
        shifted[key as keyof SimulationRatings] = Math.max(30, Math.min(95, Math.round(value + (targetCenter - 65))));
    }
    return shifted;
}
