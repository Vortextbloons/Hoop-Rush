...
import { buildFixtureBracket, buildGameSimulationInput, buildUserTeam, seedFromString, } from '@hoop-rush/test-fixtures';
import type { GameResult, RunPlayerSelection, SimulationPlayer } from '@hoop-rush/data-contracts';
import { challengeRunSchema, eraIdSchema, franchiseIdSchema, seedSchema, } from '@hoop-rush/data-contracts';
import { createEngineContext } from '../sim/context.ts';
import { simulateGame } from '../sim/game.ts';
import { abandonChallenge, acceptGameResult, createChallenge, createGameInput, createNextGameInput, simulateChallenge, validateBracketContent, type ChallengeCreation, type ChallengeCreationBase, } from './commands.ts';
import { deriveGameSeed } from './seeds.ts';
import { addGameToAggregates, zeroRunAggregates } from './aggregates.ts';
const context = createEngineContext();
function defaultSelections(players: SimulationPlayer[], franchiseId = 'lakers', eraId = '1990s'): RunPlayerSelection[] {
...
describe('challenge commands', () => {
    ...
    it('rejects an illegal lineup', () => {
        ...
        creation.lineup = {
            ...creation.lineup,
            assignments: creation.lineup.assignments.map((assignment, i) => i === 0 ? { ...assignment, positions: ['C'] } : assignment),
        };
    ...
    it('aggregates equal the sum of stored box scores (property)', () => {
        ...
        for (const player of completed.aggregates.players) {
            const pointsFromGames = completed.games.reduce((total, game) => total + (game.home.players.find((p) => p.playerId === player.playerId)?.points ?? 0), 0);
