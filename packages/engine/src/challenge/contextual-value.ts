...
function creation(player: SimulationPlayer): number {
  ...
  return (
    (r.ballHandling * 0.4 + r.passing * 0.35 + r.offensiveIq * 0.25) * 0.9 +
...
function shooting(player: SimulationPlayer): number {
  return (
    player.ratings.threePoint * 0.55 +
...
function defense(player: SimulationPlayer): number {
  return (
    player.ratings.perimeterDefense * 0.42 +
...
function rimProtection(player: SimulationPlayer): number {
  return (
    player.ratings.interiorDefense * 0.4 +
...
function rimPressure(player: SimulationPlayer): number {
  return (
    player.tendencies.rimFrequency * 0.45 +
...
function positiveReason(
  code: ContextualReason['code'],
  label: string,
...
export function evaluateLineupFit(
  ...
  if (averageRebounding < 64 && reboundGap > 0) {
    ...
    reasons.push(
      positiveReason(
        'size-and-rebounding',
        'Adds size and rebounding',
        rebounding(candidate),
        averageRebounding,
        76,
      ),
    );
  ...
  if (nonShooters >= 2 && shooting(candidate) < 55) {
    ...
    reasons.push(
      negativeReason('spacing-redundancy', 'Adds another non-shooter', shooting(candidate), 55, 88),
    );
  }
  if (highUsage >= 2 && candidate.tendencies.usageRate >= 23) {
    ...
    reasons.push(
      negativeReason(
        'role-competition',
        'Competes for on-ball possessions',
        candidate.tendencies.usageRate,
        55 - highUsage * 4,
        86,
      ),
    );
  ...
  if (candidate.tendencies.usageRate >= 26 && creation(candidate) < 62) {
    ...
    reasons.push(
      negativeReason(
        'role-competition',
        'High usage without matching creation',
        candidate.tendencies.usageRate,
        creation(candidate),
        74,
      ),
  ...
  if (size(candidate) > averageSize + 12 && reboundGap > 0) score += 0.35;
...
export function evaluateMatchup(
  ...
  const opponentTurnoverPressure = average(
    opponentPlayers.map(
      (player) => player.ratings.perimeterDefense * 0.7 + player.ratings.steal * 0.3,
    ),
  );
  ...
  if (opponentRimPressure >= 62 && candidateRimProtection > lineupRimProtection) {
    ...
    reasons.push(
      positiveReason(
        'rim-protection',
        'Rim protection versus high rim frequency',
        candidateRimProtection,
        opponentRimPressure,
        94,
      ),
    );
  ...
  if (opponentCreation >= 64 && candidateDefense > lineupPerimeterDefense) {
    ...
    reasons.push(
      positiveReason(
        'perimeter-defense',
        'Perimeter defense versus high creation',
        candidateDefense,
        opponentCreation,
        89,
      ),
  ...
  if (opponentShooting >= 64 && lineupPerimeterDefense < 64 && candidateDefense < 60) {
    ...
    reasons.push(
      negativeReason(
        'perimeter-creation',
        'Can be targeted by perimeter creation',
        candidateDefense,
        opponentShooting,
        87,
      ),
  ...
  if (opponentTurnoverPressure >= 64 && candidate.tendencies.turnoverRate > 14) {
    ...
    reasons.push(
      negativeReason(
        'turnover-pressure',
        'Turnover risk versus pressure defense',
        candidate.tendencies.turnoverRate,
        opponentTurnoverPressure,
        83,
      ),
    );
  }
  if (opponentRebounding >= 64 && rebounding(candidate) < lineupRebounding) {
    ...
    reasons.push(
      negativeReason(
        'size-and-rebounding',
        'Needs rebounding against a strong glass team',
        rebounding(candidate),
        opponentRebounding,
        80,
      ),
  ...
  if (opponentFoulPressure >= 7 && candidate.tendencies.foulRate <= 5) {
    ...
    reasons.push(
      positiveReason(
        'foul-pressure',
        'Disciplined defender against foul pressure',
        candidate.tendencies.foulRate,
        opponentFoulPressure,
        60,
      ),
    );
  }
  if (opponentShooting < 58 && lineupShooting > 62 && candidateRimProtection >= 60) score += 0.25;
...
export function evaluateContextualPlayerValue(
  candidate: SimulationPlayer,
  teammates: readonly SimulationPlayer[],
  opponent?: SimulationTeam,
): ContextualPlayerValue {
...
export function evaluateLineupMatchup(
  ...
  const averagePlayer: SimulationPlayer = {
    ...
    ratings: Object.fromEntries(
      Object.keys(firstPlayer.ratings).map((key) => [
        key,
        average(
          lineup.players.map((player) => player.ratings[key as keyof SimulationPlayer['ratings']]),
        ),
      ]),
    ) as SimulationPlayer['ratings'],
    tendencies: Object.fromEntries(
      Object.keys(firstPlayer.tendencies).map((key) => [
        key,
        average(
          lineup.players.map(
            (player) => player.tendencies[key as keyof SimulationPlayer['tendencies']],
          ),
        ),
      ]),
