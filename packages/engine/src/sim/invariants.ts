...
export function checkGameResult(result: GameResult): string[] {
    ...
    const checkSide = (side: 'home' | 'away'): void => {
        ...
        if (players.some((p) => p.minutes !== 48 + result.overtimePeriods * 5)) {
            failures.push(`${side}: player minutes must equal 48 + 5*OT (${String(48 + result.overtimePeriods * 5)}) with no bench`);
        }
        const accounting = auditSideAccounting(players, box, team.shotZones, (p) => p.playerId);
        if (accounting.playerPointsTotal !== box.points) {
            failures.push(`${side}: player points (${String(accounting.playerPointsTotal)}) != team points (${String(box.points)})`);
        }
        if (!accounting.pointsIdentityOk) {
            failures.push(`${side}: points ${String(box.points)} != 2*2fg + 3*3fg + ft (${String(accounting.pointsIdentity)})`);
        }
        ...
        for (const row of accounting.reconciliations) {
            if (row.playerTotal !== row.teamValue) {
                failures.push(`${side}: player ${row.label} (${String(row.playerTotal)}) != team ${row.label} (${String(row.teamValue)})`);
            }
        }
        if (box.diagnostics) {
            const d = box.diagnostics;
            const misses = box.fieldGoals.attempted -
                box.fieldGoals.made +
                box.freeThrows.attempted -
                box.freeThrows.made;
            if (!accounting.reboundOpportunitiesOk) {
                failures.push(`${side}: rebound opportunities (${String(d.reboundOpportunities)}) != misses (${String(misses)})`);
            }
            if (!accounting.assistedUnassistedOk) {
                failures.push(`${side}: assisted (${String(d.assistedFieldGoals)}) + unassisted (${String(d.unassistedFieldGoals)}) != made field goals (${String(box.fieldGoals.made)})`);
            }
            ...
            if (!accounting.offensiveReboundChancesOk) {
                failures.push(`${side}: player offensive-rebound chances != 5 * team rebound opportunities`);
            }
            const otherDiagnostics = result[other].box.diagnostics;
            if (otherDiagnostics &&
                players.reduce((acc, p) => acc + (p.diagnostics ? p.diagnostics.defensiveReboundChances : 0), 0) !==
                    otherDiagnostics.reboundOpportunities * 5) {
                failures.push(`${side}: player defensive-rebound chances != 5 * opponent rebound opportunities`);
            }
        }
        for (const zone of accounting.zoneSplits) {
            if (zone.playerAttempts !== zone.teamAttempts) {
                failures.push(`${side}: player zone attempts (${zone.zone}) ${String(zone.playerAttempts)} != team ${String(zone.teamAttempts)}`);
            }
            if (zone.playerMakes !== zone.teamMakes) {
                failures.push(`${side}: player zone makes (${zone.zone}) ${String(zone.playerMakes)} != team ${String(zone.teamMakes)}`);
            }
        }
        const violationCount = Math.max(accounting.usageViolations.length, accounting.assistOpportunityViolations.length);
        for (let i = 0; i < violationCount; i += 1) {
            const usage = accounting.usageViolations[i];
            if (usage !== undefined) {
                failures.push(`${side}: usage ${usage.usage.toFixed(2)} != fga + 0.44*fta + tov (${usage.identity.toFixed(2)})`);
            }
            const assist = accounting.assistOpportunityViolations[i];
            if (assist !== undefined) {
                failures.push(`${side}: assist opportunities (${String(assist.assistOpportunities)}) < assists (${String(assist.assists)})`);
            }
        }
        ...
        if (misses !== claimed) {
            failures.push(`${side}: misses (${String(misses)}) != own OReb + opponent DREB/team (${String(claimed)})`);
        }
        const opponentMisses = otherBox.fieldGoals.attempted -
        ...
        if (opponentRecoveries !== opponentMisses) {
            failures.push(`${side}: opponent miss recoveries (${String(opponentRecoveries)}) != opponent misses (${String(opponentMisses)})`);
        }
    ...
    if (homeTotal !== result.home.box.points) {
        failures.push(`home period scores (${String(homeTotal)}) != home points (${String(result.home.box.points)})`);
    }
    if (awayTotal !== result.away.box.points) {
        failures.push(`away period scores (${String(awayTotal)}) != away points (${String(result.away.box.points)})`);
    }
    ...
    if (result.periodScores.home.length !== 4 + result.overtimePeriods) {
        failures.push(`period count (${String(result.periodScores.home.length)}) != 4 + OT (${String(result.overtimePeriods)})`);
    }
