...
import { blendedZoneWeights, isThreePointZone, twoPointZoneSharesFromBlend, zoneSkillRating, type ActionType, } from './usage.ts';
...
export function contestPenalty(defender: SimulationPlayer, zone: ShotZone): number {
    ...
    const ratio = Math.min(1, Math.max(ENGINE_CONSTANTS.contestMin / ENGINE_CONSTANTS.contestMax, (contest - ENGINE_CONSTANTS.contestRatioPivot) / ENGINE_CONSTANTS.contestRatioRange));
...
export function observedTwoPointPct(shooter: SimulationPlayer): number | null {
    ...
    if (twoShare < 1e-6)
        return null;
    const threePct = anchors.threePointPct ?? reconstructed?.accuracyConservative ?? anchors.fieldGoalPct;
    const twoPct = (anchors.fieldGoalPct - threeRate * threePct) / twoShare;
    return Math.min(0.62, Math.max(0.32, twoPct));
}
export function twoPointAnchorFactor(shooter: SimulationPlayer, profile: EraSimulationProfile, blend: readonly number[] = blendedZoneWeights(shooter, profile)): number | null {
    ...
    return Math.min(ENGINE_CONSTANTS.twoPointAnchorMax, Math.max(ENGINE_CONSTANTS.twoPointAnchorMin, factor));
}
export function blockProbability(defender: SimulationPlayer, zone: ShotZone, action: ActionType): number {
    const c = ENGINE_CONSTANTS;
    const anchors = defender.anchors;
    const per48 = anchors && anchors.minutesPerGame > 0
        ? (anchors.blocksPerGame / anchors.minutesPerGame) * 48
        : 0;
    const anchorBonus = per48 > c.blockAnchorFloorPer48
    ...
    if (zone === 'rim') {
        const base = Math.min(c.blockRimMax, Math.max(0, ((defender.ratings.block - 40) / 60) * c.blockRimMax));
        return Math.min(c.blockRimMax + c.blockDriveBonus + c.blockAnchorMax, (base +
            (action === 'isolation' || action === 'pickAndRoll' ? c.blockDriveBonus : 0) +
            anchorBonus) *
            attemptFactor);
    }
    if (zone === 'shortMid') {
        return Math.min(c.blockMidMax, (Math.max(0, ((defender.ratings.block - 50) / 50) * c.blockMidMax) + anchorBonus * 0.5) *
            attemptFactor);
    }
    return Math.min(c.blockThreeMax, Math.max(0, ((defender.ratings.perimeterDefense - 60) / 40) * c.blockThreeMax) * attemptFactor);
}
export function makeProbability(shooter: SimulationPlayer, defender: SimulationPlayer, profile: EraSimulationProfile, zone: ShotZone, action: ActionType, periodSecondsRemaining: number, prep: ShotPrep, homeCourtAdjustment = 0, effectsAdjustment = 0): number {
    ...
    const era = anchoredTwo || hasObservedThree || hasReconstructed
        ? 0
    ...
    const latePenalty = periodSecondsRemaining <= 4
    ...
    const raw = base +
        ...
        homeCourtAdjustment +
        effectsAdjustment;
    const floor = threePointZone && hasReconstructed
        ? zone === 'cornerThree'
