...
export function teamMean(team: SimulationTeam, rating: keyof SimulationPlayer['ratings']): number {
    return (team.players.reduce((sum, p) => {
        const tendencyFactor = rating === 'offensiveRebound' ? 0.75 + p.tendencies.crashOffensiveGlassRate / 40 : 1;
...
export function offensiveReboundProbability(offMean: number, defMean: number, zone: ShotZone, profile: EraSimulationProfile): number {
    const c = ENGINE_CONSTANTS;
    const ratio = (offMean - defMean) / c.offensiveReboundRange;
    const zoneAdjust = zone === 'rim' || zone === 'shortMid'
        ? c.offensiveReboundRimBonus
        : -c.offensiveReboundPerimeterPenalty;
    const base = profile.parameters.offensiveReboundRate + ratio * c.offensiveReboundScale + zoneAdjust;
    return Math.min(0.45, Math.max(0.12, base));
}
export function resolveRebound(rng: Rng, offMean: number, defMean: number, zone: ShotZone, profile: EraSimulationProfile, deadBall: boolean): ReboundResult {
    if (deadBall)
        return { offensive: false, team: true };
    const p = offensiveReboundProbability(offMean, defMean, zone, profile);
    if (rng.chance(p))
        return { offensive: true, team: false };
    return { offensive: false, team: false };
}
export function rebounderWeights(team: SimulationTeam, offensive: boolean, positionModifiers?: ReadonlyMap<string, PositionResponsibilityModifiers>): number[] {
    ...
    return team.players.map((p) => {
        ...
        return (Math.max(0.5, p.ratings[rating] * (offensive ? 0.75 + p.tendencies.crashOffensiveGlassRate / 40 : 1) +
            p.ratings.vertical * 0.25 +
            historical * ENGINE_CONSTANTS.observedReboundWeight +
            heightContribution +
            weightContribution) * responsibility);
    });
}
export function pickRebounder(players: readonly SimulationPlayer[], weights: readonly number[], rng: Rng): SimulationPlayer {
