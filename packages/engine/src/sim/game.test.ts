...
import { buildGameSimulationInput, buildLegalSimulationTeam, buildSimulationPlayer, buildStrongWeakFixture, buildStrongMediumFixture, seedFromString, } from '@hoop-rush/test-fixtures';
    ...
    it('satisfies invariants for strong-vs-weak and strong-vs-medium fixtures', () => {
        ...
        for (let i = 0; i < 50; i += 1) {
            for (const base of inputs) {
                const result = simulateGame({ ...base, seed: seedFromString(`${base.seed}-${String(i)}`) }, ctx);
                expect(checkGameResult(result)).toEqual([]);
            }
        }
    });
    it('reports overtime facts on the golden overtime game', () => {
        const found = run('ot-v13-55');
...
describe('lineup strength across fixtures', () => {
    ...
    const strengthCases: StrengthCase[] = [
        ...
        },
        {
            ...
            buildHome: () => buildLegalSimulationTeam({
