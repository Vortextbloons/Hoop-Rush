...
import { buildEraSimulationProfile, buildGameSimulationInput, buildSimulationPlayer, seedFromString, } from '@hoop-rush/test-fixtures';
...
describe('property: accounting and determinism', () => {
    ...
    it('reproduces byte-equivalent results for identical inputs', () => {
        fc.assert(fc.property(inputArb, (input) => {
            ...
            expect(gameResultDigest(simulateGame(gameInput, ctx))).toBe(gameResultDigest(simulateGame(gameInput, ctx)));
        }), { numRuns: 30 });
    });
...
describe('property: zone skill correlation', () => {
    it('high-zone-skill teams make more shots in that zone', () => {
        ...
        for (let i = 0; i < 120; i += 1) {
            ...
            const badR = simulateGame(buildGameSimulationInput({ seed, profile, home: bad, away: bad }), ctx).home.box.threes;
            const goodR = simulateGame(buildGameSimulationInput({ seed, profile, home: good, away: good }), ctx).home.box.threes;
