import type { GameResult, PlayerBoxScore, PlayerDiagnostics, ShotZone, TeamBoxScore, TeamDiagnostics, ShotZoneSummary, } from '@hoop-rush/data-contracts';
...
export class GameRecorder {
    ...
    private readonly activeSlots: [
        number[],
        number[]
    ];
    constructor(rosterSize: number | [
        number,
        number
    ] = 5) {
        const sizes: [
        ...
        const makePlayers = (count: number): RecorderPlayer[] => Array.from({ length: count }, () => ({
            minutes: 0,
            seconds: 0,
            points: 0,
            fieldGoalMakes: 0,
    ...
    private playerAtRosterIndex(side: SideIndex, rosterIndex: number): RecorderPlayer {
        ...
        if (player === undefined) {
            throw new Error(`recorder: no roster player at side ${String(side)} index ${String(rosterIndex)}`);
        }
        return player;
    }
    fieldGoalAttempt(side: SideIndex, slot: number, zone: ShotZone, made: boolean, three: boolean, assisted: boolean): void {
        ...
        if (made) {
            ...
            if (assisted)
                team.assistedFieldGoals += 1;
            else
                team.unassistedFieldGoals += 1;
    ...
    private addToPlayerAndTeam(side: SideIndex, slot: number, playerKey: 'assists' | 'steals' | 'blocks' | 'turnovers' | 'fouls', teamKey: 'assists' | 'steals' | 'blocks' | 'turnovers' | 'fouls'): void {
        this.playerAt(side, slot)[playerKey] += 1;
        this.sides[side][teamKey] += 1;
    }
    private addChanceToActive(side: SideIndex, field: 'offensiveReboundChances' | 'defensiveReboundChances'): void {
        for (const active of this.activeSlots[side]) {
            this.playerAtRosterIndex(side, active)[field] += 1;
        }
    }
    ...
    offensiveReboundChance(side: SideIndex): void {
        this.addChanceToActive(side, 'offensiveReboundChances');
    }
    defensiveReboundChance(side: SideIndex): void {
        this.addChanceToActive(side, 'defensiveReboundChances');
    }
    contest(side: SideIndex, slot: number): void {
        this.playerAt(side, slot).contestedShots += 1;
    }
    assist(side: SideIndex, slot: number): void {
        this.addToPlayerAndTeam(side, slot, 'assists', 'assists');
    }
    steal(side: SideIndex, slot: number): void {
        this.addToPlayerAndTeam(side, slot, 'steals', 'steals');
    }
    block(side: SideIndex, slot: number): void {
        this.addToPlayerAndTeam(side, slot, 'blocks', 'blocks');
    }
    ...
    foul(side: SideIndex, slot: number): void {
        this.addToPlayerAndTeam(side, slot, 'fouls', 'fouls');
    }
    ...
    assignMinutes(minutesPerPlayer: number): void {
        for (const side of this.players) {
            for (const player of side)
                player.minutes = minutesPerPlayer;
        }
    }
    ...
    seasonPlayerBox(side: SideIndex, rosterIndex: number): {
        seconds: number;
        minutes: number;
        points: number;
        fieldGoals: {
    ...
    seasonTeamBox(side: SideIndex, teamId: string): Omit<TeamBoxScore, 'diagnostics'> & {
    ...
    teamBox(side: SideIndex, teamId: string): TeamBoxScore {
        return this.seasonTeamBox(side, teamId);
    }
...
function teamDiagnostics(t: RecorderSide, players: readonly RecorderPlayer[]): TeamDiagnostics {
    return {
        ...
        reboundOpportunities: t.fieldGoalAttempts - t.fieldGoalMakes + (t.freeThrowAttempts - t.freeThrowMakes),
        contestedShots: players.reduce((sum, player) => sum + player.contestedShots, 0),
    };
}
function zoneSummaryArray(attempts: Record<ShotZone, number>, makes: Record<ShotZone, number>): ShotZoneSummary[] {
...
function playerBoxBase(p: RecorderPlayer): Omit<PlayerBoxScore, 'playerId' | 'minutes' | 'diagnostics'> & {
    diagnostics: PlayerDiagnostics;
} {
    return {
        points: p.points,
