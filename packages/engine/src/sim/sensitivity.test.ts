...
import type { EraSimulationProfile, GameResult, SimulationPlayer, SimulationTeam, } from '@hoop-rush/data-contracts';
import { buildEraSimulationProfile, buildGameSimulationInput, buildLegalSimulationTeam, seedFromString, } from '@hoop-rush/test-fixtures';
import { simulateGame } from './game.ts';
import { createEngineContext } from './context.ts';
const ctx = createEngineContext();
const SEEDS = 200;
function mutatePlayers(team: SimulationTeam, mutate: (player: SimulationPlayer) => SimulationPlayer): SimulationTeam {
    return { ...team, players: team.players.map(mutate) };
}
function mutateAllRatings(team: SimulationTeam, key: keyof SimulationPlayer['ratings'], delta: number): SimulationTeam {
    return mutatePlayers(team, (p) => ({
        ...p,
        ratings: { ...p.ratings, [key]: Math.min(98, Math.max(20, p.ratings[key] + delta)) },
    }));
}
function mutateAllTendencies(team: SimulationTeam, key: keyof SimulationPlayer['tendencies'], delta: number): SimulationTeam {
    return mutatePlayers(team, (p) => ({
        ...p,
        tendencies: { ...p.tendencies, [key]: Math.min(100, Math.max(0, p.tendencies[key] + delta)) },
    }));
}
function profileWith(parameters: Partial<EraSimulationProfile['parameters']>): EraSimulationProfile {
...
function compareMany(name: string, baseTeam: SimulationTeam, changedTeam: SimulationTeam, selectors: readonly MetricSelect[]): Array<{
    base: number;
    changed: number;
}> {
    ...
    for (let i = 0; i < SEEDS; i += 1) {
        ...
        for (let s = 0; s < selectors.length; s += 1) {
            ...
            if (selector === undefined)
                throw new Error('selector index out of range');
...
function compare(name: string, baseTeam: SimulationTeam, changedTeam: SimulationTeam, select: MetricSelect): {
    base: number;
    changed: number;
} {
    const result = compareMany(name, baseTeam, changedTeam, [select]).at(0);
    if (!result)
        throw new Error('comparison did not produce a result');
    return result;
}
function expectDirection(name: string, base: number, changed: number, delta: number, maxRelative = 0.6, minRelative = 0.03): void {
    const relative = (changed - base) / Math.max(1e-9, Math.abs(base));
    expect(relative, `${name}: base=${base.toFixed(2)} changed=${changed.toFixed(2)}`).toBeGreaterThan(delta > 0 ? minRelative : -minRelative);
...
describe('sensitivity: shooting and finishing', () => {
    it.concurrent('higher insideScoring increases points and field-goal percentage', () => {
        ...
        if (!points || !fg)
            throw new Error('comparison did not produce both metrics');
...
describe('sensitivity: creation and usage', () => {
    it.concurrent('higher usageRate concentrates shot attempts on that player', () => {
        const star = mutatePlayers(baseTeam, (p) => p.playerId === 'p-fixture-1'
...
describe('sensitivity: ball security', () => {
    it.concurrent('higher ballHandling reduces turnovers', () => {
        ...
        expect(tov.changed, `base=${String(tov.base)} changed=${String(tov.changed)}`).toBeLessThan(tov.base * 0.97);
...
describe('sensitivity: defense', () => {
    it.concurrent('higher perimeterDefense lowers opponent points', () => {
        ...
        expect(oppPts.changed, `base=${String(oppPts.base)} changed=${String(oppPts.changed)}`).toBeLessThan(oppPts.base * 0.985);
    });
...
describe('sensitivity: era pace and shot mix', () => {
    it.concurrent('higher era pace increases possessions per game', () => {
        ...
        for (let i = 0; i < SEEDS; i += 1) {
            ...
            fastTotal += simulateGame(buildGameSimulationInput({ seed, profile: fast, home: baseTeam, away: baseTeam }), ctx).home.box.possessions;
            slowTotal += simulateGame(buildGameSimulationInput({ seed, profile: slow, home: baseTeam, away: baseTeam }), ctx).home.box.possessions;
        }
    ...
    it.concurrent('higher league three-point rate raises three-point attempt share', () => {
        ...
        for (let i = 0; i < SEEDS; i += 1) {
            ...
            heavyShare += pickShare(buildGameSimulationInput({ seed, profile: threeHeavy, home: baseTeam, away: baseTeam }));
            lightShare += pickShare(buildGameSimulationInput({ seed, profile: threeLight, home: baseTeam, away: baseTeam }));
        }
