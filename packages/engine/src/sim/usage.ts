...
import { sameGroupMatchWeight, type PositionResponsibilityModifiers, } from './position-responsibilities.ts';
import { SHOT_ZONES } from '../domain/zones.ts';
export type ActionType = 'isolation' | 'pickAndRoll' | 'pickAndRollRoll' | 'postUp' | 'spotUp' | 'cut' | 'transition';
export type PossessionStartType = 'neutral' | 'madeBasket' | 'deadBall' | 'liveTurnover' | 'defensiveRebound' | 'offensiveRebound';
...
export function initiatorWeight(player: SimulationPlayer, team: SimulationTeam, modifiers: PositionResponsibilityModifiers): number {
...
export function creationBurden(player: SimulationPlayer, team: SimulationTeam): number {
    if (player.tendencies.usageRate < 25)
        return 1;
...
export function actionWeights(initiator: SimulationPlayer, modifiers: PositionResponsibilityModifiers): number[] {
...
export function pickAction(initiator: SimulationPlayer, weights: readonly number[], rng: Rng, start: PossessionStartType = 'neutral'): ActionType {
    ...
    for (let i = 0; i < ACTION_TYPES.length; i += 1) {
        ...
        if (roll < w) {
            ...
            if (action === undefined)
                throw new Error('pickAction: index out of range');
            return action;
        }
        roll -= w;
    }
...
export function passProbability(initiator: SimulationPlayer, action: ActionType): number {
    const actionBase = action === 'spotUp' || action === 'cut' || action === 'transition'
        ? 0.9
        : action === 'pickAndRollRoll'
            ...
            : action === 'pickAndRoll'
                ...
                : action === 'postUp'
                    ? 0.35
                    : 0.17;
    const passingFactor = 0.65 + initiator.tendencies.passRate / 100 + creationScore(initiator) * 0.15;
    const creationFactor = 1.15 - initiator.tendencies.usageRate / 150;
    return Math.min(0.97, Math.max(0.05, actionBase * 1.2 * passingFactor * Math.max(0.7, creationFactor)));
}
...
export function teammateShotWeights(team: SimulationTeam, initiator: SimulationPlayer, action: ActionType, positionModifiers: ReadonlyMap<string, PositionResponsibilityModifiers>): TeammateShotWeights {
    const teammates = team.players.filter((p) => p.playerId !== initiator.playerId);
    const weights = teammates.map((p) => action === 'pickAndRollRoll'
...
export function pickShot(shots: TeammateShots, initiator: SimulationPlayer, action: ActionType, rng: Rng, passP?: readonly number[]): ShotSelection {
    const pass = passP === undefined
...
export function assisterWeights(team: SimulationTeam, shooter: SimulationPlayer, initiator: SimulationPlayer): number[] {
    const candidates = team.players.filter((p) => p.playerId !== shooter.playerId);
    return candidates.map((p) => {
        const observedCreation = p.anchors?.assistsPerGame;
        const roleWeight = observedCreation === undefined
...
export function pickAssister(team: SimulationTeam, shooter: SimulationPlayer, initiator: SimulationPlayer, rng: Rng): SimulationPlayer | null {
    const candidates = team.players.filter((p) => p.playerId !== shooter.playerId);
...
function defenderWeight(defender: SimulationPlayer, zone: ShotZone): number {
    const zoneRating = zone === 'rim' || zone === 'shortMid'
...
export function defenderBase(team: SimulationTeam, positionModifiers: ReadonlyMap<string, PositionResponsibilityModifiers>): DefenderBase {
    return {
        weights: SHOT_ZONES.map((zone) => team.players.map((defender) => defenderWeight(defender, zone))),
        rimProtection: team.players.map((defender) => positionModifiers.get(defender.playerId)?.rimProtection ?? 1),
        matchMatrix: team.players.map((_, defenderSlot) => team.players.map((_, shooterSlot) => sameGroupMatchWeight(defenderSlot, shooterSlot))),
    };
}
export function pickDefender(team: SimulationTeam, zone: ShotZone, rng: Rng, base: DefenderBase, shooterSlot: number): SimulationPlayer {
    ...
    for (let slot = 0; slot < count; slot += 1)
        total += Math.max(0, weightOf(slot));
    if (total <= 0)
    ...
    for (let slot = 0; slot < count; slot += 1) {
        ...
        if (roll < w) {
            ...
            if (player === undefined)
                throw new Error(`pickDefender: no player at slot ${String(slot)}`);
            return player;
        }
        roll -= w;
    }
    ...
    if (last === undefined)
        throw new Error('pickDefender: index out of range');
    return last;
}
const ZONE_INDEX = Object.fromEntries(SHOT_ZONES.map((zone, index) => [zone, index])) as Record<ShotZone, number>;
export function threePointTarget(shooter: SimulationPlayer, profile: EraSimulationProfile): number {
    ...
    if (observedRate === undefined) {
        return Math.min(0.65, Math.max(0.01, (f.threePointRate / 100) * ENGINE_CONSTANTS.threePointRateWeight +
    ...
    if (observedRate < ENGINE_CONSTANTS.threePointLowVolumeThreshold) {
        return Math.min(ENGINE_CONSTANTS.threePointLowVolumeCap, Math.max(0, observedRate + eraThreeRate * ENGINE_CONSTANTS.threePointLowVolumeEraPull));
    }
    return Math.min(0.65, Math.max(0.01, observedRate * (1 - ENGINE_CONSTANTS.threePointEraPull) +
        eraThreeRate * ENGINE_CONSTANTS.threePointEraPull));
}
export function blendedZoneWeights(shooter: SimulationPlayer, profile: EraSimulationProfile): number[] {
    const f = shooter.tendencies;
...
export function rescaleZoneWeights(blend: readonly number[], targetThreeRate: number): number[] {
    ...
    const currentTwo = Math.max(1e-9, weights.reduce((sum, value, index) => (index < 3 ? sum + value : sum), 0));
...
export function twoPointZoneSharesFromBlend(weights: readonly number[]): [
    number,
    number,
    number
] {
...
function scaledZoneWeight(index: number, base: readonly number[], action: ActionType, driveRate: number): number {
    if (index === 0) {
    ...
    if (index === 1)
        return (base[1] ?? 0) * (action === 'postUp' ? 1.05 : 1);
    return base[index] ?? 0;
}
export function pickZone(action: ActionType, prep: ZonePrep, rng: Rng): ShotZone {
    ...
    for (let i = 0; i < SHOT_ZONES.length; i += 1) {
        ...
        if (roll < w) {
            ...
            if (zone === undefined)
                throw new Error('pickZone: index out of range');
            return zone;
        }
        roll -= w;
    }
