...
import { DEFAULT_ERA_SIM_PROFILE, buildGameSimulationInput, buildRolesTeam, seedFromString, } from '@hoop-rush/test-fixtures';
...
function measureRoles(team: SimulationTeam): Map<string, RoleAccumulator> {
    ...
    for (let i = 0; i < SEEDS; i += 1) {
        ...
        for (const side of [result.home, result.away] as const) {
            ...
            const misses = (t: GameResult['home']) => t.box.fieldGoals.attempted -
...
describe('player-role behavior (roles lineup)', () => {
    ...
    it('keeps every role gate inside the frozen era targets', () => {
        const target = (key: string) => DEFAULT_ERA_SIM_PROFILE.targets.playerRoles.find((r) => r.key === key);
        const checks: Array<[
            string,
            number
        ]> = [
        ...
        for (const [key, observed] of checks) {
            ...
            expect(observed, `${key}: observed ${observed.toFixed(4)} outside ${(value - tolerance).toFixed(4)}..${(value + tolerance).toFixed(4)}`).toBeGreaterThanOrEqual(value - tolerance);
