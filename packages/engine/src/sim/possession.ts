...
import { pickAction, pickDefender, pickInitiator, pickAssister, pickShot, pickZone, isThreePointZone, zoneSkillRating, type ActionType, type PossessionStartType, } from './usage.ts';
...
import { freeThrowsForZone, freeThrowProbability, nonShootingFoulProbability, pickFouler, pickFreeThrowShooter, shootingFoulProbability, } from './fouls.ts';
...
function assistProbability(ctx: TripContext, passer: SimulationPlayer, action: ActionType, zone: ShotZone, shooter: SimulationPlayer, effectsAdjustment = 0): number {
    return assistProbabilityPure(ctx.profile, ctx.passingAnchorFactor, passer, action, zone, shooter, effectsAdjustment);
}
function creditAssist(ctx: TripContext, offenseSide: SideIndex, team: SimulationTeam, shooter: SimulationPlayer, initiator: SimulationPlayer, action: ActionType, zone: ShotZone, passed: boolean): void {
    ...
    if (!ctx.rng.chance(assistProbability(ctx, passer, action, zone, shooter, effectsAdjustment / 1000000))) {
        return;
    }
