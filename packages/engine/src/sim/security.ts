import type { EraSimulationProfile, SimulationPlayer, SimulationTeam, } from '@hoop-rush/data-contracts';
import type { Rng } from './rng.ts';
import { ENGINE_CONSTANTS } from './constants.ts';
export function defenderPressure(defender: SimulationPlayer): number {
    return ((defender.ratings.perimeterDefense * 0.5 +
...
export function turnoverProbability(handler: SimulationPlayer, pressure: number, possEstimatePerTrip: number, profile: EraSimulationProfile, extraPressure = 0, effectsAdjustment = 0): number {
    ...
    const raw = eraBase * (1 - c.turnoverObservedBlend) +
...
export function isSteal(rng: Rng, stealAbility: number, profile: EraSimulationProfile): boolean {
    const p = profile.parameters.stealShareOfTurnovers *
...
export function stealerWeights(defense: SimulationTeam): number[] {
    return defense.players.map((d) => Math.max(0.5, d.ratings.steal * (0.6 + d.tendencies.stealAttemptRate / 20)));
}
export function pickStealer(players: readonly SimulationPlayer[], weights: readonly number[], rng: Rng): SimulationPlayer {
