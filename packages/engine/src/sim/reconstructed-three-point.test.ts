...
describe('threePointTarget resolution order (spec/12)', () => {
    ...
    it('keeps the established observed-volume path unchanged and ignores the profile', () => {
        ...
        const expected = Math.min(0.65, Math.max(0.01, 0.15 * (1 - ENGINE_CONSTANTS.threePointEraPull) +
...
describe('makeProbability reconstructed three-point path (spec/12)', () => {
    ...
    it('pins the make probability to accuracyConservative plus context, minus calibration', () => {
        ...
        const result = makeProbability(shooter, averageDefender(), profile, zone, 'spotUp', 300, shotPrep());
        const expected = Math.max(reconstructedProfile().zoneFloors[zone], Math.max(0.03, 0.28 -
            contestPenalty(averageDefender(), zone) +
            ENGINE_CONSTANTS.threePointCalibrationOffset));
        expect(result).toBeCloseTo(expected, 10);
        expect(result).toBeLessThan(0.35);
        const unanchored = makeProbability(buildSimulationPlayer(), averageDefender(), profile, zone, 'spotUp', 300, shotPrep());
    ...
    it('floors at the profile zone floor when the raw value drops below it', () => {
        ...
        const result = makeProbability(shooter, elite, profile, 'aboveBreakThree', 'spotUp', 300, shotPrep());
    ...
    it('applies each profile zone floor to its own zone', () => {
        ...
        const corner = makeProbability(shooter, elite, profile, 'cornerThree', 'spotUp', 300, shotPrep());
        const aboveBreak = makeProbability(shooter, elite, profile, 'aboveBreakThree', 'spotUp', 300, shotPrep());
