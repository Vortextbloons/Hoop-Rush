...
function playerAt(team: SimulationTeam, index: number) {
  ...
  if (player === undefined) throw new Error(`fixture player ${String(index)} missing`);
  return player;
}

describe('assigned-position responsibility modifiers', () => {
  it('ships as m3-engine-v11 with the responsibility constants versioned', () => {
    expect(ENGINE_VERSION).toBe('m3-engine-v11');
    expect(ENGINE_CONSTANTS.version).toBe('m3-engine-v11');
  ...
  it('scales initiator weights by the assigned-slot initiation modifier', () => {
    ...
    expect(
      initiatorWeight(creator, PERMUTATION_ONE, gMods) /
        initiatorWeight(creator, PERMUTATION_ONE, identityModifiers),
    ).toBeCloseTo(ENGINE_CONSTANTS.positionResponsibility.G.initiation, 12);
    const cMods = responsibilityModifiersForSlot(4);
    expect(
      initiatorWeight(creator, PERMUTATION_ONE, cMods) /
        initiatorWeight(creator, PERMUTATION_ONE, identityModifiers),
    ).toBeCloseTo(ENGINE_CONSTANTS.positionResponsibility.C.initiation, 12);
  });
  ...
  it('scales handler, roll-man, and post-up action weights only', () => {
    ...
    expect(weighted[1] ?? 0).toBeCloseTo(
      (plain[1] ?? 0) * ENGINE_CONSTANTS.positionResponsibility.G.pnrHandler,
      12,
    );
    expect(weighted[2] ?? 0).toBeCloseTo(
      (plain[2] ?? 0) * ENGINE_CONSTANTS.positionResponsibility.G.rollMan,
      12,
    );
    expect(weighted[3] ?? 0).toBeCloseTo(
      (plain[3] ?? 0) * ENGINE_CONSTANTS.positionResponsibility.G.postUp,
      12,
    );
  ...
  it('never manufactures an action from a zero tendency', () => {
    ...
    expect(
      initiatorWeight(zero, buildLegalSimulationTeam(), responsibilityModifiersForSlot(4)),
    ).toBeGreaterThan(0);
  });
  ...
  it('scales roll-man teammate selection by the teammate slot modifier', () => {
    ...
    const rollWeighted = teammateShotWeights(
      PERMUTATION_ONE,
      creator,
      'pickAndRollRoll',
      rollMods,
    ).weights;
    const rollPlain = teammateShotWeights(
      PERMUTATION_ONE,
      creator,
      'pickAndRollRoll',
      new Map(),
    ...
    expect(rollWeighted[rimIndex] ?? 0).toBeCloseTo(
      (rim ?? 0) * ENGINE_CONSTANTS.positionResponsibility.C.rollMan,
      12,
    );
  ...
  it('scales rebound attribution weights by the assigned-slot rebounding modifier', () => {
    ...
    expect(offMods[4] ?? 0).toBeCloseTo(
      (rimOff ?? 0) * ENGINE_CONSTANTS.positionResponsibility.C.rebounding,
      12,
    );
    expect(defMods[4] ?? 0).toBeCloseTo(
      (rimDef ?? 0) * ENGINE_CONSTANTS.positionResponsibility.C.rebounding,
      12,
    );
    expect(offMods[0] ?? 0).toBeCloseTo(
      (off[0] ?? 0) * ENGINE_CONSTANTS.positionResponsibility.G.rebounding,
      12,
    );
  });
  ...
  it('derives all modifier tables purely from the player snapshot and slot index', () => {
    ...
    PERMUTATION_ONE.players.forEach((player) => {
      expect(b.positionModifiers.get(player.playerId)).toEqual(
        a.positionModifiers.get(player.playerId),
      );
    });
  });
});
