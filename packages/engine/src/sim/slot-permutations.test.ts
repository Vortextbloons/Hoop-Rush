...
import { buildGameSimulationInput, buildSlotPermutationTeams, seedFromString, } from '@hoop-rush/test-fixtures';
...
function runPermutation(team: SimulationTeam, seeds: string[]) {
    const games: GameResult[] = seeds.map((seed) => simulateGame(buildGameSimulationInput({
    ...
    return {
        ...
        diagnostics: a.diagnostics && b.diagnostics
            ? {
                ...
                assistOpportunities: a.diagnostics.assistOpportunities + b.diagnostics.assistOpportunities,
                offensiveReboundChances: a.diagnostics.offensiveReboundChances + b.diagnostics.offensiveReboundChances,
                defensiveReboundChances: a.diagnostics.defensiveReboundChances + b.diagnostics.defensiveReboundChances,
...
function threePointRate(player: PlayerBoxScore): number {
    return (player.threes.attempted / Math.max(1, player.threes.attempted + player.fieldGoals.attempted));
}
function teamUsage(playerBoxes: Iterable<PlayerBoxScore>): number {
    let total = 0;
    for (const player of playerBoxes)
        total += player.diagnostics?.usage ?? 0;
    return total;
}
describe('assigned-position responsibility across legal slot permutations', () => {
    it('shifts usage, rebound attribution, rim contests, and blocks by player ID', () => {
        ...
        const box = (players: Map<string, PlayerBoxScore>, playerId: string) => {
            ...
            if (player === undefined)
                throw new Error(`slot permutation fixture missing ${playerId}`);
            return player;
        };
        const avg = (results: ReturnType<typeof runPermutation>[], pick: (p: PlayerBoxScore) => number) => results.reduce((sum, result) => sum + pick(box(result.players, 'p-slot-rim')), 0) /
            results.length;
        ...
        const postAtCenter = [at(byPermutation, 3), at(byPermutation, 5)].reduce((sum, result) => sum + postBlocks(result.players), 0);
        const postAtForward = [0, 1, 2, 4].reduce((sum, i) => sum + postBlocks(at(byPermutation, i).players), 0);
        ...
        for (const playerId of [
            ...
            usageRedistribution += Math.abs(usageShare(box(at(byPermutation, 0).players, playerId), naturalUsage) -
        ...
        for (const [index, result] of byPermutation.entries()) {
            ...
            for (const playerId of [
                ...
                if (a.rebounds.defensive !== b.rebounds.defensive || a.blocks !== b.blocks)
                    differs = true;
            }
    ...
    it('preserves the shooter shot profile across permutations', () => {
        ...
        const rates = PERMUTATIONS.map((team) => {
            ...
            if (shooter === undefined)
                throw new Error('fixture missing shooter');
    ...
    it('keeps the paired win-rate spread across permutations within 6 percentage points', () => {
        ...
        for (let i = 0; i < PERMUTATIONS.length; i += 1) {
            ...
            for (const seed of seeds) {
                const result = simulateGame(buildGameSimulationInput({ seed: seedFromString(seed), home: a, away: b }), ctx);
