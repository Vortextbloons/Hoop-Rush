...
export function shootingFoulProbability(shooter: SimulationPlayer, defender: SimulationPlayer, zone: ShotZone, profile: EraSimulationProfile): number {
    ...
    const anchorFactor = observedRate === undefined
        ? 1
        : Math.min(1.8, Math.max(0.4, observedRate / Math.max(1e-9, p.leagueFtaPerFga)));
    const drawsFouls = tendencyFactor * (1 - ENGINE_CONSTANTS.observedFoulDrawBlend) +
...
export function foulerWeights(defense: SimulationTeam): number[] {
    return defense.players.map((d) => Math.max(0.5, ((d.ratings.strength + d.ratings.interiorDefense) / 2) *
        (0.55 + Math.min(20, d.tendencies.foulRate) / 10)));
}
export function pickFouler(players: readonly SimulationPlayer[], weights: readonly number[], rng: Rng): SimulationPlayer {
    return rng.weightedPick(players, weights);
}
export function freeThrowShooterWeights(team: SimulationTeam): number[] {
    return team.players.map((p) => Math.max(0.5, p.tendencies.freeThrowRate) * (0.6 + 0.8 * (p.ratings.freeThrow / 100)));
}
export function pickFreeThrowShooter(players: readonly SimulationPlayer[], weights: readonly number[], rng: Rng): SimulationPlayer {
    return rng.weightedPick(players, weights);
}
export function freeThrowsForZone(zone: ShotZone): number {
    return isThreePointZone(zone) ? 3 : 2;
}
export function freeThrowProbability(shooter: SimulationPlayer, profile: EraSimulationProfile): number {
    ...
    const probability = observedProbability === undefined
        ? ratingProbability
