...
import { MAX_PERIODS_HARD_CAP, OVERTIME_PERIOD_SECONDS, REGULATION_PERIOD_SECONDS, } from './periods.ts';
import { createGameState, createTripContext, PossessionStepper, resolveTrip, type PossessionStep, type TripContext, } from './possession.ts';
...
function driveGame(input: GameSimulationInput, mode: DriveMode): DriveResult {
    ...
    for (let period = 0; period < MAX_PERIODS_HARD_CAP; period += 1) {
        if (period > 0) {
            if (period >= 4 && recorder.sides[0].points !== recorder.sides[1].points)
                break;
            recorder.nextPeriod();
        ...
        while (secondsRemaining > 0) {
            ...
            if (mode === 'resolve') {
                ...
                for (;;) {
                    ...
                    if (step.finished || step.periodEnded)
                        break;
                    if (mode === 'pause' && step.pause && step.ended)
                        break;
                }
                terminal = step;
            }
            if (terminal.periodEnded && state.secondsRemaining > 0)
                state.secondsRemaining = 0;
            ...
            if (terminal.ended)
                offense = (1 - offense) as SideIndex;
        }
    }
    return {
        trips,
            ...
            for (const [tripIndex, steps] of tripSteps.entries()) {
                for (const [index, step] of steps.entries()) {
                    ...
                    if (step.periodEnded)
                        continue;
                    if (step.ended) {
                        expect(index, `${seed} trip ${String(tripIndex)} step ${String(index)}`).toBe(steps.length - 1);
                    }
                    else {
                        expect(index, `${seed} trip ${String(tripIndex)}`).toBeLessThan(steps.length - 1);
                        expect(steps[index + 1]?.pause, `${seed} trip ${String(tripIndex)} step ${String(index + 1)}`).toBe(false);
                    }
                }
            }
        }
    ...
    it('never splits a made basket from its free throw (and-one atomicity)', () => {
        for (const seed of GAME_SEEDS) {
            ...
            for (const [tripIndex, totals] of perTripTotals.entries()) {
                ...
                if (afterPause === undefined || last === undefined || first === undefined)
                    continue;
                const scoredBeforePause = afterPause.points > first.points;
                const ftaGrewAfterPause = last.fta > afterPause.fta;
                expect(scoredBeforePause && ftaGrewAfterPause, `${seed} trip ${String(tripIndex)} splits a made basket from its free throws`).toBe(false);
            }
        }
    });
