...
function team(teamId: string, players: PlayerBoxScore[], points: number, overrides: Partial<TeamResult> = {}): TeamResult {
...
describe('buildFacts usage fact', () => {
    it('picks the highest-usage player, not the top scorer', () => {
        ...
        const rest = Array.from({ length: 3 }, (_, i) => player({ playerId: `p-rest-${String(i)}`, points: 4, diagnostics: usageDiag(0) }));
        const home = team('user', [scorer, workhorse, ...rest], 50);
        const away = team('away', Array.from({ length: 5 }, (_, i) => player({ playerId: `p-away-${String(i)}`, diagnostics: usageDiag(1) })), 40);
    ...
    it('emits no usage fact below the share threshold', () => {
        const players = Array.from({ length: 5 }, (_, i) => player({ playerId: `p-${String(i)}`, points: 10, diagnostics: usageDiag(7) }));
        const home = team('user', players, 50);
        const away = team('away', Array.from({ length: 5 }, (_, i) => player({ playerId: `p-away-${String(i)}`, diagnostics: usageDiag(7) })), 40);
        const result = buildFacts(game(home, away, 'home'));
    ...
    it('falls back to the usage formula for legacy records without diagnostics', () => {
        ...
        const rest = Array.from({ length: 4 }, (_, i) => player({ playerId: `p-rest-${String(i)}`, points: 6, diagnostics: undefined }));
        const home = team('user', [workhorse, ...rest], 32);
        const away = team('away', Array.from({ length: 5 }, (_, i) => player({ playerId: `p-away-${String(i)}`, diagnostics: undefined })), 30);
        const result = buildFacts(game(home, away, 'home'));
        const usageFact = result.find((fact) => fact.kind === 'usage');
