...
import { ACTION_TYPES, actionWeights, defenderBase, passProbability, teamInitiatorWeights, teammateShotWeights, zonePrep, type DefenderBase, type TeammateShots, type ZonePrep, } from './usage.ts';
...
import { responsibilityModifiersForSlot, type PositionResponsibilityModifiers, } from './position-responsibilities.ts';
...
export function prepareTeam(team: SimulationTeam, profile: EraSimulationProfile): TeamPrep {
    ...
    players.forEach((player, slot) => {
        ...
        passPByPlayer.set(key, ACTION_TYPES.map((action) => passProbability(player, action)));
