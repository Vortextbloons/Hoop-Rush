...
export function auditSideAccounting<P extends AccountingPlayerInput>(players: readonly P[], box: AccountingBoxInput, teamShotZones: readonly AccountingShotZones[], keyOf: (player: P) => string): SideAccountingViolations {
    const sumOf = (select: (p: P) => number): number => players.reduce((acc, p) => acc + select(p), 0);
    ...
    if (fgm > fga)
        makesExceed.push('fieldGoal');
    if (tpm > tpa)
        makesExceed.push('three');
    if (ftm > fta)
        makesExceed.push('freeThrow');
    ...
    const playerDiag = (select: (d: AccountingPlayerDiagnostics) => number): number => players.reduce((acc, p) => acc + (p.diagnostics ? select(p.diagnostics) : 0), 0);
    ...
    if (hasAnyDiagnostics) {
        for (const zone of teamShotZones) {
            zoneSplits.push({
                zone: zone.zone,
                playerAttempts: playerDiag((p) => p.shotZones.find((z) => z.zone === zone.zone)?.attempts ?? 0),
    ...
    return {
        ...
        reboundBucketsOk: box.rebounds.offensive + box.rebounds.defensive + box.rebounds.team === box.rebounds.total,
        reconciliations,
