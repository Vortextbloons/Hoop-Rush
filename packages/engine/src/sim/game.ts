...
import { MAX_PERIODS_HARD_CAP, OVERTIME_PERIOD_SECONDS, REGULATION_PERIOD_SECONDS, overtimePeriodsOf, } from './periods.ts';
...
function playFixedFivePeriods(rng: ReturnType<EngineContext['rngFactory']>, recorder: GameRecorder, state: ReturnType<typeof createGameState>, tripContext: ReturnType<typeof createTripContext>): {
    ...
    for (;;) {
        if (period > 0) {
            ...
            if (period >= MAX_PERIODS_HARD_CAP) {
                throw new Error(`simulation: exceeded hard cap ${String(MAX_PERIODS_HARD_CAP)} periods without a winner (${String(recorder.sides[0].points)}-${String(recorder.sides[1].points)})`);
            }
            recorder.nextPeriod();
    ...
    if (homeScore === awayScore) {
        throw new Error(`simulation: game ended tied ${String(homeScore)}-${String(awayScore)} after ${String(overtimePeriods)} overtime periods`);
    }
