...
export function playFixedFivePeriods(rng: ReturnType<EngineContext['rngFactory']>, recorder: GameRecorder, state: ReturnType<typeof createGameState>, tripContext: ReturnType<typeof createTripContext>): {
    ...
    for (;;) {
        if (period > 0) {
            if (period >= 4) {
                if (recorder.sides[0].points !== recorder.sides[1].points)
                    break;
            }
