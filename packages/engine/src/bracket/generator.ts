import type { BracketOpponent, DifficultyProfile, EraSimulationProfile, OpponentBracket, OpponentTeam, PositionUnion, Seed, SimulationPlayer, SimulationAnchors, SimulationRatings, SimulationTeam, SimulationTendencies, } from '@hoop-rush/data-contracts';
...
export function generateBracket(options: BracketGenerationOptions): OpponentBracket {
    ...
    if (!franchiseIds.includes(options.openingOpponent.teamId)) {
        throw new Error(`opening opponent team ${options.openingOpponent.teamId} has no candidate pool`);
    }
    ...
    for (const candidates of options.candidates) {
        ...
        if (guards.length < 2 || forwards.length < 2 || centers.length < 1) {
            throw new Error(`${candidates.franchiseId} cannot form a legal lineup (${String(guards.length)} guards, ${String(forwards.length)} forwards, ${String(centers.length)} centers)`);
        }
    ...
    if (selectableFranchises.length !== 29) {
        throw new Error(`expected 29 selectable franchises (got ${String(selectableFranchises.length)})`);
    }
    ...
    function usedPlayerIds(excludingFranchise: string | null): Set<string> {
        ...
        for (const player of options.openingOpponent.players)
            used.add(player.playerId);
        for (const [franchiseId, current] of selected) {
            if (franchiseId === excludingFranchise)
                continue;
    ...
    function proposalScore(proposal: Proposal, target: number, used: Set<string>): {
        score: number;
        conflicts: number;
    } {
        const conflicts = proposal.players.reduce((count, p) => count + (used.has(p.playerId) ? 1 : 0), 0);
    ...
    for (let attempt = 0; attempt < 6; attempt += 1) {
        const shuffled = shuffle(selectableFranchises, createRng(`${seed}:select:order:${String(attempt)}`));
        selected.clear();
        const rankProposals = (proposals: readonly Proposal[], target: number): Proposal[] => [...proposals].sort((a, b) => inBand(a.strength, population, band) === inBand(b.strength, population, band)
        ...
        for (let round = 0; round < 100; round += 1) {
            ...
            for (const franchiseId of shuffled) {
                ...
                const proposals = proposalsByFranchise.get(franchiseId) ?? [];
                const best = [...proposals].sort((a, b) => proposalScore(a, current.target, used).score -
                    proposalScore(b, current.target, used).score || a.strength - b.strength)[0];
            ...
            const bracketMedian = (): number => medianOf(bracketPercentileList(selected, openingPercentileOfPopulation, population));
            ...
            const medianAdjustment = (direction: -1 | 1): boolean => {
                ...
                for (const [franchiseId, current] of selected) {
                    const used = usedPlayerIds(franchiseId);
                    const candidates = (proposalsByFranchise.get(franchiseId) ?? []).filter((p) => direction === 1
                        ? p.strength < current.proposal.strength
                        : p.strength > current.proposal.strength);
                    const ranked = [...candidates].sort((a, b) => proposalScore(a, current.target, used).score -
                    ...
                    const gap = movedMedian < medianBand[0]
        ...
        for (const [franchiseId, current] of selected) {
            ...
            const used = usedPlayerIds(franchiseId);
            const conflicts = current.proposal.players.reduce((count, p) => count + (used.has(p.playerId) ? 1 : 0), 0);
    ...
    }
    {
        ...
        if (conflictDetails.length > 0) {
            throw new Error(`selection could not resolve player duplication (${String(conflictDetails.length)} conflicts: ${conflictDetails.join(', ')})`);
        }
    }
    ...
    for (const franchiseId of selectableFranchises) {
        ...
        for (const player of proposal.players) {
            if (usedInBracket.has(player.playerId)) {
                throw new Error(`player ${player.playerId} appears in more than one opponent (${franchiseId})`);
            }
    ...
    const selectedPercentiles = [...selected.values()].map((s) => percentileOf(s.proposal.strength, population));
    const minP = Math.min(...selectedPercentiles);
    const maxP = Math.max(...selectedPercentiles);
    if (median < medianBand[0] || median > medianBand[1]) {
        failures.push(`bracket median percentile ${median.toFixed(3)} outside ${medianBand[0].toFixed(2)}..${medianBand[1].toFixed(2)}`);
    }
    if (minP < band[0] || maxP > band[1]) {
        failures.push(`selected percentiles span ${minP.toFixed(3)}..${maxP.toFixed(3)} outside ${band[0].toFixed(2)}..${band[1].toFixed(2)}`);
    }
    if (failures.length > 0) {
        const percentileReport = opponents
            .map((o) => `${o.opponentId}=${o.strength.percentile.toFixed(3)}`)
            .join(' ');
        throw new Error(`generated bracket fails validation: ${failures.slice(0, 8).join('; ')} :: ${percentileReport}`);
    }
    return opponentBracketSchema.parse(bracket);
}
function pickFive(rng: ReturnType<typeof createRng>, guards: readonly BracketCandidatePlayer[], forwards: readonly BracketCandidatePlayer[], centers: readonly BracketCandidatePlayer[], bias: 'strong' | 'mid' | 'uniform', isExhausted: (player: BracketCandidatePlayer) => boolean): BracketCandidatePlayer[] | null {
    const chosen: BracketCandidatePlayer[] = [];
    const used = new Set<string>();
    const picks: ReadonlyArray<{
        pool: readonly BracketCandidatePlayer[];
        count: number;
    }> = [
        { pool: guards, count: 2 },
    ...
    for (const { pool, count } of picks) {
        ...
        for (let i = 0; i < count; i += 1) {
            if (available.length === 0)
                return null;
            const weight = (score: number) => bias === 'strong'
...
function inBand(strength: number, population: readonly number[], band: [
    number,
    number
]): boolean {
...
function bestDirectionGap(median: number, band: [
    number,
    number
]): number {
    if (median < band[0])
        return band[0] - median;
    if (median > band[1])
        return median - band[1];
    return 0;
}
function bracketPercentileList(selected: Map<string, {
    proposal: Proposal;
    target: number;
}>, openingPercentile: number, population: readonly number[]): number[] {
    return [
        openingPercentile,
