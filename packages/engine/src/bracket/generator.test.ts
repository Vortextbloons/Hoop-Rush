...
describe('generateBracket (propose-review-freeze)', () => {
    ...
    it('throws when a franchise cannot form a legal lineup', () => {
        ...
        const broken = options.candidates.map((candidate) => candidate.franchiseId === 'hawks'
            ? {
                ...candidate,
                players: candidate.players.filter((p) => playableSlotGroups(p.positions).includes('G')),
            }
            : candidate);
        expect(() => generateBracket({ ...options, candidates: broken })).toThrow(/cannot form a legal lineup/);
    }, 60000);
});
