...
describe('generateSchedule (spec/01 fixed schedule)', () => {
    it('produces 82 games with game one as the opening opponent', () => {
        ...
        const schedule = generateSchedule(ids, 'lakers-1990s-opening', seedFromString('fixture-schedule'));
    ...
    it('assigns 22 opponents three games and eight opponents two games', () => {
        const schedule = generateSchedule(opponentIds(), 'lakers-1990s-opening', seedFromString('fixture-schedule'));
    ...
    it('is deterministic for the same seed', () => {
        const a = generateSchedule(opponentIds(), 'lakers-1990s-opening', seedFromString('fixture-schedule'));
        const b = generateSchedule(opponentIds(), 'lakers-1990s-opening', seedFromString('fixture-schedule'));
        expect(a).toEqual(b);
    });
    it('differs across seeds', () => {
        const a = generateSchedule(opponentIds(), 'lakers-1990s-opening', seedFromString('fixture-schedule'));
        const b = generateSchedule(opponentIds(), 'lakers-1990s-opening', seedFromString('other-schedule'));
        expect(JSON.stringify(a)).not.toBe(JSON.stringify(b));
    });
    it('rejects wrong-sized opponent sets', () => {
        expect(() => generateSchedule(opponentIds().slice(0, 29), 'lakers-1990s-opening', seedFromString('x'))).toThrow(/exactly 30/);
        expect(() => generateSchedule([...opponentIds(), 'extra'], 'lakers-1990s-opening', seedFromString('x'))).toThrow(/exactly 30/);
    });
    it('rejects an opening opponent outside the bracket', () => {
        expect(() => generateSchedule(opponentIds(), 'nope', seedFromString('x'))).toThrow(/not in the bracket/);
    });
