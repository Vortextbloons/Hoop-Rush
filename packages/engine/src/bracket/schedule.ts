...
export function generateSchedule(opponentIds: readonly string[], openingOpponentId: string, seed: Seed): BracketScheduleEntry[] {
    ...
    for (const id of opponentIds)
        counts.set(id, twoGames.has(id) ? 2 : 3);
    ...
    for (const id of distinct) {
        for (let i = 0; i < (counts.get(id) ?? 0); i += 1)
            remaining.push(id);
    }
...
export function auditScheduleEntries(schedule: readonly BracketScheduleEntry[], knownOpponentIds: ReadonlySet<string>): ScheduleAuditFacts {
    const counts = new Map<string, number>();
    ...
    for (let index = 0; index < schedule.length; index += 1) {
        ...
        if (entry === undefined)
            continue;
        const repeatsPreviousByPosition = index > 0 && entry.opponentId === schedule[index - 1]?.opponentId;
        const repeatsPreviousByGameNumber = entry.gameNumber > 1 && entry.opponentId === schedule[entry.gameNumber - 2]?.opponentId;
        entries.push({
