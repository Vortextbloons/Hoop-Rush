import type { Lineup, LineupAssignment, PositionUnion, SlotGroup, SlotIndex, } from '@hoop-rush/data-contracts';
...
export function assignLineup(players: ReadonlyArray<{
    playerId: string;
    ...
    function search(index: number, used: Map<SlotIndex, string>): LineupAssignment[] | null {
        ...
        for (const slotIndex of slots) {
            if (used.has(slotIndex))
                continue;
            if (!canFillSlot(player.positions, slotIndex))
                continue;
