...
export type Archetype = 'primaryCreator' | 'secondaryCreator' | 'floorSpacer' | 'rimRunner' | 'postAnchor' | 'glue';
...
export function classifyArchetype(player: SimulationPlayer): Archetype {
    ...
    if (t.usageRate >= 28 && creationScore(player) >= 0.6)
        return 'primaryCreator';
    if (t.usageRate >= 21)
        return 'secondaryCreator';
    if (t.threePointRate >= 25 && spacingScore(player) >= 0.55)
        return 'floorSpacer';
    if (t.rimFrequency >= 40 && interiorScoringScore(player) >= 0.55)
        return 'rimRunner';
    if (t.postUpRate >= 15)
        return 'postAnchor';
    return 'glue';
