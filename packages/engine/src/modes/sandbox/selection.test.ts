...
import { createChallenge, simulateChallenge, type ChallengeCreation, type ChallengeCreationBase, } from '../../challenge/commands.ts';
import { deriveAttemptSeed } from '../../challenge/seeds.ts';
import { BEST_OF_ATTEMPTS, chooseBestRun, chooseBestRunSeed, scoreRun, simulateChallengeBestOf, } from './selection.ts';
...
describe('sandbox selection', () => {
    ...
    it('chooses one of the derived attempt seeds', () => {
        const creation = fixtureCreation();
        const chosen = simulateChallengeBestOf(creation, creation.profile, context);
        const attemptSeeds = Array.from({ length: BEST_OF_ATTEMPTS }, (_, attempt) => deriveAttemptSeed(creation.runSeed, attempt));
    ...
    it('chosen run equals a direct simulation of the chosen seed', () => {
        const creation = fixtureCreation();
        const chosen = simulateChallengeBestOf(creation, creation.profile, context);
        const direct = simulateChallenge(createChallenge({ ...creation, runSeed: chosen.runSeed }), creation.profile, context);
...
describe('chooseBestRunSeed (worker best-of)', () => {
    it('matches simulateChallengeBestOf for the same creation', () => {
        ...
        const chosen = chooseBestRunSeed(createChallenge({ ...creation, runSeed: creation.runSeed }), creation.profile, context);
