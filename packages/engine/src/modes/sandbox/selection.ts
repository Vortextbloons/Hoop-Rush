...
import { createChallenge, createGameInput, simulateChallenge, type ChallengeCreation, } from '../../challenge/commands.ts';
...
export function simulateChallengeBestOf(creation: ChallengeCreation, profile: EraSimulationProfile, context: EngineContext, attempts: number = BEST_OF_ATTEMPTS): ChallengeRun {
...
export function chooseBestRunSeed(run: ChallengeRun, profile: EraSimulationProfile, context: EngineContext): BestOfChoice {
    let best: {
        seed: Seed;
        score: RunScore;
    } | null = null;
