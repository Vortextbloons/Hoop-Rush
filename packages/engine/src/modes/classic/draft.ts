import type { ClassicCompletedDraft, ClassicDraftCatalog, ClassicDraftCatalogEntry, ClassicDraftState, ClassicPick, ClassicRollContext, ClassicVariant, EraSimulationProfile, OpponentBracket, PlayerId, Position, Seed, SimulationPlayer, SlotGroup, SlotIndex, } from '@hoop-rush/data-contracts';
import { CLASSIC_DRAFT_SCHEMA_VERSION, CLASSIC_ROLL_VERSION, LINEUP_STRUCTURE, classicDraftCatalogSchema, } from '@hoop-rush/data-contracts';
...
export function classicRollSeed(seed: Seed, version: string, kind: ClassicRollKind, round: number): string {
...
export function sortClassicCatalog(catalog: ClassicDraftCatalog): ClassicDraftCatalog {
    return [...catalog].sort((a, b) => a.franchiseId < b.franchiseId
        ? -1
...
function catalogPlayer(catalog: ClassicDraftCatalog, franchiseId: string, eraId: string, playerId: PlayerId): {
    playerId: PlayerId;
    positions: Position[];
} | null {
...
function displacementTargetFor(catalog: ClassicDraftCatalog, incumbent: ClassicPick, targetSlot: SlotIndex, vacatingSlot: SlotIndex | null, picks: ClassicPick[]): SlotIndex | null {
    const incumbentPlayer = catalogPlayer(catalog, incumbent.franchiseId, incumbent.eraId, incumbent.playerId);
    if (!incumbentPlayer)
        return null;
...
export function classicRollCandidates(catalog: ClassicDraftCatalog, state: ClassicDraftState, kind: ClassicRollKind): ClassicDraftCatalogEntry[] {
    if (kind !== 'initial') {
        if (state.roll === null) {
    ...
    return sortClassicCatalog(catalog.filter((entry) => {
        if (kind === 'franchise-reroll') {
            if (state.roll === null)
                return false;
            if (entry.eraId !== state.roll.eraId)
                return false;
            if (entry.franchiseId === state.roll.franchiseId)
                return false;
        }
        if (kind === 'era-reroll') {
            if (state.roll === null)
        ...
        return entry.players.some((player) => !draftedIds.has(player.playerId) &&
            player.positions.some((position) => requirements.has(slotGroupOf(position))));
    }));
}
export function classicRerollAvailable(state: ClassicDraftState, kind: 'franchise' | 'era', catalog: ClassicDraftCatalog): boolean {
    if (state.roll === null)
        return false;
...
export function rollClassicPair(seed: Seed, round: number, kind: ClassicRollKind, candidates: ClassicDraftCatalogEntry[], context: EngineContext): ClassicRollContext {
...
export function createClassicDraft(input: ClassicDraftInput, context: EngineContext): ClassicDraftState {
...
export function rerollClassicFranchise(state: ClassicDraftState, catalog: ClassicDraftCatalog, context: EngineContext): ClassicDraftState {
    ...
    if (candidates.length === 0) {
        throw new Error(`no alternative franchise for era ${roll.eraId} in round ${String(state.round)}`);
    }
    const nextRoll = rollClassicPair(state.seed, state.round, 'franchise-reroll', candidates, context);
    return {
        ...state,
        roll: nextRoll,
...
export function rerollClassicEra(state: ClassicDraftState, catalog: ClassicDraftCatalog, context: EngineContext): ClassicDraftState {
    if (state.status !== 'drafting') {
    ...
    if (candidates.length === 0) {
        throw new Error(`no alternative era for franchise ${roll.franchiseId} in round ${String(state.round)}`);
    }
...
export function draftClassicPlayer(state: ClassicDraftState, catalog: ClassicDraftCatalog, input: ClassicDraftPlayerInput, context: EngineContext): ClassicDraftState {
    if (state.status !== 'drafting') {
    ...
    if (incumbentAtSlot) {
        const target = displacementTargetFor(catalog, incumbentAtSlot, input.slotIndex, null, state.picks);
        if (target === null) {
            throw new Error(`slot ${String(input.slotIndex)} is already filled`);
        }
        const picksWithDisplacement = state.picks.map((pick) => pick.playerId === incumbentAtSlot.playerId ? { ...pick, slotIndex: target } : pick);
...
export function repositionClassicPlayer(state: ClassicDraftState, catalog: ClassicDraftCatalog, input: ClassicRepositionInput): ClassicDraftState {
    ...
    if (!incumbent) {
        return {
            ...state,
            picks: state.picks.map((p) => p.playerId === input.playerId ? { ...p, slotIndex: input.slotIndex } : p),
        };
    }
    const incumbentPlayer = catalogPlayer(catalog, incumbent.franchiseId, incumbent.eraId, incumbent.playerId);
    if (!incumbentPlayer) {
        ...
        return {
            ...
            picks: state.picks.map((p) => {
                if (p.playerId === input.playerId)
                    return { ...p, slotIndex: input.slotIndex };
                if (p.playerId === incumbent.playerId)
                    return { ...p, slotIndex: pick.slotIndex };
                return p;
            }),
        };
    }
    const target = displacementTargetFor(catalog, incumbent, input.slotIndex, pick.slotIndex, state.picks);
    if (target === null) {
...
export function createClassicChallenge(draft: ClassicDraftState, env: ClassicChallengeEnvironment): ClassicChallengeCreation {
    ...
    for (let slotIndex = 0; slotIndex < 5; slotIndex += 1) {
        ...
        if (!player || player.playerId !== pick.playerId) {
            throw new Error(`slot ${String(slotIndex)} player ${player?.playerId ?? 'missing'} does not match the draft pick ${pick.playerId}`);
        }
    }
    ...
    if (!validation.ok) {
        throw new Error(`classic lineup is not legal: ${validation.issues.map((i) => i.message).join('; ')}`);
    }
