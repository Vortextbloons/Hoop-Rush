...
import { buildFixtureBracket, buildGameSimulationInput, buildSimulationPlayer, seedFromString, } from '@hoop-rush/test-fixtures';
import type { ClassicDraftCatalog, ClassicDraftCatalogEntry, ClassicDraftState, ClassicVariant, } from '@hoop-rush/data-contracts';
import { challengeRunSchema, classicDraftCatalogSchema, eraIdSchema, franchiseIdSchema, playerIdSchema, seedSchema, } from '@hoop-rush/data-contracts';
...
import { classicRerollAvailable, classicRollCandidates, createClassicChallenge, createClassicDraft, draftClassicPlayer, repositionClassicPlayer, rerollClassicEra, rerollClassicFranchise, slotRequirement, type ClassicChallengeEnvironment, } from './draft.ts';
const context = createEngineContext();
function catalogFixture(): ClassicDraftCatalog {
    return classicDraftCatalogSchema.parse([
        {
            franchiseId: franchiseIdSchema.parse('bulls'),
            eraId: eraIdSchema.parse('1990s'),
            players: [{ playerId: playerIdSchema.parse('p-7'), positions: ['PG'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('bulls'),
            eraId: eraIdSchema.parse('2010s'),
            players: [{ playerId: playerIdSchema.parse('p-11'), positions: ['SF'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('celtics'),
            eraId: eraIdSchema.parse('1990s'),
            players: [
                { playerId: playerIdSchema.parse('p-4'), positions: ['PG'] },
                { playerId: playerIdSchema.parse('p-5'), positions: ['C'] },
            ],
        },
        {
            franchiseId: franchiseIdSchema.parse('celtics'),
            eraId: eraIdSchema.parse('2010s'),
            players: [{ playerId: playerIdSchema.parse('p-10'), positions: ['C'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('heat'),
            eraId: eraIdSchema.parse('2000s'),
            players: [{ playerId: playerIdSchema.parse('p-8'), positions: ['PF', 'C'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('lakers'),
            eraId: eraIdSchema.parse('1980s'),
            players: [{ playerId: playerIdSchema.parse('p-6'), positions: ['SF'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('lakers'),
            eraId: eraIdSchema.parse('1990s'),
            players: [
                { playerId: playerIdSchema.parse('p-1'), positions: ['PG'] },
                { playerId: playerIdSchema.parse('p-2'), positions: ['PG', 'SF'] },
                { playerId: playerIdSchema.parse('p-3'), positions: ['C'] },
            ],
        },
        {
            franchiseId: franchiseIdSchema.parse('lakers'),
            eraId: eraIdSchema.parse('2010s'),
            players: [{ playerId: playerIdSchema.parse('p-9'), positions: ['SG'] }],
        },
    ]);
}
function entryOf(catalog: ClassicDraftCatalog, franchiseId: string, eraId: string): ClassicDraftCatalogEntry {
    const entry = catalog.find((e) => e.franchiseId === franchiseId && e.eraId === eraId);
    if (!entry)
        throw new Error(`catalog has no ${franchiseId}/${eraId} entry`);
    return entry;
}
function draftFixture(variant: ClassicVariant = 'ratings'): ClassicDraftState {
    return createClassicDraft({
        draftId: 'draft-1',
        variant,
        seed: seedSchema.parse(seedFromString('classic-fixture')),
        dataVersion: 'data-v1',
...
function advanceOne(state: ClassicDraftState): ClassicDraftState {
    ...
    const openSlots = [0, 1, 2, 3, 4].filter((slotIndex) => !state.picks.some((p) => p.slotIndex === slotIndex));
    for (const slotIndex of openSlots) {
        const requirement = slotRequirement(slotIndex);
        const player = entry.players.find((p) => !state.picks.some((pick) => pick.playerId === p.playerId) &&
            canPlay(p.positions, requirement));
        if (player) {
            return draftClassicPlayer(state, catalogFixture(), { playerId: player.playerId, slotIndex: slotIndex }, context);
        }
    }
...
function challengeEnvironment(draft: ClassicDraftState, overrides: Partial<ClassicChallengeEnvironment> = {}): ClassicChallengeEnvironment {
    const players = draft.picks
        .map((pick) => ({ pick, positions: positionsFor(pick.playerId) }))
        .sort((a, b) => a.pick.slotIndex - b.pick.slotIndex)
        .map(({ pick, positions }) => buildSimulationPlayer({
    ...
    return {
        runId: 'run-classic',
        runSeed: seedSchema.parse(seedFromString('classic-run')),
        players,
        dataVersion: 'data-v1',
...
function completeFixture(overrides: Partial<ClassicDraftState> = {}): ClassicDraftState {
    return {
        ...
        picks: [
            ...
            {
                round: 4,
                playerId: playerIdSchema.parse('p-11'),
                franchiseId: franchiseIdSchema.parse('bulls'),
                eraId: eraIdSchema.parse('2010s'),
                slotIndex: 3,
            },
    ...
    return classicDraftCatalogSchema.parse([
        {
            franchiseId: franchiseIdSchema.parse('bulls'),
            eraId: eraIdSchema.parse('1990s'),
            players: [{ playerId: playerIdSchema.parse('decoy-bulls-1990s'), positions: ['PG'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('bulls'),
            eraId: eraIdSchema.parse('2010s'),
            players: [{ playerId: playerIdSchema.parse('decoy-bulls-2010s'), positions: ['PG', 'SF'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('celtics'),
            eraId: eraIdSchema.parse('1980s'),
            players: [
                { playerId: playerIdSchema.parse('decoy-celtics-1980s'), positions: ['PG', 'SF', 'C'] },
            ],
        },
        {
            franchiseId: franchiseIdSchema.parse('celtics'),
            eraId: eraIdSchema.parse('1990s'),
            players: [{ playerId: playerIdSchema.parse('decoy-celtics-1990s'), positions: ['SG'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('heat'),
            eraId: eraIdSchema.parse('2000s'),
            players: [{ playerId: playerIdSchema.parse('decoy-heat-2000s'), positions: ['PG'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('knicks'),
            eraId: eraIdSchema.parse('2010s'),
            players: [{ playerId: playerIdSchema.parse('decoy-knicks-2010s'), positions: ['PG'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('lakers'),
            eraId: eraIdSchema.parse('1980s'),
            players: [{ playerId: playerIdSchema.parse('decoy-lakers-1980s'), positions: ['PG', 'SF'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('lakers'),
            eraId: eraIdSchema.parse('1990s'),
            players: [
                { playerId: playerIdSchema.parse('decoy-lakers-1990s'), positions: ['PG', 'SF', 'C'] },
            ],
        },
        {
            franchiseId: franchiseIdSchema.parse('lakers'),
            eraId: eraIdSchema.parse('2000s'),
            players: [{ playerId: playerIdSchema.parse('decoy-lakers-2000s'), positions: ['C'] }],
        },
        {
            franchiseId: franchiseIdSchema.parse('spurs'),
            eraId: eraIdSchema.parse('1980s'),
            players: [
                { playerId: playerIdSchema.parse('decoy-spurs-1980s'), positions: ['PG', 'SF', 'C'] },
            ],
        },
    ]);
}
function decoyState(overrides: Partial<ClassicDraftState> = {}): ClassicDraftState {
    return {
        ...draftFixture(),
        seed: seedSchema.parse(seedFromString('decoy-fixture')),
        round: 1,
        status: 'drafting',
        roll: { franchiseId: franchiseIdSchema.parse('lakers'), eraId: eraIdSchema.parse('1990s') },
        picks: [],
...
describe('classic draft rounds', () => {
    ...
    it('every initial roll offers an undrafted player for an open slot', () => {
        ...
        while (state.status === 'drafting') {
            if (state.roll === null)
                throw new Error('drafting state must carry a roll');
            const entry = entryOf(catalogFixture(), state.roll.franchiseId, state.roll.eraId);
            const openSlots = [0, 1, 2, 3, 4].filter((slotIndex) => !state.picks.some((p) => p.slotIndex === slotIndex));
            const fitting = entry.players.some((player) => !state.picks.some((pick) => pick.playerId === player.playerId) &&
    ...
    it('advances exactly one round per pick and stays drafting below five', () => {
        ...
        const next = draftClassicPlayer({
            ...state,
            roll: { franchiseId: franchiseIdSchema.parse('lakers'), eraId: eraIdSchema.parse('1990s') },
        }, catalogFixture(), { playerId: playerIdSchema.parse('p-1'), slotIndex: 0 }, context);
    ...
    it('rejects an empty or unfittable catalog at creation', () => {
        expect(() => createClassicDraft({
            draftId: 'draft-bad',
            variant: 'ratings',
            seed: seedSchema.parse(seedFromString('bad')),
            dataVersion: 'data-v1',
            catalog: [],
        }, context)).toThrow(/catalog is invalid/);
        expect(() => createClassicDraft({
            draftId: 'draft-bad',
            variant: 'ratings',
            seed: seedSchema.parse(seedFromString('bad')),
        ...
        }, context)).toThrow(/no eligible pool for round 1/);
    });
});
describe('classic rerolls', () => {
    it('franchise reroll changes the franchise, preserves the era, and spends once', () => {
        const catalog: ClassicDraftCatalog = [
            {
                ...
                players: [
                    { playerId: playerIdSchema.parse('p-1'), positions: ['PG'] },
                    { playerId: playerIdSchema.parse('p-2'), positions: ['PG', 'SF'] },
                    { playerId: playerIdSchema.parse('p-3'), positions: ['SF'] },
                    { playerId: playerIdSchema.parse('p-5'), positions: ['C'] },
                ],
            },
            {
                ...
                players: [
                    { playerId: playerIdSchema.parse('p-4'), positions: ['PG'] },
                    { playerId: playerIdSchema.parse('p-6'), positions: ['SF'] },
                ],
            },
            {
                franchiseId: franchiseIdSchema.parse('bulls'),
        ...
        const state = createClassicDraft({
            draftId: 'draft-era',
            variant: 'ratings',
            seed: seedSchema.parse(seedFromString('era-keep')),
            dataVersion: 'data-v1',
            catalog,
        }, context);
        const narrowed = {
    ...
    it('era reroll changes the era, preserves the franchise, and spends once', () => {
        ...
        const state = createClassicDraft({
            ...
            variant: 'ratings',
            seed: seedSchema.parse(seedFromString('franchise-keep')),
            dataVersion: 'data-v1',
            catalog,
        }, context);
        const narrowed = {
    ...
    it('franchise and era rerolls are independent in either order', () => {
        const catalog = catalogFixture();
        const bothFranchiseFirst = rerollClassicEra(rerollClassicFranchise(draftFixture(), catalog, context), catalog, context);
        expect(bothFranchiseFirst.rerolls.franchiseSpent).toBe(true);
        expect(bothFranchiseFirst.rerolls.eraSpent).toBe(true);
        const bothEraFirst = rerollClassicFranchise(rerollClassicEra(draftFixture(), catalog, context), catalog, context);
    ...
    it('rejects a reroll with no alternative without spending it', () => {
        const catalog: ClassicDraftCatalog = [
            ...
            },
            {
                franchiseId: franchiseIdSchema.parse('celtics'),
                eraId: eraIdSchema.parse('1990s'),
                players: [{ playerId: playerIdSchema.parse('p-4'), positions: ['PG'] }],
            },
        ];
        const state = createClassicDraft({
            draftId: 'draft-tight',
            variant: 'ratings',
            seed: seedSchema.parse(seedFromString('tight')),
            dataVersion: 'data-v1',
            catalog,
        }, context);
        ...
        expect(() => rerollClassicFranchise(narrowed, catalog, context)).toThrow(/no alternative franchise for era 1990s in round 5/);
        expect(() => rerollClassicEra(narrowed, catalog, context)).toThrow(/no alternative era for franchise lakers in round 5/);
        expect(narrowed.rerolls).toEqual({ franchiseSpent: false, eraSpent: false });
        expect(narrowed.roll).toEqual({
            franchiseId: franchiseIdSchema.parse('lakers'),
            eraId: eraIdSchema.parse('1990s'),
        });
    });
    it('can still complete a round when only the C slot is open', () => {
        ...
        const done = draftClassicPlayer(narrowed, catalog, { playerId: playerIdSchema.parse('p-5'), slotIndex: 4 }, context);
        expect(done.status).toBe('complete');
        expect(done.picks).toHaveLength(5);
    });
});
describe('classic reroll single-axis candidates', () => {
    it('franchise reroll preserves the era and changes the franchise across seeds', () => {
        ...
        for (let i = 0; i < 20; i += 1) {
            const state = decoyState({
                seed: seedSchema.parse(seedFromString(`decoy-franchise-${String(i)}`)),
            });
    ...
    it('era reroll preserves the franchise and changes the era across seeds', () => {
        ...
        for (let i = 0; i < 20; i += 1) {
            const state = decoyState({
                seed: seedSchema.parse(seedFromString(`decoy-era-${String(i)}`)),
            });
    ...
    it('excludes pairs that change both axes from either reroll', () => {
        ...
        const franchiseKeys = new Set(franchiseCandidates.map((entry) => `${entry.franchiseId}/${entry.eraId}`));
    ...
    it('drops same-axis entries whose only player is already drafted', () => {
        const franchiseState = decoyState({
            picks: [
                {
                    round: 1,
                    playerId: playerIdSchema.parse('decoy-celtics-1990s'),
                    franchiseId: franchiseIdSchema.parse('celtics'),
                    eraId: eraIdSchema.parse('1990s'),
                    slotIndex: 0,
                },
        ...
        const franchiseKeys = new Set(classicRollCandidates(decoyFixture(), franchiseState, 'franchise-reroll').map((entry) => `${entry.franchiseId}/${entry.eraId}`));
        ...
        const eraState = decoyState({
            picks: [
                {
                    round: 1,
                    playerId: playerIdSchema.parse('decoy-lakers-1980s'),
                    franchiseId: franchiseIdSchema.parse('lakers'),
                    eraId: eraIdSchema.parse('1980s'),
                    slotIndex: 0,
                },
        ...
        const eraKeys = new Set(classicRollCandidates(decoyFixture(), eraState, 'era-reroll').map((entry) => `${entry.franchiseId}/${entry.eraId}`));
    ...
    it('rejects an axis reroll with no alternative on that axis without spending', () => {
        const catalog: ClassicDraftCatalog = [
            {
                ...
                eraId: eraIdSchema.parse('2010s'),
                players: [
                    { playerId: playerIdSchema.parse('decoy-knicks-2010s'), positions: ['PG', 'SF', 'C'] },
                ],
            },
            {
                franchiseId: franchiseIdSchema.parse('heat'),
                eraId: eraIdSchema.parse('2000s'),
                players: [
                    { playerId: playerIdSchema.parse('decoy-heat-2000s'), positions: ['PG', 'SF', 'C'] },
                ],
            },
        ];
        const state = decoyState({
            roll: { franchiseId: franchiseIdSchema.parse('knicks'), eraId: eraIdSchema.parse('2010s') },
        });
        ...
        expect(() => rerollClassicFranchise(state, catalog, context)).toThrow(/no alternative franchise for era 2010s in round 1/);
        expect(() => rerollClassicEra(state, catalog, context)).toThrow(/no alternative era for franchise knicks in round 1/);
        expect(state.rerolls).toEqual({ franchiseSpent: false, eraSpent: false });
        expect(state.roll).toEqual({
            franchiseId: franchiseIdSchema.parse('knicks'),
            eraId: eraIdSchema.parse('2010s'),
        });
    });
    it('reroll candidates respect the open slots of the current round', () => {
        const state = decoyState({
            ...
            picks: [
                {
                    round: 1,
                    playerId: playerIdSchema.parse('decoy-lakers-1990s'),
                    franchiseId: franchiseIdSchema.parse('lakers'),
                    eraId: eraIdSchema.parse('1990s'),
                    slotIndex: 0,
                },
                {
                    round: 2,
                    playerId: playerIdSchema.parse('decoy-bulls-1990s'),
                    franchiseId: franchiseIdSchema.parse('bulls'),
                    eraId: eraIdSchema.parse('1990s'),
                    slotIndex: 1,
                },
        ...
        expect(() => rerollClassicFranchise(state, decoyFixture(), context)).toThrow(/no alternative franchise/);
        const eraCandidates = classicRollCandidates(decoyFixture(), state, 'era-reroll');
...
describe('classic candidate filtering and picks', () => {
    ...
    it('rejects a player into a slot their positions cannot fill', () => {
        ...
        expect(() => draftClassicPlayer(base, catalogFixture(), { playerId: playerIdSchema.parse('p-1'), slotIndex: 4 }, context)).toThrow(/p-1 cannot play slot 4/);
        expect(() => draftClassicPlayer(base, catalogFixture(), { playerId: playerIdSchema.parse('p-3'), slotIndex: 0 }, context)).toThrow(/p-3 cannot play slot 0/);
    });
    it('rejects duplicate players, occupied slots, and unknown players', () => {
        ...
        const one = draftClassicPlayer(base, catalogFixture(), { playerId: playerIdSchema.parse('p-1'), slotIndex: 0 }, context);
        expect(() => draftClassicPlayer(one, catalogFixture(), { playerId: playerIdSchema.parse('p-1'), slotIndex: 1 }, context)).toThrow(/p-1 is already drafted/);
        ...
        const displaced = draftClassicPlayer(displacementDraft, catalogFixture(), { playerId: playerIdSchema.parse('p-2'), slotIndex: 0 }, context);
        ...
        const twoCenters: ClassicDraftCatalog = catalogFixture().map((entry) => entry.franchiseId === 'celtics' && entry.eraId === '1990s'
            ? {
                ...entry,
                players: [
                    { playerId: playerIdSchema.parse('p-4'), positions: ['PG'] },
                    { playerId: playerIdSchema.parse('p-5'), positions: ['C'] },
                    { playerId: playerIdSchema.parse('p-10'), positions: ['C'] },
                ],
            }
            : entry);
        const centerOnly = {
            ...draftFixture(),
            roll: { franchiseId: franchiseIdSchema.parse('celtics'), eraId: eraIdSchema.parse('1990s') },
            picks: [
                {
                    round: 1,
        ...
        expect(() => draftClassicPlayer(centerOnly, twoCenters, { playerId: playerIdSchema.parse('p-10'), slotIndex: 4 }, context)).toThrow(/slot 4 is already filled/);
        expect(() => draftClassicPlayer(base, catalogFixture(), { playerId: playerIdSchema.parse('p-404'), slotIndex: 0 }, context)).toThrow(/p-404 is not in the rolled pool/);
    });
    it('rejects a draft whose catalog lacks the current roll pair', () => {
        const base = {
            ...draftFixture(),
            roll: { franchiseId: franchiseIdSchema.parse('spurs'), eraId: eraIdSchema.parse('1990s') },
        };
        expect(() => draftClassicPlayer(base, catalogFixture(), { playerId: playerIdSchema.parse('p-1'), slotIndex: 0 }, context)).toThrow(/catalog does not contain the current roll pair/);
    });
});
describe('classic repositioning', () => {
    it('moves a drafted player to an open legal slot', () => {
        ...
        const state = draftClassicPlayer(base, catalogFixture(), { playerId: playerIdSchema.parse('p-1'), slotIndex: 0 }, context);
    ...
    it('swaps two picks when both destinations are legal', () => {
        ...
        const two = draftClassicPlayer({
            ...one,
            roll: { franchiseId: franchiseIdSchema.parse('lakers'), eraId: eraIdSchema.parse('1990s') },
        }, catalogFixture(), { playerId: playerIdSchema.parse('p-2'), slotIndex: 1 }, context);
    ...
    it('drafts into an occupied slot by displacing the incumbent', () => {
        const state = {
            ...draftFixture(),
            roll: { franchiseId: franchiseIdSchema.parse('heat'), eraId: eraIdSchema.parse('2000s') },
            picks: [
                {
                    round: 1,
        ...
        const catalog: ClassicDraftCatalog = catalogFixture().map((entry) => entry.franchiseId === 'heat' && entry.eraId === '2000s'
            ? {
                ...entry,
                players: [
                    { playerId: playerIdSchema.parse('p-8'), positions: ['PF', 'C'] },
                    { playerId: playerIdSchema.parse('p-12'), positions: ['C'] },
                ],
            }
            : entry);
        const next = draftClassicPlayer(state, catalog, { playerId: playerIdSchema.parse('p-12'), slotIndex: 4 }, context);
    ...
    it('rejects illegal targets, unknown players, and stuck swaps', () => {
        ...
        const two = draftClassicPlayer({
            ...one,
            roll: { franchiseId: franchiseIdSchema.parse('lakers'), eraId: eraIdSchema.parse('1990s') },
        }, catalogFixture(), { playerId: playerIdSchema.parse('p-3'), slotIndex: 4 }, context);
        expect(() => repositionClassicPlayer(two, catalogFixture(), {
            playerId: playerIdSchema.parse('p-1'),
            slotIndex: 4,
        ...
        expect(() => repositionClassicPlayer(two, catalogFixture(), {
            playerId: playerIdSchema.parse('p-404'),
            slotIndex: 2,
    ...
    it('never changes round, status, roll, or rerolls', () => {
        ...
        const one = draftClassicPlayer(base, catalogFixture(), { playerId: playerIdSchema.parse('p-1'), slotIndex: 0 }, context);
        const moved = repositionClassicPlayer(one, catalogFixture(), {
            playerId: playerIdSchema.parse('p-1'),
            slotIndex: 1,
        });
...
describe('createClassicChallenge', () => {
    ...
    it('rejects an incomplete draft', () => {
        const state = draftFixture();
        expect(() => createClassicChallenge(state, challengeEnvironment(state))).toThrow(/requires a complete draft/);
    });
    it('rejects duplicate playerIds in the picks', () => {
        ...
        expect(() => createClassicChallenge(state, challengeEnvironment(state))).toThrow(/distinct players/);
    });
    it('rejects slot/player mismatches with the resolved players', () => {
        const done = draftFive(draftFixture());
        const env = challengeEnvironment(done, {
            players: challengeEnvironment(done).players.map((player, slotIndex) => slotIndex === 0 ? { ...player, playerId: playerIdSchema.parse('p-999') } : player),
        });
    ...
    it('rejects an illegal lineup built from the picks', () => {
        ...
        expect(() => createClassicChallenge(state, challengeEnvironment(state))).toThrow(/classic lineup is not legal/);
    });
});
describe('classic draft command sequences (property)', () => {
    const commandArb = fc.constantFrom<'pick' | 'reposition' | 'franchise-reroll' | 'era-reroll'>('pick', 'reposition', 'franchise-reroll', 'era-reroll');
    it('preserves draft invariants through any bounded command sequence', () => {
        fc.assert(fc.property(fc.array(commandArb, { maxLength: 30 }), (commands) => {
            const catalog = catalogFixture();
            let state = draftFixture();
            for (const command of commands) {
                ...
                switch (command) {
                    case 'franchise-reroll': {
                        if (!classicRerollAvailable(state, 'franchise', catalog))
                            break;
                    ...
                    case 'era-reroll': {
                        if (!classicRerollAvailable(state, 'era', catalog))
                            break;
                        const before = state.roll;
                    ...
                    case 'pick': {
                        ...
                        const slotIndex = [0, 1, 2, 3, 4].find((s) => !occupied.has(s) &&
                            entry.players.some((p) => !drafted.has(p.playerId) && canPlay(p.positions, slotRequirement(s))));
                        if (slotIndex === undefined)
                            break;
                        const player = entry.players.find((p) => !drafted.has(p.playerId) && canPlay(p.positions, slotRequirement(slotIndex)));
                        if (!player)
                            break;
                        state = draftClassicPlayer(state, catalog, { playerId: player.playerId, slotIndex: slotIndex }, context);
                        break;
                    }
                    case 'reposition': {
                        ...
                        if (!player)
                            break;
                        const target = [0, 1, 2, 3, 4].find((s) => s !== pick.slotIndex && canPlay(player.positions, slotRequirement(s)));
                        if (target === undefined)
                            break;
