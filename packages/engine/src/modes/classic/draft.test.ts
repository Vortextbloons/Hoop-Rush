...
import { classicRerollAvailable, classicRollCandidates, classicRollSeed, createClassicChallenge, createClassicDraft, draftClassicPlayer, repositionClassicPlayer, rerollClassicEra, rerollClassicFranchise, slotRequirement, sortClassicCatalog, type ClassicChallengeEnvironment, } from './draft.ts';
...
describe('classic roll seeds', () => {
    it('are byte-identical for the same inputs and distinct otherwise', () => {
        ...
        expect(classicRollSeed(seed, version, 'initial', 1)).toBe(classicRollSeed(seed, version, 'initial', 1));
        expect(classicRollSeed(seed, version, 'initial', 1)).not.toBe(classicRollSeed(seed, version, 'initial', 2));
        expect(classicRollSeed(seed, version, 'initial', 1)).not.toBe(classicRollSeed(seed, version, 'franchise-reroll', 1));
        expect(classicRollSeed(seed, version, 'franchise-reroll', 1)).not.toBe(classicRollSeed(seed, version, 'era-reroll', 1));
    });
...
describe('classic draft rounds', () => {
    ...
    it('advances exactly one round per pick and stays drafting below five', () => {
        ...
        const next = draftClassicPlayer({ ...state, roll: { franchiseId: 'lakers', eraId: '1990s' } }, catalogFixture(), { playerId: 'p-1', slotIndex: 0 }, context);
...
describe('classic candidate filtering and picks', () => {
    ...
    it('rejects a player into a slot their positions cannot fill', () => {
        ...
        expect(() => draftClassicPlayer(base, catalogFixture(), { playerId: 'p-1', slotIndex: 4 }, context)).toThrow(/p-1 cannot play slot 4/);
        expect(() => draftClassicPlayer(base, catalogFixture(), { playerId: 'p-3', slotIndex: 0 }, context)).toThrow(/p-3 cannot play slot 0/);
    });
    it('rejects duplicate players, occupied slots, and unknown players', () => {
        const base = { ...draftFixture(), roll: { franchiseId: 'lakers', eraId: '1990s' } };
        const one = draftClassicPlayer(base, catalogFixture(), { playerId: 'p-1', slotIndex: 0 }, context);
        expect(() => draftClassicPlayer(one, catalogFixture(), { playerId: 'p-1', slotIndex: 1 }, context)).toThrow(/p-1 is already drafted/);
        ...
        const displaced = draftClassicPlayer(displacementDraft, catalogFixture(), { playerId: 'p-2', slotIndex: 0 }, context);
        ...
        expect(() => draftClassicPlayer(centerOnly, twoCenters, { playerId: 'p-10', slotIndex: 4 }, context)).toThrow(/slot 4 is already filled/);
        expect(() => draftClassicPlayer(base, catalogFixture(), { playerId: 'p-404', slotIndex: 0 }, context)).toThrow(/p-404 is not in the rolled pool/);
    });
    it('rejects a draft whose catalog lacks the current roll pair', () => {
        const base = { ...draftFixture(), roll: { franchiseId: 'spurs', eraId: '1990s' } };
        expect(() => draftClassicPlayer(base, catalogFixture(), { playerId: 'p-1', slotIndex: 0 }, context)).toThrow(/catalog does not contain the current roll pair/);
    });
});
describe('classic repositioning', () => {
    it('moves a drafted player to an open legal slot', () => {
        const base = { ...draftFixture(), roll: { franchiseId: 'lakers', eraId: '1990s' } };
        const state = draftClassicPlayer(base, catalogFixture(), { playerId: 'p-1', slotIndex: 0 }, context);
    ...
    it('swaps two picks when both destinations are legal', () => {
        const base = { ...draftFixture(), roll: { franchiseId: 'lakers', eraId: '1990s' } };
        const one = draftClassicPlayer(base, catalogFixture(), { playerId: 'p-1', slotIndex: 0 }, context);
        const two = draftClassicPlayer({ ...one, roll: { franchiseId: 'lakers', eraId: '1990s' } }, catalogFixture(), { playerId: 'p-2', slotIndex: 1 }, context);
    ...
    it('rejects illegal targets, unknown players, and stuck swaps', () => {
        const base = { ...draftFixture(), roll: { franchiseId: 'lakers', eraId: '1990s' } };
        const one = draftClassicPlayer(base, catalogFixture(), { playerId: 'p-1', slotIndex: 0 }, context);
        const two = draftClassicPlayer({ ...one, roll: { franchiseId: 'lakers', eraId: '1990s' } }, catalogFixture(), { playerId: 'p-3', slotIndex: 4 }, context);
        expect(() => repositionClassicPlayer(two, catalogFixture(), { playerId: 'p-1', slotIndex: 4 })).toThrow(/p-1 cannot play slot 4/);
        expect(() => repositionClassicPlayer(two, catalogFixture(), { playerId: 'p-3', slotIndex: 0 })).toThrow(/p-3 cannot play slot 0/);
        expect(() => repositionClassicPlayer(two, catalogFixture(), { playerId: 'p-404', slotIndex: 2 })).toThrow(/p-404 is not drafted/);
...
describe('createClassicChallenge', () => {
    ...
    it('rejects slot/player mismatches with the resolved players', () => {
        ...
        const env = challengeEnvironment(done, {
            players: challengeEnvironment(done).players.map((player, slotIndex) => slotIndex === 0 ? { ...player, playerId: 'p-999' } : player),
        });
