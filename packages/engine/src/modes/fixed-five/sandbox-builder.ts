...
function candidateById(pool: readonly FixedFiveCandidate[], playerId: PlayerId): FixedFiveCandidate | null {
...
export function selectionKeepsFeasibility(pool: readonly FixedFiveCandidate[], placed: ReadonlyArray<{
    playerId: PlayerId;
    slotIndex: SlotIndex;
}>): boolean {
    ...
    for (const p of placed) {
        if (usedIds.has(p.playerId))
            return false;
        usedIds.add(p.playerId);
        const candidate = byId.get(p.playerId);
        if (!candidate)
            return false;
        if (!canPlay(candidate.positions, slotRequirement(p.slotIndex)))
            return false;
    }
    if (placed.length === 5) {
        const lineup = {
            structure: ['G', 'G', 'F', 'F', 'C'] as [
                'G',
                'G',
                'F',
                'F',
    ...
    const tryFill = (slotIdx: number, chosen: FixedFiveCandidate[]): boolean => {
        if (slotIdx === open.length) {
            ...
            const lineup = {
                ...
                assignments: full.map((p) => ({
                    slotIndex: p.slotIndex,
                    playerId: p.playerId,
                    positions: byId.get(p.playerId)?.positions ??
        ...
        for (const candidate of remaining) {
            if (chosen.some((c) => c.playerId === candidate.playerId))
                continue;
            if (!canPlay(candidate.positions, requirement))
                continue;
            if (tryFill(slotIdx + 1, [...chosen, candidate]))
                return true;
        }
        return false;
    };
    if (open.length > remaining.length)
        return false;
    if (open.length <= 3)
        return tryFill(0, []);
...
function assignLineupFeasible(pool: readonly FixedFiveCandidate[], placed: ReadonlyArray<{
    playerId: PlayerId;
    slotIndex: SlotIndex;
}>): boolean {
    const byId = new Map(pool.map((c) => [c.playerId, c]));
            ...
            const trial = [
                ...
                ...usedList.map((id) => {
                    ...
                    if (slotFor === undefined)
                        throw new Error('sandbox feasibility ran out of slots');
                    return {
                        playerId: id,
                        slotIndex: slotFor,
...
export function applySandboxBuilderCommand(state: SandboxBuilderState, pool: readonly FixedFiveCandidate[], command: SandboxBuilderCommand): SandboxBuilderState {
    if (state.locked) {
    ...
    if (command.kind === 'sandbox-place') {
        ...
        if (!selectionKeepsFeasibility(pool, next)) {
            throw new Error(`placing ${command.playerId} in slot ${String(command.slotIndex)} leaves no legal completion`);
        }
...
export function enumerateSandboxSafeMoves(pool: readonly FixedFiveCandidate[], state: SandboxBuilderState): SandboxSafeMove[] {
    if (state.locked)
        return [];
    ...
    for (const slotIndex of open) {
        for (const candidate of pool) {
            if (used.has(candidate.playerId))
                continue;
            if (!canPlay(candidate.positions, slotRequirement(slotIndex)))
                continue;
