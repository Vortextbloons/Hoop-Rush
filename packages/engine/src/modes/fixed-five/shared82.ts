import type { BracketOpponent, EraSimulationProfile, GameResult, OpponentBracket, Seed, SimulationTeam, } from '@hoop-rush/data-contracts';
...
export function findWeakestOpponent<T extends {
    opponentId: string;
    strength: {
        percentile: number;
        winRate: number;
    ...
    if (bracket.opponents.length === 0)
        throw new Error('bracket has no opponents');
    const sorted = [...bracket.opponents].sort((a, b) => {
        ...
        if (a.strength.winRate !== b.strength.winRate)
            return a.strength.winRate - b.strength.winRate;
...
export function simulateShared82(input: Shared82SimulationInput, context: EngineContext): Shared82SimulationOutput {
    ...
    for (let gameNumber = 1; gameNumber <= 82; gameNumber += 1) {
        ...
        if (!entry)
            throw new Error(`schedule missing game ${String(gameNumber)}`);
        ...
        if (p1Failures.length > 0)
            throw new Error(`shared82 p1 game ${String(gameNumber)} failed invariants: ${p1Failures.join('; ')}`);
        ...
        if (p2Failures.length > 0)
            throw new Error(`shared82 p2 game ${String(gameNumber)} failed invariants: ${p2Failures.join('; ')}`);
