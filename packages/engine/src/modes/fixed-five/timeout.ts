import type { ClassicDraftCatalog, ClassicDraftState, FixedFiveRoomMode, PlayerId, Seed, SlotIndex, } from '@hoop-rush/data-contracts';
...
import { enumerateSandboxSafeMoves, selectionKeepsFeasibility, type FixedFiveCandidate, type SandboxBuilderState, } from './sandbox-builder.ts';
...
export function enumerateClassicSafeMoves(catalog: ClassicDraftCatalog, poolById: ReadonlyMap<string, FixedFiveCandidate>, state: ClassicDraftState): ClassicSafeMove[] {
    ...
    const moves = enumerateRolledPoolMoves(entry.players, poolById, (playerId) => drafted.has(playerId), occupied);
...
export function enumerateDuelSafeMoves(catalog: ClassicDraftCatalog, poolById: ReadonlyMap<string, FixedFiveCandidate>, state: DuelDraftState): ClassicSafeMove[] {
    if (state.status !== 'drafting' || !state.currentRoll)
        return [];
    const roll = state.currentRoll;
    const entry = catalog.find((e) => e.franchiseId === roll.franchiseId && e.eraId === roll.eraId);
    if (!entry)
        return [];
    const picker = state.pickOrdinal % 2 === 0 ? state.firstPicker : state.firstPicker === 'p1' ? 'p2' : 'p1';
    const usedSlots = new Set(state.picks.filter((p) => p.participantId === picker).map((p) => p.slotIndex));
    const claimed = new Set(state.claimedVersionIds);
    const moves = enumerateRolledPoolMoves(entry.players, poolById, (playerId, versionId) => claimed.has(versionId) || claimed.has(playerId), usedSlots);
    return rankAutopickMoves(moves);
}
export function enumerateSandboxDuelSafeMoves(pool: readonly FixedFiveCandidate[], state: SandboxDuelState): ClassicSafeMove[] {
    ...
    for (const candidate of pool) {
        if (usedIds.has(candidate.playerId))
            continue;
        for (const slot of [0, 1, 2, 3, 4] as SlotIndex[]) {
            if (usedSlots.has(slot))
                continue;
            if (!canPlay(candidate.positions, slotRequirement(slot)))
                continue;
            const trial = [
...
function enumerateRolledPoolMoves(players: ClassicDraftCatalog[number]['players'], poolById: ReadonlyMap<string, FixedFiveCandidate>, isClaimed: (playerId: PlayerId, versionId: string) => boolean, usedSlots: ReadonlySet<SlotIndex>): ClassicSafeMove[] {
    const moves: ClassicSafeMove[] = [];
    for (const catalogPlayer of players) {
        ...
        if (isClaimed(catalogPlayer.playerId, versionId))
            continue;
        const positions = candidate?.positions ?? catalogPlayer.positions;
        for (const slot of [0, 1, 2, 3, 4] as SlotIndex[]) {
            if (usedSlots.has(slot))
                continue;
            if (!canPlay(positions, slotRequirement(slot)))
                continue;
            moves.push({
...
function rankAutopickMoves<T extends {
    selectionScore: number;
    playerVersionId: string;
    slotIndex: SlotIndex;
}>(moves: T[]): T[] {
...
export function chooseAutopick(rootSeed: Seed, mode: FixedFiveRoomMode, participantId: 'p1' | 'p2', pickOrdinal: number, candidates: Array<{
    playerId: PlayerId;
    playerVersionId: string;
    slotIndex: SlotIndex;
    selectionScore: number;
...
export function chooseSandboxAutopicksUntilFull(rootSeed: Seed, mode: FixedFiveRoomMode, participantId: 'p1' | 'p2', startOrdinal: number, pool: readonly FixedFiveCandidate[], initial: SandboxBuilderState): AutopickSelection[] {
    ...
    for (;;) {
        ...
        if (selections.length > 5)
            throw new Error('sandbox autopick overflow');
    }
    return selections;
}
