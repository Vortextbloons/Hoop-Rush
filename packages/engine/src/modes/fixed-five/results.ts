import type { FixedFiveDuelResult, FixedFiveShared82Result, GameResult, OpponentBracket, Seed, } from '@hoop-rush/data-contracts';
import { seedSchema } from '@hoop-rush/data-contracts';
import { checkGameResult } from '../../sim/invariants.ts';
import { FIXED_FIVE_TIEBREAK_PATH, fixedFiveDuelGameSeed, fixedFiveH2HSeed, fixedFiveSharedGameSeed, fixedFiveTiebreakWinner, } from './seeds.ts';
...
export function summarizeShared82Games(groups: Shared82GameGroups): Shared82Summary {
    ...
    if (groups.h2h.length !== h2hNumbers.length) {
        throw new Error(`shared82 needs ${String(h2hNumbers.length)} H2H results (got ${String(groups.h2h.length)})`);
    }
    const nonH2hCount = 82 - h2hNumbers.length;
    if (groups.p1NonH2h.length !== nonH2hCount || groups.p2NonH2h.length !== nonH2hCount) {
        throw new Error(`shared82 needs ${String(nonH2hCount)} non-H2H results per participant (got ${String(groups.p1NonH2h.length)}/${String(groups.p2NonH2h.length)})`);
    }
    ...
    for (let gameNumber = 1; gameNumber <= 82; gameNumber += 1) {
        if (h2hSet.has(gameNumber)) {
            ...
            if (!result)
                throw new Error(`shared82 is missing H2H game ${String(gameNumber)}`);
        ...
        if (p1Result.winner === 'home')
            p1Wins += 1;
        if (p2Result.winner === 'home')
            p2Wins += 1;
...
export function summarizeDuelGames(group: DuelGameGroup): {
    ...
    games.forEach((game, index) => {
        ...
        if (game.gameNumber !== expectedNumber) {
            throw new Error(`duel game out of order: expected ${String(expectedNumber)}, got ${String(game.gameNumber)}`);
        }
    ...
    if (p1Wins !== 4 && p2Wins !== 4) {
        throw new Error(`duel series must end 4-x within seven games (got ${String(p1Wins)}-${String(p2Wins)})`);
    }
