...
import { claimSandboxDuelPlayer, createSandboxDuelDraft, isSandboxDuelComplete, sandboxDuelAlternationHolds, sandboxDuelPicker, sandboxDuelPicksFor, type SandboxDuelState, } from './sandbox-duel.ts';
...
function claim(state: SandboxDuelState, pool: FixedFiveCandidate[], playerId: string, slotIndex: number): SandboxDuelState {
...
describe('sandbox duel draft', () => {
    ...
    it('rejects out-of-turn claims', () => {
        ...
        expect(() => claimSandboxDuelPlayer(state, pool, {
            playerId: pid('p-g1'),
            slotIndex: slot(0),
            actor: 'p2',
        })).toThrow();
