...
export function fixedFiveParticipantSeed(rootSeed: Seed, participantId: FixedFiveParticipantId): Seed {
...
export function fixedFiveDuelGameSeed(rootSeed: Seed, gameNumber: number): Seed {
    ...
    return hexSeed(`hoop-rush:${FIXED_FIVE_SEED_VERSION}:${rootSeed}:duel:game-${String(gameNumber)}`);
}
export function fixedFiveSharedGameSeed(rootSeed: Seed, participantId: FixedFiveParticipantId, gameNumber: number): Seed {
    ...
    return hexSeed(`hoop-rush:${FIXED_FIVE_SEED_VERSION}:${rootSeed}:shared82:${participantId}:game-${String(gameNumber)}`);
}
export function fixedFiveH2HSeed(rootSeed: Seed, gameNumber: number): Seed {
    if (!Number.isInteger(gameNumber) || gameNumber < 1 || gameNumber > 82) {
        throw new Error(`gameNumber must be an integer in 1..82 (got ${String(gameNumber)})`);
    }
    return hexSeed(`hoop-rush:${FIXED_FIVE_SEED_VERSION}:${rootSeed}:shared82:h2h:game-${String(gameNumber)}`);
}
export function fixedFiveAutopickSeed(rootSeed: Seed, mode: FixedFiveRoomMode, participantId: FixedFiveParticipantId, pickOrdinal: number): string {
...
export function fixedFiveAutopickSeedPath(mode: FixedFiveRoomMode, participantId: FixedFiveParticipantId, pickOrdinal: number): string {
    return `rootSeed/timeout-autopick/${mode}/${participantId}/${String(pickOrdinal)}`;
}
