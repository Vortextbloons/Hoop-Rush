...
export function sandboxDuelPicker(state: SandboxDuelState): 'p1' | 'p2' {
    if (state.status === 'complete')
        throw new Error('sandbox duel draft is complete');
    return state.pickOrdinal % 2 === 0 ? state.firstPicker : otherParticipant(state.firstPicker);
}
export function createSandboxDuelDraft(rootSeed: Seed, firstPickerOverride?: 'p1' | 'p2'): SandboxDuelState {
    return {
        rootSeed,
...
export function claimSandboxDuelPlayer(state: SandboxDuelState, pool: readonly FixedFiveCandidate[], input: SandboxDuelClaimInput): SandboxDuelState {
    if (state.status !== 'drafting')
        throw new Error('sandbox duel draft is not active');
    ...
    if (!candidate)
        throw new Error(`unknown player ${input.playerId}`);
    ...
    if (!selectionKeepsFeasibility(pool, trial)) {
        throw new Error(`claiming ${input.playerId} in slot ${String(input.slotIndex)} leaves no legal completion`);
    }
...
export function sandboxDuelPicksFor(state: SandboxDuelState, participantId: 'p1' | 'p2'): SandboxDuelPick[] {
    return state.picks
