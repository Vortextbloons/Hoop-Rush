...
import type { ClassicDraftCatalog, EraId, EraSimulationProfile, FranchiseId, PlayerId, Position, } from '@hoop-rush/data-contracts';
import { commandIdSchema, contentHashSchema, eraIdSchema, franchiseIdSchema, idSchema, playerIdSchema, seedSchema, } from '@hoop-rush/data-contracts';
...
import { FIXED_FIVE_TIEBREAK_PATH, fixedFiveAutopickSeed, fixedFiveDraftSeed, fixedFiveDuelGameSeed, fixedFiveFirstPicker, fixedFiveH2HSeed, fixedFiveParticipantSeed, fixedFiveSharedGameSeed, fixedFiveTiebreakWinner, } from './seeds.ts';
...
function classicCatalog(): ClassicDraftCatalog {
    const entry = (franchiseId: string, eraId: string, players: Array<{
        playerId: string;
        positions: Position[];
...
describe('sandbox builder', () => {
    ...
    it('rejects illegal slot assignments on the same path solo and multiplayer share', () => {
        ...
        expect(() => applySandboxBuilderCommand(state, pool, {
            kind: 'sandbox-place',
            playerId: pid(`p-c1`),
            slotIndex: 0,
        })).toThrow();
...
describe('duel draft', () => {
    it('alternates ten picks with mutual exclusivity', () => {
        ...
        for (let i = 0; i < 10; i += 1) {
            ...
            if (!roll)
                throw new Error('duel draft is missing its roll');
            ...
            const option = entry?.players.find((p) => {
                ...
                if (claimed.has(versionId) || claimed.has(p.playerId))
                    return false;
            ...
            if (!option)
                throw new Error('duel test found no claimable player');
...
describe('result digest', () => {
    ...
    it('ignores governance commands so a late proposer still agrees (room d71f)', () => {
        ...
        const p1Players = buildLegalSimulationTeam({ teamId: 'p1', displayName: 'P1' }).players;
        const digestOf = (acceptedCommands: FixedFiveCommand[]) => fixedFiveResultDigest({
            rootSeed: ROOT,
            versions: {
                dataVersion: 'data-v1',
                ratingVersion: 'ratings-v1',
        ...
        const tampered: FixedFiveCommand[] = draft.map((command) => command.ordinal === 4
            ? {
