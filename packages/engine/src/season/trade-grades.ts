import { canonicalJson, idSchema, seasonDigestHex, seasonTradeGradeLogSchema, type SeasonGameSummary, type SeasonPostseasonSummary, type SeasonRun, type SeasonTradeGrade, type SeasonTradeGradeLabel, type SeasonTradeGradeLog, } from '@hoop-rush/data-contracts';
    ...
    for (const window of run.trade?.windows ?? []) {
        ...
        for (const offer of window.offers) {
            ...
            for (const side of sides) {
                ...
                grades.push({
                    gradeId: idSchema.parse(gradeIdOf(input.runId, window.windowIndex, offer.offerId, side.franchiseId)),
