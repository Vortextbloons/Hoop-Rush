...
import { franchiseIdSchema, seasonGameIdSchema, seasonPendingBlockCandidateSchema, type SeasonGameSummary, type SeasonHealthState, type SeasonInjuryRecord, } from '@hoop-rush/data-contracts';
import { buildTestRun, emptyHealthState, pipelineInput, scheduleOf } from './block-test-support.ts';
import { advancePendingAfterForfeit, assembleSeasonPendingBlock, seasonForfeitSummaryForGame, seasonFranchiseLegalFiveFacts, seasonGameHealthSeam, } from './health.ts';
import { seasonPlayerAvailable } from './injuries.ts';
import { assembleSeasonBlockCandidate, auditSeasonBlock, seasonBlockGamesOf, simulateSeasonBlockGame, type SeasonBlockSimulationInput, } from './block.ts';
...
function runGameLoop(input: SeasonBlockSimulationInput, startIndex: number, startEffects: SeasonBlockSimulationInput['effects'], startHealth: SeasonHealthState, stopIndex?: number, initialPreviousRound?: number): {
...
describe('season franchise legal-five facts (M2.5 §9)', () => {
    it('reports legal with no injuries and lists every unavailable player otherwise', () => {
        ...
        if (roster === undefined)
            throw new Error('lakers roster');
        const allOut = roster.players.map((player, index) => injuryRecord({
        ...
        expect(illegal.unavailablePlayerVersionIds.sort()).toEqual(roster.players.map((player) => player.playerVersionId).sort());
    });
    it('uses the rotation planner machinery with position facts', () => {
        const { run, catalog } = buildTestRun();
        const positions = new Map(catalog.candidates.map((candidate) => [
        ...
        if (roster === undefined)
            throw new Error('lakers roster');
        const starters = run.rotations.find((rotation) => rotation.franchiseId === 'lakers')?.starters ?? [];
        const starterOut = roster.players
            .filter((player) => starters.includes(player.playerVersionId))
            .map((player, index) => injuryRecord({
            injuryId: `inj-${String(index).padStart(32, '0')}`,
            playerVersionId: player.playerVersionId,
            gameId: 's000001',
            seedPath: ['injuries', 's000001', player.playerVersionId, 'occurrence'],
        }));
        const withPositions = seasonFranchiseLegalFiveFacts(run, 'lakers', healthWith(starterOut), positions);
...
describe('season game health seam (M2.5 §9)', () => {
    it('produces the pregame availability for all 20 players and deterministic rolls', () => {
        ...
        const game = games[0];
        if (game === undefined)
            throw new Error('block 0 games');
        const rotationByFranchise = new Map(run.rotations.map((rotation) => [rotation.franchiseId, rotation]));
...
describe('season forfeit and pending assembly (M2.5 §10)', () => {
    it('builds the official 2-0 human-interruption forfeit summary', () => {
        ...
        const lakersGame = schedule.games.find((game) => game.homeFranchiseId === 'lakers' || game.awayFranchiseId === 'lakers');
        if (lakersGame === undefined)
            throw new Error('lakers game');
        ...
        const winnerScore = summary.homeFranchiseId === 'lakers' ? summary.awayScore : summary.homeScore;
...
describe('interruption and resume accounting (M2.5 §10)', () => {
    it('interrupts at the first human game and resumes without duplicating summaries', () => {
        ...
        if (roster === undefined)
            throw new Error('lakers roster');
        const blocked = roster.players.map((player, index) => injuryRecord({
            injuryId: `inj-${String(index).padStart(32, '0')}`,
            playerVersionId: player.playerVersionId,
            gameId: 's000001',
        ...
        const interruptionIndex = games.findIndex((game) => game.homeFranchiseId === 'lakers' || game.awayFranchiseId === 'lakers');
        const firstRun = runGameLoop(interruptedInput, 0, interruptedInput.effects, interruptedInput.health);
        ...
        expect(pending.summaries.map((summary) => summary.gameId)).toEqual(games.slice(0, interruptionIndex).map((game) => game.gameId));
        expect(pending.summaries.some((summary) => summary.gameId === games[interruptionIndex]?.gameId)).toBe(false);
        const legalInput = pipelineInput(run, catalog, 0);
        const legalBefore = runGameLoop(legalInput, 0, legalInput.effects, legalInput.health, interruptionIndex);
        ...
        const lastSimulatedRound = pending.summaries.length > 0
            ? pending.summaries[pending.summaries.length - 1]?.round
            : fromRoundOf(legalInput) - 1;
        const resumed = runGameLoop(legalInput, interruptionIndex, legalBefore.effects, legalBefore.health, undefined, lastSimulatedRound);
        ...
        const candidate = assembleSeasonBlockCandidate(legalInput, union, [], resumed.effects, resumed.health);
        const uninterrupted = assembleSeasonBlockCandidate(legalInput, legalFull.summaries, [], legalFull.effects, legalFull.health);
    ...
    it('forfeits every blocked human game deterministically and reproduces the digest', () => {
        ...
        const forfeitChain = (): string => {
            ...
            for (let index = 0; index < games.length; index += 1) {
                ...
                if ('interruption' in outcome)
                    throw new Error('unexpected interruption in an AI game');
                effects = outcome.effects;
                health = outcome.health;
                previousRound = game.round;
                summaries.push(outcome.summary);
            ...
            const humanGameCount = games.filter((game) => game.homeFranchiseId === 'lakers' || game.awayFranchiseId === 'lakers').length;
            expect(summaries.filter((summary) => summary.forfeitLoserFranchiseId === 'lakers')).toHaveLength(humanGameCount);
