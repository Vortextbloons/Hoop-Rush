...
export const ALL_FRANCHISES: readonly string[] = [
    ...SEASON_ALIGNMENT.filter((entry) => entry.conference === 'east').map((entry) => entry.franchiseId),
    ...SEASON_ALIGNMENT.filter((entry) => entry.conference === 'west').map((entry) => entry.franchiseId),
];
...
export function buildTestRun(options: {
...
function buildFreshTestRun(options: {
    humanFranchiseId?: string;
} = {}): TestRun {
    ...
    const humanPool = catalog.pools.find((pool) => pool.franchiseId === humanFranchiseId && pool.eraId === '1990s');
    if (humanPool === undefined)
        throw new Error('missing human pool');
...
const scheduleByLeague = new WeakMap<SeasonRun['league'], ReturnType<typeof generateSeasonSchedule>>();
...
export function blockCommand(run: SeasonRun, blockIndex: number, expectedRevision: number): SeasonSubmitBlockCommand {
    return {
        schemaVersion: SEASON_RUN_SCHEMA_VERSION,
        blockVersion: SEASON_BLOCK_VERSION,
...
export function pipelineInput(run: SeasonRun, catalog: SeasonDraftCatalog, blockIndex: number, priorSummaries: readonly SeasonGameSummary[] = [], effects: SeasonEffectsState = zeroEffectsOf(run)): SeasonBlockSimulationInput {
    return {
...
export function runBlock(state: RunnerState, blockIndex: number): import('@hoop-rush/data-contracts').SeasonCandidateCheckpoint {
