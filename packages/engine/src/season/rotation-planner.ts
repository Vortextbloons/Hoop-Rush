...
export function enumerateLegalFives(members: readonly PlannerMember[], available: ReadonlySet<string>): string[][] {
    ...
    const solve = (slot: number): void => {
        ...
        for (const member of members) {
            if (used.has(member.playerVersionId))
                continue;
            if (!available.has(member.playerVersionId))
                continue;
            if (!canPlay(member.playable, requirement))
                continue;
...
const plannerStateCache = new WeakMap<PlannerRotationContext, {
...
function plannerState(context: PlannerRotationContext): {
    ...
    if (state === undefined) {
        state = {
            ...
            benchIndex: new Map(context.rotation.benchOrder.map((playerVersionId, index) => [playerVersionId, index])),
        };
        plannerStateCache.set(context, state);
    }
    return state;
}
export function chooseInitialUnit(context: PlannerRotationContext, unavailable: ReadonlySet<string>): string[] | null {
    ...
    if (startersLegal)
        return [...starters];
    const available = new Set(members.map((member) => member.playerVersionId).filter((id) => !unavailable.has(id)));
...
export function planUnit(context: PlannerRotationContext, request: PlannerUnitRequest, options: {
    candidates?: readonly (readonly string[])[];
} = {}): string[] | null {
    const { members, benchIndex } = plannerState(context);
    const available = new Set(members.map((member) => member.playerVersionId).filter((id) => !request.unavailable.has(id)));
    ...
    if (preferClosing) {
        if (closingFiveIsLegal(context, request.unavailable))
            return [...context.rotation.closingFive];
        let best = first;
    ...
    for (let i = 1; i < candidates.length; i += 1) {
        ...
        const retentionCompare = overlapWith(candidate, currentUnitSet) - overlapWith(best, currentUnitSet);
        if (scoreCompare < 0 ||
...
export function plannerCandidates(context: PlannerRotationContext, unavailable: ReadonlySet<string>): readonly (readonly string[])[] {
    const members = plannerState(context).members;
    const available = new Set(members.map((member) => member.playerVersionId).filter((id) => !unavailable.has(id)));
...
function scoreParts(delta: number, context: PlannerRotationContext, request: PlannerUnitRequest): {
    base: number;
    adjustment: Map<string, number>;
} {
    let base = 0;
    ...
    const add = (playerVersionId: string): void => {
        ...
        adjustment.set(playerVersionId, Math.abs(actualSeconds + delta - targetSeconds) - Math.abs(actualSeconds - targetSeconds));
    };
...
function scoreOf(unit: readonly string[], base: number, adjustment: ReadonlyMap<string, number>): number {
    let total = base;
...
function overlapWith(unit: readonly string[], currentUnit: ReadonlySet<string>): number {
    let overlap = 0;
    for (const playerVersionId of unit) {
        if (currentUnit.has(playerVersionId))
            overlap += 1;
    }
    return overlap;
}
function currentOverlap(unit: readonly string[], currentUnit: readonly string[]): number {
    ...
    let overlap = 0;
    for (const playerVersionId of unit) {
        if (current.has(playerVersionId))
            overlap += 1;
    }
    return overlap;
}
function unitCompare(a: readonly string[], b: readonly string[], benchIndex: ReadonlyMap<string, number>): number {
...
function closingFiveIsLegal(context: PlannerRotationContext, unavailable: ReadonlySet<string>): boolean {
    ...
    for (let slot = 0; slot < STARTING_SLOTS.length; slot += 1) {
        ...
        const requirement = STARTING_SLOTS[slot];
        const playable = playerVersionId === undefined ? undefined : context.members.get(playerVersionId);
...
function closingPreferenceCompare(a: readonly string[], b: readonly string[], context: PlannerRotationContext, request: PlannerUnitRequest, benchIndex: ReadonlyMap<string, number>): number {
    const closingOverlapCompare = currentOverlap(a, context.rotation.closingFive) -
        currentOverlap(b, context.rotation.closingFive);
    if (closingOverlapCompare !== 0)
        return -closingOverlapCompare;
    const continuityCompare = currentOverlap(a, request.currentUnit) - currentOverlap(b, request.currentUnit);
