import { SEASON_INFLUENCE_FLOOR, SEASON_TRADE_VERSION, seasonNamespaceSeed, type SeasonDraftCatalog, type SeasonHealthState, type SeasonInfluenceState, type SeasonRoster, type SeasonRun, type SeasonTradeBoardTeamProfile, type SeasonTradeNegotiation, type SeasonTradeProposal, type SeasonTradeState, type SeasonTradeWindowState, } from '@hoop-rush/data-contracts';
...
function deriveNeeds(rosterIds: readonly string[], catalog: ReturnType<typeof seasonTradeCatalogFactsOf>): import('@hoop-rush/data-contracts').SeasonTradeNeed[] {
    ...
    if ((counts['G'] ?? 0) < 4)
        needs.push('ball-handling');
    if ((counts['F'] ?? 0) < 4)
        needs.push('rebounding');
    if ((counts['C'] ?? 0) < 3)
        needs.push('interior-defense');
    if (needs.length === 0)
        needs.push('depth');
    if (needs.length === 1) {
        needs.push('shooting');
...
export function generateTradeBoardProfiles(input: {
    ...
    for (const team of run.league.teams) {
        if (team.franchiseId === humanFranchiseId)
            continue;
        ...
        const priority = priorities[drawHexInt(prioritySeed) % priorities.length] as import('@hoop-rush/data-contracts').SeasonTradePriority;
...
export function evaluateTradeProposal(input: {
    ...
    const { run, windowIndex, toFranchiseId, outgoingPlayerVersionIds, incomingPlayerVersionIds, influenceAmount, influenceFromSender, catalog, rootSeed, } = input;
    ...
    if (outgoingPlayerVersionIds.length < 1 ||
    ...
    if (influenceFromSender !== null &&
        ...
        influenceFromSender !==
            run.league.teams.find((t) => t.franchiseId ===
                run.league.teams.find((x) => x.franchiseId === toFranchiseId)?.franchiseId)?.franchiseId) {
    }
    ...
    if (boardProfile) {
        ...
        for (const id of [...outgoingPlayerVersionIds, ...incomingPlayerVersionIds]) {
            const injuries = run.health.injuries.filter((inj) => inj.playerVersionId === id &&
                inj.missedGamesRemaining > 0 &&
                inj.sameGameReturned !== true);
            if (injuries.length > 0) {
                const hasMajor = injuries.some((inj) => inj.severity === 'major' || inj.severity === 'season-ending');
                if (hasMajor)
                    return { ok: false, code: 'trade-availability-risk', reason: `${id} injured` };
            }
        }
    }
    const fromAfterValues = fromAfter.map((id) => seasonTradePlayerValue(id, {
        run: run as unknown as import('./trades.ts').SeasonEconomyRun,
        catalogFacts,
        receivingFranchiseId: fromFranchiseId,
    }));
    const toAfterValues = toAfter.map((id) => seasonTradePlayerValue(id, {
        run: run as unknown as import('./trades.ts').SeasonEconomyRun,
        catalogFacts,
    ...
    const outgoingValues = outgoingPlayerVersionIds.map((id) => seasonTradePlayerValue(id, {
        run: run as unknown as import('./trades.ts').SeasonEconomyRun,
        catalogFacts,
        receivingFranchiseId: fromFranchiseId,
    }));
    const incomingValues = incomingPlayerVersionIds.map((id) => seasonTradePlayerValue(id, {
        run: run as unknown as import('./trades.ts').SeasonEconomyRun,
        catalogFacts,
        receivingFranchiseId: toFranchiseId,
    }));
...
export function openTradeInquiry(run: SeasonRun, windowIndex: number, toFranchiseId: string): {
    inquiryId: string;
    run: SeasonRun;
} | {
    error: string;
