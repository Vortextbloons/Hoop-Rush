import { SEASON_INFLUENCE_FLOOR, franchiseIdSchema, seasonNamespaceSeed, type SeasonDraftCatalog, type SeasonRun, type SeasonTradeNegotiation, type SeasonTradeProposal, type SeasonTradeState, type SeasonTradeWindowState, } from '@hoop-rush/data-contracts';
import { seasonTradeCatalogFactsOf, seasonTradePlayerValue, TRADE_BAND_1V1, TRADE_BAND_DEFAULT, } from './trades.ts';
...
export function evaluateTradeProposal(input: {
    ...
    const proposal: SeasonTradeProposal = {
        ...
        influenceFromSender: influenceFromSender === null ? null : franchiseIdSchema.parse(influenceFromSender),
        influenceAmount,
        fingerprint,
        consequenceFacts: {
...
export function openTradeInquiry(run: SeasonRun, windowIndex: number, toFranchiseId: string): {
    ...
    const negotiation: SeasonTradeNegotiation = {
        inquiryId,
        windowIndex,
        fromFranchiseId: franchiseIdSchema.parse(run.league.teams.find((t) => t.control === 'human')?.franchiseId ?? ''),
        toFranchiseId: franchiseIdSchema.parse(toFranchiseId),
        status: 'draft',
        exchangeCount: 0,
        exchanges: [],
