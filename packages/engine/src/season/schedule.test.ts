...
import { SEASON_COMMITTED_SCHEDULE_SEED, SEASON_GAME_COUNT, SEASON_ROUND_COUNT, seasonScheduleSchema, seedSchema, } from '@hoop-rush/data-contracts';
...
describe('generateSeasonSchedule', () => {
    ...
    it('applies the frozen frequency formula per team', () => {
        ...
        for (const game of schedule.games) {
            opponentCounts
                ...
                ?.set(game.awayFranchiseId, (opponentCounts.get(game.homeFranchiseId)?.get(game.awayFranchiseId) ?? 0) + 1);
            opponentCounts
                .get(game.awayFranchiseId)
                ?.set(game.homeFranchiseId, (opponentCounts.get(game.awayFranchiseId)?.get(game.homeFranchiseId) ?? 0) + 1);
        }
        for (const team of league.teams) {
    ...
    it('balances three-game pairings to six home games per team', () => {
        ...
        for (const game of schedule.games) {
            homeVersus
                .get(game.homeFranchiseId)
                ?.set(game.awayFranchiseId, (homeVersus.get(game.homeFranchiseId)?.get(game.awayFranchiseId) ?? 0) + 1);
            opponentCounts
                .get(game.homeFranchiseId)
                ?.set(game.awayFranchiseId, (opponentCounts.get(game.homeFranchiseId)?.get(game.awayFranchiseId) ?? 0) + 1);
            opponentCounts
        ...
        for (const team of league.teams) {
            ...
            for (const opponent of conferenceNonDivisionOpponentsOf(league, team.franchiseId)) {
                if ((opponentCounts.get(team.franchiseId)?.get(opponent) ?? 0) !== 3)
                    continue;
    ...
    it('supports a league with an altered alignment', () => {
        ...
        const altered = {
            ...
            teams: league.teams.map((team) => {
                if (team.franchiseId === 'hawks')
                    return { ...team, division: 'central' as const };
                if (team.franchiseId === 'pistons')
                    return { ...team, division: 'southeast' as const };
                return team;
            }),
        };
        const schedule = generateSeasonSchedule({
    ...
    it('rejects a league without 30 teams', () => {
        ...
        expect(() => generateSeasonSchedule({ league: short, seed: seedSchema.parse(seedFromString('short')) })).toThrow();
    });
});
