...
import { SEASON_MINUTE_POLICY_VERSION, SEASON_ROTATION_VERSION, franchiseIdSchema, type SeasonRotation, } from '@hoop-rush/data-contracts';
import { evaluateSeasonBlockObjective, seasonObjectiveCatalog, seasonObjectiveChoicesForBlock, } from './objectives.ts';
...
const OPPONENT_VERSIONS = Array.from({ length: 10 }, (_, index) => `pv-opp-${String(index).padStart(28, '0')}`);
function game(gameId: string, homeScore: number, awayScore: number, opts: {
...
function evaluate(objectiveId: 'win-six' | 'defense-108' | 'rebound-plus-20' | 'availability-eight' | 'bench-320' | 'turnover-130', summaries: Parameters<typeof evaluateSeasonBlockObjective>[0]['summaries'], tips: Parameters<typeof evaluateSeasonBlockObjective>[0]['tipAvailability'], rotation: SeasonRotation | null = ROTATION) {
...
describe('season objective measures', () => {
    it('win-six: succeeds with six wins, fails with five', () => {
        const sixWins = Array.from({ length: 10 }, (_, index) => game(`s${String(index + 1).padStart(6, '0')}`, index < 6 ? 100 : 90, index < 6 ? 90 : 100));
        ...
        const fiveWins = sixWins.map((summary, index) => index === 0 ? game(summary.gameId, 90, 100) : summary);
    ...
    it('win-six counts the official 2-0 result on forfeits', () => {
        const gamesList = Array.from({ length: 6 }, (_, index) => game(`s${String(index + 1).padStart(6, '0')}`, 100, 90));
        const forfeit = {
            ...game('s000007', 0, 0),
        ...
        const result = evaluate('win-six', [...gamesList, forfeit, humanWonForfeit, ...gamesList.slice(0, 2)], TEN_TIPS);
        expect(result.success).toBe(true);
        expect(result.evaluation.facts.wins).toBe(9);
    });
    it('defense-108: allows at most 1080 opponent box points', () => {
        const summaries = Array.from({ length: 10 }, (_, index) => game(`s${String(index + 1).padStart(6, '0')}`, 100, 108));
        expect(evaluate('defense-108', summaries, TEN_TIPS).success).toBe(true);
        const oneOver = summaries.map((summary, index) => index === 9 ? game(summary.gameId, 100, 109) : summary);
    ...
    it('rebound-plus-20: margin is human rebounds minus opponent rebounds', () => {
        const summaries = Array.from({ length: 10 }, (_, index) => game(`s${String(index + 1).padStart(6, '0')}`, 100, 90, {
            humanRebounds: 46,
            opponentRebounds: 44,
        }));
        ...
        const failure = evaluate('rebound-plus-20', summaries.map((summary, index) => index === 0
    ...
    it('bench-320: non-starter seconds count; starters identified by rotation', () => {
        ...
        const benchHeavy = summaries.map((summary) => ({
            ...summary,
            homePlayers: summary.homePlayers.map((line) => ROTATION.benchOrder.includes(line.playerVersionId) ? { ...line, seconds: 1920 } : line),
        }));
        ...
        const noBench = summaries.map((summary) => ({
            ...summary,
            homePlayers: summary.homePlayers.map((line) => ROTATION.benchOrder.includes(line.playerVersionId) ? { ...line, seconds: 0 } : line),
        }));
    ...
    it('bench-320 ignores rotation starters with zero target minutes', () => {
        const zeroStarterRotation: SeasonRotation = {
            ...ROTATION,
            targetMinutes: ROTATION.targetMinutes.map((entry) => entry.playerVersionId === 'pv-a' ? { ...entry, minutes: 0 } : entry),
        };
        const summaries = [game('s000001', 100, 90)];
        const result = evaluate('bench-320', summaries, [{ gameId: 's000001', availableCount: 9 }], zeroStarterRotation);
        expect(result.evaluation.facts.benchMinutes).toBe(6 * 24);
    });
    it('bench-320 measures zero when no rotation is supplied', () => {
        const summaries = [game('s000001', 100, 90)];
        const result = evaluate('bench-320', summaries, [{ gameId: 's000001', availableCount: 9 }], null);
        expect(result.evaluation.facts.benchMinutes).toBe(0);
    });
    it('turnover-130: at most 130 human turnovers', () => {
        const summaries = Array.from({ length: 10 }, (_, index) => game(`s${String(index + 1).padStart(6, '0')}`, 100, 90, { humanTurnovers: 13 }));
        const success = evaluate('turnover-130', summaries, TEN_TIPS);
        expect(success.success).toBe(true);
        expect(success.evaluation.facts.turnovers).toBe(130);
        const failure = evaluate('turnover-130', summaries.map((summary, index) => index === 9 ? game(summary.gameId, 100, 90, { humanTurnovers: 14 }) : summary), TEN_TIPS);
        expect(failure.success).toBe(false);
        expect(failure.evaluation.facts.turnovers).toBe(131);
    });
    it('records every fact from the summaries and never invents numbers', () => {
        const summaries = Array.from({ length: 10 }, (_, index) => game(`s${String(index + 1).padStart(6, '0')}`, 100 + index, 90 + index, {
            humanTurnovers: 12,
        }));
        const result = evaluate('defense-108', summaries, TEN_TIPS);
        const facts = result.evaluation.facts;
        expect(facts.games).toBe(10);
        expect(facts.pointsAllowed).toBe(summaries.reduce((sum, summary) => sum + summary.awayBox.points, 0));
        expect(facts.reboundMargin).toBe(summaries.reduce((sum, summary) => sum +
            summary.homeBox.offensiveRebounds +
            summary.homeBox.defensiveRebounds -
            summary.awayBox.offensiveRebounds -
            summary.awayBox.defensiveRebounds, 0));
        expect(facts.turnovers).toBe(summaries.reduce((sum, summary) => sum + summary.homeBox.turnovers, 0));
