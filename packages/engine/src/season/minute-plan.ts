import { SEASON_MINUTE_POLICY_VERSION, SEASON_ROTATION_PRESET_TARGETS, SEASON_ROTATION_VERSION, franchiseIdSchema, type SeasonMinutePolicyStrategy, type SeasonRotation, } from '@hoop-rush/data-contracts';
...
export function fatigueBandOf(fatigueBasisPoints: number): FatigueBand {
    if (fatigueBasisPoints < FATIGUE_BAND_FRESH_MAX)
        return 'fresh';
    if (fatigueBasisPoints < FATIGUE_BAND_READY_MAX)
        return 'ready';
    if (fatigueBasisPoints < FATIGUE_BAND_TIRED_MAX)
        return 'tired';
    return 'heavy';
}
...
export const STRATEGY_TO_PRESET: Record<SeasonMinutePolicyStrategy, keyof typeof SEASON_ROTATION_PRESET_TARGETS> = {
    'starter-heavy': 'tight',
    balanced: 'balanced',
    'bench-heavy': 'bench-heavy',
};
export function minuteStrategyOfPreset(preset: keyof typeof SEASON_ROTATION_PRESET_TARGETS): SeasonMinutePolicyStrategy {
    switch (preset) {
        case 'tight':
...
export function minuteCapacityOf(input: {
    ...
    const staminaDrain = MINUTE_PLAN_STAMINA_DRAIN * Math.max(0, Math.min(1, (95 - input.staminaRating) / 50));
    const durabilityDrain = MINUTE_PLAN_DURABILITY_DRAIN * Math.max(0, Math.min(1, (95 - input.durability) / 50));
    return Math.max(MINUTE_PLAN_CAPACITY_FLOOR, Math.min(1, 1 - fatigueDrain - staminaDrain - durabilityDrain));
}
export function projectFatigueAfterBlock(players: readonly MinutePlanPlayerInput[], minutesByVersion: ReadonlyMap<string, number>, horizon: number): Map<string, {
    fatigueBasisPoints: number;
    peakBasisPoints: number;
    band: FatigueBand;
}> {
    const state = new Map(players.map((player) => [
    ...
    for (let game = 0; game < horizon; game += 1) {
        for (const player of players) {
            ...
            if (seconds > 0) {
                const accumulated = onCourtFatigueBp(seconds, player.staminaRating, seconds, current.recentLoadBasisPoints);
        ...
        if (game < horizon - 1) {
            for (const player of players) {
                ...
                current.fatigueBasisPoints = Math.max(0, Math.min(10000, Math.round((current.fatigueBasisPoints * factor) / 10000)));
            }
        }
    }
...
function allocateGroup(entries: readonly {
    playerVersionId: string;
    weight: number;
}[], total: number): Map<string, number> {
    ...
    for (const entry of entries)
        result.set(entry.playerVersionId, 0);
    let pool = total;
        ...
        if (weightSum <= 0) {
            ...
            for (const entry of active) {
                ...
                if ((result.get(entry.playerVersionId) ?? 0) >= 48)
                    remaining.delete(entry.playerVersionId);
            }
            if (pool > 0) {
                for (const entry of [...active].sort((a, b) => a.playerVersionId < b.playerVersionId ? -1 : 1)) {
                    if (pool <= 0)
                        break;
                    if ((result.get(entry.playerVersionId) ?? 0) < 48) {
...
export function planQualityOf(targetMinutes: readonly {
    playerVersionId: string;
    minutes: number;
}[], qualityByVersion: ReadonlyMap<string, number>): number {
    const total = targetMinutes.reduce((sum, row) => sum + row.minutes, 0);
    return (targetMinutes.reduce((sum, row) => sum + row.minutes * (qualityByVersion.get(row.playerVersionId) ?? 0), 0) / Math.max(1, total));
}
export function benchReliefOf(targetMinutes: readonly {
    playerVersionId: string;
    minutes: number;
}[], benchOrder: readonly string[], qualityByVersion: ReadonlyMap<string, number>): number {
    const bench = new Set(benchOrder);
    const total = targetMinutes.reduce((sum, row) => sum + row.minutes * (qualityByVersion.get(row.playerVersionId) ?? 0), 0);
    const carried = targetMinutes.reduce((sum, row) => bench.has(row.playerVersionId)
...
export function riskScoreOf(input: {
    ...
    return (MINUTE_PLAN_SCORE_QUALITY_WEIGHT * input.quality +
...
function rotationOf(structure: MinutePlanStructure, targetMinutes: {
    playerVersionId: string;
    minutes: number;
}[], strategy: SeasonMinutePolicyStrategy, franchiseId = 'roster'): SeasonRotation {
    return {
...
function buildPlan(input: {
    ...
    const starterMinutes = allocateGroup(starterIds.map((id) => ({ playerVersionId: id, weight: weightOf(id) })), starterTotal);
    const benchMinutes = allocateGroup(benchIds.map((id) => ({ playerVersionId: id, weight: weightOf(id) })), 240 - starterTotal);
    const targetMinutes = [
    ...
    const qualityByVersion = new Map([...players.values()].map((player) => [player.playerVersionId, player.quality]));
    ...
    const fatigue = projectFatigueAfterBlock([...players.values()], new Map(targetMinutes.map((row) => [row.playerVersionId, row.minutes])), horizon);
    const maxStarterStrainBasisPoints = Math.max(0, ...starterIds.map((id) => fatigue.get(id)?.fatigueBasisPoints ?? 0));
    ...
    const heavyStrain = bands.heavy > 0 ||
...
function relativeQuality(plans: MinutePlanCandidate[]): Map<SeasonMinutePolicyStrategy, number> {
    ...
    if (max <= min)
        return new Map(plans.map((plan) => [plan.strategy, 0.5]));
    return new Map(plans.map((plan) => [
        plan.strategy,
        Math.max(0, Math.min(1, (plan.quality - min) / (max - min))),
    ]));
}
export function buildMinutePlanCandidates(input: {
    structure: MinutePlanStructure;
    players: ReadonlyMap<string, MinutePlanPlayerInput>;
    horizon: number;
}, options: {
    ...
    const quality = relativeQuality(raw.map((plan) => ({
        ...plan,
    ...
    const strategyOrder = (strategy: SeasonMinutePolicyStrategy) => MINUTE_POLICY_STRATEGIES.indexOf(strategy);
