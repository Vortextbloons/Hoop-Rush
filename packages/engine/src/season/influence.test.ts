...
import { SEASON_INFLUENCE_CAP, SEASON_INFLUENCE_FLOOR, franchiseIdSchema, seasonBlockRunContextSchema, type SeasonInfluenceState, type SeasonRun, } from '@hoop-rush/data-contracts';
...
describe('season influence block grants', () => {
    it('applies +1 to all 30 franchises and records ledger + transaction entries', () => {
        ...
        for (const franchiseId of Object.keys(outcome.influence.balances)) {
            expect(outcome.influence.balances[franchiseIdSchema.parse(franchiseId)]).toBe(franchiseId === humanFranchiseId ? 4 : 3);
        }
    ...
    it('reconciles every balance from the ledger (balanceAfter === before + applied)', () => {
        ...
        for (const franchiseId of Object.keys(second.balances)) {
            expect(second.balances[franchiseIdSchema.parse(franchiseId)]).toBe(fromLedger.get(franchiseId));
            expect(second.balances[franchiseIdSchema.parse(franchiseId)]).toBe(2 + (franchiseId === humanFranchiseId ? 3 : 2));
        }
    });
    it('cap-applies a grant at +8: appliedDelta 0 with the cap-reached explanation', () => {
        ...
        expect(influence.balances[franchiseIdSchema.parse(humanFranchiseId)]).toBe(SEASON_INFLUENCE_CAP);
        ...
        expect(atCap.influence.balances[franchiseIdSchema.parse(humanFranchiseId)]).toBe(SEASON_INFLUENCE_CAP);
...
describe('season influence spends', () => {
    ...
    it('rejects a spend that would cross the -3 floor with a typed error', () => {
        ...
        expect(influence.balances[franchiseIdSchema.parse(humanFranchiseId)]).toBe(SEASON_INFLUENCE_FLOOR);
