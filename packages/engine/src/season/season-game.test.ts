...
import { SEASON_MINUTE_POLICY_VERSION, SEASON_NEUTRAL_HOME_COURT, SEASON_ROTATION_PRESET_TARGETS, SEASON_ROTATION_VERSION, franchiseIdSchema, playerIdSchema, seasonGameSimulationResultSchema, playerVersionId, seedSchema, type Position, type SeasonGameSimulationInput, type SeasonGameTeamInput, type SeasonRotation, type SeasonRotationPreset, type SeasonRemoval, } from '@hoop-rush/data-contracts';
...
function buildSeasonTeam(side: 'home' | 'away', overrides: Partial<SeasonGameTeamInput> = {}): SeasonGameTeamInput {
...
function buildSeasonRotation(team: SeasonGameTeamInput): SeasonRotation {
    ...
    return {
        ...
        closingFive: [ids[1], ids[5], ids[6], ids[7], ids[8]].map((id) => {
            if (id === undefined)
                throw new Error('fixture closing five missing player');
            return id;
        }),
...
function buildSeasonGameInput(overrides: Partial<SeasonGameSimulationInput> = {}): SeasonGameSimulationInput {
...
describe('season game controller (M2.2)', () => {
    ...
    it('applies a tipoff-exact injury removal before choosing the opening unit', () => {
        ...
        if (playerVersionId === undefined)
            throw new Error('fixture home player missing');
    ...
    it('foul-out: a fouled-out player sits at the next pause and keeps six fouls', () => {
        ...
        const home = buildSeasonTeam('home');
        const away = buildSeasonTeam('away', {
            players: buildSeasonTeam('away').players.map((player, index) => index === 4
                ? {
                    ...player,
        ...
        for (let i = 0; i < 40 && found === null; i += 1) {
            ...
            if (result.outcome === 'completed' &&
                result.foulOuts.some((event) => event.side === 'away' && event.playerVersionId === away.players[4]?.playerVersionId)) {
                found = input;
                foundResult = result;
            }
        }
        ...
        const magnetEvent = foundResult.foulOuts.find((event) => event.side === 'away' && event.playerVersionId === away.players[4]?.playerVersionId);
        expect(magnetEvent).toBeDefined();
        const magnetPlayer = foundResult.away.players.find((player) => player.playerVersionId === away.players[4]?.playerVersionId);
        expect(magnetPlayer?.fouls).toBe(6);
        expect(checkSeasonGameResult(foundResult, found)).toEqual([]);
        const outPlayer = foundResult.away.players.find((p) => p.playerVersionId === away.players[4]?.playerVersionId);
    ...
    it('injected removal applies at the next legal boundary and is recorded', () => {
        ...
        if (event === undefined)
            throw new Error('expected a removal event');
        ...
        for (const stint of result.unitStints.filter((s) => s.side === 'away')) {
            if (!stint.players.includes(removalTarget.playerVersionId))
                continue;
            if (stint.period > 2)
                throw new Error('removed player played after the removal');
            if (stint.period === 2 && stint.endSecondsRemaining > 360) {
                throw new Error('removed player played past the removal clock');
            }
        }
        const deviation = result.deviations.find((d) => d.side === 'away' && d.playerVersionId === removalTarget.playerVersionId);
    ...
    it('applies same-period removals in descending game-clock order', () => {
        ...
        if (early === undefined || late === undefined)
            throw new Error('fixture missing starters');
        ...
        if (result.outcome !== 'completed')
            throw new Error('expected a completed game');
        const earlyEvent = result.removals.find((event) => event.playerVersionId === early.playerVersionId);
        const lateEvent = result.removals.find((event) => event.playerVersionId === late.playerVersionId);
    ...
    it('applies bench injury immediately and prevents court exposure', () => {
        ...
        if (benchPlayer === undefined)
            throw new Error('fixture missing bench player');
    ...
    it('pregame unavailability selects a contingency unit and marks causes', () => {
        ...
        const deviation = result.deviations.find((d) => d.side === 'home' && d.playerVersionId === unavailable.playerVersionId);
        expect(deviation).toBeDefined();
        expect(deviation?.reasons).toContain('pregame-unavailable');
        expect(deviation?.actualSeconds).toBe(0);
        const replacement = firstStint?.players.find((id) => !homeRotationStarters(input, 'home').includes(id));
        const replacementDeviation = result.deviations.find((d) => d.side === 'home' && d.playerVersionId === replacement);
...
describe('season game forfeits (M2.2)', () => {
    function allCenters(side: 'home' | 'away'): SeasonGameTeamInput {
        ...
        return {
            ...
            players: base.players.map((player, index) => ({
                ...
                playerVersionId: playerVersionId(`p-sg-${side}-c-${String(index)}`, base.franchiseId, '1990s', '1995-96'),
            })),
        };
    }
    it('forfeits 2-0 when one team cannot field a legal five at tipoff', () => {
        ...
        if (result.outcome !== 'forfeit')
            throw new Error('expected a forfeit');
...
describe('season game returns (M2.5 §11)', () => {
    it('applies a same-game return at the next legal boundary after the removal', () => {
        ...
        if (result.outcome !== 'completed')
            throw new Error('expected a completed game');
        const removalEvents = result.removals.filter((event) => event.playerVersionId === target.playerVersionId);
        expect(removalEvents).toHaveLength(1);
        expect(removalEvents[0]?.reason).toBe('injury');
        const returnEvents = result.away.returns.filter((event) => event.playerVersionId === target.playerVersionId);
        expect(returnEvents).toHaveLength(1);
        const returnEvent = returnEvents[0];
        if (returnEvent === undefined)
            throw new Error('expected a return event');
        ...
        const playedAfter = stints.some((stint) => stint.players.includes(target.playerVersionId) &&
            (stint.period > returnEvent.period ||
                (stint.period === returnEvent.period &&
                    stint.startSecondsRemaining <= returnEvent.secondsRemaining)));
        expect(playedAfter).toBe(true);
        const deviation = result.deviations.find((entry) => entry.side === 'away' && entry.playerVersionId === target.playerVersionId);
        expect(deviation?.reasons).toContain('injected-injury-removal');
    ...
    it('a return clock before the removal clock records both events without violating invariants', () => {
        ...
        if (result.outcome !== 'completed')
            throw new Error('expected a completed game');
        expect(result.away.returns.some((event) => event.playerVersionId === target.playerVersionId)).toBe(true);
        expect(result.removals.some((event) => event.playerVersionId === target.playerVersionId && event.reason === 'injury')).toBe(true);
        expect(checkSeasonGameResult(result, input)).toEqual([]);
    });
