import type { SeasonAiIdentity, SeasonRosterRole, SeasonStrengthBand, SimulationRatings, SimulationTendencies, } from '@hoop-rush/data-contracts';
...
export function identityScore(roleScores: Record<SeasonRosterRole, number>, identity: SeasonAiIdentity): number {
...
export function nearestRankThreshold(sortedAsc: readonly number[], percentile: number): number {
    ...
    if (percentile >= 1)
        return sortedAsc[sortedAsc.length - 1] ?? 0;
    const index = Math.ceil(percentile * sortedAsc.length) - 1;
    return sortedAsc[Math.min(sortedAsc.length - 1, Math.max(0, index))] ?? 0;
}
export function rolePercentileThresholds(canonicalScores: readonly Record<SeasonRosterRole, number>[]): Record<SeasonRosterRole, RoleThresholds> {
...
export function percentileTierOf(roleScores: Record<SeasonRosterRole, number>, thresholds: Record<SeasonRosterRole, RoleThresholds>): Record<SeasonRosterRole, PercentileTier> {
...
export function playerPercentileTier(roleTiers: Record<SeasonRosterRole, PercentileTier>): PercentileTier {
