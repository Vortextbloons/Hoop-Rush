import type { SeasonDraftCatalog, SeasonEffectsState, SeasonFreeAgencyBand, SeasonFreeAgencyCandidate, SeasonFreeAgencyCanonical, SeasonFreeAgencyDeclaration, SeasonFreeAgencyIndex, SeasonFreeAgencyIndexEntry, SeasonFreeAgencyResolutionTrace, SeasonFreeAgencyRoleExpectation, SeasonFreeAgencySigning, SeasonFreeAgencyState, SeasonFreeAgencyTarget, SeasonFreeAgencyTraceStep, SeasonFreeAgencyWindowState, SeasonInfluenceState, SeasonOwnership, SeasonRosterEntry, SeasonRosterRole, SeasonRun, SeasonTransactionEntry, } from '@hoop-rush/data-contracts';
import { SEASON_ROSTER_MAX_SIZE, SEASON_SEED_NAMESPACES, seasonNamespaceSeed, } from '@hoop-rush/data-contracts';
import type { SeasonRosterTargets } from '@hoop-rush/data-contracts';
import { ROSTER_ROLES, ROLE_COVERAGE_THRESHOLD, identityScore, roleScoresOf, type SeasonScoreMember, } from './ai-scoring.ts';
import { legalFiveAfterAnyRemoval, legalFiveExists, type SeasonRosterMemberInput, } from './roster-rules.ts';
...
export function seasonFreeAgencyUniverseOf(run: SeasonRun, index: SeasonFreeAgencyIndex): Map<string, SeasonFreeAgencyIndexEntry[]> {
    const ownedVersions = new Set(run.ownership.map((row) => row.playerVersionId));
    const representedIdentities = new Set(run.rosters.flatMap((roster) => roster.players.map((p) => p.playerId)));
    const signedVersions = new Set(run.freeAgency.windows.flatMap((window) => window.signings.map((signing) => signing.playerVersionId)));
    const universe = new Map<string, SeasonFreeAgencyIndexEntry[]>();
    for (const entry of index.candidates) {
        if (representedIdentities.has(entry.playerId))
            continue;
        if (ownedVersions.has(entry.playerVersionId))
            continue;
        if (signedVersions.has(entry.playerVersionId))
            continue;
...
export function canonicalFreeAgencyIdentity(rootSeed: string, windowIndex: number, playerId: string, entries: readonly SeasonFreeAgencyIndexEntry[]): SeasonFreeAgencyIndexEntry {
    if (entries.length === 0) {
        throw new Error(`free agency: no eligible versions for identity ${playerId}`);
    }
    if (entries.length === 1)
        return entries[0] as SeasonFreeAgencyIndexEntry;
    ...
    const completenessOf = (entry: SeasonFreeAgencyIndexEntry): number => {
        let complete = 0;
        if (entry.positions.playable.length > 0)
            complete += 1;
        if (entry.strengths.length > 0)
            complete += 1;
        if (entry.availability.healthy)
            complete += 1;
        if (entry.minutesPerGame > 0)
            complete += 1;
        return complete;
    };
    ...
    scored.sort((a, b) => {
        ...
        if (a.bandRank !== b.bandRank)
            return a.bandRank - b.bandRank;
        if (a.roleBreadth !== b.roleBreadth)
            return b.roleBreadth - a.roleBreadth;
        if (a.healthy !== b.healthy)
            return b.healthy - a.healthy;
        if (a.seedValue !== b.seedValue)
            return a.seedValue < b.seedValue ? -1 : 1;
        return a.entry.playerVersionId < b.entry.playerVersionId ? -1 : 1;
    });
    const best = scored[0];
    if (best === undefined)
        throw new Error(`free agency: no canonical version for ${playerId}`);
    return best.entry;
}
export function composeSeasonFreeAgencyWindow(context: SeasonFreeAgencyContext, windowIndex: number): SeasonFreeAgencyCandidate[] {
    ...
    for (const band of ['featured', 'role', 'development', 'emergency'] as const) {
        ...
        const groups: Array<{
            playerId: string;
            entry: SeasonFreeAgencyIndexEntry;
        }> = [];
        for (const [playerId, entries] of universe) {
            ...
            if (canonical !== undefined) {
                entry = entries.find((candidate) => candidate.playerVersionId === canonical.playerVersionId);
            }
            if (entry === undefined) {
        ...
        const seedOf = (groupId: {
            playerId: string;
        }): string => freeAgencySeed(run.rootSeed, String(windowIndex), 'composition', band, groupId.playerId);
        groups.sort((a, b) => {
    ...
    if (candidates.length > SEASON_FREE_AGENCY_WINDOW_MAX_CANDIDATES) {
        throw new Error(`free agency: window exceeds ${String(SEASON_FREE_AGENCY_WINDOW_MAX_CANDIDATES)} candidates`);
    }
    return candidates;
}
...
export function openSeasonFreeAgencyWindow(context: SeasonFreeAgencyContext, windowIndex: number, blockIndex: number): SeasonFreeAgencyWindowOpenResult {
    ...
    for (const candidate of candidates) {
        if (canonicalCandidates[candidate.playerId] !== undefined)
            continue;
    ...
    for (const team of run.league.teams) {
        if (team.franchiseId === context.humanFranchiseId)
            continue;
        declarations[team.franchiseId] = aiDeclarationOf(context, windowIndex, team.franchiseId, candidates);
    }
...
function aiDeclarationOf(context: SeasonFreeAgencyContext, windowIndex: number, franchiseId: string, candidates: SeasonFreeAgencyCandidate[]): SeasonFreeAgencyDeclaration {
    const { run } = context;
    ...
    const allowed = candidates.filter((candidate) => {
        ...
        if (!legal)
            return false;
        if (seasonSpend + candidate.minimumInfluence > 6)
            return false;
    ...
    const ranked = allowed
        ...
        if (needRank(a.need) !== needRank(b.need))
            return needRank(a.need) - needRank(b.need);
    ...
    for (const rankedEntry of ranked.slice(0, 2)) {
        if (committed >= 6)
            break;
        const influence = Math.min(Math.max(rankedEntry.candidate.minimumInfluence, 1), 3, 6 - committed);
        if (influence < rankedEntry.candidate.minimumInfluence)
            break;
        const expectation = supportedExpectationFor(context, franchiseId, identity, rankedEntry.candidate, rankedEntry.opportunity);
        targets.push({
...
function supportedExpectationFor(context: SeasonFreeAgencyContext, franchiseId: string, identity: string, candidate: SeasonFreeAgencyCandidate, opportunity: string): SeasonFreeAgencyRoleExpectation {
    const rotationEligible = candidate.supportedRoles.includes('rotation') && opportunity !== 'crowded';
    if (rotationEligible)
        return 'rotation';
    const emergencyEligible = candidate.supportedRoles.includes('emergency') &&
        emergencyRepairsRotation(context, franchiseId, candidate);
    if (emergencyEligible)
        return 'emergency';
    if (candidate.supportedRoles.includes('depth'))
        return 'depth';
    return 'depth';
}
function rosterLegalWithCandidate(context: SeasonFreeAgencyContext, roster: SeasonRosterLike | undefined, membership: SeasonRosterMemberInput): boolean {
    const members = rosterMembers(context, roster);
    members.push(membership);
    if (members.length > SEASON_ROSTER_MAX_SIZE)
        return false;
...
function rosterMembers(context: SeasonFreeAgencyContext, roster: SeasonRosterLike | undefined): SeasonRosterMemberInput[] {
...
function catalogCandidateIndex(context: SeasonFreeAgencyContext): Map<string, {
...
function memberOfCandidate(context: SeasonFreeAgencyContext, candidate: SeasonFreeAgencyCandidate): SeasonRosterMemberInput | null {
...
function aiWithinStrengthCeiling(context: SeasonFreeAgencyContext, franchiseId: string, band: string, membership: SeasonRosterMemberInput): boolean {
    ...
    const maxOutliers = context.targets?.policy.maxRosterStrengthOutliers ?? FALLBACK_MAX_ROSTER_STRENGTH_OUTLIERS;
    ...
    if (currentStrength > cap)
        return false;
    const rosterOutliers = members.filter((member) => identityScore(roleScoresOf(member), identity) > cap).length;
    const candidateOutlier = identityScore(roleScoresOf(membershipToMember(membership, context)), identity) > cap;
    if (currentStrength <= cap && !candidateOutlier)
        return true;
...
function scoreMembersOf(context: SeasonFreeAgencyContext, roster: SeasonRosterLike | undefined): SeasonFreeAgencyScoreMember[] {
...
function maxRoleCoverageIdentityScore(members: readonly SeasonFreeAgencyScoreMember[], identity: Parameters<typeof identityScore>[1]): number {
...
function membershipToMember(membership: SeasonRosterMemberInput, context: SeasonFreeAgencyContext): SeasonFreeAgencyScoreMember {
    const candidate = context.catalog.candidates.find((entry) => entry.playerVersionId === membership.playerVersionId);
    if (candidate === undefined) {
...
function needTierOf(context: SeasonFreeAgencyContext, franchiseId: string, candidate: SeasonFreeAgencyCandidate): 'high' | 'medium' | 'low' {
    const roster = context.run.rosters.find((entry) => entry.franchiseId === franchiseId);
...
function rosterRoleCoverage(context: SeasonFreeAgencyContext, roster: SeasonRosterLike | undefined): Record<SeasonRosterRole, number> {
    const coverage: Record<SeasonRosterRole, number> = {
        'primary-creation': 0,
        'secondary-creation': 0,
        'perimeter-shooting': 0,
...
function candidateRoleScores(context: SeasonFreeAgencyContext, candidate: SeasonFreeAgencyCandidate): Record<SeasonRosterRole, number> {
    const catalogCandidate = context.catalog.candidates[candidate.catalogRef.candidateIndex];
    if (catalogCandidate === undefined ||
        catalogCandidate.playerVersionId !== candidate.playerVersionId) {
...
function identityFitOf(context: SeasonFreeAgencyContext, franchiseId: string, identity: string, candidate: SeasonFreeAgencyCandidate): 'fits' | 'neutral' | 'misfit' {
    if (franchiseId === context.humanFranchiseId)
        return 'neutral';
...
function priorityRolesOf(context: SeasonFreeAgencyContext, identity: string): readonly SeasonRosterRole[] {
    const policy = (context.targets?.policy as unknown as {
        identityPriorityRoles?: Record<string, SeasonRosterRole[]>;
    } | undefined)?.identityPriorityRoles;
...
function opportunityOf(context: SeasonFreeAgencyContext, franchiseId: string, identity: string, candidate: SeasonFreeAgencyCandidate): 'immediate' | 'available' | 'crowded' {
    const roster = context.run.rosters.find((entry) => entry.franchiseId === franchiseId);
    const members = scoreMembersOf(context, roster);
    const candidateMember = membershipToMember({ playerVersionId: candidate.playerVersionId, playable: candidate.positions.playable }, context);
...
function emergencyRepairsRotation(context: SeasonFreeAgencyContext, franchiseId: string, candidate: SeasonFreeAgencyCandidate): boolean {
...
export function applyFreeAgencyDeclaration(run: SeasonRun, windowIndex: number, franchiseId: string, commandId: string, targets: SeasonFreeAgencyTarget[]): SeasonFreeAgencyState {
    ...
    return {
        ...run.freeAgency,
        windows: run.freeAgency.windows.map((entry) => entry.windowIndex === windowIndex
            ? { ...entry, declarations: { ...entry.declarations, [franchiseId]: declaration } }
            : entry),
    };
}
export function applyFreeAgencySkip(run: SeasonRun, windowIndex: number, franchiseId: string, commandId: string): SeasonFreeAgencyState {
...
type SeasonFreeAgencyRejectionLike = {
    code: string;
...
export function resolveSeasonFreeAgencyWindow(context: SeasonFreeAgencyContext, windowIndex: number, commandId: string): SeasonFreeAgencyResolutionResult {
    const { run } = context;
    const windowIndexEntry = run.freeAgency.windows.find((entry) => entry.windowIndex === windowIndex);
    ...
    const firstTargetOf = (franchiseId: string): SeasonFreeAgencyTarget | undefined => declarations[franchiseId]?.targets[0];
    const secondTargetOf = (franchiseId: string): SeasonFreeAgencyTarget | undefined => declarations[franchiseId]?.targets[1];
    for (const candidate of [...candidates].sort((a, b) => a.playerVersionId < b.playerVersionId ? -1 : 1)) {
        ...
        if (winner !== null) {
            ...
            if (declaration !== undefined && target !== undefined) {
                const signing = applySigning(context, windowIndexEntry.blockIndex, windowIndex, commandId, winner, target, candidate);
    ...
    const remaining = Object.keys(declarations)
        ...
        .sort();
    for (const candidate of [...candidates].sort((a, b) => a.playerVersionId < b.playerVersionId ? -1 : 1)) {
        if (signedVersions.has(candidate.playerVersionId))
            continue;
    ...
    for (const signing of signings) {
        const outcome = applySigningFacts(context, effects, influence, transactions, freeAgency, signing);
        resolvedSignings.push(outcome.signing);
        effects = outcome.effects;
        influence = outcome.influence;
        transactions = outcome.transactions;
        freeAgency = outcome.freeAgency;
        rosters = rosters.map((roster) => roster.franchiseId === signing.franchiseId
    ...
    freeAgency = {
        ...freeAgency,
        windows: freeAgency.windows.map((entry) => entry.windowIndex === windowIndex
            ? {
                ...entry,
...
function pickWinner(context: SeasonFreeAgencyContext, windowIndex: number, candidate: SeasonFreeAgencyCandidate, claims: string[], steps: SeasonFreeAgencyTraceStep[]): string | null {
    ...
    if (winner === null)
        return null;
    if (!claimLegal(context, winner, candidate, []))
        return null;
    return winner;
}
function claimLegal(context: SeasonFreeAgencyContext, franchiseId: string, candidate: SeasonFreeAgencyCandidate, steps: SeasonFreeAgencyTraceStep[]): boolean {
    const roster = context.run.rosters.find((entry) => entry.franchiseId === franchiseId);
    ...
    steps.push({
        ...
        citedFacts: reasons.length > 0 ? reasons.slice(0, 8) : ['roster capacity and identity checks passed'],
    });
    return reasons.length === 0;
}
function compareClaims(context: SeasonFreeAgencyContext, windowIndex: number, candidate: SeasonFreeAgencyCandidate, a: string, b: string, steps: SeasonFreeAgencyTraceStep[]): number {
    ...
    const roleRank = (credibility: string) => credibility === 'rotation'
        ? 0
    ...
    if (roleRank(bRole) !== roleRank(aRole))
        return roleRank(bRole) < roleRank(aRole) ? 1 : -1;
    ...
    if (needRank(bNeed) !== needRank(aNeed))
        return needRank(bNeed) < needRank(aNeed) ? 1 : -1;
    ...
    if (fitRank(bFit) !== fitRank(aFit))
        return fitRank(bFit) < fitRank(aFit) ? 1 : -1;
    ...
    const oppRank = (opportunity: string) => opportunity === 'immediate' ? 0 : opportunity === 'available' ? 1 : 2;
    steps.push({
        candidatePlayerVersionId: candidate.playerVersionId,
        franchiseId: b,
    ...
    if (bCommit !== aCommit)
        return bCommit > aCommit ? 1 : -1;
    const aDraw = freeAgencySeed(context.run.rootSeed, String(windowIndex), 'draw', candidate.playerVersionId, a);
    const bDraw = freeAgencySeed(context.run.rootSeed, String(windowIndex), 'draw', candidate.playerVersionId, b);
    steps.push({
        candidatePlayerVersionId: candidate.playerVersionId,
        franchiseId: b,
        criterion: 'draw',
    ...
    if (bDraw !== aDraw)
        return bDraw < aDraw ? 1 : -1;
    return a < b ? -1 : 1;
}
function roleCredibilityOf(context: SeasonFreeAgencyContext, franchiseId: string, candidate: SeasonFreeAgencyCandidate): string {
    const currentWindow = context.run.freeAgency.windows.find((window) => window.status === 'open');
    const declaration = currentWindow === undefined
        ? undefined
        : Object.values(currentWindow.declarations).find((entry) => entry.franchiseId === franchiseId);
    ...
    if (expectation === 'emergency') {
        if (emergencyRepairsRotation(context, franchiseId, candidate))
            return 'emergency';
        return 'depth';
    }
    return 'depth';
...
function assignmentIdentityOf(context: SeasonFreeAgencyContext, franchiseId: string): string {
    return (context.run.aiAssignments.find((entry) => entry.franchiseId === franchiseId)?.identity ??
        'continuity');
}
function committedInfluenceOf(context: SeasonFreeAgencyContext, franchiseId: string, candidate: SeasonFreeAgencyCandidate): number {
    const currentWindow = context.run.freeAgency.windows.find((window) => window.status === 'open');
    const declaration = currentWindow === undefined
        ? undefined
        : Object.values(currentWindow.declarations).find((entry) => entry.franchiseId === franchiseId);
    return (declaration?.targets.find((target) => target.playerVersionId === candidate.playerVersionId)
        ?.influence ?? 0);
}
...
function applySigning(context: SeasonFreeAgencyContext, blockIndex: number, windowIndex: number, commandId: string, franchiseId: string, target: SeasonFreeAgencyTarget, candidate: SeasonFreeAgencyCandidate): SeasonFreeAgencySigning {
    const seed = freeAgencySeed(context.run.rootSeed, String(windowIndex), 'signing', franchiseId, candidate.playerVersionId);
    return {
...
function applySigningFacts(context: SeasonFreeAgencyContext, effects: SeasonEffectsState, influence: SeasonInfluenceState, transactions: SeasonTransactionEntry[], freeAgency: SeasonFreeAgencyState, signing: SeasonFreeAgencySigning): SeasonSigningFacts {
    const { run } = context;
    ...
    return {
        ...
        freeAgency: {
            ...
            seasonSpend: {
                ...
                [signing.franchiseId]: (freeAgency.seasonSpend[signing.franchiseId] ?? 0) + signing.influenceCost,
            },
        },
        signing: syncedSigning,
    };
}
function rosterEntryOf(context: SeasonFreeAgencyContext, signing: SeasonFreeAgencySigning): SeasonRosterEntry {
    const catalogCandidate = context.catalog.candidates[signingCatalogIndexOf(context, signing)];
    if (catalogCandidate === undefined) {
...
function signingCatalogIndexOf(context: SeasonFreeAgencyContext, signing: SeasonFreeAgencySigning): number {
    for (const window of context.run.freeAgency.windows) {
        const candidate = window.candidates.find((entry) => entry.playerVersionId === signing.playerVersionId);
        if (candidate !== undefined)
            return candidate.catalogRef.candidateIndex;
    }
    return -1;
}
