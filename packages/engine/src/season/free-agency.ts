...
import { SEASON_ROSTER_MAX_SIZE, SEASON_SEED_NAMESPACES, commandIdSchema, franchiseIdSchema, idSchema, seasonNamespaceSeed, } from '@hoop-rush/data-contracts';
...
function syntheticCommandId(rootSeed: string, windowIndex: number, franchiseId: string): CommandId {
    return commandIdSchema.parse(`fa-ai-${freeAgencySeed(rootSeed, String(windowIndex), 'ai', franchiseId).slice(0, 32)}`);
}
