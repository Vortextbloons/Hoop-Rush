import { SEASON_MINUTE_POLICY_VERSION, SEASON_ROTATION_PRESET_TARGETS, SEASON_ROTATION_VERSION, franchiseIdSchema, seasonDigestHex, type SeasonRotation, type SeasonRotationCommandResult, type SeasonRotationPreset, type SeasonRotationRejectionCode, type SetSeasonRotationCommand, } from '@hoop-rush/data-contracts';
...
export function matchStartingFive(members: readonly SeasonRosterMemberInput[], order?: (a: SeasonRosterMemberInput, b: SeasonRosterMemberInput) => number): SeasonRosterMemberInput[] | null {
    const ordered = [...members].sort(order ?? canonicalOrder);
    if (!legalFiveExists(ordered))
        return null;
...
export function auditSeasonRotation(rotation: SeasonRotation, memberPlayable: ReadonlyMap<string, readonly Position[]>): string[] {
    return validateSeasonRotation(rotation, memberPlayable);
}
export function validateSeasonRotation(rotation: SeasonRotation, memberPlayable: ReadonlyMap<string, readonly Position[]>): string[] {
    const failures: string[] = [];
    const partition = [...rotation.starters, ...rotation.benchOrder];
    if (rotation.starters.length !== 5 || rotation.benchOrder.length !== 5) {
        failures.push(`rotation must reference exactly ten players (got ${String(rotation.starters.length)} starters and ${String(rotation.benchOrder.length)} bench)`);
    }
    ...
    if (rotation.targetMinutes.length !== 10) {
        failures.push(`rotation must have exactly ten target-minute entries (got ${String(rotation.targetMinutes.length)})`);
    }
    ...
    for (const entry of rotation.targetMinutes) {
        ...
        if (!Number.isInteger(entry.minutes) || entry.minutes < 0 || entry.minutes > 48) {
            failures.push(`target minutes for ${entry.playerVersionId} must be an integer from 0-48 (got ${String(entry.minutes)})`);
        }
    }
    for (const playerVersionId of partition) {
    ...
    if (rotation.closingFive.length !== 5) {
        failures.push(`closing five must contain exactly five players (got ${String(rotation.closingFive.length)})`);
    }
...
export function applySeasonRotationPreset(rotation: SeasonRotation, preset: SeasonRotationPreset): SeasonRotation {
...
function firstRotationRejection(rotation: SeasonRotation, memberPlayable: ReadonlyMap<string, readonly Position[]>): {
    ...
    if (message.includes('target minute') ||
...
export function handleSetSeasonRotationCommand(command: SetSeasonRotationCommand, memberPlayable: ReadonlyMap<string, readonly Position[]>, currentRotation?: SeasonRotation): SeasonRotationCommandResult {
    const rejected = (errorCode: SeasonRotationRejectionCode, message: string): SeasonRotationCommandResult => ({
        status: 'rejected',
    ...
    if (command.preset !== null) {
        ...
        const members: SeasonRosterMemberInput[] = [...memberPlayable.entries()].map(([playerVersionId, playable]) => ({ playerVersionId, playable }));
        if (members.length !== 10) {
            return rejected('ROSTER_MISMATCH', `rotation needs exactly ten rostered players (got ${String(members.length)})`);
        }
