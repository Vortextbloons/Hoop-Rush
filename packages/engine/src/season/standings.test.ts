...
function rowOf(standings: SeasonStandings, franchiseId: string) {
  ...
  if (!row) throw new Error(`no row for ${franchiseId}`);
  return row;
}
...
describe('auditSeasonStandings', () => {
  ...
  it('detects corrupted wins, losses, and scores', () => {
    const corrupt = (mutate: (standings: SeasonStandings) => SeasonStandings): string[] =>
      auditSeasonStandings(league, games, mutate(structuredClone(standings)));
    expect(
      corrupt((s) => {
        const row = s.rows.find((entry) => entry.franchiseId === 'lakers');
        if (row) row.wins += 1;
        return s;
      }).some((failure) => failure.includes('lakers wins')),
    ).toBe(true);
    expect(
      corrupt((s) => {
        const row = s.rows.find((entry) => entry.franchiseId === 'lakers');
        if (row) row.pointsFor -= 1;
        return s;
      }).some((failure) => failure.includes('lakers pointsFor')),
    ).toBe(true);
    expect(
      corrupt((s) => {
        const row = s.rows.find((entry) => entry.franchiseId === 'lakers');
        const against = row?.headToHead.find((entry) => entry.franchiseId === 'celtics');
        if (against) against.wins += 1;
        return s;
      }).some((failure) => failure.includes('head-to-head vs celtics wins')),
    ).toBe(true);
  });
  it('detects missing rows and duplicate rows', () => {
    ...
    if (!firstRow) throw new Error('no standings rows');
  ...
  it('rejects a table that contradicts a finalized game', () => {
    ...
    if (lakers) lakers.gamesPlayed = 99;
