...
import { eraIdSchema, franchiseIdSchema, playerIdSchema, seasonGameIdSchema, seasonKeySchema, seasonAwardsDigest, seasonAwardsSchema, type SeasonCompactPlayerLine, type SeasonGameSummary, type SeasonRoster, type SeasonTeamBox, } from '@hoop-rush/data-contracts';
...
function rostersOf(): SeasonRoster[] {
    ...
    return [LAKERS, CELTICS].map((franchiseId, teamIndex) => ({
        franchiseId: franchiseIdSchema.parse(franchiseId),
        players: fillerOf(franchiseId, teamIndex === 0 ? 50 : 70).map((versionId, index) => entry(franchiseId, versionId, index)),
    }));
}
interface GameSpec {
...
function summary(round: number, spec: GameSpec = {}): SeasonGameSummary {
    ...
    const lineupOf = (filler: string[], overrides?: Record<string, LineSpec>): SeasonCompactPlayerLine[] => {
        const ids = [...(overrides !== undefined ? Object.keys(overrides) : []), ...filler]
            ...
            .slice(0, 10);
        return ids.map((versionId, index) => line(versionId, {
            started: index < 5,
            ...(overrides?.[versionId] ?? {}),
        }));
    };
...
describe('season awards derivation (M2.6, awards-v1)', () => {
    ...
    it('never counts zero-second lines as appearances', () => {
        ...
        const awards = deriveSeasonAwards({ runId: 'run-1', rosters, summaries: games(10, spec) });
        expect(awards.mvp.playerVersionId).toBe(STAR);
        expect(awards.allLeagueFirstTeam.some((entry) => entry.playerVersionId === zeroMinutes)).toBe(false);
    });
    ...
    it('aggregates traded players by version with the most-appearance franchise', () => {
        ...
        const summaries = [
            ...
            ...Array.from({ length: 4 }, (_, index) => summary(index + 7, { awayLines: { [traded]: STAR_LINE } })),
        ];
    ...
    it('breaks traded-player franchise ties by starts then franchise id', () => {
        ...
        const summaries = [
            ...
            ...Array.from({ length: 5 }, (_, index) => summary(index + 6, {
                ...fiveFive,
    ...
    it('applies the availability factor to identical per-game production', () => {
        ...
        const summaries = [
            ...
            ...Array.from({ length: 3 }, (_, index) => summary(index + 8, { homeLines: { [full]: lineSpec } })),
        ];
        const awards = deriveSeasonAwards({ runId: 'run-1', rosters, summaries });
        expect(awards.mvp.playerVersionId).toBe(full);
    });
    ...
    it('produces a self-consistent deterministic digest that tracks recipients', () => {
        ...
        expect(deriveSeasonAwards({ runId: 'run-1', rosters, summaries: games(10, spec) })).toEqual(awards);
