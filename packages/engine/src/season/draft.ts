...
function selectPlayer(state: SeasonDraftState, catalog: SeasonDraftCatalog, command: SeasonDraftCommand): CommandResult {
    ...
    if (multiHumanDraft(state) && ownedPlayerIds(state, catalog).has(candidate.playerId)) {
        return {
            state,
            record: rejectedRecord(state, command, 'OWNED_VERSION', 'that player identity is already owned'),
            generation: null,
        };
    }
    ...
    if (multiHumanDraft(state)) {
        ...
        for (const pick of state.picks) {
            ...
            if (candidate === undefined) {
                return {
                    state,
                    record: rejectedRecord(state, command, 'INVALID_CATALOG', `pick references unknown version ${pick.playerVersionId}`),
                    generation: null,
                };
            }
            ...
            if (priorOwner !== undefined) {
                return {
                    state,
                    record: rejectedRecord(state, command, 'OWNED_VERSION', `human rosters claim identity ${candidate.playerId} more than once`),
                    generation: null,
                };
            }
...
export function applySeasonDraftCommand(state: SeasonDraftState | null, catalog: SeasonDraftCatalog, command: SeasonDraftCommand, deps: SeasonAiGenerationDeps): {
    ...
    switch (command.payload.kind) {
        ...
        default: {
            const exhaustive: never = command.payload;
            return assertNever(exhaustive, `unknown draft payload for ${command.commandId}`);
        }
    }
}
