...
import { type SeasonGameSummary, type SeasonPlayerAggregate, type SeasonRun, type SeasonStandings, } from '@hoop-rush/data-contracts';
...
describe('season block recap (M2.3)', () => {
    ...
    it('lists exactly the next block human games from the schedule', () => {
        ...
        const expectedCount = schedule.games.filter((game) => game.round >= fromRound &&
            game.round <= toRound &&
            (game.homeFranchiseId === 'lakers' || game.awayFranchiseId === 'lakers')).length;
