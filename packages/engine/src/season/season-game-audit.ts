...
function substitutionAudit(
  ...
  for (const candidate of subs) {
    if (candidate.reason !== 'foul-out' && candidate.reason !== 'injected-injury-removal') continue;
    const boundaryHasEvent = (
      candidate.reason === 'foul-out' ? result.foulOuts : result.removals
    ).some(
      (event) =>
        event.side === sideKey &&
