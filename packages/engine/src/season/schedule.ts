import { SEASON_GAME_COUNT, SEASON_LEAGUE_VERSION, SEASON_ROUND_COUNT, SEASON_SCHEDULE_FORMULA_VERSION, SEASON_SCHEDULE_VERSION, SEASON_TEAM_COUNT, franchiseIdSchema, seasonGameIdSchema, seasonNamespaceSeed, seasonScheduleSchema, type SeasonLeague, type SeasonSchedule, type SeasonScheduleGame, type Seed, } from '@hoop-rush/data-contracts';
import { createRng, shuffle } from '../sim/rng.ts';
import { conferenceNonDivisionOpponentsOf, divisionOpponentsOf, oppositeConferenceOpponentsOf, } from './league.ts';
    ...
    const augmentingEndpoint = (root: number): number => {
        ...
        while (head < queue.length) {
            ...
            for (const u of adjacency[v] ?? []) {
                if ((base[v] ?? v) === (base[u] ?? u) || match[v] === u)
                    continue;
...
function factorizeScheduleGraph(teamCount: number, edges: ExtraEdge[], factorCount: number): Factor[] {
    ...
    for (let factorIndex = 0; factorIndex < factorCount; factorIndex += 1) {
        ...
        for (let a = 0; a < teamCount; a += 1) {
            ...
            if (b <= a)
                continue;
            const edge = edges.find((candidate) => candidate.a === a && candidate.b === b && candidate.remaining > 0);
            if (edge === undefined) {
    ...
    for (const conference of conferences) {
        ...
        for (let a = 0; a < divisions.length; a += 1) {
            for (let b = a + 1; b < divisions.length; b += 1) {
                ...
                const rng = createRng(seasonNamespaceSeed(seed, 'schedule-formula', 'four-game', conference, left, right));
...
function orientThreeGamePairs(pairs: ReadonlyArray<readonly [
    string,
    string
    ...
    for (const team of incident.keys())
        counts.set(team, 0);
    const decided = new Set<string>();
    const remainingFor = (team: string): number => (incident.get(team) ?? []).filter((opponent) => !decided.has(pairKey(team, opponent))).length;
    const solve = (index: number): boolean => {
        ...
        if (pair === undefined)
            throw new Error('missing three-game pair');
        const [a, b] = pair;
...
function buildScheduleEdges(league: SeasonLeague, teamOrder: readonly string[], teamIndex: ReadonlyMap<string, number>, fourGame: ReadonlyMap<string, ReadonlySet<string>>, hostsTwice: ReadonlyMap<string, ReadonlySet<string>>): ExtraEdge[] {
    ...
    for (let a = 0; a < teamOrder.length; a += 1) {
        ...
        if (leftId === undefined || left === undefined)
            continue;
        for (let b = a + 1; b < teamOrder.length; b += 1) {
            ...
            if (rightId === undefined || right === undefined)
                continue;
            let remaining = 2;
            let homeCopies = 1;
...
function orientScheduleFactors(factors: readonly Factor[], edges: readonly ExtraEdge[], teamCount: number): Map<string, number> {
...
function spaceFactorRematches(factors: readonly Factor[], seed: Seed, generationVersion: string, attempt: number): Factor[] {
    const remaining = shuffle(factors.map((factor, index) => ({ factor, index, edges: factorEdgeIds(factor) })), createRng(seasonNamespaceSeed(seed, 'schedule-rounds', generationVersion, `attempt-${String(attempt)}`, 'round-order')));
    ...
    while (remaining.length > 0) {
        ...
        if (selected === undefined)
            throw new Error('missing scheduled factor');
        ordered.push(selected.factor);
        recent.unshift(selected.edges);
        if (recent.length > MINIMUM_OPPONENT_REPEAT_GAP)
            recent.pop();
    }
    return ordered;
}
function buildScheduleAttempt(league: SeasonLeague, seed: Seed, generationVersion: string, attempt: number): SeasonSchedule {
    const teamOrder = shuffle(league.teams.map((team) => team.franchiseId), createRng(seasonNamespaceSeed(seed, 'schedule-rounds', generationVersion, `attempt-${String(attempt)}`, 'team-order')));
    const teamIndex = new Map<string, number>();
    for (const teamId of teamOrder)
        teamIndex.set(teamId, teamIndex.size);
    const fourGame = fourGameOpponents(league, seed);
    const threeGamePairs: Array<readonly [
        string,
        string
    ]> = [];
    for (const team of league.teams) {
        for (const opponent of conferenceNonDivisionOpponentsOf(league, team.franchiseId)) {
            if (fourGame.get(team.franchiseId)?.has(opponent))
                continue;
            const [a, b] = [team.franchiseId, opponent].sort();
            if (a === team.franchiseId && b !== undefined)
    ...
    const factors = spaceFactorRematches(factorizeScheduleGraph(teamOrder.length, scheduleEdges, SEASON_ROUND_COUNT), seed, generationVersion, attempt);
    ...
    if (rounds.length !== SEASON_ROUND_COUNT) {
        throw new Error(`generated ${String(rounds.length)} rounds (expected ${String(SEASON_ROUND_COUNT)})`);
    }
    const games: Array<{
        round: number;
    ...
    rounds.forEach((round, index) => {
        if (round.length !== teamOrder.length / 2)
            throw new Error('generated an incomplete round');
...
export function generateSeasonSchedule(input: GenerateSeasonScheduleInput): SeasonSchedule {
    ...
    throw new Error(`schedule generation failed after ${String(maxAttempts)} attempts: ${lastError?.message ?? 'unknown'}`);
}
export function auditSeasonSchedule(schedule: SeasonSchedule, league: SeasonLeague): string[] {
    ...
    if (schedule.games.length !== SEASON_GAME_COUNT) {
        failures.push(`games must be ${String(SEASON_GAME_COUNT)} (got ${String(schedule.games.length)})`);
        return failures;
    }
    ...
    for (const game of schedule.games) {
        if (gameIds.has(game.gameId))
            failures.push(`duplicate gameId ${game.gameId}`);
    ...
    for (let round = 1; round <= SEASON_ROUND_COUNT; round += 1) {
        ...
        if (list.length !== SEASON_TEAM_COUNT / 2) {
            failures.push(`round ${String(round)} must have ${String(SEASON_TEAM_COUNT / 2)} games (got ${String(list.length)})`);
        }
        ...
        if (seen.size !== SEASON_TEAM_COUNT) {
            failures.push(`round ${String(round)} must cover all ${String(SEASON_TEAM_COUNT)} franchises (got ${String(seen.size)})`);
        }
    }
    const mostRecentOpponentRound = new Map<string, Map<string, number>>();
    for (const franchiseId of franchiseIds)
        mostRecentOpponentRound.set(franchiseId, new Map());
    for (let round = 1; round <= SEASON_ROUND_COUNT; round += 1) {
        for (const game of gamesByRound.get(round) ?? []) {
            ...
            if (previousHomeRound === round - 1) {
                failures.push(`${game.homeFranchiseId} and ${game.awayFranchiseId} repeat in consecutive rounds ${String(previousHomeRound)} and ${String(round)}`);
            }
    ...
    for (const team of league.teams) {
        ...
        if (home !== SEASON_ROUND_COUNT / 2) {
            failures.push(`${id} must play ${String(SEASON_ROUND_COUNT / 2)} home games (got ${String(home)})`);
        }
        if (away !== SEASON_ROUND_COUNT / 2) {
            failures.push(`${id} must play ${String(SEASON_ROUND_COUNT / 2)} away games (got ${String(away)})`);
        }
        ...
        for (const opponent of conferenceNonDivisionOpponentsOf(league, id)) {
            ...
            if (count === 4) {
                ...
                failures.push(`${id} must play conference opponent ${opponent} 3 or 4 times (got ${String(count)})`);
            }
        }
        if (fourGameCount !== 6 || threeGameCount !== 4) {
            failures.push(`${id} must face 6 conference opponents four times and 4 three times (got ${String(fourGameCount)} and ${String(threeGameCount)})`);
        }
        if (threeGameHome !== 6) {
            failures.push(`${id} must hold exactly 6 home games across its three-game pairings (got ${String(threeGameHome)})`);
        }
