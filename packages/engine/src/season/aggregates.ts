import { SEASON_LEADER_DEPTH, SEASON_LEADER_MIN_GAME_SHARE, SEASON_LEADERS_VERSION, type SeasonGameSummary, type SeasonLeaderCategory, type SeasonLeaderEntry, type SeasonLeaders, type SeasonPlayerAggregate, type SeasonStandings, type SeasonTeamAggregate, } from '@hoop-rush/data-contracts';
...
export function foldSeasonTeamAggregates(summaries: readonly SeasonGameSummary[]): SeasonTeamAggregate[] {
...
export function foldSeasonPlayerAggregates(summaries: readonly SeasonGameSummary[]): SeasonPlayerAggregate[] {
    ...
    return [...rows.values()].sort((a, b) => a.playerVersionId < b.playerVersionId ? -1 : a.playerVersionId > b.playerVersionId ? 1 : 0);
}
export function auditSeasonAggregates(input: {
    ...
    for (const row of input.teams) {
        ...
        for (const field of teamFields) {
            if (row[field] !== want[field]) {
                failures.push(`team ${row.franchiseId} ${field}: stored ${String(row[field])} != fold ${String(want[field])}`);
            }
        }
    }
    ...
    const playerFields: ReadonlyArray<keyof Omit<SeasonPlayerAggregate, 'playerVersionId' | 'franchiseId'>> = [
        'gamesPlayed',
        'appearances',
        'started',
        'seconds',
    ...
    for (const row of input.players) {
        ...
        if (row.franchiseId !== want.franchiseId) {
            failures.push(`player ${row.playerVersionId} franchise: stored ${row.franchiseId} != fold ${want.franchiseId}`);
        }
        for (const field of playerFields) {
            if (row[field] !== want[field]) {
                failures.push(`player ${row.playerVersionId} ${field}: stored ${String(row[field])} != fold ${String(want[field])}`);
            }
        }
    }
    ...
    for (const row of freshPlayers) {
        if (!storedPlayerIds.has(row.playerVersionId)) {
            failures.push(`player ${row.playerVersionId} played games but is missing from the stored table`);
        }
    }
    const recordOf = new Map<string, {
        wins: number;
        losses: number;
        gamesPlayed: number;
    }>();
    ...
    for (const summary of input.summaries) {
        ...
        pointsFor.set(summary.homeFranchiseId, (pointsFor.get(summary.homeFranchiseId) ?? 0) + homeScore);
        pointsFor.set(summary.awayFranchiseId, (pointsFor.get(summary.awayFranchiseId) ?? 0) + awayScore);
        pointsAgainst.set(summary.homeFranchiseId, (pointsAgainst.get(summary.homeFranchiseId) ?? 0) + awayScore);
        pointsAgainst.set(summary.awayFranchiseId, (pointsAgainst.get(summary.awayFranchiseId) ?? 0) + homeScore);
    }
    for (const row of input.standings.rows) {
        ...
        if (record !== undefined) {
            if (row.wins !== record.wins) {
                failures.push(`standings ${row.franchiseId} wins: stored ${String(row.wins)} != summaries ${String(record.wins)}`);
            }
            if (row.losses !== record.losses) {
                failures.push(`standings ${row.franchiseId} losses: stored ${String(row.losses)} != summaries ${String(record.losses)}`);
            }
            if (row.gamesPlayed !== record.gamesPlayed) {
                failures.push(`standings ${row.franchiseId} gamesPlayed: stored ${String(row.gamesPlayed)} != summaries ${String(record.gamesPlayed)}`);
            }
        }
        ...
        if (row.pointsFor !== expectedPointsFor) {
            failures.push(`standings ${row.franchiseId} pointsFor: stored ${String(row.pointsFor)} != summaries ${String(expectedPointsFor)}`);
        }
        if (row.pointsAgainst !== expectedPointsAgainst) {
            failures.push(`standings ${row.franchiseId} pointsAgainst: stored ${String(row.pointsAgainst)} != summaries ${String(expectedPointsAgainst)}`);
        }
    }
    return failures;
}
function categoryValue(player: SeasonPlayerAggregate, category: SeasonLeaderCategory): number {
    switch (category) {
        ...
        default:
            return assertNever(category, `unknown leader category ${String(category)}`);
    }
}
export function deriveSeasonLeaders(teams: readonly SeasonTeamAggregate[], players: readonly SeasonPlayerAggregate[]): SeasonLeaders {
