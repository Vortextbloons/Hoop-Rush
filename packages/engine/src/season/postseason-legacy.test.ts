...
import { SEASON_POSTSEASON_LEGACY_VERSION as SEASON_POSTSEASON_VERSION, type ConferenceId, type PlayoffBracketV1 as PlayoffBracket, type PlayoffSeriesV1 as PlayoffSeries, type SeasonPostseasonStateV1 as SeasonPostseasonState, } from '@hoop-rush/data-contracts';
import { buildSeasonLeague } from '@hoop-rush/test-fixtures';
import { createRng } from '../sim/rng.ts';
import { auditSeasonPostseason, createPlayoffBracket, currentSeriesId, setPlayInRankings, submitPlayInGame, submitPlayoffGame, } from './postseason-legacy.ts';
...
function playSeries(state: SeasonPostseasonState, seriesId: string, pattern: ReadonlyArray<'homeCourt' | 'challenger'>): SeasonPostseasonState {
    let current = state;
...
function playFullTournament(state: SeasonPostseasonState, rng: ReturnType<typeof createRng>): SeasonPostseasonState {
...
describe('setPlayInRankings', () => {
    ...
    it('rejects malformed or foreign rankings', () => {
        ...
        expect(() => setPlayInRankings(emptyPostseason(), league, { ...base, east: eastTeams.slice(0, 9) })).toThrow();
        expect(() => setPlayInRankings(emptyPostseason(), league, {
            ...base,
    ...
    it('rejects re-ranking a conference', () => {
        const state = rankedPostseason();
        expect(() => setPlayInRankings(state, league, {
            east: eastTeams.slice(0, 10),
            west: westTeams.slice(0, 10),
        })).toThrow();
    });
...
describe('submitPlayInGame', () => {
    ...
    it('rejects illegal Play-In submissions', () => {
        let state = rankedPostseason();
        expect(() => submitPlayInGame(state, 'east', 'final', { status: 'final', homeScore: 100, awayScore: 90 })).toThrow();
        state = submitPlayInGame(state, 'east', 'seven-eight', {
            status: 'final',
            homeScore: 101,
            awayScore: 99,
        });
        expect(() => submitPlayInGame(state, 'east', 'seven-eight', {
            status: 'final',
            homeScore: 110,
            awayScore: 100,
        })).toThrow();
        expect(() => submitPlayInGame(state, 'east', 'nine-ten', {
            status: 'final',
            homeScore: 90,
            awayScore: 90,
        })).toThrow();
        ...
        expect(() => submitPlayInGame(unranked, 'east', 'seven-eight', {
            status: 'final',
            homeScore: 100,
            awayScore: 90,
        })).toThrow();
...
describe('createPlayoffBracket', () => {
    it('requires both Play-In tournaments before creating the bracket', () => {
        expect(() => createPlayoffBracket(rankedPostseason(), league, eastTeams[0] ?? 'hawks')).toThrow();
    });
    ...
    it('creates fixed first-round pairings with the higher seed at home', () => {
        ...
        east.firstRound.forEach((series, index) => {
            ...
            if (!pair)
                throw new Error(`no pairing for series ${String(index + 1)}`);
...
describe('submitPlayoffGame', () => {
    it('only accepts the current series', () => {
        ...
        expect(() => submitPlayoffGame(state, 'east-first-round-2', {
            status: 'final',
            homeScore: 100,
            awayScore: 90,
        })).toThrow();
        expect(() => submitPlayoffGame(state, 'finals', { status: 'final', homeScore: 100, awayScore: 90 })).toThrow();
    });
    ...
    it('stops the series immediately at four wins', () => {
        ...
        expect(() => submitPlayoffGame(state, 'east-first-round-1', {
            status: 'final',
            homeScore: 100,
            awayScore: 90,
        })).toThrow();
    ...
    it('audits a corrupt bracket and corrupt Play-In states', () => {
        ...
        const tampered = {
            ...
            bracket: {
                ...
                east: {
                    ...bracket.east,
                    firstRound: bracket.east.firstRound.map((series, index) => index === 1
