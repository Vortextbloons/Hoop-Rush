import { SEASON_HEALTH_VERSION, SEASON_RECAP_VERSION, franchiseIdSchema, seasonGameIdSchema, type FranchiseId, type SeasonBlockInjuryEvidence, type SeasonBlockRecap, type SeasonCompactInjuryEvent, type SeasonGameSummary, type SeasonHealthState, type SeasonInfluenceState, type SeasonNotablePerformance, type SeasonObjectiveEvaluation, type SeasonObjectiveId, type SeasonPlayerAggregate, type SeasonRecordMovement, type SeasonSchedule, type SeasonStandings, type SeasonStreak, type SeasonTransactionEntry, type SeasonUpcomingHumanGame, type SeasonVersionSpotlight, type SeasonFreeAgencyBand, type SeasonFreeAgencyState, } from '@hoop-rush/data-contracts';
...
function blockSummariesOf(summaries: readonly SeasonGameSummary[], blockIndex: number): SeasonGameSummary[] {
...
function movementOf(standingsBefore: SeasonStandings, standingsAfter: SeasonStandings, franchiseId: string): SeasonRecordMovement {
...
function notableLines(blockSummaries: readonly SeasonGameSummary[], humanFranchiseId: string | null): SeasonNotablePerformance[] {
    ...
    return lines.sort((a, b) => (b.humanTeam ? 1 : 0) - (a.humanTeam ? 1 : 0) ||
        b.points - a.points ||
...
function chronologicallyOrdered(summaries: readonly SeasonGameSummary[]): SeasonGameSummary[] {
    return [...summaries].sort((a, b) => a.round - b.round || (a.gameId < b.gameId ? -1 : a.gameId > b.gameId ? 1 : 0));
}
...
function headToHead(blockSummaries: readonly SeasonGameSummary[], franchiseA: string, franchiseB: string): {
    games: number;
    winsA: number;
    winsB: number;
} {
    ...
    for (const summary of blockSummaries) {
        ...
        if (home === franchiseA && away === franchiseB) {
            games += 1;
            if (winnerOf(summary) === franchiseA)
                winsA += 1;
            else
                winsB += 1;
        }
            ...
            games += 1;
            if (winnerOf(summary) === franchiseB)
                winsB += 1;
            else
                winsA += 1;
        }
...
function versionSpotlightsOf(input: SeasonBlockRecapInput): SeasonVersionSpotlight[] {
    ...
    for (const [playerVersionId, playerId] of input.rosterPlayerIds) {
        if (!playedInBlock.has(playerVersionId))
            continue;
        const versions = versionsOfPerson.get(playerId) ?? [];
        versions.push(playerVersionId);
        versionsOfPerson.set(playerId, versions);
    }
    const aggregateOf = new Map(input.playerAggregates.map((player) => [player.playerVersionId, player]));
    const franchiseOf = (playerVersionId: string): string | null => aggregateOf.get(playerVersionId)?.franchiseId ?? null;
    ...
    for (const versions of versionsOfPerson.values()) {
        ...
        for (let i = 0; i < sorted.length; i += 1) {
            for (let j = i + 1; j < sorted.length; j += 1) {
                ...
                if (franchiseA === null || franchiseB === null)
                    continue;
    ...
    return pairs
        .sort((a, b) => b.headToHeadGames - a.headToHeadGames ||
...
function upcomingHumanGamesOf(input: SeasonBlockRecapInput): SeasonUpcomingHumanGame[] {
    ...
    return input.schedule.games
        ...
        opponentFranchiseId: game.homeFranchiseId === input.humanFranchiseId
...
export function blockInjuryEvidenceOf(input: {
    ...
    for (const summary of blockSummaries) {
        const humanSide = humanFranchiseId === null
            ? null
...
export function blockFreeAgencyEvidenceOf(input: {
    ...
    return {
        ...
        seasonSignings: input.humanFranchiseId === null
            ? 0
...
export function blockTradeEvidenceOf(input: {
    ...
    const tradesAccepted = input.transactions?.filter((entry) => entry.type === 'trade' && entry.blockIndex === input.blockIndex).length ?? 0;
    let influenceDelta = 0;
    if (input.humanFranchiseId !== null && input.influence !== undefined) {
        influenceDelta = input.influence.ledger
            .filter((entry) => entry.franchiseId === input.humanFranchiseId && entry.blockIndex === input.blockIndex)
            .reduce((sum, entry) => sum + entry.appliedDelta, 0);
    }
    return { tradesAccepted, influenceDelta };
}
export function humanInfluenceBalanceAtBlockEnd(influence: SeasonInfluenceState, humanFranchiseId: string | null, blockIndex: number): number {
...
export function buildSeasonBlockRecap(input: SeasonBlockRecapInput): SeasonBlockRecap {
    const humanRecord = input.humanFranchiseId === null
        ? null
    ...
    const notablePerformances = notableLines(blockSummariesOf(input.summaries, input.blockIndex), input.humanFranchiseId).slice(0, 10);
    ...
    return {
        ...
        objectiveEvidence: objective !== null && objective.objectiveId !== null
            ? {
        ...
        freeAgencyEvidence,
        influenceBalance: {
            humanBalance: input.influence === undefined
                ? 0
                : humanInfluenceBalanceAtBlockEnd(input.influence, input.humanFranchiseId, input.blockIndex),
        },
    };
}
export function seasonBlockRecapCanonical(recap: SeasonBlockRecap): string {
    return canonicalJson({
        ...
        standingsMovement: [...recap.standingsMovement].sort((a, b) => a.franchiseId < b.franchiseId ? -1 : 1),
        notablePerformances: [...recap.notablePerformances].sort((a, b) => (a.playerVersionId < b.playerVersionId
            ? -1
            : a.playerVersionId > b.playerVersionId
                ? 1
                : 0) || (a.gameId < b.gameId ? -1 : 1)),
        streaks: [...recap.streaks].sort((a, b) => (a.franchiseId < b.franchiseId ? -1 : a.franchiseId > b.franchiseId ? 1 : 0) ||
            (a.kind === b.kind ? 0 : a.kind === 'wins' ? -1 : 1)),
        versionSpotlights: [...recap.versionSpotlights].sort((a, b) => (a.versionA < b.versionA ? -1 : a.versionA > b.versionA ? 1 : 0) ||
            (a.versionB < b.versionB ? -1 : 1)),
        upcomingHumanGames: [...recap.upcomingHumanGames].sort((a, b) => a.gameId < b.gameId ? -1 : 1),
        freeAgencyEvidence: {
            ...recap.freeAgencyEvidence,
            signings: [...recap.freeAgencyEvidence.signings].sort((a, b) => (a.franchiseId < b.franchiseId ? -1 : a.franchiseId > b.franchiseId ? 1 : 0) ||
                (a.playerVersionId < b.playerVersionId ? -1 : 1)),
        },
    });
}
export function auditSeasonBlockRecap(recap: SeasonBlockRecap, input: SeasonBlockRecapInput): string[] {
    const failures: string[] = [];
    if (recap.runId !== input.runId)
        failures.push('recap runId does not match the input');
    if (recap.blockIndex !== input.blockIndex)
        failures.push('recap blockIndex does not match');
    ...
    for (const performance of recap.notablePerformances) {
        ...
        if (saved === undefined) {
            failures.push(`notable performance ${performance.playerVersionId} in ${performance.gameId} does not exist in the block summaries`);
        }
        else if (JSON.stringify(saved) !== JSON.stringify(performance)) {
            failures.push(`notable performance ${performance.playerVersionId} facts do not match the summary`);
        }
    }
    ...
    for (const spotlight of recap.versionSpotlights) {
        ...
        if (personA === undefined || personB === undefined || personA !== personB) {
            failures.push(`version spotlight ${spotlight.versionA}/${spotlight.versionB} is not a same-person pair`);
            continue;
        }
        ...
        if (spotlight.sameTeam !== (franchiseA === franchiseB)) {
            failures.push(`version spotlight ${spotlight.versionA}/${spotlight.versionB} sameTeam mismatch`);
        }
        if (spotlight.gamesPlayedA !== aggregateA.gamesPlayed ||
            ...
            failures.push(`version spotlight ${spotlight.versionA}/${spotlight.versionB} aggregate facts mismatch`);
        }
        const meeting = headToHead(blockSummaries, franchiseA, franchiseB);
        if (spotlight.headToHeadGames !== meeting.games ||
            spotlight.headToHeadWinsA !== meeting.winsA ||
            spotlight.headToHeadWinsB !== meeting.winsB) {
            failures.push(`version spotlight ${spotlight.versionA}/${spotlight.versionB} head-to-head facts mismatch`);
        }
    }
