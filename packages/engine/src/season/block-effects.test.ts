...
import { SEASON_AI_VERSION, SEASON_AUTHORITY_VERSION, SEASON_BLOCK_VERSION, SEASON_CHEMISTRY_VERSION, SEASON_EFFECT_TARGETS_VERSION, SEASON_FREE_AGENCY_VERSION, SEASON_GAME_SUMMARY_VERSION, SEASON_GAME_TARGETS_VERSION, SEASON_GAME_VERSION, SEASON_LEAGUE_VERSION, SEASON_MINUTE_POLICY_VERSION, SEASON_POSTSEASON_VERSION, SEASON_RECAP_VERSION, SEASON_ROSTER_GENERATION_VERSION, SEASON_ROSTER_RULES_VERSION, SEASON_ROTATION_PLANNER_VERSION, SEASON_ROTATION_VERSION, SEASON_RUN_SCHEMA_VERSION, SEASON_SCHEDULE_FORMULA_VERSION, SEASON_SCHEDULE_VERSION, SEASON_SEED_DERIVATION_VERSION, SEASON_STAMINA_VERSION, SEASON_STANDINGS_VERSION, buildInitialPostseasonState, contentHashSchema, eraIdSchema, franchiseIdSchema, idSchema, playerIdSchema, seasonKeySchema, seasonRunSchema, seedSchema, type SeasonAiPool, type SeasonCandidateCheckpoint, type SeasonDraftCatalog, type SeasonEffectsState, type SeasonGameSummary, type SeasonRosterEvaluation, type SeasonRun, } from '@hoop-rush/data-contracts';
...
import { SEASON_AGGREGATES_VERSION, SEASON_CHECKPOINT_VERSION, SEASON_HEALTH_VERSION, SEASON_INFLUENCE_TARGETS_VERSION, SEASON_INFLUENCE_VERSION, SEASON_INJURY_TARGETS_VERSION, SEASON_OBJECTIVE_CATALOG, SEASON_OBJECTIVE_VERSION, SEASON_TRADE_TARGETS_VERSION, SEASON_TRADE_VERSION, } from '@hoop-rush/data-contracts';
function synthesizeRoster(catalog: SeasonDraftCatalog, franchiseId: string): Array<{
    ...
    const pool = catalog.pools.find((entry) => entry.franchiseId === franchiseId && entry.eraId === '1990s');
    if (pool === undefined)
        throw new Error(`missing pool for ${franchiseId}`);
...
function buildSynthesizedRun(): {
    ...
    if (!parsed.success) {
        throw new Error(`synthesized run fails the schema: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
    }
    return { run, catalog };
}
...
describe('M2.4 block-level effects', () => {
    it('accumulates fatigue and chemistry across blocks and records evidence', () => {
        ...
        if (first === undefined || last === undefined)
            throw new Error('checkpoints');
        const maxFatigueFirst = first.effects.playerStates.reduce((max, player) => Math.max(max, player.fatigueBasisPoints), 0);
        const maxFatigueLast = last.effects.playerStates.reduce((max, player) => Math.max(max, player.fatigueBasisPoints), 0);
        expect(maxFatigueFirst).toBeGreaterThan(0);
        expect(maxFatigueLast).toBeGreaterThan(0);
        const mid = checkpoints[4];
        if (mid === undefined)
            throw new Error('mid checkpoint');
        const maxFatigueMid = mid.effects.playerStates.reduce((max, player) => Math.max(max, player.fatigueBasisPoints), 0);
        ...
        const sharedFirst = first.effects.pairStates.reduce((sum, pair) => sum + pair.sharedPossessions, 0);
        const sharedLast = last.effects.pairStates.reduce((sum, pair) => sum + pair.sharedPossessions, 0);
        ...
        for (const checkpoint of checkpoints) {
            for (const detail of checkpoint.retainedDetails) {
                if ((detail.mechanismEvidence ?? []).length > 0)
                    detailWithEvidence += 1;
            }
        }
        ...
        for (const checkpoint of checkpoints) {
            for (const summary of checkpoint.gameSummaries) {
                if ((summary.effectsRollup ?? []).length > 0)
                    summaryWithRollup += 1;
            }
        }
    ...
    it('keeps recovery ticks between games monotone and rounds within bounds', () => {
        ...
        const maxRound = checkpoint0.effects.playerStates.reduce((max, player) => Math.max(max, player.lastCompletedRound), 0);
        ...
        for (const player of checkpoint0.effects.playerStates) {
            ...
            expect(player.fatigueBasisPoints).toBeLessThanOrEqual(10000);
