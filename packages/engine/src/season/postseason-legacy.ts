import { SEASON_POSTSEASON_LEGACY_VERSION as SEASON_POSTSEASON_VERSION, seasonPostseasonStateV1Schema as seasonPostseasonStateSchema, type ConferenceId, type PlayInGameV1 as PlayInGame, type PlayInGameIdV1 as PlayInGameId, type PlayoffBracketV1 as PlayoffBracket, type PlayoffConferenceBracketV1 as PlayoffConferenceBracket, type PlayoffRoundV1 as PlayoffRound, type PlayoffSeriesV1 as PlayoffSeries, type PlayoffSeriesGameV1 as PlayoffSeriesGame, type SeasonLeague, type SeasonPostseasonStateV1 as SeasonPostseasonState, } from '@hoop-rush/data-contracts';
...
function resolvePlayInGame(game: PlayInGame, home: string, away: string, result: PlayInGameResult): PlayInGame {
...
function advanceConferencePlayIn(playIn: SeasonPostseasonState['playIn']['east']): SeasonPostseasonState['playIn']['east'] {
...
export function setPlayInRankings(state: SeasonPostseasonState, league: SeasonLeague, rankings: PostseasonRankings): SeasonPostseasonState {
...
export function submitPlayInGame(state: SeasonPostseasonState, conference: ConferenceId, gameId: PlayInGameId, result: PlayInGameResult): SeasonPostseasonState {
...
function seededSeries(seriesId: string, round: PlayoffRound, conference: ConferenceId, homeCourtFranchiseId: string | null, challengerFranchiseId: string | null, higherSeed: number | null, lowerSeed: number | null): PlayoffSeries {
    return {
        seriesId,
        round,
        conference,
...
function buildConferenceBracket(conference: ConferenceId, seeds: readonly string[]): PlayoffConferenceBracket {
...
export function createPlayoffBracket(state: SeasonPostseasonState, league: SeasonLeague, finalsHomeCourtFranchiseId: string): SeasonPostseasonState {
...
function pairSeries(slot: PlayoffSeries, sideA: {
    team: string;
    seed: number | null;
}, sideB: {
    team: string;
...
function advanceWinner(bracket: PlayoffBracket, seriesId: string, winner: string): PlayoffBracket {
    ...
    if (seriesId.startsWith(`${conference}-first-round-`)) {
        ...
        if (semifinal === undefined)
            throw new Error(`no semifinal slot for ${seriesId}`);
        ...
        if (nextSlot.homeCourtFranchiseId !== null && nextSlot.challengerFranchiseId !== null) {
            ...
            paired = pairSeries(nextSlot, { team: winner, seed: winnerSeedOf(feed) }, {
    ...
    if (seriesId.startsWith(`${conference}-semifinal-`)) {
        ...
        if (nextSlot.homeCourtFranchiseId !== null && nextSlot.challengerFranchiseId !== null) {
            ...
            paired = pairSeries(nextSlot, { team: winner, seed: winnerSeedOf(current) }, {
                team: other.winnerFranchiseId,
                seed: winnerSeedOf(other),
            });
        }
...
export function submitPlayoffGame(state: SeasonPostseasonState, seriesId: string, result: PlayoffGameResult): SeasonPostseasonState {
...
function auditPlayInGame(conference: ConferenceId, gameId: PlayInGameId, game: PlayInGame, failures: string[]): void {
...
function auditPlayInConference(conference: ConferenceId, playIn: SeasonPostseasonState['playIn']['east'], league: SeasonLeague, failures: string[]): void {
    const ranking = playIn.ranking;
    if (ranking !== null) {
    ...
    if (sevenEight.status !== 'scheduled') {
        if (sevenEight.homeFranchiseId !== (ranking?.[6] ?? null) ||
    ...
    if (nineTen.status !== 'scheduled') {
        if (nineTen.homeFranchiseId !== (ranking?.[8] ?? null) ||
    ...
    if (seeds !== null) {
        if (ranking === null || !sevenDone || !nineDone || !finalDone) {
            ...
            if (expectedIds.some((id) => id === null) ||
                seeds.length !== expectedIds.length ||
                seeds.some((id, i) => id !== expectedIds[i])) {
                failures.push(`${conference} playoff seeds must be seeds 1-6 plus the 7/8 and final winners`);
            }
        }
    ...
    series.games.forEach((game, index) => {
        ...
        if (game.status === 'final') {
            if (game.homeScore === null || game.awayScore === null) {
                ...
                const expectedWinner = game.homeScore > game.awayScore ? game.homeFranchiseId : game.awayFranchiseId;
...
function auditBracket(state: SeasonPostseasonState, league: SeasonLeague, failures: string[]): void {
    const bracket = state.bracket;
    if (bracket === null) {
    ...
    for (const conference of CONFERENCES) {
        ...
        conf.firstRound.forEach((series, index) => {
            ...
            if (series.homeCourtFranchiseId !== home || series.challengerFranchiseId !== away) {
                failures.push(`${conference} first-round ${String(index + 1)} must pair seeds ${String(higherIndex + 1)} and ${String(lowerIndex + 1)}`);
            }
            if (series.higherSeed !== higherIndex + 1 || series.lowerSeed !== lowerIndex + 1) {
                failures.push(`${conference} first-round ${String(index + 1)} seed numbers must match its pairing`);
            }
        });
        conf.semifinals.forEach((series, index) => {
            ...
            if (feedA === undefined || feedB === undefined)
                return;
            if (feedA.winnerFranchiseId !== null && feedB.winnerFranchiseId !== null) {
                ...
                if (series.homeCourtFranchiseId === null || series.challengerFranchiseId === null) {
                    failures.push(`${conference} semifinal ${String(index + 1)} must be paired after the first round`);
                }
                else {
                    ...
                    if (teams.size !== 2 || !teams.has(teamA) || !teams.has(teamB)) {
                        failures.push(`${conference} semifinal ${String(index + 1)} must match its first-round winners`);
                    }
                    else {
                        ...
                        if (seedA !== null && seedB !== null) {
                            ...
                            if (series.homeCourtFranchiseId !== expectedHome) {
                                failures.push(`${conference} semifinal ${String(index + 1)} home court must be the higher seed`);
                            }
                        }
                    }
                }
        ...
        if (semifinalA !== undefined &&
            ...
            if (conf.conferenceFinal.homeCourtFranchiseId === null ||
    ...
    if (eastChampion !== null && westChampion !== null) {
        ...
        if (bracket.finals.homeCourtFranchiseId === null ||
...
export function auditSeasonPostseason(state: SeasonPostseasonState, league: SeasonLeague): string[] {
