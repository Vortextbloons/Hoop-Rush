...
describe('seasonRunStateDigest', () => {
    ...
    it('changes whenever any single fact changes', () => {
        ...
        const cases: Array<[
            string,
            (facts: SeasonRunStateDigestFacts) => SeasonRunStateDigestFacts
        ]> = [
            ...
            [
                ...
                (f) => ({
                    ...f,
                    rosters: f.rosters.map((roster, index) => index === 0
                        ? {
                            ...roster,
                            players: roster.players.map((player, slot) => slot === 0 ? { ...player, displayName: `${player.displayName}X` } : player),
                        }
                        : roster),
                }),
            ],
            [
                ...
                (f) => ({
                    ...f,
                    ownership: f.ownership.map((row, index) => index === 0 ? { ...row, franchiseId: 'celtics' } : row),
                }),
            ],
            [
                'rotation starter',
                (f) => ({
                    ...f,
                    rotations: f.rotations.map((rotation, index) => index === 0 ? { ...rotation, starters: [...rotation.starters].reverse() } : rotation),
                }),
            ],
            [
                'effects player load',
                (f) => ({
                    ...
                    effects: {
                        ...f.effects,
                        playerStates: f.effects.playerStates.map((state, index) => index === 0 ? { ...state, fatigueBasisPoints: 1 } : state),
                    },
                }),
            ],
            [
                ...
                (f) => ({
                    ...
                    effects: {
                        ...f.effects,
                        pairStates: f.effects.pairStates.map((pair, index) => index === 0 ? { ...pair, sharedPossessions: pair.sharedPossessions + 1 } : pair),
                    },
                }),
            ],
            [
                ...
                (f) => ({
                    ...f,
                    stage: f.stage === 'regular-season' ? ('play-in' as const) : ('regular-season' as const),
                }),
            ],
            [
