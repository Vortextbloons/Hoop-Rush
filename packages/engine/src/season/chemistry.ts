...
export function canonicalPlayerPairs(players: readonly string[]): Array<[
    string,
    string
]> {
...
export function canonicalRosterPairs(roster: readonly string[]): Array<[
    string,
    string
]> {
    if (roster.length !== SEASON_ROSTER_SIZE || new Set(roster).size !== SEASON_ROSTER_SIZE) {
        throw new Error(`season chemistry: roster must be ten distinct versions (got ${String(roster.length)})`);
    }
    return canonicalPlayerPairs(roster);
}
export function unitPairs(unit: readonly string[]): Array<[
    string,
    string
]> {
    if (unit.length !== 5 || new Set(unit).size !== 5) {
        throw new Error(`season chemistry: unit must be five distinct versions (got ${String(unit.length)})`);
    }
...
export function pairChemistryBasisPoints(shared: number): number {
    return Math.round((shared / (shared + SEASON_CHEMISTRY_HALF_SHARED)) * SEASON_CHEMISTRY_BASIS_POINT_SCALE);
}
export function unitChemistryFromShared(unit: readonly string[], sharedOf: (a: string, b: string) => number): number {
    let sum = 0;
...
export function unitChemistryBasisPoints(pairStates: readonly SeasonPairChemistryState[], unit: readonly string[]): number {
...
export function unitSharedPossessions(pairStates: readonly SeasonPairChemistryState[], unit: readonly string[]): Map<string, number> {
    const byKey = new Map(pairStates.map((pair) => [seasonPairKey(pair.a, pair.b), pair]));
