...
import { SEASON_OBJECTIVE_CATALOG, commandIdSchema, franchiseIdSchema, seasonGameIdSchema, type SeasonGameSummary, type SeasonHealthState, type SeasonInjuryRecord, } from '@hoop-rush/data-contracts';
import { buildTestRun, pipelineInput } from './block-test-support.ts';
import { assembleSeasonBlockCandidate, auditSeasonBlock, deriveSeasonPostBlockState, handleSubmitSeasonBlockCommand, seasonBlockGamesOf, seasonBlockRejection, simulateSeasonBlock, simulateSeasonBlockGame, type SeasonBlockSimulationInput, } from './block.ts';
...
describe('M2.5 block pipeline with injuries', () => {
    ...
    it('rolls injuries into the block, carries compact events, and reconciles health', () => {
        ...
        for (const record of checkpoint.health.injuries) {
            ...
            const event = summary?.injuryEvents.find((entry) => entry.playerVersionId === record.playerVersionId);
        ...
        const sumSeverity = bySeverity.minor + bySeverity.moderate + bySeverity.major + bySeverity['season-ending'];
    ...
    it('folds the locked objective, influence grants, and transactions into the candidate', () => {
        ...
        if (offered === undefined)
            throw new Error('block 0 objective choices');
        ...
        expect(candidate.recap.influenceBalance.humanBalance).toBe(candidate.influence.balances[franchiseIdSchema.parse('lakers')] ?? 0);
        expect(candidate.transactions.some((entry) => entry.type === 'block-grant')).toBe(true);
        expect(candidate.transactions.filter((entry) => entry.type === 'objective-reward').length).toBe(candidate.objective.success ? 1 : 0);
    ...
    it('rejects invalid objectives at the command boundary', () => {
        ...
        const offered = seasonObjectiveChoicesForBlock(run.rootSeed, 0)[0];
        if (offered === undefined)
            throw new Error('objective choices');
        const objectives: SeasonBlockSimulationInput['objectives'] = {
            schemaVersion: 1,
            objectiveVersion: 'season-objective-v1',
            catalog: [...SEASON_OBJECTIVE_CATALOG],
        ...
        expect(seasonBlockRejection({
            ...base,
            command: { ...base.command, objectiveId: otherId },
        })).toMatchObject({ code: 'invalid-objective', expected: 'not-offered', objectiveId: otherId });
        expect(seasonBlockRejection({ ...base, command: { ...base.command, objectiveId: offered } })).toBeNull();
        const block8 = pipelineInput({ ...run, cursor: { schemaVersion: 1 as const, completedRounds: 80 } }, catalog, 8);
        expect(seasonBlockRejection({
            ...block8,
            objectives,
    ...
    it('interrupts mid-block and resumes to the identical uninterrupted digest', () => {
        ...
        const humanIndex = games.findIndex((game) => game.homeFranchiseId === 'lakers' || game.awayFranchiseId === 'lakers');
        expect(humanIndex).toBeGreaterThan(0);
        const fromRound = 81;
        const loop = (input: SeasonBlockSimulationInput, startIndex: number, startEffects: SeasonBlockSimulationInput['effects'], startHealth: SeasonHealthState, stopIndex?: number, initialPreviousRound?: number) => {
        ...
        const interrupted = loop(interruptedInput, 0, interruptedInput.effects, interruptedInput.health);
        ...
        const lastSimulatedRound = pending.summaries.length > 0
            ...
            : fromRound - 1;
        const resumed = loop(legalInput, humanIndex, legalBefore.effects, legalBefore.health, undefined, lastSimulatedRound);
