...
import { blockCommand, buildTestRun, freshState, pipelineInput, runBlock, scheduleOf, } from './block-test-support.ts';
describe('season block determinism and accounting (M2.3)', () => {
    it('reproduces block 3 identically across repeated and interrupted runs', () => {
        ...
        const cancelled = (() => {
            ...
            expect(() => simulateSeasonBlock(input, { cancelAfterGames: 75 })).toThrow(SeasonBlockCancelledError);
    ...
    it('reconciles aggregates and standings with the summaries after every block', () => {
        ...
        for (let blockIndex = 0; blockIndex < SEASON_BLOCK_COUNT; blockIndex += 1) {
            ...
            const standings = reduceSeasonStandings(run.league, reconstructSeasonGames(scheduleOf(run), state.summaries));
            expect(JSON.stringify(standings)).toBe(JSON.stringify(checkpoint.standings));
            const audit = auditSeasonBlock(checkpoint, {
                ...pipelineInput(preBlockRun, catalog, blockIndex, state.summaries.slice(0, state.summaries.length - expectedGameCount)),
        ...
        expect(state.summaries.filter((summary) => summary.status === 'final')).toHaveLength(state.summaries.length -
