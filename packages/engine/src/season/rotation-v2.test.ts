...
import { SEASON_MINUTE_POLICY_VERSION, SEASON_ROTATION_PRESET_TARGETS, SEASON_ROTATION_VERSION, seasonRotationCommandResultSchema, type Position, type SeasonRotation, type SeasonRotationPreset, type SetSeasonRotationCommand, } from '@hoop-rush/data-contracts';
import type { SeasonRosterMemberInput } from './roster-rules.ts';
import { applySeasonRotationPreset, buildMinimalRotation, handleSetSeasonRotationCommand, rotationTargetMinutes, validateSeasonRotation, } from './rotation.ts';
...
function presetCommand(preset: SeasonRotationPreset, franchiseId = 'lakers'): SetSeasonRotationCommand {
    return {
        schemaVersion: 1,
...
describe('applySeasonRotationPreset (season-rotation-v2)', () => {
    it('applies the balanced table and preserves everything else', () => {
        ...
        expect(result.targetMinutes).toEqual(BALANCED_TARGET_MINUTES.map((minutes, index) => ({
    ...
    it('every preset totals exactly 240 and matches the frozen tables', () => {
        for (const preset of ['balanced', 'tight', 'bench-heavy'] as const) {
            ...
            if (preset === 'balanced') {
                expect([...STARTERS.map(() => table.starters), ...table.bench]).toEqual(BALANCED_TARGET_MINUTES);
            }
            for (const starter of STARTERS) {
                expect(result.targetMinutes.find((entry) => entry.playerVersionId === starter)?.minutes).toBe(table.starters);
            }
            BENCH.forEach((playerVersionId, index) => {
                expect(result.targetMinutes.find((entry) => entry.playerVersionId === playerVersionId)?.minutes).toBe(table.bench[index]);
            });
        }
    });
...
describe('validateSeasonRotation (season-rotation-v2)', () => {
    ...
    it('rejects foreign ids and missing roster coverage', () => {
        ...
        expect(foreignFailures.some((failure) => failure.includes(`unrostered player ${pv(99)}`))).toBe(true);
        ...
        expect(missingFailures.some((failure) => failure.includes(`no target minutes for rostered player ${pv(5)}`))).toBe(true);
        ...
        expect(foreignMinuteFailures.some((failure) => failure.includes(`unrostered player ${pv(98)}`))).toBe(true);
        expect(foreignMinuteFailures.some((failure) => failure.includes(`no target minutes for rostered player ${pv(10)}`))).toBe(true);
    });
    it.each([
        ...
        expect(validateSeasonRotation(rotation, MEMBER_PLAYABLE).some((failure) => failure.includes(expectedMessage))).toBe(true);
    });
    it('rejects illegal starter slots', () => {
        ...
        expect(failures.some((failure) => failure.includes(`starter ${pv(5)} cannot play slot 3`))).toBe(true);
    });
    it('rejects an illegal closing five, including one equal to illegal starters', () => {
        ...
        const failures = validateSeasonRotation(rotation, MEMBER_PLAYABLE);
        expect(failures.some((failure) => failure.includes(`starter ${pv(5)} cannot play slot 3`))).toBe(true);
        expect(failures.some((failure) => failure.includes(`closing-five player ${pv(5)} cannot play slot 3`))).toBe(true);
    });
    it('rejects closing-five slot violations and foreign closing members', () => {
        ...
        expect(slotFailures.some((failure) => failure.includes(`closing-five player ${pv(6)} cannot play slot 4`))).toBe(true);
        const foreign = buildRotation({ closingFive: [pv(1), pv(2), pv(3), pv(4), pv(97)] });
        const foreignFailures = validateSeasonRotation(foreign, MEMBER_PLAYABLE);
        expect(foreignFailures.some((failure) => failure.includes(`unrostered player ${pv(97)}`))).toBe(true);
    });
...
describe('handleSetSeasonRotationCommand (season-rotation-v2)', () => {
    it('accepts preset commands with the exact preset table and a derived base rotation', () => {
        ...
        expect(result.rotation.targetMinutes).toEqual(BALANCED_TARGET_MINUTES.map((minutes, index) => ({
            playerVersionId: pv(index + 1),
            minutes,
        })));
    ...
    it('accepts tight and bench-heavy presets with 240-minute tables', () => {
        for (const preset of ['tight', 'bench-heavy'] as const) {
            ...
            result.rotation.starters.forEach((playerVersionId) => {
                expect(result.rotation.targetMinutes.find((entry) => entry.playerVersionId === playerVersionId)
                    ?.minutes).toBe(table.starters);
            });
            expect(seasonRotationCommandResultSchema.safeParse(result).success).toBe(true);
        }
    });
    ...
    it('maps duplicate ids to DUPLICATE_PLAYER_VERSION', () => {
        ...
        const duplicateMinutes = buildRotation();
        duplicateMinutes.targetMinutes[5] = { playerVersionId: pv(5), minutes: 16 };
        const minuteResult = handleSetSeasonRotationCommand(rotationCommand(duplicateMinutes), MEMBER_PLAYABLE);
    ...
    it('maps roster correspondence failures to ROSTER_MISMATCH', () => {
        ...
        expect(handleSetSeasonRotationCommand(rotationCommand(missingMinutes), MEMBER_PLAYABLE)).toMatchObject({
            status: 'rejected',
            errorCode: 'ROSTER_MISMATCH',
        });
    });
    it('maps target value failures to INVALID_TARGETS', () => {
        ...
        expect(handleSetSeasonRotationCommand(rotationCommand(nonInteger), MEMBER_PLAYABLE)).toMatchObject({
            status: 'rejected',
            errorCode: 'INVALID_TARGETS',
        });
    });
    ...
    it('maps closing-five violations to ILLEGAL_CLOSING_FIVE', () => {
        ...
        expect(handleSetSeasonRotationCommand(rotationCommand(rotation), MEMBER_PLAYABLE)).toMatchObject({
            status: 'rejected',
    ...
    it('rejects presets against rosters of the wrong size', () => {
        const nine = new Map(MEMBERS.slice(0, 9).map((member) => [member.playerVersionId, member.playable]));
