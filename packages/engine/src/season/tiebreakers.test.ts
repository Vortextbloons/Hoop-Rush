...
import { buildLeague, franchiseIdSchema, seasonNamespaceSeed, seasonTiebreakResolutionSchema, SEASON_SEED_NAMESPACES, type SeasonLeague, type SeasonStandings, } from '@hoop-rush/data-contracts';
...
describe('tiebreak ranking (M2.6, tiebreaker-v1)', () => {
    ...
    it('records qualification-kind resolutions for play-in boundary ties', () => {
        ...
        const resolution = ranking.resolutions.find((entry) => entry.teams.includes(franchiseIdSchema.parse(seed7)));
    ...
    it('records seeding-kind resolutions for top-seed ties', () => {
        ...
        const ranking = eastRanking(spec);
        const resolution = ranking.resolutions.find((entry) => entry.teams.includes(franchiseIdSchema.parse(a)));
