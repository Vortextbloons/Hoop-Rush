import { SEASON_GAME_SUMMARY_VERSION, commandIdSchema, franchiseIdSchema, seasonGameIdSchema, type Position, type SeasonEffectsState, type SeasonGameSummary, type SeasonHealthState, type SeasonInjuryRecord, type SeasonObjectiveId, type SeasonPendingBlockCandidate, type SeasonRetainedGameDetail, type SeasonRun, type SeasonTeamBox, } from '@hoop-rush/data-contracts';
...
export type HealthRunView = Pick<SeasonRun, 'runId' | 'rootSeed' | 'league' | 'rosters' | 'rotations' | 'versions'>;
function targetSecondsOf(rotation: SeasonRun['rotations'][number], playerVersionId: string): number {
    const entry = rotation.targetMinutes.find((row) => row.playerVersionId === playerVersionId);
    return (entry?.minutes ?? 0) * 60;
}
export function seasonFranchiseLegalFiveFacts(run: HealthRunView, franchiseId: string, health: SeasonHealthState, positions?: ReadonlyMap<string, readonly Position[]>): {
    legal: boolean;
    ...
    const unavailablePlayerVersionIds = rosterIds.filter((playerVersionId) => !seasonPlayerAvailable(health, playerVersionId));
    ...
    if (positions !== undefined) {
        ...
        const startersAvailable = rotation.starters.every((playerVersionId) => !unavailable.has(playerVersionId));
        legal = startersAvailable || rosterIds.length - unavailable.size >= 5;
    }
    return { legal, unavailablePlayerVersionIds };
}
export function seasonPregameAvailabilityOf(health: SeasonHealthState, players: readonly {
...
export function seasonGameHealthSeam(run: HealthRunView, health: SeasonHealthState, input: {
    rootSeed: string;
    gameId: string;
    round: number;
    ...
    for (const record of health.injuries) {
        ...
        if (record.rehabRecurrencePremiumApplied &&
            ...
            if (premium > curPremium)
                rehabPremiumOf.set(record.playerVersionId, premium);
        }
    }
...
export function assembleSeasonPendingBlock(input: {
    ...
    return {
        ...
        campaignOpportunityId: (input as unknown as {
            campaignOpportunityId?: string | null;
...
export function seasonForfeitSummaryForGame(run: SeasonRun, gameId: string, humanFranchiseId: string): SeasonGameSummary {
...
export function advancePendingAfterForfeit(pending: SeasonPendingBlockCandidate, forfeitedGameId: string): SeasonPendingBlockCandidate {
    ...
    if (nextNumber > blockEndNumber) {
        throw new Error(`season forfeit: ${forfeitedGameId} is the last game of block ${String(pending.blockIndex)}; the block is complete and has no next game`);
    }
    return {
        ...pending,
