import { SEASON_AI_VERSION, SEASON_MINUTE_POLICY_VERSION, SEASON_ROSTER_GENERATION_VERSION, SEASON_ROSTER_TARGETS_VERSION, SEASON_ROTATION_VERSION, franchiseIdSchema, seasonDigestHex, seasonLeagueGenerationResultSchema, seasonLeagueSchema, seasonNamespaceSeed, type EraSimulationProfile, type ProjectionModelArtifact, type SeasonAiAnchor, type SeasonAiAssignment, type SeasonAiIdentity, type SeasonAiPool, type SeasonDraftCatalog, type SeasonGenerationDiagnostics, type SeasonLeague, type SeasonLeagueGenerationResult, type SeasonMinutePlanSummary, type SeasonRosterCalibrationRun, type SeasonRosterEvaluation, type SeasonRosterRole, type SeasonRosterTargets, type SeasonStrengthBand, type Seed, type SimulationPlayer, } from '@hoop-rush/data-contracts';
import { createRng, shuffle } from '../sim/rng.ts';
import { assertNever } from '../sim/assert-never.ts';
import { ProjectionCache, projectSeasonRoster, searchRosterRotationCandidates, } from '../projection/index.ts';
import { validateDraftCatalog } from './catalog-validation.ts';
import { buildMinutePlanCandidates, minutePlanHorizonGames, type MinutePlanPlayerInput, } from './minute-plan.ts';
import { buildMinimalRotation, validateSeasonRotation } from './rotation.ts';
import { seasonGenerationDigest } from './digest.ts';
import { completionTargetsMet, groupMaskOf, rosterGroupCounts, validateSeasonRoster, type SeasonRosterMemberInput, } from './roster-rules.ts';
import { BAND_CEILING_PENALTY, ROSTER_ROLES, ROLE_COVERAGE_THRESHOLD, identityScore, overallReportOf, percentileTierOf, playerPercentileTier, rolePercentileThresholds, roleScoresOf, type PercentileTier, type RoleThresholds, type SeasonScoreMember, } from './ai-scoring.ts';
...
export const DEFAULT_IDENTITY_PRIORITY_ROLES: Record<SeasonAiIdentity, readonly SeasonRosterRole[]> = {
...
export function identityPriorityRolesOf(targets: SeasonRosterTargets, identity: SeasonAiIdentity): readonly SeasonRosterRole[] {
    const roles = (targets.policy.identityPriorityRoles as Partial<Record<SeasonAiIdentity, readonly SeasonRosterRole[]>>)[identity];
...
export function validateSeasonRosterTargets(targets: SeasonRosterTargets | undefined): void {
    if (!targets)
        throw new SeasonAiTargetsError('roster targets missing');
    const schemaVersion: number = targets.schemaVersion;
    if (schemaVersion !== 2) {
        throw new SeasonAiTargetsError(`roster targets schemaVersion must be 2 (got ${String(schemaVersion)})`);
    }
    if (targets.targetsVersion !== SEASON_ROSTER_TARGETS_VERSION) {
        throw new SeasonAiTargetsError(`roster targets version mismatch: expected ${SEASON_ROSTER_TARGETS_VERSION}, got ${targets.targetsVersion}`);
    }
    if (targets.calibration.aiVersion !== SEASON_AI_VERSION) {
        throw new SeasonAiTargetsError(`targets aiVersion mismatch: expected ${SEASON_AI_VERSION}, got ${targets.calibration.aiVersion}`);
    }
    if (targets.calibration.rosterGenerationVersion !== SEASON_ROSTER_GENERATION_VERSION) {
        throw new SeasonAiTargetsError(`targets rosterGenerationVersion mismatch: expected ${SEASON_ROSTER_GENERATION_VERSION}, got ${targets.calibration.rosterGenerationVersion}`);
    }
    for (const identity of IDENTITIES) {
        const roles = (targets.policy.identityPriorityRoles as Partial<Record<SeasonAiIdentity, readonly import('@hoop-rush/data-contracts').SeasonRosterRole[]>>)[identity];
        if (roles === undefined || roles.length === 0) {
            throw new SeasonAiTargetsError(`targets lack priority roles for identity ${identity}`);
        }
    }
    const soloTotal = targets.policy.bandQuotas.solo.contender +
        targets.policy.bandQuotas.solo.playoff +
        targets.policy.bandQuotas.solo.average +
        targets.policy.bandQuotas.solo.weaker;
    const duoTotal = targets.policy.bandQuotas.duo.contender +
...
export function attachAiProjectionSummaries(input: {
    ...
    const byId = new Map(generation.rosters.map((roster) => [
        roster.franchiseId,
        roster.players.map((entry) => entry.playerVersionId),
    ]));
    const rotationById = new Map(generation.rotations.map((rotation) => [rotation.franchiseId, rotation]));
    const playerById = new Map(catalog.candidates.map((candidate) => [
    ...
    const evaluations = generation.evaluations.map((evaluation) => {
        ...
        const searchDigest = seasonDigestHex(JSON.stringify({
...
export function assignAiBandsAndIdentities(input: {
    ...
    const aiTeams = input.league.teams.filter((team) => !input.humanFranchiseIds.includes(team.franchiseId));
    const bandQuotas = input.humanFranchiseIds.length === 2
        ? input.targets.policy.bandQuotas.duo
        : input.targets.policy.bandQuotas.solo;
    const shuffled = shuffle(aiTeams.map((team) => team.franchiseId).sort(), createRng(seasonNamespaceSeed(input.seed, 'ai-rosters', 'band-order')));
    const identityOffset = createRng(seasonNamespaceSeed(input.seed, 'ai-rosters', 'identity-order')).nextInt(0, IDENTITIES.length - 1);
...
function membersOf(state: GenerationState, versionIds: readonly string[]): SeasonRosterMemberInput[] {
...
function leaveUnassigned(state: GenerationState, versionId: string): void {
    if (!state.unassigned.has(versionId))
        return;
...
function enterUnassigned(state: GenerationState, versionId: string): void {
    if (state.unassigned.has(versionId) || state.humanOwned.has(versionId))
        return;
...
function claimIdentity(state: GenerationState, versionId: string): void {
    ...
    for (const sibling of state.versionsByIdentity.get(playerId) ?? []) {
        if (sibling !== versionId)
            leaveUnassigned(state, sibling);
    }
}
...
function roleScoresOfIds(state: GenerationState, versionIds: readonly string[]): Record<SeasonRosterRole, number> {
...
function identityScoreOf(state: GenerationState, versionId: string, identity: SeasonAiIdentity): number {
...
function maskCountsOf(versionIds: readonly string[], maskByVersion: ReadonlyMap<string, number>): number[] {
...
export function fiveReachableFromCounts(ownedCounts: {
    guards: number;
    forwards: number;
    centers: number;
    ...
    if (startG >= targetG && startF >= targetF && startC >= targetC)
        return true;
    const usedBase = 18;
...
function reachableAfterFixedAndFills(targets: {
    guards: number;
    forwards: number;
    centers: number;
}, startCounts: {
    ...
    forwards: number;
    centers: number;
}, fixedMasks: readonly number[], unassignedMasks: readonly number[], slotsToFill: number, maxUnassignedPicks: number): boolean {
    ...
    const stateIndex = (g: number, f: number, c: number): number => g * (targetF + 1) * (targetC + 1) + f * (targetC + 1) + c;
    let reachable = new Uint8Array(usedBase);
    reachable[stateIndex(capG(startCounts.guards), capF(startCounts.forwards), capC(startCounts.centers))] = 1;
            ...
            for (let f = targetF; f >= 0; f -= 1) {
                for (let c = targetC; c >= 0; c -= 1) {
                    ...
                    if (reachable[idx] === 0)
                        continue;
                    next[idx] = 1;
                    next[stateIndex(capG(g + ((mask & 1) !== 0 ? 1 : 0)), capF(f + ((mask & 2) !== 0 ? 1 : 0)), capC(c + ((mask & 4) !== 0 ? 1 : 0)))] = 1;
                }
            }
        }
        reachable = next;
    ...
    for (let pass = 0; pass <= maxPicks; pass += 1) {
        ...
        for (let used = 0; used <= maxPicks; used += 1) {
            if (withPicks[used * usedBase + stateIndex(targetG, targetF, targetC)] === 1)
                return true;
        }
        let unchanged = true;
...
function poolAdmitsTenExact(state: GenerationState, team: PoolTeam): boolean {
    ...
    if (!memberReachCapped(state, { guards: 4, forwards: 4, centers: 3 }, anchorCounts, rest, picks)) {
        return false;
    }
    if (!memberReachCapped(state, { guards: 2, forwards: 2, centers: 1 }, { guards: 0, forwards: 0, centers: 0 }, team.pool, 5)) {
        return false;
    }
    return true;
}
function poolTenFeasibleAfterAdd(state: GenerationState, team: PoolTeam, versionId: string | null): boolean {
    const anchorIds = team.anchors.map((anchor) => anchor.playerVersionId);
    const anchorSet = new Set(anchorIds);
    const anchorCounts = rosterGroupCounts(membersOf(state, anchorIds));
    const fixedMasks: number[] = [];
    for (const memberId of team.pool) {
        if (anchorSet.has(memberId))
            continue;
    ...
    if (!reachableAfterFixedAndFills({ guards: 4, forwards: 4, centers: 3 }, anchorCounts, fixedMasks, unassignedMasks, slotsToFill, picksForTen)) {
        return false;
    }
    if (!reachableAfterFixedAndFills({ guards: 2, forwards: 2, centers: 1 }, { guards: 0, forwards: 0, centers: 0 }, fixedMasks, unassignedMasks, slotsToFill, picksForTen)) {
        return false;
    }
    return true;
}
function poolCoverageFeasible(state: GenerationState, team: PoolTeam, versionId: string | null, includeUnassigned: boolean): boolean {
    let poolMask = 0;
    ...
    for (let role = 0; role < 8; role += 1) {
        if ((state.unassignedRoleCoverCounts[role] ?? 0) > 0)
            unassignedMask |= 1 << role;
    }
...
function coverageScarcityAfter(state: GenerationState, team: PoolTeam, versionId: string): boolean {
    ...
    if (stillLacking === 0)
        return true;
    for (let role = 0; role < 8; role += 1) {
        if ((stillLacking & (1 << role)) === 0)
            continue;
        if ((state.unassignedRoleCoverCounts[role] ?? 0) <= 0)
            return false;
        if ((probeMask & (1 << role)) !== 0)
            return false;
    }
    return true;
}
function poolTenFeasibleAfterAddExact(state: GenerationState, team: PoolTeam, versionId: string | null): boolean {
    const anchorIds = team.anchors.map((anchor) => anchor.playerVersionId);
    const anchorSet = new Set(anchorIds);
    const anchorCounts = rosterGroupCounts(membersOf(state, anchorIds));
    ...
    const picks = 10 - team.anchors.length;
    if (!memberReachCapped(state, { guards: 4, forwards: 4, centers: 3 }, anchorCounts, members, picks)) {
        return false;
    }
    if (!memberReachCapped(state, { guards: 2, forwards: 2, centers: 1 }, { guards: 0, forwards: 0, centers: 0 }, [...team.pool, ...(versionId !== null ? [versionId] : [])], 5)) {
        return false;
    }
    const capValue = state.targets.policy.bandPoolScoreCaps[team.band];
    let anchorOutliers = 0;
    for (const anchorId of anchorIds) {
        if (identityScoreOf(state, anchorId, team.identity) > capValue)
            anchorOutliers += 1;
    }
    if (!memberReachCappedWithOutlierBudget(state, team, { guards: 4, forwards: 4, centers: 3 }, anchorCounts, anchorOutliers, members, picks, state.targets.policy.maxRosterStrengthOutliers)) {
        return false;
    }
    return true;
}
...
function addPoolMember(state: GenerationState, team: PoolTeam, versionId: string, seedPath: string[]): void {
    ...
    if (mask !== undefined && (mask & 1) !== 0)
        team.groupCounts.guards += 1;
    if (mask !== undefined && (mask & 2) !== 0)
        team.groupCounts.forwards += 1;
    if (mask !== undefined && (mask & 4) !== 0)
        team.groupCounts.centers += 1;
    ...
    if (!team.seedPaths.includes(pathKey))
        team.seedPaths.push(pathKey);
}
function removePoolMember(state: GenerationState, team: PoolTeam, versionId: string): void {
    ...
    if (index < 0)
        throw new Error(`pool ${team.franchiseId} does not contain ${versionId}`);
...
function exhausted(state: GenerationState, phase: SeasonAiGenerationPhase, failedTeams: string[], unmetConstraints: string[]): SeasonAiGenerationError {
...
function nodeBudgetOf(targets: SeasonRosterTargets): number {
    return (targets.policy.nodeBudgets.anchorMatching +
...
function anchorOptionsFor(state: GenerationState, team: PoolTeam): AnchorOption[] {
    ...
    for (const candidate of state.canonicalCandidates) {
        if (state.humanOwned.has(candidate.playerVersionId))
            continue;
        if (state.claimedIdentities.has(candidate.playerId))
            continue;
...
function anchorBranchViolations(state: GenerationState, order: readonly PoolTeam[], optionsByTeam: ReadonlyMap<string, readonly AnchorOption[]>, startIndex: number): string[] {
    ...
    for (let i = startIndex; i < order.length; i += 1) {
        ...
        if (need > 0) {
            ...
            for (const option of options) {
                if (!inAnyPool(state, option.versionId))
                    left += 1;
            }
            if (left < need) {
                violations.push(`${team.franchiseId} has ${String(left)} options left for ${String(need)} guarantees`);
            }
        }
    }
    return violations;
...
function matchGuaranteedAnchors(state: GenerationState): void {
    ...
    for (const team of teams) {
        ...
        seedRank.set(team.franchiseId, createRng(seasonNamespaceSeed(state.seed, 'ai-rosters', 'anchors', team.franchiseId, 'team-rank')).next());
    }
    const order = [...teams].sort((a, b) => {
        const optionDiff = (optionsByTeam.get(a.franchiseId)?.length ?? 0) -
    ...
    for (const team of teams) {
        const rng = createRng(seasonNamespaceSeed(state.seed, 'ai-rosters', 'anchors', team.franchiseId, 'option-order'));
        const options = optionsByTeam.get(team.franchiseId) ?? [];
        const ranked = [...options]
            ...
            (a.option.versionId < b.option.versionId
                ...
                : a.option.versionId > b.option.versionId
                    ? 1
                    : 0));
        branchOrder.set(team.franchiseId, ranked.map((entry) => entry.option.versionId));
    }
    ...
    if (!solve(0)) {
        ...
        const unmet = order
            .filter((team) => team.anchors.length < state.targets.policy.guaranteedAnchors[team.band])
            .map((team) => `${team.franchiseId} missing ${String(state.targets.policy.guaranteedAnchors[team.band] - team.anchors.length)} guaranteed anchors`);
        throw exhausted(state, 'anchors', failedTeams, unmet);
    }
}
function addAnchor(state: GenerationState, team: PoolTeam, option: AnchorOption, seedPath: string[]): void {
...
function bestExtraEliteOption(state: GenerationState, team: PoolTeam): AnchorOption | null {
    ...
    const rng = createRng(seasonNamespaceSeed(state.seed, 'ai-rosters', 'anchors', team.franchiseId, 'extra-elite-order'));
    const ranked = [...options]
...
function rollExtraEliteAnchors(state: GenerationState): void {
    for (const teamId of state.teamOrder) {
        ...
        state.nodesByPhase.anchors += 1;
        checkBudget(state);
        const rng = createRng(seasonNamespaceSeed(state.seed, 'ai-rosters', 'anchors', team.franchiseId, 'extra-elite'));
...
function weakestRoles(poolRoleScores: Record<SeasonRosterRole, number>, count: number): SeasonRosterRole[] {
    return [...ROSTER_ROLES].sort((a, b) => poolRoleScores[a] - poolRoleScores[b]).slice(0, count);
}
function poolPickScore(state: GenerationState, team: PoolTeam, versionId: string, rngRanks: ReadonlyMap<string, number>, priorityRoles: readonly SeasonRosterRole[], weakest: readonly SeasonRosterRole[], poolCounts: {
    guards: number;
    forwards: number;
    centers: number;
}): number {
    ...
    let weakTotal = 0;
    for (const role of weakest)
        weakTotal += memberScores?.[role] ?? 0;
    ...
    if (tier !== 'depth') {
        ...
        if (cumulative > tierRange[1])
            score -= (cumulative - tierRange[1]) * TIER_DEFICIT_FACTOR[tier];
    }
    const completion = state.targets.policy.completionTargets;
    const mask = state.maskByVersion.get(versionId) ?? 0;
    let positionHelp = 0;
    if (Math.max(0, completion.guards - poolCounts.guards) > 0 && (mask & 1) !== 0)
        positionHelp += 1;
...
function tierOverage(state: GenerationState, team: PoolTeam, tier: PercentileTier, extra: number): number {
...
function pickForPool(state: GenerationState, team: PoolTeam, round: number): string | null {
    ...
    const slotsLeft = state.targets.policy.poolSize - team.pool.length - 1;
    for (let mask = 1; mask <= 7; mask += 1) {
        if ((state.unassignedMaskCountsArr[mask] ?? 0) === 0)
            continue;
        if (!positionScarcityAfter(state, team, mask))
            continue;
        const probeId = probeForMask(state, team, mask);
        if (probeId === undefined)
            continue;
        const feasible = slotsLeft <= 0
            ? poolTenFeasibleAfterAddExact(state, team, probeId)
            : poolTenFeasibleAfterAdd(state, team, probeId);
        if (feasible)
            feasibleMasks.add(mask);
    }
    if (feasibleMasks.size === 0)
        return null;
    const rngRanks = new Map<string, number>();
    const rng = createRng(seasonNamespaceSeed(state.seed, 'ai-rosters', 'pool-fill', String(round), team.franchiseId, 'candidate-order'));
    for (const candidate of state.canonicalCandidates) {
    ...
    for (const candidate of state.canonicalCandidates) {
        ...
        if (mask === 0 || !feasibleMasks.has(mask))
            continue;
        if (!state.unassigned.has(id))
            continue;
        if (state.claimedIdentities.has(candidate.playerId))
            continue;
        if (state.bans.has(`${team.franchiseId}:${id}`))
            continue;
        if (!poolCoverageFeasible(state, team, id, true))
            continue;
        if (!coverageScarcityAfter(state, team, id))
            continue;
...
function probeForMask(state: GenerationState, team: PoolTeam, mask: number): string | undefined {
    for (const candidate of state.canonicalCandidates) {
        ...
        if (state.bans.has(`${team.franchiseId}:${id}`))
            continue;
        if ((state.maskByVersion.get(id) ?? 0) === mask)
            return id;
    }
    return undefined;
}
function positionScarcityAfter(state: GenerationState, team: PoolTeam, mask: number): boolean {
    ...
    return (supply.guards >= need.guards &&
...
function fillPools(state: GenerationState): void {
    ...
    for (let round = 0; round < poolSize; round += 1) {
        const order = shuffle([...state.teamOrder], createRng(seasonNamespaceSeed(state.seed, 'ai-rosters', 'pool-fill', String(round), 'team-order')));
        for (const teamId of order) {
            const team = state.teams.get(teamId);
            if (team === undefined || team.pool.length >= poolSize)
                continue;
            const pick = pickForPool(state, team, round);
            if (pick === null)
                throw new PoolFillDeadlock(teamId, round);
...
function roleCoverCountsOf(versionIds: Iterable<string>, coverageMaskByVersion: ReadonlyMap<string, number>): number[] {
    const counts = new Array<number>(8).fill(0);
...
function restorePools(state: GenerationState, snapshot: PoolSnapshot): void {
    ...
    for (const entry of snapshot.pools) {
        ...
        if (team === undefined)
            throw new Error(`missing team ${entry.franchiseId}`);
    ...
    state.unassignedRoleCoverCounts = roleCoverCountsOf(state.unassigned, state.coverageMaskByVersion);
    for (const teamId of state.teamOrder) {
        const team = state.teams.get(teamId);
        if (team === undefined)
            continue;
...
function localPoolRepair(state: GenerationState, teamId: string): boolean {
    ...
    for (const memberId of original) {
        ...
        for (const candidate of state.canonicalCandidates) {
            const id = candidate.playerVersionId;
            if (!state.unassigned.has(id) || state.bans.has(`${teamId}:${id}`))
                continue;
            state.nodes += 1;
            state.nodesByPhase['pool-fill'] += 1;
            checkBudget(state);
...
function crossPoolRepair(state: GenerationState, teamId: string): boolean {
    ...
    for (const donorId of state.teamOrder) {
        ...
        for (const memberId of [...donor.pool]) {
            if (state.bans.has(`${teamId}:${memberId}`))
                continue;
            if (donorAnchorSet.has(memberId))
                continue;
            state.nodes += 1;
            state.nodesByPhase['pool-fill'] += 1;
            checkBudget(state);
...
function poolViolations(state: GenerationState, teamId: string): string[] {
    ...
    if (new Set(team.pool).size !== team.pool.length)
        violations.push('pool contains duplicates');
    if (!uniqueIdentities(state, team.pool))
        violations.push('pool contains duplicate identities');
    if (!poolAdmitsTenExact(state, team)) {
...
function memberReachCapped(state: GenerationState, targets: {
    guards: number;
    forwards: number;
    centers: number;
}, startCounts: {
    ...
    forwards: number;
    centers: number;
}, memberIds: readonly string[], maxPicks: number): boolean {
    const targetG = targets.guards;
    const targetF = targets.forwards;
    const targetC = targets.centers;
    ...
    reachable[index(cap(startCounts.guards, targetG), cap(startCounts.forwards, targetF), cap(startCounts.centers, targetC))] = 1;
            ...
            for (let g = targetG; g >= 0; g -= 1) {
                for (let f = targetF; f >= 0; f -= 1) {
                    for (let c = targetC; c >= 0; c -= 1) {
                        ...
                        if (reachable[idx] === 0)
                            continue;
                        reachable[(used + 1) * usedBase +
                            index(cap(g + ((mask & 1) !== 0 ? 1 : 0), targetG), cap(f + ((mask & 2) !== 0 ? 1 : 0), targetF), cap(c + ((mask & 4) !== 0 ? 1 : 0), targetC))] = 1;
                    }
                }
            }
        }
    ...
    for (let used = 0; used <= maxPicks; used += 1) {
        if (reachable[used * usedBase + index(targetG, targetF, targetC)] === 1)
            return true;
    }
    return false;
}
function memberReachCappedWithOutlierBudget(state: GenerationState, team: PoolTeam, targets: {
    guards: number;
    forwards: number;
    centers: number;
}, startCounts: {
    ...
    forwards: number;
    centers: number;
}, startOutliers: number, memberIds: readonly string[], maxPicks: number, maxOutliers: number): boolean {
    const targetG = targets.guards;
    const targetF = targets.forwards;
    const targetC = targets.centers;
    const strideF = targetC + 1;
    ...
    const index = (g: number, f: number, c: number, o: number): number => g * strideG + f * strideF + c + o * strideO;
    const cap = (value: number, max: number): number => Math.min(max, value);
    if (maxPicks < 0 || startOutliers > maxOutliers)
        return false;
    const capValue = state.targets.policy.bandPoolScoreCaps[team.band];
    const reachable = new Uint8Array(usedBase * (maxPicks + 1));
    reachable[index(cap(startCounts.guards, targetG), cap(startCounts.forwards, targetF), cap(startCounts.centers, targetC), startOutliers)] = 1;
    for (const id of memberIds) {
        const mask = state.maskByVersion.get(id) ?? 0;
        if (mask === 0)
            continue;
                    ...
                    for (let c = targetC; c >= 0; c -= 1) {
                        for (let o = maxOutliers; o >= 0; o -= 1) {
                            ...
                            if (o + isOutlier <= maxOutliers) {
                                reachable[(used + 1) * usedBase +
                                    index(cap(g + ((mask & 1) !== 0 ? 1 : 0), targetG), cap(f + ((mask & 2) !== 0 ? 1 : 0), targetF), cap(c + ((mask & 4) !== 0 ? 1 : 0), targetC), o + isOutlier)] = 1;
                            }
                        }
                    }
                }
    ...
    for (let used = 0; used <= maxPicks; used += 1) {
        for (let o = 0; o <= maxOutliers; o += 1) {
            if (reachable[used * usedBase + index(targetG, targetF, targetC, o)] === 1)
                return true;
        }
    }
    return false;
...
function legalTenExists(state: GenerationState, members: readonly string[]): boolean {
    ...
    const rec = (index: number): boolean => {
        if (picked.length === 10) {
            ...
            return (validateSeasonRoster(roster).length === 0 &&
        ...
        for (let i = index; i < ordered.length; i += 1) {
            ...
            if (!memberReachCapped(state, { guards: 4, forwards: 4, centers: 3 }, probeCounts, remaining, slotsLeft - 1)) {
                continue;
            }
...
function fillPoolsWithRepair(state: GenerationState): void {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        try {
            ...
            if (!(error instanceof PoolFillDeadlock))
                throw error;
            state.backtracks += 1;
            if (localPoolRepair(state, error.teamId))
                continue;
            if (crossPoolRepair(state, error.teamId))
                continue;
            ...
            if (team !== undefined) {
                ...
                if (next !== null)
                    state.bans.add(`${error.teamId}:${next}`);
            }
            continue;
        }
        const violators = state.teamOrder.filter((teamId) => poolViolations(state, teamId).length > 0 ||
            !legalTenExists(state, state.teams.get(teamId)?.pool ?? []));
        if (violators.length === 0)
            return;
        const repaired = violators.some((teamId) => {
            if (localPoolRepair(state, teamId))
                return true;
            if (crossPoolRepair(state, teamId))
                return true;
            return false;
        });
        if (repaired)
        ...
        if (team !== undefined) {
            ...
            const next = pickForPool(state, team, round);
            if (next !== null)
                state.bans.add(`${team.franchiseId}:${next}`);
        }
        state.backtracks += 1;
    }
    const failedTeams = state.teamOrder.filter((teamId) => poolViolations(state, teamId).length > 0);
    throw exhausted(state, 'pool-fill', failedTeams, failedTeams.map((teamId) => poolViolations(state, teamId).join('; ')));
}
function coverageFeasibleFromPool(state: GenerationState, uncovered: readonly SeasonRosterRole[], memberIds: readonly string[], slots: number): boolean {
    if (uncovered.length === 0)
        return true;
    if (uncovered.length > slots)
        return false;
    let subsetMask = 0;
    ...
    for (const id of memberIds) {
        ...
        ROSTER_ROLES.forEach((role, roleIndex) => {
            if (scores[role] >= ROLE_COVERAGE_THRESHOLD)
                coveredMask |= 1 << roleIndex;
        });
        const masked = coveredMask & subsetMask;
        if (masked !== 0)
            maskCounts[masked] = (maskCounts[masked] ?? 0) + 1;
    }
    ...
    for (let pass = 0; pass <= slots; pass += 1) {
        ...
        if (reachable[slots * (full + 1) + full] === 1)
            return true;
        let unchanged = true;
...
function rosterLegal(state: GenerationState, team: PoolTeam, ids: readonly string[]): boolean {
    ...
    if (!completionTargetsMet(members))
        return false;
    if (uncoveredRoles(roleScoresOfIds(state, ids)).length > 0)
        return false;
    let rotation;
    try {
    ...
    const playable = new Map(ids.map((id) => [id, state.byId.get(id)?.positions.playable ?? ([] as const)]));
    return validateSeasonRotation(rotation, playable).length === 0;
}
function rosterOutlierCount(state: GenerationState, team: PoolTeam, ids: readonly string[]): number {
    const cap = state.targets.policy.bandPoolScoreCaps[team.band];
    let outliers = 0;
    for (const id of ids) {
        if (identityScoreOf(state, id, team.identity) > cap)
            outliers += 1;
    }
    return outliers;
}
function rosterOutlierBudgetOk(state: GenerationState, team: PoolTeam, ids: readonly string[]): boolean {
...
function selectionRanks(state: GenerationState, team: PoolTeam): Map<string, number> {
    ...
    const rng = createRng(seasonNamespaceSeed(state.seed, 'ai-rosters', 'selection', team.franchiseId, 'candidate-order'));
    for (const candidate of state.canonicalCandidates) {
        ranks.set(candidate.playerVersionId, rng.next());
    }
    return ranks;
}
function selectionPickScore(state: GenerationState, team: PoolTeam, picked: readonly string[], id: string, uncovered: readonly SeasonRosterRole[], rngRanks: ReadonlyMap<string, number>): number {
...
function greedySelection(state: GenerationState, team: PoolTeam): string[] | null {
    ...
    while (picked.length < 10) {
        ...
        for (const id of sortedPool) {
            ...
            const remaining = team.pool.filter((memberId) => !picked.includes(memberId) && memberId !== id);
            if (!memberReachCapped(state, { guards: 4, forwards: 4, centers: 3 }, probeCounts, remaining, slotsLeft)) {
                continue;
            }
            if (!memberReachCapped(state, { guards: 2, forwards: 2, centers: 1 }, { guards: 0, forwards: 0, centers: 0 }, [...picked, id, ...remaining], 5)) {
                continue;
            }
            const memberScores = state.roleScores.get(id);
            const uncoveredAfter = memberScores === undefined
                ? uncovered
                : uncovered.filter((role) => memberScores[role] < ROLE_COVERAGE_THRESHOLD);
            if (!coverageFeasibleFromPool(state, uncoveredAfter, remaining, slotsLeft))
                continue;
            if (!rosterOutlierBudgetOk(state, team, [...picked, id]))
                continue;
        ...
        if (selectionBudgetExceeded(state))
            return null;
    }
    return picked;
}
function rosterSelectionScore(state: GenerationState, team: PoolTeam, ten: readonly string[]): number {
    ...
    if (elite > ranges.elite[1])
        score -= (elite - ranges.elite[1]) * 1.5;
...
function improveSelection(state: GenerationState, team: PoolTeam, ten: readonly string[]): string[] {
    let current = [...ten];
    ...
    for (;;) {
        ...
        for (const outId of current) {
            for (const inId of pool) {
                ...
                if (!rosterLegal(state, team, trial))
                    continue;
                if (!rosterOutlierBudgetOk(state, team, trial))
                    continue;
...
function bestTenDfs(state: GenerationState, team: PoolTeam, members: readonly string[], incumbent: string[] | null): string[] | null {
    ...
    const rec = (index: number): void => {
        ...
        for (let i = index; i < ordered.length; i += 1) {
            ...
            if (!memberReachCapped(state, { guards: 2, forwards: 2, centers: 1 }, { guards: 0, forwards: 0, centers: 0 }, [...afterPicked, ...remaining], 5)) {
                continue;
            }
            const uncoveredAfter = uncoveredRoles(roleScoresOfIds(state, afterPicked));
            ...
            if (!rosterOutlierBudgetOk(state, team, afterPicked))
                continue;
            picked.push(id);
            rec(i + 1);
            picked.pop();
...
function bestRosterFromPool(state: GenerationState, team: PoolTeam): string[] | null {
    ...
    return bestTenDfs(state, team, team.pool, greedy !== null &&
        rosterLegal(state, team, greedy) &&
        rosterOutlierBudgetOk(state, team, greedy)
        ? greedy
        : null);
...
function selectRosters(state: GenerationState): void {
    ...
    for (const teamId of state.teamOrder) {
        ...
        if (ten === null) {
            throw exhausted(state, 'selection', [teamId], [`no legal roster selected from the pool of ${teamId}`]);
        }
        team.selections = ten;
    }
}
function toSeasonAiPool(state: GenerationState, team: PoolTeam): SeasonAiPool {
    ...
    return {
        ...
        selections,
        allocationSeedPaths: selections.map((versionId) => team.memberPaths.get(versionId) ?? ['ai-rosters', team.franchiseId]),
        repairCount: team.repairCount,
    };
}
function qualityWeightsFromRatings(members: readonly {
...
function finalizeResult(state: GenerationState, league: SeasonLeague, humanFranchiseIds: readonly string[], humanRosters: readonly {
    franchiseId: string;
    playerVersionIds: string[];
}[]): SeasonLeagueGenerationResult {
    const rosters = league.teams.map((team) => {
        const aiTeam = state.teams.get(team.franchiseId);
        const ids = aiTeam?.selections ??
            humanRosters.find((r) => r.franchiseId === team.franchiseId)?.playerVersionIds;
        if (!ids)
            throw new Error(`no roster resolved for ${team.franchiseId}`);
        const players = ids.map((playerVersionId) => {
            const candidate = state.byId.get(playerVersionId);
            if (!candidate)
                throw new Error(`missing candidate ${playerVersionId}`);
            return {
                playerVersionId,
                playerId: candidate.playerId,
    ...
    const ownership = rosters.flatMap((roster) => roster.players.map((player) => ({
    ...
    const rotations = rosters.map((roster) => {
        const members = roster.players.map((player) => {
            ...
            if (!candidate)
                throw new Error(`missing candidate ${player.playerVersionId}`);
    ...
    const rotationByFranchise = new Map(rotations.map((rotation) => [rotation.franchiseId, rotation]));
    ...
    const plannedRotations = rosters.map((roster) => {
        ...
        if (base === undefined)
            throw new Error(`missing rotation for ${roster.franchiseId}`);
        const members = roster.players.map((player) => {
            const candidate = state.byId.get(player.playerVersionId);
            if (!candidate)
                throw new Error(`missing candidate ${player.playerVersionId}`);
        ...
        try {
            ...
            const players = new Map<string, MinutePlanPlayerInput>(members.map((member) => [
                member.playerVersionId,
                {
            ...
            if (plan === undefined)
                throw new Error('no recommended minute plan');
    ...
    const aiPools = state.teamOrder.map((teamId) => {
        const team = state.teams.get(teamId);
        if (team === undefined)
            throw new Error(`missing AI team ${teamId}`);
...
export function generateAiLeague(input: SeasonAiGenerationInput): SeasonLeagueGenerationResult {
    ...
    const canonicalCandidates = [...catalog.candidates].sort((a, b) => a.playerVersionId < b.playerVersionId ? -1 : a.playerVersionId > b.playerVersionId ? 1 : 0);
    ...
    const assignments = new Map(assignAiBandsAndIdentities({
        seed: input.seed,
        league,
    ...
    for (const franchiseId of teamOrder) {
        ...
        if (!assignment)
            throw new Error(`no AI assignment for ${franchiseId}`);
    ...
    const initialUnassigned = canonicalCandidates
        .filter((candidate) => !humanOwned.has(candidate.playerVersionId) && !humanOwnedIdentities.has(candidate.playerId))
...
export function runSeasonRosterCalibrationSeeds(input: {
    ...
    return input.seeds.map((seed) => {
        ...
        const superTeamIncidence = generation.evaluations
            .filter((evaluation) => (evaluation.band === 'average' || evaluation.band === 'weaker') &&
