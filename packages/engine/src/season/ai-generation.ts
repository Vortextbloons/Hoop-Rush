...
export function identityPriorityRolesOf(targets: SeasonRosterTargets, identity: SeasonAiIdentity): readonly SeasonRosterRole[] {
    const roles = (targets.policy.identityPriorityRoles as unknown as Record<string, SeasonRosterRole[]>)[identity];
...
export function validateSeasonRosterTargets(targets: SeasonRosterTargets): void {
    ...
    if (raw.schemaVersion !== 2) {
        throw new SeasonAiTargetsError(`roster targets schemaVersion must be 2 (got ${String(raw.schemaVersion)})`);
    }
    if (raw.targetsVersion !== SEASON_ROSTER_TARGETS_VERSION) {
        throw new SeasonAiTargetsError(`roster targets version mismatch: expected ${SEASON_ROSTER_TARGETS_VERSION}, got ${String(raw.targetsVersion)}`);
    }
    ...
    if (calibration === undefined || calibration.aiVersion !== SEASON_AI_VERSION) {
        throw new SeasonAiTargetsError(`targets aiVersion mismatch: expected ${SEASON_AI_VERSION}, got ${String(calibration?.aiVersion)}`);
    }
    if (calibration.rosterGenerationVersion !== SEASON_ROSTER_GENERATION_VERSION) {
        throw new SeasonAiTargetsError(`targets rosterGenerationVersion mismatch: expected ${SEASON_ROSTER_GENERATION_VERSION}, got ${String(calibration.rosterGenerationVersion)}`);
    }
    const policy = raw.policy as Record<string, unknown> | undefined;
    const priorityRoles = policy?.['identityPriorityRoles'] as Record<string, SeasonRosterRole[]> | undefined;
...
function quotaTotal(bandQuotas: Record<string, Record<string, number>> | undefined, key: string): number {
    const quotas = bandQuotas?.[key];
    if (quotas === undefined)
        return -1;
    return ((quotas.contender ?? 0) + (quotas.playoff ?? 0) + (quotas.average ?? 0) + (quotas.weaker ?? 0));
}
