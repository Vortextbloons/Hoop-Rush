...
export function seasonPostseasonHumanEliminated(state: SeasonPostseasonState, humanFranchiseId: string): boolean {
    if (state.bracket === null) {
        for (const conference of CONFERENCES) {
            ...
            const nineTen = playIn.games.nineTen;
            const final = playIn.games.final;
            const wonAny = [sevenEight, nineTen, final].some((game) => game.status !== 'scheduled' && game.winnerFranchiseId === humanFranchiseId);
            if (wonAny)
                return false;
...
function resolvePlayInGame(game: PlayInGame, home: string, away: string, facts: SeasonPostseasonGameFacts): PlayInGame {
    ...
    if (seriesId.startsWith(`${conference}-first-round-`)) {
        ...
        if (nextSlot.homeCourtFranchiseId !== null && nextSlot.challengerFranchiseId !== null) {
            if (other === undefined || other.winnerFranchiseId === null) {
                throw new SeasonPostseasonInvariantError(`first-round winner ${winner} cannot pair a semifinal`);
            }
            paired = pairSeededSeries(nextSlot, { team: winner, seed: winnerSeedOf(feed) }, { team: other.winnerFranchiseId, seed: winnerSeedOf(other) });
        }
        const confUpdated = {
            ...confBracket,
    ...
    if (seriesId.startsWith(`${conference}-semifinal-`)) {
        ...
        if (nextSlot.homeCourtFranchiseId !== null && nextSlot.challengerFranchiseId !== null) {
            if (other.winnerFranchiseId === null) {
                throw new SeasonPostseasonInvariantError(`semifinal winner ${winner} cannot pair the conference final`);
            }
            paired = pairSeededSeries(nextSlot, { team: winner, seed: winnerSeedOf(current) }, { team: other.winnerFranchiseId, seed: winnerSeedOf(other) });
        }
...
export function decideSeasonFinalsHomeCourt(input: {
    ...
    const rowOf = (franchiseId: string) => standings.rows.find((row) => row.franchiseId === franchiseId);
    ...
    const resolution: SeasonTiebreakResolution = {
        resolutionId: `tb-finals-home-court-${seasonDigestHex([east, west, input.drawSeed, rule].join('\u0000'))}`,
...
function applyPlayoffGameResult(state: SeasonPostseasonState, facts: SeasonPostseasonGameFacts, league: SeasonLeague, standings: SeasonStandings): SeasonPostseasonState {
    ...
    const champion = completed && seriesId === 'finals'
...
export function seasonPostseasonSummaryFromGame(input: {
    ...
    const winnerFranchiseId = seasonSummary.status === 'forfeit'
        ...
        : homeWon
            ? input.homeFranchiseId
            : input.awayFranchiseId;
    const loserFranchiseId = seasonSummary.status === 'forfeit'
