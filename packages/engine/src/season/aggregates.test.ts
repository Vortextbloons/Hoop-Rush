...
import { SEASON_LEADER_MIN_GAME_SHARE, franchiseIdSchema, seasonGameIdSchema, seasonLeadersSchema, seasonPlayerAggregateSchema, seasonTeamAggregateSchema, type SeasonGameSummary, type SeasonStandings, } from '@hoop-rush/data-contracts';
...
function finalSummary(overrides: Partial<SeasonGameSummary> = {}): SeasonGameSummary {
    ...
    const box = (players: typeof homePlayers) => {
        ...
        return {
            franchiseId: franchiseIdSchema.parse(players[0]?.playerVersionId.startsWith('pv-h') ? 'lakers' : 'celtics'),
