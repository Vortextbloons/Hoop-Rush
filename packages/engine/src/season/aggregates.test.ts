...
import { SEASON_LEADER_MIN_GAME_SHARE, seasonLeadersSchema, type SeasonGameSummary, type SeasonStandings, } from '@hoop-rush/data-contracts';
import { auditSeasonAggregates, deriveSeasonLeaders, foldSeasonPlayerAggregates, foldSeasonTeamAggregates, provisionalStandingOrder, } from './aggregates.ts';
function finalSummary(overrides: Partial<SeasonGameSummary> = {}): SeasonGameSummary {
    const homeLine = (playerVersionId: string, points: number): SeasonGameSummary['homePlayers'][number] => ({
        playerVersionId,
        seconds: 1440,
        points,
...
function standingsOf(summaries: readonly SeasonGameSummary[]): SeasonStandings {
    ...
    return {
        ...
        rows: franchiseIds.map((franchiseId) => {
            const record = summaries.reduce((acc, summary) => {
                ...
                const pointsFor = summary.homeFranchiseId === franchiseId ? summary.homeScore : summary.awayScore;
                const pointsAgainst = summary.homeFranchiseId === franchiseId ? summary.awayScore : summary.homeScore;
                return {
...
describe('season aggregate folding (M2.3)', () => {
    ...
    it('flags tampered aggregates and standings in the audit', () => {
        ...
        const tamperedTeams = teams.map((team) => team.franchiseId === 'lakers' ? { ...team, wins: team.wins + 1 } : team);
        const failures = auditSeasonAggregates({
            teams: tamperedTeams,
            players,
            summaries,
        ...
        const tamperedStandings = {
            ...standings,
            rows: standings.rows.map((row) => row.franchiseId === 'lakers' ? { ...row, pointsFor: row.pointsFor + 5 } : row),
        };
