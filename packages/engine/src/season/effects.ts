import type { SeasonArchivedPairChemistryState, SeasonEffectsState, SeasonGameEffectsTransition, SeasonMechanism, SeasonMechanismEvidence, SeasonPairChemistryState, SeasonPlayerLoadState, SeasonRoster, SeasonRotation, SeasonStaminaInput, } from '@hoop-rush/data-contracts';
import { franchiseIdSchema } from '@hoop-rush/data-contracts';
import type { SideIndex } from '../sim/recorder.ts';
import { canonicalRosterPairs, pairChemistryBasisPoints, seasonPairIsCanonical, seasonPairKey, unitPairs, } from './chemistry.ts';
...
function pairKeysOf(unit: readonly string[]): Array<{
    ...
    for (const [a, b] of unitPairs(unit))
        out.push({ a, b, key: seasonPairKey(a, b) });
    return out;
}
...
export interface SeasonEffectsBuffer {
    ...
    finishGame(homeRegulationSeconds: ReadonlyMap<string, number>, awayRegulationSeconds: ReadonlyMap<string, number>): {
...
class EffectsBufferImpl implements SeasonEffectsBuffer {
    ...
    constructor(pregame: ReadonlyMap<string, SeasonPlayerLoadState>, homeStamina: ReadonlyMap<string, SeasonStaminaInput>, awayStamina: ReadonlyMap<string, SeasonStaminaInput>, pairStates: readonly SeasonPairChemistryState[]) {
        this.pregame = pregame;
        this.stamina = new Map([...homeStamina.values(), ...awayStamina.values()].map((input) => [
            input.playerVersionId,
            input.rating,
        ]));
        this.homeRoster = new Set(homeStamina.keys());
        this.awayRoster = new Set(awayStamina.keys());
        this.game = new Map([...pregame.values()].map((state) => [
    ...
    readonly hook: SeasonEffectsHook = {
        makeAdjustment: (facts) => {
            ...
            this.record('shooter-fatigue', offenseSide === 0 ? 'home' : 'away', shooterDelta, shooterFrac, 'shooter');
            ...
            this.record('defensive-unit-fatigue', defenseSide === 0 ? 'home' : 'away', defenseDelta, defenseFrac, 'defenseMean');
            ...
            this.record('help-defense', defenseSide === 0 ? 'home' : 'away', helpDelta, chemFrac, 'unitChemistry');
        ...
        turnoverAdjustment: (facts) => {
            ...
            this.record('handler-fatigue', offenseSide === 0 ? 'home' : 'away', handlerDelta, handlerFrac, 'handler');
            ...
            this.record('turnover-security', offenseSide === 0 ? 'home' : 'away', securityDelta, chemFrac, 'unitChemistry');
        ...
        assistAdjustment: (facts) => {
            ...
            this.record('assist-conversion', facts.offenseSide === 0 ? 'home' : 'away', delta, chemFrac, 'unitChemistry');
            return delta;
        },
        ...
        halftime: () => {
            ...
            for (const roster of [this.homeRoster, this.awayRoster]) {
                for (const version of roster) {
                    const state = this.game.get(version);
                    const rating = this.stamina.get(version);
                    if (state === undefined || rating === undefined)
                        continue;
    ...
    private applyRoleBonuses(side: SideIndex, unit: readonly string[], handler: string, shooter: string | undefined, defender: string | undefined, reboundContests: number): void {
        for (const version of unit) {
            const state = this.game.get(version);
            if (state === undefined)
                continue;
            let bonus = 0;
            if (version === handler || version === shooter)
                bonus += 12;
            if (version === defender)
                bonus += 8;
    ...
    private record(mechanism: SeasonMechanism, side: 'home' | 'away', delta: number, inputFraction: number, inputField: 'shooter' | 'handler' | 'defenseMean' | 'unitChemistry'): void {
    ...
    finishGame(homeRegulationSeconds: ReadonlyMap<string, number>, awayRegulationSeconds: ReadonlyMap<string, number>): {
        ...
        const evidence = [...this.evidence.values()]
            ...
            .sort((x, y) => x.mechanism < y.mechanism ? -1 : x.mechanism > y.mechanism ? 1 : x.side < y.side ? -1 : 1);
        return { postgamePlayerStates, pairIncrements, evidence };
    }
}
export function createSeasonEffectsBuffer(pregame: SeasonEffectsState, homeStamina: ReadonlyMap<string, SeasonStaminaInput>, awayStamina: ReadonlyMap<string, SeasonStaminaInput>): SeasonEffectsBuffer {
    const pregameByVersion = new Map(pregame.playerStates.map((player) => [player.playerVersionId, player]));
    return new EffectsBufferImpl(pregameByVersion, homeStamina, awayStamina, pregame.pairStates);
}
export function createSeasonEffectsState(playerStaminaInputs: readonly SeasonStaminaInput[]): SeasonEffectsState {
    if (playerStaminaInputs.length !== 300) {
        throw new Error(`season effects: expected 300 stamina inputs, got ${String(playerStaminaInputs.length)}`);
    }
...
export function applySeasonGameEffectsTransition(previous: SeasonEffectsState, transition: SeasonGameEffectsTransition): SeasonEffectsState {
    const previousPlayers = new Map(previous.playerStates.map((player) => [player.playerVersionId, player]));
    ...
    const postgamePlayers = new Map(transition.postgamePlayerStates.map((player) => [player.playerVersionId, player]));
    ...
    const pairByKey = new Map(previous.pairStates.map((pair) => [seasonPairKey(pair.a, pair.b), pair]));
    ...
    for (const increment of transition.pairIncrements) {
        ...
        if (pair === undefined) {
            throw new Error(`season effects: pair increment on untracked pair ${increment.a}-${increment.b}`);
        }
...
export function reconcileSeasonEffects(input: SeasonEffectsReconcileInput): SeasonEffectsState {
    ...
    for (const roster of rosters) {
        for (const player of roster.players) {
            if (ownerOf.has(player.playerVersionId)) {
                throw new Error(`season effects: ${player.playerVersionId} appears on two rosters during reconciliation`);
            }
    ...
    for (const rotation of rotations) {
        ...
        if (ids.length !== 10) {
            throw new Error(`season effects: rotation for ${rotation.franchiseId} must contain exactly ten players`);
        }
        for (const id of ids) {
            if (!rosterIds.has(id)) {
                throw new Error(`season effects: rotation member ${id} is not on roster ${rotation.franchiseId}`);
            }
        }
    ...
    const previousActive = new Map(previous.playerStates.map((player) => [player.playerVersionId, player]));
    const previousInactive = new Map(previous.inactivePlayerStates.map((player) => [player.playerVersionId, player]));
    ...
    if (playerStates.length !== 300) {
        throw new Error(`season effects: reconciliation produced ${String(playerStates.length)} active loads`);
    }
    const inactivePlayerStates: SeasonPlayerLoadState[] = [];
    for (const [id, state] of previousActive) {
        if (!activeIds.has(id))
            inactivePlayerStates.push({ ...state });
    }
    ...
    if (inactivePlayerStates.length > 150) {
        throw new Error(`season effects: ${String(inactivePlayerStates.length)} inactive loads exceed 150`);
    }
    const previousPairByKey = new Map(previous.pairStates.map((pair) => [seasonPairKey(pair.a, pair.b), pair]));
    ...
    if (pairStates.length !== 1350) {
        throw new Error(`season effects: reconciliation produced ${String(pairStates.length)} active pairs`);
    }
    ...
    for (const pair of previous.pairStates) {
        ...
        if (franchiseA === undefined || franchiseA !== franchiseB)
            continue;
