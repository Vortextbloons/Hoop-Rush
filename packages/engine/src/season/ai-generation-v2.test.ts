...
import { SEASON_MINUTE_POLICY_VERSION, seasonDigestHex, seedSchema, type SeasonDraftCatalog, type SeasonDraftCandidate, type SeasonLeague, type SeasonLeagueGenerationResult, type SeasonRosterTargets, type SeasonRotation, } from '@hoop-rush/data-contracts';
import { buildSeasonDraftCatalog, seedFromString } from '@hoop-rush/test-fixtures';
import { SeasonAiGenerationError, SeasonAiTargetsError, fiveReachableFromCounts, generateAiLeague, validateSeasonRosterTargets, } from './ai.ts';
import { nearestRankThreshold, percentileTierOf, playerPercentileTier, rolePercentileThresholds, ROSTER_ROLES, } from './ai-scoring.ts';
import { MINUTE_POLICY_STRATEGIES, buildMinutePlanCandidates, minutePlanHorizonGames, } from './minute-plan.ts';
import { completionTargetsMet, legalFiveAfterAnyRemoval, rosterFeasibleFromCounts, rosterGroupCounts, validateSeasonRoster, } from './roster-rules.ts';
import { auditSeasonRotation, buildMinimalRotation, rotationTargetMinutes, validateSeasonRotation, } from './rotation.ts';
import { buildTestTargets, CATALOG, humanRoster, LEAGUE, membersOf, soloInput, } from './ai-test-support.ts';
function brandedSolo(seed: string, catalog: SeasonDraftCatalog = CATALOG, league: SeasonLeague = LEAGUE, humanFranchiseId = 'lakers') {
...
function canonicalFacts(result: SeasonLeagueGenerationResult): string {
    ...
    const ownership = [...result.ownership].sort((a, b) => a.playerVersionId < b.playerVersionId ? -1 : 1);
...
describe('v2 league-wide private pools', () => {
    ...
    it('gives contenders two qualifying anchors and playoffs one', () => {
        ...
        for (const assignment of result.aiAssignments) {
            if (assignment.franchiseId === 'lakers')
                continue;
...
describe('v2 roster legality', () => {
    it('every selected roster passes every legality contract', () => {
        ...
        for (const roster of result.rosters) {
            ...
            const playable = new Map(members.map((member) => [
        ...
        const rosterIdentities = result.rosters.flatMap((roster) => roster.players.map((player) => player.playerId));
    ...
    it('every pool admits a legal ten (4/4/3 completion and a legal five)', () => {
        ...
        for (const pool of result.aiPools) {
            const members = pool.playerVersionIds.map((id) => {
                ...
                if (!candidate)
                    throw new Error('unknown pool member');
            ...
            expect(rosterFeasibleFromCounts({ guards: 0, forwards: 0, centers: 0 }, masks, 10)).toBe(true);
...
describe('v2 determinism', () => {
    ...
    it('produces meaningful variation across seeds', () => {
        ...
        const byTeam = (result: SeasonLeagueGenerationResult) => new Map(result.aiPools.map((pool) => [pool.franchiseId, new Set(pool.playerVersionIds)]));
...
describe('v2 scarcity and failure', () => {
    ...
    it('fails typed at pool fill when the catalog is too small (repair ladder, then exhaustion)', () => {
        ...
        for (const candidate of catalog.candidates) {
            if (keepC.has(candidate.playerVersionId))
                continue;
    ...
    it.each([
    ...
    ] as const)('enforces the %s node budget without relaxing rules', (_label, nodeBudgets, seed, expectedPhase, expectedBudget) => {
        const targets = {
            ...buildTestTargets(),
            policy: {
        ...
        try {
            ...
            if (expectedBudget !== null)
                expect(error.diagnostics.nodeBudget).toBe(expectedBudget);
        }
    });
    it('rejects null or mismatched targets before any allocation', () => {
        expect(() => generateAiLeague({
...
describe('v2 minute-policy rotations (projection milestone)', () => {
    it('different AI teams can select different strategies', () => {
        ...
        const strategies = result.evaluations.map((evaluation) => evaluation.minutePlanSummary?.strategy);
        for (const strategy of strategies) {
            expect(MINUTE_POLICY_STRATEGIES).toContain(strategy);
        }
        expect(new Set(strategies)).toEqual(new Set(['starter-heavy']));
        for (const evaluation of result.evaluations) {
            const rotation = result.rotations.find((candidate) => candidate.franchiseId === evaluation.franchiseId);
    ...
    it('rotations carry the minute policy matching the evaluation summary per franchise', () => {
        ...
        for (const evaluation of result.evaluations) {
            ...
            expect(summary.maxStarterStrainBasisPoints).toBeLessThanOrEqual(10000);
    ...
    it('every generated rotation passes the rotation audit and totals 240 minutes', () => {
        ...
        for (const rotation of result.rotations) {
            const playable = new Map(membersOf(result, rotation.franchiseId, CATALOG).map((member) => [
                member.playerVersionId,
                CATALOG.candidates.find((candidate) => candidate.playerVersionId === member.playerVersionId)?.positions.playable ?? [],
            ]));
    ...
    it('target minutes are dynamic, never the flat 32/16 template', () => {
        ...
        for (const rotation of result.rotations) {
            ...
            expect(rotationTargetMinutes(rotation)).toBe(240);
            expect(rotation.targetMinutes.every((row) => row.minutes === 32 || row.minutes === 16)).toBe(false);
    ...
    it('honors per-roster stamina and durability differences through the AI path', () => {
        ...
        for (const evaluation of result.evaluations) {
            if (evaluation.franchiseId === 'lakers')
                continue;
            const summary = evaluation.minutePlanSummary;
            expect(summary).toBeDefined();
            if (summary === undefined)
            ...
            expect(recommended.strategy).toBe(summary.strategy);
            expect(recommended.rotation.targetMinutes).toEqual(rotation.targetMinutes);
            const playable = new Map(membersOf(result, rotation.franchiseId, skewed).map((member) => [
                member.playerVersionId,
                skewed.candidates.find((candidate) => candidate.playerVersionId === member.playerVersionId)?.positions.playable ?? [],
            ]));
            expect(auditSeasonRotation(rotation, playable)).toEqual([]);
            expect(rotationTargetMinutes(rotation)).toBe(240);
        }
...
function recommendMinutes(result: SeasonLeagueGenerationResult, franchiseId: string, catalog: SeasonDraftCatalog = CATALOG): {
    ...
    const members = roster.players.map((player) => {
        const candidate = catalog.candidates.find((row) => row.playerVersionId === player.playerVersionId);
        if (!candidate)
            throw new Error('roster references an unknown candidate');
        return {
    ...
    const { plans, recommended } = buildMinutePlanCandidates({
        ...
        players: new Map(members.map((member) => [
            member.playerVersionId,
            {
...
function qualityWeightsOf(members: readonly {
