import { SEASON_SEED_NAMESPACES, SEASON_TRADE_VERSION, seasonNamespaceSeed, type Position, type SeasonDraftCatalog, type SeasonEffectsState, type SeasonHealthState, type SeasonInfluenceState, type SeasonRoster, type SeasonRotation, type SeasonRun, type SeasonTradeOffer, type SeasonTradeOfferValueBand, type SeasonTradeRosterChange, type SeasonTradeState, type SeasonTradeWindowState, type SeasonTransactionEntry, type SimulationRatings, } from '@hoop-rush/data-contracts';
...
export function packageSizesOf(kind: SeasonTradePackageKind): {
    ...
    if (kind === '2-2')
        return { outgoing: 2, incoming: 2 };
    if (kind === '1-2')
        return { outgoing: 1, incoming: 2 };
...
export function seasonTradePlayerHealthFacts(health: SeasonHealthState, playerVersionId: string): SeasonTradePlayerHealthFacts {
    const activeInjuryIds = health.injuries
        .filter((injury) => injury.playerVersionId === playerVersionId &&
...
export function seasonEconomyRunOf(run: SeasonRun, effects?: SeasonEffectsState): SeasonEconomyRun {
    ...
    throw new SeasonTradeFactsError('the effects state is required (the persistence record keeps it beside the run snapshot)');
}
...
export function seasonTradePlayerValue(playerVersionId: string, context: SeasonTradeValueContext): number {
    ...
    const offense = mean(ratings.insideScoring, ratings.closeShot, ratings.midrange, ratings.threePoint, ratings.freeThrow, ratings.ballHandling, ratings.passing, ratings.offensiveIq);
    const defense = mean(ratings.perimeterDefense, ratings.interiorDefense, ratings.steal, ratings.block, ratings.defensiveIq, ratings.offensiveRebound, ratings.defensiveRebound);
    const physical = mean(ratings.speed, ratings.strength, ratings.vertical);
    const contribution = VALUE_OFFENSE_WEIGHT * offense +
    ...
    const load = context.run.effects.playerStates.find((player) => player.playerVersionId === playerVersionId)
...
export function seasonTradeValueBandFor(input: {
    ...
    if (outgoing <= 0)
        throw new SeasonTradeInvariantError('outgoing trade value must be positive');
    const raw = Math.round((1000 * incoming) / outgoing);
    const ratioBasisPoints = Math.min(RATIO_SCHEMA_BOUNDS.upper, Math.max(RATIO_SCHEMA_BOUNDS.lower, raw));
...
export function ratioMutuallyWithinBand(ratioBasisPoints: number, kind: SeasonTradePackageKind): boolean {
    const bounds = kind === '1-1' ? BAND_85_115 : BAND_80_120;
    if (ratioBasisPoints < bounds.lower || ratioBasisPoints > bounds.upper)
        return false;
...
export function rosterPlayerVersionIdsOf(run: SeasonRun, franchiseId: string): string[] {
    ...
    if (roster === undefined)
        throw new SeasonTradeInvariantError(`unknown roster ${franchiseId}`);
...
function participantFranchiseIdsOfInput(input: SeasonOpenTradeWindowInput): readonly string[] {
    if (input.participantFranchiseIds && input.participantFranchiseIds.length > 0)
        return input.participantFranchiseIds;
    if (input.humanFranchiseId)
        return [input.humanFranchiseId];
    return [];
}
...
function rankedBySeed<T>(items: readonly T[], seedOf: (item: T) => string, keyOf: (item: T) => string): T[] {
...
function pickDistinct<T>(items: readonly T[], seedOf: (item: T) => string, keyOf: (item: T) => string, k: number): T[] {
    if (k < 1 || k > items.length) {
        throw new SeasonTradeInvariantError(`cannot pick ${String(k)} of ${String(items.length)} items`);
    }
...
function swappedRosterIds(rosterIds: readonly string[], removed: readonly string[], added: readonly string[]): string[] {
...
function rosterLegalityReasons(rosterIds: readonly string[], facts: SeasonTradeCatalogFacts): string[] {
...
function rotationSubsetExists(members: readonly SeasonRosterMemberInput[]): boolean {
    ...
    for (let mask = 0; mask < total; mask += 1) {
        ...
        if (subset.length === 10 && validateTenMemberRotation(subset))
            return true;
    }
    return false;
}
...
function humanOfferCandidate(context: OfferGenerationContext, seedPath: string[], aiFranchiseId: string, kind: SeasonTradePackageKind, probeIndex: number): OfferCandidate | null {
    ...
    const outgoing = pickDistinct(humanRosterIds, (id) => tradeSeed(rootSeed, ...seedPath, 'outgoing', String(probeIndex), id), (id) => id, sizes.outgoing);
    const incoming = pickDistinct(aiRosterIds, (id) => tradeSeed(rootSeed, ...seedPath, 'incoming', String(probeIndex), id), (id) => id, sizes.incoming);
    ...
    const outgoingValues = outgoing.map((id) => seasonTradePlayerValue(id, {
        run,
        catalogFacts,
        receivingFranchiseId: humanFranchiseId,
        candidateRosterIds: humanAfter,
    }));
    const incomingValues = incoming.map((id) => seasonTradePlayerValue(id, {
        run,
        catalogFacts,
        receivingFranchiseId: humanFranchiseId,
        candidateRosterIds: humanAfter,
    }));
    const rawRatio = Math.round((1000 * incomingValues.reduce((sum, value) => sum + value, 0)) /
        outgoingValues.reduce((sum, value) => sum + value, 0));
    return { aiFranchiseId, kind, outgoing, incoming, rawRatio };
}
function rankedAiFranchises(context: OfferGenerationContext, seedPath: string[], usedFranchiseIds: readonly string[]): string[] {
    const ai = aiFranchiseIdsOf(context.run, context.humanFranchiseId);
    return rankedBySeed(ai.filter((franchiseId) => !usedFranchiseIds.includes(franchiseId)), (id) => tradeSeed(context.rootSeed, ...seedPath, 'franchise', id), (id) => id);
}
...
export function generateHumanTradeOffer(context: OfferGenerationContext, seedPath: string[], usedFranchiseIds: readonly string[]): SeasonTradeOffer | null {
    ...
    for (const aiFranchiseId of franchises) {
        ...
        for (const kind of kinds) {
            for (let probe = 0; probe < OFFER_PROBE_BUDGET; probe += 1) {
                ...
                if (candidate === null)
                    continue;
                const inRange = candidate.rawRatio >= RATIO_SCHEMA_BOUNDS.lower &&
...
function assembleHumanOffer(context: OfferGenerationContext, seedPath: string[], candidate: OfferCandidate): SeasonTradeOffer {
    ...
    const outgoingValues = candidate.outgoing.map((id) => seasonTradePlayerValue(id, {
        run,
        catalogFacts,
        receivingFranchiseId: humanFranchiseId,
        candidateRosterIds: humanAfter,
    }));
    const incomingValues = candidate.incoming.map((id) => seasonTradePlayerValue(id, {
        run,
        catalogFacts,
        receivingFranchiseId: humanFranchiseId,
        candidateRosterIds: humanAfter,
    ...
    const outgoingHealth = candidate.outgoing.map((id) => seasonTradePlayerHealthFacts(run.health, id));
    const incomingHealth = candidate.incoming.map((id) => seasonTradePlayerHealthFacts(run.health, id));
    ...
    const minutesById = new Map((rotation?.targetMinutes ?? []).map((entry) => [entry.playerVersionId, entry.minutes]));
    ...
    const removedPairs = canonicalPlayerPairs(humanRosterIds).filter(([a, b]) => candidate.outgoing.includes(a) || candidate.outgoing.includes(b)).length;
    const newPairs = canonicalPlayerPairs(humanAfter).filter(([a, b]) => candidate.incoming.includes(a) || candidate.incoming.includes(b)).length;
...
function coverageDepthOf(rosterIds: readonly string[], movedIn: readonly string[], facts: SeasonTradeCatalogFacts): number {
    const groups = new Set<SlotGroup>();
    ...
    for (const id of rosterIds) {
        ...
        if (playable === undefined)
            continue;
        if ([...groups].some((group) => canPlayGroup(playable, group)))
            depth += 1;
    }
    return depth;
}
export function generatedExtraOfferForSpend(rootSeed: string, run: SeasonEconomyRun, windowIndex: number, humanFranchiseId: string, catalog?: SeasonDraftCatalog): SeasonTradeOffer {
    if (catalog === undefined) {
        throw new SeasonTradeFactsError('generatedExtraOfferForSpend requires the packaged catalog (player positions + ratings)');
    }
    ...
    const priorOffers = (run.trade?.windows.find((window) => window.windowIndex === windowIndex)?.offers ?? [])
...
function aiTradeCandidate(run: SeasonEconomyRun, context: OfferGenerationContext, attempt: number, usedPairs: ReadonlySet<string>, protectedPlayers: ReadonlySet<string>): {
    a: string;
    b: string;
    kind: SeasonTradePackageKind;
    outgoing: string[];
    ...
    const a = rankedBySeed(ai, (id) => tradeSeed(rootSeed, ...basePath, 'a', id), (id) => id)[0];
    if (a === undefined)
        return null;
    const b = rankedBySeed(ai.filter((id) => id !== a), (id) => tradeSeed(rootSeed, ...basePath, 'b', id), (id) => id)[0];
    if (b === undefined)
        return null;
    ...
    for (const probeKind of kindOrderStartingAt(kind)) {
        ...
        const outgoing = pickDistinct(rosterA, (id) => tradeSeed(rootSeed, ...basePath, 'outgoing', id), (id) => id, probeSizes.outgoing);
        const incoming = pickDistinct(rosterB, (id) => tradeSeed(rootSeed, ...basePath, 'incoming', id), (id) => id, probeSizes.incoming);
        if (outgoing.some((id) => protectedPlayers.has(id)) ||
        ...
        const incomingValues = outgoing.map((id) => seasonTradePlayerValue(id, {
            run,
            catalogFacts,
            receivingFranchiseId: b,
            candidateRosterIds: bAfter,
        }));
        const outgoingValues = incoming.map((id) => seasonTradePlayerValue(id, {
            run,
            catalogFacts,
            receivingFranchiseId: b,
            candidateRosterIds: bAfter,
        ...
        const rawRatio = Math.round((1000 * incomingValues.reduce((sum, value) => sum + value, 0)) /
            outgoingValues.reduce((sum, value) => sum + value, 0));
        if (!ratioMutuallyWithinBand(rawRatio, probeKind))
            continue;
        return { a, b, kind: probeKind, outgoing, incoming, rawRatio };
    }
    return null;
}
function assembleAiOffer(run: SeasonEconomyRun, context: OfferGenerationContext, candidate: {
    a: string;
    b: string;
    kind: SeasonTradePackageKind;
    outgoing: string[];
    incoming: string[];
}, attempt: number): SeasonTradeOffer {
    const { rootSeed, windowIndex, catalogFacts } = context;
    ...
    const outgoingValues = candidate.incoming.map((id) => seasonTradePlayerValue(id, {
        run,
        catalogFacts,
        receivingFranchiseId: candidate.b,
        candidateRosterIds: bAfter,
    }));
    const incomingValues = candidate.outgoing.map((id) => seasonTradePlayerValue(id, {
        run,
        catalogFacts,
        receivingFranchiseId: candidate.b,
        candidateRosterIds: bAfter,
    ...
    const outgoingHealth = candidate.incoming.map((id) => seasonTradePlayerHealthFacts(run.health, id));
    const incomingHealth = candidate.outgoing.map((id) => seasonTradePlayerHealthFacts(run.health, id));
    ...
    const removedPairs = canonicalPlayerPairs(rosterB).filter(([x, y]) => candidate.incoming.includes(x) || candidate.incoming.includes(y)).length;
    const newPairs = canonicalPlayerPairs(bAfter).filter(([x, y]) => candidate.outgoing.includes(x) || candidate.outgoing.includes(y)).length;
    return {
        offerId,
        windowIndex,
        seedPath,
...
function applyAiInfluenceSpends(run: SeasonRun, rootSeed: string, windowIndex: number, blockIndex: number, humanFranchiseId: string): AiSpendResult {
    ...
    for (const franchiseId of ai) {
        const extraSeed = tradeSeed(rootSeed, 'window', String(windowIndex), 'ai-spend', franchiseId, 'extra-offer');
        const wantExtra = seedInt(extraSeed, 100) < AI_EXTRA_OFFER_WILLINGNESS_PERCENT;
        const spentExtra = (influence.windows[franchiseId] ?? []).some((window) => window.windowIndex === windowIndex && window.extraOfferSpent);
        ...
        if (wantExtra && !spentExtra && balance >= 1) {
            ...
            transactions.push(seasonTransactionEntry({
        ...
        const activeInjuries = health.injuries
            .filter((injury) => injury.franchiseId === franchiseId &&
            injury.sameGameReturned !== true &&
            injury.missedGamesRemaining > 0)
            .sort((x, y) => (x.injuryId < y.injuryId ? -1 : 1));
        if (activeInjuries.length > 0) {
            const pick = rankedBySeed(activeInjuries, (injury) => tradeSeed(rootSeed, 'window', String(windowIndex), 'ai-spend', franchiseId, 'rehab', injury.injuryId), (injury) => injury.injuryId)[0];
            const rehabSeed = tradeSeed(rootSeed, 'window', String(windowIndex), 'ai-spend', franchiseId, 'rehab');
...
function applyAiTrades(run: SeasonEconomyRun, rootSeed: string, windowIndex: number, catalog: SeasonDraftCatalog, humanFranchiseId: string, appliedAtStateRevision: number, protectedPlayers: ReadonlySet<string>): {
    run: SeasonEconomyRun;
    ...
    while (recorded < budget && attempted < AI_TRADE_ATTEMPT_BUDGET) {
        ...
        if (candidate !== null) {
            ...
            working = applied.run;
            offers.push(offer);
            usedPairs.add(candidate.a < candidate.b
...
export function openSeasonTradeWindow(input: SeasonOpenTradeWindowInput): SeasonWindowOpenResult | null {
    const { run, blockIndex, rootSeed, humanFranchiseId, catalog } = input;
    const windowIndex = WINDOW_BLOCK_INDEX_TO_INDEX[blockIndex];
    if (windowIndex === undefined || humanFranchiseId === null)
        return null;
    if (run.trade !== null &&
    ...
    if (catalog === undefined) {
        throw new SeasonTradeFactsError('openSeasonTradeWindow requires the packaged catalog (player positions + ratings); the block runner supplies it');
    }
    ...
    const spends = applyAiInfluenceSpends(economyRun, rootSeed, windowIndex, blockIndex, humanFranchiseId);
    ...
    const aiTrades = applyAiTrades(working, rootSeed, windowIndex, catalog, humanFranchiseId, appliedAtStateRevision, protectedPlayers);
...
export function applySeasonTrade(run: SeasonEconomyRun, offer: SeasonTradeOffer, catalog?: SeasonDraftCatalog, options: SeasonTradeApplicationOptions = {}): SeasonTradeApplicationResult {
    if (catalog === undefined) {
        throw new SeasonTradeFactsError('applySeasonTrade requires the packaged catalog (player positions + ratings); the command layer supplies it');
    }
    ...
    const rosterEntriesByFranchise = new Map(run.rosters.map((roster) => [roster.franchiseId, roster]));
    ...
    const ownershipByVersion = new Map(run.ownership.map((row) => [row.playerVersionId, row.ownerFranchiseId]));
    ...
    for (const id of moved) {
        ...
        if (ownershipByVersion.get(id) !== expectedOwner) {
            throw new SeasonTradeInvariantError(`ownership conflict: ${id} is owned by ${String(ownershipByVersion.get(id))}, offer expects ${expectedOwner}`);
        }
    }
    const toEntries = [
    ...
    if (legalityFailures.length > 0) {
        throw new SeasonTradeInvariantError(`traded rosters fail legality: ${legalityFailures.join('; ')}`);
    }
    const ownership = run.ownership.map((row) => moved.includes(row.playerVersionId)
        ? {
            ...row,
    ...
    if (activeMoved) {
        const nextRosters = run.rosters.map((roster) => roster.franchiseId === toFranchiseId
        ...
        effects = {
            ...effects,
            archivedPairs: effects.archivedPairs.filter((pair) => !movedSet.has(pair.a) && !movedSet.has(pair.b)),
        };
    }
    const health: SeasonHealthState = {
        ...run.health,
        injuries: run.health.injuries.map((injury) => moved.includes(injury.playerVersionId)
            ? {
                ...injury,
    ...
    if (trade !== null) {
        trade = {
            ...trade,
            windows: trade.windows.map((window) => window.windowIndex === offer.windowIndex
                ? {
                    ...window,
                    offers: window.offers.map((recorded) => recorded.offerId === offer.offerId
    ...
    const next: SeasonEconomyRun = {
        ...run,
        rosters: run.rosters.map((roster) => roster.franchiseId === toFranchiseId
            ? { ...roster, players: toEntries }
            : roster.franchiseId === fromFranchiseId
                ? { ...roster, players: fromEntries }
                : roster),
...
function windowBlockIndexOf(run: SeasonRun, windowIndex: number): number | null {
    return (run.trade?.windows.find((window) => window.windowIndex === windowIndex)?.blockIndex ?? null);
}
function repairRotationAfterTrade(oldRotation: SeasonRotation, facts: SeasonTradeCatalogFacts, newRosterIds: readonly string[], movedOut: readonly string[], movedIn: readonly string[]): SeasonRotation {
    ...
    const retained: SeasonRosterMemberInput[] = members.filter((member) => rotationIds.has(member.playerVersionId));
    ...
    if (rotationMembers.length !== 10) {
        throw new SeasonTradeInvariantError(`rotation repair for ${oldRotation.franchiseId} could not select ten members`);
    }
    const base = buildMinimalRotation({
        franchiseId: oldRotation.franchiseId,
        members: rotationMembers,
    });
    const minutesById = new Map(oldRotation.targetMinutes.map((entry) => [entry.playerVersionId, entry.minutes]));
    const pairing = pairIncomingToOutgoing(movedIn, movedOut, facts);
    const minutesByIncoming = new Map(pairing.map(([incomingId, outgoingId]) => [incomingId, outgoingId]));
    ...
    const targetMinutes = rotationMemberIds.map((playerVersionId) => {
        ...
        const minutes = inheritedFrom !== undefined
    ...
    for (const playerVersionId of rotationMemberIds) {
        const playable = facts.playable.get(playerVersionId);
        if (playable !== undefined)
            memberPlayable.set(playerVersionId, playable);
    }
    const failures = validateSeasonRotation(rotation, memberPlayable);
    if (failures.length > 0) {
        throw new SeasonTradeInvariantError(`rotation repair for ${oldRotation.franchiseId} failed: ${failures.join('; ')}`);
    }
    return rotation;
}
function pairIncomingToOutgoing(movedIn: readonly string[], movedOut: readonly string[], facts: SeasonTradeCatalogFacts): Array<[
    string,
    string
]> {
    const pairs: Array<[
    ...
    for (const incomingId of [...movedIn].sort()) {
        ...
        for (const outgoingId of movedOut) {
            if (used.has(outgoingId))
                continue;
            const overlap = slotGroupsOf(facts, outgoingId).filter((group) => incomingGroups.has(group)).length;
...
export function expireTradeOffersForBlock(trade: SeasonTradeState | null, blockIndex: number): SeasonTradeState | null {
    if (trade === null)
        return null;
    const windows: SeasonTradeWindowState[] = trade.windows.map((window) => {
        if (window.status === 'open' && window.blockIndex + 1 === blockIndex) {
            return {
                ...window,
                status: 'closed',
                offers: window.offers.map((offer) => offer.status === 'open' ? { ...offer, status: 'expired' as const } : offer),
            };
        }
        return window;
    });
