...
function applyAiInfluenceSpends(run: SeasonRun, rootSeed: string, windowIndex: number, blockIndex: number, humanFranchiseId: string): AiSpendResult {
    ...
    for (const franchiseId of ai) {
        ...
        const spentExtra = (influence.windows[fid] ?? []).some((window) => window.windowIndex === windowIndex && window.extraOfferSpent);
