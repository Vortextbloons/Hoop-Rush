...
describe('free-agency universe and canonical selection', () => {
  ...
  it('reuses canonical identity choices in later windows', () => {
    ...
    const reopened = openSeasonFreeAgencyWindow(
      { ...context, run: { ...run, freeAgency: opened.freeAgency } },
      1,
      4,
    );
    for (const candidate of opened.window.candidates) {
      const again = reopened.window.candidates.find(
        (entry) => entry.playerId === candidate.playerId,
      );
      if (again !== undefined) {
  ...
  it('names the free-agency seed paths under the frozen namespace', () => {
    const { run } = fixture();
    expect(freeAgencySeed(run.rootSeed, '0', 'canonical', 'p-x')).toBe(
      freeAgencySeed(run.rootSeed, '0', 'canonical', 'p-x'),
    );
    expect(freeAgencySeed(run.rootSeed, '0', 'canonical', 'p-x')).not.toBe(
      freeAgencySeed(run.rootSeed, '0', 'canonical', 'p-y'),
    );
    expect(freeAgencySeed(run.rootSeed, '0', 'canonical', 'p-x')).not.toBe(
      freeAgencySeed(run.rootSeed, '1', 'canonical', 'p-x'),
    );
  });
});
describe('free-agency declarations', () => {
  it('records an immutable declaration and rejects a second one', () => {
    ...
    if (candidate === undefined) throw new Error('no candidates');
  ...
  it('rejects unknown targets, duplicate priorities, and out-of-range influence', () => {
    ...
    expect(() =>
      applyFreeAgencyDeclaration(runWithWindow, 0, HUMAN, 'cmd-1', [
        {
    ...
    if (candidate === undefined) throw new Error('no candidates');
    expect(() =>
      applyFreeAgencyDeclaration(runWithWindow, 0, HUMAN, 'cmd-2', [
        { playerVersionId: candidate.playerVersionId, roleExpectation: 'rotation', influence: 4 },
      ]),
    ).toThrow(FreeAgencyValidationRejection);
    expect(() =>
      applyFreeAgencyDeclaration(runWithWindow, 0, HUMAN, 'cmd-3', [
        { playerVersionId: candidate.playerVersionId, roleExpectation: 'rotation', influence: 1 },
  ...
  it('rejects declarations on a window that is not open', () => {
    const { run } = fixture();
    expect(() => applyFreeAgencyDeclaration(run, 0, HUMAN, 'cmd-1', [])).toThrow(
      FreeAgencyValidationRejection,
    );
  });
});
describe('free-agency resolution', () => {
  it('resolves deterministically across repeated calls', () => {
    ...
    const declared = applyFreeAgencyDeclaration(
      ...
      HUMAN,
      'cmd-d',
      [{ playerVersionId: target.playerVersionId, roleExpectation: 'rotation', influence: 1 }],
    );
    const first = resolveSeasonFreeAgencyWindow(
      { ...context, run: { ...run, freeAgency: declared } },
      0,
      'cmd-r',
    );
    const second = resolveSeasonFreeAgencyWindow(
      { ...context, run: { ...run, freeAgency: declared } },
      0,
      'cmd-r',
    );
  ...
  it('applies a signing atomically: roster, ownership, ledger, transaction, caps', () => {
    ...
    expect(
      roster?.players.some((player) => player.playerVersionId === humanSigning.playerVersionId),
    ).toBe(true);
    expect(
      resolved.ownership.some((row) => row.playerVersionId === humanSigning.playerVersionId),
    ).toBe(true);
    const ledgerEntry = resolved.influence.ledger.find(
      (entry) => entry.entryId === humanSigning.ledgerEntryId,
    );
    expect(ledgerEntry).toBeDefined();
    expect(ledgerEntry?.source).toBe('free-agent-signing');
    expect(ledgerEntry?.appliedDelta).toBe(-humanSigning.influenceCost);
    const transaction = resolved.transactions.find(
      (entry) => entry.transactionId === humanSigning.transactionId,
    );
  ...
  it('rejects resolution before the human declares or skips', () => {
    ...
    const skipped = applyFreeAgencySkip(
      { ...run, freeAgency: opened.freeAgency },
      0,
      HUMAN,
      'cmd-s',
  ...
  it('keeps every signing within the season caps across all three windows', () => {
    ...
    for (const [windowIndex, blockIndex] of [
      ...
      const skipped = applyFreeAgencySkip(
        { ...currentRun, freeAgency: opened.freeAgency },
        windowIndex,
        HUMAN,
        `cmd-skip-${String(windowIndex)}`,
      );
      const resolved = resolveSeasonFreeAgencyWindow(
        { ...context, run: { ...currentRun, freeAgency: skipped } },
        windowIndex,
        `cmd-resolve-${String(windowIndex)}`,
      );
...
describe('effects reconciliation (season-chemistry-v2)', () => {
  ...
  it('freezes a demoted player load into the inactive set', () => {
    ...
    const inactiveCandidate = catalog.candidates.find(
      (candidate) => candidate.playerVersionId === inactiveId,
    );
    ...
    const rosters = run.rosters.map((roster) =>
      roster.franchiseId === HUMAN ? expandedRoster : roster,
    );
    const demoted = humanRotation.starters[0];
    if (demoted === undefined) throw new Error('fixture');
    const nextRotations = run.rotations.map((rotation) =>
      rotation.franchiseId === HUMAN
        ? {
            ...rotation,
    ...
    const touched: SeasonEffectsState = {
      ...
      playerStates: base.playerStates.map((player) =>
        player.playerVersionId === demoted ? { ...player, fatigueBasisPoints: 4000 } : player,
      ),
    };
    const reconciled = reconcileSeasonEffects({
      previous: touched,
    ...
    expect(reconciled.playerStates).toHaveLength(300);
    expect(reconciled.pairStates).toHaveLength(1350);
    const frozen = reconciled.inactivePlayerStates.find(
      (player) => player.playerVersionId === demoted,
    );
  ...
  it('creates zero pair state for a newly promoted signing', () => {
    ...
    const members = ids.map((id) => {
      const catalogCandidate = catalog.candidates.find(
        (candidate) => candidate.playerVersionId === id,
      );
      return {
        playerVersionId: id,
    ...
    const nextRotations = run.rotations.map((rotation) =>
      rotation.franchiseId === HUMAN ? repairedRotation : rotation,
    );
    const reconciled = reconcileSeasonEffects({
    ...
    const newPairs = reconciled.pairStates.filter(
      (pair) => pair.a === target.playerVersionId || pair.b === target.playerVersionId,
    );
