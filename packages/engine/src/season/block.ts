import { SEASON_ENDING_MISSED_GAMES_SENTINEL, SEASON_BLOCK_COUNT, SEASON_FREE_AGENCY_VERSION, SEASON_ROSTER_MAX_SIZE, SEASON_ROSTER_MIN_SIZE, SEASON_ROUND_COUNT, SEASON_SEED_NAMESPACES, SEASON_TEAM_COUNT, blockRoundRange, franchiseIdSchema, seasonGameIdSchema, seasonNamespaceSeed, type EraSimulationProfile, type FranchiseId, type Position, type SeasonBlockRunContext, type SeasonCandidateCheckpoint, type SeasonCheckpointState, type SeasonCheckpointVersions, type SeasonCompactInjuryEvent, type SeasonDraftCatalog, type SeasonEffectsState, type SeasonFreeAgencyIndex, type SeasonFreeAgencyState, type SeasonFreeAgencyWindowState, type SeasonGamePlayerInput, type SeasonGameSimulationInput, type SeasonGameSummary, type SeasonHealthState, type SeasonInfluenceState, type SeasonInjuryRecord, type SeasonInvalidRosterInterruption, type SeasonObjectiveId, type SeasonObjectiveState, type SeasonCampaignState, type SeasonPendingBlockCandidate, type SeasonRemovalEvent, type SeasonResumeSeasonBlockCommand, type SeasonResumeSeasonBlockResult, type SeasonRetainedGameDetail, type SeasonReturnEvent, type SeasonRoster, type SeasonRotation, type SeasonRun, type SeasonSchedule, type SeasonScheduleGame, type SeasonSubmitBlockCommand, type SeasonSubmitBlockRejection, type SeasonSubmitBlockResult, type SeasonTransactionEntry, type SeasonRosterTargets, } from '@hoop-rush/data-contracts';
import { createEngineContext } from '../sim/context.ts';
import { auditSeasonAggregates, foldSeasonPlayerAggregates, foldSeasonTeamAggregates, } from './aggregates.ts';
import { reconstructSeasonGames, seasonCheckpointDigest } from './checkpoint.ts';
import { auditSeasonGameSummary, seasonGameSummaryFromResult, seasonRetainedDetailFromResult, } from './game-summary.ts';
...
import { seasonFranchiseLegalFiveFacts, seasonGameHealthSeam, seasonPregameAvailabilityOf, } from './health.ts';
...
import { SEASON_CAMPAIGN_TARGETS_VERSION, SEASON_CAMPAIGN_VERSION, evaluateSeasonCampaignOpportunity, generateSeasonCampaignOffers, normalizeCampaignState, } from './campaign.ts';
...
export function seasonRunStateDigestFactsOf(next: SeasonRun, effects: SeasonEffectsState): Parameters<typeof seasonRunStateDigest>[0] {
    return {
...
export function expandSeasonRunRosters(run: SeasonBlockRunContext, catalog: SeasonDraftCatalog): Map<string, SeasonGamePlayerInput> {
    ...
    for (const roster of run.rosters) {
        for (const player of roster.players) {
            ...
            if (candidate === undefined) {
                throw new Error(`roster ${roster.franchiseId} references unknown catalog version ${playerVersionId}`);
            }
    ...
    if (expanded.size < minExpanded || expanded.size > maxExpanded) {
        throw new Error(`expanded rosters must own between ${String(minExpanded)} and ${String(maxExpanded)} distinct versions (got ${String(expanded.size)})`);
    }
    return expanded;
}
...
export function seasonBlockRejection(input: SeasonBlockSimulationInput, acceptedCommandIds?: readonly string[]): SeasonSubmitBlockRejection | null {
    ...
    if (input.objectives !== undefined) {
        if (command.blockIndex <= 7) {
            ...
            const selectedAndOffered = selection !== undefined &&
    ...
    if (input.campaignState !== undefined) {
        ...
        if (command.blockIndex <= 7) {
            const expected = (command as unknown as {
                campaignOpportunityId?: string | null;
    ...
    const preBlockFreeAgency = input.freeAgency ?? (input.run as Partial<SeasonRun>).freeAgency ?? emptyFreeAgencyStateOf(run);
    ...
    for (const rotation of run.rotations) {
        ...
        for (const player of roster?.players ?? []) {
            if (!activePlayerIds.has(player.playerVersionId))
                continue;
...
function requireValidSeasonBlockCommand(input: SeasonBlockSimulationInput): void {
    ...
    if (rejection !== null)
        throw new SeasonBlockValidationError(rejection);
}
export function simulateSeasonBlock(input: SeasonBlockSimulationInput, options: SeasonBlockSimulationOptions = {}): SeasonCandidateCheckpoint {
    ...
    for (const game of seasonBlockGamesOf(input.schedule, input.command.blockIndex)) {
        ...
        if ('interruption' in outcome) {
            throw new SeasonBlockInvariantError(`game ${game.gameId} interrupted: the human franchise ${outcome.interruption.humanFranchiseId} cannot field a legal five from health availability`, { seed: input.command.runId, gameId: game.gameId, blockIndex: input.command.blockIndex });
        }
...
export function seasonBlockGamesOf(schedule: SeasonSchedule, blockIndex: number): SeasonScheduleGame[] {
    ...
    if (blockGames.length !== expectedGameCount) {
        throw new SeasonBlockInvariantError(`block ${String(blockIndex)} holds ${String(blockGames.length)} games, expected ${String(expectedGameCount)}`, { blockIndex });
    }
    return blockGames;
}
...
function isSimulateSeasonBlockGameArgs(value: SimulateSeasonBlockGameArgs | SeasonBlockSimulationInput): value is SimulateSeasonBlockGameArgs {
...
export function simulateSeasonBlockGame(input: SeasonBlockSimulationInput, game: SeasonScheduleGame, effects: SeasonEffectsState, health: SeasonHealthState, options?: SeasonBlockGameSimulationOptions): SeasonBlockGameOutcome;
export function simulateSeasonBlockGame(argsOrInput: SimulateSeasonBlockGameArgs | SeasonBlockSimulationInput, positionalGame?: SeasonScheduleGame, positionalEffects?: SeasonEffectsState, positionalHealth?: SeasonHealthState, positionalOptions: SeasonBlockGameSimulationOptions = {}): SeasonBlockGameOutcome {
    ...
    if (positionalGame !== undefined &&
        positionalEffects !== undefined &&
        positionalHealth !== undefined) {
        if (isSimulateSeasonBlockGameArgs(argsOrInput)) {
            throw new SeasonBlockInvariantError('simulateSeasonBlockGame: ambiguous call shape (object and positional args)');
        }
        input = argsOrInput;
        game = positionalGame;
    ...
    const command = input.command;
    const run = input.run;
    const gameNumberById = options.gameNumberById ??
        new Map(input.schedule.games.map((game, index) => [game.gameId, index + 1]));
    const rotationByFranchise = options.rotationByFranchise ??
        new Map(run.rotations.map((rotation) => [rotation.franchiseId, rotation]));
    const rosterByFranchise = options.rosterByFranchise ?? new Map(run.rosters.map((roster) => [roster.franchiseId, roster]));
    ...
    if (homeRoster === undefined ||
        ...
        throw new SeasonBlockInvariantError(`game ${game.gameId} references a roster or rotation outside the run`, { gameId: game.gameId, blockIndex: command.blockIndex });
    }
    ...
    const participantFranchiseIds = input.participantFranchiseIds ?? (input.humanFranchiseId ? [input.humanFranchiseId] : []);
    const humanPlays = participantFranchiseIds.some((id) => game.homeFranchiseId === id || game.awayFranchiseId === id);
    let homeLegalFacts: {
        legal: boolean;
    ...
    if (humanPlays) {
        for (const pid of participantFranchiseIds) {
            if (game.homeFranchiseId !== pid && game.awayFranchiseId !== pid)
                continue;
            const facts = seasonFranchiseLegalFiveFacts(run, pid, health, positions);
            if (pid === game.homeFranchiseId)
                homeLegalFacts = facts;
            if (pid === game.awayFranchiseId)
    ...
    const { result, transition } = simulateSeasonGameWithEffects(gameInput, createEngineContext(), pregame);
    if (result.seed !== seed) {
        throw new SeasonBlockInvariantError(`game ${game.gameId} result seed ${result.seed} does not match the derived seed ${seed}`, { seed, gameId: game.gameId, blockIndex: command.blockIndex });
    }
    if (result.outcome === 'no-legal-five-both') {
        throw new SeasonBlockInvariantError(`game ${game.gameId} has no legal five on either side (seed ${seed}); refusing to fabricate a winner`, { seed, gameId: game.gameId, blockIndex: command.blockIndex });
    }
...
function staminaByVersionOf(input: SeasonBlockSimulationInput): Map<string, number> {
    ...
    for (const player of input.expanded.values()) {
        if (player.stamina === undefined) {
            throw new SeasonBlockInvariantError(`expanded player ${player.playerVersionId} has no stamina profile`);
        }
        ratings.set(player.playerVersionId, player.stamina.rating);
    }
    return ratings;
}
function targetMinutesOf(input: SeasonBlockSimulationInput, game: SeasonScheduleGame, rotationByFranchise: ReadonlyMap<string, SeasonRotation>): Map<string, number> {
    ...
    for (const franchiseId of [game.homeFranchiseId, game.awayFranchiseId]) {
        ...
        if (rotation === undefined) {
            throw new SeasonBlockInvariantError(`game ${game.gameId} references rotation ${franchiseId}`, { gameId: game.gameId, blockIndex: input.command.blockIndex });
        }
...
function sideOfPlayer(game: SeasonScheduleGame, playerVersionId: string, rosterByFranchise: ReadonlyMap<string, SeasonRoster>): 'home' | 'away' {
    const homeRoster = rosterByFranchise.get(game.homeFranchiseId);
...
function sameGameReturnResolutionsOf(newInjuries: readonly SeasonInjuryRecord[], result: ReturnType<typeof simulateSeasonGameWithEffects>['result']): {
    injuryId: string;
    returned: boolean;
}[] {
...
function compactInjuryEventsOf(game: SeasonScheduleGame, rosterByFranchise: ReadonlyMap<string, SeasonRoster>, seam: ReturnType<typeof seasonGameHealthSeam>, result: ReturnType<typeof simulateSeasonGameWithEffects>['result']): SeasonCompactInjuryEvent[] {
    if (seam.newInjuries.length === 0)
        return [];
    ...
    const rolledRemovalByPlayer = new Map(seam.removals.map((removal) => [removal.playerVersionId, removal.clock]));
    return seam.newInjuries.map((record) => {
        ...
        const removedClock = appliedRemoval !== undefined
        ...
        return {
            ...
            returnClock: appliedReturn !== undefined
...
function isAssembleSeasonBlockCandidateArgs(value: AssembleSeasonBlockCandidateArgs | SeasonBlockSimulationInput): value is AssembleSeasonBlockCandidateArgs {
    return ('summaries' in value && 'retainedDetails' in value && 'effects' in value && 'health' in value);
}
export function assembleSeasonBlockCandidate(args: AssembleSeasonBlockCandidateArgs): SeasonCandidateCheckpoint;
export function assembleSeasonBlockCandidate(input: SeasonBlockSimulationInput, summaries: readonly SeasonGameSummary[], retainedDetails: readonly SeasonRetainedGameDetail[], effects: SeasonEffectsState, health: SeasonHealthState): SeasonCandidateCheckpoint;
export function assembleSeasonBlockCandidate(argsOrInput: AssembleSeasonBlockCandidateArgs | SeasonBlockSimulationInput, positionalSummaries?: readonly SeasonGameSummary[], positionalRetainedDetails?: readonly SeasonRetainedGameDetail[], positionalEffects?: SeasonEffectsState, positionalHealth?: SeasonHealthState): SeasonCandidateCheckpoint {
    let input: SeasonBlockSimulationInput;
    let summaries: readonly SeasonGameSummary[];
    let retainedDetails: readonly SeasonRetainedGameDetail[];
    let effects: SeasonEffectsState;
    let health: SeasonHealthState;
    if (positionalSummaries !== undefined &&
        positionalRetainedDetails !== undefined &&
        positionalEffects !== undefined &&
        positionalHealth !== undefined) {
        if (isAssembleSeasonBlockCandidateArgs(argsOrInput)) {
            throw new SeasonBlockInvariantError('assembleSeasonBlockCandidate: ambiguous call shape (object and positional args)');
        }
        input = argsOrInput;
        ...
        if (!isAssembleSeasonBlockCandidateArgs(argsOrInput)) {
            throw new SeasonBlockInvariantError('assembleSeasonBlockCandidate: missing summaries/retainedDetails/effects/health');
        }
        input = argsOrInput.input;
    ...
    const standingsBefore = reduceSeasonStandings(run.league, reconstructSeasonGames(input.schedule, input.priorSummaries));
    const standings = reduceSeasonStandings(run.league, reconstructSeasonGames(input.schedule, allSummaries));
    ...
    if (input.campaignState !== undefined) {
        ...
        const primaryOppId = input.campaignOpportunityId ??
            input.campaignOpportunityIds?.get(primaryFranchiseId ?? '') ??
            null;
        if (primaryOppId !== null) {
            ...
            if (opportunity) {
                ...
                if (primaryFranchiseId)
                    campaignEvaluations[primaryFranchiseId] = evalResult;
            }
        }
        for (const pid of participantIds) {
            if (pid === primaryFranchiseId)
                continue;
    ...
    const franchiseIds = run.league.teams.map((team) => team.franchiseId);
    const preBlockInfluence = input.influence ??
    ...
    if (aggregateFailures.length > 0 || recapFailures.length > 0) {
        throw new SeasonBlockInvariantError(`block ${String(command.blockIndex)} audit failed: ${[...aggregateFailures, ...recapFailures].join('; ')}`, { blockIndex: command.blockIndex });
    }
    const versions: SeasonCheckpointVersions = {
        ...
        campaignVersion: (run.versions as unknown as {
            campaignVersion?: typeof SEASON_CAMPAIGN_VERSION;
        }).campaignVersion ?? SEASON_CAMPAIGN_VERSION,
        campaignTargetsVersion: (run.versions as unknown as {
...
function expandedPlayer(input: SeasonBlockSimulationInput, gameId: string, playerVersionId: string): SeasonGamePlayerInput {
    const player = input.expanded.get(playerVersionId);
    if (player === undefined) {
        throw new SeasonBlockInvariantError(`game ${gameId} references an unexpanded version ${playerVersionId}`, { gameId });
    }
    return player;
}
export function handleSubmitSeasonBlockCommand(input: SeasonSubmitBlockCommandInput): SeasonSubmitBlockResult {
...
function objectivesWithBlockSuccess(objectives: SeasonObjectiveState, candidate: SeasonCandidateCheckpoint): SeasonObjectiveState {
    if (candidate.blockIndex === 8)
        return objectives;
...
function campaignWithBlockEvaluation(campaignState: import('@hoop-rush/data-contracts').SeasonCampaignState | undefined, candidate: SeasonCandidateCheckpoint): import('@hoop-rush/data-contracts').SeasonCampaignState | undefined {
    ...
    const branchId = (candidate.campaign.evaluation as unknown as {
        branchId?: string;
    ...
    if (bId) {
        if (evalResult.outcome === 'missed')
            nextBranchState[bId] = 'missed';
        else if (evalResult.outcome === 'completed')
            nextBranchState[bId] = 'open';
        else
            nextBranchState[bId] = 'completed';
    }
    ...
    for (const rid of evalResult.appliedRewardIds) {
        if (!applied.includes(rid)) {
            ...
            const reward = offerForReward?.completedReward.rewardId === rid
            ...
            if (reward) {
                ...
                else if (reward.type === 'trade-inquiry-credit')
                    inquiryCredits += reward.amount;
                else if (reward.type === 'trade-board-information')
                    informationBenefits += reward.amount;
                else
                    followUpUnlocks.push(rid);
            }
        }
    }
    return {
...
export function deriveSeasonPostBlockState(input: {
    ...
    const campaign = campaignWithBlockEvaluation((input.run as {
        campaign?: import('@hoop-rush/data-contracts').SeasonCampaignState;
    }).campaign, input.candidate);
...
export function completeSeasonBlockCommit(input: {
    ...
    if (nextBlockIdxForCampaign <= 7 && postBlockRun.campaign?.startingIdentity) {
        ...
        if (!nextCampaignState.offers[nextBlockIdxForCampaign]) {
            ...
            if (!needsEvolution) {
                const scheduleForCampaign = (input as unknown as {
    ...
    if (freeAgencyBlockIndexes.includes(input.candidate.blockIndex) &&
        freeAgencyUnresolvedWindowIndex(freeAgency) === null) {
        const freeAgencyIndex = input.freeAgencyIndex;
        if (freeAgencyIndex === undefined) {
            throw new SeasonBlockInvariantError(`block ${String(input.candidate.blockIndex)} needs the packaged free-agency index to open its market window`, { blockIndex: input.candidate.blockIndex });
        }
        const freeAgencyCatalog = input.catalog;
        if (freeAgencyCatalog === undefined) {
            throw new SeasonBlockInvariantError(`block ${String(input.candidate.blockIndex)} needs the packaged catalog to open its market window`, { blockIndex: input.candidate.blockIndex });
        }
...
export function auditSeasonBlock(candidate: SeasonCandidateCheckpoint, input: SeasonBlockSimulationInput): string[] {
    const failures: string[] = [];
    const command = input.command;
    const run = input.run;
    if (candidate.runId !== run.runId)
        failures.push('candidate runId does not match the run');
    ...
    if (candidate.revision !== command.expectedRevision) {
        failures.push(`candidate revision ${String(candidate.revision)} does not match expectedRevision ${String(command.expectedRevision)}`);
    }
    ...
    if (candidate.completedRounds !== toRound) {
        failures.push(`candidate completedRounds ${String(candidate.completedRounds)} must be the block end ${String(toRound)} (cursor advanced)`);
    }
    ...
    if (candidate.gameSummaries.length !== expectedCount) {
        failures.push(`candidate must carry exactly ${String(expectedCount)} summaries (got ${String(candidate.gameSummaries.length)})`);
    }
    ...
    const blockGameIds = new Set(input.schedule.games
        .filter((game) => game.round >= fromRound && game.round <= toRound)
    ...
    for (const summary of candidate.gameSummaries) {
        failures.push(...auditSeasonGameSummary(summary).map((failure) => `summary ${summary.gameId}: ${failure}`));
    }
    if (candidate.playerAggregates.length > SEASON_TEAM_COUNT * 10) {
        failures.push(`candidate must carry at most 300 player aggregates (got ${String(candidate.playerAggregates.length)})`);
    }
    ...
    if (candidate.teamAggregates.length !== SEASON_TEAM_COUNT) {
        failures.push(`candidate must carry 30 team aggregates (got ${String(candidate.teamAggregates.length)})`);
    }
    const allSummaries = [...input.priorSummaries, ...candidate.gameSummaries];
    failures.push(...auditSeasonStandings(run.league, reconstructSeasonGames(input.schedule, allSummaries), candidate.standings));
    failures.push(...auditSeasonAggregates({
    ...
    if (input.humanFranchiseId !== null) {
        for (const detail of candidate.retainedDetails) {
            if (detail.homeFranchiseId !== input.humanFranchiseId &&
    ...
    failures.push(...auditSeasonBlockRecap(candidate.recap, {
        runId: run.runId,
        blockIndex: command.blockIndex,
        completedRounds: candidate.completedRounds,
        humanFranchiseId: input.humanFranchiseId,
        summaries: allSummaries,
        standingsBefore: reduceSeasonStandings(run.league, reconstructSeasonGames(input.schedule, input.priorSummaries)),
    ...
    if (effectsVersions.size !== 300) {
        failures.push(`effects state must carry 300 distinct players (got ${String(effectsVersions.size)})`);
    }
    const expandedVersions = new Set(input.expanded.keys());
    if (effectsVersions.size === expandedVersions.size &&
    ...
    for (const player of candidate.effects.playerStates) {
        if (player.lastCompletedRound > candidate.completedRounds) {
            failures.push(`effects ${player.playerVersionId} lastCompletedRound ${String(player.lastCompletedRound)} exceeds ${String(candidate.completedRounds)}`);
        }
    }
    ...
    for (const record of candidate.health.injuries) {
        if (!expandedVersionsSet.has(record.playerVersionId)) {
            failures.push(`injury ${record.injuryId} references an unrostered version ${record.playerVersionId}`);
        }
        ...
        if (record.actualReturnRound !== null && record.actualReturnRound > candidate.completedRounds) {
            failures.push(`injury ${record.injuryId} actualReturnRound ${String(record.actualReturnRound)} exceeds ${String(candidate.completedRounds)}`);
        }
        ...
        if (record.seasonEnding &&
            ...
            failures.push(`season-ending injury ${record.injuryId} must carry the missed-games sentinel ${String(SEASON_ENDING_MISSED_GAMES_SENTINEL)}`);
        }
    ...
    if (candidate.objective.objectiveId !== command.objectiveId) {
        failures.push(`candidate objective ${String(candidate.objective.objectiveId)} does not match the command ${String(command.objectiveId)}`);
    }
    ...
    if (candidate.stateRevision !== 0 || candidate.stateDigest !== '0'.repeat(32)) {
        failures.push('candidate post-block state facts must be the assembly placeholders (the commit side derives them)');
    }
