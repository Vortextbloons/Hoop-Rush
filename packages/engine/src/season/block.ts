...
import { SEASON_CAMPAIGN_TARGETS_VERSION, SEASON_CAMPAIGN_VERSION, buildEmptyCampaignState, evaluateSeasonCampaignOpportunity, generateSeasonCampaignOffers, normalizeCampaignState, } from './campaign.ts';
...
function campaignWithBlockEvaluation(campaignState: import('@hoop-rush/data-contracts').SeasonCampaignState | undefined, candidate: SeasonCandidateCheckpoint): import('@hoop-rush/data-contracts').SeasonCampaignState | undefined {
    if (!candidate.campaign || !candidate.campaign.evaluation)
        return campaignState as unknown as import('@hoop-rush/data-contracts').SeasonCampaignState | undefined;
    ...
    if (bId) {
        ...
        else if (evalResult.outcome === 'breakthrough')
        ...
        if (!applied.includes(rid)) {
            ...
            if (reward) {
                if (reward.type === 'influence') {
                    ...
                    const appliedDelta = Math.min(requested, 8 -
                ...
                else if (reward.type === 'follow-up-unlock')
...
export function deriveSeasonPostBlockState(input: {
    ...
    }).campaign as unknown as import('@hoop-rush/data-contracts').SeasonCampaignState | undefined, input.candidate);
