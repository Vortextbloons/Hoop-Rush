...
export function assembleSeasonBlockCandidate(
  ...
  const campaignEvaluations: Record<
    string,
    ...
    if (!nextCampaignState.offers[nextBlockIdxForCampaign]) {
      ...
      if (!needsEvolution) {
        try {
          ...
          console.warn(
            `campaign generation failed for block ${String(nextBlockIdxForCampaign)}`,
            error,
          );
          throw error;
        }
    ...
    const opened = openSeasonFreeAgencyWindow(
      {
        ...
        humanFranchiseId: primaryFranchiseId,
        participantFranchiseIds: participantIds.length > 1 ? participantIds : undefined,
      } as unknown as Parameters<typeof openSeasonFreeAgencyWindow>[0],
      windowIndex,
