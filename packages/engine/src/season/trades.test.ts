...
import type { Position, SeasonDraftCatalog, SeasonEffectsState, SeasonRun, SeasonTradeOffer, SeasonTradeState, } from '@hoop-rush/data-contracts';
...
import { applySeasonTrade, expireTradeOffersForBlock, generatedExtraOfferForSpend, openSeasonTradeWindow, ratioMutuallyWithinBand, seasonTradePlayerValue, seasonTradeCatalogFactsOf, SeasonTradeFactsError, SeasonTradeInvariantError, type SeasonWindowOpenResult, } from './trades.ts';
import { aiTradeCountOf, buildEconomyTestRun, injuryIdOf, withInjury, zeroEffectsOf, } from './season-economy-test-support.ts';
...
function openWindow(run: SeasonRun, catalog: SeasonDraftCatalog, blockIndex: number): SeasonWindowOpenResult {
    ...
    if (result === null)
        throw new Error(`window did not open for block ${String(blockIndex)}`);
    return result;
}
function applyWindowResult(run: SeasonRun, result: SeasonWindowOpenResult): SeasonRun & {
...
function seasonWindows(seed: string): {
    run: SeasonRun;
    results: SeasonWindowOpenResult[];
} {
    const { run: base, catalog } = fixture(seed);
    let run: SeasonRun & {
        effects: SeasonEffectsState;
    } = { ...base, effects: zeroEffectsOf(base) };
...
describe('season trade window opening', () => {
    ...
    it('creates three open base offers for the human franchise with 1-for-1 or 2-for-2 shapes', () => {
        ...
        const baseOffers = (window?.offers ?? []).filter((offer) => offer.toFranchiseId === HUMAN && offer.status === 'open');
        ...
        for (const result of results) {
            for (const window of result.trade.windows) {
                for (const offer of window.offers) {
                    ...
                    const inBand = offer.valueBand.ratioBasisPoints >= (bounds[0] ?? 0) &&
    ...
    it('records health facts for every moved player from the run health state', () => {
        ...
        const offersMentioning = window.offers.filter((offer) => [...offer.outgoingPlayerVersionIds, ...offer.incomingPlayerVersionIds].includes(injuredVersion));
    ...
    it('records AI-to-AI activity as accepted offers that never involve the human roster', () => {
        const { results } = sharedSeason();
        const aiOffers = results.flatMap((result) => result.trade.windows.flatMap((window) => window.offers.filter((offer) => offer.status === 'accepted')));
        for (const offer of aiOffers) {
            ...
            const kind = offer.outgoingPlayerVersionIds.length === 1 && offer.incomingPlayerVersionIds.length === 1
                ? ('1-1' as const)
    ...
    it('records AI influence spends with synthetic command ids and window tracking', () => {
        ...
        const spendTransactions = result.transactions.filter((entry) => entry.type === 'influence-spend');
        ...
        const spendLedger = result.influence.ledger.filter((entry) => entry.source === 'extra-trade-offer');
        ...
        for (const entry of spendLedger) {
            ...
            const priorDeltas = result.influence.ledger
                .filter((prior) => prior.franchiseId === entry.franchiseId && prior.entryId <= entry.entryId)
            ...
            expect(windows?.some((window) => window.windowIndex === 0 && window.extraOfferSpent)).toBe(true);
        }
        for (const franchiseId of Object.keys(result.influence.balances)) {
            expect(result.influence.balances[franchiseIdSchema.parse(franchiseId)] ?? 0).toBeGreaterThanOrEqual(0);
        }
    });
...
describe('season trade invariants (property over seeds)', () => {
    it('keeps ownership unique, rosters legal, rotations legal, and effects canonical', () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 64 }), (seedIndex) => {
            ...
            for (const blockIndex of [2, 4, 5]) {
                ...
                for (const row of run.ownership) {
                    expect(run.rosters.find((roster) => roster.franchiseId === row.ownerFranchiseId)).toBeDefined();
                }
...
describe('season trade chemistry zero-state', () => {
    it('resets the pairs of traded players to zero shared possessions', () => {
        ...
        if (moved.size === 0)
            throw new Error('expected AI activity in this seed');
        const pairsByKey = new Map(result.effects.pairStates.map((pair) => [`${pair.a}\u0000${pair.b}`, pair]));
...
describe('season applySeasonTrade', () => {
    it('applies a 1-for-1 trade atomically and returns both roster changes', () => {
        ...
        if (humanRoster === undefined || aiRoster === undefined)
        ...
        expect(next.ownership.find((row) => row.playerVersionId === outgoingId)?.ownerFranchiseId).toBe('celtics');
        expect(next.ownership.find((row) => row.playerVersionId === incomingId)?.ownerFranchiseId).toBe(HUMAN);
        ...
        for (const count of ownedCount.values())
            expect(count).toBe(1);
        ...
        const incomingMinutes = humanRotation?.targetMinutes.find((entry) => entry.playerVersionId === incomingId);
        ...
        expect(next.effects.playerStates.find((player) => player.playerVersionId === incomingId)).toBeDefined();
        ...
        const payload = tradeEntry?.payload as {
        ...
        const incomingAfter = humanAfter?.players.find((player) => player.playerVersionId === incomingId);
    ...
    it('moves injury records with their players', () => {
        ...
        if (movedId === undefined)
            throw new Error('no celtics player');
        ...
        if (outgoingId === undefined)
            throw new Error('no lakers player');
        const offer: SeasonTradeOffer = {
    ...
    it('throws typed invariants for ownership conflicts and unknown players', () => {
        ...
        expect(() => applySeasonTrade({ ...run, effects }, { ...baseOffer, incomingPlayerVersionIds: [wrongRosterId] }, catalog)).toThrow(SeasonTradeInvariantError);
        expect(() => applySeasonTrade({ ...run, effects }, {
            ...baseOffer,
            toFranchiseId: franchiseIdSchema.parse('celtics'),
            fromFranchiseId: franchiseIdSchema.parse('celtics'),
        }, catalog)).toThrow(SeasonTradeInvariantError);
...
describe('season trade deadlines', () => {
    ...
    it('closes window 0 at block 3, window 1 at block 5, window 2 at block 6', () => {
        ...
        const openOfferCount = () => trade.windows[0]?.offers.filter((offer) => offer.status === 'open').length ?? 0;
        ...
        const expiredAfter = closed?.windows[0]?.offers.filter((offer) => offer.status === 'expired') ?? [];
    ...
    it('opens all three windows with their own block indices and close order', () => {
        ...
        const full = trades[2];
        if (full === undefined)
            throw new Error('no third window');
...
describe('season generated extra offer', () => {
    it('generates a deterministic fourth offer distinct from the base three', () => {
        const { run, catalog } = fixture('a1b2c3d4e5f60718293a4b5c6d7e8f9a');
        const result = openWindow(run, catalog, 2);
        const baseOfferIds = new Set(result.trade.windows[0]?.offers.map((offer) => offer.offerId) ?? []);
        const extra1 = generatedExtraOfferForSpend(run.rootSeed, { ...run, effects: result.effects }, 0, HUMAN, catalog);
        const extra2 = generatedExtraOfferForSpend(run.rootSeed, { ...run, effects: result.effects }, 0, HUMAN, catalog);
...
describe('season contextual player value', () => {
    it('is bounded, availability- and workload-sensitive, and never reads Overall', () => {
        ...
        const effects: SeasonEffectsState = {
            ...run.effects,
            playerStates: run.effects.playerStates.map((player) => player.playerVersionId === version ? { ...player, recentLoadBasisPoints: 10000 } : player),
        };
...
describe('season AI risky-rehab spends (health seam stubbed)', () => {
    it('records seeded rehabs in ledgers, transactions, and the health state', () => {
        ...
        for (const [injuryId, state] of Object.entries(result.influence.rehabs)) {
            ...
            expect(result.health.injuries.find((injury) => injury.injuryId === injuryId)?.franchiseId).toBe(state.franchiseId);
        }
