...
export function auditSeasonStandings(
  ...
  for (const row of standings.rows) {
    ...
    if (row.wins !== want.wins)
      failures.push(
        `${row.franchiseId} wins: expected ${String(want.wins)} got ${String(row.wins)}`,
      );
    if (row.losses !== want.losses)
      failures.push(
        `${row.franchiseId} losses: expected ${String(want.losses)} got ${String(row.losses)}`,
      );
    if (row.gamesPlayed !== want.gamesPlayed) {
      failures.push(
        `${row.franchiseId} gamesPlayed: expected ${String(want.gamesPlayed)} got ${String(row.gamesPlayed)}`,
      );
    }
    if (row.homeWins !== want.homeWins)
      failures.push(
        `${row.franchiseId} homeWins: expected ${String(want.homeWins)} got ${String(row.homeWins)}`,
      );
    if (row.homeLosses !== want.homeLosses)
      failures.push(
        `${row.franchiseId} homeLosses: expected ${String(want.homeLosses)} got ${String(row.homeLosses)}`,
      );
    if (row.awayWins !== want.awayWins)
      failures.push(
        `${row.franchiseId} awayWins: expected ${String(want.awayWins)} got ${String(row.awayWins)}`,
      );
    if (row.awayLosses !== want.awayLosses)
      failures.push(
        `${row.franchiseId} awayLosses: expected ${String(want.awayLosses)} got ${String(row.awayLosses)}`,
      );
    if (row.conferenceWins !== want.conferenceWins)
      failures.push(
        `${row.franchiseId} conferenceWins: expected ${String(want.conferenceWins)} got ${String(row.conferenceWins)}`,
      );
    if (row.conferenceLosses !== want.conferenceLosses)
      failures.push(
        `${row.franchiseId} conferenceLosses: expected ${String(want.conferenceLosses)} got ${String(row.conferenceLosses)}`,
      );
    if (row.divisionWins !== want.divisionWins)
      failures.push(
        `${row.franchiseId} divisionWins: expected ${String(want.divisionWins)} got ${String(row.divisionWins)}`,
      );
    if (row.divisionLosses !== want.divisionLosses)
      failures.push(
        `${row.franchiseId} divisionLosses: expected ${String(want.divisionLosses)} got ${String(row.divisionLosses)}`,
      );
    if (row.pointsFor !== want.pointsFor)
      failures.push(
        `${row.franchiseId} pointsFor: expected ${String(want.pointsFor)} got ${String(row.pointsFor)}`,
      );
    if (row.pointsAgainst !== want.pointsAgainst) {
      failures.push(
        `${row.franchiseId} pointsAgainst: expected ${String(want.pointsAgainst)} got ${String(row.pointsAgainst)}`,
      );
    }
    for (const wantH2h of want.headToHead) {
      ...
      if (gotH2h.wins !== wantH2h.wins) {
        failures.push(
          `${row.franchiseId} head-to-head vs ${wantH2h.franchiseId} wins: expected ${String(wantH2h.wins)} got ${String(gotH2h.wins)}`,
        );
      }
      if (gotH2h.losses !== wantH2h.losses) {
        failures.push(
          `${row.franchiseId} head-to-head vs ${wantH2h.franchiseId} losses: expected ${String(wantH2h.losses)} got ${String(gotH2h.losses)}`,
        );
      }
    }
    ...
    if (
      row.wins !== row.homeWins + row.awayWins ||
  ...
  for (const row of standings.rows) gamesPlayed.set(row.franchiseId, 0);
  for (const game of games) {
    if (game.status === 'scheduled') continue;
  ...
  if (totalWins !== totalLosses) {
    failures.push(
      `league wins (${String(totalWins)}) must equal league losses (${String(totalLosses)})`,
    );
  }
  return failures;
}
