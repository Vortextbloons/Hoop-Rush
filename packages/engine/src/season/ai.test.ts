...
import { seasonLeagueGenerationResultSchema, seasonRosterCalibrationRunSchema, seedSchema, type SeasonDraftCandidate, type SeasonDraftCatalog, type SeasonLeague, type SeasonLeagueGenerationResult, } from '@hoop-rush/data-contracts';
import { buildSeasonDraftCatalog, seedFromString } from '@hoop-rush/test-fixtures';
import { SeasonAiGenerationError, assignAiBandsAndIdentities, evaluateSeasonRoster, generateAiLeague, runSeasonRosterCalibrationSeeds, } from './ai.ts';
...
describe('season AI band and identity assignment', () => {
    ...
    it('rotates the smaller identity count with the seed', () => {
        const countsOf = (seed: string) => {
            ...
            for (const a of rows)
                map.set(a.identity, (map.get(a.identity) ?? 0) + 1);
        ...
        if (different === null)
            throw new Error('no differing offsets found');
    });
});
        ...
        for (const s of seeds) {
            ...
            for (const evaluation of result.evaluations) {
                if (evaluation.franchiseId === 'lakers')
                    continue;
                if (evaluation.band === 'contender')
                    scores.contender.push(evaluation.strengthScore);
                if (evaluation.band === 'weaker')
                    scores.weaker.push(evaluation.strengthScore);
            }
        }
