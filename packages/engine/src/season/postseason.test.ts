...
import { SEASON_ENDING_MISSED_GAMES_SENTINEL, SEASON_RUN_SCHEMA_VERSION, buildInitialPostseasonState, commandIdSchema, franchiseIdSchema, playoffGameIdOf, playInGameIdOf, seasonNamespaceSeed, seasonPostseasonSummarySchema, seasonRunSchema, seedSchema, type FranchiseId, type Seed, type SeasonDraftCatalog, type SeasonEffectsState, type SeasonGamePlayerInput, type SeasonGameSimulationInput, type SeasonGameSimulationResult, type SeasonGameSideResult, type SeasonLeague, type SeasonPostseasonState, type SeasonPostseasonSummary, type SeasonRotation, type SeasonRun, type SeasonRunCommand, type SeasonStandings, type Position, } from '@hoop-rush/data-contracts';
...
function rankedState(league: SeasonLeague, seed: string | Parameters<typeof buildInitialPostseasonState>[0] = TEST_SEED): SeasonPostseasonState {
...
function seasonEndingInjuries(run: SeasonRun, franchiseId: FranchiseId | string, count: number, prefix: string): SeasonRun['health']['injuries'] {
...
function repairedRotation(run: SeasonRun, franchiseId: FranchiseId | string, catalog: SeasonDraftCatalog): SeasonRotation {
    const roster = run.rosters.find((entry) => entry.franchiseId === franchiseId);
    if (roster === undefined)
        throw new Error(`no roster for ${franchiseId}`);
...
function playThroughCommands(context: SeasonRunCommandContext & {
    ...
    const start = handleSeasonRunCommand(commandOf(ctx.run, {
    ...
    for (let index = 1; index < 400; index += 1) {
        const advance = handleSeasonRunCommand(commandOf(ctx.run, {
...
describe('command-driven postseason flow', () => {
    ...
    it('simulates human games with the carried rotation when the rotation stays valid', () => {
        ...
        const first = handleSeasonRunCommand(commandOf(start.run, {
            command: 'advance-postseason',
    ...
    it('stops at a human game whose rotation is invalid and continues after the repair', () => {
        ...
        const nextAdvance = handleSeasonRunCommand(commandOf(submit.run, {
            command: 'advance-postseason',
    ...
    it('rejects advance with integrity-failure when both AI teams of a game are invalid', () => {
        ...
        const advance = handleSeasonRunCommand(commandOf(run, { command: 'advance-postseason', commandId: commandIdSchema.parse('adv-1') }), { ...context, run });
        const result = advance.result;
        if (result.command !== 'advance-postseason')
            throw new Error('unexpected');
    ...
    it('spectates after elimination and fast-forwards to the champion', () => {
        ...
        const fastForward = handleSeasonRunCommand(commandOf(spectate.run, {
    ...
    it('fast-forwards an AI-only run to completion in canonical order', () => {
        ...
        const start = handleSeasonRunCommand(commandOf(run, { command: 'start-postseason', commandId: commandIdSchema.parse('start-1') }), aiOnlyContext);
        if (start.result.result.status !== 'accepted')
            throw new Error('start rejected');
        const fastForward = handleSeasonRunCommand(commandOf(start.run, {
            command: 'fast-forward-postseason',
            commandId: commandIdSchema.parse('ff-1'),
        }), { ...context, run: start.run });
        const ffResult = fastForward.result;
