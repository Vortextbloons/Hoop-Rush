import type { SeasonFoulOut, SeasonGamePlayerInput, SeasonGameSimulationInput, SeasonGameSimulationResult, SeasonGameSideResult, SeasonGameTeamInput, SeasonGameEffectsTransition, SeasonPlayerLoadState, SeasonRemoval, SeasonRemovalEvent, SeasonReturn, SeasonReturnEvent, SeasonRotation, SeasonRotationDeviation, SeasonRotationDeviationReason, SeasonStaminaInput, SeasonSubstitution, SeasonSubstitutionReason, SeasonUnitStint, SeasonEffectsState, } from '@hoop-rush/data-contracts';
...
import { MAX_PERIODS_HARD_CAP, OVERTIME_PERIOD_SECONDS, REGULATION_PERIOD_SECONDS, overtimePeriodsOf, } from '../sim/periods.ts';
...
export function defaultSeasonGameSeam(input: SeasonGameSimulationInput): SeasonGameAvailabilitySeam {
    return {
...
export function simulateSeasonGame(input: SeasonGameSimulationInput, context: EngineContext, options: {
...
export function simulateSeasonGameWithEffects(input: SeasonGameSimulationInput, context: EngineContext, state: SeasonEffectsState, options: {
    seam?: SeasonGameAvailabilitySeam;
} = {}): {
    ...
    for (const player of input.home.players) {
        if (player.stamina === undefined) {
            throw new Error(`season effects: home player ${player.playerVersionId} has no stamina profile`);
        }
    ...
    for (const player of input.away.players) {
        if (player.stamina === undefined) {
            throw new Error(`season effects: away player ${player.playerVersionId} has no stamina profile`);
        }
...
function compareQueueEntries(a: {
    period: number;
...
class SideState {
    ...
    constructor(side: 'home' | 'away', sideIndex: SideIndex, teamInput: SeasonGameTeamInput, rotation: SeasonRotation, pregame: ReadonlyMap<string, boolean>) {
        this.side = side;
        this.sideIndex = sideIndex;
        this.teamInput = teamInput;
        this.rotation = rotation;
        this.roster = teamInput.players;
        this.rosterIndexByVersion = new Map(teamInput.players.map((player, index) => [player.playerVersionId, index]));
    ...
    consumeCheckpoints(period: number, clock: number): boolean {
        ...
        while (this.checkpointIndex < CHECKPOINT_MARKS.length &&
...
class SeasonGameController {
    ...
    constructor(input: SeasonGameSimulationInput, context: EngineContext, seam: SeasonGameAvailabilitySeam, effectsMode: SeasonGameEffectsMode | null = null) {
        this.input = input;
        ...
        this.tripContext = createTripContext(this.rng, this.recorder, this.state, this.profile, placeholder, seasonHomeCourtMechanisms(input.homeCourt), effectsMode?.buffer.hook);
    ...
    run(): SeasonGameSimulationResult {
        ...
        if (tipoffForfeit !== null)
            return this.exitWithEffects(tipoffForfeit);
        for (this.period = 1;; this.period += 1) {
            if (this.period > MAX_PERIODS_HARD_CAP) {
                throw new Error(`season: exceeded hard cap ${String(MAX_PERIODS_HARD_CAP)} periods without a winner (${String(this.recorder.sides[0].points)}-${String(this.recorder.sides[1].points)})`);
            }
            if (this.period > 1) {
                if (this.period >= 5) {
                    if (this.recorder.sides[0].points !== this.recorder.sides[1].points)
                        break;
                }
                if (this.period > MAX_PERIODS_HARD_CAP) {
                    throw new Error(`season: exceeded hard cap ${String(MAX_PERIODS_HARD_CAP)} periods without a winner`);
                }
                ...
                if (this.period === 3)
                    this.effectsMode?.buffer.hook.halftime();
            }
    ...
    private exitWithEffects(result: SeasonGameSimulationResult): SeasonGameSimulationResult {
        ...
        if (effects !== null && this.effectsTransition === null) {
            const finished = effects.buffer.finishGame(this.home.regulationSeconds, this.away.regulationSeconds);
    ...
    private processBoundary(periodEnded: boolean): SeasonGameSimulationResult | null {
        ...
        for (const side of [this.home, this.away]) {
            if (gameContinues)
                this.reopenStint(side, period, periodEnded);
        }
        return null;
    }
    ...
    private applyDueReturns(period: number, floatClock: number, boundaryClock: number, periodEnded: boolean): void {
        this.drainQueue(this.returnQueue, period, floatClock, boundaryClock, periodEnded, (ret, side) => {
            if (side.fouledOut.has(ret.playerVersionId))
                return;
    ...
    private drainQueue<T extends {
        period: number;
        secondsRemaining: number;
        side: 'home' | 'away';
    }>(queue: T[], period: number, floatClock: number, boundaryClock: number, periodEnded: boolean, apply: (entry: T, side: SideState) => void): void {
    ...
    private queueEntryIsDue(entry: {
        period: number;
        secondsRemaining: number;
    }, period: number, floatClock: number, periodEnded: boolean): boolean {
        if (entry.period > period)
            return false;
    ...
    private applyFoulOuts(side: SideState, period: number, clock: number): void {
        for (const playerVersionId of side.unit) {
            if (side.fouledOut.has(playerVersionId))
                continue;
            const rosterIndex = side.rosterIndexByVersion.get(playerVersionId);
            const rosterRecord = rosterIndex === undefined ? undefined : this.recorder.players[side.sideIndex][rosterIndex];
            if (rosterRecord === undefined)
                continue;
    ...
    private planFor(side: SideState, period: number, periodEnded: boolean, clock: number): {
    ...
    private applySubstitutionIfChanged(side: SideState, planned: readonly string[], period: number, clock: number, reason: SeasonSubstitutionReason): void {
    ...
    private accumulateStintInterval(side: SideState, period: number, clock: number): void {
        ...
        if (stint.period !== period) {
            throw new Error(`season: stint opened in period ${String(stint.period)} closed in period ${String(period)}`);
        }
        ...
        for (const playerVersionId of stint.unit) {
            ...
            if (period <= 4) {
                side.regulationSeconds.set(playerVersionId, (side.regulationSeconds.get(playerVersionId) ?? 0) + duration);
            }
    ...
    private finalizeStint(side: SideState, period: number, clock: number, periodEnded: boolean): void {
        const stint = side.stint;
        if (stint === null) {
            throw new Error(`season: no open stint for ${side.side}`);
        }
    ...
    private forfeitResult(loser: SideState, trigger: 'no-legal-five-tipoff' | 'no-legal-five-after-removal'): SeasonGameSimulationResult {
    ...
    private buildResult(): SeasonGameSimulationResult {
        ...
        if (homeScore === awayScore) {
            throw new Error(`season: game ended tied ${String(homeScore)}-${String(awayScore)} after ${String(overtimePeriods)} overtime periods`);
        }
