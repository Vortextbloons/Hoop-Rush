...
import { SEASON_MINUTE_POLICY_VERSION, SEASON_ROTATION_VERSION, franchiseIdSchema, } from '@hoop-rush/data-contracts';
import { canPlay } from '../domain/positions.ts';
import { chooseInitialUnit, enumerateLegalFives, planUnit, type PlannerMember, type PlannerRotationContext, type PlannerUnitRequest, } from './rotation-planner.ts';
...
function plannerContext(rotation: SeasonRotation = buildRotation()): PlannerRotationContext {
    ...
    const targets = new Map(rotation.targetMinutes.map((entry) => [entry.playerVersionId, entry.minutes * 60]));
...
function isLegalUnit(unit: readonly string[], members: ReadonlyMap<string, readonly Position[]>): boolean {
...
function bruteForceLegalFives(members: readonly PlannerMember[], available: ReadonlySet<string>): string[][] {
    ...
    const solve = (slot: number): void => {
        ...
        for (const playerVersionId of candidateIds) {
            if (used.has(playerVersionId))
                continue;
            const playable = playableById.get(playerVersionId);
            if (playable === undefined || !canPlay(playable, requirement))
                continue;
...
function deviationScoreOf(unit: readonly string[], delta: number, context: PlannerRotationContext, actualSeconds: ReadonlyMap<string, number>): number {
    ...
    it('never includes unavailable players and still matches the brute-force set', () => {
        ...
        for (const unit of units) {
            ...
            for (const playerVersionId of unit)
                expect(unavailable.has(playerVersionId)).toBe(false);
        }
        const brute = bruteForceLegalFives(MEMBERS, available);
...
describe('chooseInitialUnit (rotation-planner-v1)', () => {
    ...
    it('returns null when no legal five exists', () => {
        const context = plannerContext();
        expect(chooseInitialUnit(context, new Set(['pv-05', 'pv-06', 'pv-07', 'pv-08', 'pv-09', 'pv-10']))).toBeNull();
    });
});
describe('planUnit normal scoring (rotation-planner-v1)', () => {
    const centerCompetition = (): {
        context: PlannerRotationContext;
        base: PlannerUnitRequest;
    } => {
        const context = plannerContext();
...
describe('planUnit closing window and overtime (rotation-planner-v1)', () => {
    ...
    it('ranks contingencies by closing-five overlap before continuity', () => {
        const context = plannerContext(buildRotation({ closingFive: ['pv-01', 'pv-02', 'pv-03', 'pv-04', 'pv-10'] }));
        const result = planUnit(context, {
            ...request({
...
describe('planner property tests (rotation-planner-v1)', () => {
    const benchPosArb = fc.constantFrom<readonly Position[]>(['PG'], ['SG'], ['SF'], ['PF'], ['C'], ['PG', 'SG'], ['SF', 'PF'], ['PF', 'C'], ['PG', 'SF']);
    const rosterArb = fc.array(benchPosArb, { minLength: 5, maxLength: 5 }).map((benchPositions): {
    ...
    it('planned units are always legal, available, deterministic, and score-minimal', () => {
        fc.assert(fc.property(rosterArb, unavailableArb, fc.array(fc.integer({ min: 0, max: 2880 }), { minLength: 10, maxLength: 10 }), fc.integer({ min: 0, max: 720 }), fc.integer({ min: 1, max: 6 }), fc.boolean(), fc.integer({ min: 0, max: 40 }), ({ members }, unavailable, seconds, secondsRemaining, period, closingWindow, scoreMargin) => {
            ...
            const context: PlannerRotationContext = {
                ...
                targets: new Map(rotation.targetMinutes.map((entry) => [entry.playerVersionId, entry.minutes * 60])),
            };
            const actualSeconds = new Map(members.map((member, index) => [member.playerVersionId, seconds[index] ?? 0]));
            ...
            if ((closingWindow || period > 4) &&
                ...
                const scores = candidates.map((candidate) => deviationScoreOf(candidate, delta, context, actualSeconds));
                expect(deviationScoreOf(unit, delta, context, actualSeconds)).toBe(Math.min(...scores));
            }
            else {
                const overlap = (candidate: readonly string[]) => candidate.filter((id) => closing.includes(id)).length;
    ...
    it('chooseInitialUnit is deterministic and prefers legal starters', () => {
        fc.assert(fc.property(rosterArb, unavailableArb, ({ members }, unavailable) => {
            const rotation = buildRotation();
            const context: PlannerRotationContext = {
                rotation,
                members: new Map(members.map((member) => [member.playerVersionId, member.playable])),
