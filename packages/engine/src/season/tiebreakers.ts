import { SEASON_SEED_NAMESPACES, franchiseIdSchema, idSchema, seasonNamespaceSeed, seedSchema, type SeasonLeague, type SeasonStandings, type SeasonStandingsRow, type SeasonTiebreakResolution, type SeasonTiebreakRule, } from '@hoop-rush/data-contracts';
...
function headToHeadOf(ctx: RankContext, a: string, b: string): {
    wins: number;
    losses: number;
} {
...
function recordAmongOf(ctx: RankContext, a: string, group: readonly string[]): {
    wins: number;
    losses: number;
} {
    let wins = 0;
...
function recordVsEligibleOf(ctx: RankContext, a: string, eligible: Set<string> | null): {
    wins: number;
    losses: number;
} {
    let wins = 0;
...
function isDivisionChampion(ctx: RankContext, franchiseId: string): boolean {
    ...
    for (const other of ctx.league.teams) {
        if (other.franchiseId === franchiseId || other.division !== team.division)
            continue;
...
function divisionRecordCriterion(): TiebreakCriterion {
    ...
    return {
        ...
        applicable(group, ctx) {
            ...
            if (team === undefined)
                return false;
            return group.every((franchiseId) => ctx.league.teams.find((entry) => entry.franchiseId === franchiseId)?.division ===
                team.division);
        },
        compare(a, b, _group, ctx) {
            const aRow = rowOf(ctx, a);
            const bRow = rowOf(ctx, b);
            return compareWinPct(aRow.divisionWins, aRow.divisionLosses, bRow.divisionWins, bRow.divisionLosses);
        },
        evidenceOf(teams, ctx) {
            return teams.map((teamId) => {
...
function conferenceRecordCriterion(): TiebreakCriterion {
    return {
        ...
        compare(a, b, _group, ctx) {
            ...
            return compareWinPct(aRow.conferenceWins, aRow.conferenceLosses, bRow.conferenceWins, bRow.conferenceLosses);
        },
        evidenceOf(teams, ctx) {
            return teams.map((teamId) => {
...
function drawSeedOf(ctx: RankContext, group: readonly string[]): string {
    return seasonNamespaceSeed(ctx.seed, SEASON_SEED_NAMESPACES.postseasonTies, 'draw', ...[...group].sort());
}
function recordResolution(ctx: RankContext, teams: readonly string[], rule: SeasonTiebreakRule, evidence: readonly CriterionValue[], drawSeed: string | null): void {
...
function partitionByCriterion(ctx: RankContext, group: readonly string[], criterion: TiebreakCriterion): string[][] | null {
    const ids = [...group];
...
function recordWindows(ctx: RankContext, ordered: readonly string[], rule: SeasonTiebreakRule, evidenceOf: (teams: readonly string[]) => readonly CriterionValue[], drawSeed: string | null): void {
    if (ordered.length === 2 || ordered.length === 3) {
        recordResolution(ctx, ordered, rule, evidenceOf(ordered), drawSeed);
        return;
    }
    for (let i = 0; i < ordered.length - 1; i += 1) {
        recordResolution(ctx, ordered.slice(i, i + 2), rule, evidenceOf(ordered.slice(i, i + 2)), drawSeed);
    }
}
function resolveTieGroup(ctx: RankContext, group: readonly string[]): string[] {
    const criteria = criteriaOf(group);
    for (const criterion of criteria) {
        if (!criterion.applicable(group, ctx))
            continue;
        ...
        if (clusters.length === group.length) {
            ...
            recordWindows(ctx, ordered, criterion.rule, (teams) => criterion.evidenceOf(teams, ctx), null);
            return ordered;
        }
        for (let i = 0; i < clusters.length - 1; i += 1) {
            ...
            if (boundary !== '' && next !== '') {
                recordResolution(ctx, [boundary, next], criterion.rule, criterion.evidenceOf([boundary, next], ctx), null);
            }
        }
    ...
    recordWindows(ctx, drawn, 'random-draw', () => [{ label: 'deciding rule', value: 'random-draw' }], drawSeed);
    return drawn;
}
...
function rankBothWith(ctx: RankContext, eastEligible: Set<string> | null, westEligible: Set<string> | null): {
    east: string[];
    west: string[];
} {
...
export function rankSeasonPostseason(league: SeasonLeague, standings: SeasonStandings, seed: string): SeasonPostseasonRankings {
...
function finishConference(league: SeasonLeague, standings: SeasonStandings, seed: string, conference: SeasonConferenceId, sameEligible: Set<string> | null, oppositeEligible: Set<string> | null, expectedOrder: readonly string[]): SeasonConferenceRanking {
    ...
    for (const resolution of ctx.resolutions) {
        const slots = resolution.teams
            ...
            .filter((slot) => slot > 0)
            .sort((a, b) => a - b);
        if (slots.length === 0 || (slots[slots.length - 1] ?? 0) > 10)
            continue;
