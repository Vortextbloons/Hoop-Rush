...
import { franchiseIdSchema, seasonGameIdSchema, seasonTradeGradeLogSchema, type SeasonCompactPlayerLine, type SeasonGameSummary, type SeasonPostseasonSummary, type SeasonTeamBox, type SeasonTradeOffer, type SeasonTradeState, } from '@hoop-rush/data-contracts';
import { deriveSeasonTradeGrades, seasonTradeGradeLabelOf, SEASON_TRADE_GRADE_MIN_SAMPLE, SEASON_TRADE_GRADE_NEUTRAL_SCORE, } from './trade-grades.ts';
...
function boxOf(franchiseId: string, lines: readonly SeasonCompactPlayerLine[]): SeasonTeamBox {
    const sum = (pick: (entry: SeasonCompactPlayerLine) => number) => lines.reduce((total, entry) => total + pick(entry), 0);
    return {
...
function summary(round: number, spec: GameSpec = {}): SeasonGameSummary {
    ...
    const lineupOf = (filler: string[], overrides?: Record<string, LineSpec>): SeasonCompactPlayerLine[] => {
        const ids = [...(overrides !== undefined ? Object.keys(overrides) : []), ...filler]
            .filter((id, index, all) => all.indexOf(id) === index)
            .slice(0, 10);
        return ids.map((versionId, index) => line(versionId, { started: index < 5, ...(overrides?.[versionId] ?? {}) }));
    };
...
function offer(windowIndex: number, toFranchiseId: string, fromFranchiseId: string, outgoing: string[], incoming: string[], offerId = `off-${String(windowIndex).padStart(4, '0')}${'a'.repeat(28)}`): SeasonTradeOffer {
    return {
        offerId,
        windowIndex,
...
describe('season trade grade derivation (trade-grade-v1)', () => {
    it('grades both sides of an accepted trade from recorded post-trade games', () => {
        ...
        expect(lakers?.components.production ?? 0).toBeGreaterThan(celtics?.components.production ?? 100);
    ...
    it.each([
        [34, 4],
        [0, 0],
    ] as const)('grades neutral below the five-game floor with %s post-trade games', (rounds, expectedSample) => {
        const { run } = runWithTrade();
        const grades = deriveSeasonTradeGrades({
            runId: run.runId,
            run,
    ...
    it('handles a player traded twice across windows without double counting', () => {
        ...
        expect(celticsWindowOne?.components.production ?? 0).toBeGreaterThan(lakersWindowOne?.components.production ?? 100);
    });
    ...
    it('credits the received side when production is realized through the champion', () => {
        ...
        const postseason = Array.from({ length: 4 }, (_, index) => postseasonSummary(`po-finals-g${String(index + 1)}`, {
            homeLines: { [STAR]: STAR_LINE },
            awayLines: { [SECOND]: BENCH_LINE },
        }));
        const grades = deriveSeasonTradeGrades({
        ...
        const productionReason = lakers?.reasons.find((reason) => reason.startsWith('received production'));
