import { SEASON_OBJECTIVE_CATALOG, SEASON_SEED_NAMESPACES, seasonNamespaceSeed, type SeasonGameSummary, type SeasonObjectiveDefinition, type SeasonObjectiveEvaluation, type SeasonObjectiveId, type SeasonRotation, } from '@hoop-rush/data-contracts';
export function seasonObjectiveCatalog(): readonly SeasonObjectiveDefinition[] {
    return SEASON_OBJECTIVE_CATALOG;
}
export function seasonObjectiveChoicesForBlock(rootSeed: string, blockIndex: number): SeasonObjectiveId[] {
    const ranked = SEASON_OBJECTIVE_CATALOG.map((entry) => ({
        entry,
        hash: seasonNamespaceSeed(rootSeed, SEASON_SEED_NAMESPACES.objectives, 'choices', String(blockIndex), entry.objectiveId),
    })).sort((a, b) => {
        if (a.hash !== b.hash)
...
export function evaluateSeasonBlockObjective(input: SeasonObjectiveEvaluationInput): SeasonObjectiveEvaluationResult {
    ...
    const games = input.summaries.filter((summary) => summary.homeFranchiseId === human || summary.awayFranchiseId === human);
    let wins = 0;
    let pointsAllowed = 0;
    let reboundMargin = 0;
    let turnovers = 0;
    for (const summary of games) {
        ...
        if (summary.status === 'forfeit') {
            if (summary.forfeitLoserFranchiseId !== human)
                wins += 1;
        }
        else if (homeIsHuman) {
    ...
    if (input.rotation !== null) {
        ...
        const startersWithMinutes = new Set(rotation.targetMinutes
        ...
        for (const summary of games) {
            ...
            for (const line of lines) {
                if (!startersWithMinutes.has(line.playerVersionId))
                    benchSeconds += line.seconds;
            }
        }
        benchMinutes = Math.floor(benchSeconds / 60);
    }
    const tipsWithAtLeastEightAvailable = input.tipAvailability.filter((tip) => tip.availableCount >= 8).length;
