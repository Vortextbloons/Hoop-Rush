...
import { SEASON_MINUTE_POLICY_VERSION, SEASON_NEUTRAL_HOME_COURT, SEASON_ROTATION_PRESET_TARGETS, SEASON_ROTATION_VERSION, SEASON_STAMINA_VERSION, franchiseIdSchema, playerIdSchema, playerVersionId, seedSchema, type Position, type SeasonEffectsState, type SeasonGameSimulationInput, type SeasonGameTeamInput, type SeasonRotation, type SeasonStaminaInput, } from '@hoop-rush/data-contracts';
...
import { applySeasonGameEffectsTransition, createSeasonEffectsBuffer, createSeasonEffectsState, SEASON_EFFECTS_MECHANISM_CAPS, } from './effects.ts';
import { pairChemistryBasisPoints, unitChemistryBasisPoints, unitPairs } from './chemistry.ts';
import { applySeasonRecoveryTick, onCourtFatigueBp, recentLoadAfterGame, regulationShareBp, stintMultiplierBp, } from './stamina.ts';
...
function buildGameInput(home: SeasonGameTeamInput, away: SeasonGameTeamInput, seed = 'season-effects-1'): SeasonGameSimulationInput {
    return {
        schemaVersion: 1,
...
type StaminaRoster = SeasonGameTeamInput & {
    staminaInputs: SeasonStaminaInput[];
};
function buildLeagueState(home: StaminaRoster, away: StaminaRoster): {
    ...
    for (let roster = 0; roster < 30; roster += 1) {
        for (let slot = 0; slot < 10; slot += 1) {
            if (roster === 0) {
                ...
                const fillerId = playerVersionId(`p-filler-${String(roster)}-${String(slot)}`, `filler-${String(roster)}`, '1990s', '1995-96');
                allInputs.push({
                    schemaVersion: 1,
...
function fatigueOf(state: SeasonEffectsState, version: string): number {
    ...
    if (player === undefined)
        throw new Error(`no load state for ${version}`);
...
describe('M2.4 effects zero-profile identity', () => {
    ...
    it('returns exactly +0 from every adjustment query on a zero-state buffer', () => {
        ...
        expect(buffer.hook.makeAdjustment({ shooterVersion: shooter, offenseSide: 0, defenseSide: 1 })).toBe(0);
        ...
        expect(buffer.hook.makeAdjustment({
...
describe('M2.4 fixed-point fatigue mechanics', () => {
    ...
    it('applies the consecutive-stint multiplier ramp from 6 to 12 minutes', () => {
        expect(stintMultiplierBp(0)).toBe(10000);
        expect(stintMultiplierBp(360)).toBe(10000);
        expect(stintMultiplierBp(720)).toBe(12500);
        expect(stintMultiplierBp(540)).toBe(11250);
        expect(stintMultiplierBp(800)).toBe(12500);
    });
    it('never reduces fatigue when minutes, stint length, or recent load increase', () => {
        ...
        fresh.hook.setActiveUnits(unit, away.players.slice(0, 5).map((p) => p.playerVersionId));
        fresh.hook.recordStintSeconds(0, 600, unit);
        const afterShort = fatigueOf(applySeasonGameEffectsTransition(state, {
            schemaVersion: 1,
        ...
        longer.hook.setActiveUnits(unit, away.players.slice(0, 5).map((p) => p.playerVersionId));
        longer.hook.recordStintSeconds(0, 1200, unit);
        const afterLong = fatigueOf(applySeasonGameEffectsTransition(state, {
            schemaVersion: 1,
            pregamePlayerStates: state.playerStates,
        ...
        broken.hook.setActiveUnits(unit, away.players.slice(0, 5).map((p) => p.playerVersionId));
        broken.hook.recordStintSeconds(0, 600, unit);
        broken.hook.recordStintSeconds(0, 0, []);
        broken.hook.recordStintSeconds(0, 600, unit);
        const afterBroken = fatigueOf(applySeasonGameEffectsTransition(state, {
            schemaVersion: 1,
            pregamePlayerStates: state.playerStates,
    ...
    it('updates recent load as 60% previous + 40% regulation-minute share', () => {
        ...
        expect(regulationShareBp(2880)).toBe(10000);
        expect(recentLoadAfterGame(4000, 1440)).toBe(Math.round(0.6 * 4000 + 0.4 * 5000));
        expect(recentLoadAfterGame(10000, 2880)).toBe(10000);
    });
});
    ...
    it('reduces every positive fatigue state, never below zero, capped rounds', () => {
        ...
        const fatigueState = applySeasonGameEffectsTransition(state, {
            ...
            postgamePlayerStates: state.playerStates.map((player) => ({
                ...player,
                fatigueBasisPoints: Math.min(10000, player.fatigueBasisPoints + 3000),
            })),
            pairIncrements: [],
            evidence: [],
        });
        ...
        const ratings = new Map(state.playerStates.map((player) => [
        ...
        for (const player of ticked.playerStates) {
            ...
            expect(player.fatigueBasisPoints).toBeLessThanOrEqual(fatigueOf(fatigueState, player.playerVersionId));
...
describe('M2.4 pair chemistry', () => {
    it('converts shared possessions monotonically to basis points', () => {
        ...
        expect(pairChemistryBasisPoints(100000)).toBeGreaterThan(9900);
    });
    ...
    it('increments a pair only when both members share a completed trip', () => {
        ...
        const incrementOf = (a: string, b: string): number => increments.find((i) => i.a === a && i.b === b)?.sharedPossessions ?? 0;
...
describe('M2.4 mechanism evidence', () => {
    it('records opportunities, inputs, totals, and min/max within caps', () => {
        ...
        const shooterRow = evidence.find((row) => row.mechanism === 'shooter-fatigue' && row.side === 'home');
        expect(shooterRow).toBeDefined();
        expect(shooterRow?.opportunities).toBe(1);
        expect(shooterRow?.inputTotals.shooter).toBe(500000);
        expect(shooterRow?.deltaTotals).toBe(-25000);
        expect(shooterRow?.deltaMin).toBe(-25000);
        expect(shooterRow?.deltaMax).toBe(-25000);
...
describe('M2.4 effects state construction', () => {
    ...
    it('validates transitions and folds pair increments additively', () => {
        ...
        const next = applySeasonGameEffectsTransition(state, {
            ...
            postgamePlayerStates: state.playerStates.map((player) => ({
                ...player,
                fatigueBasisPoints: Math.min(10000, player.fatigueBasisPoints + 100),
            })),
