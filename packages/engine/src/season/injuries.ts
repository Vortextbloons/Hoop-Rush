import { SEASON_ENDING_MISSED_GAMES_SENTINEL, SEASON_SEED_NAMESPACES, franchiseIdSchema, seasonDigestHex, seasonNamespaceSeed, type SeasonHealthState, type SeasonInjuryRecord, type SeasonInjurySeverity, type SeasonInjuryType, } from '@hoop-rush/data-contracts';
...
export function applyRiskyRehabOutcome(health: SeasonHealthState, injuryId: string, outcome: 'success' | 'failure'): SeasonHealthState {
    ...
    if (record.seasonEnding) {
        ...
        updated.missedGamesRemaining = Math.min(SEASON_ENDING_MISSED_GAMES_SENTINEL - 1, updated.missedGamesRemaining + 1);
        updated.rehabModifier = 1;
        updated.rehabAttempted = true;
