...
// Deterministic hex id helper uses seasonNamespaceSeed
function hexId(prefix: string, seed: string): string {
  // seed is already hex 32, slice appropriate
...
// Catalog defines feasible templates. Thresholds tuned for 10-game block.
// All templates use block window unless noted.
...
// Branch definitions: each branch is ordered list of templateIds of length 2-3
// Derived from catalog grouping by branchId
function branches(): Map<string, CampaignTemplate[]> {
  ...
  // sort each branch by templateId for canonical order
...
export function normalizeCampaignState(state: unknown): SeasonCampaignState {
  ...
  // Validate via safeParse? Use direct cast if passes structure check
  // For old saves without campaign, provide empty.
  // We try to parse as SeasonCampaignState, fallback to empty.
  // For old saves without campaign, provide empty.
...
function resolvePlayerForTemplate(
  ...
  // Find rotation-eligible available players
  ...
  // Deterministically pick one based on template and block using seeded selection (canonical)
  // Use createRng with seedPath campaign/{blockIndex}/offers/candidate/{templateId}
...
function scheduleFactsForTemplate(
  ...
  // For sweep-opponent with required opponent, check existence; otherwise pick deterministic
  const templateOpponent =
    (template.target as { opponentFranchiseId?: string }).opponentFranchiseId ??
    (template.breakthrough as { opponentFranchiseId?: string } | null)?.opponentFranchiseId;
...
function transactionFactsForTemplate(
  template: CampaignTemplate,
  input: SeasonCampaignGenerationInput,
): { feasible: boolean; facts: Record<string, unknown>; audit: string | null } {
  ...
  // Look for recent trade or free-agency signing for human
  ...
  // Also consider roster size >10 as indicator of new player
  const roster = input.rosters.find((r) => r.franchiseId === humanFranchiseId);
  ...
  // Deterministically pick transaction id if needed
...
function isBranchOpen(
  template: CampaignTemplate,
  campaignState: SeasonCampaignState,
): { open: boolean; prerequisiteId: string | null; audit: string | null } {
  ...
  if (idx === 0) {
    // First in branch always open unless branch missed/locked
  ...
  // For follow-up, need previous template's evaluation completed/breakthrough
  const prev = branchTemplates[idx - 1];
  if (!prev) return { open: false, prerequisiteId: null, audit: 'no previous in branch' };
  // Find evaluation for prev's opportunity? Need to map template to opportunityId? We track branchState.
  // If branchState indicates missed, closed.
  const state = campaignState.branchState[template.branchId];
  if (state === 'missed' || state === 'locked') {
    return {
      open: false,
  ...
  // For now, rely on branchState: if state === 'open', follow-up is open
  if (state === 'open') {
    // Find prerequisite opportunityId as last selected in branch
    // Look at selections: find latest block where branch's template was selected
    // Simpler: return open with prerequisite as previous template's opportunity id placeholder
    // We will resolve prerequisiteId later as actual opportunityId if exists
    return { open: true, prerequisiteId: prev.templateId, audit: null };
  }
  // If no state yet, only first is open
  return {
    open: false,
    prerequisiteId: null,
...
function weightForCandidate(
  ...
  // Branch follow-ups get higher ordering
  return weight;
}
...
export function generateSeasonCampaignOffers(
  ...
  for (const tpl of CAMPAIGN_TEMPLATES) {
    ...
    // Determine if any required fact missing => reject candidate
    ...
    // Build feasibilityFacts merging all
    ...
    // Determine concrete playerVersionId for player templates to embed into condition
    // We need to override target playerVersionId if resolved
    // Keep original template's threshold but replace playerVersionId
    feasible.push({
      template: tpl,
  ...
  // Canonicalize before seeded selection for order invariance
  const canonical = canonicalSort(feasible);

  // Apply weighting (identity emphasis + branch follow-ups)
  // For weighting, detect follow-up: branch with previous selections
  const branchHasPrior = (branchId: string): boolean => {
    // If branchState is 'open' then there was a prior completed
  ...
  // Deterministic selection: for each slot, use rng weightedPick on remaining
  ...
  const opportunities: SeasonCampaignOpportunity[] = selected.map((candidate, slot) => {
    ...
    // Resolve concrete target/breakthrough with actual playerVersionId if needed
    ...
    if (candidate.playerVersionId !== null) {
      // Clone with updated playerVersionId
      if ('playerVersionId' in target) {
        ...
        // Template may have placeholder pv-0, replace
        target = {
          ...target,
          playerVersionId: candidate.playerVersionId,
    ...
    // Determine prerequisiteId: if this is follow-up, find previous opportunityId in branch
    ...
    if (idx > 0) {
      ...
      // Find if previous was selected/evaluated in campaignState
      // Search for offer with that template in prior blocks
      ...
      if (prerequisiteId === null && prevTpl) {
        // Fallback to templateId-based placeholder but keep null for type; use previous template's derived opportunity
        prerequisiteId = null;
      }
    }

    // Seed path as required
  ...
  if (opportunities[0]?.templateId === opportunities[1]?.templateId) {
    // Should be distinct templates, but if same due to player variations, ensure distinct
    // Already ensured distinct candidate selection, so okay
  }
...
export interface SeasonCampaignEvaluationInput {
  ...
  transactions: readonly SeasonTransactionEntry[];
  health: SeasonHealthState;
  // optional tip availability for bench? Not needed
}
...
function evaluateCondition(
...
): { met: boolean; factValue: number; facts: Record<string, unknown> } {
  const human = input.humanFranchiseId;
  ...
  switch (condition.kind) {
    ...
    case 'win-over-higher': {
      ...
      // Simplified: if any win and standings show opponent higher before? Use current standings as proxy: opponent has more wins than human before block.
      // For test determinism, count wins where opponent standings wins > human wins
    ...
    case 'beat-conference-leader': {
      const opponentId = (condition as { opponentFranchiseId?: string }).opponentFranchiseId;
    ...
    case 'bench-contribution': {
      // Estimate bench minutes: if summaries have no bench distinction, use simple proxy
      let benchPoints = 0;
      ...
      // Fallback if no starters info: use total points as proxy
      if (benchPoints === 0 && summaries.length > 0) {
        // Use turnovers as proxy? No
    ...
    case 'player-minutes': {
      const pid = (condition as { playerVersionId: string }).playerVersionId;
      let seconds = 0;
      for (const s of summaries) {
        const lines = s.homeFranchiseId === human ? s.homePlayers : s.awayPlayers;
    ...
    case 'player-availability': {
      const pid = (condition as { playerVersionId: string }).playerVersionId;
      // Use appearances or availability via health
      let avail = 0;
      for (const s of summaries) {
        ...
        else if (seasonPlayerAvailable(input.health, pid)) avail += 1; // proxy
      }
      // If no summaries, use health availability
    ...
    case 'roster-new-player-minutes': {
      const pid = (condition as { playerVersionId?: string }).playerVersionId;
      // Find new players via transactions
    ...
    case 'roster-replace-unavailable': {
      // Check if unavailable player was replaced by new player playing
...
function applyInfluenceReward(
  ...
  explanation: string,
  rewardId: string,
): { influence: SeasonInfluenceState; entry: SeasonInfluenceState['ledger'][number] } {
...
export function applySeasonCampaignReward(
  ...
  // For completed/breakthrough, apply rewards in order: completed then breakthrough
  ...
  // Branch progression: completed unlocks normal follow-up (open), breakthrough may unlock ambitious
...
export function generateSeasonCampaignEvolutionOffers(
  ...
  // adapt: based on roster/standing evidence
  // Determine focus suggestion from most common family in evaluations? Or from transactions
  ...
  // Adapt option: retain identity, add focus
  ...
  // pivot option: switch identity if struggling
  if (offers.length < 3) {
  ...
  // Ensure at most 3, at least 1 (double-down). Deterministic shuffle by seed then slice
  const canonical = [...offers].sort((a, b) => (a.offerId < b.offerId ? -1 : 1));
  // Keep double-down first, rest ordered by kind then id for determinism
  canonical.sort((a, b) => {
...
// Helpers for audit and determinism checks
