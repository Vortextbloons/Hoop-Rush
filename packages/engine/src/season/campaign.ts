...
function scheduleFactsForTemplate(
  ...
  const targetOpponent =
    'opponentFranchiseId' in template.target ? template.target.opponentFranchiseId : undefined;
  const breakthroughOpponent =
    template.breakthrough !== null && 'opponentFranchiseId' in template.breakthrough
        ...
        if (t.type === 'trade' || t.type === 'free-agent-signing') {
          ...
          const added: string[] = Array.isArray(rawIncoming)
            ...
            : typeof rawSingle === 'string'
              ? [rawSingle]
              : [];
          for (const id of added) newIds.add(id);
        }
      }
      let minutes = 0;
...
export function evaluateSeasonCampaignOpportunity(
  ...
  const facts: Record<string, unknown> = {
    ...
    breakthrough:
      breakthrough !== null && breakthroughRes !== null
        ? {
