...
function scheduleFactsForTemplate(template: CampaignTemplate, input: SeasonCampaignGenerationInput): {
    ...
    const templateOpponent = (template.target as {
        opponentFranchiseId?: string;
    }).opponentFranchiseId ??
        (template.breakthrough as {
            opponentFranchiseId?: string;
        } | null)?.opponentFranchiseId;
...
function applyInfluenceReward(influence: SeasonInfluenceState, franchiseId: string, requestedDelta: number, blockIndex: number, commandId: string | null, explanation: string, rewardId: string): {
    ...
    const entry = {
        entryId: deriveSeasonInfluenceEntryId(`influence-campaign-${String(blockIndex)}-${franchiseId}-${rewardId}`),
        franchiseId,
