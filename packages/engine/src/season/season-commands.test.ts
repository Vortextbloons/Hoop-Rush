...
import { SEASON_RUN_SCHEMA_VERSION, commandIdSchema, franchiseIdSchema, idSchema, playoffGameIdOf, seasonGameIdSchema, seasonRunSchema, type FranchiseId, type SeasonDraftCatalog, type SeasonEffectsState, type SeasonGameEffectsTransition, type SeasonGameSimulationInput, type SeasonGameSimulationResult, type SeasonGameSideResult, type SeasonGameSummary, type SeasonPendingBlockCandidate, type SeasonRotation, type SeasonRun, type SeasonRunCommand, type SeasonTradeOffer, type Position, } from '@hoop-rush/data-contracts';
...
describe('accept-trade-offer command', () => {
    ...
    it('rejects ownership-conflict when a moved player is owned elsewhere', () => {
        ...
        const tampered: SeasonRun = {
            ...run,
            ownership: run.ownership.map((row) => row.playerVersionId === conflictingVersion
...
describe('start-postseason command', () => {
    ...
    it('rejects an incomplete regular season and a run that already started', () => {
        ...
        const incompleteOutput = handleSeasonRunCommand(commandOf(incomplete, {
            command: 'start-postseason',
        ...
        const started = handleSeasonRunCommand(commandOf(context.run, {
            command: 'start-postseason',
            commandId: commandIdSchema.parse('start-1'),
        }), context);
        if (started.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const again = handleSeasonRunCommand(commandOf(started.run, {
            command: 'start-postseason',
    ...
    it('rejects invalid rankings with integrity-failure and a stale state', () => {
        ...
        const stale = handleSeasonRunCommand(commandOf({ ...context.run, stateRevision: context.run.stateRevision + 5 }, { command: 'start-postseason', commandId: commandIdSchema.parse('start-stale') }), context);
    ...
    it('rejects a run mismatch', () => {
        ...
        const mismatch = handleSeasonRunCommand(commandOf(other, {
            command: 'start-postseason',
...
describe('advance-postseason command', () => {
    it('runs the whole tournament when the human rotation stays valid', () => {
        ...
        const start = handleSeasonRunCommand(commandOf(context.run, {
            command: 'start-postseason',
            commandId: commandIdSchema.parse('start-1'),
        }), context);
        if (start.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const output = handleSeasonRunCommand(commandOf(start.run, {
    ...
    it('rejects wrong-game targets, invalid stages, and stale states', () => {
        ...
        const stale = handleSeasonRunCommand(commandOf({ ...start.run, stateRevision: start.run.stateRevision + 3 }, { command: 'advance-postseason', commandId: commandIdSchema.parse('adv-stale') }), { ...context, run: start.run });
        if (stale.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(stale.result.result.rejection.code).toBe('stale-state');
    });
...
describe('submit-postseason-rotation command', () => {
    ...
    function advancedToHumanGame(): {
        ...
        const advance = handleSeasonRunCommand(commandOf(injured, {
            command: 'advance-postseason',
            commandId: commandIdSchema.parse('adv-1'),
        }), { ...context, run: injured });
...
describe('fast-forward-postseason command', () => {
    ...
    it('rejects an active human, an invalid stage, and a bad target', () => {
        const context = postseasonFixture({ resolver: forcedPostseasonResolver(humanWinsEveryGame) });
        const activeStart = handleSeasonRunCommand(commandOf(context.run, {
            command: 'start-postseason',
            commandId: commandIdSchema.parse('start-1'),
        }), context);
        ...
        const start = handleSeasonRunCommand(commandOf(aiOnly.run, {
            command: 'start-postseason',
            commandId: commandIdSchema.parse('start-1'),
        }), aiOnly);
        if (start.result.result.status !== 'accepted')
