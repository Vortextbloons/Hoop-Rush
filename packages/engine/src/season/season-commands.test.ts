...
import { buildEconomyTestRun, injuryIdOf, withInjury, zeroEffectsOf, } from './season-economy-test-support.ts';
...
vi.mock('./injuries.ts', async (importOriginal) => {
    ...
    return {
        ...
        applyRiskyRehabOutcome: (health: SeasonRun['health'], injuryId: string, outcome: 'success' | 'failure') => ({
            ...health,
            injuries: health.injuries.map((injury) => injury.injuryId === injuryId
                ? {
                    ...injury,
                    missedGamesRemaining: outcome === 'success'
...
vi.mock('./health.ts', async (importOriginal) => {
    ...
    return {
        ...original,
        seasonForfeitSummaryForGame: (run: SeasonRun, gameId: string, humanFranchiseId: string): SeasonGameSummary => {
        ...
        advancePendingAfterForfeit: (pending: SeasonPendingBlockCandidate, forfeitedGameId: string): SeasonPendingBlockCandidate => ({
            ...pending,
...
function withClosedWindow(run: SeasonRun, windowIndex: number): SeasonRun {
    ...
    if (trade === null)
        throw new Error('no trade state');
    return {
        ...run,
        trade: {
            ...trade,
            windows: trade.windows.map((window) => window.windowIndex === windowIndex ? { ...window, status: 'closed' as const } : window),
        },
    };
}
...
describe('select-block-objective command', () => {
    it('accepts a selection for the current playable block and advances the state chain', () => {
        ...
        if (outputResult.command !== 'select-block-objective')
    ...
    it('rejects with run-mismatch, duplicate-command, and stale-state in order', () => {
        ...
        if (runMismatch.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(runMismatch.result.result.rejection.code).toBe('run-mismatch');
        const transactionCommandId = run.transactions.find((entry) => entry.commandId !== null)?.commandId;
        expect(transactionCommandId).toBeDefined();
        const duplicate = handleSeasonRunCommand(commandOf(run, {
            command: 'select-block-objective',
            commandId: transactionCommandId ?? 'missing',
            blockIndex: 0,
            objectiveId: offered,
        }), context);
        if (duplicate.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(duplicate.result.result.rejection.code).toBe('duplicate-command');
        const ledgerCommandId = run.influence.ledger.find((entry) => entry.commandId !== null)?.commandId;
        expect(ledgerCommandId).toBeDefined();
        const ledgerDuplicate = handleSeasonRunCommand(commandOf(run, {
            command: 'select-block-objective',
            commandId: ledgerCommandId ?? 'missing',
            blockIndex: 0,
            objectiveId: offered,
        }), context);
        if (ledgerDuplicate.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        ...
        if (staleRevision.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        ...
        if (staleDigest.result.result.status !== 'rejected')
            throw new Error('expected rejection');
    ...
    it('rejects duplicate-command after the same commandId was already applied', () => {
        const { run, context } = windowedFixture();
        const offered = seasonObjectiveChoicesForBlock(run.rootSeed, 0)[0] ?? 'win-six';
        const first = handleSeasonRunCommand(commandOf(run, {
            command: 'select-block-objective',
            commandId: 'select-obj-again',
            blockIndex: 0,
            objectiveId: offered,
        }), context);
        if (first.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const replay = handleSeasonRunCommand(commandOf(first.run, {
            command: 'select-block-objective',
            commandId: 'select-obj-again',
            blockIndex: 0,
            objectiveId: offered,
        ...
        if (replay.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(replay.result.result.rejection.code).toBe('duplicate-command');
    });
    it('rejects not-at-boundary and objective-not-offered', () => {
        const { run, context } = windowedFixture();
        const notAtBoundary = handleSeasonRunCommand(commandOf(run, {
            command: 'select-block-objective',
            commandId: 'cmd-boundary',
            blockIndex: 2,
            objectiveId: 'win-six',
        }), context);
        if (notAtBoundary.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        ...
        const notOfferedId = ([
            ...
            'bench-320',
            'turnover-130',
        ] as const).find((id) => !offered.includes(id));
        const notOffered = handleSeasonRunCommand(commandOf(run, {
            command: 'select-block-objective',
            commandId: 'cmd-not-offered',
            blockIndex: 0,
            objectiveId: notOfferedId ?? 'win-six',
        }), context);
        if (notOffered.result.result.status !== 'rejected')
            throw new Error('expected rejection');
    ...
    it('rejects objective-already-selected and not-at-boundary for future blocks', () => {
        ...
        const again = handleSeasonRunCommand(commandOf(current, {
            command: 'select-block-objective',
        ...
        if (again.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(again.result.result.rejection.code).toBe('objective-already-selected');
        const futureBlock = handleSeasonRunCommand(commandOf(current, {
            command: 'select-block-objective',
        ...
        if (futureBlock.result.result.status !== 'rejected')
            throw new Error('expected rejection');
...
describe('spend-influence command', () => {
    it('accepts an extra-trade-offer spend, tracks the window, and generates offer #4', () => {
        ...
        const output = handleSeasonRunCommand(command, context);
        const outputResult = output.result;
        if (outputResult.command !== 'spend-influence')
            throw new Error('unexpected command');
        const result = outputResult.result;
        if (result.status !== 'accepted')
            throw new Error('expected acceptance');
        ...
        expect(window?.offers.some((offer) => offer.offerId === result.generatedOffer?.offerId)).toBe(true);
    ...
    it('rejects insufficient-balance, window-not-open, already-spent, and no-window', () => {
        ...
        const insufficient = handleSeasonRunCommand(commandOf(atFloor, {
            command: 'spend-influence',
        ...
        if (insufficient.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(insufficient.result.result.rejection.code).toBe('insufficient-balance');
        const notOpen = handleSeasonRunCommand(commandOf(run, {
            command: 'spend-influence',
        ...
        if (notOpen.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        ...
        if (first.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const second = handleSeasonRunCommand(commandOf(first.run, {
            command: 'spend-influence',
        ...
        if (second.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        ...
        if (noWindowTrade === null)
            throw new Error('no trade state');
        const windowZero = noWindowTrade.windows[0];
        if (windowZero === undefined)
            throw new Error('no window 0');
        ...
        const noWindow = handleSeasonRunCommand(commandOf(noWindowRun, {
            command: 'spend-influence',
        ...
        if (noWindow.result.result.status !== 'rejected')
            throw new Error('expected rejection');
    ...
    it('accepts a risky-rehab spend on an active injury and records the outcome', () => {
        ...
        if (version === undefined)
            throw new Error('no human player');
        ...
        const output = handleSeasonRunCommand(commandOf(injured, {
            command: 'spend-influence',
    ...
    it('rejects injury-not-active and already-rehabbed', () => {
        const { run, context } = windowedFixture();
        const notActive = handleSeasonRunCommand(commandOf(run, {
            command: 'spend-influence',
        ...
        if (notActive.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        ...
        const output = handleSeasonRunCommand(commandOf(already, {
            command: 'spend-influence',
...
describe('accept-trade-offer command', () => {
    it('applies an open offer atomically and reports both roster changes', () => {
        ...
        const output = handleSeasonRunCommand(command, context);
        const outputResult = output.result;
        if (outputResult.command !== 'accept-trade-offer')
            throw new Error('unexpected command');
        const result = outputResult.result;
        if (result.status !== 'accepted')
            throw new Error('expected acceptance');
        ...
        expect(output.run.stateRevision).toBe(run.stateRevision + 1);
        const recorded = output.run.trade?.windows[0]?.offers.find((entry) => entry.offerId === offer.offerId);
    ...
    it('rejects offer-unknown, window-not-open, and offer-not-open', () => {
        ...
        if (notOpen.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(notOpen.result.result.rejection.code).toBe('window-not-open');
        const declined = handleSeasonRunCommand(commandOf(run, {
            command: 'decline-trade-offer',
            commandId: 'decline-first',
            windowIndex: 0,
            offerId: offer.offerId,
        }), context);
        if (declined.result.result.status !== 'accepted')
            throw new Error('expected decline');
        const acceptAfterDecline = handleSeasonRunCommand(commandOf(declined.run, {
            command: 'accept-trade-offer',
    ...
    it('rejects ownership-conflict when a moved player is owned elsewhere', () => {
        ...
        const tampered: SeasonRun = {
            ...run,
            ownership: run.ownership.map((row) => row.playerVersionId === conflictingVersion ? { ...row, ownerFranchiseId: 'celtics' } : row),
        };
        const output = handleSeasonRunCommand(commandOf(tampered, {
            command: 'accept-trade-offer',
    ...
    it('rejects roster-illegal when the swap breaks ten-player legality', () => {
        ...
        const playableOf = (version: string): readonly string[] => catalog.candidates.find((candidate) => candidate.playerVersionId === version)?.positions
            .playable ?? [];
        const centerCapable = humanRoster.players.filter((player) => playableOf(player.playerVersionId).includes('C'));
        if (centerCapable.length < 2)
            throw new Error('fixture human roster needs two centers');
        const outgoing = centerCapable.slice(0, 2).map((player) => player.playerVersionId);
        const nonCenters = celticsRoster.players.filter((player) => !playableOf(player.playerVersionId).includes('C'));
        const incoming = nonCenters.slice(0, 2).map((player) => player.playerVersionId);
        if (outgoing.length < 2 || incoming.length < 2)
            throw new Error('fixture lacks swap players');
        ...
        const output = handleSeasonRunCommand(commandOf(withOffer, {
            command: 'accept-trade-offer',
...
describe('decline-trade-offer command', () => {
    it('marks an open offer declined and advances the state chain', () => {
        const { run, context } = windowedFixture();
        const offer = baseOfferOf(run);
        const output = handleSeasonRunCommand(commandOf(run, {
            command: 'decline-trade-offer',
            commandId: 'decline-1',
            windowIndex: 0,
            offerId: offer.offerId,
        ...
        const outputResult = output.result;
        if (outputResult.command !== 'decline-trade-offer')
            throw new Error('unexpected command');
        const result = outputResult.result;
        if (result.status !== 'accepted')
            throw new Error('expected acceptance');
...
describe('resume-season-block command', () => {
    it('accepts a matching pending block without mutating the run', () => {
        ...
        const outputResult = output.result;
        if (outputResult.command !== 'resume-season-block')
            throw new Error('unexpected command');
        const result = outputResult.result;
        if (result.status !== 'accepted')
            throw new Error('expected acceptance');
    ...
    it('rejects no-pending-block, block-mismatch, and rotation-digest-mismatch', () => {
        const { run, context } = windowedFixture();
        const pending = pendingOf(run);
        const noPending = handleSeasonRunCommand(commandOf(run, {
            command: 'resume-season-block',
            commandId: 'resume-nopending',
            blockIndex: 3,
            rotationDigest: '0'.repeat(32),
        }), context);
        if (noPending.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(noPending.result.result.rejection.code).toBe('no-pending-block');
        const mismatch = handleSeasonRunCommand(commandOf(run, {
            command: 'resume-season-block',
            commandId: 'resume-mismatch',
            blockIndex: 4,
            rotationDigest: '0'.repeat(32),
        }), { ...context, run, pending });
        if (mismatch.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(mismatch.result.result.rejection.code).toBe('block-mismatch');
        const digestMismatch = handleSeasonRunCommand(commandOf(run, {
            command: 'resume-season-block',
            commandId: 'resume-digest',
            blockIndex: 3,
            rotationDigest: '1'.repeat(32),
        }), { ...context, run, pending });
        if (digestMismatch.result.result.status !== 'rejected')
            throw new Error('expected rejection');
...
describe('forfeit-interrupted-game command', () => {
    it('accepts a matching forfeit, appends the summary, and advances the pending', () => {
        ...
        const outputResult = output.result;
        if (outputResult.command !== 'forfeit-interrupted-game')
            throw new Error('unexpected command');
        const result = outputResult.result;
        if (result.status !== 'accepted')
            throw new Error('expected acceptance');
    ...
    it('rejects no-pending-block, block-mismatch, and game-mismatch', () => {
        ...
        if (mismatch.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(mismatch.result.result.rejection.code).toBe('block-mismatch');
        const gameMismatch = handleSeasonRunCommand(commandOf(run, {
            command: 'forfeit-interrupted-game',
            commandId: 'forfeit-gamemismatch',
            blockIndex: 3,
            nextGameId: 's000999',
        }), { ...context, run, pending });
        if (gameMismatch.result.result.status !== 'rejected')
            throw new Error('expected rejection');
...
function forcedPostseasonResolver(plan: (home: string, away: string, gameId: string) => 'home' | 'away'): SeasonPostseasonGameResolver {
...
function postseasonFixture(options: {
    seed?: string;
...
function restedRotation(run: SeasonRun, franchiseId: string, catalog: SeasonDraftCatalog) {
    ...
    if (saved === undefined)
        throw new Error(`no rotation for ${franchiseId}`);
...
describe('start-postseason command', () => {
    it('accepts from a completed regular season and moves to the play-in stage', () => {
        ...
        const output = handleSeasonRunCommand(commandOf(context.run, { command: 'start-postseason', commandId: 'start-1' }), context);
        const outputResult = output.result;
        if (outputResult.command !== 'start-postseason')
            throw new Error('unexpected command');
        const result = outputResult.result;
        if (result.status !== 'accepted')
            throw new Error('expected acceptance');
    ...
    it('rejects an incomplete regular season and a run that already started', () => {
        ...
        const incompleteOutput = handleSeasonRunCommand(commandOf(incomplete, { command: 'start-postseason', commandId: 'start-incomplete' }), { ...context, run: incomplete });
        if (incompleteOutput.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(incompleteOutput.result.result.rejection.code).toBe('invalid-stage');
        const started = handleSeasonRunCommand(commandOf(context.run, { command: 'start-postseason', commandId: 'start-1' }), context);
        if (started.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const again = handleSeasonRunCommand(commandOf(started.run, { command: 'start-postseason', commandId: 'start-2' }), { ...context, run: started.run });
        if (again.result.result.status !== 'rejected')
            throw new Error('expected rejection');
    ...
    it('rejects invalid rankings with integrity-failure and a stale state', () => {
        ...
        const output = handleSeasonRunCommand(commandOf(context.run, { command: 'start-postseason', commandId: 'start-bad' }), badRankings);
        if (output.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(output.result.result.rejection.code).toBe('integrity-failure');
        const stale = handleSeasonRunCommand(commandOf({ ...context.run, stateRevision: context.run.stateRevision + 5 }, { command: 'start-postseason', commandId: 'start-stale' }), context);
        if (stale.result.result.status !== 'rejected')
            throw new Error('expected rejection');
    ...
    it('rejects a run mismatch', () => {
        ...
        const mismatch = handleSeasonRunCommand(commandOf(other, { command: 'start-postseason', commandId: 'start-mismatch' }), context);
        if (mismatch.result.result.status !== 'rejected')
            throw new Error('expected rejection');
...
describe('advance-postseason command', () => {
    it('runs the whole tournament when the human rotation stays valid', () => {
        ...
        if (start.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const output = handleSeasonRunCommand(commandOf(start.run, { command: 'advance-postseason', commandId: 'adv-1' }), { ...context, run: start.run });
        const outputResult = output.result;
        if (outputResult.command !== 'advance-postseason')
            throw new Error('unexpected command');
        const result = outputResult.result;
        if (result.status !== 'accepted')
            throw new Error('expected acceptance');
    ...
    it('stops at a human game whose rotation plans minutes for injured players', () => {
        ...
        if (humanRoster === undefined)
            throw new Error('no human roster');
        ...
        const output = handleSeasonRunCommand(commandOf(injured, { command: 'advance-postseason', commandId: 'adv-1' }), { ...context, run: injured });
        const outputResult = output.result;
        if (outputResult.command !== 'advance-postseason')
            throw new Error('unexpected command');
        const result = outputResult.result;
    ...
    it('rejects wrong-game targets, invalid stages, and stale states', () => {
        ...
        if (start.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const firstAdvance = handleSeasonRunCommand(commandOf(start.run, {
            command: 'advance-postseason',
            commandId: 'adv-first',
            targetGameId: 'pi-east-nine-ten',
        }), { ...context, run: start.run });
        if (firstAdvance.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const wrongGame = handleSeasonRunCommand(commandOf(firstAdvance.run, {
            command: 'advance-postseason',
            commandId: 'adv-wrong',
            targetGameId: 'pi-east-seven-eight',
        }), { ...context, run: firstAdvance.run });
        if (wrongGame.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(wrongGame.result.result.rejection.code).toBe('wrong-game');
        if (wrongGame.result.result.rejection.code !== 'wrong-game')
            throw new Error('unexpected');
        expect(wrongGame.result.result.rejection.nextGameId).toBe('pi-east-final');
        const wrongStage = handleSeasonRunCommand(commandOf(context.run, { command: 'advance-postseason', commandId: 'adv-stage' }), context);
        if (wrongStage.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(wrongStage.result.result.rejection.code).toBe('invalid-stage');
        const stale = handleSeasonRunCommand(commandOf({ ...start.run, stateRevision: start.run.stateRevision + 3 }, { command: 'advance-postseason', commandId: 'adv-stale' }), { ...context, run: start.run });
        if (stale.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(stale.result.result.rejection.code).toBe('stale-state');
    });
...
describe('submit-postseason-rotation command', () => {
    ...
    function advancedToHumanGame(): {
        ...
        if (advanceResult.result.status !== 'accepted')
            throw new Error('expected acceptance');
        if (advanceResult.result.nextDecision !== 'rotation' ||
    ...
    it('applies a risky-rehab spend with the recorded ledger entry and transaction', () => {
        ...
        const output = handleSeasonRunCommand(commandOf(withInjuryRun, {
            command: 'submit-postseason-rotation',
            commandId: 'sub-rehab',
            targetGameId: 'pi-west-seven-eight',
        ...
        if (outputResult.command !== 'submit-postseason-rotation')
            throw new Error('unexpected command');
        if (outputResult.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const ledgerEntry = output.run.influence.ledger.find((entry) => entry.commandId === 'sub-rehab');
    ...
    it('rejects wrong-game targets, invalid rotations, unavailable players, and rehab balance', () => {
        const { context, run } = advancedToHumanGame();
        const rotation = restedRotation(run, HUMAN, context.catalog);
        const wrongTarget = handleSeasonRunCommand(commandOf(run, {
            command: 'submit-postseason-rotation',
            commandId: 'sub-wrongtarget',
            targetGameId: 'pi-east-final',
            rotation: { franchiseId: HUMAN, rotation },
        }), context);
        if (wrongTarget.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        ...
        const invalidRotation = handleSeasonRunCommand(commandOf(run, {
            command: 'submit-postseason-rotation',
            commandId: 'sub-invalid',
            targetGameId: 'pi-west-seven-eight',
            rotation: { franchiseId: HUMAN, rotation: broken },
        }), context);
        if (invalidRotation.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        ...
        if (saved === undefined)
            throw new Error('no human rotation');
        const unavailable = handleSeasonRunCommand(commandOf(withInjuryRun, {
            command: 'submit-postseason-rotation',
            commandId: 'sub-unavailable',
            targetGameId: 'pi-west-seven-eight',
            rotation: { franchiseId: HUMAN, rotation: saved },
        }), { ...context, run: withInjuryRun });
        if (unavailable.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        ...
        const insufficient = handleSeasonRunCommand(commandOf(poor, {
            command: 'submit-postseason-rotation',
            commandId: 'sub-poor',
            targetGameId: 'pi-west-seven-eight',
            rotation: {
        ...
        if (firstRehab.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
...
describe('spectate-postseason-game command', () => {
    it('spectates the current game after elimination and rejects wrong targets', () => {
        ...
        if (advanceResult.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const wrongTarget = handleSeasonRunCommand(commandOf(advance.run, {
        ...
        if (wrongTarget.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(wrongTarget.result.result.rejection.code).toBe('wrong-game');
        const output = handleSeasonRunCommand(commandOf(advance.run, {
            command: 'spectate-postseason-game',
            commandId: 'spec-1',
            targetGameId: 'po-west-first-round-1-g2',
        }), { ...context, run: advance.run });
        const outputResult = output.result;
        if (outputResult.command !== 'spectate-postseason-game')
            throw new Error('unexpected command');
        const result = outputResult.result;
        if (result.status !== 'accepted')
            throw new Error('expected acceptance');
    ...
    it('rejects an invalid stage (regular season) and a stale state', () => {
        const context = postseasonFixture();
        const wrongStage = handleSeasonRunCommand(commandOf(context.run, {
            command: 'spectate-postseason-game',
        ...
        if (wrongStage.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(wrongStage.result.result.rejection.code).toBe('invalid-stage');
        const stale = handleSeasonRunCommand(commandOf({ ...context.run, stateRevision: context.run.stateRevision + 1 }, {
            command: 'spectate-postseason-game',
...
describe('fast-forward-postseason command', () => {
    it('completes an eliminated run with the champion and completion state', () => {
        ...
        if (start.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const output = handleSeasonRunCommand(commandOf(start.run, { command: 'fast-forward-postseason', commandId: 'ff-1' }), { ...context, run: start.run });
        const outputResult = output.result;
        if (outputResult.command !== 'fast-forward-postseason')
            throw new Error('unexpected command');
        const result = outputResult.result;
        if (result.status !== 'accepted')
            throw new Error('expected acceptance');
    ...
    it('rejects an active human, an invalid stage, and a bad target', () => {
        const context = postseasonFixture({ resolver: forcedPostseasonResolver(humanWinsEveryGame) });
        const activeStart = handleSeasonRunCommand(commandOf(context.run, { command: 'start-postseason', commandId: 'start-1' }), context);
        if (activeStart.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const activeHuman = handleSeasonRunCommand(commandOf(activeStart.run, {
            command: 'fast-forward-postseason',
            commandId: 'ff-active',
            targetGameId: 'po-east-first-round-1-g1',
        }), { ...context, run: activeStart.run });
        if (activeHuman.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(activeHuman.result.result.rejection.code).toBe('integrity-failure');
        const wrongStage = handleSeasonRunCommand(commandOf(context.run, { command: 'fast-forward-postseason', commandId: 'ff-stage' }), context);
        if (wrongStage.result.result.status !== 'rejected')
            throw new Error('expected rejection');
        expect(wrongStage.result.result.rejection.code).toBe('invalid-stage');
        ...
        const start = handleSeasonRunCommand(commandOf(aiOnly.run, { command: 'start-postseason', commandId: 'start-1' }), aiOnly);
        if (start.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const advance = handleSeasonRunCommand(commandOf(start.run, {
            command: 'advance-postseason',
        ...
        if (advance.result.result.status !== 'accepted')
            throw new Error('expected acceptance');
        const badTarget = handleSeasonRunCommand(commandOf(advance.run, {
            command: 'fast-forward-postseason',
            commandId: 'ff-badtarget',
            targetGameId: 'pi-east-seven-eight',
        }), { ...aiOnly, run: advance.run });
        if (badTarget.result.result.status !== 'rejected')
            throw new Error('expected rejection');
...
describe('campaign commands', () => {
    ...
    it('select-gm-identity generates two offers for the current block', () => {
        ...
        const output = handleSeasonRunCommand(commandOf(context.run, {
