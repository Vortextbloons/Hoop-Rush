import { SEASON_HOME_COURT_VERSION, SEASON_HOME_WIN_RATE_TARGET, type SeasonHomeCourtProfile, } from '@hoop-rush/data-contracts';
...
export function seasonHomeCourtMechanisms(profile: SeasonHomeCourtProfile): SeasonHomeCourtMechanisms {
    const communication = profile.homeDefensiveCommunication;
    return {
        homeDefenseShotAdjustment: communication === 0 ? 0 : -communication * SEASON_HOME_COURT_DEFENSE_MAX_ADJUSTMENT,
        awayTurnoverPressureAdjustment: profile.awayTurnoverPressure * SEASON_HOME_COURT_TURNOVER_MAX_ADJUSTMENT,
    };
}
