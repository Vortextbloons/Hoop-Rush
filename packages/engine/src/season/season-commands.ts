...
function baseValidation(command: SeasonRunCommand, run: SeasonRun, pending: SeasonPendingBlockCandidate | null, context?: SeasonRunCommandContext): SeasonRunCommandOutput | null {
    ...
    if (authority !== null) {
        if (authority.kind === 'season-multiplayer') {
            ...
            const directFid = 'franchiseId' in command && typeof command.franchiseId === 'string'
                ? command.franchiseId
                : undefined;
        ...
        const targetFranchiseId = 'franchiseId' in command && typeof command.franchiseId === 'string'
            ? command.franchiseId
            : null;
...
function handleOpenTradeInquiry(command: SeasonOpenTradeInquiryCommand, context: SeasonRunCommandContext): SeasonRunCommandOutput {
    ...
    if ('error' in result) {
        throw new Error(`open-trade-inquiry failed after validation: ${result.error} window ${String(command.windowIndex)}`);
    }
...
function handleSubmitTradeProposal(command: SeasonSubmitTradeProposalCommand, context: SeasonRunCommandContext): SeasonRunCommandOutput {
    ...
    if (!evalResult.ok) {
        ...
        switch (evalResult.code) {
            ...
            case 'trade-insufficient-talent':
                return rejectedSubmitTradeProposal(command, { code: 'trade-insufficient-talent', reason }, run);
    ...
    if (!inquiryId) {
        ...
        if ('error' in opened) {
            throw new Error(`submit-trade-proposal open inquiry failed after validation: ${opened.error} window ${String(command.windowIndex)}`);
        }
    ...
    if (existingForUpdate) {
        ...
        const newNegotiation: import('@hoop-rush/data-contracts').SeasonTradeNegotiation = {
            inquiryId: inquiryId,
            windowIndex: command.windowIndex,
            fromFranchiseId: franchiseIdSchema.parse(context.humanFranchiseId ??
    ...
    if (evalResult.proposal.influenceAmount > 0 && evalResult.proposal.influenceFromSender) {
        ...
        const updateWindowCash = (franchiseId: string, field: 'tradeCashSent' | 'tradeCashReceived', delta: number) => {
            ...
            if (idx >= 0) {
                ...
                const nw: import('@hoop-rush/data-contracts').SeasonInfluenceWindowState = field === 'tradeCashSent'
...
function handleAdvancePostseason(command: SeasonAdvancePostseasonCommand, context: SeasonRunCommandContext): SeasonRunCommandOutput {
    ...
    const completion = stage === 'completed' && championFranchiseId !== null
        ? {
            championFranchiseId,
...
function freeAgencyRejectionTo(error: FreeAgencyValidationRejection): SeasonRunCommandRejection {
    ...
    if (!parsed.success) {
        throw new Error(`invalid free-agency rejection ${error.rejection.code}: ${parsed.error.message}`);
    }
    return parsed.data;
}
