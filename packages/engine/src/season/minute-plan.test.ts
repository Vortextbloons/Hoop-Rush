...
function playersOf(
  options: { quality?: number; stamina?: number; fatigue?: number } = {},
...
describe('minute-plan legality and structure', () => {
  ...
  it('is deterministic: identical inputs produce identical plans', () => {
    ...
    expect(second.plans.map((plan) => plan.rotation.targetMinutes)).toEqual(
      first.plans.map((plan) => plan.rotation.targetMinutes),
    );
  ...
  it('plans are not flat when quality, stamina, or fatigue differ', () => {
    ...
    players.set('b', player('b', { quality: 0.1, staminaRating: 50, fatigueBasisPoints: 8_000 }));
    const candidates = buildMinutePlanCandidates({
      structure,
      players,
      horizon: 10,
    ...
    for (const plan of candidates.plans) {
      const minutesOf = new Map(
        plan.rotation.targetMinutes.map((row) => [row.playerVersionId, row.minutes]),
      );
...
describe('minute-plan envelopes', () => {
  it('starter share is ordered starter-heavy > balanced > bench-heavy', () => {
    ...
    const starterShare = (plan: { rotation: SeasonRotation }) =>
  ...
  it('falls back to the best score when every plan is heavy-strain', () => {
    const players = playersOf();
    for (const id of IDS) {
      players.set(id, player(id, { staminaRating: 55, fatigueBasisPoints: 9_000 }));
    }
    const candidates = buildMinutePlanCandidates({
      structure: structureOf(),
      players,
...
describe('minute-plan horizon and fatigue forward model', () => {
  ...
  it('projects more fatigue over a longer horizon and respects bands', () => {
    ...
    expect(long.get('a')?.fatigueBasisPoints ?? 0).toBeGreaterThan(
      short.get('a')?.fatigueBasisPoints ?? 0,
    );
  ...
  it('keeps heavy load in the heavy band and fresh load in fresh', () => {
    ...
    expect(fatigueBandOf(1_499)).toBe('fresh');
    expect(fatigueBandOf(1_500)).toBe('ready');
    expect(fatigueBandOf(3_499)).toBe('ready');
    expect(fatigueBandOf(3_500)).toBe('tired');
    expect(fatigueBandOf(5_999)).toBe('tired');
    expect(fatigueBandOf(6_000)).toBe('heavy');
    expect(fatigueBandOf(10_000)).toBe('heavy');
  });

  it('0-minute players still recover through the between-game recovery tick', () => {
    const players = [player('a', { staminaRating: 70, fatigueBasisPoints: 5_000 })];
    const minutes = new Map([['a', 0]]);
    const after = projectFatigueAfterBlock(players, minutes, 2);
    expect(after.get('a')?.fatigueBasisPoints ?? 0).toBeLessThan(5_000);
  });
});
