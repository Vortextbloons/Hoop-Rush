...
import { SEASON_DRAFT_OFFER_SIZE, SEASON_DRAFT_SAFE_MINIMUM, SEASON_ROSTER_TARGETS_VERSION, SEASON_ROTATION_VERSION, eraIdSchema, franchiseIdSchema, playerIdSchema, positionSchema, seasonDraftStateSchema, seasonLeagueGenerationResultSchema, seasonNamespaceSeed, seedSchema, seasonDigestHex, type SeasonDraftCandidate, type SeasonDraftCatalog, type SeasonDraftCommand, type SeasonDraftCommandRecord, type SeasonDraftState, type SeasonDraftAcceptedRecord, type SeasonDraftRejectedRecord, type SeasonDraftOffer, type SeasonAiPool, type SeasonGenerationDiagnostics, type SeasonLeague, type SeasonLeagueGenerationResult, type Seed, } from '@hoop-rush/data-contracts';
import { buildSeasonAiAssignments, buildSeasonDraftCandidate, buildSeasonDraftCatalog, buildSeasonLeague, buildSeasonRotation, buildFixtureEvaluations, seedFromString, } from '@hoop-rush/test-fixtures';
import { applySeasonDraftCommand, seasonDraftStateCanonical, type SeasonAiGenerationDeps, type SeasonAiGenerationInput, } from './draft.ts';
...
function requireState(state: SeasonDraftState | null, what: string): SeasonDraftState {
    if (state === null)
        throw new Error(`expected state after ${what}`);
    return state;
}
...
function buildFakeGeneration(input: FakeGenerationInput): SeasonLeagueGenerationResult {
    ...
    const rosters = input.league.teams.map((team) => {
        ...
        for (const versionId of players)
            owned.add(versionId);
        const resolved = players.map((playerVersionId) => {
            ...
            if (!candidate)
                throw new Error('fake generation: missing candidate');
            return {
                playerVersionId,
    ...
    const rotations = rosters.map((roster) => buildSeasonRotation(roster.franchiseId, roster.players.map((p) => p.playerVersionId)));
...
function createSolo(catalog: SeasonDraftCatalog = CATALOG, league: SeasonLeague = LEAGUE, rootSeed: Seed | string = SEED) {
    return applySeasonDraftCommand(null, catalog, cmd('c-create', 0, {
...
function createDuo(catalog: SeasonDraftCatalog = FULL_CATALOG, league: SeasonLeague = LEAGUE, rootSeed: Seed | string = SEED) {
    return applySeasonDraftCommand(null, catalog, cmd('c-create', 0, {
        kind: 'create-season-draft',
        runId: 'run-1',
        rootSeed: seedSchema.parse(rootSeed),
...
function pickBestSelectable(state: SeasonDraftState, catalog: SeasonDraftCatalog, commandId: string): SeasonDraftState {
    const offer = state.currentOffer;
    if (offer === null)
        throw new Error('no offer drawn for the pick');
    const candidates = catalog.candidates
        .filter((c) => offer.cards.some((card) => card.playerVersionId === c.playerVersionId))
        .filter((c) => offer.cards.find((card) => card.playerVersionId === c.playerVersionId)?.selectable)
    ...
    if (candidates.length === 0)
        throw new Error('no selectable card in the offer');
    const first = candidates[0];
    if (first === undefined)
        throw new Error('no selectable card in the offer');
    const result = applySeasonDraftCommand(state, catalog, cmd(commandId, state.revision, {
    ...
    if (result.record.status !== 'accepted' || result.state === null) {
        throw new Error(`pickBestSelectable rejected: ${result.record.status === 'rejected' ? result.record.message : 'no state'}`);
    }
    return result.state;
}
function drawAndPick(state: SeasonDraftState, catalog: SeasonDraftCatalog, commandIdPrefix: string, sequence: number): {
    ...
    const drawn = applySeasonDraftCommand(state, catalog, cmd(`${commandIdPrefix}-draw-${String(sequence)}`, state.revision, {
    ...
    if (offer === null)
        throw new Error('no offer after draw');
    return {
        state: pickBestSelectable(drawnState, catalog, `${commandIdPrefix}-pick-${String(sequence)}`),
        offer,
    };
}
function playToFinalized(catalog: SeasonDraftCatalog, league: SeasonLeague, rootSeed: Seed | string, deps: SeasonAiGenerationDeps, ids: string[], commandIdPrefix: string): SeasonDraftState {
    const created = applySeasonDraftCommand(null, catalog, cmd(`${commandIdPrefix}-create`, 0, {
        kind: 'create-season-draft',
        runId: 'run-1',
        rootSeed: seedSchema.parse(rootSeed),
        league,
    ...
    const finalized = applySeasonDraftCommand(state, catalog, cmd(`${commandIdPrefix}-finalize`, state.revision, { kind: 'finalize-human-rosters' }), deps);
    expectAccepted(finalized.record);
    return requireState(finalized.state, 'finalize');
}
function playFullDraft(catalog: SeasonDraftCatalog, league: SeasonLeague, rootSeed: Seed | string, deps: SeasonAiGenerationDeps, ids: string[], commandIdPrefix: string): {
    state: SeasonDraftState;
    generation: SeasonLeagueGenerationResult;
} {
    const finalized = playToFinalized(catalog, league, rootSeed, deps, ids, commandIdPrefix);
    const generated = applySeasonDraftCommand(finalized, catalog, cmd(`${commandIdPrefix}-generate`, finalized.revision, { kind: 'generate-ai-league' }), deps);
...
function customCatalog(entries: Array<{
    franchiseId: string;
    eraId: string;
    positions: string[][];
}>): SeasonDraftCatalog {
    ...
    for (const entry of entries) {
        ...
        for (let i = 0; i < 12; i += 1) {
            ...
            if (!playable || playable.length === 0)
                throw new Error('customCatalog missing positions');
...
function stateWithNinePicks(catalog: SeasonDraftCatalog, rootSeed: Seed | string): SeasonDraftState {
    ...
    if (picks.length !== 9)
        throw new Error('expected nine hand picks');
...
describe('season draft create', () => {
    ...
    it('is deterministic for the same seed and different for another seed', () => {
        ...
        expect(seasonDraftStateCanonical(requireState(a.state, 'create'))).toBe(seasonDraftStateCanonical(requireState(b.state, 'create')));
        const c = createSolo(CATALOG, LEAGUE, seedFromString('another-seed'));
        expect(seasonDraftStateCanonical(requireState(c.state, 'create'))).not.toBe(seasonDraftStateCanonical(requireState(a.state, 'create')));
    });
    it('rejects creation when a draft already exists', () => {
        const first = createSolo();
        const result = applySeasonDraftCommand(first.state, CATALOG, cmd('c-create-2', first.state?.revision ?? 0, {
            kind: 'create-season-draft',
            runId: 'run-2',
            rootSeed: SEED,
            league: LEAGUE,
    ...
    it('rejects non-create commands before any draft exists', () => {
        const result = applySeasonDraftCommand(null, CATALOG, cmd('c-draw', 0, { kind: 'draw-season-offer', participantId: 'p1' }), fakeDeps());
    ...
    it('rejects an invalid catalog and an invalid league', () => {
        ...
        const result = applySeasonDraftCommand(null, badCatalog as SeasonDraftCatalog, cmd('c-create', 0, {
            kind: 'create-season-draft',
            runId: 'run-1',
            rootSeed: SEED,
            league: LEAGUE,
        ...
        const result2 = applySeasonDraftCommand(null, CATALOG, cmd('c-create-2', 0, {
            kind: 'create-season-draft',
            runId: 'run-1',
            rootSeed: SEED,
            league: badLeague,
    ...
    it('rejects creation when the catalog has fewer than eight candidates', () => {
        ...
        const result = applySeasonDraftCommand(null, small, cmd('c-create', 0, {
            kind: 'create-season-draft',
            runId: 'run-1',
            rootSeed: SEED,
            league: LEAGUE,
    ...
    it('rejects duplicate participant ids', () => {
        const result = applySeasonDraftCommand(null, CATALOG, cmd('c-create', 0, {
            kind: 'create-season-draft',
            runId: 'run-1',
            rootSeed: SEED,
            league: LEAGUE,
...
function handOffer(catalog: SeasonDraftCatalog, participantId: string, round: number, pickOrdinal: number, cards: Array<{
...
describe('season draft offers', () => {
    it('draws exactly eight distinct cards with at least three selectable', () => {
        ...
        const drawn = applySeasonDraftCommand(state, CATALOG, cmd('c-draw-1', 1, { kind: 'draw-season-offer', participantId: 'p1' }), fakeDeps());
        expectAccepted(drawn.record);
        ...
        expect(offer?.cards.filter((c) => c.selectable).length).toBeGreaterThanOrEqual(SEASON_DRAFT_SAFE_MINIMUM);
    ...
    it('rejects draw for the wrong participant and re-drawing is an accepted no-op', () => {
        ...
        const wrongTurn = applySeasonDraftCommand(state, FULL_CATALOG, cmd('c-draw-wrong', 1, { kind: 'draw-season-offer', participantId: 'p2' }), fakeDeps());
        expect(expectRejected(wrongTurn.record).errorCode).toBe('WRONG_TURN');
        const drawn = applySeasonDraftCommand(state, FULL_CATALOG, cmd('c-draw-1', 1, {
            kind: 'draw-season-offer',
            participantId: state.firstPickParticipantId,
        }), fakeDeps());
        expectAccepted(drawn.record);
        const next = requireState(drawn.state, 'draw');
        const again = applySeasonDraftCommand(next, FULL_CATALOG, cmd('c-draw-1b', next.revision, {
            kind: 'draw-season-offer',
            participantId: state.firstPickParticipantId,
        }), fakeDeps());
    ...
    it('reproduces offers byte-for-byte from identical seeds and commands', () => {
        const first = playFullDraft(FULL_CATALOG, LEAGUE, seedFromString('offer-repro'), fakeDeps(), ['p1'], 'or');
        const second = playFullDraft(FULL_CATALOG, LEAGUE, seedFromString('offer-repro'), fakeDeps(), ['p1'], 'or2');
    ...
    it('rejects a draw with NO_FEASIBLE_GLOBAL_OFFER when fewer than three safe candidates remain', () => {
        ...
        const result = applySeasonDraftCommand(state, catalog, cmd('c-draw-10', 10, { kind: 'draw-season-offer', participantId: 'p1' }), fakeDeps());
    ...
    it('rejects a draw when fewer than eight unowned candidates remain', () => {
        ...
        const state = requireState(createSolo(tiny, LEAGUE, seedFromString('few-left')).state, 'create');
        ...
        const result = applySeasonDraftCommand(onePick, tiny, cmd('c-draw', 2, { kind: 'draw-season-offer', participantId: 'p1' }), fakeDeps());
        expect(expectRejected(result.record).errorCode).toBe('NO_FEASIBLE_GLOBAL_OFFER');
...
describe('season draft picks', () => {
    ...
    it('rejects picks for the wrong turn, no offer, out-of-offer, owned, and disabled cards', () => {
        ...
        const first = state.firstPickParticipantId;
        const other = first === 'p1' ? 'p2' : 'p1';
        const drawn = applySeasonDraftCommand(state, FULL_CATALOG, cmd('c-draw-1', 1, { kind: 'draw-season-offer', participantId: first }), fakeDeps());
        const next = requireState(drawn.state, 'draw');
        const offer = next.currentOffer;
        expect(offer).not.toBeNull();
        const inOffer = offer?.cards[0];
        if (!inOffer)
            throw new Error('no offer cards');
        const outOfOffer = FULL_CATALOG.candidates.find((c) => !offer.cards.some((card) => card.playerVersionId === c.playerVersionId));
        if (!outOfOffer)
            throw new Error('no out-of-offer candidate');
        const wrongTurn = applySeasonDraftCommand(next, FULL_CATALOG, cmd('c-pick-wrong', next.revision, {
            kind: 'select-draft-player',
            participantId: other,
            playerVersionId: inOffer.playerVersionId,
        }), fakeDeps());
        expect(expectRejected(wrongTurn.record).errorCode).toBe('WRONG_TURN');
        const noOffer = applySeasonDraftCommand(state, FULL_CATALOG, cmd('c-pick-nooffer', 1, {
            kind: 'select-draft-player',
            participantId: first,
            playerVersionId: inOffer.playerVersionId,
        }), fakeDeps());
        expect(expectRejected(noOffer.record).errorCode).toBe('NO_OFFER_DRAWN');
        const outside = applySeasonDraftCommand(next, FULL_CATALOG, cmd('c-pick-out', next.revision, {
            kind: 'select-draft-player',
            participantId: first,
        ...
        const ownedAgain = applySeasonDraftCommand(ownedTurn, FULL_CATALOG, cmd('c-pick-owned', ownedTurn.revision, {
            kind: 'select-draft-player',
            participantId: first,
            playerVersionId: inOffer.playerVersionId,
        }), fakeDeps());
        ...
        const withDisabled = seasonDraftStateSchema.parse({
            ...
            currentOffer: {
                ...offer,
                cards: offer.cards.map((card, index) => index === 0
                    ? {
                        ...card,
                        selectable: false,
                        coverageReason: 'Selecting this version would leave the 4G/4F/3C completion targets unreachable with the remaining picks',
                    }
                    : card),
            },
        });
        const disabled = applySeasonDraftCommand(withDisabled, FULL_CATALOG, cmd('c-pick-disabled', withDisabled.revision, {
            kind: 'select-draft-player',
            participantId: first,
            playerVersionId: inOffer.playerVersionId,
        }), fakeDeps());
    ...
    it('rejects a pick when the roster is already full', () => {
        ...
        const extra = FULL_CATALOG.candidates.find((c) => !walk.picks.some((p) => p.playerVersionId === c.playerVersionId));
        if (!extra)
            throw new Error('no extra candidate');
        const offerCard = FULL_CATALOG.candidates.find((c) => c.playerVersionId !== extra.playerVersionId &&
            !walk.picks.some((p) => p.playerVersionId === c.playerVersionId));
        if (!offerCard)
            throw new Error('no offer card candidate');
        ...
        const result = applySeasonDraftCommand(fullRoster, FULL_CATALOG, cmd('c-pick-full', fullRoster.revision, {
            kind: 'select-draft-player',
            participantId: 'p1',
    ...
    it('owns versions (not players) so same-person versions coexist', () => {
        ...
        const lakers90 = catalog.candidates.find((c) => c.franchiseId === 'lakers' && c.eraId === '1990s');
        const lakers00 = catalog.candidates.find((c) => c.franchiseId === 'lakers' && c.eraId === '2000s');
        if (!lakers90 || !lakers00)
        ...
        const state = requireState(createSolo(catalog, LEAGUE, seedFromString('same-person')).state, 'create');
        ...
        const first = applySeasonDraftCommand(withOffer1, catalog, cmd('c-pick-90', 1, {
            kind: 'select-draft-player',
            participantId: 'p1',
        ...
        const second = applySeasonDraftCommand(withOffer2, catalog, cmd('c-pick-00', 2, {
            kind: 'select-draft-player',
            participantId: 'p1',
    ...
    it('rejects the same player identity across duo human rosters', () => {
        ...
        const lakers = catalog.candidates.find((c) => c.franchiseId === 'lakers' && c.eraId === '1990s');
        const celtics = catalog.candidates.find((c) => c.franchiseId === 'celtics' && c.eraId === '1990s');
        if (!lakers || !celtics)
            throw new Error('candidates missing');
        celtics.playerId = lakers.playerId;
        celtics.playerVersionId = `pv-${seasonDigestHex(`duo-${lakers.playerId}`)}`;
        const celticsPool = catalog.pools.find((p) => p.franchiseId === 'celtics' && p.eraId === '1990s');
        if (!celticsPool)
            throw new Error('pool missing');
        ...
        if (firstPicker === null)
            throw new Error('missing first picker');
        ...
        const first = applySeasonDraftCommand(withOffer1, catalog, cmd('c-pick-first', 1, {
            kind: 'select-draft-player',
        ...
        const second = applySeasonDraftCommand(withOffer2, catalog, cmd('c-pick-second', 2, {
            kind: 'select-draft-player',
...
describe('season draft exact version ownership', () => {
    it('never offers an exact version that was already picked, in any later offer', () => {
        const { state } = playFullDraft(FULL_CATALOG, LEAGUE, seedFromString('ownership'), fakeDeps(), ['p1', 'p2'], 'ow');
        const picksByOffer = new Map(state.offers.map((offer, index) => {
            const pick = state.picks[index];
            if (!pick)
                throw new Error(`no pick for offer ${String(index)}`);
        ...
        state.offers.forEach((offer, index) => {
            for (const card of offer.cards) {
                ...
                if (pickedAt !== undefined && pickedAt < index) {
                    throw new Error(`offer ${String(index)} re-offers ${card.playerVersionId} picked at offer ${String(pickedAt)}`);
                }
            }
...
describe('season draft finalize and generation', () => {
    it('rejects finalize before the roster is complete', () => {
        const state = requireState(createSolo().state, 'create');
        const result = applySeasonDraftCommand(state, CATALOG, cmd('c-finalize', 1, { kind: 'finalize-human-rosters' }), fakeDeps());
        expect(expectRejected(result.record).errorCode).toBe('UNCOMPLETABLE_ROSTER');
    });
    it('rejects legacy v1 commands with UNSUPPORTED_COMMAND', () => {
        const state = requireState(createSolo().state, 'create');
        const reveal = applySeasonDraftCommand(state, CATALOG, cmd('c-reveal', 1, { kind: 'reveal-draft-roll', participantId: 'p1' }), fakeDeps());
        expect(expectRejected(reveal.record).errorCode).toBe('UNSUPPORTED_COMMAND');
        const claim = applySeasonDraftCommand(state, CATALOG, cmd('c-claim', 1, {
    ...
    it('finalizes and generates the AI league with the injected generator', () => {
        const { state, generation } = playFullDraft(FULL_CATALOG, LEAGUE, seedFromString('happy-path'), fakeDeps(), ['p1'], 'hp');
    ...
    it('rejects generate before finalize and records GENERATION_EXHAUSTED failures', () => {
        const state = requireState(createSolo().state, 'create');
        const early = applySeasonDraftCommand(state, CATALOG, cmd('c-gen', 1, { kind: 'generate-ai-league' }), fakeDeps());
        ...
        const finalized = playToFinalized(FULL_CATALOG, LEAGUE, seedFromString('exhausted'), fakeDeps(), ['p1'], 'ex');
        const result = applySeasonDraftCommand(finalized, FULL_CATALOG, cmd('c-gen-2', finalized.revision, { kind: 'generate-ai-league' }), exploding);
...
describe('season draft idempotency, revisions, and replay', () => {
    it('is idempotent for duplicate command ids', () => {
        const state = requireState(createSolo().state, 'create');
        const drawn = applySeasonDraftCommand(state, CATALOG, cmd('c-draw-dup', 1, { kind: 'draw-season-offer', participantId: 'p1' }), fakeDeps());
        const next = requireState(drawn.state, 'draw');
        const before = seasonDraftStateCanonical(next);
        const again = applySeasonDraftCommand(next, CATALOG, cmd('c-draw-dup', 1, { kind: 'draw-season-offer', participantId: 'p1' }), fakeDeps());
    ...
    it('rejects stale revisions and logs the rejection', () => {
        ...
        const stale = applySeasonDraftCommand(state, CATALOG, cmd('c-draw-stale', 99, { kind: 'draw-season-offer', participantId: 'p1' }), fakeDeps());
    ...
    it('replays byte-for-byte continuously and from the command log', () => {
        const deps = fakeDeps();
        const continuous = playFullDraft(FULL_CATALOG, LEAGUE, seedFromString('replay'), deps, ['p1'], 'rp');
    ...
    it('survives chunked property replay with interleaved stale commands', () => {
        const deps = fakeDeps();
        fc.assert(fc.property(fc.integer({ min: 1, max: 40 }), (chunk) => {
            const continuous = playFullDraft(FULL_CATALOG, LEAGUE, seedFromString('chunked'), deps, ['p1'], 'ck');
            const commands = continuous.state.commandLog
                .filter((r) => r.status === 'accepted')
                .map((r) => r.command);
            ...
            for (let start = 0; start < commands.length; start += chunk) {
                ...
                for (const command of slice) {
                    const stale = applySeasonDraftCommand(walk, FULL_CATALOG, { ...command, commandId: `${command.commandId}-stale`, expectedRevision: 9999 }, deps);
                    if (walk !== null) {
                        expect(expectRejected(stale.record).errorCode).toBe('STALE_REVISION');
                    }
    ...
    it('completes a two-human draft with snake reversal across ten rounds', () => {
        const { state } = playFullDraft(FULL_CATALOG, LEAGUE, seedFromString('snake'), fakeDeps(), ['p1', 'p2'], 'sn');
        expect(state.status).toBe('complete');
        expect(state.picks.filter((p) => p.participantId === 'p1')).toHaveLength(10);
        ...
        for (const pick of state.picks) {
            const offer = state.offers.find((o) => o.participantId === pick.participantId &&
                o.round === pick.round &&
    ...
    it('the AI generation never reuses a human-selected version', () => {
        const { state, generation } = playFullDraft(FULL_CATALOG, LEAGUE, seedFromString('ai-exclusion'), fakeDeps(), ['p1', 'p2'], 'ai');
        ...
        for (const roster of generation.rosters) {
            if (humanFranchises.has(roster.franchiseId))
                continue;
...
describe('season draft offer seed derivation', () => {
    ...
    it('derives different offers for different seeds', () => {
        const a = requireState(createSolo(FULL_CATALOG, LEAGUE, seedFromString('offer-a')).state, 'create');
        const b = requireState(createSolo(FULL_CATALOG, LEAGUE, seedFromString('offer-b')).state, 'create');
        const offerA = requireState(applySeasonDraftCommand(a, FULL_CATALOG, cmd('c-draw-a', 1, { kind: 'draw-season-offer', participantId: 'p1' }), fakeDeps()).state, 'draw-a').currentOffer;
        const offerB = requireState(applySeasonDraftCommand(b, FULL_CATALOG, cmd('c-draw-b', 1, { kind: 'draw-season-offer', participantId: 'p1' }), fakeDeps()).state, 'draw-b').currentOffer;
        expect(offerA?.cards.map((c) => c.playerVersionId)).not.toEqual(offerB?.cards.map((c) => c.playerVersionId));
    });
});
