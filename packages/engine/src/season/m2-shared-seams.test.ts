...
describe('season rotation (season-rotation-v2)', () => {
  it('builds a deterministic 240-minute rotation with starters as closing five', () => {
    ...
    const starterMinutes = rotation.targetMinutes.filter((m) =>
      rotation.starters.includes(m.playerVersionId),
    );
    const benchMinutes = rotation.targetMinutes.filter((m) =>
      rotation.benchOrder.includes(m.playerVersionId),
    );
