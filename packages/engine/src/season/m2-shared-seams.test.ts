...
import { eraIdSchema, franchiseIdSchema, playerIdSchema, seasonKeySchema, seedSchema, } from '@hoop-rush/data-contracts';
...
describe('season generation digest', () => {
    it('is canonical regardless of input order', () => {
        ...
        const rotation = (id: string) => buildSeasonRotation(id, Array.from({ length: 10 }, (_, i) => versionId(i + 1)));
        const base = {
            seed: seedSchema.parse(seed),
            aiVersion: 'season-ai-v2',
            rosterGenerationVersion: 'roster-generation-v2',
