import { SEASON_GAME_SUMMARY_VERSION, type SeasonCompactInjuryEvent, type SeasonCompactPlayerLine, type SeasonEffectsRollup, type SeasonGameEffectsTransition, type SeasonGameSimulationResult, type SeasonGameSummary, type SeasonMechanismEvidence, type SeasonRetainedGameDetail, type SeasonScheduleGame, type SeasonTeamBox, } from '@hoop-rush/data-contracts';
function compactLineOf(player: {
    ...
    turnovers: number;
    fouls: number;
}, started: boolean): SeasonCompactPlayerLine {
    return {
...
function sortedLines(lines: readonly SeasonCompactPlayerLine[]): SeasonCompactPlayerLine[] {
    return [...lines].sort((a, b) => a.playerVersionId < b.playerVersionId ? -1 : a.playerVersionId > b.playerVersionId ? 1 : 0);
}
...
function openingStartersOf(result: SeasonGameSimulationResult, side: 'home' | 'away'): Set<string> {
    ...
    for (const stint of result.unitStints) {
        if (stint.side !== side || stint.period !== 1)
            continue;
...
export function seasonGameSummaryFromResult(result: SeasonGameSimulationResult, game: SeasonScheduleGame, effectsTransition?: SeasonGameEffectsTransition, injuryEvents: readonly SeasonCompactInjuryEvent[] = []): SeasonGameSummary {
    if (result.outcome === 'no-legal-five-both') {
        throw new Error(`season summary: game ${game.gameId} has no legal five on either side ` +
    ...
    return {
        ...
        homePlayers: sortedLines(result.home.players.map((player) => compactLineOf(player, openingStartersOf(result, 'home').has(player.playerVersionId)))),
        awayPlayers: sortedLines(result.away.players.map((player) => compactLineOf(player, openingStartersOf(result, 'away').has(player.playerVersionId)))),
...
export function seasonRetainedDetailFromResult(result: SeasonGameSimulationResult, game: SeasonScheduleGame, runId: string, effectsTransition?: SeasonGameEffectsTransition, injuryEvents: readonly SeasonCompactInjuryEvent[] = []): SeasonRetainedGameDetail {
    return {
        schemaVersion: 1,
        runId,
        gameId: game.gameId,
...
export function seasonEffectsRollupFromEvidence(evidence: readonly SeasonMechanismEvidence[]): SeasonEffectsRollup[] {
...
export function seasonEffectsEvidenceOf(transition: SeasonGameEffectsTransition): SeasonMechanismEvidence[] {
...
export function auditSeasonGameSummary(summary: SeasonGameSummary): string[] {
    ...
    for (const side of sides) {
        ...
        if (summary[`${side}Score`] !== box.points) {
            failures.push(`${side} score ${String(summary[`${side}Score`])} != box points ${String(box.points)}`);
        }
        if (lines.length !== 10) {
        ...
        const sumOf = (select: (line: SeasonCompactPlayerLine) => number): number => lines.reduce((acc, line) => acc + select(line), 0);
        const checks: Array<[
            string,
            number,
            number
        ...
        for (const [label, playerTotal, boxTotal] of checks) {
            if (playerTotal !== boxTotal) {
                failures.push(`${side} player ${label} (${String(playerTotal)}) != box ${label} (${String(boxTotal)})`);
            }
        }
    ...
    if (summary.status === 'forfeit') {
        ...
        for (const event of summary.injuryEvents) {
            ...
            if (!event.returned && event.returnClock !== null) {
                failures.push(`injury event for ${event.playerVersionId} carries a return clock without returning`);
            }
        }
        return failures;
    }
    ...
    }
    {
        ...
        for (const event of summary.injuryEvents) {
            ...
            if (rosteredSide === undefined) {
                ...
                failures.push(`injury event side ${event.side} does not match the rostered side ${rosteredSide} for ${event.playerVersionId}`);
            }
            ...
            if (!event.returned && event.returnClock !== null) {
                failures.push(`non-returned injury event for ${event.playerVersionId} carries a return clock`);
            }
        }
    }
    return failures;
