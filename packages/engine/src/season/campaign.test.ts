...
import { SEASON_CAMPAIGN_VERSION, buildEmptyCampaignState, commandIdSchema, franchiseIdSchema, seasonNamespaceSeed, seedSchema, } from '@hoop-rush/data-contracts';
...
describe('campaign branching', () => {
    ...
    it('breakthrough may unlock ambitious follow-up recorded on offer', () => {
        ...
        expect(after.campaignState.rewardEntitlements.followUpUnlocks).toContain(breakthroughReward.rewardId);
...
describe('campaign evolution', () => {
    it('generates evolution offers after block 4 with correct seed path', () => {
        ...
        expect(evolved.evolutionSelection?.resultingIdentity).toBe(firstEvolutionOffer.resultingIdentity);
    });
});
