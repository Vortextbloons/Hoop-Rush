...
import { SEASON_MINUTE_POLICY_VERSION, SEASON_NEUTRAL_HOME_COURT, SEASON_ROTATION_VERSION, SEASON_HOME_WIN_RATE_TARGET, franchiseIdSchema, playerIdSchema, seasonGameSimulationInputSchema, seasonHomeCourtProfileSchema, playerVersionId, seedSchema, type Position, type SeasonGameSimulationInput, type SeasonGameTeamInput, type SeasonRotation, } from '@hoop-rush/data-contracts';
...
import { SEASON_HOME_COURT_DEFENSE_MAX_ADJUSTMENT, SEASON_HOME_COURT_PROFILE, SEASON_HOME_COURT_TURNOVER_MAX_ADJUSTMENT, seasonHomeCourtMechanisms, } from './home-court.ts';
...
function buildInput(seed: string, homeCourt: SeasonGameSimulationInput['homeCourt']): SeasonGameSimulationInput {
...
describe('season home-court seam (M2.3)', () => {
    ...
    it('derives exactly two signed bounded mechanisms from a profile', () => {
        ...
        expect(Math.abs(mechanisms.homeDefenseShotAdjustment)).toBeLessThanOrEqual(SEASON_HOME_COURT_DEFENSE_MAX_ADJUSTMENT);
        expect(mechanisms.awayTurnoverPressureAdjustment).toBeLessThanOrEqual(SEASON_HOME_COURT_TURNOVER_MAX_ADJUSTMENT);
        ...
        expect(higher.awayTurnoverPressureAdjustment).toBeGreaterThan(mechanisms.awayTurnoverPressureAdjustment);
