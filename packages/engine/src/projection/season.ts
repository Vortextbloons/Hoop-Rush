import type { BaseFiveProjectionInput, Position, ProjectionModelArtifact, SeasonProjection, SeasonProjectionInput, SeasonProjectionMetrics, SeasonProjectionPlanFacts, SeasonProjectionUnit, SimulationPlayer, } from '@hoop-rush/data-contracts';
...
import { benchReliefOf, fatigueBandOf, MINUTE_PLAN_HEAVY_THRESHOLD_BP, projectFatigueAfterBlock, riskScoreOf, type FatigueBand, type MinutePlanPlayerInput, } from '../season/minute-plan.ts';
...
import { traceContext, traceRotationClose, traceRotationNormal, type RotationTraceResult, } from './rotation-trace.ts';
...
function blendedTrace(normal: RotationTraceResult, close: RotationTraceResult, closeWeight: number): Map<string, {
    minutes: number;
    players: readonly string[];
}> {
...
function planFactsOf(input: {
    ...
    const { rotation, allVersions, players, byVersion, minutePlan, profile, model, cache, metrics } = input;
    ...
    const minutesByVersion = new Map(rotation.targetMinutes.map((row) => [row.playerVersionId, row.minutes]));
    const fatigue = projectFatigueAfterBlock(minutePlanPlayers, minutesByVersion, minutePlan.horizonGames);
    const maxStarterStrainBasisPoints = Math.max(0, ...rotation.starters.map((versionId) => fatigue.get(versionId)?.fatigueBasisPoints ?? 0));
    ...
    const heavyStrain = fatigueBands.heavy > 0 ||
...
export function projectSeasonRoster(input: SeasonProjectionInput, options: SeasonProjectionOptions = {}): SeasonProjection {
    const cache = options.cache ?? new ProjectionCache();
    const roster = input.roster;
    if (roster.length !== 10) {
        throw new Error(`projection: Season roster must have exactly 10 players (got ${String(roster.length)})`);
    }
    ...
    const memberPlayable = new Map<string, readonly Position[]>(members.map((member) => [member.playerVersionId, member.playable]));
    ...
    const totalMinutes = Math.max(1, [...blended.values()].reduce((sum, entry) => sum + entry.minutes, 0));
    ...
    const benchHeavyStarters = benchHeavyTrace.units.find((unit) => unitKeyOf(unit.players) === benchHeavyKey);
    ...
    const minutes = allVersions.map((versionId) => {
        const target = input.rotation.targetMinutes.find((row) => row.playerVersionId === versionId)?.minutes ?? 0;
    ...
    const weightedRating = (picker: (unit: SeasonProjectionUnit) => number) => {
        ...
        return (weighted.reduce((sum, unit) => sum + unit.weight * picker(unit), 0) / Math.max(1e-9, total));
    };
    ...
    const weightedUnitStrength = weighted.reduce((sum, unit) => sum + unit.weight * weightedNet(unit), 0);
    ...
    const mixedQuality = mixedUnits.length > 0
        ? mixedUnits.reduce((sum, unit) => sum + unit.weight * weightedNet(unit), 0) /
            Math.max(1e-9, mixedUnits.reduce((sum, unit) => sum + unit.weight, 0))
        : weightedNetRating;
    ...
    const foulResilience = highestFoul !== undefined
        ? (contingencyUnits.find((unit) => unit.unitId === `contingency-${highestFoul.playerVersionId ?? ''}`)?.base.offense.defense.score ?? 0)
        : 0;
    ...
    const matchupMean = matchupNets.length > 0 ? matchupNets.reduce((s, v) => s + v, 0) / matchupNets.length : 0;
    ...
    const secondBest = (picker: (unit: SeasonProjectionUnit) => number) => {
        ...
        if (best === undefined || second === undefined || best <= 0)
            return 100;
        return Math.min(100, Math.max(0, (second / best) * 100));
    };
    const redundancy = Math.min(100, secondBest((unit) => unit.base.offense.creation.score) * 0.5 +
    ...
    const planFacts = input.minutePlan === undefined
        ? undefined
        : planFactsOf({
            rotation: input.rotation,
            allVersions,
    ...
    const inputDigest = seasonDigestHex(JSON.stringify({
