...
import { RATINGS_VERSION, contentHashSchema, eraIdSchema, playerIdSchema, seedSchema, type EraId, type ProjectionModelArtifact, } from '@hoop-rush/data-contracts';
import { DEFAULT_ERA_SIM_PROFILE } from '@hoop-rush/test-fixtures';
import { buildHumanSeasonRoster, projectSeasonRoster, rankCandidates, searchRosterRotationCandidates, } from '../projection/index.ts';
...
describe('searchRosterRotationCandidates', () => {
    ...
    it('finds complete legal candidates and is deterministic', () => {
        ...
        expect(second.ranked.map((candidate) => candidate.candidateId)).toEqual(first.ranked.map((candidate) => candidate.candidateId));
        ...
        for (const candidate of first.ranked) {
            ...
            const memberPlayable = new Map(rosterVersions.map((id) => {
    ...
    it('produces optimizer rotations carrying minutePolicy and plan facts', () => {
        ...
        for (const candidate of result.ranked) {
            ...
            expect(['starter-heavy', 'balanced', 'bench-heavy']).toContain(candidate.rotation.minutePolicy.strategy);
    ...
    it('produces dynamic (quality/stamina-driven) minute allocations', () => {
        ...
        const minutesOf = new Map((rotation?.targetMinutes ?? []).map((row) => [row.playerVersionId, row.minutes]));
        expect(minutesOf.get(locked[0] as string) ?? 0).not.toBe(minutesOf.get(locked[1] as string) ?? 0);
...
describe('buildHumanSeasonRoster', () => {
    it('preserves locked picks and returns a legal selected roster', () => {
        ...
        for (const pick of locked)
            expect(selected).toContain(pick);
        ...
        const memberPlayable = new Map(auditRoster.map((id) => {
            const member = catalog.candidates.find((candidate) => candidate.playerVersionId === id);
            return [id, member?.positions.playable ?? []];
        }));
        expect(auditSeasonRotation(result.rotation as NonNullable<typeof result.rotation>, memberPlayable)).toEqual([]);
