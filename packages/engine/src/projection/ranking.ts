...
export function rankingVectorOf(
  ...
  return {
    ...
    rebounding: normalizeComponent(
      model,
      'rebounding',
      orebRate * 100 * 0.4 + drebRate * 100 * 0.6,
    ),
...
export function hardGateReasons(gates: RankingGates, projection: SeasonProjection): string[] {
  ...
  if (!gates.coverageOk) reasons.push('positional coverage targets not met');
  if (!gates.bandOk) reasons.push('band quota violation');
  if (!gates.anchorsOk) reasons.push('anchor guarantee violation');
  if (!gates.ownershipOk) reasons.push('ownership violation');
  if (!gates.rolesOk) reasons.push('role coverage violation');
  if (!gates.feasibilityOk) reasons.push('future feasibility violation');
...
export function rankCandidates(input: {
  ...
  for (const candidate of candidates) {
    ...
    const finalScore =
      model.weights.basketballMean * basketballMean +
