...
import { baseFiveProjectionSchema, RATINGS_VERSION, playerIdSchema, projectionModelArtifactSchema, type BaseFiveProjection, type ProjectionMatchupArchetype, type ProjectionModelArtifact, type SimulationPlayer, } from '@hoop-rush/data-contracts';
...
function lineupPlayer(index: number, ratings?: Partial<SimulationPlayer['ratings']>, tendencies?: Partial<SimulationPlayer['tendencies']>): SimulationPlayer {
    const player = buildSimulationPlayer({
        playerId: playerIdSchema.parse(`p-l${String(index + 1)}`),
        displayName: `L ${String(index + 1)}`,
        positions: POSITIONS[index],
    });
    if (ratings !== undefined)
        player.ratings = { ...player.ratings, ...ratings };
    if (tendencies !== undefined)
        player.tendencies = { ...player.tendencies, ...tendencies };
    return player;
}
...
function buildProjectionLineup(ratings?: Partial<SimulationPlayer['ratings']>, tendencies?: Partial<SimulationPlayer['tendencies']>): BaseFiveProjection {
...
describe('projectBaseFive', () => {
    it('produces a schema-valid projection with sane ratings', () => {
        ...
        expect(parsed.ratings.netRating).toBe(parsed.ratings.offensiveRating - parsed.ratings.defensiveRatingAllowed);
    ...
    it('keeps ledger accounting exact', () => {
        ...
        expect(ledger.effectiveFieldGoalPct).toBeCloseTo((ledger.fieldGoalMakes + 0.5 * ledger.threePointMakes) / ledger.fieldGoalAttempts, 10);
        expect(ledger.trueShootingPct).toBeCloseTo(ledger.points / (2 * (ledger.fieldGoalAttempts + 0.44 * ledger.freeThrowAttempts)), 10);
        ...
        expect(ledger.twoPointAttempts + ledger.threePointAttempts).toBeCloseTo(ledger.fieldGoalAttempts, 10);
    ...
    it('is sensitive to shooting, creation, security, defense, and rebounding', () => {
        const base = buildProjectionLineup();
        const shooters = buildProjectionLineup({ threePoint: 95, midrange: 95, insideScoring: 95, freeThrow: 90 }, { threePointRate: 40 });
        expect(shooters.ratings.offensiveRating).toBeGreaterThan(base.ratings.offensiveRating);
        expect(shooters.offense.ledger.effectiveFieldGoalPct).toBeGreaterThan(base.offense.ledger.effectiveFieldGoalPct);
        const creators = buildProjectionLineup({ ballHandling: 95, passing: 95, offensiveIq: 95 }, { usageRate: 34 });
        ...
        expect(defense.ratings.defensiveRatingAllowed).toBeLessThan(base.ratings.defensiveRatingAllowed);
        const rebounders = buildProjectionLineup({ offensiveRebound: 95, defensiveRebound: 95, vertical: 95 }, { crashOffensiveGlassRate: 40 });
        expect(rebounders.offense.ledger.offensiveReboundRate).toBeGreaterThan(base.offense.ledger.offensiveReboundRate);
    });
    it('records weaknesses from thresholds', () => {
        const weak = projectBaseFive({
            lineup: [
                {
                    player: lineupPlayer(0, { ballHandling: 30, passing: 30, offensiveIq: 30 }, { usageRate: 5 }),
                    slot: 'G1',
                },
                {
                    player: lineupPlayer(1, { ballHandling: 30, passing: 30, offensiveIq: 30 }, { usageRate: 5 }),
                    slot: 'G2',
                },
                {
                    player: lineupPlayer(2, { ballHandling: 30, passing: 30, offensiveIq: 30 }, { usageRate: 5 }),
                    slot: 'F1',
                },
                {
                    player: lineupPlayer(3, { ballHandling: 30, passing: 30, offensiveIq: 30 }, { usageRate: 5 }),
                    slot: 'F2',
                },
                {
                    player: lineupPlayer(4, { ballHandling: 30, passing: 30, offensiveIq: 30 }, { usageRate: 5 }),
                    slot: 'C',
                },
            ],
            eraProfile: DEFAULT_ERA_SIM_PROFILE,
...
describe('projectExpectedLedger', () => {
    it('keeps both directions exact and cross-terms reconciled', () => {
        ...
        expect(result.offense.ledger.steals).toBeCloseTo(result.defense.ledger.turnovers *
            Math.min(0.9, Math.max(0.3, DEFAULT_ERA_SIM_PROFILE.parameters.stealShareOfTurnovers)), 9);
