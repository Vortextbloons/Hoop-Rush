...
function smallModel(): ProjectionModelArtifact {
  ...
  return projectionModelArtifactSchema.parse({
    ...
    references: {
      '1990s': {
        neutral: referenceFive('neutral'),
        archetypes: [
          referenceFive('perimeter'),
          referenceFive('interior'),
          referenceFive('pressure'),
          referenceFive('size-switch'),
        ],
      },
    },
    scales: {
...
function byVersionOf(players: readonly SimulationPlayer[]) {
  return new Map<string, SimulationPlayer>(
    players.map((player) => [player.playerVersionId ?? player.playerId, player]),
  );
}
function loadOf(
...
function lineupOf(players: readonly string[], byVersion: ReadonlyMap<string, SimulationPlayer>) {
  const slotOf = (index: number): BaseFiveProjectionInput['lineup'][number]['slot'] =>
    index === 4 ? 'C' : index >= 2 ? (index === 2 ? 'F1' : 'F2') : index === 0 ? 'G1' : 'G2';
...
describe('projectedQualityWeights', () => {
  ...
  it('derives starter quality from the starters unit contribution rows', () => {
    ...
    if (starterId === undefined) throw new Error('fixture starters missing');
    ...
    expect(quality.get(starterId)).toBe(
      Math.max(0, Math.min(1, ((offense?.expectedPoints ?? 0) + defensiveContribution) / 200)),
    );
  });
  ...
  it('recommends starter-heavy for a healthy high-stamina star team', () => {
    ...
    const starTeam = players.map((player) => {
      ...
      const ratings = Object.fromEntries(
        Object.entries(player.ratings).map(([key]) => [key, star ? 90 : 55]),
