import type { BaseFiveProjectionInput, EraSimulationProfile, Position, ProjectionModelArtifact, SeasonMinutePolicyStrategy, SeasonRotation, SimulationPlayer, } from '@hoop-rush/data-contracts';
import { buildMinutePlanCandidates, MINUTE_POLICY_STRATEGIES, riskScoreOf, type MinutePlanCandidate, type MinutePlanPlayerInput, } from '../season/minute-plan.ts';
...
export function projectedQualityWeights(input: {
    ...
    const members = new Map<string, readonly Position[]>(players.map((player) => [player.playerVersionId ?? player.playerId, player.positions]));
    const units: Array<{
        players: readonly string[];
    ...
    const benchHeavyTrace = traceRotationNormal(traceContext({ rotation: benchHeavyRotation, members }));
    const startersKey = unitKeyOf(rotation.starters);
    const benchHeavyUnit = benchHeavyTrace.units.find((unit) => unitKeyOf(unit.players) === startersKey)?.players ??
        benchHeavyTrace.units[0]?.players;
    if (benchHeavyUnit !== undefined)
        push(benchHeavyUnit);
    ...
    for (const player of players) {
        ...
        result.set(versionId, Math.max(0, Math.min(1, (offense.expectedPoints + defensiveContribution) / 200)));
    }
    return result;
}
...
export function optimizeSeasonRotation(input: {
    ...
    const memberPlayable = new Map<string, readonly Position[]>(players.map((player) => [player.playerVersionId ?? player.playerId, player.positions]));
    ...
    const minutePlanPlayers = new Map<string, MinutePlanPlayerInput>(players.map((player) => {
    ...
    const projected = built.plans.map((plan) => {
        const projection = projectSeasonRoster({ roster, rotation: plan.rotation, eraProfile, model }, { cache });
        return {
            ...plan,
    ...
    const relative = new Map<SeasonMinutePolicyStrategy, number>(projected.map((plan) => [
        plan.strategy,
        maxNet <= minNet
            ? 0.5
