...
function normalize(
  raw: number,
  key: string,
  scale?: { baseline: number; perPoint: number; min: number; max: number },
): number {
...
function coverageOf(players: readonly SimulationPlayer[], prep: TeamPrep): ProjectionDefense {
  ...
  const rimProtection =
    ratingMean(players, 'block') * 0.6 + ratingMean(players, 'interiorDefense') * 0.4;
  const stealOpportunity =
    ratingMean(players, 'steal') * 0.7 + tendencyMean(players, 'stealAttemptRate') * 0.3;
  const blockOpportunity =
    ratingMean(players, 'block') * 0.7 + tendencyMean(players, 'blockAttemptRate') * 0.3;
  const foulExposure = tendencyMean(players, 'foulRate');
  const heightScore = mean(
    players.map((player) =>
      player.heightInches === null ? 0 : (Math.max(0, player.heightInches - 72) / 24) * 100,
    ),
  );
  const defensiveRebounding =
    ratingMean(players, 'defensiveRebound') * 0.6 +
...
function creationOf(input: {
  ...
  return {
    ...
    initiatorShare: Object.fromEntries(
      SLOT_ORDER.map((slot, index) => [slot, initiatorShares[index] ?? 0]),
    ) as ProjectionCreation['initiatorShare'],
...
function validateAndBuildTeam(input: BaseFiveProjectionInput): SimulationTeam {
  ...
  for (const entry of input.lineup) {
    ...
    if (!canPlay(entry.player.positions, SLOT_GROUP[entry.slot])) {
      throw new Error(
        `projection: ${entry.player.playerId} (${entry.player.positions.join('/')}) cannot fill ${SLOT_GROUP[entry.slot]} slot ${entry.slot}`,
      );
    }
  }
  const ordered = SLOT_ORDER.map((slot) => {
    const entry = input.lineup.find((candidate) => candidate.slot === slot);
    if (entry === undefined) throw new Error(`projection: missing slot ${slot}`);
    return entry.player;
  });
  return {
...
function sideOf(input: {
  ...
  spacingRaw: number;
  spacingScale?: { baseline: number; perPoint: number; min: number; max: number };
  creationScale?: { baseline: number; perPoint: number; min: number; max: number };
}): ProjectionSide {
  const {
    facts,
    players,
  ...
  const contributions: ProjectionPlayerContribution[] = facts.players.map((row) => ({
    ...
    spacingContribution:
      ((row.player.ratings.threePoint / 100) *
  ...
  return {
    ...
    zones: facts.zones,
    shooters: Object.fromEntries(
      SLOT_ORDER.map((slot, index) => [slot, facts.shooters[index] ?? 0]),
    ) as ProjectionSide['shooters'],
