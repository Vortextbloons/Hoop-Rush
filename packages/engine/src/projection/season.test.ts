...
import { playerIdSchema, projectionModelArtifactSchema, seasonProjectionSchema, RATINGS_VERSION, type ProjectionMatchupArchetype, type ProjectionModelArtifact, type SimulationPlayer, } from '@hoop-rush/data-contracts';
import { DEFAULT_ERA_SIM_PROFILE } from '@hoop-rush/test-fixtures';
import { buildInput } from './season.test-helpers.ts';
import { ProjectionCache, projectSeasonRoster, traceRotationClose, traceRotationNormal, traceContext, } from '../projection/index.ts';
...
describe('rotation trace', () => {
    it('covers exactly 48 regulation minutes across legal units', () => {
        ...
        const memberPlayable = new Map(players.map((player) => [player.playerVersionId ?? '', player.positions]));
    ...
    it('is deterministic and applies the closing window in close games', () => {
        ...
        const closeUnits = close.units.some((unit) => unit.players.includes(rotation.closingFive[0] ?? ''));
...
describe('projectSeasonRoster plan facts', () => {
    ...
    it('attaches plan facts mirroring the rotation minute policy', () => {
        ...
        const bandTotal = (facts?.fatigueBands.fresh ?? 0) +
    ...
    it('orders starter strain by the rotation target minutes', () => {
        ...
        expect(heavy.planFacts?.starterStrainAfterBlock).toBeGreaterThan(light.planFacts?.starterStrainAfterBlock ?? 0);
    });
    it('changes the digest when plan facts differ but not the input digest', () => {
        ...
        expect(loaded.planFacts?.starterStrainAfterBlock).toBeGreaterThan(fresh.planFacts?.starterStrainAfterBlock ?? 0);
