...
export function shrunkRatio(numerator: number, denominator: number, prior: number, priorAttempts = 80): number {
...
export function anchorsForPlayer(player: PoolPlayer): PlayerAnchors {
    ...
    const offensiveKnown = stats['offensiveRebounds'] !== null && stats['offensiveRebounds'] !== undefined;
    const defensiveKnown = stats['defensiveRebounds'] !== null && stats['defensiveRebounds'] !== undefined;
    const hasSplit = offensiveKnown &&
        defensiveKnown &&
    ...
    return {
        ...
        fieldGoalPct: shrunkRatio(stats['fieldGoalsMade'] ?? 0, stats['fieldGoalsAttempted'] ?? 0, 0.45),
        threePointPct: (stats['threesAttempted'] ?? 0) > 0
            ? shrunkRatio(stats['threesMade'] ?? 0, stats['threesAttempted'] ?? 0, 0.34)
            : null,
        freeThrowPct: shrunkRatio(stats['freeThrowsMade'] ?? 0, stats['freeThrowsAttempted'] ?? 0, 0.75),
        threePointAttemptRate: ratio(stats['threesAttempted'] ?? 0, stats['fieldGoalsAttempted'] ?? 0, 0),
        freeThrowAttemptRate: ratio(stats['freeThrowsAttempted'] ?? 0, stats['fieldGoalsAttempted'] ?? 0, 0.2),
    };
}
export function buildOpponentArtifact(pool: {
    players: PoolPlayer[];
}): OpponentArtifact {
...
export function run(options?: {
    ...
    for (const player of artifact.players) {
        console.log(`${player.playerId}: ${player.displayName} ${player.positions.join('/')} ft=${String(player.ratings.freeThrow)} pass=${String(player.ratings.passing)}`);
    }
