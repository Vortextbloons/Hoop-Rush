import { THREE_POINT_RECONSTRUCTION_VERSION, type ReconstructedThreePointProfile, type ThreePointReconstructionArtifact, } from '@hoop-rush/data-contracts';
import { normalQuantile, sigmoid } from './math.ts';
import { extractRawFeatures, missingFeatureCount, type FeatureContext, type ReconstructionRow, } from './rows.ts';
export function featureContextFromArtifact(artifact: ThreePointReconstructionArtifact): FeatureContext {
    return {
...
export function posteriorPrediction(coefficients: readonly number[], covariance: readonly (readonly number[])[], x: readonly number[], quantile: number): PosteriorPrediction {
...
export function predictReconstructedProfile(artifact: ThreePointReconstructionArtifact, row: ReconstructionRow): ReconstructedProfileResult {
    ...
    const accuracy = posteriorPrediction(coefficientsOf(artifact.models.accuracy), artifact.models.accuracy.covariance, [1, ...x], artifact.posteriorQuantiles.accuracy);
    const attempt = posteriorPrediction(coefficientsOf(artifact.models.attemptRate), artifact.models.attemptRate.covariance, [1, ...x], artifact.posteriorQuantiles.attemptRate);
    const thresholds = artifact.confidenceThresholds;
    const confidence: ReconstructedThreePointProfile['confidence'] = accuracy.stdDev < thresholds.highStdDev && missing === 0
        ? 'high'
...
export function ratingFromAccuracy(artifact: ThreePointReconstructionArtifact, accuracy: number): number {
    const points = [...artifact.ratingMapping.points].sort((a, b) => a.accuracy - b.accuracy);
    const first = points[0];
    const last = points[points.length - 1];
    if (first === undefined || last === undefined)
        return artifact.ratingMapping.clampMin;
    if (accuracy <= first.accuracy)
        return first.rating;
