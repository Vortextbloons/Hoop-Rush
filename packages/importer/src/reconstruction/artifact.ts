...
import { RATINGS_VERSION, THREE_POINT_RECONSTRUCTION_VERSION, DERIVATION_METHOD_VERSION, threePointReconstructionArtifactSchema, type ReconstructionMetrics, type ThreePointReconstructionArtifact, } from '@hoop-rush/data-contracts';
import { PUBLIC_DATA } from '../config.ts';
import { fileExists } from '../json.ts';
import { fitModel, pickLambda, predictConservative, runGroupedHoldout, validateTranslatedAttemptRate, } from './cross-validation.ts';
import { round6 } from './predict.ts';
import { MODERN_VALIDATION_SEASONS, RECONSTRUCTION_FEATURE_NAMES, RECONSTRUCTION_SEASONS, buildFeatureContext, cohortPriors, extractRawFeatures, featureVector, loadCohortRows, type ReconstructionRow, } from './rows.ts';
export function reconstructionArtifactPath(): string {
    return join(PUBLIC_DATA, 'three-point-reconstruction-v1.json');
}
export function weightedPercentile(values: readonly {
    value: number;
    weight: number;
...
export function buildRatingMapping(values: readonly number[]): ThreePointReconstructionArtifact['ratingMapping'] {
...
export const ATTEMPT_TRANSLATION = {
    ...
    description: 'conservative modern translation: reconstructed early-era attempt volume scaled 2.5x and capped per position below modern-era norms (G ~0.30, F ~0.20, C ~0.08); accuracy is never translated',
};
export function fitThreePointReconstruction(rows: readonly ReconstructionRow[] = loadCohortRows(), modernRows: readonly ReconstructionRow[] = loadCohortRows([...MODERN_VALIDATION_SEASONS])): FitResult {
    ...
    const holdout = runGroupedHoldout(rows, context, { accuracyLambda: lambdas.accuracyLambda, attemptRateLambda: lambdas.attemptRateLambda }, priors);
    const translatedAttemptRateModern = validateTranslatedAttemptRate(modernRows, context, attemptModel, ATTEMPT_TRANSLATION);
    const gates = {
    ...
    const coefficients = (model: ReturnType<typeof fitModel>['fitted']) => RECONSTRUCTION_FEATURE_NAMES.reduce<Record<string, number>>((acc, name, j) => {
    ...
    const artifact: ThreePointReconstructionArtifact = {
        ...
        fitCohort: {
            ...
            description: '1979-80 through 1983-84 early three-point cohort; the shot was nascent and closest to the world pre-1979 players would face',
        },
        featureNames: [...RECONSTRUCTION_FEATURE_NAMES],
        normalization: Object.fromEntries(RECONSTRUCTION_FEATURE_NAMES.map((name, j) => [
            name,
            {
...
export function loadThreePointReconstructionArtifact(path: string = reconstructionArtifactPath()): ThreePointReconstructionArtifact {
...
function loadThreePointReconstructionArtifactUncached(path: string): ThreePointReconstructionArtifact {
    if (!fileExists(path)) {
        throw new Error(`three-point reconstruction artifact missing at ${path}; run \`hoop-rush calibrate three-point --write\` first`);
    }
    const parsed = threePointReconstructionArtifactSchema.safeParse(JSON.parse(readFileSync(path, 'utf8')) as unknown);
    if (!parsed.success) {
        throw new Error(`three-point reconstruction artifact ${path} fails validation: ${parsed.error.issues[0]?.message ?? 'unknown'}`);
    }
    return parsed.data;
}
export function writeThreePointReconstructionArtifact(artifact: ThreePointReconstructionArtifact, path: string = reconstructionArtifactPath()): void {
    threePointReconstructionArtifactSchema.parse(artifact);
...
export function validateThreePointReconstructionArtifact(artifact: ThreePointReconstructionArtifact): {
    valid: boolean;
    failures: string[];
} {
