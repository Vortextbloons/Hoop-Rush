...
export function attemptWeightedMean(numerator: readonly (number | null)[], denominator: readonly (number | null)[]): number | null {
    let total = 0;
    let weight = 0;
...
export interface FeatureContext {
    ...
    missingDefaults: Record<PositionGroup, {
        heightInches: number;
        weightLbs: number;
        age: number;
    }>;
...
export function extractRawFeatures(row: ReconstructionRow, context: FeatureContext): RawFeatures {
    ...
    const ftPctShrunk = row.ftm !== null && row.fta !== null ? (row.ftm + ftPrior * 80) / (row.fta + 80) : ftPrior;
    const twoPct = ratio(row.fgm !== null && row.tpm !== null ? row.fgm - row.tpm : null, row.fga !== null && row.tpa !== null ? row.fga - row.tpa : null);
...
export function loadCohortRows(seasons: readonly string[] = [...RECONSTRUCTION_SEASONS]): ReconstructionRow[] {
    ...
    for (let s = 0; s < seasons.length; s += 1) {
        ...
        for (const stats of statsList) {
            ...
            const row: ReconstructionRow = {
                ...
                heightInches: intOrNull(stats.heightInches) ??
                    (rosterPlayer ? intOrNull(rosterPlayer.heightInches) : null),
                weightLbs: intOrNull(stats.weightLbs) ?? (rosterPlayer ? intOrNull(rosterPlayer.weightLbs) : null),
...
export function buildFeatureContext(rows: readonly ReconstructionRow[]): FeatureContext {
    ...
    const missingDefaults = {} as Record<PositionGroup, {
        heightInches: number;
        weightLbs: number;
        age: number;
    }>;
    for (const group of ['G', 'F', 'C'] as const) {
        ...
        ftPriors[group] =
            attemptWeightedMean(groupRows.map((row) => row.ftm), groupRows.map((row) => row.fta)) ?? 0.75;
        twoPctMeans[group] =
            attemptWeightedMean(groupRows.map((row) => (row.fgm !== null && row.tpm !== null ? row.fgm - row.tpm : null)), groupRows.map((row) => (row.fga !== null && row.tpa !== null ? row.fga - row.tpa : null))) ?? 0.45;
...
export function cohortPriors(rows: readonly ReconstructionRow[]): {
    ...
    const accuracyPrior = attemptWeightedMean(rows.map((row) => row.tpm), rows.map((row) => row.tpa));
    const attemptRatePrior = attemptWeightedMean(rows.map((row) => row.tpa), rows.map((row) => row.fga));
    return {
