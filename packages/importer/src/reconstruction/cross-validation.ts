...
import { RECONSTRUCTION_FEATURE_NAMES, extractRawFeatures, featureVector, type FeatureContext, type ReconstructionRow, } from './rows.ts';
...
export function eligibleRows(rows: readonly ReconstructionRow[], model: ReconstructionModel): ReconstructionRow[] {
    if (model === 'accuracy')
        return rows.filter((row) => row.tpa !== null && row.tpa > 0);
...
export function standardize(raw: readonly number[][], normalization: Normalization): number[][] {
    return raw.map((row) => row.map((value, j) => (value - (normalization.mean[j] ?? 0)) / (normalization.std[j] ?? 1)));
}
...
export function computeNormalization(raw: readonly number[][]): Normalization {
    ...
    for (const row of raw) {
        for (let j = 0; j < k; j += 1)
            mean[j] = (mean[j] ?? 0) + (row[j] ?? 0);
    }
    for (let j = 0; j < k; j += 1)
        mean[j] = (mean[j] ?? 0) / Math.max(1, raw.length);
...
export function fitModel(rows: readonly ReconstructionRow[], context: FeatureContext, model: ReconstructionModel, lambda: number, priors: PriorInput, maxIterations = 40, tolerance = 1e-9): FittedModel {
    ...
    const priorTrials = model === 'accuracy' ? priors.accuracyPriorAttempts : priors.attemptRatePriorTrials;
    const penalties = RECONSTRUCTION_FEATURE_NAMES.map((name) => name === 'isGuard' || name === 'isCenter' ? lambda * DUMMY_FEATURE_PENALTY_MULTIPLIER : lambda);
    return {
        fitted: fitBinomialLogistic(design, makes, trials, lambda, priorRate * priorTrials, priorTrials, maxIterations, tolerance, penalties),
        normalization,
    };
}
export function predictConservative(model: FittedModel, rawFeatures: readonly number[], quantile: number): number {
    const standardized = rawFeatures.map((value, j) => (value - (model.normalization.mean[j] ?? 0)) / (model.normalization.std[j] ?? 1));
...
export function validateTranslatedAttemptRate(modernRows: readonly ReconstructionRow[], context: FeatureContext, attemptModel: FittedModel, translation: AttemptTranslation): ReconstructionMetrics {
...
const FP_FN: Record<ReconstructionModel, {
    threshold: number;
    lowerBound: number;
    upperBound: number;
}> = {
...
export function runGroupedHoldout(rows: readonly ReconstructionRow[], context: FeatureContext, lambdas: {
    accuracyLambda: number;
    attemptRateLambda: number;
}, priors: PriorInput, foldCount = 5): {
...
export function metricsOf(predictions: readonly HeldOutPrediction[], model: ReconstructionModel): ReconstructionMetrics {
    ...
    for (const p of predictions) {
        ...
        bias += error * p.weight;
        if (p.prediction > p.observed)
            overprediction += p.weight;
        addBand(byPosition, p.row.positionGroup, p, error);
        addBand(byEvidence, evidenceBandOf(p.row.minutes), p, error);
        if (p.prediction >= threshold && p.observed < lowerBound)
            falsePositives += 1;
        if (p.prediction < threshold && p.observed > upperBound)
            falseNegatives += 1;
    }
    return {
...
function addBand(bands: Map<string, {
    mae: number;
    bias: number;
    count: number;
    weight: number;
}>, key: string, p: HeldOutPrediction, error: number): void {
...
function bandMetrics(stats: {
    ...
    if (stats === undefined || stats.weight <= 0)
        return { mae: 0, bias: 0, count: 0 };
    return {
...
export function evidenceBandOf(minutes: number): string {
    if (minutes < 500)
        return 'under-500-min';
    if (minutes < 1500)
        return '500-1499-min';
    return '1500-plus-min';
}
export const LAMBDA_GRID = [0.5, 1, 2, 4] as const;
export function pickLambda(rows: readonly ReconstructionRow[], context: FeatureContext, priors: PriorInput, foldCount = 5): {
    accuracyLambda: number;
    attemptRateLambda: number;
} {
    let bestAccuracy: {
    ...
    for (const lambda of LAMBDA_GRID) {
        const report = runGroupedHoldout(rows, context, { accuracyLambda: lambda, attemptRateLambda: lambda }, priors, foldCount);
