...
const FP_FN: Record<
  ReconstructionModel,
  { threshold: number; lowerBound: number; upperBound: number }
> = {
...
export function runGroupedHoldout(
  rows: readonly ReconstructionRow[],
  context: FeatureContext,
  lambdas: { accuracyLambda: number; attemptRateLambda: number },
  priors: PriorInput,
  foldCount = 5,
): { accuracy: ReconstructionMetrics; attemptRate: ReconstructionMetrics; foldCount: number } {
...
export function metricsOf(
  ...
  const byPosition = new Map<
    string,
    { mae: number; bias: number; count: number; weight: number }
  >();
...
function addBand(
  bands: Map<string, { mae: number; bias: number; count: number; weight: number }>,
  key: string,
  p: HeldOutPrediction,
  error: number,
): void {
...
function bandMetrics(
  stats: { mae: number; bias: number; count: number; weight: number } | undefined,
): { mae: number; bias: number; count: number } {
...
export function pickLambda(
  ...
  priors: PriorInput,
  foldCount = 5,
): { accuracyLambda: number; attemptRateLambda: number } {
  let bestAccuracy: { lambda: number; mae: number } | null = null;
  let bestAttempt: { lambda: number; mae: number } | null = null;
