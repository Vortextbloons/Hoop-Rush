...
import { CONFIDENCE_POLICY_VERSION, DATA_VERSION, MIN_TEAM_GAMES, POSITION_NORMALIZATION_VERSION, RATINGS_VERSION, SCHEMA_VERSION, SELECTION_SCORE_VERSION, allPoolTargets, buildStats, sanitizeAnchors, candidateKey, computePool, defaultPoolWorkers, loadBbrefIds, loadCareerPositionLabels, normalizePoolOveralls, overallBandForPercentile, parsePoolTargets, partitionPoolTargets, run, rawOverallScoreFor, selectionScore, type Candidate, type Manifest, type Pool, type PoolBuildFailure, } from './compute.ts';
...
const FULL_RATINGS_55: Record<string, number> = Object.fromEntries(Object.entries(FULL_RATINGS_60).map(([key, value]) => [key, value - 5]));
...
function stint(season: string, pid: string, games: number, minutes: number): Record<string, unknown> {
    return { season, playerExternalId: pid, teamExternalId: TEAM, gamesPlayed: games, minutes };
}
function statsRow(season: string, pid: string, overrides: Record<string, unknown> = {}): Record<string, unknown> {
    return {
...
function writeSeason(root: FixtureRoot, season: string, roster: RosterSpec[], stints: Array<Record<string, unknown>>, stats: Array<Record<string, unknown>>): void {
...
describe('buildStats', () => {
    it('truncates floats to ints like Python int() and maps keys', () => {
        const stats = buildStats(statsRow('1991-92', '9', {
            gamesPlayed: 60.9,
            minutes: 645.7,
            points: 156.2,
            rebounds: 50.5,
    ...
    it('maps null and non-numeric advanced stats to null', () => {
        const stats = buildStats(statsRow('1991-92', '5', {
            per: null,
            boxPlusMinus: null,
            usageRate: null,
            tsPct: null,
...
describe('selectionScore', () => {
    ...
    it('treats null usage as 0 and guards zero team games', () => {
        ...
        expect(selectionScore(60, 60, 60, 10, 30, 0)).toBe(58.176);
    });
});
describe('rawOverallScoreFor', () => {
    it('prefers raw and canonical profile values over normalized summary overall', () => {
        expect(rawOverallScoreFor({ ratingProfile: { rawOverallScore: 64, canonicalOverall: 62 } }, { overallRating: 91 })).toBe(64);
        expect(rawOverallScoreFor({ ratingProfile: { canonicalOverall: 62 } }, { overallRating: 91 })).toBe(62);
    });
...
describe('normalizePoolOveralls', () => {
    function row(playerId: string, franchiseId: string, rawOverallScore?: number, canonicalOverall?: number) {
        return {
            playerId,
            franchiseId,
            seasonKey: '1996-97',
...
describe('computePool (fixture)', () => {
    ...
    it('applies the 40-game stint rule, joins, and selects peaks by tie-break key', () => {
        ...
        expect(alpha?.selectionScore).toBe(60.731);
        ...
        expect(bravo?.seasonKey).toBe('1991-92');
        expect(bravo?.selectionScore).toBe(59.752);
        expect(bravo?.eligibility.teamGames).toBe(60);
        expect(bravo?.eligibility.teamMinutes).toBe(1200);
        ...
        expect(foxtrot?.selectionScore).toBe(54.805);
        ...
        expect(hotel?.selectionScore).toBe(54.805);
...
describe('computePool error and skip paths', () => {
    ...
    it('logs a warning instead of annotating assets (stays in annotate-markers.mjs)', () => {
        ...
        expect(messages(log).some((m) => m.includes('headshot/photo asset annotation stays in scripts/annotate-markers.mjs'))).toBe(true);
    });
});
describe('loadBbrefIds', () => {
    it('warns and returns {} when the cache file is missing', () => {
        ...
        expect(messages(log)).toContain('  [WARN] bbref_ids.json missing; run fetch_bbref_ids or run_all (no altIds)');
    });
...
describe('loadCareerPositionLabels', () => {
    ...
    it('scans packaged rosters (primary + secondary labels) and writes the versioned cache when missing', () => {
        ...
        expect(JSON.parse(readFileSync(join(root.cache, 'career-position-labels-v5.json'), 'utf8'))).toEqual(expect.objectContaining({ '1': ['SG'], '5': ['C'] }));
    ...
    it('collects non-empty secondaryPositions labels alongside the primary label', () => {
        ...
        expect(JSON.parse(readFileSync(join(root.cache, 'career-position-labels-v5.json'), 'utf8'))).toEqual(expect.objectContaining({ '1': ['PG', 'SG'], '2': ['PG'] }));
        expect(messages(log).some((m) => m.includes('[OK] career position labels for'))).toBe(true);
    });
});
...
describe('candidateKey', () => {
    it('orders by score, minutes, games, then earlier season', () => {
        const mk = (season: string, minutes: number, games: number, usage: number | null): Candidate => ({
            season,
        ...
        const higherScore = mk('1992-93', 2400, 80, 30);
...
describe('run / writePool / updateManifest', () => {
    it('writes a valid pool, digests it, and merges the manifest sorted by key', async () => {
        ...
        expect(messages(log).some((m) => m.includes('[OK] wrote lakers-1990s.json (6 players, '))).toBe(true);
