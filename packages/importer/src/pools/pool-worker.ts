...
const careerLabels = data.careerLabels === null
    ? null
    : new Map(data.careerLabels.map(([pid, labels]) => [pid, new Set(labels)] as [
        string,
        Set<string>
    ]));
const results: PoolWorkerResult['results'] = [];
for (const [franchiseId, eraId] of data.targets) {
    results.push(buildPoolForTarget(franchiseId, eraId, data.manifest, data.bbrefIds, data.withAssets, careerLabels, true));
}
