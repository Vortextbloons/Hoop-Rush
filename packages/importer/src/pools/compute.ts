...
import { COHORT_NORMALIZATION_VERSION, DERIVATION_METHOD_VERSION, LINEAGE_RULE_VERSION, POSITION_NORMALIZATION_VERSION, RATINGS_VERSION, POOL_SCHEMA_VERSION, REQUIRED_RATING_KEYS, SELECTION_SCORE_VERSION, SOURCE_VERSION, overallBandForPercentile, franchiseEraPoolSchema, coverageSummarySchema, playerSeasonStatsSchema, provenanceMapSchema, ratingProfileSchema, reconstructedThreePointProfileSchema, simulationAnchorsSchema, simulationRatingsSchema, simulationTendenciesSchema, summaryRatingsSchema, unavailabilityReasonSchema, type Confidence, type CoverageSummary, type PeakPlayerSeason, type PlayerSeasonStats, type ProvenanceMap, type RatingProfile, type SimulationAnchors, type SimulationRatings, type SimulationTendencies, type SummaryRatings, type UnavailabilityReason, } from '@hoop-rush/data-contracts';
...
import { clamp, clampUnitInterval, fileExists, readJsonLoose, safeFloat, safeInt, sha256FileWithRetry, writeJson, writeJsonRetry, } from '../json.ts';
...
import { LINEAGE_SEGMENTS, MODERN_SLOTS, firstSupportedSeason, resolveHistoricalIdentity, } from '../lineage.ts';
...
export function loadCareerPositionLabels(): Map<string, Set<string>> {
    ...
    const cacheObject = Object.fromEntries([...labelsByPlayer.entries()].map(([pid, labels]) => [pid, [...labels].sort()]));
...
function refreshedFallbackPlayer(player: RosterInput, season: string, stats: SeasonStatsInput, playerExternalId: string): RosterInput {
    const position = str(player.position) || 'SF';
    const height = player.heightInches;
    const heightInches = typeof height === 'number' && Number.isFinite(height) ? Math.trunc(height) : 78;
...
function loadFallbackRosterPlayers(): Map<string, RosterInput> {
    if (fallbackRosterCache !== null)
        return fallbackRosterCache;
        ...
        try {
            ...
            for (const value of parsed.data.players) {
                ...
                const secondaryPositions = Array.isArray(positionsRaw?.secondary)
                    ? positionsRaw.secondary.filter((secondary): secondary is string => typeof secondary === 'string' && secondary !== '')
                    : [];
...
export function partitionPoolTargets(targets: ReadonlyArray<[
    string,
    string
]>, workers: number): Array<Array<[
    string,
    string
]>> {
    ...
    const byEra = new Map<string, Array<[
        string,
        string
    ]>>();
...
function runPoolChunk(chunk: Array<[
    string,
    string
]>, manifest: Manifest, bbrefIds: Record<string, string>, careerLabels: Map<string, Set<string>>, withAssets: boolean): Promise<PoolWorkerResult> {
    return runWorker<PoolWorkerResult>(new URL('./pool-worker.ts', import.meta.url), {
        targets: chunk,
        manifest,
        bbrefIds,
        careerLabels: [...careerLabels.entries()].map(([pid, labels]) => [pid, [...labels]] as [
            string,
            string[]
        ]),
        withAssets,
...
export function buildStats(seasonStats: unknown): PoolStats {
    ...
    const threesMade = threesAttempted !== null && threesMadeRaw !== null
...
export type SummaryRatingsRaw = Partial<Pick<SummaryRatings, 'overallRating' | 'offenseRating' | 'defenseRating'>>;
...
export function rawOverallScoreFor(player: OverallScoreInput, summary: SummaryRatingsRaw | undefined): number {
...
export function selectionScore(rawOverallScore: number, offenseRating: number, defenseRating: number, _usageRate: number | null, teamMinutes: number, teamGames: number): number {
...
export function candidateKey(candidate: Candidate): readonly number[] {
    ...
    return [
        selectionScore(rawOverallScoreFor(candidate.player, summary), safeFloat(summary?.offenseRating), safeFloat(summary?.defenseRating), nullableFrom(candidate.stats.usageRate), minutes, games),
        minutes,
        games,
        -seasonStart,
    ];
...
export function loadExistingAssetAltIds(franchiseId: string, eraId: string): Map<string, NonNullable<PeakPlayerSeason['altIds']>> {
    ...
    try {
        ...
        for (const player of parsed.data.players) {
            ...
            if (!altIdsParsed.success)
                continue;
            const data = altIdsParsed.data;
            if (data.nbaHeadshotAvailable === undefined && data.photoUrl === undefined)
                continue;
...
export type PoolPlayer = {
    ...
    altIds: Pick<NonNullable<PeakPlayerSeason['altIds']>, 'bbref' | 'nbaHeadshotAvailable' | 'photoUrl'> | null;
    positions: Pick<PeakPlayerSeason['positions'], 'primary' | 'secondary' | 'playable' | 'sourceLabels' | 'normalizationVersion'>;
    heightInches: PeakPlayerSeason['heightInches'];
    weightLbs: PeakPlayerSeason['weightLbs'];
    eligibility: Pick<PeakPlayerSeason['eligibility'], 'minimumTeamGames' | 'teamGames' | 'teamMinutes'>;
    ...
    source: Pick<PeakPlayerSeason['source'], 'dataVersion' | 'ratingsVersion' | 'selectionScoreVersion' | 'sourceVersion' | 'derivationMethodVersion' | 'lineageRuleVersion'>;
...
function failure(reason: UnavailabilityReason, detail: string, firstSupportedSeason?: string): PoolBuildFailure {
    return {
        reason,
        detail,
...
export function coverageBandForSeasons(seasons: readonly string[]): CoverageSummary['coverageBand'] {
    ...
    if (earliest >= '1996-97')
        return 'advanced-supported';
    if (earliest >= '1979-80')
        return 'complete-box-derived';
    if (earliest >= '1973-74')
        return 'late-historical';
    return 'reconstructed';
}
export function legalLineupCovered(players: readonly PoolPlayer[]): boolean {
    const slotGroupsOf = (player: PoolPlayer): readonly string[] => playableSlotGroups(player.positions.playable);
...
export function buildCoverageSummary(players: readonly PoolPlayer[], seasons: readonly string[]): CoverageSummary {
    ...
    for (const player of players) {
        for (const [field, provenance] of Object.entries(player.provenance)) {
            if (provenance.kind === 'observed')
                observedFamilies.add(fieldFamily(field));
            else if (provenance.kind === 'derived')
                derivedFamilies.add(fieldFamily(field));
            else if (provenance.kind === 'reconstructed')
                reconstructedFamilies.add(fieldFamily(field));
            else
                estimatedFamilies.add(fieldFamily(field));
    ...
    for (const field of requiredFields) {
        ...
        if (provenance !== undefined && provenance.confidence === 'low')
            lowConfidence += 1;
    }
...
function fieldFamily(field: string): string {
    ...
    if (tendency === 'usageRate' || tendency === 'passRate' || tendency === 'shotRate')
        return 'usage';
    if (tendency.includes('Frequency') || tendency === 'threePointRate')
        return 'shot-mix';
    if (tendency === 'freeThrowRate')
        return 'fouls';
    if (tendency === 'turnoverRate')
        return 'turnovers';
    if (tendency.includes('Rebound'))
        return 'rebounding';
    if (tendency.includes('AttemptRate'))
        return 'defensive-events';
    if (tendency.includes('Rate'))
        return 'usage';
    if (field === 'threePoint' || field === 'midrange' || field === 'closeShot')
        return 'shooting';
    if (field === 'insideScoring')
        return 'shooting';
    if (field === 'freeThrow')
        return 'fouls';
    if (field === 'passing' || field === 'ballHandling' || field === 'offensiveIq')
        return 'creation';
    if (field.includes('Rebound'))
        return 'rebounding';
    if (field.includes('Defense') ||
        field === 'defensiveIq' ||
        field === 'steal' ||
        field === 'block') {
        return 'defense';
    }
    if (field === 'speed' || field === 'strength' || field === 'vertical')
        return 'athleticism';
    if (field === 'pointsPerGame')
        return 'scoring';
    if (field === 'assistsPerGame')
        return 'playmaking';
    if (field === 'stealsPerGame' || field === 'blocksPerGame')
        return 'defensive-events';
    if (field === 'turnoversPerGame')
        return 'turnovers';
    if (field.includes('Rebound'))
        return 'rebounding';
    if (field === 'fieldGoalPct' || field === 'threePointPct' || field === 'freeThrowPct')
        return 'shooting';
    if (field.includes('AttemptRate'))
        return 'shot-mix';
    return tendency;
}
...
function playerLowConfidenceShare(player: PoolPlayer): number {
    ...
    for (const field of fields) {
        if (player.provenance[field]?.confidence === 'low')
            low += 1;
    }
    return fields.length > 0 ? low / fields.length : 0;
}
export function computePool(franchiseId: string, eraId: string, manifest: Manifest, bbrefIds?: Record<string, string>, withAssets = true, careerLabels?: Map<string, Set<string>> | null, keepSeasonCache = false): Pool | PoolBuildFailure {
    ...
    const eraHasLineage = LINEAGE_SEGMENTS.some((segment) => segment.modernFranchiseId === franchiseId &&
    ...
    if (!eraHasLineage) {
        ...
        return failure('no-franchise-history', `no NBA history for ${franchiseId} between ${era.fromSeasonKey} and ${era.toSeasonKey}`, first ?? undefined);
    }
    const teamExternalId = slot.teamExternalId;
    const seasons = listSeasonKeys().filter((season) => era.fromSeasonKey <= season && season <= era.toSeasonKey);
    if (seasons.length === 0) {
    ...
    for (const season of seasons) {
        ...
        for (const stint of stints) {
            ...
            const player = rosterByExtId[pid] !== undefined
                ? sourcePlayer
    ...
    for (const pid of [...eligible.keys()].sort()) {
        ...
        const peakSecondary = Array.isArray(player.secondaryPositions)
            ? player.secondaryPositions.filter((secondary): secondary is string => typeof secondary === 'string' && secondary !== '')
            : [];
        const override = positionOverrideFor(pid);
        const career = careerLabelsMap.get(pid);
        const labels = career !== undefined && career.size > 0 ? career : new Set([peakPrimary, ...peakSecondary]);
        ...
        if (unknownLabels.length > 0) {
            console.log(`  [WARN] ${str(player.firstName)} ${str(player.lastName)} (${pid}) unknown position labels: ${formatList(unknownLabels)}`);
        }
        ...
        const [firstName, lastName] = canonicalPlayerName(pid, str(player.firstName), str(player.lastName));
        ...
        playersOut.push({
            ...
            selectionScore: selectionScore(rawOverallScoreFor(player, summary), safeFloat(summary?.offenseRating), safeFloat(summary?.defenseRating), nullableFrom(stats.usageRate), teamMinutes, teamGames),
            ...
            ...(player.reconstructedThreePoint != null &&
                ...
                ? {
                    reconstructedThreePoint: reconstructedThreePointProfileSchema.parse(player.reconstructedThreePoint),
                }
                : {}),
    ...
    if (playersOut.length === 0) {
        const detail = identityFailures.length > 0
    ...
    if (!legalLineupCovered(playersOut)) {
        return failure('position-coverage-failed', `cannot form G,G,F,F,C from ${String(playersOut.length)} players`);
    }
    ...
    if (policyFailures.length > 0) {
        return failure('confidence-failed', `${String(policyFailures.length)} players exceed the low-confidence share under ${CONFIDENCE_POLICY_VERSION}`);
    }
    if (withAssets) {
        console.log('  [WARN] headshot/photo asset annotation stays in scripts/annotate-markers.mjs; skipping');
    }
    return {
        schemaVersion: SCHEMA_VERSION,
        dataVersion: DATA_VERSION,
...
export function logPoolValidation(pool: Pool): void {
    try {
        ...
        console.error(`  [VALIDATION] parsePool rejected ${pool.franchiseId}-${pool.eraId} ` +
...
export function writePool(pool: Pool): string {
    ...
    console.log(`  [OK] wrote ${basename(path)} (${String(pool.players.length)} players, ${digest.slice(0, 12)}...)`);
    return digest;
}
export function parsePoolTargets(raw: readonly string[]): Array<[
    string,
    string
]> {
    const targets: Array<[
...
export function allPoolTargets(manifest: Manifest = loadManifest()): Array<[
    string,
    string
]> {
    ...
    for (const slot of MODERN_SLOTS) {
        for (const era of manifest.eras) {
            const hasLineage = LINEAGE_SEGMENTS.some((segment) => segment.modernFranchiseId === slot.franchiseId &&
                segment.validFromSeasonKey <= era.toSeasonKey &&
                (segment.validThroughSeasonKey === undefined ||
                    segment.validThroughSeasonKey >= era.fromSeasonKey));
            const overlaps = hasLineage &&
                [...packagedSeasons].some((season) => era.fromSeasonKey <= season && season <= era.toSeasonKey);
            if (overlaps) {
...
export function buildPoolForTarget(franchiseId: string, eraId: string, manifest: Manifest, bbrefIds: Record<string, string>, withAssets: boolean, careerLabels: Map<string, Set<string>> | null, keepSeasonCache = false): TargetBuildResult {
    const pool = computePool(franchiseId, eraId, manifest, bbrefIds, withAssets, careerLabels, keepSeasonCache);
    if ('reason' in pool) {
...
function applyOverallCohortNormalization(): Array<{
    ...
    if (diagnostics.totalRowCount > 0) {
        console.log(`  [OK] overall cohort normalization (${COHORT_NORMALIZATION_VERSION}): ${String(diagnostics.totalRowCount)} rows across ${String(rewritten.length)} pools (${String(diagnostics.rowsWithoutRawOverall)} without rawOverallScore)`);
    }
    return rewritten;
}
export async function run(targets: Array<[
    string,
    string
]> | null = null, withAssets = true, workers?: number): Promise<void> {
    if (targets === null) {
    ...
    const workerCount = workers === undefined ? defaultPoolWorkers() : Math.max(1, Math.trunc(workers));
    let results: TargetBuildResult[];
    if (workerCount <= 1 || targets.length <= 1) {
        results = [];
        for (const [franchiseId, eraId] of targets) {
            results.push(buildPoolForTarget(franchiseId, eraId, manifest, bbrefIds, withAssets, careerLabels, true));
        }
    }
    else {
        const chunks = partitionPoolTargets(targets, workerCount);
        const workerResults = await Promise.all(chunks.map((chunk) => runPoolChunk(chunk, manifest, bbrefIds, careerLabels, withAssets)));
...
export function recordCoverageReport(entries: CoverageReportEntry[]): void {
    const existing = new Map(loadCoverageReport().map((entry) => [`${entry.franchiseId}/${entry.eraId}`, entry]));
    ...
    console.log(`  [OK] coverage report updated: ${String(merged.filter((e) => e.status === 'available').length)} available, ${String(merged.filter((e) => e.status === 'unavailable').length)} unavailable`);
}
export function classifyUnattempted(franchiseId: string, eraId: string, manifest: Manifest): PoolBuildFailure {
    ...
    const eraHasLineage = era === undefined
        ? false
        : LINEAGE_SEGMENTS.some((segment) => segment.modernFranchiseId === franchiseId &&
            segment.validFromSeasonKey <= era.toSeasonKey &&
            (segment.validThroughSeasonKey === undefined ||
                segment.validThroughSeasonKey >= era.fromSeasonKey));
    if (!eraHasLineage) {
    ...
    const hasSeasons = listSeasonKeys().some((season) => era !== undefined && era.fromSeasonKey <= season && season <= era.toSeasonKey);
    if (!hasSeasons) {
        return failure('source-incomplete', `no packaged seasons for era ${eraId}`);
    }
    return failure('insufficient-players', 'not attempted in the last coverage build');
}
export function updateManifest(entries: Array<{
    franchiseId: string;
    eraId: string;
    url: string;
    contentHash: string;
    ...
    const existing = new Map(manifest.pools.map((entry) => [`${entry.franchiseId}/${entry.eraId}`, entry]));
    for (const entry of entries) {
        existing.set(`${entry.franchiseId}/${entry.eraId}`, entry);
    }
    ...
    console.log(`  [OK] manifest updated: ${String(manifest.pools.length)} pools, dataVersion ${DATA_VERSION}`);
}
function listSeasonKeys(): string[] {
    return readdirSync(NBA_ROOT, { withFileTypes: true })
        .filter((entry) => entry.isDirectory())
        .map((entry) => entry.name)
        .filter((name) => fileExists(join(NBA_ROOT, name, 'roster.json')) ||
