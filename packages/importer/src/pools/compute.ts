...
import { COHORT_NORMALIZATION_VERSION, DERIVATION_METHOD_VERSION, LINEAGE_RULE_VERSION, POSITION_NORMALIZATION_VERSION, RATINGS_VERSION, POOL_SCHEMA_VERSION, REQUIRED_RATING_KEYS, SELECTION_SCORE_VERSION, SOURCE_VERSION, type Confidence, type CoverageSummary, type HistoricalValueProvenance, type Position, type RatingProfile, type UnavailabilityReason, } from '@hoop-rush/data-contracts';
...
import { fileExists, safeFloat, safeInt, sha256File, writeJson, writeJsonRetry, clamp, clampUnitInterval, } from '../json.ts';
...
function refreshedFallbackPlayer(player: Record<string, unknown>, season: string, stats: Record<string, unknown>, playerExternalId: string): Record<string, unknown> {
...
function loadFallbackRosterPlayers(): Map<string, Record<string, unknown>> {
    ...
    for (const file of sortedJsonFiles(poolDir())) {
        try {
            const raw = readJsonLoose(join(poolDir(), file)) as {
                players?: unknown;
            };
            ...
            for (const value of raw.players) {
                ...
                const secondaryPositions = Array.isArray(positions?.secondary)
                    ? positions.secondary.filter((secondary): secondary is string => typeof secondary === 'string' && secondary !== '')
                    : [];
...
export function rawOverallScoreFor(player: Record<string, unknown>, summary: SummaryRatingsRaw | undefined): number {
    const profile = player.ratingProfile as {
        rawOverallScore?: unknown;
        canonicalOverall?: unknown;
    } | undefined;
...
export function selectionScore(rawOverallScore: number, offenseRating: number, defenseRating: number, usageRate: number | null, teamMinutes: number, teamGames: number): number {
    ...
    const raw = 0.6 * rawOverallScore + 0.25 * offenseRating + 0.15 * defenseRating + 0.05 * usage + 0.02 * mpg;
...
export function candidateKey(candidate: Candidate): readonly number[] {
    ...
    return [
        selectionScore(rawOverallScoreFor(candidate.player, summary), safeFloat(summary?.offenseRating), safeFloat(summary?.defenseRating), nullableValue(candidate.stats, 'usageRate'), minutes, games),
        minutes,
        games,
        -seasonStart,
    ];
...
export function loadExistingAssetAltIds(franchiseId: string, eraId: string): Map<string, Record<string, unknown>> {
...
export function legalLineupCovered(players: readonly PoolPlayer[]): boolean {
    const slotGroupsOf = (player: PoolPlayer): readonly string[] => playableSlotGroups(player.positions.playable as Position[]);
...
export function computePool(franchiseId: string, eraId: string, manifest: Manifest, bbrefIds?: Record<string, string>, withAssets = true, careerLabels?: Map<string, Set<string>> | null, keepSeasonCache = false): Pool | PoolBuildFailure {
    ...
    for (const pid of [...eligible.keys()].sort()) {
        ...
        playersOut.push({
            ...
            selectionScore: selectionScore(rawOverallScoreFor(player, summary), safeFloat(summary?.offenseRating), safeFloat(summary?.defenseRating), nullableValue(stats, 'usageRate'), teamMinutes, teamGames),
