...
import { LINEAGE_RULE_VERSION, MANIFEST_SCHEMA_VERSION, parsePool, parsePlayersIndex, parseRosterDetails, PLAYERS_INDEX_SCHEMA_VERSION, RATING_MODEL_VERSION, } from '@hoop-rush/data-contracts';
...
export function rebuildPlayersIndex(dataDir = PUBLIC_DATA, preloaded?: ReadonlyMap<string, unknown>): {
    url: string;
    contentHash: string;
} | null {
...
export function rebuildRosterDetails(dataDir = PUBLIC_DATA, preloaded?: ReadonlyMap<string, unknown>): {
    url: string;
    contentHash: string;
} | null {
    const poolsDir = join(dataDir, 'pools');
...
export function refreshPlayersIndexInManifest(dataDir = PUBLIC_DATA): void {
    ...
    if (entry === null && detailsEntry === null)
        return;
    ...
    if (detailsEntry !== null)
        manifest['rosterDetails'] = detailsEntry;
    writeJsonRetry(manifestPath, manifest, true);
    console.log(`updated players index (${entry?.contentHash.slice(0, 8) ?? 'n/a'}…) and roster details (${detailsEntry?.contentHash.slice(0, 8) ?? 'n/a'}…)`);
}
...
export function run(dataDir = PUBLIC_DATA): void {
    ...
    const coverage = new Map(loadCoverageReport().map((entry) => [`${entry.franchiseId}/${entry.eraId}`, entry]));
    const availability: unknown[] = [];
    for (const slot of MODERN_SLOTS) {
        for (const era of manifest['eras'] as Array<{
            eraId: string;
        }>) {
    ...
    for (const name of sortedJsonFiles(simDir)) {
        const profile = readJson(join(simDir, name)) as {
            eraId: string;
        };
        profiles.push({
            eraId: profile.eraId,
    ...
    const seasonRefs: Record<string, {
        url: string;
        contentHash: string;
    }> = {};
    ...
    console.log(`slots=${String(MODERN_SLOTS.length)} lineageSegments=${String(LINEAGE_SEGMENTS.length)} pools=${String(poolEntries.length)} availability=${String(availability.length)}`);
}
