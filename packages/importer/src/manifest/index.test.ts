...
describe('manifest run', () => {
    it('rebuilds the v2 manifest: slots, lineage, pools, availability, profiles, bracket', { timeout: TEST_TIMEOUT }, () => {
        ...
        expect(manifest.franchiseLineage.every((segment) => (segment.logoCandidates?.length ?? 0) > 0)).toBe(true);
        ...
        const lakers = manifest.availability.find((entry) => entry['franchiseId'] === 'lakers' && entry['status'] === 'available');
        expect(lakers?.['playerCount'] ?? null).toBe(1);
        const pelicans = manifest.availability.find((entry) => entry['franchiseId'] === 'pelicans') as Record<string, unknown>;
