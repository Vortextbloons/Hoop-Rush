...
export function extractCalibrationPayload(raw: string): CalibrationPayload {
    ...
    if (!parsed.success) {
        throw new Error(`expected a calibrate run report, got (missing payload): ${parsed.error.issues[0]?.message ?? 'invalid'}`);
    }
...
export function freezeTargets(reportPath: string, eraId = '1990s', profilePath?: string): void {
    ...
    if (!profileParsed.success) {
        throw new Error(`era profile ${resolved} fails validation: ${profileParsed.error.issues[0]?.message ?? 'invalid'}`);
    }
    ...
    const setTarget = (path: string, value: number): void => {
        ...
        Reflect.set(node, 'minimumSample', MINIMUM_SAMPLES[path] ?? (typeof existingSample === 'number' ? existingSample : 200));
    };
