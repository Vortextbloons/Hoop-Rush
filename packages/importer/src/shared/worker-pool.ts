...
export async function runParallel<T, R>(items: readonly T[], workerUrl: URL, toWorkerData: (chunk: T[]) => unknown, chunkFn: (items: readonly T[], workers: number) => T[][] = chunkList, workers?: number, cap = 8): Promise<R[]> {
