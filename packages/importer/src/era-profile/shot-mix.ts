...
export function computePoolShotMix(players: readonly PoolPlayerLike[], leagueThreeRate: number): ZoneMix {
    const totalUsage = players.reduce((sum, p) => sum + (p.tendencies.usageRate ?? 0), 0) || 1.0;
    const weighted = (zone: 'rimFrequency' | 'shortMidFrequency' | 'longMidFrequency' | 'cornerThreeFrequency' | 'aboveBreakThreeFrequency') => players.reduce((sum, p) => sum + (p.tendencies[zone] ?? 0) * (p.tendencies.usageRate ?? 0), 0) /
        totalUsage;
    ...
    const total = weightedRim + weightedShortMid + weightedLongMid + weightedCorner + weightedAbove || 1.0;
    let mix: ZoneMix = {
...
export function poolShotMixAndAnchors(eraId: string, leagueThreeRate: number): ShotMixAndAnchors {
    ...
    const pool = readJson(poolPath) as {
        players: PoolPlayerLike[];
    };
    const players = pool.players;
    const mix = computePoolShotMix(players, leagueThreeRate);
    const ftMean = players.reduce((sum, p) => sum + (p.detailedRatings.freeThrow ?? 50), 0) / players.length;
    const passMean = players.reduce((sum, p) => sum + (p.detailedRatings.passing ?? 50), 0) / players.length;
