...
export function deriveSeasonAggregatesFromStints(season: string, stints: readonly StintRow[]): LeagueAggregates {
    ...
    const family = (name: string): number | null => {
        if (!fieldAvailableFrom(FAMILY_KEY[name] ?? name, season))
            return null;
...
export function deriveLeagueAggregates(seasons: readonly string[]): LeagueAggregates {
    ...
    combined.pairs = {
        stealShare: pairSeasons.stealShare.count > 0
            ? {
            ...
            : null,
        reboundSplit: pairSeasons.reboundSplit.count > 0
            ? {
            ...
            : null,
        turnoverPerPossession: pairSeasons.turnoverPerPossession.count > 0
            ? {
