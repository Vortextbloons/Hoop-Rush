...
import { DERIVATION_METHOD_VERSION, SOURCE_VERSION, type ProvenanceKind, } from '@hoop-rush/data-contracts';
...
export interface EraProfileTargets {
    ...
    zoneMix: Record<'rim' | 'shortMid' | 'longMid' | 'cornerThree' | 'aboveBreakThree', CalibrationTarget>;
...
export function packagedSeasons(): string[] {
    return readdirSync(NBA_ROOT, { withFileTypes: true })
        ...
        .filter((name) => existsSync(join(NBA_ROOT, name, 'stints.json')) ||
        existsSync(join(NBA_ROOT, name, 'roster.json')))
        .sort();
}
export function eraSeasons(era: EraDef): string[] {
    return packagedSeasons().filter((season) => era.fromSeasonKey <= season && season <= era.toSeasonKey);
}
...
function estimatedProvenance(fields: string[], sourceStatus: 'unavailable' | 'not-applicable' | 'available' = 'unavailable', notesCode?: string): ParameterProvenance {
    return {
        kind: 'estimated',
        confidence: 'low',
        methodVersion: DERIVATION_METHOD_VERSION,
...
export function computeEraProfile(era: EraDef): EraSimProfile {
    ...
    const tsPct = a.possessions !== null
    ...
    const tovPerPoss = a.pairs.turnoverPerPossession !== null
    ...
    const stealShare = a.pairs.stealShare !== null && a.pairs.stealShare.tov > 0
        ? a.pairs.stealShare.stl / a.pairs.stealShare.tov
        : null;
    const orebRate = a.pairs.reboundSplit !== null
    ...
    const foulsPerPoss = a.possessions !== null
    ...
    parameterProvenance['shootingFoulShare'] = estimatedProvenance(['prior'], 'unavailable', 'not-derivable-from-box-scores');
    ...
    const parameters: EraSimParameters = {
        ...
        turnoverPerPossession: tovPerTrip !== null
            ? round4(tovPerTrip)
        ...
        offensiveReboundRate: orebRate !== null ? round4(orebRate) : ERA_RULE_ESTIMATES.offensiveReboundRateBeforeSplits,
    ...
    return {
        ...
        targets: {
            ...
            offensiveReboundsPerGame: target(orebPerGame ?? pace * ERA_RULE_ESTIMATES.offensiveReboundRateBeforeSplits * 0.5, 1.5),
