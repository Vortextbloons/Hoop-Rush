...
export const SALARY_TIERS: ReadonlyArray<readonly [
    number,
    number
]> = [
    [95, 65000000],
...
export function deriveContract(overall: number, age: number, rng?: unknown): Contract {
    ...
    const signingBonusByYear: number[] = Array.from({ length: years }, () => Math.trunc(signingBonus / years));
    if (years > 1) {
