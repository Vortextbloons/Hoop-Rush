...
import { z } from 'zod';
import { provenanceMapSchema, ratingProfileSchema, simulationAnchorsSchema, simulationRatingsSchema, simulationTendenciesSchema, summaryRatingsSchema, } from '@hoop-rush/data-contracts';
        ...
        try {
            ...
            if (fileParsed.success && Array.isArray(fileParsed.data.rows)) {
                ...
                if (arrayParsed.success)
                    rows = arrayParsed.data;
            }
            for (const row of rows) {
                const rawId = row.PLAYER_ID ?? row.playerExternalId ?? '';
                const pid = typeof rawId === 'string' || typeof rawId === 'number' || typeof rawId === 'boolean'
                    ? String(rawId)
                    : '';
                if (!pid)
                    continue;
...
function teamWinPctForPlayer(stats: RatingsStatsRow, rosterTeamId: string | null, winMap: Map<string, number>): number | null {
...
export function pooledRatePriors(roster: readonly RosterPlayer[], statsList: readonly RatingsStatsRow[]): Map<string, {
    ...
    for (const [group, acc] of sums) {
        ...
        if (acc.tpa > 0)
            prior.threePointPctPrior = acc.tpm / acc.tpa;
        if (acc.fta > 0)
            prior.freeThrowPctPrior = acc.ftm / acc.fta;
...
export function computeForSeason(season: string, force = false): void {
    ...
    for (const player of roster) {
        const extId = player.externalId ?? '';
        const [canonicalFirstName, canonicalLastName] = canonicalPlayerName(extId, player.firstName ?? '', player.lastName ?? '');
...
export async function run(seasons?: string[], force = false, workers?: number): Promise<void> {
    ...
    const workerCount = workers === undefined ? defaultRatingsWorkers() : Math.max(1, Math.trunc(workers));
