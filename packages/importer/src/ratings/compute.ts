...
function loadPlayerWinPctMap(season: string): Map<string, number> {
    ...
    for (const path of candidates) {
        ...
        try {
            ...
            const rows: Array<Record<string, unknown>> = Array.isArray((data as {
                rows?: unknown;
            }).rows)
                ? (data as {
...
function teamWinPctForPlayer(stats: StatsRow, rosterTeamId: string | null, winMap: Map<string, number>): number | null {
...
export function pooledRatePriors(roster: readonly RosterPlayer[], statsList: readonly StatsRow[]): Map<string, {
...
export function computeForSeason(season: string, force = false): void {
    ...
    for (const player of roster) {
        ...
        const rosterTeamId = typeof (player as Record<string, unknown>)['teamExternalId'] === 'string'
