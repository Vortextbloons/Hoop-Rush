...
export function deriveTraits(ratings: Record<string, number>, stats: StatsRow, position: string, rng?: unknown): Record<string, number> {
    void position;
    void rng;
    ...
    const defensiveVersatility = clampRating(60 + (ratings['defensiveIq'] ?? 55) * 0.3 + (ratings['steal'] ?? 50) * 0.2);
    return {
