...
function completeRatings(ratings: Record<string, number>): SimulationRatings {
    const filled = Object.fromEntries(RATING_KEYS.map((key) => [key, 50])) as Record<keyof SimulationRatings, number>;
    return { ...filled, ...ratings };
}
export function computeSummaryRatings(ratings: Record<string, number>, tendencies: Record<string, number>): {
...
export function computeRealOverall(ratings: Record<string, number>, position: string, stats: StatsRow, heightInches?: number | null): number {
    if (Object.keys(ratings).length < 18) {
        ...
        if (measuredPer === null && measuredBpm === null)
            return neutral;
        return clampRating(neutral +
