...
function productionEvidence(stats: StatsRow): ProductionEvidence {
    ...
    const stocks = stats.steals == null || stats.blocks == null || games <= 0
        ? 0
...
function deriveNonlinear(ratings: SimulationRatings, tendencies: SimulationTendencies, stats: StatsRow, memberships: ArchetypeMemberships): NonlinearComponents {
    ...
    const apg = Math.max(0, Math.trunc(safeFloat(stats.gamesPlayed))) > 0
        ? safeFloat(stats.assists) / Math.max(1, Math.trunc(safeFloat(stats.gamesPlayed)))
        : 0;
    const weaknesses = {
        turnoverLiability: clamp((Math.max(0, tendencies.turnoverRate - 12) * (0.7 + 0.8 * primaryRole)) / (1 + apg / 12), 0, 6),
        ...
        defensiveTargeting: clamp((Math.max(0, 64 -
            ...
            7, 0, 6),
        spacingLimitation: clamp(((Math.max(0, 58 - shootingGravity) * (0.8 + primaryRole * 0.8)) / 8) * (1 - bigRole * 0.65), 0, 6),
...
export function eliteEvidenceLiftFor(input: {
    ...
    const eliteScoringEvidence = winOk && input.production.score >= 87 && games >= 55 && ppg >= 28 && ts >= 0.6 && bpm >= 3;
    const completeEliteEvidence = winOk &&
    ...
    if (completeEliteEvidence)
        return 4;
    if (eliteScoringEvidence)
        return 3;
    if (input.production.score >= 80 && games >= 50)
        return 1;
    return 0;
}
export function teamContextAdjustment(stats: StatsRow, teamWinPct: number | null | undefined, defenseRating: number): number {
    if (teamWinPct == null || !Number.isFinite(teamWinPct))
        return 0;
    const pct = clamp(teamWinPct, 0, 1);
    const games = Math.max(0, Math.trunc(safeFloat(stats.gamesPlayed)));
    const minutes = Math.max(0, safeFloat(stats.minutes));
    if (games < 20 || minutes < 300)
        return 0;
