...
describe('parity: pools vs committed artifacts', () => {
    for (const [franchiseId, eraId] of TARGETS) {
        it(`${franchiseId}/${eraId} eligibility and top-5 selection match`, { timeout: 30000 }, () => {
            ...
            const pool = result;
            const committed = readJson(`${PUBLIC_DATA}/pools/${franchiseId}-${eraId}.json`) as CommittedPool;
...
const PYTHON_CONFIG = resolve(dirname(fileURLToPath(import.meta.url)), '../../../scripts/import-nba/config.py');
...
describe('parity: era profiles vs committed artifacts', () => {
    it('all era parameters match the committed profiles within rounding tolerance', { timeout: 30000 }, () => {
