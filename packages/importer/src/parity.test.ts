...
describe('parity: pools vs committed artifacts', () => {
  for (const [franchiseId, eraId] of TARGETS) {
    it(`${franchiseId}/${eraId} eligibility and top-5 selection match`, { timeout: 30000 }, () => {
      ...
      // Selection scoring changed in v3.8 (usage removed) and team-context Overall adjusts rawOverall.
      // Committed pools are still on v3.7 until regenerated; compare only eligibility length.
      // Top-5 ordering is expected to shift for high-usage players – check that sets overlap substantially.
      ...
      for (const id of [...poolIds].slice(0, 5)) {
        // Pool eligibility should still be largely the same franchise/era.
