import { SEASON_AI_VERSION, SEASON_DRAFT_CATALOG_VERSION, SEASON_DRAFT_VERSION, SEASON_DURABILITY_VERSION, PLAYER_VERSION_ID_VERSION, POSITION_NORMALIZATION_VERSION, RATINGS_VERSION, SEASON_MINUTE_POLICY_VERSION, SEASON_ROSTER_GENERATION_VERSION, SEASON_ROSTER_TARGETS_VERSION, SEASON_ROTATION_VERSION, SEASON_STAMINA_VERSION, eraIdSchema, franchiseIdSchema, playerIdSchema, playerVersionId, seasonDigestHex, seasonDraftCatalogSchema, seasonDraftStateSchema, seasonRotationSchema, seedSchema, seasonKeySchema, type SeasonAiAssignment, type SeasonAiPool, type SeasonDraftCatalog, type SeasonDraftCatalogPool, type SeasonDraftCandidate, type SeasonDraftState, type SeasonLeague, type SeasonRoster, type SeasonRosterTargets, type SeasonRotation, type SeasonRun, } from '@hoop-rush/data-contracts';
...
function candidateRating(key: 'insideScoring' | 'threePoint' | 'perimeterDefense' | 'interiorDefense' | 'offensiveRebound' | 'defensiveRebound' | 'ballHandling' | 'passing', archetypeIndex: number, n: number): number {
...
export function buildSeasonDraftCandidate(input: {
    ...
    if (!archetype)
        throw new Error('missing position archetype');
    ...
    return {
        ...
        detailedRatings: {
            ...
            offensiveRebound: candidateRating('offensiveRebound', index % POSITION_ARCHETYPES.length, index),
            defensiveRebound: candidateRating('defensiveRebound', index % POSITION_ARCHETYPES.length, index),
            perimeterDefense: candidateRating('perimeterDefense', index % POSITION_ARCHETYPES.length, index),
            interiorDefense: candidateRating('interiorDefense', index % POSITION_ARCHETYPES.length, index),
            steal: 40 + ((index * 3) % 45),
            block: 40 + ((index * 3) % 45),
...
export function buildSeasonDraftCatalog(input: {
    franchiseIds?: string[];
    eras?: string[];
...
export function buildSeasonRotation(franchiseId: string, playerVersionIds: string[]): SeasonRotation {
    if (playerVersionIds.length !== 10)
        throw new Error('rotation needs ten players');
...
export function buildSeasonAiPools(assignments: SeasonAiAssignment[], humanFranchiseId: string): SeasonAiPool[] {
    return assignments
...
export function buildFixtureEvaluations(rosters: SeasonRoster[], assignments: SeasonAiAssignment[]): SeasonRun['evaluations'] {
