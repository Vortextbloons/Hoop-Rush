import { POSITION_SLOTS, SIMULATION_RATINGS, SIMULATION_TENDENCIES, bracketOpponentSchema, challengeRunSchema, eraIdSchema, eraSimulationProfileSchema, franchiseEraPoolSchema, franchiseIdSchema, gameSimulationInputSchema, hoopRushManifestSchema, opponentBracketSchema, opponentTeamSchema, peakPlayerSeasonSchema, playerIdSchema, seedFromString, simulationPlayerSchema, simulationTeamSchema, } from '@hoop-rush/data-contracts';
import type { BracketOpponent, BracketScheduleEntry, ChallengeRun, DifficultyProfile, EraSimulationProfile, FranchiseEraPool, GameSimulationInput, HoopRushManifest, ModernFranchiseSlot, OpponentBracket, OpponentTeam, PeakPlayerSeason, PlayerSeasonStats, RunAggregates, SimulationPlayer, SimulationRatings, SimulationTeam, SummaryRatings, } from '@hoop-rush/data-contracts';
...
function fullProvenance(): PeakPlayerSeason['provenance'] {
    ...
    for (const key of Object.keys(DEFAULT_SIM_RATINGS))
        provenance[key] = { ...entry };
    for (const key of Object.keys(DEFAULT_SIM_TENDENCIES))
        provenance[key] = { ...entry };
    for (const key of [
        'gamesPlayed',
        'minutesPerGame',
...
export function buildPool(players: PeakPlayerSeason[], overrides: Partial<FranchiseEraPool> = {}): FranchiseEraPool {
...
export function buildUserTeam(): SimulationTeam {
    return simulationTeamSchema.parse(buildLegalSimulationTeam({
        teamId: 'user',
        displayName: 'Los Angeles Lakers',
        players: legalFive('p-'),
    }));
...
function buildBracketOpponent(franchiseId: string, opponentId: string, index: number, overrides: Partial<BracketOpponent> = {}): BracketOpponent {
    const positions: SimulationPlayer['positions'][] = POSITION_SLOTS.map((position) => [position]);
    const players = positions.map((position, slot) => buildSimulationPlayer({
...
export function buildFixtureBracket(overrides: Partial<OpponentBracket> = {}): OpponentBracket {
    const opponents = ALL_FRANCHISE_SLOTS.map((slot, index) => buildBracketOpponent(slot.franchiseId, index === 0 ? 'lakers-1990s-opening' : `bracket-${slot.franchiseId}`, index));
...
function legalFive(prefix: string, centerInteriorDefense?: number): SimulationPlayer[] {
    const positions: SimulationPlayer['positions'][] = POSITION_SLOTS.map((position) => [position]);
    return positions.map((position, i) => buildSimulationPlayer({
...
function fixtureScale(targetCenter: number) {
    return (_element: unknown, index: number): SimulationPlayer => {
        ...
        const shifted: SimulationRatings = Object.fromEntries(Object.entries(base.ratings).map(([key, value]) => {
...
export function buildEqualFixture(): {
    home: SimulationTeam;
    away: SimulationTeam;
} {
    return {
...
function strengthPair(secondCenter: number, secondTeamId: string, secondDisplayName: string): {
    strong: SimulationTeam;
    second: SimulationTeam;
} {
    return {
...
export function buildEraSimulationProfile(overrides: Partial<EraSimulationProfile> = {}): EraSimulationProfile {
    return eraSimulationProfileSchema.parse({ ...DEFAULT_ERA_SIM_PROFILE, ...overrides });
}
export function buildGameSimulationInput(overrides: Partial<GameSimulationInput> = {}): GameSimulationInput {
