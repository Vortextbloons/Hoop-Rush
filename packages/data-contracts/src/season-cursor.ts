import { z } from 'zod';
import { SEASON_BLOCK_COUNT, SEASON_BLOCK_TEAM_GAMES, SEASON_FINAL_BLOCK_TEAM_GAMES, SEASON_ROUND_COUNT, } from './season-versions.ts';
...
export function blockRoundRange(blockIndex: number): {
    ...
    if (blockIndex < 0 || blockIndex >= SEASON_BLOCK_COUNT) {
        throw new Error(`block index ${String(blockIndex)} out of range 0..${String(SEASON_BLOCK_COUNT - 1)}`);
    }
    const fromRound = blockIndex * SEASON_BLOCK_TEAM_GAMES + 1;
    const toRound = blockIndex === SEASON_BLOCK_COUNT - 1
        ? SEASON_ROUND_COUNT
