...
describe('season run stage and completion (M2.6)', () => {
  ...
  it('completion state reconciles with the postseason champion', () => {
    ...
    expect(() =>
      seasonRunCompletionSchema.parse({
        championFranchiseId: 'lakers',
...
describe('postseason game ids (M2.6)', () => {
  it('derives stable ids reproducibly from the same inputs', () => {
    ...
    expect(playoffGameIdOf('east-first-round-1', 2)).not.toBe(
      playoffGameIdOf('east-first-round-1', 1),
    );
...
describe('awards, almanac, and replay exports (M2.6)', () => {
  ...
  it('round-trips awards with a deterministic digest', () => {
    ...
    expect(digest).toMatch(/^[0-9a-f]{32}$/);
    expect(
      seasonAwardsDigest(seasonAwardsSchema.parse({ ...parsed, digest: 'f'.repeat(32) })),
    ).toBe(digest);
    expect(() =>
      seasonAwardsSchema.parse({ ...parsed, mvp: { playerVersionId: 'bad' } }),
    ).toThrow();
  });
  it('round-trips an almanac whose digest reconciles with the run completion', () => {
    ...
    expect(digest).toMatch(/^[0-9a-f]{32}$/);
    expect(
      seasonAlmanacDigest(seasonAlmanacSchema.parse({ ...parsed, digest: 'f'.repeat(32) })),
    ).toBe(digest);
  });
...
describe('command log (M2.6, command-log-v1)', () => {
  function entry(
    ordinal: number,
    overrides: Partial<SeasonCommandLogEntry> = {},
