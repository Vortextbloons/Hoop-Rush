...
export type SeasonCheckpointVerificationState = z.infer<
  typeof seasonCheckpointVerificationStateSchema
>;
...
export function buildMultiplayerAuthority(input: {
  ...
  return {
    ...
    p1: {
      participantId: 'p1',
      franchiseId: franchiseIdSchema.parse(input.p1FranchiseId),
      seat: 'p1',
    },
    p2: {
      participantId: 'p2',
      franchiseId: franchiseIdSchema.parse(input.p2FranchiseId),
      seat: 'p2',
    },
    pace: input.pace,
...
export function authorityForFranchise(
  ...
  if (authority.p1.franchiseId === franchiseId) return 'p1';
  if (authority.p2.franchiseId === franchiseId) return 'p2';
  return null;
}
export function franchiseForParticipant(
  authority: SeasonRunAuthority,
  participantId: SeasonParticipantId,
): string | null {
  if (authority.kind === 'local-solo') {
