import { FIXED_FIVE_CODE_TTL_MS, FIXED_FIVE_ENVELOPE_MAX_BYTES, FIXED_FIVE_MULTIPLAYER_VERSION, FIXED_FIVE_ROOM_PROTOCOL_VERSION, FIXED_FIVE_ROOM_SCHEMA_VERSION, fixedFiveCommandSchema, fixedFiveTimeoutMsForMode, type FixedFiveCommand, type FixedFiveCommandReceipt, type FixedFiveMultiplayerTransport, type FixedFiveParticipantId, type FixedFiveRoomCode, type FixedFiveRoomMembership, type FixedFiveRoomSettings, type FixedFiveRoomSnapshot, } from './fixed-five-multiplayer.ts';
...
export function createInMemoryFixedFiveTransport(options?: {
    ...
    return {
        async create(settingsInput) {
            ...
            pruneExpiredCodes();
            const now = clock();
            const recent = createTimestamps.filter((t) => now - t < 60000);
            if (recent.length >= 6) {
        ...
        async join(code) {
            ...
            const updated = record.snapshot.members.map((m) => m.participantId === 'p2' ? { ...m, online: true, lastSeenAt: nowIsoWith(clock) } : m);
            void p2;
            record.snapshot = {
                ...record.snapshot,
                members: updated,
        ...
        async leave(roomId, participantId) {
            ...
            record.snapshot = {
                ...record.snapshot,
                members: record.snapshot.members.map((m) => m.participantId === participantId ? { ...m, online: false } : m),
                revision: record.snapshot.revision + 1,
            };
            emit(record);
        },
        ...
        async complete(roomId, resultDigest) {
            ...
            const proposed = record.commands.some((c) => c.payload.kind === 'propose-result' && c.payload.resultDigest === resultDigest);
            const confirmed = record.commands.some((c) => c.payload.kind === 'confirm-result' &&
                c.payload.verified &&
        ...
        async fail(roomId) {
            ...
            const digests = new Set(record.commands
                .filter((c) => c.payload.kind === 'propose-result')
                .map((c) => (c.payload.kind === 'propose-result' ? c.payload.resultDigest : '')));
            const denied = record.commands.some((c) => c.payload.kind === 'confirm-result' && !c.payload.verified);
