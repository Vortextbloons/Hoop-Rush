...
import { PROJECTION_MODEL_VERSION, PROJECTION_SCHEMA_VERSION, RATINGS_VERSION, SEASON_DRAFT_CATALOG_VERSION, SEASON_DRAFT_CATALOG_V3, SEASON_PROJECTION_TARGETS_VERSION, SEASON_PROJECTION_VERSION, SIMULATION_RATINGS, SIMULATION_TENDENCIES, baseFiveProjectionSchema, contentHashSchema, eraIdSchema, franchiseIdSchema, playerIdSchema, projectionModelArtifactSchema, seasonDraftCatalogSchema, seasonProjectionSchema, seasonProjectionTargetsSchema, seasonKeySchema, type BaseFiveProjection, type ProjectionMatchupArchetype, type ProjectionModelArtifact, type ProjectionPlayerContribution, type ProjectionSide, type SeasonDraftCatalog, type SeasonProjectionUnitKind, type SimulationPlayer, type SeasonDraftCandidate, } from './index.ts';
...
describe('projection model artifact schema', () => {
    ...
    it('rejects wrong model versions', () => {
        expect(() => projectionModelArtifactSchema.parse({ ...buildModel(), modelVersion: 'projection-model-v2' })).toThrow();
    });
...
describe('base five projection schema', () => {
    ...
    it('rejects a net rating that does not reconcile', () => {
        ...
        expect(() => baseFiveProjectionSchema.parse({
            ...base,
...
describe('season draft catalog v4', () => {
    function buildCatalog(): SeasonDraftCatalog {
        const candidate = (n: number, positions: [
            Position,
            ...Position[]
