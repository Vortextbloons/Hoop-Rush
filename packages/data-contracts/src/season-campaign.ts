...
export type SeasonCampaignComparisonOperator = z.infer<
  typeof seasonCampaignComparisonOperatorSchema
>;
...
export type SeasonCampaignEvolutionSelection = z.infer<
  typeof seasonCampaignEvolutionSelectionSchema
>;
...
export const seasonCampaignStateSchema = z
  .object({
    ...
    offers: z.record(
      ...
      z.array(seasonCampaignOpportunitySchema).length(2),
    ),
    selections: z.record(
      z.coerce.number().int().min(0).max(7),
      z.object({
    ...
    branchState: z.record(
      seasonCampaignBranchIdSchema,
      z.enum(['open', 'completed', 'missed', 'locked']),
    ),
  ...
  .superRefine((state, ctx) => {
    ...
    for (const ev of state.evaluations) {
      ...
      if (!(ev.blockIndex in state.selections) && state.startingIdentity !== null) {
        // Evaluations should have a selection, but allow empty early state
      }
    }
