...
export function resolveHistoricalIdentitySpans(manifest: HoopRushManifest, franchiseId: string, eraId: string): HistoricalIdentitySpan[] {
    ...
    for (const segment of manifest.franchiseLineage) {
        if (segment.modernFranchiseId !== franchiseId)
            continue;
        const fromSeasonKey = maxSeason(segment.validFromSeasonKey, era.fromSeasonKey);
        const throughSeasonKey = minSeason(segment.validThroughSeasonKey ?? INFINITE_SEASON, era.toSeasonKey);
        if (fromSeasonKey > throughSeasonKey)
            continue;
        spans.push({
            segment,
...
export function resolveEraTeamIdentity(manifest: HoopRushManifest, franchiseId: string, eraId: string): EraTeamIdentity {
...
function computeEraTeamIdentity(manifest: HoopRushManifest, franchiseId: string, eraId: string): EraTeamIdentity {
