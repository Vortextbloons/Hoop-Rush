...
import { SEASON_SEED_NAMESPACES, SEASON_TIEBREAK_VERSION, buildInitialPostseasonState, playoffGameIdOf, seasonAwardsDigest, seasonAwardsSchema, seasonNamespaceSeed, seasonPostseasonStateSchema, seasonRunCompletionSchema, seasonRunSchema, seasonTiebreakResolutionSchema, type SeasonAwards, type SeasonPostseasonState, type SeasonRun, type SeasonTiebreakResolution, type PlayoffRound, } from './index.ts';
...
describe('tiebreak resolution contract (M2.6, tiebreaker-v1)', () => {
    it('enforces teams (2-3), slots (1-10, 1-3 entries), and evidence bounds', () => {
        ...
        expect(schema.safeParse(buildTiebreakResolution({ teams: ['team-7', 'team-8', 'team-9', 'team-10'] })).success).toBe(false);
    ...
    it('enforces the drawSeed/rule coupling at parse time', () => {
        expect(seasonTiebreakResolutionSchema.safeParse(buildTiebreakResolution({ rule: 'head-to-head', drawSeed: SEED })).success).toBe(false);
        ...
        expect(seasonTiebreakResolutionSchema.safeParse(buildTiebreakResolution({ rule: 'random-draw', drawSeed: SEED })).success).toBe(true);
    });
...
describe('finals home court representability (M2.6)', () => {
    ...
    it('derives finalsHomeCourtDrawSeed deterministically as a pure function of the root seed', () => {
        ...
        expect(buildInitialPostseasonState('f'.repeat(32)).finalsHomeCourtDrawSeed).not.toBe(first.finalsHomeCourtDrawSeed);
