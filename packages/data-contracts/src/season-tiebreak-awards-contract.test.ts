...
import { SEASON_SEED_NAMESPACES, SEASON_TIEBREAK_VERSION, buildInitialPostseasonState, franchiseIdSchema, idSchema, playoffGameIdOf, seasonAwardsDigest, seasonAwardsSchema, seasonNamespaceSeed, seasonPostseasonStateSchema, seasonRunCompletionSchema, seasonRunSchema, seasonTiebreakResolutionSchema, seedSchema, type SeasonAwards, type SeasonPostseasonState, type SeasonRun, type SeasonTiebreakResolution, type PlayoffRound, } from './index.ts';
import { buildPostseason, buildRun, SEED, fixturePlayerId } from './season-schemas-fixtures.ts';
const ALMANAC_DIGEST = 'a'.repeat(32);
export function buildTiebreakResolution(overrides: Partial<SeasonTiebreakResolution> = {}): SeasonTiebreakResolution {
    return {
...
export function buildCompletedBracket(champion: string): NonNullable<SeasonPostseasonState['bracket']> {
...
export function buildCompletedPostseason(seed: string, champion = 'lakers', resolutions: SeasonTiebreakResolution[] = buildFinalsHomeCourtResolutions()): SeasonPostseasonState {
    const base = buildPostseason(seed);
    const seeds = Array.from({ length: 8 }, (_, i) => franchiseIdSchema.parse(`team-${String(i + 1)}`));
    const parsedChampion = franchiseIdSchema.parse(champion);
    return {
        ...base,
...
function rosterOf(run: SeasonRun, index: number): SeasonRun['rosters'][number] {
    ...
    if (roster === undefined)
        throw new Error(`fixture roster ${String(index)} missing`);
    return roster;
}
function playerOf(roster: SeasonRun['rosters'][number], slot: number): SeasonRun['rosters'][number]['players'][number] {
    const player = roster.players[slot];
    if (player === undefined)
        throw new Error(`fixture player slot ${String(slot)} missing`);
    return player;
}
...
describe('tiebreak resolution contract (M2.6, tiebreaker-v1)', () => {
    it('enforces teams (2-3), slots (1-10, 1-3 entries), and evidence bounds', () => {
        ...
        expect(schema.safeParse(buildTiebreakResolution({ teams: [franchiseIdSchema.parse('team-7')] }))
            .success).toBe(false);
        expect(schema.safeParse(buildTiebreakResolution({
            teams: [
                franchiseIdSchema.parse('team-7'),
                franchiseIdSchema.parse('team-8'),
                franchiseIdSchema.parse('team-9'),
        ...
        expect(schema.safeParse(buildTiebreakResolution({ evidence: nineEvidence })).success).toBe(false);
        expect(schema.safeParse(buildTiebreakResolution({ evidence: [{ label: '', value: 1 }] })).success).toBe(false);
        expect(schema.safeParse(buildTiebreakResolution({ evidence: [{ label: 'x'.repeat(65), value: 1 }] }))
            .success).toBe(false);
        expect(seasonTiebreakResolutionSchema.safeParse({
            ...buildTiebreakResolution(),
    ...
    it('enforces identity, conference, and drawSeed shapes', () => {
        const schema = seasonTiebreakResolutionSchema;
        expect(schema.safeParse({ ...buildTiebreakResolution(), resolutionId: 'Bad id!' }).success).toBe(false);
        expect(schema.safeParse({ ...buildTiebreakResolution(), conference: 'central' }).success).toBe(false);
        expect(schema.safeParse({ ...buildTiebreakResolution(), kind: 'tiebreak' }).success).toBe(false);
        expect(schema.safeParse({ ...buildTiebreakResolution(), rule: 'coin-flip' }).success).toBe(false);
    ...
    it('enforces the drawSeed/rule coupling at parse time', () => {
        expect(seasonTiebreakResolutionSchema.safeParse(buildTiebreakResolution({ rule: 'head-to-head', drawSeed: seedSchema.parse(SEED) })).success).toBe(false);
        expect(seasonTiebreakResolutionSchema.safeParse(buildTiebreakResolution({ rule: 'random-draw', drawSeed: null })).success).toBe(false);
        expect(seasonTiebreakResolutionSchema.safeParse(buildTiebreakResolution({ rule: 'random-draw', drawSeed: seedSchema.parse(SEED) })).success).toBe(true);
    });
    it('records the finals-home-court decision at a positive slot (position 1)', () => {
        ...
        expect(seasonTiebreakResolutionSchema.safeParse(buildTiebreakResolution({ kind: 'finals-home-court', slots: [0] })).success).toBe(false);
    });
});
describe('finals home court representability (M2.6)', () => {
    ...
    it('derives finalsHomeCourtDrawSeed deterministically as a pure function of the root seed', () => {
        ...
        expect(first.finalsHomeCourtDrawSeed).toBe(seasonNamespaceSeed(tiesSeed, SEASON_SEED_NAMESPACES.postseasonDraws, 'finals-home-court'));
        expect(buildPostseason(SEED).finalsHomeCourtDrawSeed).toBe(first.finalsHomeCourtDrawSeed);
        expect(buildInitialPostseasonState(seedSchema.parse('f'.repeat(32))).finalsHomeCourtDrawSeed).not.toBe(first.finalsHomeCourtDrawSeed);
...
describe('tie resolutions on a completed postseason state (M2.6)', () => {
    ...
    it('rejects a bracket whose champion does not match the state champion', () => {
        ...
        if (state.bracket === null)
            throw new Error('fixture bracket missing');
        expect(() => seasonPostseasonStateSchema.parse({ ...state, championFranchiseId: null })).toThrow();
...
describe('season awards contract (M2.6, awards-v1)', () => {
    ...
    it('rejects malformed recipients', () => {
        const awards = buildAwardsForRun(buildRun());
        expect(() => seasonAwardsSchema.parse({ ...awards, mvp: { playerVersionId: 'bad' } })).toThrow();
        expect(() => seasonAwardsSchema.parse({
            ...awards,
...
describe('run-level stage/completion/awards coupling (M2.6)', () => {
    ...
    it('enforces the awards/stage coupling at parse time', () => {
        const run = buildRun();
        expect(seasonRunSchema.safeParse({ ...run, awards: buildAwardsForRun(run) }).success).toBe(false);
        expect(seasonRunSchema.safeParse({
            ...buildCompletedRunWithAwards('lakers'),
            stage: 'playoffs',
            completion: null,
        }).success).toBe(true);
        expect(seasonRunSchema.safeParse({ ...buildCompletedRunWithAwards('lakers'), awards: null }).success).toBe(true);
    });
});
