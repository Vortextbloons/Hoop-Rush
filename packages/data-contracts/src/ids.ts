...
export const seasonGameIdSchema = z.string().regex(/^s[0-9]{6}$/).brand<'SeasonGameId'>();
...
export const seasonKeySchema = z.string().regex(/^(19|20)\d{2}-\d{2}$/).brand<'SeasonKey'>();
...
export const seedSchema = z.string().regex(/^[0-9a-f]{16,64}$/).brand<'Seed'>();
export type Seed = z.infer<typeof seedSchema>;
export const contentHashSchema = z.string().regex(/^[0-9a-f]{64}$/).brand<'ContentHash'>();
