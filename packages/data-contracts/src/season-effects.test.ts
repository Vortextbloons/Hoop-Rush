...
function buildCheckpoint(effects: SeasonEffectsState): SeasonCandidateCheckpoint {
  ...
  return {
    ...
    versions: {
      ...
      influenceTargetsVersion: 'influence-targets-v3',
      freeAgencyVersion: SEASON_FREE_AGENCY_VERSION,
      freeAgencyIndexVersion: 'free-agency-index-v2',
      freeAgencyTargetsVersion: 'free-agency-targets-v2',
    },
    blockIndex: 0,
    completedRounds: 0,
    revision: 0,
...
describe('season stamina input schema (M2.4)', () => {
  ...
  it('round-trips a valid profile', () => {
    ...
    expect(parsed.derivationVersion).toBe('season-stamina-v2');
  });
...
describe('season pair chemistry state schema (M2.4)', () => {
  it('accepts canonical pairs only', () => {
    ...
    expect(() =>
      seasonPairChemistryStateSchema.parse({ ...canonical, sharedPossessions: 10000001 }),
    ).toThrow();
  });
});
...
describe('season mechanism evidence schema (M2.4)', () => {
  ...
  it('rejects unknown mechanisms, sides, and out-of-range accumulators', () => {
    ...
    expect(() =>
      seasonMechanismEvidenceSchema.parse({ ...MECHANISM_EVIDENCE, opportunities: 1000001 }),
    ).toThrow();
    ...
    expect(() =>
      seasonMechanismEvidenceSchema.parse({ ...MECHANISM_EVIDENCE, deltaMin: -1000001 }),
    ).toThrow();
    expect(() =>
      seasonMechanismEvidenceSchema.parse({ ...MECHANISM_EVIDENCE, deltaMax: 1000001 }),
    ).toThrow();
  });
});
...
describe('season run schema version 7 (M2.5)', () => {
  ...
  it('freezes the seven M2.5 material versions and the state chain on the run', () => {
    ...
    expect(run.versions.tradeVersion).toBe('season-trade-v7');
