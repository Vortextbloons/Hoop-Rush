import { z } from 'zod';
import { commandIdSchema, contentHashSchema, franchiseIdSchema, idSchema, playerIdSchema, seedSchema, } from './ids.ts';
...
export { FIXED_FIVE_ROOM_SCHEMA_VERSION, FIXED_FIVE_ROOM_PROTOCOL_VERSION, FIXED_FIVE_MULTIPLAYER_VERSION, FIXED_FIVE_AUTOPICK_VERSION, FIXED_FIVE_WORKER_WIRE_VERSION, FIXED_FIVE_ENVELOPE_MAX_BYTES, FIXED_FIVE_CODE_TTL_MS, FIXED_FIVE_ROOM_RETENTION_MS, FIXED_FIVE_CLASSIC_PICK_TIMEOUT_MS, FIXED_FIVE_DUEL_PICK_TIMEOUT_MS, FIXED_FIVE_SANDBOX_BUILD_TIMEOUT_MS, } from './fixed-five-versions.ts';
...
export function defaultFixedFiveVersionLocks(overrides: Partial<FixedFiveVersionLocks>): FixedFiveVersionLocks {
    return {
...
export type FixedFiveShared82ParticipantSummary = z.infer<typeof fixedFiveShared82ParticipantSummarySchema>;
...
export interface FixedFiveMultiplayerTransport {
    create(settings: Omit<FixedFiveRoomSettings, 'schemaVersion' | 'timerPolicyVersion'> & Partial<Pick<FixedFiveRoomSettings, 'schemaVersion' | 'timerPolicyVersion'>>): Promise<{
    ...
    subscribe(roomId: string, handler: (snapshot: FixedFiveRoomSnapshot) => void): {
        unsubscribe: () => void;
    };
    refetch(roomId: string, afterOrdinal: number): Promise<FixedFiveCommand[]>;
    submitCommand(command: Omit<FixedFiveCommand, 'ordinal'> & {
        ordinal?: number;
        expectedRevision?: number;
    }): Promise<FixedFiveCommandReceipt>;
    resolveTimeout(roomId: string): Promise<FixedFiveCommandReceipt | null>;
    removeGuest(roomId: string, targetParticipantId: FixedFiveParticipantId): Promise<FixedFiveRoomSnapshot>;
    leave(roomId: string, participantId: FixedFiveParticipantId): Promise<void>;
    rematch(roomId: string): Promise<{
        snapshot: FixedFiveRoomSnapshot;
        code: FixedFiveRoomCode;
    }>;
    complete(roomId: string, resultDigest: string): Promise<{
        completed: boolean;
        phase: FixedFiveRoomPhase;
    }>;
    fail(roomId: string): Promise<{
        failed: boolean;
        phase: FixedFiveRoomPhase;
    }>;
}
