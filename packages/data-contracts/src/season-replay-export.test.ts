...
import { buildRun, contentHashSchema, eraIdSchema, franchiseIdSchema, idSchema, seasonAlmanacSchema, seasonCommandLogEntrySchema, seasonCommandLogSchema, seasonPostseasonSummarySchema, seasonReplayExportSchema, seasonRunReplayExportSchema, seedSchema, type SeasonAlmanac, type SeasonCommandLog, type SeasonCommandLogEntry, type SeasonPostseasonSummary, type SeasonRunReplayExport, type SeasonRunReplayExportInput, } from './index.ts';
import { buildSeasonRunReplayExport, seasonRunReplayExportDigest } from './season-replay-export.ts';
import { SEASON_EMPTY_COMMAND_LOG_DIGEST, seasonCommandLogDigest, seasonCommandResultDigest, } from './season-command-log.ts';
...
describe('full-run replay export (replay-export-v1)', () => {
    it('builds a byte-stable export: identical inputs, identical bytes and digest', () => {
        ...
        const parsed = seasonRunReplayExportSchema.parse(JSON.parse(JSON.stringify(first)) as SeasonRunReplayExport);
    ...
    it('rejects identity mismatches and an unreconciled almanac', () => {
        const input = exportInput();
        expect(() => buildSeasonRunReplayExport({ ...input, runId: 'other-run' })).toThrow(/command log targets a different run/);
        expect(() => buildSeasonRunReplayExport({
            ...input,
            championFranchiseId: 'celtics',
...
describe('seasonCommandResultDigest (command-log-v1 shared convention)', () => {
    it('is a stable pure function of the recorded result facts', () => {
        ...
        expect(seasonCommandResultDigest({
            commandId: 'cmd-adv-2',
