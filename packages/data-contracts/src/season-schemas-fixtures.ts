import type { SeasonCandidateCheckpoint, SeasonEffectsState, SeasonGame, SeasonGamePlayerInput, SeasonGameSummary, SeasonHealthState, SeasonInfluenceState, SeasonLeague, SeasonPairChemistryState, SeasonPendingBlockCandidate, SeasonPlayerLoadState, SeasonRun, SeasonSchedule, SeasonTeamAggregate, SeasonPlayerAggregate, } from './index.ts';
...
import { commandIdSchema, franchiseIdSchema, idSchema, seasonGameIdSchema, seedSchema, } from './ids.ts';
...
export const DIVISION_OF: Record<string, string> = Object.fromEntries(SEASON_ALIGNMENT.map((entry) => [entry.franchiseId, entry.division]));
    ...
    for (let round = 1; round <= 82; round += 1) {
        for (let g = 0; g < 15; g += 1) {
            ...
            if (home === undefined || away === undefined)
                throw new Error('fixture teams out of range');
            sequence += 1;
            games.push({
...
export function buildEmptyCampaign() {
    return {
        ...
        campaignVersion: 'season-campaign-v2' as const,
...
export function buildRun(): SeasonRun {
    ...
    const rosters = league.teams.map((team, teamIndex) => seasonRosterSchema.parse({
        franchiseId: team.franchiseId,
        players: Array.from({ length: 10 }, (_, slot) => ({
            playerVersionId: `pv-${String(teamIndex).padStart(2, '0')}${String(slot).padStart(2, '0')}`.padEnd(3 + 32, '0'),
    ...
    return seasonRunSchema.parse({
        ...
        versions: {
            ...
            checkpointVersion: 'season-checkpoint-v6',
            ...
            campaignVersion: 'season-campaign-v2',
            ...
            replayExportVersion: 'replay-export-v2',
...
function buildFixtureAiPools(league: SeasonLeague, aiAssignments: SeasonRun['aiAssignments']): SeasonRun['aiPools'] {
    return aiAssignments
        ...
        if (anchor === undefined)
            throw new Error('fixture pool too small');
...
export function buildFixturePlayerLine(playerVersionId: string): SeasonGameSummary['homePlayers'][number] {
    return {
        playerVersionId,
        seconds: 0,
        points: 0,
...
export function buildSummaryFixture(): SeasonGameSummary {
    return seasonGameSummarySchema.parse({
        ...
        homePlayers: Array.from({ length: 10 }, (_, index) => buildFixturePlayerLine(fixturePlayerId(index))),
        awayPlayers: Array.from({ length: 10 }, (_, index) => buildFixturePlayerLine(fixturePlayerId(10 + index))),
        injuryEvents: [],
    });
}
