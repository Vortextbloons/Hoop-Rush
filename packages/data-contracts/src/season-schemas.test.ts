...
import { seasonCursorSchema, seasonGameSchema, seasonLeagueSchema, seasonPostseasonStateSchema, playoffSeriesSchema, seasonRunSchema, seasonScheduleSchema, seasonStandingsSchema, seasonDraftCatalogSchema, seasonDraftStateSchema, seasonDraftLegacyStateSchema, storedSeasonDraftStateSchema, seasonDraftCommandSchema, seasonDraftCommandRecordSchema, seasonRotationSchema, seasonAiAssignmentSchema, seasonAiPoolSchema, seasonGenerationDiagnosticsSchema, seasonLeagueGenerationResultSchema, seasonRosterTargetsSchema, seasonDraftRejectedRecordSchema, seasonHealthStateSchema, seasonInjuryRecordSchema, seasonTransactionEntrySchema, seasonInfluenceStateSchema, seasonInfluenceLedgerEntrySchema, seasonObjectiveStateSchema, seasonObjectiveEvaluationSchema, seasonTradeOfferSchema, seasonPendingBlockCandidateSchema, seasonInvalidRosterInterruptionSchema, seasonSelectBlockObjectiveCommandSchema, seasonSpendInfluenceCommandSchema, seasonAcceptTradeOfferCommandSchema, seasonDeclineTradeOfferCommandSchema, seasonResumeSeasonBlockCommandSchema, seasonForfeitInterruptedGameCommandSchema, seasonStartPostseasonCommandSchema, seasonAdvancePostseasonCommandSchema, seasonSubmitPostseasonRotationCommandSchema, seasonSpectatePostseasonGameCommandSchema, seasonFastForwardPostseasonCommandSchema, seasonRunCommandSchema, seasonRunCommandRejectionSchema, seasonSelectBlockObjectiveResultSchema, seasonSpendInfluenceResultSchema, seasonAcceptTradeOfferResultSchema, seasonSubmitBlockCommandSchema, seasonInvalidObjectiveRejectionSchema, seasonSubmitBlockRejectionSchema, seasonCandidateCheckpointSchema, seasonAcceptedBlockSchema, seasonCheckpointStateSchema, seasonWorkerStartRequestSchema, seasonWorkerCancelRequestSchema, seasonWorkerCompleteMessageSchema, seasonGameSummarySchema, seasonCompactInjuryEventSchema, seasonBlockRecapSchema, SEASON_NEUTRAL_HOME_COURT, RATINGS_VERSION, seasonMinutePolicySchema, } from './index.ts';
import { buildEmptyHealth, buildInitialInfluence, buildCheckpointFixture, buildPendingBlockFixture, buildLeague, buildPostseason, buildRun, buildSchedule, buildSummaryFixture, fixturePlayerId, SEED, } from './season-schemas-fixtures.ts';
...
describe('season league schema', () => {
    ...
    it('rejects a wrong league version', () => {
        expect(() => seasonLeagueSchema.parse({ ...buildLeague(), leagueVersion: 'league-v2' })).toThrow();
    });
    ...
    it('rejects invalid team counts', () => {
        ...
        expect(() => seasonLeagueSchema.parse({ ...league, teams: league.teams.slice(0, 29) })).toThrow();
    ...
    it('rejects an unknown franchise id and division', () => {
        ...
        expect(() => seasonLeagueSchema.parse({
            ...league,
...
describe('season schedule schema', () => {
    ...
    it('rejects wrong versions and wrong counts', () => {
        ...
        expect(() => seasonScheduleSchema.parse({ ...schedule, scheduleVersion: 'schedule-v9' })).toThrow();
        ...
        expect(() => seasonScheduleSchema.parse({ ...schedule, games: schedule.games.slice(0, 1229) })).toThrow();
    });
...
describe('season game schema', () => {
    ...
    it('rejects corrupt game states', () => {
        ...
        expect(() => seasonGameSchema.parse({ ...forfeit, forfeitLoserFranchiseId: 'celtics' })).not.toThrow();
        expect(() => seasonGameSchema.parse({ ...forfeit, forfeitLoserFranchiseId: 'bulls' })).toThrow();
        expect(() => seasonGameSchema.parse({ ...base, status: 'scheduled', homeScore: null, awayScore: null })).not.toThrow();
...
describe('season standings schema', () => {
    ...
    it('rejects wrong versions and team counts', () => {
        ...
        expect(() => seasonStandingsSchema.parse({ ...standings, standingsVersion: 'standings-v2' })).toThrow();
        expect(() => seasonStandingsSchema.parse({ ...standings, rows: standings.rows.slice(0, 29) })).toThrow();
    });
    it('carries accounting invariants as domain facts, not schema rules', () => {
        const run = buildRun();
        const standings = run.standings;
        const rows = standings.rows.map((row, index) => index === 0 ? { ...row, wins: 5, losses: 2, gamesPlayed: 6 } : row);
...
describe('season postseason schema (M2.6 postseason-v2)', () => {
    ...
    it('rejects corrupt play-in states', () => {
        ...
        expect(() => seasonPostseasonStateSchema.parse({ ...state, championFranchiseId: 'lakers' })).toThrow();
    });
...
describe('season run schema', () => {
    ...
    it('accepts 300-450 ownership rows; uniqueness is an engine-level audit', () => {
        ...
        if (!first)
            throw new Error('no ownership rows');
        expect(() => seasonRunSchema.parse({ ...run, ownership: [...duplicated, { ...first }] })).not.toThrow();
        expect(() => seasonRunSchema.parse({ ...run, ownership: duplicated.slice(0, 299) })).toThrow();
    });
    it('rejects malformed rosters', () => {
        const run = buildRun();
        const rosters = run.rosters.map((roster, index) => index === 0 ? { ...roster, players: roster.players.slice(0, 9) } : roster);
    ...
    it('accepts two versions of the same person on one roster', () => {
        ...
        const rosters = run.rosters.map((roster, index) => {
            ...
            return {
                ...roster,
                players: roster.players.map((player, slot) => slot === 1 ? { ...player, playerId: first.playerId } : player),
            };
        });
    ...
    it('rejects duplicate player versions on one roster', () => {
        ...
        const rosters = run.rosters.map((roster, index) => {
            ...
            return {
                ...roster,
                players: roster.players.map((player, slot) => slot === 1 ? { ...player, playerVersionId: first.playerVersionId } : player),
            };
        });
        expect(() => seasonRunSchema.parse({ ...run, rosters })).toThrow();
    });
    it('rejects a roster entry with an undecodable playerVersionId', () => {
        const run = buildRun();
        const rosters = run.rosters.map((roster, index) => index === 0
            ? {
                ...roster,
                players: roster.players.map((player, slot) => slot === 0 ? { ...player, playerVersionId: 'pv-nothex' } : player),
            }
            : roster);
        expect(() => seasonRunSchema.parse({ ...run, rosters })).toThrow();
    });
    ...
    it('rejects an invalid cursor and wrong game count', () => {
        const run = buildRun();
        expect(() => seasonRunSchema.parse({ ...run, cursor: { schemaVersion: 1, completedRounds: 99 } })).toThrow();
...
describe('season seed derivation', () => {
    it('is deterministic, namespaced, and order-independent', async () => {
        ...
        const games = seasonNamespaceSeed(seed, SEASON_SEED_NAMESPACES.scheduleGames, 'lakers', 'celtics');
        const gamesSwapped = seasonNamespaceSeed(seed, SEASON_SEED_NAMESPACES.scheduleGames, 'celtics', 'lakers');
        expect(games).not.toBe(gamesSwapped);
        expect(games).toBe(seasonNamespaceSeed(seed, SEASON_SEED_NAMESPACES.scheduleGames, 'lakers', 'celtics'));
    ...
    it('pins stable derivation vectors for the committed run seed', async () => {
        ...
        expect(seasonNamespaceSeed(seed, SEASON_SEED_NAMESPACES.draft)).toBe('bf6f373420fee6f04bf1da36074ce784');
        expect(seasonNamespaceSeed(seed, SEASON_SEED_NAMESPACES.aiRosters)).toBe('d5edcf36a61755a2ff9f80582bb5ee46');
        expect(seasonNamespaceSeed(seed, SEASON_SEED_NAMESPACES.scheduleGames, 'lakers', 'celtics')).toBe('c70523e788d2756bde47e3a90fadf0d7');
        expect(seasonNamespaceSeed(seed, SEASON_SEED_NAMESPACES.postseasonTies)).toBe('cc022296a9b3989a9ea1d1943d6e5186');
        expect(seasonNamespaceSeed(seed, SEASON_SEED_NAMESPACES.injuries)).toBe('3c4fc7a600628bba49f5d95c9815f3f6');
        expect(seasonNamespaceSeed(seed, SEASON_SEED_NAMESPACES.trades)).toBe('5a0b6036864e4e3a80a4a8b460544a46');
        expect(seasonNamespaceSeed(seed, SEASON_SEED_NAMESPACES.upgrades)).toBe('4b8c47c4507c7a30616462d624a142f4');
        expect(playerVersionId('p-synth-1', 'lakers', '1990s', '1995-96')).toBe('pv-345baf6811b5d914c163685a20974538');
    });
});
describe('season draft catalog schema (M2.1)', () => {
    ...
    it('rejects wrong catalog and identity versions', () => {
        expect(() => seasonDraftCatalogSchema.parse({ ...buildCatalog(), catalogVersion: 'season-draft-v1' })).toThrow();
        expect(() => seasonDraftCatalogSchema.parse({ ...buildCatalog(), catalogVersion: 'season-draft-v2' })).toThrow();
        expect(() => seasonDraftCatalogSchema.parse({ ...buildCatalog(), playerVersionIdVersion: 'pv-v2' })).toThrow();
        expect(() => seasonDraftCatalogSchema.parse({ ...buildCatalog(), staminaVersion: 'season-stamina-v3' })).toThrow();
        expect(() => seasonDraftCatalogSchema.parse({ ...buildCatalog(), durabilityVersion: 'durability-v2' })).toThrow();
    });
    ...
    it('rejects a candidate without a valid stamina profile', () => {
        const catalog = buildCatalog();
        const candidates = catalog.candidates.map((candidate, index) => index === 0 ? { ...candidate, stamina: { ...candidate.stamina, rating: 44 } } : candidate);
        expect(() => seasonDraftCatalogSchema.parse({ ...catalog, candidates })).toThrow();
    });
    it('rejects a candidate without a valid durability rating', () => {
        const catalog = buildCatalog();
        const candidates = catalog.candidates.map((candidate, index) => index === 0
...
describe('season draft state schema (M2.3.5 season-draft-v2)', () => {
    ...
    it('rejects wrong draft versions and malformed offers', () => {
        expect(() => seasonDraftStateSchema.parse({ ...baseState, draftVersion: 'season-draft-v1' })).toThrow();
        expect(() => seasonDraftStateSchema.parse({ ...baseState, catalogVersion: 'season-draft-v1' })).toThrow();
        expect(() => seasonDraftStateSchema.parse({
            ...baseState,
        ...
        expect(() => seasonDraftStateSchema.parse({
            ...
            currentOffer: {
                ...baseState.currentOffer,
                cards: cards.map((card, index) => index === 0 ? { ...card, coverageReason: 'must be null' } : card),
            },
        })).toThrow();
        expect(() => seasonDraftStateSchema.parse({
            ...baseState,
            currentOffer: {
                ...baseState.currentOffer,
                cards: cards.map((card, index) => index === 4 ? { ...card, coverageReason: null } : card),
            },
        })).toThrow();
    });
...
describe('season draft command records (M2.1)', () => {
    ...
    it('round-trips an accepted record and rejects mismatched kinds', () => {
        ...
        expect(() => seasonDraftCommandRecordSchema.parse({
            status: 'accepted',
            commandId: 'x',
            revisionBefore: 0,
            revisionAfter: 1,
    ...
    it('round-trips a rejected record with a typed error code', () => {
        ...
        expect(roundTrip(seasonDraftRejectedRecordSchema, record).errorCode).toBe('GENERATION_EXHAUSTED');
        expect(() => seasonDraftRejectedRecordSchema.parse({ ...record, errorCode: 'NOT_A_CODE' })).toThrow();
    });
    it('validates command envelope shapes', () => {
        expect(() => seasonDraftCommandSchema.parse({ ...command, expectedRevision: -1 })).toThrow();
        expect(() => seasonDraftCommandSchema.parse({
            ...command,
...
describe('season rotation schema (M2.1)', () => {
    ...
    it('rejects illegal rotations', () => {
        expect(() => seasonRotationSchema.parse({
            ...rotation,
            targetMinutes: rotation.targetMinutes.slice(0, 9),
        })).toThrow();
        expect(() => seasonRotationSchema.parse({ ...rotation, rotationVersion: 'season-rotation-v1' })).toThrow();
        expect(() => seasonRotationSchema.parse({
            ...rotation,
    ...
    it('round-trips the minute policy and rejects bad strategies or versions', () => {
        ...
        expect(() => seasonMinutePolicySchema.parse({
            policyVersion: 'minute-policy-v1',
            strategy: 'tight',
        })).toThrow();
        expect(() => seasonMinutePolicySchema.parse({
    ...
    it('requires a minute policy on every rotation', () => {
        ...
        expect(() => seasonRotationSchema.parse({ ...rotation, rotationVersion: 'season-rotation-v2' })).toThrow();
    });
});
describe('season AI contracts (M2.1, M2.4 roster-generation-v2)', () => {
    ...
    it('round-trips a pool and rejects duplicate versions, outside selections, and invalid anchors', () => {
        ...
        expect(() => seasonAiPoolSchema.parse({
            ...pool,
    ...
    it('rejects wrong versions in the generation result and targets', () => {
        ...
        expect(() => seasonLeagueGenerationResultSchema.parse({ ...result, aiVersion: 'season-ai-v1' })).toThrow();
        expect(() => seasonLeagueGenerationResultSchema.parse({ ...result, schemaVersion: 1 })).toThrow();
        expect(() => seasonLeagueGenerationResultSchema.parse({
            ...result,
    ...
    it('rejects the v1 targets artifact, wrong target versions, and malformed v2 policy', () => {
        ...
        expect(() => seasonRosterTargetsSchema.parse({
            schemaVersion: 2,
            targetsVersion: 'roster-targets-v1',
        })).toThrow();
    ...
    it('round-trips a schema-11 run with its aiPools and M2.5.5 state', () => {
        ...
        expect(() => seasonRunSchema.parse({ ...run, aiPools: run.aiPools.slice(0, 28) })).not.toThrow();
...
describe('season health family (M2.5, season-health-v2)', () => {
    it('round-trips an empty health state and rejects wrong versions', () => {
        ...
        expect(() => seasonHealthStateSchema.parse({ ...buildEmptyHealth(), healthVersion: 'season-health-v1' })).toThrow();
        expect(() => seasonHealthStateSchema.parse({ ...buildEmptyHealth(), schemaVersion: 2 })).toThrow();
    });
    it('round-trips an injury record and rejects corrupt shapes', () => {
        ...
        expect(() => seasonInjuryRecordSchema.parse({ ...record, missedGamesRemaining: 10001 })).toThrow();
    });
});
describe('season transaction family (M2.5)', () => {
    it('round-trips an entry and rejects corrupt shapes', () => {
        ...
        expect(() => seasonTransactionEntrySchema.parse({ ...entry, transactionId: 'Tx-Bad' })).toThrow();
        expect(() => seasonTransactionEntrySchema.parse({ ...entry, explanation: 'x'.repeat(513) })).toThrow();
...
describe('season influence family (M2.5, season-influence-v2)', () => {
    ...
    it('rejects missing franchises, wrong versions, and out-of-range balances', () => {
        ...
        expect(() => seasonInfluenceStateSchema.parse({ ...state, influenceVersion: 'season-influence-v1' })).toThrow();
        expect(() => seasonInfluenceStateSchema.parse({
            ...state,
...
describe('season trade family (M2.5, season-trade-v1)', () => {
    ...
    it('round-trips a 2-for-2 offer and rejects shape violations', () => {
        ...
        expect(() => seasonTradeOfferSchema.parse({
            ...buildOffer(),
        ...
        expect(() => seasonTradeOfferSchema.parse({ ...buildOffer(), projectedRotationChanges: 'x'.repeat(513) })).toThrow();
    });
...
describe('season pending block family (M2.5)', () => {
    ...
    it('rejects wrong versions and corrupt fields', () => {
        ...
        expect(() => seasonPendingBlockCandidateSchema.parse({ ...pending, blockVersion: 'season-block-v2' })).toThrow();
        expect(() => seasonPendingBlockCandidateSchema.parse({ ...pending, nextGameId: 'not-a-game' })).toThrow();
        expect(() => seasonPendingBlockCandidateSchema.parse({
            ...pending,
            summaries: Array.from({ length: 151 }, () => buildSummaryFixture()),
        })).toThrow();
        expect(() => seasonPendingBlockCandidateSchema.parse({ ...pending, rotationDigest: 'x'.repeat(32) })).toThrow();
    });
    it('round-trips an invalid-roster interruption', () => {
        ...
        expect(roundTrip(seasonInvalidRosterInterruptionSchema, interruption).code).toBe('invalid-roster');
        expect(() => seasonInvalidRosterInterruptionSchema.parse({
            ...interruption,
            unavailablePlayerVersionIds: [],
        })).toThrow();
        expect(() => seasonInvalidRosterInterruptionSchema.parse({ ...interruption, code: 'invalid-rotation' })).toThrow();
    });
});
describe('season commands (M2.5/M2.6, schema 11)', () => {
    ...
    it('parses per-command rejection unions and result envelopes', () => {
        ...
        if (acceptedObjective.status !== 'accepted')
        ...
        if (rejectedObjective.status !== 'rejected')
        ...
        if (acceptedSpend.status !== 'accepted')
            throw new Error('expected accepted');
        ...
        if (acceptedTrade.status !== 'accepted')
            throw new Error('expected accepted');
    ...
    it('parses submit-season-block with the M2.5 objective and state fields', () => {
        ...
        expect(seasonRunCommandSchema.safeParse(command).success).toBe(true);
        expect(() => seasonSubmitBlockCommandSchema.parse({ ...command, blockVersion: 'season-block-v2' })).toThrow();
        expect(() => seasonSubmitBlockCommandSchema.parse({ ...command, expectedStateDigest: 'bad' })).toThrow();
    });
    it('parses the invalid-objective rejection on the submit rejection union', () => {
        expect(() => seasonInvalidObjectiveRejectionSchema.parse({
        ...
        expect(() => seasonSubmitBlockRejectionSchema.parse({
            code: 'invalid-objective',
...
describe('season checkpoint M2.5 facts', () => {
    ...
    it('rejects a season-checkpoint-v2 candidate and missing M2.5 facts', () => {
        ...
        expect(() => seasonCandidateCheckpointSchema.parse({
            ...checkpoint,
            checkpointVersion: 'season-checkpoint-v2',
        })).toThrow();
        expect(() => seasonCandidateCheckpointSchema.parse({ ...checkpoint, health: undefined })).toThrow();
        expect(() => seasonCandidateCheckpointSchema.parse({ ...checkpoint, stateRevision: undefined })).toThrow();
        expect(() => seasonCandidateCheckpointSchema.parse({
            ...checkpoint,
    ...
    it('round-trips an accepted block with the state chain and a checkpoint state', () => {
        ...
        expect(() => seasonCheckpointStateSchema.parse({ ...checkpointState, checkpointDigest: 'bad' })).toThrow();
    });
});
describe('season worker wire v7 (M2.5.5)', () => {
    ...
    it('rejects wire schema 3 requests and messages', () => {
        ...
        expect(() => seasonWorkerCancelRequestSchema.parse({
            schemaVersion: 3,
    ...
    it('round-trips complete messages with committed and interrupted results', () => {
        ...
        expect(() => seasonWorkerCompleteMessageSchema.parse({
            schemaVersion: 7,
            type: 'season-block-complete',
            requestId: 'req-1',
...
describe('season game summary injury events (M2.5)', () => {
    it('round-trips a summary with compact injury events', () => {
        ...
        expect(() => seasonGameSummarySchema.parse({ ...buildSummaryFixture(), injuryEvents: undefined })).toThrow();
        expect(() => seasonCompactInjuryEventSchema.parse({
            playerVersionId: fixturePlayerId(0),
            side: 'away',
            type: 'soft-tissue',
            severity: 'minor',
