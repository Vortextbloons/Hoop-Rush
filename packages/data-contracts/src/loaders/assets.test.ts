...
import { eraIdSchema, franchiseIdSchema, peakPlayerSeasonSchema, playerIdSchema, seasonKeySchema, } from '../index.ts';
import { isNbaCdnHeadshotUrl, resolveHeadshotUrl, resolveHeadshotUrls, resolveLogoUrl, resolveLogoUrls, resolveLogoUrlsWithHistorical, resolveSecondaryHeadshotUrl, resolveSecondaryLogoUrl, shouldStallTimeoutHeadshot, } from './assets.ts';
const assets: HoopRushManifest['assets'] = {
    ...
    headshotUrlTemplateSecondary: 'https://www.basketball-reference.com/req/20200617/images/headshots/{altIds.bbref}.jpg',
...
const player = (overrides: Partial<PeakPlayerSeason> = {}): PeakPlayerSeason => peakPlayerSeasonSchema.parse({
    schemaVersion: 3,
...
describe('headshot URL resolution', () => {
    it('resolves the primary NBA CDN template', () => {
        expect(resolveHeadshotUrl(manifest(), '77142')).toBe('https://cdn.nba.com/headshots/nba/latest/1040x760/77142.png');
    });
    ...
    it('resolves the secondary template from the player alt id', () => {
        expect(resolveSecondaryHeadshotUrl(manifest(), player({ altIds: { bbref: 'jordami01' } }))).toBe('https://www.basketball-reference.com/req/20200617/images/headshots/jordami01.jpg');
    });
    ...
    it('returns null for the secondary URL without a template', () => {
        expect(resolveSecondaryHeadshotUrl(manifest({ headshotUrlTemplateSecondary: null }), player({ altIds: { bbref: 'jordami01' } }))).toBeNull();
    });
    ...
    it('skips the primary NBA CDN url when the packaged record marks it unavailable', () => {
        expect(resolveHeadshotUrls(manifest(), player({ altIds: { bbref: 'jordami01', nbaHeadshotAvailable: false } }))).toEqual(['https://www.basketball-reference.com/req/20200617/images/headshots/jordami01.jpg']);
    });
    it('detects NBA CDN headshot urls', () => {
        expect(isNbaCdnHeadshotUrl('https://cdn.nba.com/headshots/nba/latest/1040x760/101.png')).toBe(true);
        expect(isNbaCdnHeadshotUrl('https://www.basketball-reference.com/req/20200617/images/headshots/jordami01.jpg')).toBe(false);
    });
    it('keeps confirmed NBA headshots from timing out into wiki-only fallbacks', () => {
        ...
        expect(shouldStallTimeoutHeadshot(nba, [nba, wiki], 0, {
            altIds: { nbaHeadshotAvailable: true },
        })).toBe(false);
        expect(shouldStallTimeoutHeadshot(nba, [nba, bbref, wiki], 0, {
            altIds: { nbaHeadshotAvailable: true },
        })).toBe(true);
        expect(shouldStallTimeoutHeadshot(nba, [nba, wiki], 0, {
    ...
    it('appends the direct photo url after the secondary candidate', () => {
        expect(resolveHeadshotUrls(manifest(), player({
            altIds: {
                bbref: 'jordami01',
    ...
    it('returns an empty list when nothing can resolve', () => {
        expect(resolveHeadshotUrls(manifest({ headshotUrlTemplate: null, headshotUrlTemplateSecondary: null }), player())).toEqual([]);
    });
});
describe('logo URL resolution', () => {
    it('resolves the primary NBA CDN template', () => {
        expect(resolveLogoUrl(manifest(), '1610612747')).toBe('https://cdn.nba.com/logos/nba/1610612747/global/L/logo.svg');
    });
    it('resolves the secondary template from the franchise abbreviation', () => {
        expect(resolveSecondaryLogoUrl(manifest(), 'lakers')).toBe('https://a.espncdn.com/i/teamlogos/nba/500/lal.png');
    });
    it('uses the legacy host slugs for the Pelicans and Jazz', () => {
        expect(resolveSecondaryLogoUrl(manifest(), 'pelicans')).toBe('https://a.espncdn.com/i/teamlogos/nba/500/no.png');
        expect(resolveSecondaryLogoUrl(manifest(), 'jazz')).toBe('https://a.espncdn.com/i/teamlogos/nba/500/utah.png');
    });
    it('returns null for the secondary URL without a template', () => {
        expect(resolveSecondaryLogoUrl(manifest({ logoUrlTemplateSecondary: null }), 'lakers')).toBeNull();
    });
    it('orders candidates primary before secondary', () => {
    ...
    it('returns an empty list when nothing can resolve', () => {
        expect(resolveLogoUrls(manifest({ logoUrlTemplate: null, logoUrlTemplateSecondary: null }), 'lakers', '1610612747')).toEqual([]);
    });
    it('orders verified historical candidates before the modern chain', () => {
        expect(resolveLogoUrlsWithHistorical(manifest(), 'thunder', '1610612760', [
    ...
    it('equals the modern chain when no historical candidates exist', () => {
        expect(resolveLogoUrlsWithHistorical(manifest(), 'lakers', '1610612747', [])).toEqual(resolveLogoUrls(manifest(), 'lakers', '1610612747'));
    });
});
