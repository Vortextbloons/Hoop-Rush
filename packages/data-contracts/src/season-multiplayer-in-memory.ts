...
export class InMemorySeasonMultiplayerTransport implements SeasonMultiplayerTransport {
    ...
    private isOutdatedRoom(room: InMemoryRoom): boolean {
        const settings = room.settings as unknown as {
            multiplayerVersion: string;
            roomProtocolVersion: number;
        };
        return (settings.multiplayerVersion !== SEASON_MULTIPLAYER_VERSION ||
            settings.roomProtocolVersion !== SEASON_ROOM_PROTOCOL_SCHEMA_VERSION ||
            !!room.isOutdated);
    }
    ...
    create(settings: SeasonRoomSettings, rootSeed: string): Promise<SeasonRoomPublicSnapshot & {
        ...
        const incomingMode = (settings as unknown as {
            mode?: SeasonRoomMode;
        }).mode ?? 'season';
