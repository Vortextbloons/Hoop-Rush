...
interface InMemoryRoom {
    ...
    memberPrivate: Map<string, { control: 'human' | 'ai-takeover' | 'surrendered'; missStreak: number }>;
...
export class InMemorySeasonMultiplayerTransport implements SeasonMultiplayerTransport {
    ...
    async create(settings: SeasonRoomSettings, rootSeed: string): Promise<SeasonRoomPublicSnapshot> {
        ...
        const room: InMemoryRoom = {
            ...
            cursor: 'draft-0',
            revision: 0,
            digest: digestOf({ rootSeed, settings }),
            code,
    ...
    async preview(code: string): Promise<SeasonRoomPublicSnapshot> {
        ...
        if (!room || room.code !== code || (room.codeExpiresAt !== null && room.codeExpiresAt <= this.clock())) {
            throw Object.assign(new Error('invalid-code'), { code: 'invalid-code' });
        }
        return this.publicSnapshotOf(room);
    }
    ...
    async join(code: string): Promise<SeasonRoomMembership> {
        ...
        if (recentHour.length >= 20) throw Object.assign(new Error('rate-limit'), { code: 'rate-limit' });
        recentMin.push(now);
    ...
    subscribe(roomId: string, handler: (snapshot: SeasonRoomPublicSnapshot) => void): { unsubscribe: () => void } {
        const room = this.rooms.get(roomId);
        if (!room) throw Object.assign(new Error('membership'), { code: 'membership' });
    ...
    async submitPrivateDecision(submission: SeasonPrivateDecisionSubmission): Promise<{ locked: boolean }> {
    ...
    async publishAttestation(
        ...
        if (existing) {
            if (existing.inputDigest === attestation.inputDigest && existing.resultDigest === attestation.resultDigest) {
                // idempotent
            } else {
        ...
        if (list.length === 2) {
            ...
            const failure: SeasonIntegrityFailure2 = {
                ...
                attestations: list.slice(0, 2) as [SeasonCheckpointAttestation, SeasonCheckpointAttestation],
                terminal: true,
            };
    ...
    async requestReclaim(roomId: string, participantId: 'p1' | 'p2'): Promise<void> {
        ...
        if (priv.control === 'surrendered') throw Object.assign(new Error('authorization'), { code: 'authorization' });
