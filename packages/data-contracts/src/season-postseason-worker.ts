import { z } from 'zod';
import { commandIdSchema, contentHashSchema, franchiseIdSchema, idSchema, seedSchema, } from './ids.ts';
...
export type SeasonPostseasonWorkerStartRequest = z.infer<typeof seasonPostseasonWorkerStartRequestSchema>;
export const seasonPostseasonWorkerCancelRequestSchema = z.object({
    schemaVersion: z.literal(SEASON_POSTSEASON_WORKER_WIRE_SCHEMA_VERSION),
    type: z.literal('season-postseason-cancel'),
    requestId: z.string().min(1).max(64),
});
export type SeasonPostseasonWorkerCancelRequest = z.infer<typeof seasonPostseasonWorkerCancelRequestSchema>;
...
export type SeasonPostseasonWorkerWarmRequest = z.infer<typeof seasonPostseasonWorkerWarmRequestSchema>;
export const seasonPostseasonWorkerWarmAckMessageSchema = z.object({
    schemaVersion: z.literal(SEASON_POSTSEASON_WORKER_WIRE_SCHEMA_VERSION),
    type: z.literal('season-postseason-warm-ack'),
    requestId: z.string().min(1).max(64),
});
export type SeasonPostseasonWorkerWarmAckMessage = z.infer<typeof seasonPostseasonWorkerWarmAckMessageSchema>;
...
export type SeasonPostseasonWorkerProgressMessage = z.infer<typeof seasonPostseasonWorkerProgressMessageSchema>;
...
export type SeasonPostseasonWorkerAcceptedResult = z.infer<typeof seasonPostseasonWorkerAcceptedResultSchema>;
...
export type SeasonPostseasonWorkerCompleteMessage = z.infer<typeof seasonPostseasonWorkerCompleteMessageSchema>;
...
export type SeasonPostseasonWorkerErrorMessage = z.infer<typeof seasonPostseasonWorkerErrorMessageSchema>;
