...
import { seasonObjectiveEvaluationFactsSchema, seasonObjectiveIdSchema, } from './season-objective.ts';
...
export const seasonBlockEffectsEvidenceSchema = z.object({
    ...
    blockOpportunities: z.number().int().min(0).max(10000000),
    blockDeltaTotal: z.number().int().min(-10000000000000).max(10000000000000),
});
