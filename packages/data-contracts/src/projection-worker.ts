...
export const projectionRosterBuildRequestSchema = z.object({
    schemaVersion: z.literal(PROJECTION_WORKER_WIRE_SCHEMA_VERSION),
    type: z.literal('build-roster'),
    requestId: z.string().min(1).max(64),
}).passthrough();
export const projectionRotationOptimizeRequestSchema = z.object({
    schemaVersion: z.literal(PROJECTION_WORKER_WIRE_SCHEMA_VERSION),
    type: z.literal('optimize-rotation'),
    requestId: z.string().min(1).max(64),
}).passthrough();
export const projectionWorkerRequestSchema = z.discriminatedUnion('type', [projectionRosterBuildRequestSchema, projectionRotationOptimizeRequestSchema]);
