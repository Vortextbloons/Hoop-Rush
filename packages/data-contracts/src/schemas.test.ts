...
import { challengeRunSchema, classicCompletedDraftSchema, classicDraftStateSchema, coverageSummarySchema, franchiseAbbreviation, franchiseEraPoolSchema, hoopRushManifestSchema, lineupSchema, opponentTeamSchema, peakPlayerSeasonSchema, provenanceMapSchema, reconstructedThreePointProfileSchema, simulationAnchorsSchema, simulationPlayerSchema, simulationTeamSchema, threePointReconstructionArtifactSchema, workerMessageSchema, workerRequestSchema, } from './index.ts';
...
function makeRun() {
    ...
    const fivePlayers = five.map((playerId, slotIndex) => makeSimulationPlayer(playerId, SLOT_POSITIONS[slotIndex] ?? ['PG', 'SG']));
    ...
    const schedule = (opponentIds: string[]) => Array.from({ length: 82 }, (_, index) => {
...
describe('player-season contracts', () => {
    ...
    it('accepts a player without alt ids', () => {
        ...
        expect(peakPlayerSeasonSchema.safeParse({ ...validPlayer, altIds: { bbref: null } }).success).toBe(true);
    });
...
describe('classic draft contracts (M4)', () => {
    ...
    it('rejects more than five picks', () => {
        const picks = Array.from({ length: 6 }, (_, i) => pick((i % 5) + 1, `p-${String(i + 1)}`, i % 5));
        expect(classicDraftStateSchema.safeParse({ ...baseDraft, picks }).success).toBe(false);
    });
    it('rejects an invalid variant', () => {
        expect(classicDraftStateSchema.safeParse({ ...baseDraft, variant: 'draft-order' }).success).toBe(false);
    });
    it('rejects a non-hex seed', () => {
        expect(classicDraftStateSchema.safeParse({ ...baseDraft, seed: 'not-a-seed' }).success).toBe(false);
    });
    it('rejects a null roll while drafting and accepts one once complete', () => {
        ...
        expect(classicDraftStateSchema.safeParse({ ...baseDraft, status: 'complete', roll: null }).success).toBe(true);
    });
    ...
    it('accepts a completed draft snapshot and rejects short pick lists', () => {
        const picks = [0, 1, 2, 3, 4].map((slotIndex) => pick(slotIndex + 1, `p-${String(slotIndex + 1)}`, slotIndex));
        const snapshot = {
            draftId: 'draft-1',
            variant: 'ratings',
            seed: 'abcd1234abcd1234abcd1234abcd1234',
        ...
        expect(classicCompletedDraftSchema.safeParse({ ...snapshot, picks: picks.slice(0, 4) }).success).toBe(false);
    });
});
...
describe('three-point reconstruction contracts (spec/12)', () => {
    ...
    it('rejects out-of-range accuracy and attempt values', () => {
        expect(reconstructedThreePointProfileSchema.safeParse({ ...validProfile, accuracyConservative: 1.2 })
            .success).toBe(false);
        expect(reconstructedThreePointProfileSchema.safeParse({
            ...validProfile,
            attemptRateConservative: -0.1,
        }).success).toBe(false);
    });
    it('rejects an unknown model version', () => {
        expect(reconstructedThreePointProfileSchema.safeParse({ ...validProfile, modelVersion: 'other-v1' })
            .success).toBe(false);
    });
    it('accepts the profile on a simulation player and a peak player season', () => {
        expect(simulationPlayerSchema.safeParse({
            playerId: 'p-1',
        ...
        expect(peakPlayerSeasonSchema.safeParse({
            ...validPlayer,
            schemaVersion: 5,
            reconstructedThreePoint: validProfile,
        }).success).toBe(true);
    ...
    it('accepts schema-version 5 pools', () => {
        expect(franchiseEraPoolSchema.safeParse({
            schemaVersion: 5,
    ...
    it('accepts reconstructed provenance and the coverage family', () => {
        expect(provenanceMapSchema.safeParse({
            threePoint: {
                kind: 'reconstructed',
                confidence: 'medium',
        ...
        expect(coverageSummarySchema.safeParse({
            coverageBand: 'reconstructed',
            observedFamilies: [],
            derivedFamilies: [],
            estimatedFamilies: [],
    ...
    it('accepts threePointAttemptRate null and retains numeric zero for observed zeros', () => {
        expect(simulationAnchorsSchema.safeParse({ ...validPlayer.anchors, threePointAttemptRate: null })
            .success).toBe(true);
        expect(simulationAnchorsSchema.safeParse({ ...validPlayer.anchors, threePointAttemptRate: 0 })
            .success).toBe(true);
        expect(simulationAnchorsSchema.safeParse({ ...validPlayer.anchors, threePointAttemptRate: 1.5 })
            .success).toBe(false);
    });
    it('parses the versioned reconstruction artifact', () => {
        ...
        expect(threePointReconstructionArtifactSchema.safeParse({ ...artifact, artifactVersion: 'other-v1' })
            .success).toBe(false);
    });
});
