...
async function headAvailable(externalId) {
    if (externalId in cache)
        return cache[externalId];
    try {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), 15000);
        const resp = await fetch(`https://cdn.nba.com/headshots/nba/latest/1040x760/${externalId}.png`, { method: 'HEAD', redirect: 'follow', signal: controller.signal });
        clearTimeout(timer);
        if (resp.status !== 200) {
