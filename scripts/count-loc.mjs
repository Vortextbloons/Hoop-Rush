...
for (const arg of args) {
    if (arg.startsWith('--chars-per-line='))
        ...
        topN = Math.max(0, parseInt(arg.split('=')[1], 10) || 0);
...
process.stdout.write(`Files: ${files.length}  Physical lines: ${totalLines}  Code lines: ${totalLines - totalBlank}  Blank: ${totalBlank}\n`);
process.stdout.write(`Total chars (excl. newlines): ${totalChars}  Chars/line: ${charsPerLine}  Normalized LOC (ceil(chars/${charsPerLine})): ${totalNorm}\n`);
process.stdout.write(`\nBy extension:\n`);
process.stdout.write(`  ext        files    physical   normalized\n`);
for (const [ext, agg] of [...byExt.entries()].sort((a, b) => b[1].norm - a[1].norm)) {
    process.stdout.write(`  ${ext.padEnd(10)} ${pad(agg.files, 6)} ${pad(agg.lines, 10)} ${pad(agg.norm, 12)}\n`);
}
if (perFile) {
