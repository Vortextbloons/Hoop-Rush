...
async function draftFull(page) {
  ...
  for (let round = 1; round <= 10; round += 1) {
    // draw
    const drawBtn = page.getByRole('button', { name: new RegExp(`Draw round ${round} offer`, 'i') });
    if (await drawBtn.isVisible()) {
      await drawBtn.click();
      await expect(page.getByText(new RegExp(`Offer · pick ${round}`, 'i'))).toBeVisible({ timeout: 8000 });
    }
...
test.describe('season m2.5.5: campaign and trade board', () => {
  ...
  test('draft → GM identity → campaign opportunity → block 0 recap → history', { tag: '@smoke' }, async ({ page }) => {
    ...
    if (await genBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await genBtn.click();
      await expect(page.getByRole('button', { name: /Open the league hub/ })).toBeVisible({ timeout: 20000 });
    ...
    await expect(page.getByText(/Simulating block/)).toBeVisible({ timeout: 5000 }).catch(() => {});
    await expect(page.locator('[data-season-checkpoint-block="0"]')).toBeVisible({ timeout: 30000 }).catch(async () => {
    ...
    await expect(page.locator('[data-season-checkpoint-block="0"]')).toBeVisible({ timeout: 10000 });
    ...
    await expect(priorSection.getByText(/reward — ledger shows requested vs applied|ledger shows requested vs applied/)).toBeVisible();
    ...
    await expect(page.getByText(/60% cuts one game/)).toBeVisible({ timeout: 5000 }).catch(async () => {
    ...
    await expect(page.getByRole('heading', { name: /Completed seasons|History/ })).toBeVisible({ timeout: 5000 });
  });

  test('trade board workspace: browsing is free, package builder constraints, inquiry counters, and history', { tag: '@smoke' }, async ({ page }) => {
    // reuse existing run if any, else create quick run via same flow but abbreviated
    await page.goto('/season/run');
    const hasRun = await page.locator('[data-testid="campaign-panel"]').isVisible({ timeout: 3000 }).catch(() => false);
    if (!hasRun) {
      await clearSaves(page);
      await page.goto('/');
      await page.getByRole('link', { name: /Start season run/ }).click();
    ...
    await expect(page.getByRole('heading', { name: 'Trade Board' })).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.getByText(/acceptable.*close needs more value.*wrong roster fit/)).toBeVisible();
  ...
  test('rehab and Influence floor: shows 0 floor, 60% 1→0 and 60bp premium', async ({ page }) => {
    ...
    if (await influencePanel.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await expect(influencePanel.getByText(/at the 0 floor/)).toBeVisible().catch(async () => {
        await expect(influencePanel.getByText(/floor 0 — spends are rejected/)).toBeVisible();
      });
      // risky rehab description
      await expect(influencePanel.getByText(/60% cuts one game/)).toBeVisible().catch(async () => {
        await expect(influencePanel.getByText(/Risky rehab/)).toBeVisible();
      });
    }
  });

  test('evolution after block 4: shows double-down + evidence-backed adapt/pivot before block 5', async ({ page }) => {
    ...
    if (await campaignPanel.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const isEvolutionVisible = await evolutionPicker.isVisible({ timeout: 1000 }).catch(() => false);
      // either evolution is visible (if at block 4) or campaign cards are visible
      if (!isEvolutionVisible) {
        await expect(campaignPanel.getByText(/Prior block result|Choose your GM identity|Choose one for block/)).toBeVisible();
      }
    }
  });
});
