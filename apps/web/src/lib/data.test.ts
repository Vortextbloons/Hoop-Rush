...
import type { FranchiseEraPool, HoopRushManifest, PlayersIndex, PoolIndexEntry, } from '@hoop-rush/data-contracts';
import { contentHashSchema, eraIdSchema, franchiseIdSchema, playerIdSchema, seasonKeySchema, } from '@hoop-rush/data-contracts';
import { getPool, getPlayersIndex, clearDataLoaderCaches, getManifest, warmPlayersIndex, } from './data';
...
describe('data asset loading with a stale manifest', () => {
    ...
    ])('recovers a stale pool hash ($playerId, manifest refreshed: $staleManifestRefresh)', async ({ playerId, franchiseId, eraId, url, staleManifestRefresh }) => {
