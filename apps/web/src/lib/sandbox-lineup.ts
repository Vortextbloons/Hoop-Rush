...
export async function loadRunPlayersById(currentRun: ChallengeRun, manifest: HoopRushManifest): Promise<Map<string, PeakPlayerSeason>> {
    const pairs: Array<{
        franchiseId: string;
        eraId: string;
    }> = [];
    if (currentRun.selections) {
        for (const selection of currentRun.selections) {
            const known = pairs.some((p) => p.franchiseId === selection.franchiseId && p.eraId === selection.eraId);
            if (!known)
                pairs.push({ franchiseId: selection.franchiseId, eraId: selection.eraId });
        }
    }
    ...
    const loaded = await Promise.all(pairs.map(async (pair) => {
        const entry = manifest.pools.find((p) => p.franchiseId === pair.franchiseId && p.eraId === pair.eraId);
        if (!entry)
            return [];
        try {
...
export function lineupPlayersFromRun(currentRun: ChallengeRun, byId: Map<string, PeakPlayerSeason>): PeakPlayerSeason[] | null {
