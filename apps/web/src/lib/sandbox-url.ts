import type { HoopRushManifest, PlayersIndex, PlayersIndexEntry, RunPlayerSelection, Seed, } from '@hoop-rush/data-contracts';
import { eraIdSchema, franchiseIdSchema, LINEUP_STRUCTURE, playerIdSchema, seedSchema, } from '@hoop-rush/data-contracts';
...
const SLOT_PATTERN = /^([^@]+)@([^/]+)\/([^/]+)$/;
export function parseSandboxUrl(url: URL, manifest: HoopRushManifest | null, index: PlayersIndex | null): UrlStateValidation {
    ...
    if (index !== null) {
        ...
        for (const slot of slots) {
            const entry = index.players.find((p) => p.playerId === slot.playerId &&
...
export function buildSandboxHref(slots: RunPlayerSelection[]): SandboxHref {
    ...
    params.set('slots', slots.map((slot) => `${slot.playerId}@${slot.franchiseId}/${slot.eraId}`).join(','));
