...
async function ensureAnonymous(client: FixedFiveClient): Promise<void> {
    ...
    if (signed.error)
        throw new Error(`anonymous sign-in failed: ${signed.error.message}`);
}
export function createFixedFiveTransport(options?: {
    ...
    function memberFromRow(row: FixedFiveMemberRow): FixedFiveMemberSnapshot | null {
        if (row.participant_id !== 'p1' && row.participant_id !== 'p2')
            return null;
        return {
    ...
    function roomRowToSnapshot(row: FixedFiveRoomRow, memberRows: FixedFiveMemberRow[]): FixedFiveRoomSnapshot {
        const byId = new Map<FixedFiveParticipantId, FixedFiveMemberSnapshot>();
        for (const memberRow of memberRows) {
            const member = memberFromRow(memberRow);
            if (member)
                byId.set(member.participantId, member);
        }
    ...
    async function fetchSnapshot(roomId: string): Promise<FixedFiveRoomSnapshot> {
        ...
        if (memberResponse.error)
            throw new Error(`members failed: ${memberResponse.error.message}`);
    ...
    return {
        async create(settingsInput) {
            ...
            if (!payload.code)
                throw new Error('create failed: room code missing');
        ...
        async preview(code) {
            ...
            const phase = fixedFiveRoomPhaseSchema.parse(typeof record['phase'] === 'string' ? record['phase'] : 'lobby');
        ...
        async resume(roomId) {
            ...
            if (!uid)
                throw new Error('membership: not signed in');
            const memberResponse = await client
                .from('fixed_five_room_members')
        ...
        async refetch(roomId, afterOrdinal) {
            ...
            if (response.error)
                throw new Error(`refetch failed: ${response.error.message}`);
            const rows = response.data as unknown as FixedFiveCommandRow[];
            return rows.map((row) => fixedFiveCommandSchema.parse({
                schemaVersion: 1,
                roomId: parsedRoomId,
        ...
        async submitCommand(command) {
            ...
            if (response.error)
                throw new Error(`command failed: ${response.error.message}`);
        ...
        async removeGuest(roomId, targetParticipantId) {
            ...
            if (response.error)
                throw new Error(`remove-guest failed: ${response.error.message}`);
        ...
        async rematch(roomId) {
            ...
            const snapshot = await fetchSnapshot(payload.room_id);
            if (!payload.code)
                throw new Error('rematch failed: room code missing');
        ...
        async complete(roomId, resultDigest) {
            ...
            const record = response.data as unknown as Record<string, unknown>;
            const phase = fixedFiveRoomPhaseSchema.parse(typeof record['phase'] === 'string' ? record['phase'] : 'awaiting-confirmation');
