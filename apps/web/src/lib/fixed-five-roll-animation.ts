...
function isRollAffecting(mode: FixedFiveRoomMode, kind: FixedFiveCommand['payload']['kind']): boolean {
    if (mode === 'duel') {
        return (kind === 'start' ||
            kind === 'reroll' ||
            kind === 'duel-claim' ||
    ...
    if (mode === 'sandbox-shared-82')
        return false;
    return (kind === 'start' || kind === 'reroll' || kind === 'classic-pick' || kind === 'timeout-autopick');
}
export function rollAnimationFor(commands: FixedFiveCommand[], mode: FixedFiveRoomMode, viewer: 'p1' | 'p2' | null = null): RollAnimation {
    let nonce = 0;
    let axis: RollAnimationAxis = 'both';
    for (const command of commands) {
        if (!isRollAffecting(mode, command.payload.kind))
            continue;
        if (mode === 'classic-shared-82' &&
            viewer !== null &&
