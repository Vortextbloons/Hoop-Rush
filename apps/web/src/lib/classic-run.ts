...
import { classic, createChallenge, createEngineContext, toSimulationPlayer, } from '@hoop-rush/engine';
...
export async function startClassicRun(draft: ClassicDraftState, runSeed: Seed): Promise<void> {
    ...
    await challengeRepository.promoteClassicDraftToRun({ recordId: 'active', saveSchemaVersion: 2, run }, draft.draftId);
