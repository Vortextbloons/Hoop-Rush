...
import { buildInitialPostseasonState, type PlayoffSeries, type SeasonPostseasonState, type SeasonRun, } from '@hoop-rush/data-contracts';
import { franchiseIdSchema, commandIdSchema, seedSchema, seasonGameIdSchema, idSchema, } from '@hoop-rush/data-contracts';
import { awardsViewModel, bracketColumnsOf, describePostseasonRejection, humanSeriesOf, mobileBracketCardsOf, playInColumnViewModel, postseasonRankingsOf, rankedEntriesOf, riskyRehabOptionsOf, roundLabel, seriesCardViewModel, } from './season-postseason-presentation';
...
function series(seriesId: string, round: PlayoffSeries['round'], conference: 'east' | 'west' | null, higherSeed: number | null, lowerSeed: number | null, homeCourt: string | null, challenger: string | null, homeWins: number, challengerWins: number, winner: string | null): PlayoffSeries {
...
function decidedPostseason(humanFranchiseId: string): SeasonPostseasonState {
    ...
    const bracket = (conference: 'east' | 'west') => ({
        ...
        firstRound: [
            series(`po-${conference}1-8`, 'first-round', conference, 1, 8, `${conference}1`, `${conference}8`, 4, 1, `${conference}1`),
            series(`po-${conference}4-5`, 'first-round', conference, 4, 5, `${conference}4`, `${conference}5`, 2, 4, `${conference}5`),
            series(`po-${conference}3-6`, 'first-round', conference, 3, 6, `${conference}3`, `${conference}6`, 4, 0, `${conference}3`),
            series(`po-${conference}2-7`, 'first-round', conference, 2, 7, `${conference}2`, `${conference}7`, 4, 2, `${conference}2`),
        ],
        semifinals: [
            series(`po-${conference}1-5`, 'conference-semifinal', conference, 1, 5, `${conference}1`, `${conference}5`, 4, 3, `${conference}1`),
            series(`po-${conference}3-2`, 'conference-semifinal', conference, 3, 2, `${conference}3`, `${conference}2`, 1, 4, `${conference}2`),
        ],
        conferenceFinal: series(`po-${conference}1-2`, 'conference-final', conference, 1, 2, `${conference}1`, `${conference}2`, 4, 2, `${conference}1`),
    });
    state.bracket = {
        schemaVersion: 1,
...
describe('series card view models', () => {
    it('reports score, winner, and status', () => {
        const card = seriesCardViewModel(series('po-x1', 'first-round', 'east', 1, 8, 'a', 'b', 4, 1, 'a'), 'lakers');
    ...
    it('names the next game host from the 2-2-1-1-1 pattern', () => {
        const card = seriesCardViewModel(series('po-x2', 'first-round', 'east', 1, 8, 'a', 'b', 2, 1, null), null);
        expect(card.status).toBe('in-progress');
        expect(card.nextGame?.gameNumber).toBe(4);
        expect(card.nextGame?.homeFranchiseId).toBe('b');
        const game5 = seriesCardViewModel(series('po-x3', 'first-round', 'east', 1, 8, 'a', 'b', 2, 2, null), null);
        expect(game5.nextGame?.gameNumber).toBe(5);
        expect(game5.nextGame?.homeFranchiseId).toBe('a');
    });
    it('marks the human franchise series', () => {
        const card = seriesCardViewModel(series('po-x4', 'conference-semifinal', 'west', 1, 5, 'lakers', 'b', 2, 2, null), 'lakers');
...
describe('bracket columns', () => {
    ...
    it('exposes only the play-in column before the bracket exists', () => {
        const state = buildInitialPostseasonState(SEED);
        state.playIn.east.ranking = ['e1', 'e2', 'e3', 'e4', 'e5', 'e6', 'e7', 'e8', 'e9', 'e10'].map((id) => franchiseIdSchema.parse(id));
        state.playIn.west.ranking = ['w1', 'w2', 'w3', 'w4', 'w5', 'w6', 'w7', 'w8', 'w9', 'w10'].map((id) => franchiseIdSchema.parse(id));
    ...
    it('orders mobile cards play-in first, then round by round', () => {
        ...
        const seriesIds = cards
            .filter((card): card is Extract<(typeof cards)[number], {
            kind: 'series';
        }> => card.kind === 'series')
...
describe('play-in cards', () => {
    it('maps seeds 7-10 from the ranking', () => {
        const state = buildInitialPostseasonState(SEED);
        state.playIn.west.ranking = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j'].map((id) => franchiseIdSchema.parse(id));
...
describe('typed rejection copy', () => {
    it('maps every M2.6 postseason rejection code', () => {
        expect(describePostseasonRejection('advance-postseason', {
            code: 'invalid-stage',
        ...
        expect(describePostseasonRejection('submit-postseason-rotation', {
        ...
        expect(describePostseasonRejection('start-postseason', {
            code: 'stale-state',
