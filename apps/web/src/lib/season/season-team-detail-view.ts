import type { SeasonGameSummary, SeasonLeague, SeasonPlayerAggregate, SeasonRotation, SeasonRoster, SeasonRosterEntry, SeasonStandings, } from '@hoop-rush/data-contracts';
...
function mapStatToDisplay(raw: number, range: {
    min: number;
    max: number;
}): number {
    ...
    const scaled = TEAM_PROJECTION_DISPLAY_FLOOR +
        ((raw - range.min) / (range.max - range.min)) *
            (TEAM_PROJECTION_DISPLAY_CEILING - TEAM_PROJECTION_DISPLAY_FLOOR);
    return Math.max(TEAM_PROJECTION_DISPLAY_FLOOR, Math.min(TEAM_PROJECTION_DISPLAY_CEILING, Math.round(scaled)));
}
export function rawSeasonTeamRatings(input: {
    ...
    const minutes = new Map(input.rotation.targetMinutes.map((target) => [target.playerVersionId, target.minutes]));
    ...
    for (const entry of input.roster.players) {
        ...
        if (rating === null || minutesPlayed <= 0)
            continue;
...
export function buildLeagueProjectionBaselines(input: {
    ...
    const rotationByFranchise = new Map(input.rotations.map((rotation) => [rotation.franchiseId, rotation]));
...
export function normalizeTeamProjection(raw: SeasonTeamProjectionRaw, baselines: LeagueProjectionBaselines): SeasonTeamProjection {
    return {
...
export function seasonLeagueTeamProjections(input: {
    ...
    for (const roster of input.rosters) {
        const rotation = input.rotationOverrides?.get(roster.franchiseId) ??
...
export function seasonTeamRatings(input: {
    ...
    if (raw === null)
        return null;
    const baselines = input.leagueBaselines ??
...
export function seasonTeamDetail(input: {
    ...
    if (standingsRow === undefined)
        return null;
    const conference = league.teams.find((team) => team.franchiseId === roster.franchiseId)?.conference ?? 'east';
    ...
    const minutesOf = new Map(rotation.targetMinutes.map((target) => [target.playerVersionId, target.minutes]));
    ...
    return {
        ...
        projection: seasonLeagueTeamProjections({
            rosters,
            rotations,
            summaryRatingsOf: input.summaryRatingsOf,
