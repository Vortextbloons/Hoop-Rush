import { buildSeasonRunReplayExport, humanFranchiseIdOf, type HoopRushManifest, type SeasonCompletedSeason, type SeasonReplayAssetHashes, type SeasonRunReplayExport, } from '@hoop-rush/data-contracts';
import { deriveSeasonTradeGrades } from '@hoop-rush/engine';
export function seasonReplayAssetHashesOf(manifest: HoopRushManifest, eraId: string): SeasonReplayAssetHashes {
    const season = manifest.season;
    if (season === undefined) {
        throw new Error('manifest is missing the season artifact index');
    }
    const eraProfile = manifest.eraSimulationProfiles.find((entry) => entry.eraId === eraId)?.contentHash ?? '';
    return {
...
function seasonEraIdOf(season: SeasonCompletedSeason): string {
    ...
    const roster = humanFranchiseId === null
        ? null
...
export function buildCompletedSeasonRunReplayExport(season: SeasonCompletedSeason, manifest: HoopRushManifest): SeasonRunReplayExport {
