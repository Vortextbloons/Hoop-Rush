...
export function createSeasonBlockRunner(deps: SeasonBlockRunnerDeps = {}): SeasonBlockRunner {
    ...
    function consumePreWorkerCancellation(requestId: string, blockIndex: number): boolean {
        if (!cancelledBeforeWorker.delete(requestId))
            return false;
    ...
    function assertExpectedState(checkpoint: SeasonCandidateCheckpoint, input: SeasonBlockStartInput, failures: string[]): void {
        if (checkpoint.expectedStateRevision !== input.run.stateRevision) {
            failures.push(`candidate expectedStateRevision ${String(checkpoint.expectedStateRevision)} does not match the run state ${String(input.run.stateRevision)}`);
        }
    ...
    async function acceptCandidate(checkpoint: SeasonCandidateCheckpoint): Promise<void> {
        ...
        if (checkpoint.runId !== state.input.run.runId)
            failures.push('candidate runId mismatch');
        ...
        try {
            ...
            const freeAgencyAssets = state.blockIndex === 2 || state.blockIndex === 4 || state.blockIndex === 6
            ...
            const priorRetainedDetails = runState?.runId === checkpoint.runId ? runState.retainedDetails : [];
    ...
    async function acceptInterruption(pending: SeasonPendingBlockCandidate): Promise<void> {
        ...
        try {
            ...
            const availability = seasonFranchiseLegalFiveFacts(state.input.run, state.input.humanFranchiseId, pending.health);
        ...
        startBlock(input: SeasonBlockStartInput): string {
            ...
            void (async () => {
                try {
                    ...
                    if (consumePreWorkerCancellation(requestId, input.blockIndex))
                        return;
                    if (currentRequestId !== requestId)
                        return;
                    if (runState === null ||
                        ...
                        if (snapshot === null)
                            throw new Error('no active season run to advance');
                        ...
                        if (runState !== null &&
                            ...
                            throw new Error(`stale cursor: the run is at revision ${String(persistedRevision)}, command expects ${String(input.expectedRevision)}`);
                        }
                        runState = {
                    ...
                    if (input.expectedRevision !== revision) {
                        throw new Error(`stale cursor: the run is at revision ${String(revision)}, command expects ${String(input.expectedRevision)}`);
                    }
                    ...
                    if (next === null || next !== input.blockIndex) {
                        throw new Error(`non-boundary block: the run expects ${String(next)}, command submits ${String(input.blockIndex)}`);
                    }
        ...
        resumeBlock(input: SeasonBlockResumeInput): string {
            ...
            void (async () => {
                try {
                    ...
                    const snapshot = await repository.loadActiveRun();
                    if (snapshot === null)
                        throw new Error('no active season run to resume');
                    ...
                    if (pending.blockIndex !== input.blockIndex) {
                        throw new Error(`pending blockIndex ${String(pending.blockIndex)} does not match resume ${String(input.blockIndex)}`);
                    }
                    ...
                    const startInput: SeasonBlockStartInput = {
                        ...
                        campaignOpportunityId: (pending as unknown as {
                            campaignOpportunityId?: string | null;
...
function objectivesWithSuccess(run: SeasonRun, checkpoint: SeasonCandidateCheckpoint): SeasonObjectiveState {
    if (checkpoint.blockIndex === 8)
...
function clonePlain<T>(value: T): T {
    ...
    if (value instanceof Map) {
        return new Map([...value.entries()].map(([key, item]) => [clonePlain(key), clonePlain(item)])) as T;
    }
    if (value instanceof Set)
        return new Set([...value].map((item) => clonePlain(item))) as T;
