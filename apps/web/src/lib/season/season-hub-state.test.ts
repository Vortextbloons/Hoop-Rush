...
import { SEASON_DRAFT_CATALOG_V3, SEASON_DURABILITY_VERSION, SEASON_ROUND_COUNT, SEASON_RUN_SCHEMA_VERSION, SEASON_STAMINA_VERSION, SIMULATION_RATINGS, SIMULATION_TENDENCIES, PLAYER_VERSION_ID_VERSION, canonicalJson, seasonDigestHex, type Position, type SeasonDraftCandidate, type SeasonDraftCatalog, type SeasonEffectsState, type SeasonPostseasonRotationPayload, type SeasonRun, type SeasonRunCommand, type SeasonStandings, } from '@hoop-rush/data-contracts';
import type { SeasonRunSnapshot } from '@hoop-rush/persistence';
import { generateSeasonSchedule, handleSeasonRunCommand, seasonObjectiveChoicesForBlock, seasonPostseasonNextGame, seasonRunStateDigest, } from '@hoop-rush/engine';
...
import type { SeasonBlockResumeInput, SeasonBlockRunner, SeasonBlockStartInput, SeasonRunnerEvent, } from './season-block-runner';
import type { SeasonPostseasonEvent, SeasonPostseasonRunner, SeasonPostseasonRunInput, } from './season-postseason-runner';
...
describe('SeasonHubState between-block commands', () => {
    ...
    it('keeps the post-command snapshot in the session cache (stale snapshot cache regression)', async () => {
        ...
        const repo = {
            loadActiveRun: vi.fn(() => Promise.resolve(active)),
            loadActiveRunIndex: vi.fn(() => Promise.resolve({
                runId: run.runId,
                rootSeed: seed,
            ...
            loadPendingBlock: vi.fn(() => Promise.resolve(null)),
            discardPendingBlock: vi.fn(() => Promise.resolve()),
            applySeasonRunCommand: vi.fn((input: {
                run: SeasonRunSnapshot['run'];
            }) => {
...
async function advanceHubRunToHumanGame(repo: ReturnType<typeof hubPostseasonRepo>, run: SeasonRun, effects: SeasonEffectsState, catalog: SeasonDraftCatalog): Promise<{
    run: SeasonRun;
    nextGameId: string;
}> {
    ...
    for (;;) {
        guard += 1;
        if (guard > 30)
            throw new Error('the run never reached a human rotation decision');
...
describe('SeasonHubState postseason commands (M2.6)', () => {
    ...
    it('submitPostseasonRotation accepts a legal rotation for the human team', async () => {
        ...
        const atHumanGame = await advanceHubRunToHumanGame(repo, started, hubZeroEffects(run), hubCatalogOf(run));
        await hub.refresh();
        const humanNext = atHumanGame.nextGameId;
        const humanRotation = atHumanGame.run.rotations.find((rotation) => rotation.franchiseId === POSTSEASON_HUMAN);
        if (humanRotation === undefined)
            throw new Error('expected a human rotation');
        ...
        if (commit?.command.command !== 'submit-postseason-rotation')
            return;
        expect(commit.command.targetGameId).toBe(humanNext);
        expect(commit.command.rotation.franchiseId).toBe(POSTSEASON_HUMAN);
        expect(hub.snapshot?.run.rotations.find((r) => r.franchiseId === POSTSEASON_HUMAN)).toEqual(humanRotation);
        hub.destroy();
    });
    ...
    it('maps every M2.6 postseason rejection code in describeCommandRejection', () => {
        const rejectionOf = (code: string) => describeCommandRejection('advance-postseason', {
            code,
            ...dummyFacts(code),
        } as never);
        ...
        expect(describeCommandRejection('advance-postseason', { code: 'unknown' } as never)).toContain('rejected');
    });
});
