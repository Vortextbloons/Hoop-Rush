...
import { generateSeasonSchedule, openSeasonFreeAgencyWindow, seasonRunStateDigest, } from '@hoop-rush/engine';
...
function hubFixture(): HubFixture {
    ...
    const opened = openSeasonFreeAgencyWindow({ run: base, effects, catalog, index, humanFranchiseId: HUMAN }, 0, 2);
    const run: SeasonRun = {
        ...base,
...
function hubRepo(initial: SeasonRunSnapshot | null) {
    ...
    return {
        ...
        loadActiveRunIndex: vi.fn(() => Promise.resolve(active === null
            ? null
            : {
        ...
        applySeasonRunCommand: vi.fn<SeasonRunRepository['applySeasonRunCommand']>((input: SeasonRunCommandApplication) => {
            ...
            if (expected.expectedStateRevision !== stored.stateRevision ||
                expected.expectedStateDigest !== stored.stateDigest) {
                return Promise.reject(new Error(`season run command ${String(expected.expectedStateRevision)} asserts stale state (expected revision ${String(stored.stateRevision)})`));
            }
            active = {
                ...(active ?? {
                    run: input.run,
...
describe('SeasonHubState free-agency commands (M2.6.5)', () => {
    ...
    it('replays the declaration from the run snapshot after a reload', async () => {
        ...
        const first = window?.candidates[0];
        if (first === undefined)
            throw new Error('expected a window candidate');
    ...
    it('rejects a stale declaration with stale-state instead of double-applying', async () => {
        ...
        if (staleEnvelope.result.status === 'accepted')
            return;
...
describe('block-submit gating (free-agency-unresolved)', () => {
    ...
    it('does not gate submission once the window is resolved', async () => {
        ...
        const run: SeasonRun = {
            ...
            freeAgency: {
                ...base.freeAgency,
                windows: base.freeAgency.windows.map((entry) => entry.windowIndex === 0 ? { ...entry, status: 'resolved' } : entry),
            },
        };
        const result = await buildSubmitBlockEnvelope(gatingShell(run, fixture.effects));
        if (result.ok) {
