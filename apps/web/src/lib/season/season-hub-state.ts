import { humanFranchiseIdOf, SEASON_RUN_SCHEMA_VERSION, franchiseIdSchema, postseasonGameIdSchema, seasonSubmitBlockCommandSchema, type EraSimulationProfile, type FranchiseId, type Id, type SeasonActiveRunIndex, type SeasonCampaignOpportunity, type SeasonCampaignState, type SeasonDraftCatalog, type SeasonEffectsState, type SeasonFreeAgencyIndex, type SeasonFreeAgencyRoleExpectation, type SeasonGameSummary, type SeasonInvalidRosterInterruption, type SeasonObjectiveId, type SeasonPendingBlockCandidate, type SeasonPostseasonRotationPayload, type SeasonPostseasonScoreline, type SeasonPostseasonSummary, type SeasonRetainedGameDetail, type SeasonRosterTargets, type SeasonRun, type SeasonRunCommand, type SeasonRunCommandRejection, type SeasonScoreline, type SeasonSpendInfluenceCommand, type SeasonSubmitBlockCommand, type SeasonTradeBoardTeamProfile, type SeasonTradeNegotiation, type SeasonTradeState, type SeasonTradeValueTrend, type SeasonTradeWindowState, } from '@hoop-rush/data-contracts';
...
export function campaignViewModel(run: SeasonRun | null, nextBlockIndex: number | null): CampaignViewModel | null {
    ...
    const nextOffers = targetBlock >= 0 && targetBlock < 8 ? (campaign.offers[targetBlock] ?? null) : null;
    return {
