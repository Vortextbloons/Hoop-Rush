import { humanFranchiseIdOf, SEASON_RUN_SCHEMA_VERSION, seasonSubmitBlockCommandSchema, type EraSimulationProfile, type SeasonActiveRunIndex, type SeasonCampaignOpportunity, type SeasonCampaignState, type SeasonDraftCatalog, type SeasonEffectsState, type SeasonFreeAgencyIndex, type SeasonFreeAgencyRoleExpectation, type SeasonGameSummary, type SeasonInvalidRosterInterruption, type SeasonObjectiveId, type SeasonPendingBlockCandidate, type SeasonPostseasonRotationPayload, type SeasonPostseasonSummary, type SeasonRetainedGameDetail, type SeasonRosterTargets, type SeasonRun, type SeasonRunCommand, type SeasonRunCommandRejection, type SeasonScoreline, type SeasonSpendInfluenceCommand, type SeasonSubmitBlockCommand, type SeasonTradeBoardTeamProfile, type SeasonTradeNegotiation, type SeasonTradeState, type SeasonTradeValueTrend, type SeasonTradeWindowState, } from '@hoop-rush/data-contracts';
import { generateSeasonCampaignOffers, handleSeasonRunCommand, type SeasonRunCommandContext, } from '@hoop-rush/engine';
...
import type { SeasonPostseasonEvent, SeasonPostseasonRunner, } from '$lib/season/season-postseason-runner';
import { seasonPostseasonCommitResultDigest, seasonPostseasonTransactionIdsOf, } from '$lib/season/season-postseason-simulation';
import type { CommitPostseasonAdvancementInput, SeasonPostseasonRepository, SeasonRunRepository, SeasonRunSnapshot, } from '@hoop-rush/persistence';
...
export class SeasonHubState {
    ...
    constructor(repo: SeasonRunRepository & SeasonPostseasonRepository, runner: SeasonBlockRunner, postseasonRunner?: SeasonPostseasonRunner, options: SeasonHubStateOptions = {}) {
        this.repo = repo;
        this.runner = runner;
    ...
    private resolvePostseasonRunner(): Promise<SeasonPostseasonRunner> {
        if (this.postseasonRunner !== null)
            return Promise.resolve(this.postseasonRunner);
        if (this.postseasonRunnerPromise === null) {
            this.postseasonRunnerPromise = import('./season-repo').then((module) => module.getSeasonPostseasonRunner());
            this.postseasonRunnerPromise
                .then((runner) => {
                if (this.postseasonRunner === null)
                    this.attachPostseasonRunner(runner);
            })
                .catch(() => {
    ...
    private loadProfile(): Promise<EraSimulationProfile> {
        if (this.profilePromise === null) {
            this.profilePromise = import('./season-assets').then((module) => module.loadSeasonEraProfile());
    ...
    private async ensureFreeAgencyIndex(): Promise<SeasonFreeAgencyIndex> {
        if (this.freeAgencyIndex !== null)
            return this.freeAgencyIndex;
    ...
    private async ensureFreeAgencyTargets(): Promise<SeasonRosterTargets> {
        if (this.freeAgencyTargets !== null)
            return this.freeAgencyTargets;
        const module = await import('./season-assets');
    ...
    loadPlayerSlice(runId: string): Promise<import('@hoop-rush/persistence').SeasonRunPlayerSliceEntry[] | null> {
        return this.repo.loadSeasonRunPlayerSlice(runId);
    }
    upsertPlayerSlice(runId: string, entries: import('@hoop-rush/persistence').SeasonRunPlayerSliceEntry[]): Promise<void> {
    ...
    async openTradeInquiry(input: {
        ...
        const command: SeasonRunCommand = {
            ...
            toFranchiseId: input.toFranchiseId as never,
        };
        await this.dispatch(command);
    }
    async submitTradeProposal(input: {
        windowIndex: number;
        toFranchiseId: string;
        ...
        const command: SeasonRunCommand = {
            ...
            windowIndex: input.windowIndex,
            toFranchiseId: input.toFranchiseId as never,
            outgoingPlayerVersionIds: input.outgoingPlayerVersionIds as never,
            incomingPlayerVersionIds: input.incomingPlayerVersionIds as never,
            influenceAmount: input.influenceAmount,
            influenceFromSender: input.influenceFromSender as never,
        };
        await this.dispatch(command);
    }
    async respondToTradeCounter(input: {
        ...
        const command: SeasonRunCommand = {
            ...
            expectedStateDigest: this.requiredStateDigest(),
            windowIndex: input.windowIndex,
            inquiryId: input.inquiryId as never,
            accept: input.accept,
        };
        await this.dispatch(command);
    }
    async walkAwayFromTrade(input: {
        windowIndex: number;
        inquiryId: string;
    }): Promise<void> {
        const command: SeasonRunCommand = {
    ...
    async purchaseTradeInquiry(input: {
        windowIndex: number;
    }): Promise<void> {
        const command: SeasonRunCommand = {
            schemaVersion: SEASON_RUN_SCHEMA_VERSION,
    ...
    async skipFreeAgentMarket(input: {
        windowIndex: number;
    }): Promise<void> {
        const command: SeasonRunCommand = {
            schemaVersion: SEASON_RUN_SCHEMA_VERSION,
    ...
    async resolveFreeAgentMarket(input: {
        windowIndex: number;
    }): Promise<void> {
    ...
    cancel(): void {
        ...
        if (this.block.phase !== 'running' || requestId === null)
            return;
    ...
    async advancePostseason(input?: {
        targetGameId?: string;
    }): Promise<void> {
        const command: SeasonRunCommand['command'] = 'advance-postseason';
        if (!this.requirePostseasonStage(command))
            return;
    ...
    async spectatePostseasonGame(input: {
        targetGameId: string;
    }): Promise<void> {
    ...
    async fastForwardPostseason(input?: {
        targetGameId?: string;
    }): Promise<void> {
    ...
    cancelPostseason(): void {
        ...
        if (this.postseason.phase !== 'running' || requestId === null || runner === null)
            return;
    ...
    private requirePostseasonStage(command: SeasonRunCommand['command']): boolean {
        ...
        if (stage === 'play-in' || stage === 'playoffs')
            return true;
        this.commandError = {
            command,
            rejection: {
    ...
    async retry(): Promise<void> {
        if (this.block.command === null || this.block.startInput === null)
            return;
        if (this.block.phase !== 'cancelled' && this.block.phase !== 'failed')
            return;
        clearCachedSeasonSnapshot();
        await this.refresh();
    ...
    private requiredRunId(): string {
        const runId = this.snapshot?.run.runId;
        if (runId === undefined)
            throw new Error('no active season run to command');
        return runId;
    }
    ...
    private requiredHumanFranchiseId(): string {
        ...
        if (franchiseId === null)
            throw new Error('the active run has no human franchise');
        return franchiseId;
    }
...
function indexAfterCommit(index: SeasonActiveRunIndex | null, snapshot: SeasonRunSnapshot, now: () => number): SeasonActiveRunIndex | null {
    const humanFranchiseId = humanFranchiseIdOf(snapshot.run.league);
    const humanRow = humanFranchiseId === null
        ? null
...
export function campaignViewModel(run: SeasonRun | null, nextBlockIndex: number | null): CampaignViewModel | null {
    if (run === null)
        return null;
    ...
    const isEvolutionRequired = completedBlocks === 5 &&
        campaign.evolutionOffers !== null &&
        campaign.evolutionSelection === null;
    const targetBlock = nextBlockIndex ?? completedBlocks;
    const isOpportunityRequired = !isIdentityRequired &&
        !isEvolutionRequired &&
        targetBlock >= 0 &&
        targetBlock < 8 &&
        campaign.selections[targetBlock] === undefined;
    const nextOffers = targetBlock !== null && targetBlock >= 0 && targetBlock < 8
...
export function generatePreviewCampaignOffers(run: SeasonRun, blockIndex: number, schedule: import('@hoop-rush/data-contracts').SeasonSchedule, humanFranchiseId: string | null): SeasonCampaignOpportunity[] | null {
    try {
...
export interface TradeBoardViewModel {
    ...
    openWindow: SeasonTradeWindowState | null;
    ...
    inquiriesRemaining: number;
    purchasedInquiryUsed: boolean;
    earnedInquiryUsed: boolean;
    activeInquiryId: string | null;
    exchangeCounts: Record<string, number>;
}
export function tradeBoardViewModel(run: SeasonRun | null): TradeBoardViewModel | null {
    if (run === null || run.trade === null) {
        return {
            windows: [],
            openWindow: null,
            boardProfiles: [],
    ...
    const trade: SeasonTradeState = run.trade;
    const openWindow = trade.windows.find((w) => w.status === 'open') ?? null;
    const allowance = openWindow?.inquiryAllowance ?? 3;
    const used = openWindow?.negotiations?.length ?? 0;
    const purchasedUsed = openWindow?.purchasedInquiryUsed ?? false;
    const earnedUsed = openWindow?.earnedInquiryUsed ?? false;
    const boardProfiles = openWindow?.boardProfiles ?? [];
    const negotiations = openWindow?.negotiations ?? [];
    const valueTrends = openWindow?.valueTrends ?? [];
    const exchangeCounts: Record<string, number> = {};
    for (const n of negotiations) {
        exchangeCounts[n.inquiryId] = n.exchangeCount;
    }
    return {
        ...
        negotiations,
        valueTrends,
        inquiryAllowance: allowance,
        inquiriesUsed: used,
        inquiriesRemaining: Math.max(0, allowance - used),
        purchasedInquiryUsed: purchasedUsed,
        earnedInquiryUsed: earnedUsed,
        activeInquiryId: openWindow?.activeInquiryId ?? null,
        exchangeCounts,
    };
}
export function describeCommandRejection(command: SeasonRunCommand['command'], rejection: SeasonRunCommandRejection): string {
    switch (rejection.code) {
        ...
        case 'not-at-boundary':
            return `Objective selection must target the current block (block ${String(rejection.nextUnselectedBlockIndex + 1)}).`;
        ...
        case 'insufficient-balance':
            return `Influence balance ${String(rejection.balance)} cannot cover the ${String(-rejection.requestedDelta)}-point spend (floor ${String(rejection.floor)}).`;
        ...
        case 'insufficient-rehab-resources':
            return `Influence balance ${String(rejection.balance)} cannot cover the ${String(rejection.required)}-point postseason rehab spend.`;
        ...
        case 'free-agency-unresolved':
            return `The free-agency market window ${String(rejection.windowIndex + 1)} is still open — resolve it on the free-agency screen (/season/run/free-agency) before the next block can submit.`;
        case 'free-agency-window-not-open':
            return `The free-agency market window ${String(rejection.windowIndex + 1)} is not open right now.`;
        case 'free-agency-already-resolved':
            return `The free-agency market window ${String(rejection.windowIndex + 1)} is already resolved.`;
        case 'free-agency-already-declared':
            return `The franchise already declared or skipped the free-agency market window ${String(rejection.windowIndex + 1)}.`;
        ...
        case 'free-agency-invalid-influence':
            return `The committed Influence must be the candidate's minimum through 3 (minimum ${String(rejection.minimum)}).`;
        case 'free-agency-roster-cap':
            return `The franchise roster (${String(rejection.rosterSize)} players) is already at the 15-player cap.`;
        case 'free-agency-season-signing-cap':
            return `The franchise already signed ${String(rejection.signingCount)} free agents this season (cap 3).`;
        case 'free-agency-season-influence-cap':
            return `The franchise already spent ${String(rejection.seasonSpend)} Influence on free agency this season (cap 6).`;
        case 'free-agency-insufficient-balance':
            return `Influence balance ${String(rejection.balance)} cannot cover the ${String(rejection.required)}-point free-agency commitment.`;
        ...
        case 'campaign-evolution-required':
            return 'Complete the midseason evolution choice first.';
        case 'campaign-opportunity-required':
            return `Select a campaign opportunity for block ${String(rejection.blockIndex + 1)}.`;
        case 'campaign-opportunity-not-offered':
            return 'That campaign opportunity was not offered for this block.';
        case 'campaign-already-selected':
            return `Block ${String(rejection.blockIndex + 1)} already has a campaign selection.`;
        case 'campaign-evolution-already-selected':
            return 'GM evolution already selected.';
        case 'campaign-evolution-not-offered':
            return 'That evolution option was not offered.';
        case 'trade-inquiry-cap':
            return `Trade inquiry cap reached (${String(rejection.inquiriesUsed)}/${String(rejection.allowance)}).`;
        case 'trade-active-negotiation':
            return 'Finish the active trade negotiation first.';
        case 'trade-duplicate-proposal':
            return 'That package is a duplicate of a previous proposal.';
        case 'trade-protected-player':
            return `Player ${rejection.playerVersionId} is protected.`;
        case 'trade-exchange-limit':
            return 'This inquiry has reached its exchange limit.';
        case 'trade-cash-cap':
