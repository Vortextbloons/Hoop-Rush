...
import type { SeasonPrivateDecisionSubmission, SeasonCheckpointAttestation, SeasonPublicCommandEnvelope, SeasonMultiplayerTransport, } from '@hoop-rush/data-contracts';
...
import { buildFixtureRosterTargets, buildSeasonDraftCatalog, buildSeasonLeague, } from '@hoop-rush/test-fixtures';
...
describe('multiplayer two-client deterministic flow (claim 11)', () => {
    it('retries guest command delivery when applying a realtime batch fails', async () => {
        ...
        const coordinator = createInMemorySeasonRoomCoordinator({
            ...
            onCommands: (commands) => {
                attempts += 1;
                if (attempts === 1)
                    return Promise.reject(new Error('guest controller still loading'));
    ...
    it('replays and retries a draft draw after a transient stale ordinal', async () => {
        ...
        const created = await backing.create(settingsLiveSeason(), ROOT_SEED);
        const roomId = created.roomId;
        const code = (created as unknown as {
            code: string;
        }).code;
        ...
        const transport = new Proxy(backing as SeasonMultiplayerTransport, {
            get(target, property, receiver) {
                ...
                return async (envelope: SeasonPublicCommandEnvelope) => {
                    ...
                    if (command.payload?.kind === 'draw-season-offer' &&
    ...
    it('create → join → drafting → lock → attest → advance covers phases, ordering, idempotency, impersonation, stale-revision', async () => {
        ...
        const p1MembershipDirect = (created as unknown as {
            membership: {
        ...
        const afterSettings = await transport.updateSettings(roomId, { mode: 'season', pace: 'live' }, 0);
        ...
        await expect(wrongCtrl.submitPick(secondTurn, selectable.playerVersionId)).rejects.toMatchObject({ code: 'WRONG_TURN' });
        ...
        const dupOffer = {
            ...
            pickOrdinal: dupStateBase.picks.filter((p) => p.participantId === dupStateBase.currentTurnParticipantId)
                .length + 1,
            seedPath: [
                'draft',
                'offer',
        ...
        expect((dupRes.record as unknown as {
            errorCode: string;
        }).errorCode).toBe('OWNED_VERSION');
        ...
        expect(refetchedP1.map((c) => c.ordinal)).toEqual([...refetchedP1].map((c) => c.ordinal).sort((a, b) => a - b));
        ...
        const lastActor = (lastState.picks[lastState.picks.length - 1]?.participantId as 'p1' | 'p2') ?? 'p1';
        ...
        const staleCode = (staleCreated as unknown as {
            code: string;
        }).code;
        ...
        expect((attestRes1 as {
            reason: string;
        }).reason).toBeDefined();
        const attestRes2 = await transport.publishAttestation(att2);
        expect((attestRes2 as {
            acceptedAt: string;
        }).acceptedAt).toBeDefined();
        ...
        expect((mis2 as {
            reason: string;
        }).reason).toMatch(/hash mismatch/i);
