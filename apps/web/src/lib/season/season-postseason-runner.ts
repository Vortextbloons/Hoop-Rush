import { SEASON_RUN_SCHEMA_VERSION, canonicalJson, commandIdSchema, seasonAlmanacDigest, seasonCommandLogDigest, seasonDigestHex, seasonPostseasonWorkerCancelRequestSchema, seasonPostseasonWorkerMessageSchema, seasonPostseasonWorkerWarmRequestSchema, seasonTradeGradeLogDigest, type CommandId, type FranchiseId, type Id, type PostseasonGameId, type SeasonAdvancePostseasonRejection, type SeasonAlmanac, type SeasonRun, type SeasonRunCommand, type SeasonRunStage, type SeasonPostseasonScoreline, type SeasonSchedule, type SeasonPostseasonWorkerCompleteMessage, type SeasonPostseasonWorkerErrorMessage, type SeasonPostseasonWorkerProgressMessage, type SeasonPostseasonWorkerStartRequest, type Seed, } from '@hoop-rush/data-contracts';
import { POSTSEASON_ALMANAC_DIGEST_PLACEHOLDER, deriveSeasonTradeGrades, seasonPostseasonHumanEliminated, seasonPostseasonNextGame, seasonPostseasonUpcomingGames, } from '@hoop-rush/engine';
...
export type SeasonPostseasonSimulatorFn = (request: SeasonPostseasonWorkerStartRequest, onProgress: (progress: SeasonPostseasonWorkerProgressMessage) => void) => Promise<SeasonPostseasonWorkerCompleteMessage | SeasonPostseasonWorkerErrorMessage>;
export interface SeasonPostseasonRunner {
    advancePostseason(input: SeasonPostseasonRunInput): string;
    spectatePostseasonGame(input: SeasonPostseasonRunInput & {
        targetGameId: string;
    }): string;
...
export function createSeasonPostseasonRunner(deps: SeasonPostseasonRunnerDeps = {}): SeasonPostseasonRunner {
    ...
    let warmed = false;
    const pending = new Map<string, (message: SeasonPostseasonWorkerCompleteMessage | SeasonPostseasonWorkerErrorMessage) => void>();
    ...
    function createWorker(): Worker {
        ...
        worker.addEventListener('message', (event: MessageEvent<unknown>) => {
            ...
            if (message.type === 'season-postseason-warm-ack') {
                if (message.requestId === warmRequestId)
                    warmRequestId = null;
                return;
            }
    ...
    function simulate(request: SeasonPostseasonWorkerStartRequest): Promise<SeasonPostseasonWorkerCompleteMessage | SeasonPostseasonWorkerErrorMessage> {
    ...
    async function orchestrate(requestId: string, input: SeasonPostseasonRunInput, mode: SeasonPostseasonMode): Promise<void> {
        let commitCount = 0;
        try {
            ...
            for (;;) {
                ...
                if (snapshot.run.runId !== input.runId) {
                    fail(requestId, 'internal', 'the active run does not match the submitted run', snapshot.run.rootSeed);
                    return;
                }
                ...
                if (mode === 'fast-forward') {
                    ...
                    if (humanFranchiseId !== null &&
                        ...
                        fail(requestId, 'internal', `the human franchise ${humanFranchiseId} still has postseason decisions; fast-forward requires elimination`, run.rootSeed);
                        return;
                    }
                }
                ...
                if (decision.kind === 'complete') {
                    if (run.stage === 'completed') {
                        ...
                        fail(requestId, 'invariant-failure', 'the postseason ended without a champion', run.rootSeed);
                    }
                    return;
                }
                ...
                if (mode === 'spectate') {
                    ...
                    if (terminalTarget !== null) {
                        ...
                        if (position < 0) {
                            fail(requestId, 'internal', `the fast-forward target ${terminalTarget} is not an upcoming postseason game`, run.rootSeed);
                            return;
                        }
                        if (position >= SEASON_POSTSEASON_CHUNK_MAX_GAMES) {
                            fail(requestId, 'internal', `the fast-forward target is beyond the ${String(SEASON_POSTSEASON_CHUNK_MAX_GAMES)}-game chunk bound`, run.rootSeed);
                            return;
                        }
                ...
                const commandId = firstCommandId !== null
                    ? firstCommandId
                ...
                const commitInput: CommitPostseasonAdvancementInput = {
                    ...
                    resultDigest: seasonPostseasonCommitResultDigest(commandId, accepted.advancedGameIds, accepted.summaries),
                ...
                try {
                    ...
                    if (error instanceof SeasonRunCommandStaleStateError ||
                        error instanceof SeasonRunCommandRunMismatchError) {
                        fail(requestId, 'internal', `the run moved before the commit could apply: ${error instanceof Error ? error.message : String(error)}`, run.rootSeed);
                        return;
                    }
                    fail(requestId, 'internal', `postseason commit failed: ${error instanceof Error ? error.message : String(error)}`, run.rootSeed);
                    return;
                }
    ...
    function fail(requestId: string, code: 'invariant-failure' | 'cancelled' | 'internal', message: string, seed: Seed | null): void {
        if (cancelled)
            return;
        emit({
            type: 'error',
    ...
    function emitComplete(requestId: string, runId: Id, snapshot: SeasonRunSnapshot, promoted: boolean, decision: {
        nextDecision: 'rotation' | 'none';
        nextGameId: PostseasonGameId | null;
        aiNextGameId: PostseasonGameId | null;
    }): void {
    ...
    async function promoteAndComplete(requestId: string, repository: SeasonPostseasonRepository, run: SeasonRun, snapshot: SeasonRunSnapshot): Promise<void> {
        try {
            ...
            fail(requestId, 'internal', `champion promotion failed: ${error instanceof Error ? error.message : String(error)}`, run.rootSeed);
        }
    }
    return {
        advancePostseason(input: SeasonPostseasonRunInput): string {
        ...
        cancel(requestId: string): void {
            if (requestId !== currentRequestId || cancelled)
                return;
            cancelled = true;
            if (worker !== null && currentWireRequestId !== null) {
                worker.postMessage(seasonPostseasonWorkerCancelRequestSchema.parse({
                    schemaVersion: 1,
...
export async function promoteSeasonChampion(repository: SeasonPostseasonRepository, run: SeasonRun, snapshot: SeasonRunSnapshot): Promise<SeasonAlmanac> {
