...
import { SEASON_AI_VERSION, SEASON_DRAFT_VERSION, SEASON_ROSTER_GENERATION_VERSION, SEASON_ROTATION_VERSION, seedSchema, type SeasonDraftState, type SeasonLeagueGenerationResult, } from '@hoop-rush/data-contracts';
import type { SeasonDraftRepository, StoredSeasonDraft } from '@hoop-rush/persistence';
import { buildSeasonAiAssignments, buildSeasonAiPools, buildSeasonDraftCatalog, buildSeasonLeague, buildSeasonRotation, buildSeasonRosters, buildFixtureEvaluations, } from '@hoop-rush/test-fixtures';
...
async function pickBestFromOffer(flow: SeasonDraftFlow): Promise<void> {
    ...
    if (state === null)
        throw new Error('expected a draft state');
    ...
    const best = [...selectable].sort((a, b) => (byId.get(b.playerVersionId)?.summaryRatings.overallRating ?? 0) -
        (byId.get(a.playerVersionId)?.summaryRatings.overallRating ?? 0) ||
        a.playerVersionId.localeCompare(b.playerVersionId))[0];
    if (!best)
