...
export function humanUpcomingGames(games: readonly SeasonGame[], humanFranchiseId: string, blockIndex: number): UpcomingGame[] {
    if (gamesToLockForBlock(blockIndex) === 0)
        return [];
    ...
    return games
        ...
        opponentFranchiseId: game.homeFranchiseId === humanFranchiseId ? game.awayFranchiseId : game.homeFranchiseId,
    }));
}
export function buildLockPreview(input: {
    ...
    const { pendingHumanRotation, baselineHumanRotation, pendingSetDigest, lastLockedDigest, blockIndex, names, games, humanFranchiseId, fatigue, objective, } = input;
    const changes: RotationChange[] = [];
    const pendingMinutes = new Map(pendingHumanRotation.targetMinutes.map((entry) => [entry.playerVersionId, entry.minutes]));
    const baselineMinutes = new Map(baselineHumanRotation.targetMinutes.map((entry) => [entry.playerVersionId, entry.minutes]));
    ...
    if (fatigue !== null && fatigue !== undefined) {
        ...
        for (const playerVersionId of projected) {
            ...
            fatigueProjections.push({
                ...
                bandAfterBlock: projectedFatigueBandName(load.fatigueBasisPoints, minutes, stamina, gamesToLockForBlock(blockIndex)),
                continuous,
            });
        }
...
function projectedFatigueBandName(currentFatigueBp: number, minutesPerGame: number, staminaRating: number, games: number): string {
    return FATIGUE_BAND_LABEL[projectedFatigueBand(currentFatigueBp, minutesPerGame, staminaRating, games)];
}
export function pendingRotationSetDigest(runRotations: readonly SeasonRotation[], pendingHumanRotation: SeasonRotation): string {
    const merged = runRotations.map((rotation) => rotation.franchiseId === pendingHumanRotation.franchiseId ? pendingHumanRotation : rotation);
