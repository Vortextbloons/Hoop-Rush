...
import type { SeasonMultiplayerTransport, SeasonRoomPublicSnapshot, SeasonRoomSettings, SeasonRoomMembership, SeasonPublicCommandEnvelope, SeasonCommandReceipt, SeasonPrivateDecisionSubmission, SeasonCheckpointAttestation, SeasonAcceptedCheckpoint, SeasonRerunRequest, SeasonIntegrityFailure2, SeasonRoomCode, SeasonRoomMode, SeasonRoomPace, } from '@hoop-rush/data-contracts';
import { PRESENCE_OFFLINE_AFTER_MS, SEASON_MULTIPLAYER_VERSION, SEASON_ROOM_PROTOCOL_SCHEMA_VERSION, isSeasonRoomProtocolOutdated, } from '@hoop-rush/data-contracts';
...
function getOrCreateDevUid(): string {
    if (_memoDevUid && /^[0-9a-f-]{36}$/.test(_memoDevUid))
        return _memoDevUid;
    try {
...
async function ensureAnonAuth(client: SupabaseClient): Promise<void> {
    ...
    const isAnonDisabled = code === 'anonymous_provider_disabled' || msg.toLowerCase().includes('anonymous');
    if (!isAnonDisabled)
...
async function callEdge<T>(client: SupabaseClient, config: SupabaseSeasonTransportConfig, fn: string, body: unknown, captchaSiteKey?: string): Promise<T> {
    ...
    if (token)
        ...
        throw Object.assign(new Error('missing session token'), { code: 'authorization' });
    if (captchaSiteKey)
        headers['x-captcha-token'] = captchaSiteKey;
    if (devUid)
        headers['x-dev-uid'] = devUid;
...
function toPublicSnapshot(row: {
...
}, memberCount = 0): SeasonRoomPublicSnapshot {
    ...
    return {
        ...
        digest: row.digest,
        memberCount,
        codeActive: !!row.code && !!row.code_expires_at && new Date(row.code_expires_at).getTime() > Date.now(),
...
export function createSupabaseSeasonTransport(config: SupabaseSeasonTransportConfig): SeasonMultiplayerTransport {
    ...
    if (!_global.__hoopRushSupabaseClients)
    ...
    return {
        async create(settings: SeasonRoomSettings, rootSeed: string): Promise<SeasonRoomPublicSnapshot & {
            code?: string;
            ...
            const res = await callEdge<{
                ...
                roomId: string;
                membership?: SeasonRoomMembership;
            }>(client, config, 'season-room-create', {
                pace: settings.pace,
            ...
            if (!(snap.settings as unknown as {
                mode?: string;
            }).mode) {
        ...
        async resume(roomId: string, afterOrdinal?: number): Promise<SeasonRoomPublicSnapshot & {
            membership?: SeasonRoomMembership;
            ...
            if (Array.isArray(res.commands) && res.commands.length > 0) {
                ...
                for (const c of res.commands)
                    byOrd.set(c.ordinal, c as SeasonPublicCommandEnvelope);
                if (byOrd.size > 200) {
                    const sortedKeys = [...byOrd.keys()].sort((a, b) => a - b);
                    for (let i = 0; i < sortedKeys.length - 200; i += 1)
                        byOrd.delete(sortedKeys[i]!);
                }
                (snap as unknown as {
                    commands?: SeasonPublicCommandEnvelope[];
        ...
        async refresh(roomId: string): Promise<SeasonRoomPublicSnapshot & {
            membership?: SeasonRoomMembership;
        }> {
            const res = await callEdge<{
                snapshot: SeasonRoomPublicSnapshot;
        ...
        subscribe(roomId: string, handler: (snap: SeasonRoomPublicSnapshot) => void) {
            ...
            function envelopeFromRealtimeRow(row: Record<string, unknown>): SeasonPublicCommandEnvelope | null {
                try {
            ...
            function synthesizeRoomsSnapshot(row: Record<string, unknown>): SeasonRoomPublicSnapshot | null {
                try {
                    ...
                    const snap = toPublicSnapshot({
                        ...
                        locks: (row as unknown as {
                            locks?: SeasonRoomPublicSnapshot['locks'];
                        }).locks ?? lastSnap?.locks,
                        attestationSummary: (row as unknown as {
                            attestationSummary?: SeasonRoomPublicSnapshot['attestationSummary'];
                        }).attestationSummary ?? lastSnap?.attestationSummary,
                    }, mc);
                    if (!row.presence && lastSnap?.presence)
                        snap.presence = lastSnap.presence;
                    if (!snap.locks && lastSnap?.locks)
                        snap.locks = lastSnap.locks;
            ...
            function patchLocksFromDecision(row: Record<string, unknown>): SeasonRoomPublicSnapshot | null {
                if (!lastSnap)
                    return null;
            ...
            function patchAttestationFromRow(row: Record<string, unknown>): SeasonRoomPublicSnapshot | null {
                if (!lastSnap)
                    return null;
            ...
            const coalescedFetchSnap = async (): Promise<SeasonRoomPublicSnapshot | null> => {
                ...
                const p = fetchSnap().finally(() => {
                    if (inflightSnapByRoom.get(roomId) === p)
                        inflightSnapByRoom.delete(roomId);
                });
            ...
            function cacheEnvelope(env: SeasonPublicCommandEnvelope): void {
                ...
                if (byOrd.size > 200) {
                    ...
                    for (let i = 0; i < sorted.length - 200; i += 1)
                        byOrd.delete(sorted[i]!);
                }
            }
            const start = async () => {
                await ensureAnonAuth(client);
                ...
                try {
                    channel = client
                        ...
                        .on('postgres_changes', { event: '*', schema: 'public', table: 'season_rooms', filter: `id=eq.${roomId}` }, (payload: unknown) => {
                        void (async () => {
                            try {
                                const p = payload as {
        ...
        async publishAttestation(attestation: SeasonCheckpointAttestation): Promise<SeasonAcceptedCheckpoint | SeasonRerunRequest | SeasonIntegrityFailure2> {
            const res = await callEdge<{
                kind: 'accepted';
        ...
        async updateSettings(roomId: string, settings: {
            mode: SeasonRoomMode;
            pace: SeasonRoomPace;
            ...
            try {
                const res = await callEdge<{
                    snapshot: SeasonRoomPublicSnapshot;
                }>(client, config, 'season-room-update-settings', { roomId, mode: settings.mode, pace: settings.pace, expectedSettingsRevision });
                return res.snapshot;
            }
            catch (err) {
                const code = (err as {
        ...
        async setReady(roomId: string, participantId: 'p1' | 'p2', ready: boolean, expectedSettingsRevision?: number): Promise<SeasonRoomPublicSnapshot> {
            const res = await callEdge<{
                snapshot: SeasonRoomPublicSnapshot;
            }>(client, config, 'season-room-set-ready', { roomId, participantId, ready, expectedSettingsRevision });
            return res.snapshot;
        },
...
export function isSupabaseConfigured(): boolean {
    const url = (import.meta as unknown as {
        env: Record<string, string | undefined>;
    }).env
        ?.VITE_SUPABASE_URL;
    const key = (import.meta as unknown as {
        env: Record<string, string | undefined>;
    }).env
        ?.VITE_SUPABASE_PUBLISHABLE_KEY;
    const flag = (import.meta as unknown as {
        env: Record<string, string | undefined>;
    }).env
