...
import type { SeasonMultiplayerTransport, SeasonRoomPublicSnapshot, SeasonPublicCommandEnvelope, SeasonPrivateDecisionSubmission, SeasonCheckpointAttestation, SeasonRoomMode, SeasonRoomPace, SeasonRoomMembership, SeasonRoomCode, } from '@hoop-rush/data-contracts';
import { SEASON_MULTIPLAYER_VERSION, SEASON_ROOM_PROTOCOL_SCHEMA_VERSION, SEASON_TIMER_POLICY_VERSION, } from '@hoop-rush/data-contracts';
import { saveMembership, loadMembership, saveCode, loadCode, saveLastRoomId, clearMembership, clearCode, } from './season-room-identity';
...
export function createSeasonRoomCoordinator(deps: SeasonRoomCoordinatorDeps) {
    ...
    function afterOrdinal(): number {
        ...
        if (typeof fromCursor === 'number' && Number.isFinite(fromCursor))
            return fromCursor;
    ...
    function startHeartbeat(roomId: string) {
        ...
        const pid = state.participantId;
        if (pid)
            void deps.transport.heartbeat(roomId, pid).catch(() => { });
    }
    ...
    return {
        ...
        async joinRoom(code: string) {
            ...
            const snap = snapRes;
            const effectiveMembership = (snap as unknown as {
                membership?: SeasonRoomMembership;
            }).membership ?? membership;
            if ((snap as unknown as {
                membership?: SeasonRoomMembership;
            }).membership) {
                saveMembership((snap as unknown as {
                    membership: SeasonRoomMembership;
                }).membership);
            }
            else {
        ...
        async setReady(roomId: string, ready: boolean) {
            const pid = state.participantId;
            if (!pid)
                throw Object.assign(new Error('not a member'), { code: 'membership' });
            const revision = state.publicSnapshot?.settingsRevision;
            ...
            const sub = deps.transport.subscribe(roomId, (snap) => {
                ...
                commandSync = commandSync
                    ...
                    if (cmds.length > 0) {
                        ...
                        if (last && typeof last.ordinal === 'number')
                            lastAcceptedOrdinal = last.ordinal;
                    }
                })
                    .catch((error: unknown) => {
        ...
        async submitCommand(envelope: SeasonPublicCommandEnvelope) {
            ...
            if (receipt.accepted)
                lastAcceptedOrdinal = receipt.ordinal;
            return receipt;
        },
...
export function createInMemorySeasonRoomCoordinator(deps: Omit<SeasonRoomCoordinatorDeps, 'transport'> & {
    transport?: SeasonMultiplayerTransport;
}) {
