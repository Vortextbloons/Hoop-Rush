import { SEASON_POSTSEASON_RISKY_REHAB_COST, rankSeasonPostseason, seasonPostseasonGameTeamsOf, seasonPostseasonHumanEliminated, seasonPostseasonHumanPlaysGame, seasonPostseasonNextGame, type SeasonPostseasonRankings, } from '@hoop-rush/engine';
import { type ConferenceId, type PlayInMatchupId, type PlayoffRound, type PlayoffSeries, type SeasonAwards, type SeasonInjurySeverity, type SeasonPostseasonRotationPayload, type SeasonPostseasonState, type SeasonPostseasonSummary, type SeasonRun, type SeasonRunCommand, type SeasonRunCommandRejection, type SeasonStandings, type SeasonStandingsRow, type SeasonTiebreakResolution, } from '@hoop-rush/data-contracts';
...
export interface SeasonPostseasonHubMethods {
    ...
    spectatePostseasonGame(input: {
        targetGameId: string;
    }): Promise<void>;
    fastForwardPostseason(input?: {
        targetGameId?: string;
    }): Promise<void>;
...
export const POSTSEASON_ORCHESTRATION_UNAVAILABLE = 'Postseason simulation is not available in this build yet. Save your run — it is safe — and update the app to continue.';
...
export function rankedEntriesOf(rankings: SeasonPostseasonRankings, standings: SeasonStandings): Array<{
...
export function nextGameTeamsOf(run: SeasonRun, gameId: string): {
    home: string;
    away: string;
} | null {
...
export function tiebreakResolutionViewModel(resolution: SeasonTiebreakResolution, franchiseName: (franchiseId: string) => string): TiebreakResolutionViewModel {
...
export function seriesCardViewModel(series: PlayoffSeries, humanFranchiseId: string | null): SeriesCardViewModel {
    ...
    return {
        ...
        nextGame: completed || series.homeCourtFranchiseId === null || nextGameNumber > 7
            ? null
            : {
        ...
        humanSeries: humanFranchiseId !== null &&
            (series.homeCourtFranchiseId === humanFranchiseId ||
                series.challengerFranchiseId === humanFranchiseId),
    };
}
export function humanSeriesOf(run: SeasonRun, humanFranchiseId: string): SeriesCardViewModel | null {
    ...
    for (const series of allSeries) {
        if (series.homeCourtFranchiseId === humanFranchiseId ||
            series.challengerFranchiseId === humanFranchiseId) {
...
export function playInGameCardViewModel(state: SeasonPostseasonState, conference: ConferenceId, matchup: PlayInMatchupId, humanFranchiseId: string | null): PlayInGameCardViewModel {
    const playIn = state.playIn[conference];
    const gameKey: 'sevenEight' | 'nineTen' | 'final' = matchup === 'seven-eight' ? 'sevenEight' : matchup === 'nine-ten' ? 'nineTen' : 'final';
    ...
    const seedOf = (franchiseId: string | null): number | null => {
        if (franchiseId === null || ranking === null)
            return null;
        const position = ranking.indexOf(franchiseId);
        return position === -1 ? null : position + 1;
    };
    const consequence = matchup === 'seven-eight'
    ...
    return {
        ...
        consequence,
        humanGame: humanFranchiseId !== null &&
...
export function playInColumnViewModel(state: SeasonPostseasonState, conference: ConferenceId, humanFranchiseId: string | null): PlayInColumnViewModel {
    const playIn = state.playIn[conference];
    ...
    return {
        conference,
        seeds,
        games: (['seven-eight', 'nine-ten', 'final'] as const).map((matchup) => playInGameCardViewModel(state, conference, matchup, humanFranchiseId)),
    };
}
...
export function bracketColumnsOf(state: SeasonPostseasonState, humanFranchiseId: string | null): BracketColumnViewModel[] {
    const playInColumn: BracketColumnViewModel = {
        key: 'play-in',
        title: 'Play-In',
        subtitle: 'Seeds 7–10 · win or go home',
        playIn: (['east', 'west'] as const).map((conference) => playInColumnViewModel(state, conference, humanFranchiseId)),
        series: [],
    };
...
export function mobileBracketCardsOf(state: SeasonPostseasonState, humanFranchiseId: string | null): Array<{
    kind: 'play-in';
    column: PlayInColumnViewModel;
} | {
    kind: 'series';
    columnKey: BracketColumnViewModel['key'];
...
export function postseasonSummaryRow(summary: SeasonPostseasonSummary, humanFranchiseId: string): PostseasonSummaryRow {
    const humanGame = summary.homeFranchiseId === humanFranchiseId || summary.awayFranchiseId === humanFranchiseId;
    const humanWon = humanGame ? summary.winnerFranchiseId === humanFranchiseId : null;
    const roundLabelText = summary.phase === 'play-in'
    ...
    return {
        ...
        scoreLabel: summary.status === 'forfeit'
            ? '2–0 · forfeit'
...
export function awardsViewModel(awards: SeasonAwards, playerName: (playerVersionId: string) => string, franchiseName: (franchiseId: string) => string): {
    awards: AwardViewModel[];
    firstTeam: AwardViewModel[];
} {
    const titleOf = (key: AwardViewModel['key']): string => key === 'mvp'
...
export function riskyRehabOptionsOf(run: SeasonRun, humanFranchiseId: string, playerName: (playerVersionId: string) => string): RiskyRehabOption[] {
...
export function describePostseasonRejection(command: SeasonRunCommand['command'], rejection: SeasonRunCommandRejection): string {
