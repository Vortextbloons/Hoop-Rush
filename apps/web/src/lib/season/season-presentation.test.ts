...
describe('boxScoreFromSummary', () => {
    it('derives the human-side box with names and positions', () => {
        ...
        const box = boxScoreFromSummary(summary({ gameId: 's000001', round: 1 }), 'lakers', names, playable);
        if (box === null) {
    ...
    it('returns null for an unrelated franchise', () => {
        const box = boxScoreFromSummary(summary({ gameId: 's000001', round: 1 }), 'nuggets', new Map(), new Map());
        expect(box).toBeNull();
    });
});
