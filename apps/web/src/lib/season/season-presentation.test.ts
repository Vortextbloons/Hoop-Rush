...
import { SEASON_GAME_SUMMARY_VERSION, type SeasonGame, type SeasonGameSummary, type SeasonLeague, type SeasonStandings, } from '@hoop-rush/data-contracts';
...
import { boxScoreFromSummary, deriveBlockRecap, finalizeGameRecords, foldSeasonAggregates, franchiseStreak, franchiseStreaks, humanScheduleRows, ordinal, provisionalRanking, rebaseStandingsBefore, recordLabel, streakLabel, winPct, } from './season-presentation';
const LEAGUE = buildSeasonLeague({}, { humanFranchiseId: franchiseIdSchema.parse('lakers') });
function playerLine(playerVersionId: string, points: number): SeasonGameSummary['homePlayers'][number] {
    return {
        playerVersionId,
        seconds: 1800,
        points,
...
function summary(overrides: Partial<SeasonGameSummary> & {
    gameId: string;
    round: number;
}): SeasonGameSummary {
    const homePlayers = Array.from({ length: 10 }, (_, i) => playerLine(`pv-home-${String(i)}`, 10 + i));
    const awayPlayers = Array.from({ length: 10 }, (_, i) => playerLine(`pv-away-${String(i)}`, 8 + i));
    return {
        schemaVersion: 1,
...
function standingsRow(standings: SeasonStandings, franchiseId: string): SeasonStandings['rows'][number] {
...
function teamAggregateOf(teams: readonly {
    franchiseId: string;
    wins: number;
    losses: number;
    points: number;
...
describe('boxScoreFromSummary', () => {
    it('derives the human-side box with names and positions', () => {
        ...
        const box = boxScoreFromSummary(summary({ gameId: seasonGameIdSchema.parse('s000001'), round: 1 }), 'lakers', names, playable);
        if (box === null) {
    ...
    it('returns null for an unrelated franchise', () => {
        const box = boxScoreFromSummary(summary({ gameId: seasonGameIdSchema.parse('s000001'), round: 1 }), 'nuggets', new Map(), new Map());
        expect(box).toBeNull();
    });
});
