...
import { SEASON_COMMAND_LOG_VERSION, SEASON_DRAFT_CATALOG_V3, SEASON_DURABILITY_VERSION, SEASON_ROUND_COUNT, SEASON_RUN_SCHEMA_VERSION, SEASON_STAMINA_VERSION, SIMULATION_RATINGS, SIMULATION_TENDENCIES, PLAYER_VERSION_ID_VERSION, canonicalJson, seasonAlmanacDigest, seasonCommandLogDigest, seasonDigestHex, seasonPostseasonSummarySchema, seasonPostseasonWorkerStartRequestSchema, seasonPostseasonWorkerCancelRequestSchema, type Position, type SeasonActiveRunIndex, type SeasonCommandLog, type SeasonCommandLogEntry, type SeasonDraftCandidate, type SeasonDraftCatalog, type SeasonEffectsState, type SeasonGameSimulationInput, type SeasonGameSimulationResult, type SeasonGameSideResult, type SeasonInjuryRecord, type SeasonPostseasonSummary, type SeasonRun, type SeasonRunCommand, type SeasonStandings, } from '@hoop-rush/data-contracts';
import { eraIdSchema, commandIdSchema, franchiseIdSchema, idSchema, seedSchema, seasonGameIdSchema, } from '@hoop-rush/data-contracts';
import { handleSeasonRunCommand, seasonRunStateDigest, seasonPostseasonNextGame, seasonPostseasonHumanEliminated, zeroSeasonGameTransition, type SeasonPostseasonGameResolver, } from '@hoop-rush/engine';
import { buildEraSimulationProfile, buildSeasonLeague, buildSeasonRunFixture, } from '@hoop-rush/test-fixtures';
import { generateSeasonSchedule } from '@hoop-rush/engine';
import { SeasonRunCommandDuplicateError, SeasonRunCommandRunMismatchError, SeasonRunCommandStaleStateError, type CommitPostseasonAdvancementInput, type PromoteChampionInput, type SeasonCompletedSeason, type SeasonCompletedRunIndexEntry, type SeasonRunRepository, type SeasonRunSnapshot, } from '@hoop-rush/persistence';
import { createSeasonPostseasonRunner, promoteSeasonChampion, SEASON_POSTSEASON_CHUNK_MAX_GAMES, type SeasonPostseasonEvent, type SeasonPostseasonRunner, } from './season-postseason-runner';
...
function catalogOf(run: SeasonRun): SeasonDraftCatalog {
    const candidates: SeasonDraftCandidate[] = run.rosters.flatMap((roster) => roster.players.map((player, slot) => {
        const playable = SLOT_POSITIONS[slot];
        if (playable === undefined)
            throw new Error(`no position pattern for slot ${String(slot)}`);
        const primary = playable[0];
        if (primary === undefined)
            throw new Error(`no primary position for slot ${String(slot)}`);
        return {
...
function withHumanStarterInjury(run: SeasonRun): SeasonRun {
    ...
    if (starter === undefined)
        throw new Error('fixture run has no human rotation starter');
...
function forcedCompletedResult(gameInput: SeasonGameSimulationInput, homeScore: number, awayScore: number): {
...
function humanLosesResolver(): SeasonPostseasonGameResolver {
    return ({ gameInput, pregameEffects }) => {
        ...
        return {
            ...forcedCompletedResult(gameInput, winner === 'home' ? 110 : 90, winner === 'home' ? 90 : 110),
            transition: zeroSeasonGameTransition(pregameEffects),
        };
    };
}
function contextOf(run: SeasonRun, effects: SeasonEffectsState, catalog: SeasonDraftCatalog, profile: ReturnType<typeof buildEraSimulationProfile>, resolver: SeasonPostseasonGameResolver): Parameters<typeof handleSeasonRunCommand>[1] {
    return {
        run,
        pending: null,
...
function commandOf(run: SeasonRun, command: 'start-postseason' | 'advance-postseason', commandId: string, extra: Record<string, unknown> = {}): SeasonRunCommand {
    return {
...
class FakePostseasonRepository implements SeasonRunRepository {
    ...
    commitPostseasonAdvancement(input: CommitPostseasonAdvancementInput): Promise<void> {
        ...
        if (current.stateRevision !== input.command.expectedStateRevision ||
            current.stateDigest !== input.command.expectedStateDigest) {
            return Promise.reject(new SeasonRunCommandStaleStateError(input.command.commandId, input.command.expectedStateRevision, current.stateRevision));
        }
        ...
        if (input.run.stateRevision !== input.command.expectedStateRevision + 1) {
            return Promise.reject(new Error('advancement must advance the state revision by exactly one'));
        }
        ...
        const effects = (input.run as SeasonRun & {
    ...
    promoteChampionToCompleted(input: PromoteChampionInput): Promise<void> {
        ...
        if (input.run.postseason.championFranchiseId !== input.almanac.championFranchiseId ||
...
function makeRunner(repo: FakePostseasonRepository, catalog: SeasonDraftCatalog, profile: ReturnType<typeof buildEraSimulationProfile>, resolver: SeasonPostseasonGameResolver): SeasonPostseasonRunner {
...
async function startPostseason(repo: FakePostseasonRepository, run: SeasonRun, fixture: Pick<RunnerFixture, 'catalog' | 'profile' | 'effects' | 'resolver'>): Promise<SeasonRun> {
    const command = commandOf(run, 'start-postseason', 'xt-start-1');
    const output = handleSeasonRunCommand(command, contextOf(run, fixture.effects, fixture.catalog, fixture.profile, fixture.resolver));
    ...
    await repo.commitPostseasonAdvancement({
        ...
        resultDigest: seasonDigestHex(canonicalJson({ commandId: command.commandId, gameIds: [], summaryDigests: [] })),
        relatedGameIds: [],
        transactionIds: [],
    });
    return output.run;
}
async function advanceUntilEliminated(repo: FakePostseasonRepository, run: SeasonRun, fixture: Pick<RunnerFixture, 'catalog' | 'profile' | 'effects' | 'resolver'>): Promise<SeasonRun> {
    let current = run;
    let guard = 0;
    while (!seasonPostseasonHumanEliminated(current.postseason, HUMAN) && guard < 40) {
        ...
        const output = handleSeasonRunCommand(command, contextOf(current, fixture.effects, fixture.catalog, fixture.profile, fixture.resolver));
        const envelope = output.result;
        ...
        await repo.commitPostseasonAdvancement({
            ...
            preStateRevision: command.expectedStateRevision,
            preStateDigest: command.expectedStateDigest,
            resultDigest: seasonDigestHex(canonicalJson({
...
function collectUntil(runner: SeasonPostseasonRunner, terminal: SeasonPostseasonEvent['type']): Promise<SeasonPostseasonEvent[]> {
    const events: SeasonPostseasonEvent[] = [];
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
            unsubscribe();
            reject(new Error(`timed out waiting for ${terminal}; got ${events.map((e) => e.type).join(', ')}`));
        }, 20000);
...
function setActiveRun(repo: FakePostseasonRepository, run: SeasonRun): void {
    ...
    if (current === null)
        throw new Error('no active snapshot to replace');
...
describe('season postseason runner (M2.6 orchestration)', () => {
    ...
    it('retries idempotently when the first command id was already committed', async () => {
        ...
        if (firstGame.kind !== 'game')
            throw new Error('expected a first game');
        const first = commandOf(started, 'advance-postseason', 'adv-session-retry', {
            targetGameId: firstGame.gameId,
        });
        const output = handleSeasonRunCommand(first, contextOf(started, fixture.effects, fixture.catalog, fixture.profile, fixture.resolver));
        const envelope = output.result;
        if (envelope.command !== 'advance-postseason' || envelope.result.status !== 'accepted') {
    ...
    it('resumes from the accepted state after a reload between games', async () => {
        ...
        expect(repo.summaries.filter((summary) => summary.gameId === 'pi-east-seven-eight')).toHaveLength(1);
    });
    ...
    it('promotes the champion atomically with reconciling almanac digests', async () => {
        ...
        expect(completed.run.completion?.championFranchiseId).toBe(completed.almanac.championFranchiseId);
        expect(completed.run.completion?.championFranchiseId).toBe(completed.run.postseason.championFranchiseId);
        const facts = {
        ...
        expect(completed.almanac.commandLogDigest).toBe(seasonCommandLogDigest(completed.commandLog.entries));
        expect(completed.almanac.postseasonDigest).toBe(seasonDigestHex(canonicalJson(completed.run.postseason)));
        expect(completed.almanac.awardsDigest).toBe(seasonDigestHex(canonicalJson(completed.run.awards)));
        ...
        expect((await repo.listCompletedSeasonRuns()).some((entry) => entry.runId === eliminated.runId)).toBe(true);
        for (const summary of repo.summaries) {
            expect(() => seasonPostseasonSummarySchema.parse(summary)).not.toThrow();
        }
    });
