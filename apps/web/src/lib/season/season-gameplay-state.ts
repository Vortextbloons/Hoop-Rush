...
export function deriveGameplayState(draft: SeasonDraftState, generation: SeasonLeagueGenerationResult | null, bootstrap: GameplayBootstrapResult | null, snapshot?: {
    phase?: string;
    locks?: {
        p1Locked: boolean;
        p2Locked: boolean;
    ...
    if (draft.status === 'complete' && generation && bootstrap) {
        ...
        if (snapshot?.phase === 'hash-verification' || snapshot?.phase === 'checkpoint-setup')
            base.phase = 'hash-verification';
        if (snapshot?.phase === 'integrity-failed')
            base.phase = 'hash-verification';
