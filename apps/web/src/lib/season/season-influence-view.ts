import { SEASON_INFLUENCE_CAP, SEASON_INFLUENCE_FLOOR, SEASON_OBJECTIVE_CATALOG, type SeasonHealthState, type SeasonInfluenceLedgerEntry, type SeasonInfluenceRehabOutcome, type SeasonInfluenceState, type SeasonObjectiveId, type SeasonRun, type SeasonTradeWindowState, } from '@hoop-rush/data-contracts';
...
export function influenceViewModel(state: SeasonInfluenceState, humanFranchiseId: string, health: SeasonHealthState | null = null, openWindow: SeasonTradeWindowState | null = null): InfluenceViewModel {
    ...
    const recentEntries = state.ledger
        ...
        .slice(-5)
        .reverse();
    const windowSpends = new Map((state.windows[humanFranchiseId] ?? []).map((window) => [
        window.windowIndex,
    ...
    const activeInjuries = health === null
        ? []
        : health.injuries.filter((record) => record.franchiseId === humanFranchiseId &&
    ...
    for (const injuryId of injuryIds) {
        ...
        affordances.push({
            ...
            affordable: (outcome === null || outcome === 'pending') && balance - 2 >= SEASON_INFLUENCE_FLOOR,
...
function lastEvaluatedSelection(run: SeasonRun, definitions: Map<SeasonObjectiveId, (typeof SEASON_OBJECTIVE_CATALOG)[number]>): ObjectiveChoicesViewModel['lastEvaluation'] {
...
export function currentObjectiveBlock(run: SeasonRun): number | null {
    if (run.cursor.completedRounds >= SEASON_ROUND_COUNT)
        return null;
