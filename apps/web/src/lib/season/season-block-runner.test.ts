...
import { SEASON_NEUTRAL_HOME_COURT, SEASON_FREE_AGENCY_VERSION, seasonWorkerCancelRequestSchema, seasonWorkerContinueRequestSchema, seasonWorkerStartRequestSchema, seasonWorkerWarmRequestSchema, type SeasonBlockRecap, type SeasonCandidateCheckpoint, type SeasonGameSummary, type SeasonPendingBlockCandidate, type SeasonRun, type SeasonSchedule, type SeasonStandings, } from '@hoop-rush/data-contracts';
...
vi.mock('@hoop-rush/engine', async (importOriginal) => ({
    ...
    completeSeasonBlockCommit: vi.fn((input: {
        run: SeasonRun;
...
import { generateSeasonSchedule, completeSeasonBlockCommit, seasonCheckpointDigest, seasonRotationSetDigest, } from '@hoop-rush/engine';
class FakeWorker {
...
describe('season block runner (M2.5 wire)', () => {
    ...
    it('digests the LOCKED rotation set at commit, not the pre-submission run', async () => {
        ...
        const storedHuman = commitInput.run.rotations.find((rotation) => rotation.franchiseId === 'lakers');
