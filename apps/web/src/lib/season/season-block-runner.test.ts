...
import type { CommitSeasonBlockInput, SeasonRunRepository, SeasonRunSnapshot, } from '@hoop-rush/persistence';
import { SEASON_NEUTRAL_HOME_COURT, seasonCandidateCheckpointSchema, seasonWorkerCancelRequestSchema, seasonWorkerContinueRequestSchema, seasonWorkerStartRequestSchema, seasonWorkerWarmRequestSchema, type SeasonBlockRecap, type SeasonCandidateCheckpoint, type SeasonEffectsState, type SeasonGameSummary, type SeasonPendingBlockCandidate, type SeasonRun, type SeasonSchedule, type SeasonStandings, } from '@hoop-rush/data-contracts';
...
import { generateSeasonSchedule, seasonCheckpointDigest, seasonFranchiseLegalFiveFacts, seasonRotationSetDigest, } from '@hoop-rush/engine';
class FakeWorker {
    ...
    postMessage(data: unknown): void {
        if (FakeWorker.clonePostedMessages)
...
function startInput(run: SeasonRun, partial: Partial<SeasonBlockStartInput> = {}): SeasonBlockStartInput {
    return {
        run,
        effects: buildZeroEffects(run),
...
function makeCandidate(run: SeasonRun, partial: Partial<SeasonCandidateCheckpoint> = {}): SeasonCandidateCheckpoint {
    ...
    const playerAggregates = run.rosters.flatMap((roster) => roster.players.map((player) => ({
        playerVersionId: player.playerVersionId,
...
describe('season block runner (M2.5 wire)', () => {
    ...
    it('carries the authoritative pre-block free-agency state through the commit', async () => {
        ...
        expect(complete.snapshot.run.freeAgency.canonicalCandidates).toEqual(run.freeAgency.canonicalCandidates);
    });
    it('digests the LOCKED rotation set at commit, not the pre-submission run', async () => {
        ...
        if (human === undefined)
            throw new Error('fixture run has no human rotation');
        ...
        const locked = run.rotations.map((rotation) => rotation.franchiseId === 'lakers' ? pending : rotation);
        runner.startBlock(startInput(run, { rotations: locked, rotationDigest: seasonRotationSetDigest(locked) }));
        await flush();
        const worker = FakeWorker.instances[0];
        expect(worker).toBeDefined();
        const started = events.find((event) => event.type === 'started');
        ...
        await flush();
        const repositoryInput = repository.commitSeasonBlock.mock.calls[0]?.[0] as CommitSeasonBlockInput | undefined;
        expect(repositoryInput?.rotations).toEqual(locked);
        const storedHuman = repositoryInput?.rotations.find((rotation) => rotation.franchiseId === 'lakers');
    ...
    it('sends authoritative effects and health on a full reset after roster context changes', async () => {
        ...
        if (firstPair === undefined)
            throw new Error('fixture has no chemistry pair');
        changedEffects.pairStates[0] = { ...firstPair, sharedPossessions: 77 };
        runner.startBlock(startInput(changedRun, {
    ...
    it('persists an invalid-roster interruption and emits the typed event', async () => {
        ...
        const expectedAvailability = seasonFranchiseLegalFiveFacts(run, franchiseIdSchema.parse('lakers'), pending.health);
        expect(interruption.unavailablePlayerVersionIds).toEqual(expectedAvailability.unavailablePlayerVersionIds);
