...
function isSeasonRunMutation(value: unknown): value is SeasonRunMutation {
    ...
    if (typeof mutation.committedAt !== 'number')
        return false;
    if (typeof mutation.sourceId !== 'string')
        return false;
    if (mutation.kind !== 'clear' && typeof mutation.runId !== 'string')
        return false;
    return true;
}
...
export function createSeasonRunChannel(): SeasonRunChannel {
    ...
    channel.onmessage = (event: MessageEvent<SeasonRunMutation>) => {
        ...
        if (!isSeasonRunMutation(mutation))
            return;
