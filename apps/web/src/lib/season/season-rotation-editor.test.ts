...
import { canPlay, SEASON_MINUTE_POLICY_VERSION, SEASON_ROTATION_VERSION, type Position, type SeasonRotation, } from '@hoop-rush/data-contracts';
import { franchiseIdSchema } from '@hoop-rush/data-contracts';
import { ROTATION_PRESETS, createRotationEditor, failurePlayerVersionId, indexRotationFailures, presetLabel, rotationEditorNeedsPositionRefresh, rotationRoleOf, strategyLabel, SLOT_GROUPS, type RotationMember, } from './season-rotation-editor';
import { CANDIDATES, legalRotation, rotationMembers, rotationPlayableOf, } from './season-rotation-test-support';
...
function guardBenchPlayer(e: ReturnType<typeof editor>): string | null {
    return (e.rotation.benchOrder.find((id) => playableOf(id).includes('PG') || playableOf(id).includes('SG')) ?? null);
}
function forwardBenchPlayer(e: ReturnType<typeof editor>): string | null {
    return (e.rotation.benchOrder.find((id) => playableOf(id).includes('SF') || playableOf(id).includes('PF')) ?? null);
}
function centerOnlyPlayer(): string | null {
    const candidate = CANDIDATES.find((c) => c.positions.playable.length === 1 && c.positions.playable[0] === 'C');
...
describe('RotationEditor', () => {
    ...
    it('throws when the member list is outside ten to fifteen players', () => {
        expect(() => createRotationEditor(rotation(), members().slice(0, 9))).toThrow(/10 to 15 roster members/);
...
describe('RotationEditor.eligibleForSlot', () => {
    it('offers only players who can legally play the slot group', () => {
        const e = editor();
        const eligibleFor = (slotIndex: number) => new Set(e.eligibleForSlot(slotIndex).map((member) => member.playerVersionId));
    ...
    it('includes every current starter in their own slot, and a center-only player only in C slots', () => {
        ...
        expect(e.eligibleForSlot(4).map((m) => m.playerVersionId)).toContain(centerOnly.playerVersionId);
        expect(e.eligibleForSlot(0).map((m) => m.playerVersionId)).not.toContain(centerOnly.playerVersionId);
    });
});
describe('RotationEditor.applyRotation', () => {
    it('commits a valid external candidate and returns the committed rotation', () => {
        ...
        const candidate: SeasonRotation = {
            ...e.rotation,
            targetMinutes: e.rotation.targetMinutes.map((entry, index) => index === 0
    ...
    it('rejects an invalid external candidate and leaves the rotation unchanged', () => {
        ...
        const invalid: SeasonRotation = {
            ...e.rotation,
            targetMinutes: e.rotation.targetMinutes.map((entry, index) => index === 0 ? { ...entry, minutes: entry.minutes + 1 } : entry),
        };
...
describe('failurePlayerVersionId', () => {
    it('extracts ids from per-player audit messages', () => {
        ...
        expect(failurePlayerVersionId('target minutes for pv-2 must be an integer from 0-48 (got 7)')).toBe('pv-2');
    ...
    it('returns null for global audit messages', () => {
        ...
        expect(failurePlayerVersionId('rotation must reference exactly ten players (got 5)')).toBeNull();
    });
});
...
describe('rotationEditorNeedsPositionRefresh', () => {
    it('returns true when the slice gains positions the cached editor lacks', () => {
        ...
        expect(rotationEditorNeedsPositionRefresh(cached, rosterIds, (id) => playableOf(id))).toBe(true);
    });
    it('returns false when the editor already has loaded playables', () => {
        const cached = editor();
        const rosterIds = members().map((member) => member.playerVersionId);
        expect(rotationEditorNeedsPositionRefresh(cached, rosterIds, (id) => playableOf(id))).toBe(false);
    });
});
