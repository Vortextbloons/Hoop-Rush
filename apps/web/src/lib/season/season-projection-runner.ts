...
import type { ProjectionRotationLoadRow, ProjectionRotationOptimizeRequest, ProjectionRosterBuildRequest, ProjectionWorkerRequest, ProjectionWorkerResponse, } from './season-projection-wire';
...
export function createProjectionRunner(deps: ProjectionRunnerDeps = {}): ProjectionRunner {
    ...
    return {
        ...
        async optimizeRotation(input: ProjectionRotationOptimizeInput): Promise<MinutePlanOptimizationResult> {
            const urls = await seasonArtifactUrls();
            if (urls.modelUrl === undefined || urls.modelHash === undefined) {
                throw missingModelError();
            }
...
function missingModelError(): Error {
    return new Error('The projection model artifact is unavailable; run `projection build --write` and refresh the manifest.');
}
class ProjectionWorkerClient {
    ...
    private ensureWorker(): Worker {
        if (this.worker !== null)
            return this.worker;
        const worker = this.deps.workerUrl !== undefined
        ...
        worker.addEventListener('message', (event: MessageEvent<unknown>) => {
            ...
            if (message.type === 'complete') {
                entry.resolve((message as unknown as {
                    result: unknown;
                }).result);
                return;
            }
            entry.reject(new Error((message as unknown as {
                message: string;
            }).message));
        });
