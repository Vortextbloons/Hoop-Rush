import { SEASON_INFLUENCE_CAP, SEASON_INFLUENCE_FLOOR, SEASON_RECAP_VERSION, SEASON_ROSTER_MAX_SIZE, SEASON_ROSTER_MIN_SIZE, SEASON_ROUND_COUNT, blockRoundRange, seasonUpcomingHumanGameSchema, type FranchiseId, type SeasonGameId, type HoopRushManifest, type SeasonBlockInjuryEvidence, type SeasonBlockRecap, type SeasonCampaignCondition, type SeasonCampaignEvaluation, type SeasonCampaignOpportunity, type SeasonCampaignReward, type SeasonCampaignState, type SeasonFreeAgencyState, type SeasonGame, type SeasonGameSummary, type SeasonLeaderCategory, type SeasonLeague, type SeasonNotablePerformance, type SeasonPlayerAggregate, type SeasonRecordMovement, type SeasonRosterEntry, type SeasonRun, type SeasonStandings, type SeasonStandingsRow, type SeasonStreak, type SeasonTeamAggregate, type SeasonTeamBox, type SeasonTradeValueTrend, type SeasonUpcomingHumanGame, type SeasonVersionSpotlight, } from '@hoop-rush/data-contracts';
import { humanUpcomingGames } from './season-lock-preview';
export function provisionalRanking(standings: SeasonStandings, league: SeasonLeague): {
    row: SeasonStandingsRow;
    rank: number;
    conference: 'east' | 'west';
}[] {
    const conferenceOf = new Map(league.teams.map((team) => [team.franchiseId, team.conference]));
    const rows = [...standings.rows].sort((a, b) => b.wins - a.wins ||
...
function streakOfGames(games: readonly SeasonGameSummary[], franchiseId: string): {
    kind: 'wins' | 'losses';
    length: number;
} | null {
    if (games.length === 0)
...
export function franchiseStreak(summaries: readonly SeasonGameSummary[], franchiseId: string): {
    kind: 'wins' | 'losses';
    length: number;
} | null {
    const games = summaries
        .filter((summary) => summary.homeFranchiseId === franchiseId || summary.awayFranchiseId === franchiseId)
        .sort((a, b) => a.round - b.round);
    return streakOfGames(games, franchiseId);
}
export function franchiseStreaks(summaries: readonly SeasonGameSummary[], franchiseIds: readonly string[]): Map<string, {
    kind: 'wins' | 'losses';
    length: number;
} | null> {
...
export function finalizeGameRecords(games: readonly SeasonGame[], summaries: readonly SeasonGameSummary[]): SeasonGame[] {
...
export function boxScoreFromSummary(summary: SeasonGameSummary, franchiseId: string, names: ReadonlyMap<string, string>, playable: ReadonlyMap<string, readonly string[]>): BoxScore | null {
    const isHome = summary.homeFranchiseId === franchiseId;
    if (!isHome && summary.awayFranchiseId !== franchiseId)
        return null;
...
export function humanScheduleRows(games: readonly SeasonGame[], humanFranchiseId: string): ScheduleRow[] {
    return games
        .filter((game) => game.homeFranchiseId === humanFranchiseId || game.awayFranchiseId === humanFranchiseId)
        .sort((a, b) => a.round - b.round)
        .map((game) => {
...
const aggregateFoldCache = new WeakMap<readonly SeasonGameSummary[], {
...
export function foldSeasonAggregates(summaries: readonly SeasonGameSummary[]): {
    ...
    const result = {
        ...
        players: [...players.values()].sort((a, b) => a.playerVersionId.localeCompare(b.playerVersionId)),
    };
    aggregateFoldCache.set(summaries, result);
    return result;
}
export function rebaseStandingsBefore(standings: SeasonStandings, league: SeasonLeague, blockSummaries: readonly SeasonGameSummary[]): SeasonStandings {
    const byId = new Map(standings.rows.map((row) => [
        row.franchiseId,
        {
            ...row,
    ...
    const adjust = (franchiseId: FranchiseId, delta: {
        wins?: number;
        losses?: number;
        gamesPlayed?: number;
        home?: number;
    ...
    const headToHead = (franchiseId: FranchiseId, opponentId: FranchiseId): {
        wins: number;
        losses: number;
    } => {
        const row = byId.get(franchiseId);
    ...
    for (const summary of blockSummaries) {
        ...
        if (loserHh)
            loserHh.losses = hhLoser.losses - 1;
    }
    return {
        ...standings,
...
export function deriveNotablePerformances(blockSummaries: readonly SeasonGameSummary[], humanFranchiseId: string): SeasonNotablePerformance[] {
    const lines: Array<{
        playerVersionId: string;
    ...
    const sort = (entries: typeof lines) => [...entries].sort((a, b) => b.points - a.points ||
...
export function deriveVersionSpotlights(blockSummaries: readonly SeasonGameSummary[], rosters: readonly SeasonRosterEntry[]): SeasonVersionSpotlight[] {
    ...
    for (const versions of byPerson.values()) {
        ...
        for (let i = 0; i < versions.length; i += 1) {
            for (let j = i + 1; j < versions.length; j += 1) {
                ...
                for (const summary of blockSummaries) {
                    const meeting = (summary.homeFranchiseId === ta.franchiseId &&
                    ...
                    if (!meeting)
                        continue;
                    headToHeadGames += 1;
                    if (didWin(summary, ta.franchiseId))
                        headToHeadWinsA += 1;
                    else
                        headToHeadWinsB += 1;
                }
    ...
    spotlights.sort((a, b) => b.pointsA + b.pointsB - (a.pointsA + a.pointsB) || a.versionA.localeCompare(b.versionA));
...
export function deriveBlockRecap(input: {
    ...
    const { runId, blockIndex, completedRounds, standings, league, blockSummaries, allSummaries, rosters, games, humanFranchiseId, run, } = input;
    const before = rebaseStandingsBefore(standings, league, blockSummaries);
    const conferenceByFranchise = new Map(league.teams.map((team) => [team.franchiseId, team.conference]));
    ...
    for (const row of standings.rows) {
        ...
        if (list)
            ...
            sortedByConference.set(conference, [row]);
    }
    ...
    for (const row of before.rows) {
        ...
        if (list)
            ...
            sortedBeforeByConference.set(conference, [row]);
    }
    ...
    const rankOf = (rows: SeasonStandingsRow[], franchiseId: string, conference: string): number => {
        const cached = rows === standings.rows
    ...
    const streaksByFranchise = franchiseStreaks(allSummaries, league.teams.map((team) => team.franchiseId));
    ...
    const upcomingHumanGames: SeasonUpcomingHumanGame[] = nextBlockIndex <= 8 ? humanUpcomingGamesFromGames(games, humanFranchiseId, nextBlockIndex) : [];
    return {
        schemaVersion: 1,
        ...
        tradeEvidence: {
            tradesAccepted: run.transactions.filter((entry) => entry.type === 'trade' && entry.blockIndex === blockIndex).length,
            influenceDelta: run.influence.ledger
                .filter((entry) => entry.franchiseId === humanFranchiseId && entry.blockIndex === blockIndex)
...
export function blockFreeAgencyEvidenceOf(input: {
    ...
    const parsedHuman = input.humanFranchiseId === null ? null : (input.humanFranchiseId as FranchiseId);
...
export function humanBalanceAtBlockEnd(run: SeasonRun, humanFranchiseId: string, blockIndex: number): number {
...
export function deriveBlockInjuryEvidence(input: {
    ...
    for (const summary of blockSummaries) {
        const humanSide = summary.homeFranchiseId === humanFranchiseId
            ? 'home'
        ...
        for (const event of summary.injuryEvents) {
            ...
            if (event.returned)
                sameGameReturns += 1;
            if (event.severity === 'season-ending')
                seasonEnding += 1;
    ...
    const returnedThisBlock = health.injuries.filter((record) => record.actualReturnRound !== null &&
        record.actualReturnRound >= fromRound &&
        record.actualReturnRound <= toRound).length;
...
export function humanUpcomingGamesFromGames(games: readonly SeasonGame[], humanFranchiseId: string, blockIndex: number): SeasonUpcomingHumanGame[] {
    return humanUpcomingGames(games, humanFranchiseId, blockIndex).map((game) => seasonUpcomingHumanGameSchema.parse(game));
}
...
export function formatCampaignCondition(condition: SeasonCampaignCondition): string {
    const op = condition.comparisonOperator === 'gte'
        ? '≥'
...
export function campaignRewardSummary(evaluations: readonly SeasonCampaignEvaluation[], appliedRewardIds: readonly string[]): string {
...
export function campaignTimelineViewModel(run: SeasonRun | null, nextBlockIndex: number | null): CampaignTimelineViewModel | null {
    if (run === null)
        return null;
    ...
    const offersForBlock = targetBlock >= 0 && targetBlock < 8 ? (campaign.offers[targetBlock] ?? []) : [];
...
export function inquiryCounterLabel(allowance: number, used: number, purchasedUsed: boolean, earnedUsed: boolean): string {
    const base = 3;
...
export function packageConsequenceFacts(input: {
    ...
    const legal = fromAfter >= SEASON_ROSTER_MIN_SIZE &&
    ...
    const influenceNote = input.influenceAmount === 0
