...
export function rebaseStandingsBefore(standings: SeasonStandings, league: SeasonLeague, blockSummaries: readonly SeasonGameSummary[]): SeasonStandings {
    ...
    const adjust = (franchiseId: string, delta: {
        wins?: number;
        losses?: number;
        gamesPlayed?: number;
        home?: number;
    ...
    const headToHead = (franchiseId: string, opponentId: string): {
        wins: number;
        losses: number;
    } => {
        const row = byId.get(franchiseId);
...
export function campaignTimelineViewModel(run: SeasonRun | null, nextBlockIndex: number | null): CampaignTimelineViewModel | null {
    ...
    const offersForBlock = targetBlock !== null && targetBlock >= 0 && targetBlock < 8
