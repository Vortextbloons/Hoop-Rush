...
import { buildLockPreview, gamesToLockForBlock, humanUpcomingGames, pendingRotationSetDigest, } from './season-lock-preview';
...
const NAMES = new Map<string, string>(CATALOG.candidates.map((c) => [c.playerVersionId, c.displayName]));
...
describe('humanUpcomingGames', () => {
    ...
    it('agrees with the season-presentation helper on every block', () => {
        for (let block = 0; block < 9; block += 1) {
            expect(humanUpcomingGamesFromGames(games(), 'lakers', block)).toEqual(humanUpcomingGames(games(), 'lakers', block));
        }
    });
});
