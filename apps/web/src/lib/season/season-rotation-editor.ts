import { SEASON_ROSTER_MAX_SIZE, SEASON_ROTATION_PRESET_TARGETS, SEASON_ROTATION_SIZE, canPlay, type Position, type SeasonMinutePolicyStrategy, type SeasonRotation, type SeasonRotationPreset, type SlotGroup, } from '@hoop-rush/data-contracts';
...
export function rotationRoleOf(rotation: SeasonRotation, playerVersionId: string): string {
    ...
    if (benchIndex !== -1)
        return `Bench ${String(benchIndex + 1)}`;
    return 'Inactive';
}
export class RotationEditor {
    ...
    constructor(rotation: SeasonRotation, members: RotationMember[]) {
        if (members.length < SEASON_ROTATION_SIZE || members.length > SEASON_ROSTER_MAX_SIZE) {
            throw new Error(`rotation editor needs ${String(SEASON_ROTATION_SIZE)} to ${String(SEASON_ROSTER_MAX_SIZE)} roster members (got ${String(members.length)})`);
        }
        const partition = [...rotation.starters, ...rotation.benchOrder];
        if (partition.length !== SEASON_ROTATION_SIZE) {
            throw new Error(`rotation must reference exactly ${String(SEASON_ROTATION_SIZE)} players (got ${String(partition.length)})`);
        }
        this.rotation = rotation;
        this.members = members;
        this.activeIds = new Set(partition);
        this.memberPlayable = new Map(members
    ...
    promoteToRotation(inactivePlayerVersionId: string, replacedPlayerVersionId: string): string[] {
        if (inactivePlayerVersionId === replacedPlayerVersionId)
            return [];
        ...
        const promoted = this.members.find((member) => member.playerVersionId === inactivePlayerVersionId);
        if (promoted === undefined) {
            return [`${inactivePlayerVersionId} is not on the roster`];
        }
        const candidate: SeasonRotation = {
            ...this.rotation,
            starters: this.rotation.starters.map((id) => id === replacedPlayerVersionId ? inactivePlayerVersionId : id),
            benchOrder: this.rotation.benchOrder.map((id) => id === replacedPlayerVersionId ? inactivePlayerVersionId : id),
            targetMinutes: this.rotation.targetMinutes.map((entry) => entry.playerVersionId === replacedPlayerVersionId
                ? { playerVersionId: inactivePlayerVersionId, minutes: entry.minutes }
                : entry),
            closingFive: this.rotation.closingFive.map((id) => id === replacedPlayerVersionId ? inactivePlayerVersionId : id),
        };
    ...
    rows(): Array<{
        ...
        return this.members.map((member) => {
            const minutes = this.rotation.targetMinutes.find((t) => t.playerVersionId === member.playerVersionId)
                ?.minutes ?? 0;
            return {
                member,
                minutes,
    ...
    eligibleForSlot(slotIndex: number): RotationMember[] {
        ...
        if (group === undefined)
            return [];
        return this.members.filter((member) => this.activeIds.has(member.playerVersionId) && canPlay(member.playable, group));
    }
    minutesFor(playerVersionId: string): number {
        return (this.rotation.targetMinutes.find((t) => t.playerVersionId === playerVersionId)?.minutes ?? 0);
    }
    validate(): string[] {
    ...
    setMinutes(playerVersionId: string, minutes: number): string[] {
        ...
        this.rotation = {
            ...this.rotation,
            targetMinutes: this.rotation.targetMinutes.map((entry) => entry.playerVersionId === playerVersionId ? { playerVersionId, minutes: clamped } : entry),
        };
    ...
    rebalanceMinutes(playerVersionId: string, minutes: number): RebalanceResult {
        ...
        const byId = new Map(this.rotation.targetMinutes.map((entry) => [entry.playerVersionId, entry]));
        const set = (id: string, minutesValue: number) => byId.set(id, { playerVersionId: id, minutes: minutesValue });
    ...
    assignStarter(slotIndex: number, playerVersionId: string): string[] {
        ...
        if (benchIndex !== -1) {
            ...
            if (current !== undefined)
                benchOrder[benchIndex] = current;
        }
        else {
            const otherSlot = starters.indexOf(playerVersionId);
            if (otherSlot === -1)
                return ['that player is not a starter or bench member'];
            starters[slotIndex] = playerVersionId;
            if (current !== undefined)
                starters[otherSlot] = current;
        }
...
export function createRotationEditor(rotation: SeasonRotation, roster: Array<{
    playerVersionId: string;
    displayName: string;
    playable: readonly Position[];
}>): RotationEditor {
...
export function rotationEditorNeedsPositionRefresh(editor: RotationEditor, rosterPlayerVersionIds: readonly string[], playableOf: (playerVersionId: string) => readonly Position[]): boolean {
