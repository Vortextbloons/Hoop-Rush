...
export function getSeasonRunRepository(schedule?: SeasonSchedule): Promise<SeasonRunRepository & SeasonPostseasonRepository> {
    if (schedule) {
...
export function getSeasonBlockRunner(): Promise<SeasonBlockRunner> {
    ...
    if (typeof window !== 'undefined' && window.__HOOP_RUSH_E2E_FAKE_RUNNER__) {
        return import('./fake-season-block-runner').then((module) => module.createFakeSeasonBlockRunner());
    }
...
export function getSeasonPostseasonRunner(): Promise<SeasonPostseasonRunner> {
    ...
    if (typeof window !== 'undefined' && window.__HOOP_RUSH_E2E_FAKE_POSTSEASON_RUNNER__) {
        return import('./fake-season-postseason-runner').then((module) => module.createFakeSeasonPostseasonRunner());
    }
    if (postseasonRunnerPromise === null) {
        postseasonRunnerPromise = import('$lib/season/season-postseason-runner').then((module) => module.getSeasonPostseasonRunner());
