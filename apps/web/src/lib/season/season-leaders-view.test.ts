...
function player(playerVersionId: string, franchiseId: string, gamesPlayed: number, overrides: Partial<SeasonPlayerAggregate> = {}): SeasonPlayerAggregate {
    return {
        playerVersionId,
        franchiseId,
        gamesPlayed,
...
describe('engineOrderLeaderTables', () => {
    ...
    it('caps each category at depth 5', () => {
        const players = Array.from({ length: 9 }, (_, index) => player(`v-${String(index)}`, 'lakers', 10, { points: 200 - index }));
        const tables = engineOrderLeaderTables(players, teams);
