...
import { buildVersionFaceIndex, catalogCandidateOfFreeAgency, freeAgencyVersionTuples, mergeFreeAgencyFaces, versionTupleOfRosterEntry, } from './season-branding';
...
describe('freeAgencyVersionTuples', () => {
    ...
    it('falls back to playerVersionId when catalogRef index is stale', () => {
        ...
        if (candidate === undefined)
            throw new Error('fixture has no candidate');
        expect(catalogCandidateOfFreeAgency(catalog as never, candidate as never)?.playerVersionId).toBe('pv-fa-1');
    });
});
describe('mergeFreeAgencyFaces', () => {
    ...
    it('adds market faces without dropping roster faces', () => {
        ...
        const merged = mergeFreeAgencyFaces(playersIndex, catalog as never, freeAgency as never, rosterFaces);
