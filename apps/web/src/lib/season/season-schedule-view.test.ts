...
import { SEASON_GAME_SUMMARY_VERSION, seasonGameSchema, seasonGameSummarySchema, type SeasonCompactPlayerLine, type SeasonGame, type SeasonGameSummary, } from '@hoop-rush/data-contracts';
import { playedScheduleCount, scheduleBlockGroups, scheduleBlockRows, type ScheduleBlockRow, } from './season-schedule-view';
const HUMAN = 'lakers';
const OPPONENT = 'celtics';
function game(gameId: string, round: number, homeFranchiseId: string, awayFranchiseId: string, status: SeasonGame['status'] = 'scheduled', score?: {
    home: number;
    away: number;
}, forfeitLoserFranchiseId: string | null = null): SeasonGame {
...
function summary(gameId: string, round: number, homeFranchiseId: string, awayFranchiseId: string, status: 'final' | 'forfeit', homeScore: number, awayScore: number, forfeitLoserFranchiseId: string | null = null): SeasonGameSummary {
