...
type FakeCommitOutput = Pick<EngineCommitOutput, 'checkpointState' | 'stateRevision' | 'stateDigest' | 'window' | 'freeAgency'>;
...
function freeAgencyEvidenceOf(input: {
    ...
    const parsedHuman = input.humanFranchiseId === null ? null : franchiseIdSchema.parse(input.humanFranchiseId);
...
export class FakeSeasonBlockRunner implements SeasonBlockRunner {
    ...
    private committedSnapshot(input: SeasonBlockStartInput, checkpoint: SeasonCandidateCheckpoint, commandId: import('@hoop-rush/data-contracts').CommandId, committed: {
        checkpointState: SeasonCheckpointState;
        stateRevision: number;
        stateDigest: string;
        window: SeasonWindowOpenResult | null;
    ...
    private async buildCheckpoint(input: SeasonBlockStartInput, summaries: SeasonGameSummary[], health: SeasonHealthState, effectsOverride: SeasonEffectsState | null): Promise<SeasonCandidateCheckpoint> {
        ...
        const recap: SeasonBlockRecap = {
            ...
            influenceBalance: {
                humanBalance: input.humanFranchiseId === null
                    ? 0
    ...
    private async commitCheckpoint(input: SeasonBlockStartInput, commandId: import('@hoop-rush/data-contracts').CommandId, checkpoint: SeasonCandidateCheckpoint, m25: FakeM25CommitInput): Promise<void> {
        const repo = await getSeasonRunRepository();
    ...
    private committedFacts(input: SeasonBlockStartInput, checkpoint: EngineSimulateBlockOutput, commandId: import('@hoop-rush/data-contracts').CommandId, freeAgencyAssets?: {
        freeAgencyIndex?: SeasonFreeAgencyIndex;
        freeAgencyTargets?: SeasonRosterTargets;
    }): FakeCommitOutput {
