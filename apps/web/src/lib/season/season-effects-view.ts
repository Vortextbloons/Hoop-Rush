import { fatigueBandOf, pairChemistryBasisPoints, unitChemistryBasisPoints, } from '@hoop-rush/engine';
import type { SeasonEffectsState, SeasonMechanismEvidence, SeasonRetainedGameDetail, } from '@hoop-rush/data-contracts';
...
export function fatiguePercent(fatigueBasisPoints: number): number {
    return Math.round((fatigueBasisPoints / 10000) * 100);
}
...
export function activeLineupChemistryBp(state: SeasonEffectsState, unit: readonly string[]): number {
    if (unit.length !== 5)
        return 0;
    try {
...
export function strongestAndWeakestPairs(state: SeasonEffectsState, rosterVersions: readonly string[]): {
    strongest: Array<{
        a: string;
        b: string;
        shared: number;
...
export function projectedFatigueBand(currentFatigueBp: number, minutesPerGame: number, staminaRating: number, games = 10): FatigueBand {
    let fatigue = currentFatigueBp;
    for (let i = 0; i < games; i += 1) {
        const seconds = minutesPerGame * 60;
        const base = (seconds * 40 * (110 - staminaRating)) / 10000;
        fatigue += base;
        const factor = (4500 - 20 * staminaRating) / 10000;
        fatigue = Math.max(0, Math.round(fatigue * factor));
    }
    return fatigueBand(Math.min(10000, fatigue));
}
...
export function aggregateMechanismEvidence(details: readonly SeasonRetainedGameDetail[]): BlockMechanismEvidenceRow[] {
    ...
    for (const detail of details) {
        for (const row of detail.mechanismEvidence ?? []) {
            ...
            const inputTotal = row.inputTotals.shooter +
    ...
    return [...rows.values()]
        ...
        deltaMin: row.deltaMin,
        deltaMax: row.deltaMax,
        avgInputFraction: row.opportunities > 0 ? row.inputTotal / row.opportunities / 1000000 : 0,
    }))
