import { SEASON_ENDING_MISSED_GAMES_SENTINEL, type SeasonCompactInjuryEvent, type SeasonGameSummary, type SeasonHealthState, type SeasonInjuryRecord, type SeasonInjurySeverity, type SeasonInjuryType, type SeasonRoster, } from '@hoop-rush/data-contracts';
export type InjuryStatus = 'active' | 'returned' | 'none';
export function activeInjuriesOf(health: SeasonHealthState, playerVersionId: string): SeasonInjuryRecord[] {
    return health.injuries.filter((record) => record.playerVersionId === playerVersionId &&
...
export function injuryStatusOf(health: SeasonHealthState, playerVersionId: string): InjuryStatus {
    if (activeInjuriesOf(health, playerVersionId).length > 0)
        return 'active';
...
export function recoveryEstimate(record: SeasonInjuryRecord, futureTeamGames: readonly {
    gameId: string;
    round: number;
}[]): RecoveryEstimate {
    const seasonEnding = record.seasonEnding || record.missedGamesRemaining >= SEASON_ENDING_MISSED_GAMES_SENTINEL;
...
export function availabilityStripRows(health: SeasonHealthState, roster: SeasonRoster, franchiseGames?: readonly {
    gameId: string;
    round: number;
}[], names?: ReadonlyMap<string, string>): AvailabilityStripRow[] {
    const gamesByFranchise = franchiseGames === undefined
        ? null
    ...
    for (const entry of roster.players) {
        ...
        const active = records.filter((record) => record.missedGamesRemaining > 0 && record.sameGameReturned !== true);
        const status: InjuryStatus = active.length > 0 ? 'active' : records.length > 0 ? 'returned' : 'none';
        ...
        if (activeRecord !== undefined) {
            ...
            const future = gamesByFranchise === null
                ? []
            ...
            if (estimate.seasonEnding) {
                ...
                if (round !== null)
                    returnRange = { min: round, max: round };
            }
        }
...
function occurrenceRoundOf(record: SeasonInjuryRecord, games: readonly {
    gameId: string;
    round: number;
}[] | undefined): number {
    if (games === undefined)
...
export function humanInjuryTimeline(health: SeasonHealthState, roster: SeasonRoster, franchiseId: string, summaries?: readonly SeasonGameSummary[]): InjuryTimelinePlayer[] {
    ...
    for (const entry of roster.players) {
        const records = health.injuries.filter((record) => record.playerVersionId === entry.playerVersionId && record.franchiseId === franchiseId);
        if (records.length === 0)
            continue;
