import type { SeasonGameSummary, SeasonPlayerAggregate, SeasonRoster, } from '@hoop-rush/data-contracts';
...
export type SeasonPlayerStatsSortKey = 'displayName' | 'gamesPlayed' | 'minutesPerGame' | 'pointsPerGame' | 'reboundsPerGame' | 'assistsPerGame' | 'stealsPerGame' | 'blocksPerGame' | 'turnoversPerGame' | 'minutes' | 'points' | 'rebounds' | 'assists' | 'steals' | 'blocks' | 'turnovers' | 'fouls' | 'fieldGoalPct' | 'threePointPct' | 'freeThrowPct';
...
export function humanSeasonPlayerStats(input: {
    ...
    const rows: SeasonPlayerStatsRow[] = roster.players.map((entry) => {
        ...
        return {
            ...
            fieldGoalPct: aggregate === null ? null : pct(aggregate.fieldGoalsMade, aggregate.fieldGoalsAttempted),
            threePointPct: aggregate === null
                ? null
                : pct(aggregate.threePointersMade, aggregate.threePointersAttempted),
            freeThrowPct: aggregate === null ? null : pct(aggregate.freeThrowsMade, aggregate.freeThrowsAttempted),
        };
    });
    ...
    for (const summary of input.summaries) {
        ...
        for (const lines of [summary.homePlayers, summary.awayPlayers]) {
            for (const line of lines) {
                if (line.playerVersionId !== input.playerVersionId)
                    continue;
                gamesPlayed += 1;
