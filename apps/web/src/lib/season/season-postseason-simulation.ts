import { SEASON_RUN_SCHEMA_VERSION, canonicalJson, seasonDigestHex, seasonPostseasonWorkerStartRequestSchema, type CommandId, type EraSimulationProfile, type FranchiseId, type Id, type SeasonAdvancePostseasonCommand, type SeasonAdvancePostseasonRejection, type SeasonDraftCatalog, type SeasonEffectsState, type SeasonGameSummary, type SeasonPostseasonSummary, type SeasonRun, type SeasonRunStage, type PostseasonGameId, type Seed, type SeasonPostseasonScoreline, type SeasonPostseasonWorkerStartRequest, } from '@hoop-rush/data-contracts';
import { handleSeasonRunCommand, SeasonPostseasonInvariantError, type SeasonPostseasonGameResolver, } from '@hoop-rush/engine';
...
export function simulateSeasonPostseasonCommand(request: SeasonPostseasonSimulationRequest): SeasonPostseasonSimulationOutcome {
...
export function postseasonPostCommandEffects(run: SeasonRun, prior: SeasonEffectsState): SeasonEffectsState {
    const withEffects = run as SeasonRun & {
        effects?: SeasonEffectsState;
    };
    return withEffects.effects ?? prior;
}
export function seasonPostseasonCommitResultDigest(commandId: string, relatedGameIds: readonly string[], summaries: readonly SeasonPostseasonSummary[]): string {
...
export function seasonPostseasonWireRequestOf(request: Omit<SeasonPostseasonSimulationRequest, 'catalog' | 'profile'> & {
    requestId: string;
    rootSeed: Seed;
    catalogUrl: string;
    catalogHash: string;
