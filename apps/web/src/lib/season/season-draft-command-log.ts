import type { SeasonDraftCommand, SeasonDraftState, SeasonPublicCommandEnvelope, } from '@hoop-rush/data-contracts';
import { seasonDigestHex, seasonNamespaceSeed, seasonDraftCommandSchema, seasonDraftCommandPayloadSchema, } from '@hoop-rush/data-contracts';
...
export function envelopeToDraftCommand(env: SeasonPublicCommandEnvelope, state: SeasonDraftState | null): SeasonDraftCommand | null {
    ...
    switch (kind) {
        ...
        case 'room-draft-pick': {
            if (!('participantId' in raw) || !('playerVersionId' in raw))
                return null;
            ...
            if (!parsed.success)
                return null;
            if (parsed.data.kind !== 'select-draft-player')
                return null;
            return {
                commandId: env.commandId,
                expectedRevision: state?.revision ?? 0,
