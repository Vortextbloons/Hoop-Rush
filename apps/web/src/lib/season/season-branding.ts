import { franchiseAbbreviation, resolveEraTeamIdentity, type HoopRushManifest, type PlayersIndexAltIds, type PlayersIndexEntry, type SeasonDraftCatalog, type SeasonDraftCandidate, type SeasonFreeAgencyCandidate, type SeasonFreeAgencyState, } from '@hoop-rush/data-contracts';
...
export function versionTupleOfRosterEntry(entry: SeasonVersionTuple, catalogCandidate?: Pick<SeasonDraftCandidate, 'franchiseId' | 'eraId' | 'seasonKey'> | null): SeasonVersionTuple {
    return {
...
export function catalogCandidateOfFreeAgency(catalog: SeasonDraftCatalog, candidate: SeasonFreeAgencyCandidate): SeasonDraftCandidate | null {
    const { catalogRef } = candidate;
    if (catalog.catalogVersion === catalogRef.catalogVersion &&
    ...
    return (catalog.candidates.find((entry) => entry.playerVersionId === candidate.playerVersionId) ?? null);
}
export function freeAgencyVersionTuples(freeAgency: SeasonFreeAgencyState | null | undefined, catalog: SeasonDraftCatalog | null): SeasonVersionTuple[] {
    if (freeAgency === null || freeAgency === undefined || catalog === null)
        return [];
...
export function mergeFreeAgencyFaces(playersIndex: readonly PlayersIndexEntry[] | null, catalog: SeasonDraftCatalog | null, freeAgency: SeasonFreeAgencyState | null | undefined, rosterFaces: ReadonlyMap<string, SeasonFaceRef>): Map<string, SeasonFaceRef> {
...
export function buildVersionFaceIndex(playersIndex: readonly PlayersIndexEntry[], rosterEntries: readonly SeasonVersionTuple[]): Map<string, SeasonFaceRef> {
    ...
    for (const entry of rosterEntries) {
        const match = byTuple.get(tupleKey(entry.playerId, entry.franchiseId, entry.eraId, entry.seasonKey));
...
export function franchiseIdentityOf(manifest: HoopRushManifest, franchiseId: string): SeasonFranchiseIdentity | null {
...
export function eraIdentityOf(manifest: HoopRushManifest, franchiseId: string, eraId: string): {
...
export function initialsOf(displayName: string): string {
    ...
    if (parts.length === 0)
        return '?';
    const firstChar = (part: string | undefined): string => part === undefined || part.length === 0 ? '' : (part[0] ?? '');
    if (parts.length === 1) {
        const only = parts[0];
