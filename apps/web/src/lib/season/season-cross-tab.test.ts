...
import { createSeasonRunChannel, SEASON_RUN_CHANNEL_NAME, type SeasonRunAnnouncement, type SeasonRunMutation, } from './season-cross-tab';
describe('Season Run cross-tab channel', () => {
    ...
    it('drops invalid payloads and unknown shapes', async () => {
        ...
        probeChannel.announce({
            kind: 'commit',
            runId: idSchema.parse('probe'),
            revision: 1,
            committedAt: 2,
        });
