import type { SeasonDraftCatalog, SeasonRun, SeasonTradeOffer, SeasonTradeState, SeasonTradeWindowState, } from '@hoop-rush/data-contracts';
...
function roleGroupsLabel(roles: readonly string[]): string {
    ...
    if (unique.length === 0)
        return 'player';
    if (unique.length === 1)
        return slotGroupLabel(unique[0] ?? 'player');
...
function playerPositionLabel(player: TradePlayerViewModel, roleGroup: string): string {
    if (player.playable.length > 0)
        return formatPositions(player.playable);
...
function roleFitInsightOf(offer: SeasonTradeOffer, outgoingPlayers: TradePlayerViewModel[], incomingPlayers: TradePlayerViewModel[]): TradeOfferInsight {
...
function rosterNeedInsightOf(offer: SeasonTradeOffer, incomingPlayers: TradePlayerViewModel[]): TradeOfferInsight {
...
function rotationInsightOf(outgoingPlayers: TradePlayerViewModel[], incomingPlayers: TradePlayerViewModel[]): TradeOfferInsight {
...
export function tradeOfferViewModel(offer: SeasonTradeOffer, run: SeasonRun, catalog: SeasonDraftCatalog | null, franchiseName: (franchiseId: string) => string): TradeOfferViewModel {
    ...
    const playableOf = (playerVersionId: string): readonly string[] => {
        if (catalog === null)
            return [];
        return (catalog.candidates.find((c) => c.playerVersionId === playerVersionId)?.positions.playable ??
            []);
    };
    const rotation = (run.rotations as SeasonRun['rotations'] | undefined)?.find((entry) => entry.franchiseId === offer.toFranchiseId);
    ...
    const viewPlayer = (playerVersionId: string, health: SeasonTradeOffer['outgoingHealth'][number], options: {
        rotationMinutes?: number | null;
        projectedMinutes?: number | null;
    ...
    const statusLabel = offer.status === 'open'
        ? 'Open'
        : offer.status === 'accepted'
            ...
            : offer.status === 'declined'
                ? 'Declined'
                : 'Expired';
    const healthEntryOf = (list: SeasonTradeOffer['outgoingHealth'], index: number): SeasonTradeOffer['outgoingHealth'][number] => list[index] ?? { available: true, activeInjuryIds: [] };
    const outgoingPlayers = offer.outgoingPlayerVersionIds.map((id, index) => viewPlayer(id, healthEntryOf(offer.outgoingHealth, index), {
        rotationMinutes: outgoingMinutes[index] ?? null,
    }));
    const incomingPlayers = offer.incomingPlayerVersionIds.map((id, index) => viewPlayer(id, healthEntryOf(offer.incomingHealth, index), {
...
export function humanTradeOffersOf(trade: SeasonTradeState | null, humanFranchiseId: string | null): SeasonTradeOffer[] {
    const window = openWindowOf(trade);
    if (window === null || humanFranchiseId === null)
        return [];
