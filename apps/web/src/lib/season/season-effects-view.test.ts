...
describe('M2.4 season effects view', () => {
  it('maps fatigue basis points to the frozen bands', () => {
    ...
    expect(fatigueBand(10_000)).toBe('heavy');
  ...
  it('aggregates retained-detail mechanism evidence per mechanism and side', () => {
    const base = {
      ...
      side: 'home' as const,
      opportunities: 3,
      inputTotals: { shooter: 1_500_000, handler: 0, defenseMean: 0, unitChemistry: 0 },
      deltaTotals: -37_500,
      deltaMin: -12_500,
      deltaMax: -12_500,
    };
        ...
        mechanismEvidence: [
          {
            ...
            side: 'home',
            opportunities: 1,
            inputTotals: { shooter: 500_000, handler: 0, defenseMean: 0, unitChemistry: 0 },
            deltaTotals: -12_500,
            deltaMin: -12_500,
            deltaMax: -12_500,
          },
          {
            ...
            side: 'home',
            opportunities: 2,
            inputTotals: { shooter: 0, handler: 0, defenseMean: 0, unitChemistry: 1_200_000 },
            deltaTotals: 42_000,
            deltaMin: 21_000,
            deltaMax: 21_000,
          },
    ...
    expect(shooter?.deltaTotals).toBe(-50_000);
    expect(shooter?.deltaMin).toBe(-12_500);
    expect(shooter?.deltaMax).toBe(-12_500);
    ...
    expect(deltaToPp(25_000)).toBe(2.5);
    expect(deltaToPp(-12_500)).toBe(-1.25);
  });
  ...
  it('projects fatigue bands deterministically and monotonically with minutes', () => {
    ...
    expect(projectedFatigueBand(10_000, 48, 45, 10)).not.toBe('heavy');
  });
});
