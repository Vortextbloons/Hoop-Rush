...
import { normalizeTeamProjection, rawSeasonTeamRatings, seasonLeagueTeamProjections, seasonTeamDetail, type SeasonTeamDetail, } from './season-team-detail-view';
...
function zeroLine(roster: SeasonRoster, index: number, seconds = 720, points = 10) {
    ...
    if (entry === undefined)
        throw new Error('roster index out of range');
    return {
...
function summary(home: SeasonRoster, away: SeasonRoster): SeasonGameSummary {
    ...
    return {
        ...
        homePlayers: home.players.map((_, index) => zeroLine(home, index, 720, index % 2 === 0 ? 12 : 4)),
...
function detailOf(input: Parameters<typeof seasonTeamDetail>[0]): SeasonTeamDetail {
    ...
    if (detail === null)
        throw new Error('expected a team detail');
    return detail;
}
...
describe('seasonLeagueTeamProjections', () => {
    it('maps the weakest and strongest franchises to the display floor and ceiling', () => {
        const ratings = new Map<string, {
            overallRating: number;
            offenseRating: number;
            defenseRating: number;
        }>();
        for (const roster of run.rosters) {
            const tier = roster.franchiseId === 'lakers' ? 'high' : roster.franchiseId === 'celtics' ? 'low' : 'mid';
    ...
    it('weights heavy-minute stars more than equal-minute benches on the same roster', () => {
        ...
        const starHeavy = normalizeTeamProjection(rawSeasonTeamRatings({
            roster,
            rotation: {
                ...input.rotation,
            ...
            summaryRatingsOf,
        }) ?? zeroRawProjection(), baselines);
        const balanced = normalizeTeamProjection(rawSeasonTeamRatings({
            roster,
            rotation: {
                ...input.rotation,
