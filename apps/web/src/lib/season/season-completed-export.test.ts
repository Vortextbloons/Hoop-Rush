...
function completedSeason(): SeasonCompletedSeason {
  ...
  (season as unknown as { almanac: { commandLogDigest: string } }).almanac.commandLogDigest =
    seasonCommandLogDigest(
      (season as unknown as { commandLog: { entries: unknown[] } }).commandLog.entries as never,
    );
  return season;
}
