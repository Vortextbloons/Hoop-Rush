...
import { buildCompletedSeasonRunReplayExport, seasonReplayAssetHashesOf, } from './season-completed-export';
import type { HoopRushManifest, SeasonCompletedSeason } from '@hoop-rush/data-contracts';
import { SEASON_EMPTY_COMMAND_LOG_DIGEST, seasonCommandLogDigest } from '@hoop-rush/data-contracts';
function manifest(): HoopRushManifest {
    return {
        ...
        pools: [],
        availability: [],
        eraSimulationProfiles: [{ eraId: '1990s', url: 'era-sim/1990s.json', contentHash: 'a'.repeat(64) }],
        season: {
...
function completedSeason(): SeasonCompletedSeason {
    return {
        run: {
            ...
            objectives: null,
            campaign: null,
            effects: { schemaVersion: 1, staminaVersion: 'season-stamina-v2', chemistryVersion: 'season-chemistry-v2', players: [], pairs: [] },
            freeAgency: null,
    ...
    season.almanac.commandLogDigest = seasonCommandLogDigest(season.commandLog.entries);
    return season;
}
