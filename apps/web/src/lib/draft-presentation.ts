...
export function sortDraftRows(rows: PlayersIndexEntry[], presentation: DraftPresentation): PlayersIndexEntry[] {
    const compare = presentation === 'ball-knowledge' ? compareNameAsc : compareOverallDesc;
    return [...rows].sort(compare);
}
export function ratingBadges(player: PlayersIndexEntry, presentation: DraftPresentation): RatingBadge[] {
