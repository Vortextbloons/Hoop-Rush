import { fixedFiveRoomCodeSchema, fixedFiveRoomMembershipSchema, idSchema, type FixedFiveRoomMembership, type Id, } from '@hoop-rush/data-contracts';
...
export function friendlyFixedFiveJoinError(error: unknown): string {
    ...
    if (message.includes('code-expired'))
        return 'That code expired. Ask the host for a fresh code.';
    if (message.includes('room-full'))
    ...
    if (message.includes('stale-revision'))
        return 'The room changed. Syncing and retrying once.';
...
export function loadFixedFiveMembership(roomId: string): StoredFixedFiveMembership | null {
    try {
        ...
        if (!result.success)
            return null;
        if (result.data.roomId !== parsedRoomId.data)
            return null;
        return result.data;
    }
    catch {
