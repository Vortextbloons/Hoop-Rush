<script lang="ts">import type { FixedFiveParticipantId, FixedFiveRoomMode, PlayersIndexEntry, PlayerId, SlotIndex, } from '@hoop-rush/data-contracts';
...
import { isFixedFiveDraftTurn, type DraftReplay, type FixedFiveAssets, } from '$lib/fixed-five-room-state';
let { mode, selfId, replay, assets, presentation, rollAxis = 'both', rollNonce = 0, disabled = false, deadlineText = null, lastAutopick = null, error = null, onPick, onReroll, onRemove, onLock, }: {
...
const catalogPairs = $derived(assets.catalog.map((e) => ({ franchiseId: e.franchiseId, eraId: e.eraId })));
...
const sandboxDuelView = $derived.by((): SandboxDuelView | null => {
    if (mode !== 'duel' || replay.mode !== 'sandbox-duel')
        return null;
    ...
    return {
        ...
        turnText: complete
            ...
            : turn
                ...
                : 'Opponent is picking…',
        myClaimed: new Set(replay.state.picks.filter((p) => p.participantId === selfId).map((p) => p.playerId)),
    };
});
const sandboxDuelRows = $derived.by((): PlayersIndexEntry[] => {
    if (!sandboxDuelView || sandboxDuelView.complete)
        return [];
    return sortDraftRows(assets.index.players.filter((p) => !sandboxDuelView.myClaimed.has(p.playerId)), presentation);
});
const sandboxDuelCountLabel = $derived(`${sandboxDuelRows.length} players · ${poolSortLabel(presentation)}`);
const rollRows = $derived.by((): PlayersIndexEntry[] => {
    if (!rollView || rollView.complete)
        return [];
    const rows = classicPoolRows(assets.index, { franchiseId: rollView.franchiseId, eraId: rollView.eraId }, presentation);
    if (replay.mode === 'duel') {
...
const myPicks = $derived.by((): Array<{
    playerId: PlayerId;
    slotIndex: SlotIndex;
}> => {
    if (replay.mode === 'duel') {
...
const sandboxLocked = $derived(replay.mode === 'sandbox-shared-82' ? (selfId === 'p1' ? replay.p1 : replay.p2).locked : false);
...
const rollFranchise = $derived(rollView && !rollView.complete
    ? (rollManifest.modernFranchiseSlots.find((e) => e.franchiseId === rollView.franchiseId) ??
        null)
    : null);
const rollEra = $derived(rollView && !rollView.complete
    ? (rollManifest.eras.find((e) => e.eraId === rollView.eraId) ?? null)
    : null);
const rollIdentity = $derived(rollView && !rollView.complete
    ? resolveEraTeamIdentity(rollManifest, rollView.franchiseId, rollView.eraId)
    : null);
const rollFranchiseAbbreviation = $derived(rollView && !rollView.complete
    ? (rollIdentity?.abbreviationLabel ??
        (rollFranchise ? franchiseAbbreviation(rollFranchise.franchiseId) : rollView.franchiseId))
    : '');
const rollFranchiseDisplayName = $derived(rollView && !rollView.complete
    ? (rollIdentity?.displayLabel ?? rollFranchise?.displayName ?? null)
    : null);
const rollEraLabel = $derived(rollView && !rollView.complete ? (rollEra?.label ?? rollView.eraId) : '');
const poolHeading = $derived(rollView && !rollView.complete && rollFranchise && rollEra && rollIdentity
...
$effect(() => {
    ...
    let cancelled = false;
    resolvePlayerRefs(refs, manifest).then((players) => {
        if (!cancelled)
            resolvedLineupPlayers = players;
    }, () => {
        if (!cancelled)
            resolvedLineupPlayers = [];
    });
...
function placeWithDisplacement(player: PlayersIndexEntry, slotIndex: number) {
    ...
    if (incumbent && incumbent.playerId !== player.playerId && allowDisplacement) {
        moveTarget = displacementTargetFor(slots, incumbent, slotIndex, subjectSlot) as SlotIndex | null;
    }
    closePicker();
