<script lang="ts">
  ...
  /**
   * Challenge result (spec/08): final record and 82-0 outcome, the full game
   * strip, aggregate shooting, turnover, rebound, free-throw, and possession
   * facts, the user's five-player season table, and best single-game
   * performance. Shared by Sandbox and Classic; the owning route keeps the
   * page header, data loading, and the mode-specific Retry / Edit team
   * implementations.
   */
...
>
  ...
  <div class="mt-5 flex flex-wrap items-center gap-2">
    ...
    {#if editHref && editLabel !== null}
      <a
        href={resolve(editHref as RouteId)}
    ...
    {:else if onEditTeam && editLabel !== null}
      <button
        type="button"
        onclick={onEditTeam}
        disabled={retrying}
        class="inline-flex items-center gap-2 rounded-lg border border-border bg-surface-1 px-4 py-2 text-sm font-semibold transition-colors hover:border-line-strong hover:text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50"
      >
        <Pencil class="h-4 w-4" />
        {editLabel}
      </button>
