...
function reward(type: SeasonCampaignReward['type'], amount: number, id: string): SeasonCampaignReward {
  return { rewardId: id as SeasonCampaignReward['rewardId'], type, amount };
}

function opportunity(overrides: Partial<SeasonCampaignOpportunity> = {}): SeasonCampaignOpportunity {
  const base: SeasonCampaignOpportunity = {
    ...
    family: 'results',
    prerequisiteId: null,
    target: { kind: 'block-wins', comparisonOperator: 'gte', threshold: 6, window: 'block' } as unknown as SeasonCampaignOpportunity['target'],
    breakthrough: { kind: 'block-wins', comparisonOperator: 'gte', threshold: 8, window: 'block' } as unknown as SeasonCampaignOpportunity['breakthrough'],
  ...
  return base as SeasonCampaignOpportunity;
}
...
function campaignState(overrides: Partial<SeasonCampaignState> = {}): SeasonCampaignState {
  return {
    ...
    rewardEntitlements: { influenceEarned: 0, inquiryCredits: 0, informationBenefits: 0, followUpUnlocks: [] },
    appliedRewardIds: [],
    ...overrides,
  };
}
...
function runWithCampaign(campaign: SeasonCampaignState, completedRounds = 0): SeasonRun {
  return {
    ...
    league: { teams: [{ franchiseId: 'lakers', control: 'human', conference: 'west', division: 'pacific' }] } as unknown as SeasonRun['league'],
    rosters: [{ franchiseId: 'lakers', players: [{ playerVersionId: 'pv-aaa', playerId: 'p1', franchiseId: 'lakers', eraId: '1990s', seasonKey: '1995-96', displayName: 'Player A' }] } as unknown as SeasonRun['rosters'][number]],
    standings: { rows: [] } as unknown as SeasonRun['standings'],
    health: { schemaVersion: 1, healthVersion: 'season-health-v2', injuries: [] } as unknown as SeasonRun['health'],
...
describe('season-presentation campaign helpers', () => {
  it('formats campaign conditions without leaking thresholds as odds', () => {
    expect(formatCampaignCondition({ kind: 'block-wins', comparisonOperator: 'gte', threshold: 6, window: 'block' } as unknown as SeasonCampaignOpportunity['target'])).toContain('6');
  ...
  it('campaignTimelineViewModel shows prior outcome + 2 cards, block 8 no opportunity', () => {
    const state = campaignState({
      ...
      offers: { 0: [opportunity({ opportunityId: 'copp-aaaaaaaa', blockIndex: 0 }), opportunity({ opportunityId: 'copp-bbbbbbbb', blockIndex: 0, templateId: 'ctpl-dddddddd' })] },
      evaluations: [{ opportunityId: 'copp-aaaaaaaa', blockIndex: 0, outcome: 'completed', facts: { wins: 7 }, appliedRewardIds: ['rew-aaaaaaaaaaaaaaaa'], explanation: 'Won 7 games' }],
    ...
    expect(vm?.currentOffers.length).toBe(0); // block 1 has no offers yet
...
describe('EvolutionPicker', () => {
  it('renders double-down plus up to 2 evidence-backed options', async () => {
    const onSelect = vi.fn();
    const offers = [
      { offerId: 'evo-aaaaaaaa', kind: 'double-down' as const, evidence: 'Won 7 of last 10 with current identity', resultingIdentity: 'win-now' as const, resultingFocus: null },
      { offerId: 'evo-bbbbbbbb', kind: 'adapt' as const, evidence: 'Bench scored 320 pts — depth focus credible', resultingIdentity: 'win-now' as const, resultingFocus: 'depth' as const },
      { offerId: 'evo-cccccccc', kind: 'pivot' as const, evidence: 'Roster now favors development', resultingIdentity: 'player-development' as const, resultingFocus: null },
    ];
...
describe('CampaignPanel', () => {
  it('shows prior outcome with evidence, reward id, branch state, and 2 cards', async () => {
    const state = campaignState({
      ...
      offers: {
        1: [
          opportunity({ opportunityId: 'copp-11111111', blockIndex: 1, family: 'style', identity: 'team-identity', target: { kind: 'three-point-volume', comparisonOperator: 'gte', threshold: 90, window: 'block' } as unknown as SeasonCampaignOpportunity['target'], completedReward: reward('trade-board-information', 1, 'rew-11111111'), feasibilityFacts: { scheduleGamesInBlock: ['s000011'] } }),
          opportunity({ opportunityId: 'copp-22222222', blockIndex: 1, family: 'results', identity: 'win-now', target: { kind: 'block-wins', comparisonOperator: 'gte', threshold: 6, window: 'block' } as unknown as SeasonCampaignOpportunity['target'], completedReward: reward('influence', 1, 'rew-22222222'), feasibilityFacts: { standingRows: 30 } }),
        ],
      },
      evaluations: [{ opportunityId: 'copp-aaaaaaaa', blockIndex: 0, outcome: 'breakthrough', facts: { wins: 8, threePointersMade: 95 }, appliedRewardIds: ['rew-aaaaaaaaaaaaaaaa', 'rew-bbbbbbbbbbbbbbbb'], explanation: 'Breakthrough: 8 wins and hot shooting' }],
      branchState: { 'cbr-bbbbbbbb': 'completed', 'cbr-aaaaaaaa': 'open' },
      rewardEntitlements: { influenceEarned: 2, inquiryCredits: 1, informationBenefits: 1, followUpUnlocks: ['cbr-bbbbbbbb'] },
    ...
    render(CampaignPanel, { props: { run, nextBlockIndex: 1, onSelectIdentity, onSelectOpportunity, onEvolve } });
  ...
  it('fresh run leads with SelectGmIdentity, explaining no ratings change', () => {
    ...
    render(CampaignPanel, { props: { run, nextBlockIndex: 0, onSelectIdentity: vi.fn(), onSelectOpportunity: vi.fn(), onEvolve: vi.fn() } });
    expect(screen.getByText('Choose your GM identity')).toBeTruthy();
    expect(document.body.textContent).toContain('never');
    expect(document.body.textContent).toContain('ratings');
  });
  it('after block 4 shows Evolve before block-5 opportunities', () => {
    const state = campaignState({
      ...
      offers: {},
      evolutionOffers: [
        { offerId: 'evo-aaaaaaaa', kind: 'double-down', evidence: 'Evidence for double-down', resultingIdentity: 'win-now', resultingFocus: null },
        { offerId: 'evo-bbbbbbbb', kind: 'adapt', evidence: 'Evidence for adapt', resultingIdentity: 'win-now', resultingFocus: 'depth' },
      ],
      evaluations: [{ opportunityId: 'copp-1', blockIndex: 4, outcome: 'completed', facts: {}, appliedRewardIds: ['rew-1'], explanation: 'Completed' }],
    });
    const run = runWithCampaign(state, 50); // 5 blocks completed (rounds 50)
    render(CampaignPanel, { props: { run, nextBlockIndex: 5, onSelectIdentity: vi.fn(), onSelectOpportunity: vi.fn(), onEvolve: vi.fn() } });
    expect(screen.getByText('Evolve your campaign')).toBeTruthy();
  ...
  it('block 8 shows no opportunity', () => {
    ...
    render(CampaignPanel, { props: { run, nextBlockIndex: 8, onSelectIdentity: vi.fn(), onSelectOpportunity: vi.fn(), onEvolve: vi.fn() } });
...
describe('PackageBuilder', () => {
  it('enforces 1-2/1-2, Influence one side never both, never Influence-only, shows consequence facts', async () => {
    ...
    // before submission consequence facts
    ...
    // try Influence both sides should not exist as option — there is no "both" as dual sender
    expect(screen.queryByText('Both sides send')).toBeNull();
    // select Influence from you
    ...
    expect(onSubmit).toHaveBeenCalledWith({ outgoing: ['pv-you-1'], incoming: ['pv-them-1'], influenceAmount: 1, influenceFromSender: 'lakers' });
  });
  it('shows availability risk and illegal roster', async () => {
    const yourPlayers = [{ playerVersionId: 'pv-y1', displayName: 'Y1', playable: ['PG'], available: true }];
    const theirPlayers = [{ playerVersionId: 'pv-t1', displayName: 'T1', playable: ['C'], available: false }];
    render(PackageBuilder, {
      props: {
        yourPlayers,
        theirPlayers,
...
describe('NegotiationTranscript', () => {
  it('shows 3 exchanges, categorical feedback, duplicate no increment note, walk-away no penalty without moving focus', async () => {
    const negotiation: SeasonTradeNegotiation = {
      ...
      exchangeCount: 2,
      exchanges: [
        { exchangeIndex: 1, kind: 'human-proposal', proposalId: 'prop-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', proposalFingerprint: 'a|b', responseCause: null, atStateRevision: 1 },
        { exchangeIndex: 2, kind: 'ai-counter', proposalId: null, proposalFingerprint: null, responseCause: 'close-needs-more-value', atStateRevision: 2 },
      ],
    ...
    render(NegotiationTranscript, { props: { negotiation, inquiryAllowance: 4, onWalkAway, busy: false } });
    ...
    // walked-away state shows no penalty — test with a walked-away negotiation
    const walked: SeasonTradeNegotiation = { ...negotiation, status: 'walked-away', finalReason: 'negotiations-closed' as const, activeProposalId: null };
    const { container: c2 } = render(NegotiationTranscript, { props: { negotiation: walked, inquiryAllowance: 4 } });
  ...
  it('announces accepted/rejected without moving focus', () => {
    const negotiation: SeasonTradeNegotiation = {
      ...
      status: 'accepted',
      exchangeCount: 1,
      exchanges: [{ exchangeIndex: 1, kind: 'human-proposal', proposalId: 'prop-bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', proposalFingerprint: 'a|b', responseCause: 'acceptable', atStateRevision: 1 }],
...
describe('ValueTrendCell', () => {
  it('shows rising/stable/falling without Overall', () => {
    render(ValueTrendCell, { props: { trend: { playerVersionId: 'pv-aaa', trend: 'rising', basis: 'Averaging 22 pts on high usage, durable minutes' }, basis: 'Averaging 22 pts on high usage, durable minutes', playerName: 'Player A' } });
...
describe('TradeBoardWorkspace distinctive design & a11y', () => {
  it('renders board 8, inquiry counters, package builder constraints, and history', () => {
    const run = { rosters: [{ franchiseId: 'lakers', players: [{ playerVersionId: 'pv-l-1', displayName: 'Laker One' }] }, { franchiseId: 'celtics', players: [{ playerVersionId: 'pv-c-1', displayName: 'Celtic One' }, { playerVersionId: 'pv-c-2', displayName: 'Celtic Two' }] }], trade: { windows: [{ windowIndex: 0, blockIndex: 2, status: 'open', offers: [], inquiryAllowance: 4, purchasedInquiryUsed: false, earnedInquiryUsed: false, activeInquiryId: null, negotiations: [], boardProfiles: [], valueTrends: [] }] }, influence: { balances: { lakers: 3, celtics: 2 } } } as unknown as SeasonRun;
    const profiles: SeasonTradeBoardTeamProfile[] = [
      { franchiseId: 'celtics', needs: ['shooting', 'depth'], priority: 'fit', listedPlayerIds: ['pv-c-1'], discussablePlayerIds: ['pv-c-1', 'pv-c-2'], protectedPlayerIds: ['pv-c-protected'], hardConstraints: ['Roster must stay 10-15', 'Protected: pv-c-protected'], rationale: 'Needs shooting based on roster gaps; priority fit', competitorInterest: { 'pv-c-1': 'possible' } },
    ];
    ...
    render(TradeBoardWorkspace, {
      props: {
        ...
        catalog: null,
        manifest,
        windowState: { windowIndex: 0, blockIndex: 2, status: 'open', offers: [], boardProfiles: profiles, inquiryAllowance: 4, purchasedInquiryUsed: false, earnedInquiryUsed: false, activeInquiryId: null, negotiations: [] } as unknown as import('@hoop-rush/data-contracts').SeasonTradeWindowState,
    ...
    // package builder not yet visible until team selected — shows Pick a team
    expect(screen.getByText('Pick a team')).toBeTruthy();
  });
  it('keyboard operable and responsive', async () => {
    const profiles: SeasonTradeBoardTeamProfile[] = [
      { franchiseId: 'celtics', needs: ['shooting'], priority: 'talent', listedPlayerIds: ['pv-c-1'], discussablePlayerIds: ['pv-c-1'], protectedPlayerIds: ['pv-p'], hardConstraints: ['Roster must stay 10-15'], rationale: 'Needs shooting' },
    ];
    const run = { rosters: [{ franchiseId: 'lakers', players: [] }, { franchiseId: 'celtics', players: [] }], trade: { windows: [] }, influence: { balances: { lakers: 2 } } } as unknown as SeasonRun;
    render(TradeBoardWorkspace, {
      props: {
        run,
        catalog: null,
        manifest: buildManifest(),
        windowState: { windowIndex: 0, blockIndex: 2, status: 'open', offers: [], boardProfiles: profiles } as unknown as any,
        boardProfiles: profiles,
        negotiations: [],
        valueTrends: [],
        humanFranchiseId: 'lakers',
  ...
  it('categorical feedback never shows Overall or ratio', () => {
    ...
    expect(packageConsequenceFacts({ fromRosterSize: 12, toRosterSize: 12, outgoingIds: ['a'], incomingIds: ['b'], outgoingAvailable: [true], incomingAvailable: [true], influenceAmount: 0, influenceFromSender: null, humanFranchiseId: 'lakers', toFranchiseId: 'celtics' }).influenceNote).not.toContain('Overall');
  });
});
