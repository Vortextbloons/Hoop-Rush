...
import type { SeasonCampaignOpportunity, SeasonCampaignReward, SeasonCampaignState, SeasonRun, SeasonTradeBoardTeamProfile, SeasonTradeNegotiation, } from '@hoop-rush/data-contracts';
...
describe('season-presentation campaign helpers', () => {
    it('formats campaign conditions without leaking thresholds as odds', () => {
        expect(formatCampaignCondition({
            kind: 'block-wins',
            comparisonOperator: 'gte',
            threshold: 6,
            window: 'block',
        } as unknown as SeasonCampaignOpportunity['target'])).toContain('6');
...
describe('GmIdentityPicker', () => {
    it('renders three identities, explains no ratings change, requires focus for team-identity', async () => {
        ...
        if (!(identitySubmit instanceof HTMLButtonElement))
...
describe('PackageBuilder', () => {
    it('enforces 1-2/1-2, Influence one side never both, never Influence-only, shows consequence facts', async () => {
        ...
        if (!(packageSubmit instanceof HTMLButtonElement))
            throw new Error('expected package submit');
...
describe('TradeBoardWorkspace distinctive design & a11y', () => {
    ...
    it('categorical feedback never shows Overall or ratio', () => {
        ...
        expect(packageConsequenceFacts({
            fromRosterSize: 12,
            toRosterSize: 12,
            outgoingIds: ['a'],
            incomingIds: ['b'],
        ...
        }).influenceNote).not.toContain('Overall');
    });
});
