...
import type { SeasonHealthState, SeasonInfluenceState, SeasonInjuryRecord, SeasonInvalidRosterInterruption, SeasonPendingBlockCandidate, SeasonRun, SeasonTradeOffer, } from '@hoop-rush/data-contracts';
...
import { commandIdSchema, eraIdSchema, franchiseIdSchema, idSchema, playerIdSchema, seasonGameIdSchema, seasonKeySchema, } from '@hoop-rush/data-contracts';
mockSvelteKitApp();
...
function tradeOffers() {
    return [
        tradeOfferViewModel(offer(), runWithRosters(), null, (id: string) => id === 'celtics' ? 'Boston Celtics' : id),
    ];
}
...
describe('HealthStrip', () => {
    it('renders statuses, return range, recurrence chip, and consequences', () => {
        ...
        expect(container.querySelector('[role="status"]')?.textContent).toContain('1 player out, 1 returning from injury');
    });
});
...
function tradePanelProps(offers: ReturnType<typeof tradeOffers>, handlers: {
...
describe('TradeOffersPanel', () => {
    ...
    it('renders the resolution of a resolved offer', () => {
        const resolved = tradeOfferViewModel(offer({ status: 'expired' }), runWithRosters(), null, (id: string) => id);
        render(TradeOffersPanel, {
    ...
    it('lists unavailable players, the three paths, and resumes', async () => {
        ...
        render(InterruptionPanel, {
            props: {
                ...
                pending: pending(),
                playerName: (id: string) => id === PLAYER_A ? 'Player 1' : id === PLAYER_B ? 'Player 2' : id,
                injuryPlayerName: (injuryId: string) => injuryId === 'inj-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' ? 'Player 1' : injuryId,
