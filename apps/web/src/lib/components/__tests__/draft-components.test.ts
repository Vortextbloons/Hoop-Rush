...
import { eraIdSchema, franchiseIdSchema, playerIdSchema, seedSchema, seasonKeySchema, } from '@hoop-rush/data-contracts';
...
mockSvelteKitApp();
function row(partial: Omit<Partial<PlayersIndexEntry>, 'playerId' | 'franchiseId' | 'eraId' | 'seasonKey'> & {
    playerId: string;
    franchiseId?: string;
    eraId?: string;
    seasonKey?: string;
}): PlayersIndexEntry {
    const { playerId, franchiseId = 'lakers', eraId = '1990s', seasonKey = '1996-97', ...rest } = partial;
    return {
...
function renderPoolBrowser(props: Partial<{
...
describe('DraftPoolBrowser parity', () => {
    it.each(['ratings', 'sandbox'] as const)('%s presentation shows only the Overall badge', (presentation) => {
