...
import { SEASON_DRAFT_CATALOG_V3, SEASON_DURABILITY_VERSION, SEASON_STAMINA_VERSION, SIMULATION_RATINGS, SIMULATION_TENDENCIES, PLAYER_VERSION_ID_VERSION, type Position, type SeasonDraftCandidate, type SeasonDraftCatalog, type SeasonEffectsState, type SeasonFreeAgencyBand, type SeasonFreeAgencyIndex, type SeasonFreeAgencyState, type SeasonFreeAgencyWindowState, type SeasonRun, } from '@hoop-rush/data-contracts';
...
function catalogCandidate(playerVersionId: string, playerId: string, playable: readonly Position[]): SeasonDraftCandidate {
    const primary = playable[0];
    if (primary === undefined)
        throw new Error('no primary position');
    return {
        playerVersionId,
        playerId,
        franchiseId: 'lakers',
...
function fixtureCatalog(run: SeasonRun): SeasonDraftCatalog {
    ...
    for (const roster of run.rosters) {
        roster.players.forEach((player, slot) => {
            ...
            if (playable === undefined)
                throw new Error('no position pattern for slot');
    ...
    for (let i = 0; i < 30; i += 1) {
        ...
        if (playable === undefined)
            throw new Error('no position pattern for extra');
        candidates.push(catalogCandidate(`pv-extra-${String(i).padStart(2, '0')}`, `p-extra-${String(i).padStart(2, '0')}`, playable));
    }
    return {
        schemaVersion: 1,
...
type SeasonFreeAgencyIndexEntryRoles = SeasonFreeAgencyIndex['candidates'][number]['supportedRoles'];
...
function fixtureRun(): SeasonRun {
    ...
    const opened = openSeasonFreeAgencyWindow({ run: base, effects: zeroEffectsOf(base), catalog, index, humanFranchiseId: HUMAN }, 0, 2);
...
function windowOf(freeAgency: SeasonFreeAgencyState): SeasonFreeAgencyWindowState {
    ...
    if (window === undefined)
        throw new Error('fixture has no open window');
    return window;
}
function playableByVersionOf(run: SeasonRun): Map<string, readonly Position[]> {
    ...
    for (const roster of run.rosters) {
        roster.players.forEach((entry, slot) => {
            const playable = SLOT_POSITIONS[slot];
            if (playable !== undefined)
                map.set(entry.playerVersionId, playable);
        });
    }
    return map;
}
...
function declaredRun(run: SeasonRun, targets: {
...
function resolvedRun(run: SeasonRun, humanSigned: boolean): SeasonRun {
    ...
    if (second === undefined)
        throw new Error('need two candidates');
...
describe('free-agency market overview (M2.6.5)', () => {
    ...
    it('lists franchises with recorded interest, human first', () => {
        const run = fixtureRun();
        const window = windowOf(run.freeAgency);
        const busy = window.candidates.find((candidate) => Object.values(window.declarations).some((declaration) => declaration.targets.some((target) => target.playerVersionId === candidate.playerVersionId)));
        expect(busy).toBeDefined();
        const { container } = renderRoute(run);
        const interests = Array.from(container.querySelectorAll('[data-fa-interest]')).filter((el) => el.textContent.trim().length > 0);
...
describe('free-agency declaration step (M2.6.5)', () => {
    ...
    it('targets two ordered players through the priority pickers', async () => {
        ...
        const cardOf = (playerVersionId: string) => {
            const card = candidateCards(container).find((el) => el.querySelector(`#fa-priority-${playerVersionId}`));
            if (card === undefined)
                throw new Error(`no card for ${playerVersionId}`);
            return card;
        };
        const priorityOf = (card: HTMLElement) => card.querySelector('[data-fa-candidate-priority]') as HTMLSelectElement;
    ...
    it('adds a target via the Target button without using the priority picker', async () => {
        ...
        const first = window.candidates[0];
        if (first === undefined)
            throw new Error('need a candidate');
        const { container } = renderRoute(run);
        const card = candidateCards(container).find((el) => el.querySelector(`#fa-priority-${first.playerVersionId}`));
        if (card === undefined)
            throw new Error(`no card for ${first.playerVersionId}`);
    ...
    it('keeps at most one first and one second priority (swap on conflict)', async () => {
        ...
        const first = window.candidates[0];
        const second = window.candidates[1];
        const third = window.candidates[2];
        if (first === undefined || second === undefined || third === undefined) {
            throw new Error('need three candidates');
        }
        const { container } = renderRoute(run);
        const priorityOf = (playerVersionId: string) => {
            const select = container.querySelector(`#fa-priority-${playerVersionId}`) as HTMLSelectElement;
            return select;
        };
        ...
        const priorities = candidateCards(container).map((card) => Number((card.querySelector('[data-fa-candidate-priority]') as HTMLSelectElement).value));
        const held = priorities.filter((value) => value !== 0);
        expect(held).toContain(1);
        expect(held.filter((value) => value === 1).length).toBeLessThanOrEqual(1);
        expect(held.filter((value) => value === 2).length).toBeLessThanOrEqual(1);
    });
    it('bounds the Influence commitment to the candidate minimum through 3', async () => {
        ...
        if (first === undefined)
            throw new Error('no candidates');
        const { container } = renderRoute(run);
        await fireEvent.change(container.querySelector(`#fa-priority-${first.playerVersionId}`) as HTMLSelectElement, { target: { value: '1' } });
    ...
    it('blocks submission when the commitment exceeds the season budget', async () => {
        ...
        const select = container.querySelector(`#fa-priority-${first.playerVersionId}`) as HTMLSelectElement;
        await fireEvent.change(select, { target: { value: '1' } });
        await fireEvent.input(container.querySelector('[data-fa-influence-input]') as HTMLInputElement, {
            target: { value: '2' },
        });
        const submit = container.querySelector('[data-fa-declare-submit]') as HTMLButtonElement;
    ...
    it('submits the declaration through the shell with ordered targets', async () => {
        ...
        const select = (playerVersionId: string) => container.querySelector(`#fa-priority-${playerVersionId}`) as HTMLSelectElement;
...
describe('keyboard-accessible declaration controls (M2.6.5)', () => {
    it('operates the priority picker as a native select and announces state', async () => {
        const run = fixtureRun();
        const window = windowOf(run.freeAgency);
        const first = window.candidates[0];
        if (first === undefined)
        ...
        if (card === undefined)
            throw new Error(`no card for ${first.playerVersionId}`);
        const select = within(card).getByLabelText<HTMLSelectElement>(/^Target /);
        expect(select.tagName).toBe('SELECT');
        await fireEvent.change(select, { target: { value: '1' } });
        const input = within(card).getByRole<HTMLInputElement>('spinbutton');
    ...
    it('clamps typed influence to the candidate bounds', async () => {
        const run = fixtureRun();
        const window = windowOf(run.freeAgency);
        const first = window.candidates[0];
        if (first === undefined)
            throw new Error('no candidates');
        const { container } = renderRoute(run);
        await fireEvent.change(container.querySelector(`#fa-priority-${first.playerVersionId}`) as HTMLSelectElement, { target: { value: '2' } });
        const input = container.querySelector('[data-fa-influence-input]') as HTMLInputElement;
...
describe('free-agency window lifecycle across the route (M2.6.5)', () => {
    it('declares then shows the review step in the same route state machine', () => {
        ...
        const declared = renderRoute(declaredRun(run, [
            { playerVersionId: first.playerVersionId, roleExpectation: 'rotation', influence: 2 },
        ]));
...
describe('free-agency recap evidence (M2.6.5)', () => {
    it('renders the block recap free-agency section from recorded evidence', async () => {
        ...
        const recap: import('@hoop-rush/data-contracts').SeasonBlockRecap = {
            ...
            influenceBalance: { humanBalance: 3 },
        };
        ...
        const section = container.querySelector('[data-recap-free-agency-evidence]');
        expect(section).not.toBeNull();
        expect(section?.textContent).toContain('Window 1 resolved with');
        expect(section?.textContent).toContain('1 signing');
        expect(section?.textContent).toContain('lakers (featured)');
        expect(section?.textContent).toContain('1/3 signings');
        expect(section?.textContent).toContain('2/6 Influence');
    });
});
describe('free-agency hub CTA (M2.6.5)', () => {
    it('links the open window from the hub page and hides without one', async () => {
        const { default: SeasonRunShellWrapper } = await import('../../../test/SeasonRunShellWrapper.svelte');
        const run = fixtureRun();
