...
import type { HoopRushManifest, SeasonLeague, SeasonRosterEntry, SeasonStandings, } from '@hoop-rush/data-contracts';
...
function renderLeaders() {
    return render(LeadersTable, {
        props: {
            ...
            rosterByVersion: new Map(LEADER_ENTRIES.map((entry) => [
                entry.playerVersionId,
                rosterEntry(entry.playerVersionId, '1995-96'),
            ])),
            faces: new Map(LEADER_ENTRIES.map((entry) => [entry.playerVersionId, face(entry.playerVersionId)])),
            manifest: MANIFEST,
