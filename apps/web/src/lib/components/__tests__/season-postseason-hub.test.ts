...
import { franchiseIdSchema, idSchema, seasonGameIdSchema, seedSchema, } from '@hoop-rush/data-contracts';
...
describe('hub: play-in stage with the human lineup decision', () => {
    ...
    it('routes the typed rejection message into the decision panel', () => {
        ...
        const { container } = render(SeasonRunShellWrapper, { props: { shell } });
        expect(container.querySelector('[data-season-postseason-lineup]')?.textContent).toContain('starter PG cannot play slot');
    });
    it('submits the editor rotation with the selected risky-rehab injury', async () => {
        ...
        await fireEvent.click(container.querySelector('[data-season-postseason-submit]') as HTMLElement);
        expect(submit).toHaveBeenCalledTimes(1);
        const firstCall = submit.mock.calls[0];
        if (firstCall === undefined)
            throw new Error('expected a submit call');
...
describe('hub: eliminated stage', () => {
    it('offers spectate-next and fast-forward with progress + cancellation', async () => {
        ...
        const cancelButton = [...progress.container.querySelectorAll('button')].find((button) => button.textContent.includes('Cancel'));
