...
function editorFor(count: number) {
    ...
    const editor = createRotationEditor(rotation, roster.map((member) => ({
        playerVersionId: member.id,
...
describe('rotation editor: active/inactive roster (M2.6.5)', () => {
    ...
    it('promotes an inactive player through the replace picker (engine-audited swap)', async () => {
        ...
        const inactiveRow = Array.from(container.querySelectorAll<HTMLElement>('[data-rotation-inactive-row]')).find((row) => row.textContent.includes(inactive.id));
        if (inactiveRow === undefined)
            throw new Error('inactive row missing');
        ...
        if (option === undefined)
            throw new Error('replace option missing');
    ...
    it('demotes an active player through the demote picker', async () => {
        ...
        if (option === undefined)
            throw new Error('promote option missing');
        await fireEvent.click(option);
        expect(editor.isActive(inactive.id)).toBe(true);
        expect(editor.isActive(active.id)).toBe(false);
        expect(editor.validate()).toEqual([]);
    ...
    it('rejects an illegal promotion with a visible rejection (no commit)', async () => {
        ...
        if (depth === undefined || center === undefined)
            throw new Error('fixture too small');
...
describe('roster list: active/inactive presentation (M2.6.5)', () => {
    it('marks the ten active and the inactive depth rows', () => {
        ...
        const roleOf = (playerVersionId: string) => {
            if (!editor.isActive(playerVersionId))
                return { role: 'Inactive', minutes: '—' as const };
        ...
        const inactive = rows.filter((row) => row.getAttribute('data-season-roster-status') === 'inactive');
