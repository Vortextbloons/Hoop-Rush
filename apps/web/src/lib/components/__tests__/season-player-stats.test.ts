...
function lineOf(roster: SeasonRoster, index: number, overrides: Partial<Record<string, number>> = {}) {
...
function playedGame(): SeasonGameSummary {
    ...
    return {
        ...game,
        homePlayers: home.players.map((_, index) => lineOf(home, index, index === 0 ? { points: 30, assists: 9 } : index === 1 ? { offensiveRebounds: 10 } : {})),
    };
}
