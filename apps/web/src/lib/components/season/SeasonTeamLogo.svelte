<script lang="ts">import { resolveLogoUrlsWithHistorical, type HoopRushManifest } from '@hoop-rush/data-contracts';
import { franchiseAbbreviation } from '@hoop-rush/data-contracts';
import { useImageFallback } from '$lib/use-image-fallback.svelte';
let { manifest, franchiseId, teamExternalId, logoCandidates = [], alt = '', size = 'md', eager = false, }: {
...
const urls = $derived(resolveLogoUrlsWithHistorical(manifest, franchiseId, teamExternalId, [...logoCandidates]));
const fallback = useImageFallback({
    urls: () => urls,
    key: () => `${franchiseId}:${teamExternalId}:${logoCandidates.join('|')}`,
});
const boxClass = $derived(size === 'lg'
