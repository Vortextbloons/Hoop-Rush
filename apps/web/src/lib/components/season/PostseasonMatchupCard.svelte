  ...
  const footerText = $derived.by(() => {
    if (series !== null) {
      if (series.status === 'complete' && series.winnerFranchiseId !== null) {
        return `${franchiseName(series.winnerFranchiseId)} wins the series ${String(
          series.homeWins,
        )}–${String(series.awayWins)}`;
      }
      if (series.nextGame !== null) {
        return `Next: Game ${String(series.nextGame.gameNumber)} · at ${franchiseName(
          series.nextGame.homeFranchiseId,
        )}`;
      }
