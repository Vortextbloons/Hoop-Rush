...
>
  <div class="relative overflow-hidden rounded-2xl border border-line-strong bg-surface-1">
    <div
      class="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-primary via-amber-500 to-sky-500 opacity-80"
    ></div>
      ...
      {#if windowState !== null}
        <div class="flex flex-wrap items-center gap-2">
          ...
          {#if commandError !== null}
            <span
              role="alert"
              class="rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-1.5 text-sm text-destructive"
              >{commandError}</span
            >
          {/if}
        </div>
  ...
  <div
    ...
    role="tablist"
    aria-label="Trade board sections"
  >
    {#each [{ id: 'board', label: 'Board' }, { id: 'build', label: 'Builder' }, { id: 'negotiate', label: 'Negotiation' }] as tab (tab.id)}
      <button
        ...
        class="flex-1 rounded-lg px-3 py-2 min-h-11 text-sm font-semibold outline-none focus-visible:ring-2 focus-visible:ring-ring {mobileTab ===
        tab.id
  ...
  <div class="grid gap-4 lg:grid-cols-[360px_minmax(0,1fr)_380px] lg:items-start">
    ...
    >
      <div class="rounded-xl border border-border bg-surface-1">
        <div
          class="border-b border-border bg-surface-2 px-4 py-3 flex items-baseline justify-between"
        >
          <h3
            ...
            {#each boardProfiles as profile (profile.franchiseId)}
              ...
              <li>
                <button
                  ...
                  class="group relative flex w-full flex-col gap-2 rounded-xl border bg-card p-3 text-left outline-none transition-all hover:-translate-y-0.5 hover:shadow-md focus-visible:ring-2 focus-visible:ring-ring {selectedFranchiseId ===
                  profile.franchiseId
                ...
                >
                  <span
                    class="pointer-events-none absolute left-1/2 top-1.5 h-2 w-2 -translate-x-1/2 rounded-full bg-primary shadow-sm ring-2 ring-primary/20"
                  ></span>

                  <span
                    class="pointer-events-none absolute inset-y-0 left-0 w-1 rounded-l-xl bg-[repeating-linear-gradient(180deg,transparent_0_6px,var(--color-border)_6px_8px)] opacity-40"
                  ></span>
                  ...
                  <span class="flex items-center gap-2 pt-1">
                    ...
                    <span class="min-w-0 flex-1">
                      <span
                        class="block truncate font-display text-sm font-extrabold uppercase tracking-tight"
                        >{manifest
                          ? (manifest.modernFranchiseSlots.find(
                              (s) => s.franchiseId === profile.franchiseId,
                            )?.displayName ?? profile.franchiseId)
                          : profile.franchiseId}</span
                      >
                      <span class="block font-mono text-[10px] text-muted-foreground"
                        >Priority: {formatTradePriority(profile.priority)} · {formatTradeNeeds(
                          profile.needs,
                        )}</span
                      >
                    </span>
                    {#if activeNegotiation?.toFranchiseId === profile.franchiseId}
                      <span
                        class="shrink-0 rounded-full bg-primary px-2 py-0.5 font-mono text-[10px] font-bold text-primary-foreground"
                        >Active</span
                      >
                    {/if}
                  </span>
                  ...
                  <span class="flex flex-wrap gap-1.5">
                    {#each profile.needs as need (need)}<span
                        class="rounded-full bg-surface-3 px-2 py-0.5 font-mono text-[10px] font-semibold text-muted-foreground"
                        >{formatTradeNeeds([need])}</span
                      >{/each}
                  </span>

                  <span class="text-xs leading-snug text-muted-foreground line-clamp-2"
                    >{profile.rationale}</span
                  >
                  ...
                  <span class="flex flex-col gap-1 rounded-lg bg-surface-2 p-2">
                    ...
                    <span class="text-xs"
                      ...
                      {profile.listedPlayerIds.map(playerName).join(' · ') || '—'}</span
                    >
                    <span class="text-xs"
                      ><span class="font-semibold text-foreground">Discussable:</span>
                      {profile.discussablePlayerIds.map(playerName).join(' · ') || '—'}</span
                    >
                    <span class="text-xs"
                      ><span class="font-semibold text-destructive">Protected:</span>
                      {profile.protectedPlayerIds.map(playerName).join(' · ') || '—'}</span
                    >
                  </span>

                  <span class="rounded-lg border border-amber-500/20 bg-amber-500/5 p-2">
                    <span
                      class="block font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-amber-700 dark:text-amber-300"
                      >Hard constraints</span
                    >
                    <ul class="mt-1 flex flex-col gap-0.5">
                      {#each profile.hardConstraints as c (c)}<li
                          class="text-xs text-amber-700/90 dark:text-amber-300/90"
                        >
                          • {c}
                        </li>{/each}
                    </ul>
                  ...
                  {#if profile.competitorInterest}
                    <span class="flex flex-wrap gap-1">
                      {#each Object.entries(profile.competitorInterest) as [pid, interest] (pid)}
                        <span
                          class="rounded-full border border-border bg-surface-1 px-2 py-0.5 font-mono text-[10px]"
                          >{playerName(pid)}: {competitorInterestLabel(interest)}</span
                        >
                      {/each}
                    </span>
                  {/if}
            ...
            {#each humanTrends as trend (trend.playerVersionId)}
              <li>
                <ValueTrendCell
                  {trend}
                  basis={trend.basis}
                  playerName={playerName(trend.playerVersionId)}
                />
              </li>
            {/each}
          </ul>
    ...
    <div
      ...
      class="flex flex-col gap-3 {mobileTab !== 'build' ? 'hidden lg:flex' : ''}"
    >
      {#if windowState === null}
      ...
      {:else}
        ...
        {@const theirLites = targetRoster.players.map((p) => ({
          playerVersionId: p.playerVersionId,
          displayName: p.displayName,
          playable: playableOf(p.playerVersionId),
          available: availableOf(p.playerVersionId),
        ...
        <PackageBuilder
          ...
          targetFranchiseName={manifest
            ? (manifest.modernFranchiseSlots.find((s) => s.franchiseId === safeTargetId)
                ?.displayName ?? safeTargetId)
            : safeTargetId}
          {inquiryAllowance}
          {inquiriesUsed}
          allowanceLabel={inquiryCounterLabel(
            inquiryAllowance,
            inquiriesUsed,
            purchasedUsed,
            earnedUsed,
    ...
    <div
      ...
      class="flex flex-col gap-3 {mobileTab !== 'negotiate' ? 'hidden lg:flex' : ''}"
    >
            ...
            {#each negotiations as n (n.inquiryId)}
              <li>
                <button
                  ...
                  class="flex w-full items-center justify-between gap-2 rounded-lg border px-3 py-2 min-h-11 text-left text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring {activeNegotiation?.inquiryId ===
                  n.inquiryId
