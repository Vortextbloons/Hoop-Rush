...
const injurySummary = $derived.by(() => {
    ...
    if (recap.injuryEvidence.injuries > 0) {
        parts.push(`${String(recap.injuryEvidence.injuries)} injur${recap.injuryEvidence.injuries === 1 ? 'y' : 'ies'} (${String(recap.injuryEvidence.returnedThisBlock)} back)`);
    }
    else {
        parts.push('No new injuries');
    }
    if (recap.tradeEvidence.tradesAccepted > 0) {
        parts.push(`${String(recap.tradeEvidence.tradesAccepted)} trade${recap.tradeEvidence.tradesAccepted === 1 ? '' : 's'}`);
    }
