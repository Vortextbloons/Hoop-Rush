<script lang="ts">
  ...
  const injurySummary = $derived(
    `${String(recap.injuryEvidence.injuries)} ${recap.injuryEvidence.injuries === 1 ? 'injury' : 'injuries'} across the league · ${String(recap.injuryEvidence.sameGameReturns)} same-game return${recap.injuryEvidence.sameGameReturns === 1 ? '' : 's'} · ${String(recap.injuryEvidence.returnedThisBlock)} returned · ${String(recap.injuryEvidence.activeAtBlockEnd)} still out at block end.`,
  );
