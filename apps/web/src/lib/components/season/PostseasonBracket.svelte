<script lang="ts">
  ...
  const semifinals = $derived(
    columns.find((column) => column.key === 'conference-semifinal')?.series ?? [],
  );
  const conferenceFinals = $derived(
    columns.find((column) => column.key === 'conference-final')?.series ?? [],
  );
  const finals = $derived(columns.find((column) => column.key === 'finals')?.series ?? []);
  const conferenceSeries = (cards: typeof firstRound, conference: 'east' | 'west') =>
    cards.filter((card) => card.conference === conference);
</script>

<div class="bracket-shell min-w-0">
  <div class="play-in-dock hidden lg:block">
    <div class="dock-heading">
      <span>Play-In runway</span><small>Seeds 7–10 · single elimination</small>
    </div>
    <div class="grid grid-cols-2 gap-6">
      {#each playIn as column (column.conference)}
        <section aria-label={`${column.conference === 'east' ? 'East' : 'West'} Play-In`}>
          <p class="conference-label">
            {column.conference === 'east' ? 'Eastern Conference' : 'Western Conference'}
          </p>
  ...
  <div class="desktop-bracket hidden lg:grid" aria-label="Playoff bracket">
    {#each ['east', 'west'] as conference (conference)}
      ...
      <section
        class="conference-draw"
        data-side={side}
        aria-label={`${side === 'east' ? 'Eastern' : 'Western'} Conference bracket`}
      >
        <div class="round-column round-first">
          <h3>{side === 'east' ? 'East' : 'West'} · First round</h3>
          <div class="round-stack four">
            {#each conferenceSeries(firstRound, side) as card (card.seriesId)}<SeriesCard
                {card}
                {franchiseName}
                {franchiseAbbrev}
                {manifest}
        ...
        <div class="round-column round-semis">
          <h3>Conference semis</h3>
          <div class="round-stack two">
            {#each conferenceSeries(semifinals, side) as card (card.seriesId)}<SeriesCard
                {card}
                {franchiseName}
                {franchiseAbbrev}
                {manifest}
        ...
        <div class="round-column round-final">
          ...
          <div class="round-stack one">
            {#each conferenceSeries(conferenceFinals, side) as card (card.seriesId)}<SeriesCard
                {card}
                {franchiseName}
                {franchiseAbbrev}
                {manifest}
    ...
    <section class="finals-column" aria-label="League Finals">
      ...
      <h3>League Finals</h3>
      {#each finals as card (card.seriesId)}<SeriesCard
          {card}
          {franchiseName}
          {franchiseAbbrev}
          {manifest}
      ...
      <li>
        {#if entry.kind === 'play-in'}
          <section aria-label={`${entry.column.conference === 'east' ? 'East' : 'West'} Play-In`}>
            <h3 class="mobile-round">
              Play-In · {entry.column.conference === 'east' ? 'East' : 'West'}
            </h3>
            <ul class="mt-2 flex flex-col gap-2">
              {#each entry.column.games as game (game.gameId)}<li>
                  <PlayInCard
                    card={game}
                    {franchiseName}
                    {franchiseAbbrev}
        ...
        {:else}
          <section aria-label={entry.card.label}>
            <h3 class="mobile-round">
              {entry.card.conference === null
                ? 'League'
                : entry.card.conference === 'east'
                  ? 'East'
                  : 'West'} · {entry.card.label}
            </h3>
            <div class="mt-2">
              <SeriesCard
                card={entry.card}
...
<style>
  .bracket-shell {
    --bracket-line: color-mix(in srgb, var(--color-primary) 30%, var(--color-border));
  }
  .play-in-dock {
    margin-bottom: 1rem;
    padding: 1rem;
    ...
    background: linear-gradient(
      135deg,
      color-mix(in srgb, var(--color-primary) 8%, var(--color-surface-1)),
      var(--color-surface-1)
    );
  }
  .dock-heading {
  ...
  .desktop-bracket {
    ...
    background:
      linear-gradient(
        90deg,
        color-mix(in srgb, #174ea6 5%, transparent),
        transparent 38% 62%,
        color-mix(in srgb, #b91c1c 5%, transparent)
      ),
      var(--color-surface-0);
  }
  .conference-draw {
  ...
  .conference-draw[data-side='east'] {
    grid-column: 1;
    grid-row: 1;
  }
  .conference-draw[data-side='west'] {
    grid-column: 3;
    grid-row: 1;
  }
  .conference-draw[data-side='west'] {
  ...
  .round-column:not(.round-final)::after {
    content: '';
    position: absolute;
    top: 50%;
    right: -0.8rem;
  ...
  .conference-draw[data-side='west'] .round-column:not(.round-final)::after {
    right: auto;
    left: -0.8rem;
  }
  .finals-column {
    ...
    border-radius: 1rem;
    background: color-mix(in srgb, var(--color-primary) 7%, var(--color-surface-1));
  }
  .finals-column :global(article) {
    text-align: left;
  }
  .finals-column p {
    margin-top: 0.7rem;
