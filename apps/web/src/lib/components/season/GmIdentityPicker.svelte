<script lang="ts">
  ...
  let {
    ...
    onSelect: (input: { identity: SeasonCampaignGmIdentity; focus: SeasonCampaignFocus | null }) => void;
  } = $props();

  let selectedIdentity: SeasonCampaignGmIdentity | null = $state(null);
  let selectedFocus: SeasonCampaignFocus | null = $state(null);

  const identities: Array<{ id: SeasonCampaignGmIdentity; label: string; blurb: string; accent: string }> = [
    {
      id: 'win-now',
      label: 'Win now',
  ...
  const focuses: Array<{ id: SeasonCampaignFocus; label: string; note: string }> = [
...
>
  <div class="pointer-events-none absolute inset-0 opacity-[0.04]">
    <div class="h-full w-full" style="background-image: repeating-linear-gradient( -8deg, transparent 0 14px, currentColor 14px 15px );"></div>
  </div>

  <div class="relative">
    <div class="border-b border-border bg-gradient-to-r from-surface-2 to-surface-1 px-4 py-3 sm:px-5">
      <p class="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-primary">Front Office Memo — Confidential</p>
      <h2 id="gm-identity-heading" class="font-display mt-1 text-xl font-extrabold uppercase tracking-tight sm:text-2xl">
      ...
      <p class="mt-1 max-w-2xl text-sm leading-relaxed text-muted-foreground">
        ...
        branch reasoning. It <strong class="font-semibold text-foreground">never</strong> changes ratings, shooting odds, health, officiating, or schedule.
      </p>
    </div>

    {#if commandError !== null}
      <p role="alert" class="mx-4 mt-3 rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm sm:mx-5">
        {commandError}
      </p>
    {/if}
    ...
    <div class="grid gap-3 p-4 sm:p-5 lg:grid-cols-3">
      {#each identities as identity (identity.id)}
        <button
          ...
          tabindex={selectedIdentity === identity.id ? 0 : selectedIdentity === null && identity.id === 'win-now' ? 0 : -1}
          onclick={() => {
            selectedIdentity = identity.id;
            if (identity.id !== 'team-identity') selectedFocus = null;
          }}
          onkeydown={(e) => onIdentityKeydown(e, identity.id)}
          class="group relative flex min-h-[148px] flex-col gap-2 rounded-xl border bg-card p-4 text-left outline-none transition-all hover:-translate-y-0.5 hover:shadow-lg focus-visible:ring-2 focus-visible:ring-ring {selectedIdentity === identity.id ? `${identity.accent} ring-1 ring-current bg-primary/5` : 'border-border hover:border-line-strong'}"
        >
          <span class="absolute right-3 top-3 h-2 w-2 rounded-full {selectedIdentity === identity.id ? 'bg-primary animate-pulse' : 'bg-muted-foreground/30'}"></span>
          <span class="font-display text-base font-extrabold uppercase tracking-tight">{identity.label}</span>
          <span class="text-sm leading-snug text-muted-foreground">{identity.blurb}</span>
          <span class="mt-auto font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground">
    ...
    {#if selectedIdentity === 'team-identity'}
      <div class="border-t border-border bg-surface-2/60 px-4 py-4 sm:px-5">
        <fieldset class="flex flex-col gap-2">
          <legend class="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-muted-foreground">
            Pick a style focus — required for Team Identity
          </legend>
          <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            {#each focuses as focus (focus.id)}
              <label
                class="flex cursor-pointer items-center gap-3 rounded-lg border bg-card px-3 py-2.5 outline-none transition-colors has-[input:focus-visible]:ring-2 has-[input:focus-visible]:ring-ring {selectedFocus === focus.id ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/40'}"
              >
                <input
                  type="radio"
                  name="gm-focus"
    ...
    <div class="flex flex-col gap-2 border-t border-border bg-surface-1 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
      <p class="font-mono text-[10px] leading-relaxed text-muted-foreground sm:max-w-[420px]">
        Locked identity enters the run digest and command log. You keep it for the whole season except the one controlled evolution after block 4.
      </p>
      <button
        type="button"
        onclick={submit}
  ...
  <p class="sr-only" role="status" aria-live="polite">
    ...
    {#if selectedIdentity !== null}Selected {selectedIdentity}{selectedFocus ? ` with ${selectedFocus}` : ''}{/if}
  </p>
</section>
