...
>
  <div
    class="bg-gradient-to-br from-surface-2 to-surface-1 px-4 py-3 sm:px-5 border-b border-border"
  >
    <p class="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-primary">
      Midseason — Block 4 complete
    </p>
    <h2
      id="evolution-heading"
      class="font-display mt-1 text-xl font-extrabold uppercase tracking-tight"
    >
      Evolve your campaign
    </h2>
    <p class="mt-1 text-sm text-muted-foreground">
      Choose one. <strong class="text-foreground">Double-down</strong> keeps your identity and
      unlocks its strongest final branch.
      <strong class="text-foreground">Adapt</strong> adds a secondary focus.
      <strong class="text-foreground">Pivot</strong> replaces your identity. Up to two adapt/pivot options
      appear only if the last five blocks showed evidence for them.
    </p>
  </div>
  ...
  <div class="grid gap-3 p-4 sm:p-5" role="radiogroup" aria-labelledby="evolution-heading">
    {#each offers as offer (offer.offerId)}
      <label
        class="group relative flex cursor-pointer gap-3 rounded-xl border bg-card p-4 outline-none transition-colors has-[input:focus-visible]:ring-2 has-[input:focus-visible]:ring-ring {selectedOfferId ===
        offer.offerId
          ? kindAccent(offer.kind) + ' ring-1 ring-current'
        ...
        <span class="min-w-0 flex-1">
          <span class="flex flex-wrap items-baseline gap-2">
            <span
              ...
              >{kindLabel(offer.kind)}</span
            >
            <span class="font-display text-sm font-extrabold uppercase tracking-tight"
              >{offer.resultingIdentity}</span
            >
            {#if offer.resultingFocus}
              <span class="font-mono text-[10px] text-muted-foreground"
                >· {offer.resultingFocus}</span
              >
            {/if}
          </span>
          <span class="mt-1 block text-sm leading-snug text-muted-foreground">{offer.evidence}</span
          >
          <span class="mt-1 block font-mono text-[10px] text-muted-foreground/70"
            >Evidence-backed · branch {offer.offerId.slice(0, 10)}…</span
          >
        </span>
      </label>
    {/each}
  ...
  <div
    class="flex flex-col gap-2 border-t border-border px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5 bg-surface-1"
  >
    <p class="font-mono text-[10px] text-muted-foreground max-w-[480px]">
      Evolution happens once. There are no skill points, levels, or reputation meters. Your choice
      enters the digest and unlocks the final branch.
    </p>
    <button
      type="button"
