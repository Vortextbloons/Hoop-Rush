...
>
  <div class="flex flex-wrap items-baseline justify-between gap-2 border-b border-border pb-3">
    ...
    {:else}
      <span class="font-mono text-[10px] text-muted-foreground"
        >No active inquiry — pick a team to start</span
      >
    {/if}
  </div>
  ...
  {#if negotiation === null}
    <p class="rounded-lg bg-surface-2 p-3 text-sm text-muted-foreground">
      Open a team from the board to begin. One active negotiation at a time, up to three exchanges.
      Walk-away has no penalty; history is preserved after the window closes.
    </p>
  {:else}
    <ol class="flex flex-col gap-2" aria-label="Negotiation exchanges">
      {#each exchanges as ex (ex.exchangeIndex)}
        <li class="flex gap-3 rounded-lg border border-border bg-card p-3">
          <span
            class="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-surface-3 font-mono text-xs font-bold"
            >{ex.exchangeIndex}</span
          >
          <div class="min-w-0 flex-1">
            <p class="text-sm font-semibold">
              {kindLabel(ex.kind)}
              {#if ex.responseCause !== null}· <span
                  class="font-mono text-xs {ex.responseCause === 'acceptable'
                    ? 'text-positive'
                    : ex.responseCause === 'close-needs-more-value'
                      ? 'text-amber-600 dark:text-amber-400'
                      : 'text-destructive'}">{responseCauseLabel(ex.responseCause)}</span
                >{/if}
            </p>
            {#if ex.proposalFingerprint !== null}
              <p class="mt-1 font-mono text-[10px] text-muted-foreground">
                Fingerprint {ex.proposalFingerprint.slice(0, 16)}… · at revision {ex.atStateRevision}
              </p>
            {/if}
    ...
    {#if negotiation.finalReason !== null}
      <p
        class="rounded-lg border p-2.5 text-sm {negotiation.finalReason === 'acceptable'
      ...
      <div class="rounded-lg bg-surface-2 p-3">
        ...
        <ul class="mt-1 flex flex-wrap gap-1.5">
          {#each negotiation.expressedInterests as interest (interest)}
            <li class="rounded-full bg-card border border-border px-2.5 py-1 text-xs">
              {interest}
            </li>
          {/each}
        </ul>
    ...
    {#if negotiation.rejectedPlayerVersionIds.length > 0}
      <p class="font-mono text-[10px] text-muted-foreground">
        Rejected players: {negotiation.rejectedPlayerVersionIds.join(' · ')}
      </p>
    {/if}

    {#if isFinal}
      <p
        class="rounded-lg bg-surface-2 p-3 text-sm text-muted-foreground"
        role="status"
        aria-live="polite"
      >
        {#if negotiation.status === 'accepted'}Accepted — ownership will transfer and rotations will
          repair at commit. Announcement does not move focus.{/if}
        {#if negotiation.status === 'declined'}Declined — walk-away has no penalty. You can open
          another inquiry if cap remains.{/if}
        {#if negotiation.status === 'walked-away'}You walked away — no penalty. History preserved
          for this window.{/if}
    ...
    {:else if canAct}
      ...
      <p class="font-mono text-[10px] text-muted-foreground">
        3 exchanges total: 1 initial, 2 counter, 3 final accept/revision/walk-away. Duplicate
        fingerprint rejects do not increment.
      </p>
    {:else if negotiation.status === 'draft'}
      <p class="text-sm text-muted-foreground">
        Draft — submit your first proposal above. That consumes the inquiry.
      </p>
    {/if}

    <p class="sr-only" role="status" aria-live="polite">
      {#if negotiation !== null}Negotiation {negotiation.inquiryId.slice(0, 8)}
        {negotiation.status} with {exchangeCount} exchanges{/if}
    </p>
  {/if}
</section>
