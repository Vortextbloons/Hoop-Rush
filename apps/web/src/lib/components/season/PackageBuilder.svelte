...
<div class="flex flex-col gap-4 rounded-xl border border-border bg-card" data-testid="package-builder">
  <div class="border-b border-border bg-surface-2 px-4 py-3">
    ...
    <p class="mt-1 font-mono text-[10px] text-muted-foreground">
      Pick 1–2 from each side. Optionally add 1–2 Influence from one side — never both, never alone. Rosters must stay 10–15 and keep a legal ten.
    </p>
    <p class="mt-2 rounded-lg bg-primary/10 px-2.5 py-1.5 font-mono text-[10px] text-primary">
      Inquiry: {allowanceLabel} · {inquiriesRemaining} remaining · {willConsumeInquiry ? 'this proposal will consume 1 inquiry' : 'at cap — purchase or earned credit needed'} · browsing is free
    </p>
  </div>

  {#if commandError !== null}
    <p role="alert" class="mx-4 rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm">{commandError}</p>
  {/if}
        ...
        {#each yourPlayers as player (player.playerVersionId)}
          <li>
            <button
              ...
              class="flex w-full items-center gap-2 rounded-lg border px-3 py-2 text-left outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50 {outgoingSet.has(player.playerVersionId) ? 'border-primary bg-primary/10' : 'border-border bg-card hover:border-primary/30'}"
            >
              <span class="grid h-4 w-4 place-items-center rounded border {outgoingSet.has(player.playerVersionId) ? 'bg-primary border-primary text-primary-foreground' : 'border-muted-foreground/30 bg-transparent'}">
                {#if outgoingSet.has(player.playerVersionId)}✓{/if}
              </span>
              <span class="min-w-0 flex-1">
                <span class="block truncate text-sm font-semibold">{player.displayName}</span>
                <span class="block truncate font-mono text-[10px] text-muted-foreground">{player.playable.join('·') || '—'} {player.available ? '' : '· OUT — availability risk'}</span>
              </span>
            </button>
          </li>
        {/each}
        ...
        {#each theirPlayers as player (player.playerVersionId)}
          <li>
            <button
              ...
              class="flex w-full items-center gap-2 rounded-lg border px-3 py-2 text-left outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50 {incomingSet.has(player.playerVersionId) ? 'border-primary bg-primary/10' : 'border-border bg-card hover:border-primary/30'}"
            >
              <span class="grid h-4 w-4 place-items-center rounded border {incomingSet.has(player.playerVersionId) ? 'bg-primary border-primary text-primary-foreground' : 'border-muted-foreground/30 bg-transparent'}">
                {#if incomingSet.has(player.playerVersionId)}✓{/if}
              </span>
              <span class="min-w-0 flex-1">
                <span class="block truncate text-sm font-semibold">{player.displayName}</span>
                <span class="block truncate font-mono text-[10px] text-muted-foreground">{player.playable.join('·') || '—'} {player.available ? '' : '· OUT'}</span>
              </span>
            </button>
          </li>
        {/each}
  ...
  <fieldset class="mx-4 rounded-xl border border-border bg-surface-1 p-3">
    ...
    <div class="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
      {#each influenceOptions as opt (`${String(opt.amount)}-${opt.from ?? 'none'}`)}
        <label
          class="flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 outline-none transition-colors has-[input:focus-visible]:ring-2 has-[input:focus-visible]:ring-ring {selectedInfluence?.amount === opt.amount && selectedInfluence?.from === opt.from ? 'border-primary bg-primary/10' : 'border-border bg-card hover:border-primary/30'} {opt.disabledReason ? 'opacity-50' : ''}"
        >
          <input
            type="radio"
            name="influence"
            checked={selectedInfluence?.amount === opt.amount && selectedInfluence?.from === opt.from}
  ...
  <div class="mx-4 rounded-xl border border-amber-500/30 bg-amber-500/5 p-3" aria-live="polite">
    ...
    <ul class="mt-2 flex flex-col gap-1 text-sm">
      <li class="flex gap-2">
        ...
        <span class="{consequence.legal ? 'text-foreground' : 'text-destructive font-semibold'}">You {yourRosterSize} → {consequence.fromAfter} · {targetFranchiseName} {theirRosterSize} → {consequence.toAfter} {consequence.legal ? '· legal 10–15' : '· ILLEGAL — must stay 10–15'}</span>
      </li>
      <li class="flex gap-2"><span class="shrink-0 font-mono text-[10px] text-muted-foreground">Rotation:</span><span>Will rebuild around incoming minutes · starters/bench/closing five repaired deterministically</span></li>
      <li class="flex gap-2"><span class="shrink-0 font-mono text-[10px] text-muted-foreground">Availability:</span><span>{consequence.roleCoverage}</span></li>
      <li class="flex gap-2"><span class="shrink-0 font-mono text-[10px] text-muted-foreground">Chemistry:</span><span>{chemistryFootnote(consequence.chemistryRemoved, consequence.chemistryNew)}</span></li>
      <li class="flex gap-2"><span class="shrink-0 font-mono text-[10px] text-muted-foreground">Influence:</span><span>{consequence.influenceNote} · balances you {yourBalance} · them {theirBalance}</span></li>
    </ul>
    <p class="mt-2 font-mono text-[10px] text-muted-foreground">Never shows Overall, exact value ratio, threshold, or seeded RNG. Values are board facts only.</p>
  </div>

  <div class="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between border-t border-border bg-surface-2/40">
    <p class="font-mono text-[10px] text-muted-foreground">Browsing the board is free. Submitting consumes one inquiry. Duplicate fingerprints are rejected without consuming another exchange.</p>
    <button
      type="button"
      onclick={submit}
      disabled={!canSubmit}
