import type { SeasonFreeAgencyBand, SeasonFreeAgencyCandidate, SeasonFreeAgencyDeclaration, SeasonFreeAgencyRoleExpectation, SeasonFreeAgencySigning, SeasonFreeAgencyState, SeasonFreeAgencyTraceStep, SeasonFreeAgencyWindowState, } from '@hoop-rush/data-contracts';
...
export function openFreeAgencyWindowOf(freeAgency: SeasonFreeAgencyState | null): SeasonFreeAgencyWindowState | null {
    if (freeAgency === null)
        return null;
    return freeAgency.windows.find((window) => window.status === 'open') ?? null;
}
export function humanDeclarationOf(window: SeasonFreeAgencyWindowState, franchiseId: string | null): SeasonFreeAgencyDeclaration | null {
    if (franchiseId === null)
        return null;
    const parsedFranchiseId = franchiseIdSchema.parse(franchiseId);
    return window.declarations[parsedFranchiseId] ?? null;
}
export function humanSkipped(window: SeasonFreeAgencyWindowState, franchiseId: string | null): boolean {
...
export function interestedTeamsOf(window: SeasonFreeAgencyWindowState, candidatePlayerVersionId: string, humanFranchiseId: string | null): InterestedTeam[] {
    const teams: InterestedTeam[] = [];
    for (const declaration of Object.values(window.declarations)) {
        const priority = declaration.targets.findIndex((target) => target.playerVersionId === candidatePlayerVersionId);
        if (priority === -1)
            continue;
        teams.push({
...
export function humanSigningOf(window: SeasonFreeAgencyWindowState, franchiseId: string | null): SeasonFreeAgencySigning | null {
    if (franchiseId === null)
        return null;
    return window.signings.find((signing) => signing.franchiseId === franchiseId) ?? null;
}
export function candidateFitFacts(candidate: SeasonFreeAgencyCandidate, activeRotationIds: readonly string[], playableOf: (playerVersionId: string) => readonly string[], otherInterestedCount: number): CandidateFitFacts {
    const primary = candidate.positions.primary;
    let covering = 0;
    for (const playerVersionId of activeRotationIds) {
        if (playableOf(playerVersionId).includes(primary))
            covering += 1;
    }
    const needTier: NeedTier = covering <= 1 ? 'High' : covering === 2 ? 'Medium' : 'Low';
    const opportunity: Opportunity = otherInterestedCount === 0
        ? 'Immediate'
...
export function bestFitOrder(candidates: readonly SeasonFreeAgencyCandidate[], fitOf: (candidate: SeasonFreeAgencyCandidate) => CandidateFitFacts, limit = 5): string[] {
...
export function nextFreePriority(entries: Iterable<{
    priority: 1 | 2;
}>): 1 | 2 | null {
    const used = new Set<1 | 2>();
    for (const entry of entries)
        used.add(entry.priority);
    if (!used.has(1))
        return 1;
    if (!used.has(2))
        return 2;
...
export function validateDeclaration(input: DeclarationValidationInput): string[] {
    ...
    const byVersion = new Map(input.candidates.map((candidate) => [candidate.playerVersionId, candidate]));
    ...
    for (const target of input.targets) {
        ...
        if (!candidate.supportedRoles.includes(target.roleExpectation)) {
            failures.push(`${candidate.displayName} does not support the ${ROLE_EXPECTATION_LABEL[target.roleExpectation]} role.`);
        }
        if (target.influence < candidate.minimumInfluence ||
            target.influence > FREE_AGENCY_MAX_COMMITMENT) {
            failures.push(`${candidate.displayName} needs ${String(candidate.minimumInfluence)}-${String(FREE_AGENCY_MAX_COMMITMENT)} Influence.`);
        }
        committed += target.influence;
    }
    if (committed > input.balance) {
        failures.push(`The committed Influence (${String(committed)}) exceeds your balance (${String(input.balance)}).`);
    }
    const remainingBudget = FREE_AGENCY_SPEND_CAP - input.seasonSpend;
    if (committed > remainingBudget) {
        failures.push(`The committed Influence (${String(committed)}) exceeds the remaining season free-agency budget (${String(remainingBudget)} of ${String(FREE_AGENCY_SPEND_CAP)}).`);
    }
    return failures;
}
