...
export async function resolvePlayerRefs(refs: readonly PlayerRef[], manifest: HoopRushManifest): Promise<PeakPlayerSeason[]> {
    ...
    await Promise.all(uniqueKeys.map(async (key) => {
        ...
        const poolEntry = manifest.pools.find((p) => p.franchiseId === key.slice(0, slash) && p.eraId === key.slice(slash + 1));
        if (!poolEntry)
    ...
    return refs.map((ref) => {
        ...
        if (!player)
            throw new Error(`Drafted player ${ref.playerId} is unavailable.`);
        return player;
    });
}
