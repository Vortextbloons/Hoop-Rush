...
import { workerMessageSchema, workerRequestSchema, type ChallengeRun, type EraSimulationProfile, type GameResult, type WorkerMessage, } from '@hoop-rush/data-contracts';
...
export class ChallengeRunner {
    ...
    private async handleStartResult(envelope: Extract<WorkerMessage, {
        ...
        const updatedRun = envelope.chosenRunSeed === run.runSeed ? run : { ...run, runSeed: envelope.chosenRunSeed };
        if (updatedRun !== run) {
            try {
