import { canonicalJson, contentHashSchema, fixedFiveTimeoutMsForMode, idSchema, playerIdSchema, playerVersionId, type ClassicDraftCatalog, type ClassicDraftState, type ContentHash, type EraSimulationProfile, type FixedFiveCommand, type FixedFiveCommandPayload, type FixedFiveCompetitionResult, type FixedFiveCompetitionRun, type FixedFiveLineupEntry, type FixedFiveParticipantId, type FixedFiveRoomMode, type FixedFiveRoomPhase, type FixedFiveRoomSettings, type FixedFiveRoomSnapshot, type FixedFiveWorkerResultEntry, type FixedFiveWorkerTeam, type HoopRushManifest, type OpponentBracket, type PeakPlayerSeason, type PlayersIndex, type Seed, type SimulationPlayer, type SlotIndex, } from '@hoop-rush/data-contracts';
import { FIXED_FIVE_TIEBREAK_PATH, applyClassicBuilderCommand, applySandboxBuilderCommand, chooseAutopick, claimDuelPlayer, claimSandboxDuelPlayer, createDuelDraft, createEngineContext, createParticipantClassicDraft, createSandboxDuelDraft, duelCurrentPicker, createSandboxBuilder, enumerateClassicSafeMoves, enumerateDuelSafeMoves, enumerateSandboxDuelSafeMoves, enumerateSandboxSafeMoves, sandboxDuelPicker, fixedFiveDraftSeed, fixedFiveResultDigest, fixedFiveTiebreakWinner, rerollDuel, summarizeDuelGames, summarizeShared82Games, toSimulationPlayer, type AutopickSelection, type ClassicBuilderCommand, type DuelDraftState, type EngineContext, type FixedFiveCandidate, type SandboxBuilderState, type SandboxDuelState, } from '@hoop-rush/engine';
...
export function loadFixedFiveAssets(): Promise<FixedFiveAssets> {
    assetsPromise ??= (async () => {
        ...
        const pool: FixedFiveCandidate[] = index.players.map((entry) => ({
            ...
            playerVersionId: playerVersionId(entry.playerId, entry.franchiseId, entry.eraId, entry.seasonKey),
...
function applyClassicPayload(state: ClassicDraftState, assets: FixedFiveAssets, payload: FixedFiveCommandPayload): ClassicDraftState {
...
function applySandboxPayload(state: SandboxBuilderState, assets: FixedFiveAssets, payload: FixedFiveCommandPayload): SandboxBuilderState {
...
function applyDuelPayload(state: DuelDraftState, assets: FixedFiveAssets, actor: FixedFiveParticipantId, payload: FixedFiveCommandPayload): DuelDraftState {
    if (payload.kind === 'reroll') {
        return rerollDuel(state, assets.catalog, assets.poolById, payload.axis, actor, assets.context);
    }
    if (payload.kind === 'duel-claim' ||
        payload.kind === 'classic-pick' ||
        payload.kind === 'timeout-autopick') {
        return claimDuelPlayer(state, assets.catalog, assets.poolById, { playerId: payload.playerId, slotIndex: payload.slotIndex, actor }, assets.context);
    }
    return state;
}
function applySandboxDuelPayload(state: SandboxDuelState, assets: FixedFiveAssets, command: FixedFiveCommand): SandboxDuelState {
...
function isDraftPayload(mode: FixedFiveRoomMode, kind: FixedFiveCommandPayload['kind'], sandboxDuel = false): boolean {
    if (mode === 'duel') {
        if (sandboxDuel) {
...
export function replayFixedFiveLog(mode: FixedFiveRoomMode, roomId: string, rootSeed: Seed, dataVersion: string, variant: FixedFiveRoomSettings['variant'], assets: FixedFiveAssets, commands: FixedFiveCommand[], sourceMode: FixedFiveRoomSettings['sourceMode'] = 'classic'): DraftReplay {
    ...
    if (mode === 'duel' && sourceMode === 'sandbox') {
        ...
        for (const command of ordered) {
            if (!isDraftPayload(mode, command.payload.kind, true))
                continue;
            try {
    ...
    if (mode === 'duel') {
        ...
        let skipped = 0;
        for (const command of ordered) {
            if (!isDraftPayload(mode, command.payload.kind))
                continue;
            try {
    ...
    let p1 = createParticipantClassicDraft(draftIdFor(roomId, 'p1'), variant, fixedFiveDraftSeed(rootSeed, 'p1'), dataVersion, assets.catalog, assets.context);
    let p2 = createParticipantClassicDraft(draftIdFor(roomId, 'p2'), variant, fixedFiveDraftSeed(rootSeed, 'p2'), dataVersion, assets.catalog, assets.context);
    let skipped = 0;
    for (const command of ordered) {
        if (!isDraftPayload(mode, command.payload.kind))
            continue;
...
export function isDraftComplete(replay: DraftReplay): boolean {
    if (replay.mode === 'duel')
        return replay.state.status === 'complete';
    if (replay.mode === 'sandbox-duel')
        return replay.state.status === 'complete';
    if (replay.mode === 'sandbox-shared-82')
        return replay.p1.locked && replay.p2.locked;
    return replay.p1.status === 'complete' && replay.p2.status === 'complete';
}
export function isFixedFiveDraftTurn(replay: DraftReplay, participant: FixedFiveParticipantId): boolean {
    if (replay.mode === 'duel') {
        return (replay.state.status === 'drafting' &&
...
export function mergeFixedFiveCommands(existing: FixedFiveCommand[], incoming: FixedFiveCommand[]): FixedFiveCommand[] {
    ...
    for (const command of [...existing, ...incoming]) {
        ...
        if (atOrdinal) {
            if (atOrdinal.commandId !== command.commandId ||
...
export function restoreFixedFiveCommandSyncState(storedCommands: FixedFiveCommand[]): {
    ...
    for (const command of merged) {
        if (command.ordinal !== lastOrdinal + 1)
            break;
...
export function roomLogFacts(commands: FixedFiveCommand[]): RoomLogFacts {
    ...
    for (const command of [...commands].sort((a, b) => a.ordinal - b.ordinal)) {
        ...
        if (payload.kind === 'ready')
            facts.ready[command.actorParticipantId] = payload.ready;
...
export function deriveEffectivePhase(serverPhase: FixedFiveRoomPhase, replay: DraftReplay, localSimDone: boolean): FixedFiveRoomPhase {
    if (serverPhase !== 'lobby')
        return serverPhase;
    if (!replay.hasStart)
        return 'lobby';
    if (!isDraftComplete(replay))
        return 'drafting';
    if (!localSimDone)
        return 'simulating';
    return 'awaiting-confirmation';
}
export function overlaySnapshotProgress(snapshot: FixedFiveRoomSnapshot, replay: DraftReplay, facts: RoomLogFacts): FixedFiveRoomSnapshot {
    return {
        ...snapshot,
...
export function refsForParticipant(replay: DraftReplay, assets: FixedFiveAssets, participant: FixedFiveParticipantId): PickRef[] {
    if (replay.mode === 'duel') {
    ...
    if (replay.mode === 'sandbox-duel') {
        return replay.state.picks
            ...
            if (!candidate)
                throw new Error(`sandbox duel pick ${pick.playerId} has no pool record`);
            return {
    ...
    if (replay.mode === 'sandbox-shared-82') {
        ...
        return builder.placements.map((placement) => {
            ...
            if (!candidate)
                throw new Error(`sandbox pick ${placement.playerId} has no pool record`);
            return {
...
export async function buildSimulationTeam(manifest: HoopRushManifest, teamId: string, displayName: string, refs: PickRef[]): Promise<FixedFiveWorkerTeam> {
    ...
    return {
        lineup: {
            ...
            assignments: refs.map((ref) => {
                ...
                if (!player)
                    throw new Error(`lineup is missing snapshot for ${ref.playerId}`);
...
export function pickOrdinalOf(replay: DraftReplay, participant: FixedFiveParticipantId): number {
    if (replay.mode === 'duel')
        return replay.state.pickOrdinal;
    if (replay.mode === 'sandbox-duel')
        return replay.state.pickOrdinal;
    if (replay.mode === 'sandbox-shared-82') {
...
export function computeDueAutopick(mode: FixedFiveRoomMode, rootSeed: Seed, replay: DraftReplay, assets: FixedFiveAssets, participant: FixedFiveParticipantId): AutopickSelection | null {
    if (!replay.hasStart || isDraftComplete(replay))
        return null;
    ...
    if (mode === 'duel') {
        if (replay.mode === 'sandbox-duel') {
            if (!isFixedFiveDraftTurn(replay, participant))
                return null;
