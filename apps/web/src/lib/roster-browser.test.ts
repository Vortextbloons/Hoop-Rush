...
import { defaultDirection, filterRoster, groupRoster, lowercaseName, paginateGroupedRows, paginateItems, sortRoster, type RosterDetailRow, } from './roster-browser';
function row(partial: Omit<Partial<RosterDetailRow>, 'playerId' | 'franchiseId' | 'eraId' | 'seasonKey'> & {
    playerId: string;
    franchiseId?: string;
    eraId?: string;
    seasonKey?: string;
}): RosterDetailRow {
    const { playerId, franchiseId = 'lakers', eraId = '1990s', seasonKey = '1990-91', ...rest } = partial;
    return {
