...
import { defaultDirection, filterRoster, formatPct, formatPerGame, groupRoster, lowercaseName, paginateGroupedRows, paginateItems, perGame, shotPct, sortRoster, type RosterDetailRow, } from './roster-browser';
function row(partial: Partial<RosterDetailRow> & {
    playerId: string;
}): RosterDetailRow {
    return {
        franchiseId: 'lakers',
...
describe('filterRoster', () => {
    ...
    it('applies franchise, decade, position, and query filters together', () => {
        expect(filterRoster([lakers90, bulls90, lakers00], {
            franchiseId: 'lakers',
            eraId: null,
            position: null,
            query: '',
        }).map((r) => r.playerId)).toEqual(['a', 'c']);
        expect(filterRoster([lakers90, bulls90, lakers00], {
            franchiseId: null,
            eraId: '1990s',
            position: 'SF',
            query: '',
        }).map((r) => r.playerId)).toEqual(['b']);
        expect(filterRoster([lakers90, bulls90, lakers00], {
            franchiseId: null,
            eraId: null,
            position: null,
            query: 'shaq',
        }).map((r) => r.playerId)).toEqual(['c']);
        expect(filterRoster([lakers90, bulls90, lakers00], {
            franchiseId: null,
            eraId: null,
            position: null,
...
describe('paginateItems', () => {
    it('keeps group headers leading into the page and counts only players', () => {
        ...
        expect(page.map((item) => (item.type === 'player' ? item.player.playerId : item.type))).toEqual(['group', 'a', 'group', 'b']);
    });
});
describe('paginateGroupedRows', () => {
    ...
    it('stops at exactly count player items', () => {
        ...
        expect(page.map((item) => (item.type === 'player' ? item.player.playerId : item.type))).toEqual(['group', 'a', 'b', 'group', 'c', 'd', 'e']);
...
describe('stat helpers', () => {
    it('computes per-game values and guards zero games', () => {
        ...
        expect(perGame(row({ playerId: 'a', stats: { ...row({ playerId: 'x' }).stats, gamesPlayed: 0 } }).stats, 'points')).toBe(0);
    });
