import type { PlayersIndexEntry, PlayerSeasonStats, RosterDetailsEntry, } from '@hoop-rush/data-contracts';
...
export type RosterSortId = 'none' | 'name' | 'overall' | 'offense' | 'defense' | 'points' | 'per' | 'season' | 'team' | 'decade' | 'position';
...
function compareBy<T>(rows: RosterDetailRow[], value: (row: RosterDetailRow) => T, direction: RosterSortDirection, compare: (a: T, b: T) => number): RosterDetailRow[] {
...
export function sortRoster(rows: RosterDetailRow[], sortId: RosterSortId, direction: RosterSortDirection): RosterDetailRow[] {
    switch (sortId) {
        case 'none':
            return [...rows];
        case 'name':
        ...
        case 'position':
            return compareBy(rows, (r) => POSITION_ORDER[r.positionsPlayable[0] ?? ''] ?? 99, direction, compareNumber);
    }
}
export function filterRoster(rows: RosterDetailRow[], filters: RosterFilters): RosterDetailRow[] {
    ...
    if (!franchiseId && !eraId && !position && !query)
        return rows;
    return rows.filter((row) => (!franchiseId || row.franchiseId === franchiseId) &&
...
export function paginateItems<T extends {
    type: string;
...
export function paginateGroupedRows<T extends PlayersIndexEntry>(rows: T[], count: number): RosterListItem<T>[] {
