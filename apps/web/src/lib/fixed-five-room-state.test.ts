...
import { commandIdSchema, eraIdSchema, franchiseIdSchema, idSchema, playerIdSchema, seedSchema, } from '@hoop-rush/data-contracts';
...
import { computeDueAutopick, deriveEffectivePhase, isDraftComplete, isFixedFiveDraftTurn, mergeFixedFiveCommands, pickOrdinalOf, picksCommittedOf, refsForParticipant, replayFixedFiveLog, restoreFixedFiveCommandSyncState, roomLogFacts, type DraftReplay, type FixedFiveAssets, } from '$lib/fixed-five-room-state';
const ROOM = idSchema.parse('room-1');
const ROOT = seedSchema.parse('0123456789abcdef0123456789abcdef');
function duelReplay(overrides: Partial<Extract<DraftReplay, {
    mode: 'duel';
}>> = {}): DraftReplay {
    return {
        mode: 'duel',
...
function command(ordinal: number, actor: 'p1' | 'p2', payload: FixedFiveCommand['payload']): FixedFiveCommand {
    return {
        schemaVersion: 1,
        roomId: ROOM,
...
describe('mergeFixedFiveCommands', () => {
    ...
    it('rejects contradictory commands at the same accepted ordinal', () => {
        ...
        expect(() => mergeFixedFiveCommands([first], [conflict])).toThrow('fixed-five command log conflicts at ordinal 4');
    });
});
...
describe('deriveEffectivePhase', () => {
    ...
    it('walks lobby, drafting, simulating, and awaiting-confirmation from the log', () => {
        ...
        expect(deriveEffectivePhase('lobby', duelReplay({ state: completeState }), false)).toBe('simulating');
        expect(deriveEffectivePhase('lobby', duelReplay({ state: completeState }), true)).toBe('awaiting-confirmation');
    });
...
describe('sandbox duel replay', () => {
    ...
    it('replays alternating free picks from sandbox-place commands', () => {
        const replay = replayFixedFiveLog('duel', ROOM, ROOT, 'data-v1', 'ratings', poolAssets(), [
    ...
    it('keeps classic-sourced duels on the roll draft', () => {
        const replay = replayFixedFiveLog('duel', ROOM, ROOT, 'data-v1', 'ratings', poolAssets(), [command(0, 'p1', { kind: 'start' }), place(1, 'p1', 'player-g1', 0)], 'classic');
