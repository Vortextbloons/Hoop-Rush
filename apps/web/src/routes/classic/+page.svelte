<script lang="ts">
  ...
  async function discardAndLeave() {
    ...
    void goto(resolve((raw.startsWith(base) ? raw.slice(base.length) : raw) as any));
  }
</script>
...
<section class="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
  ...
  {:else}
    ...
    {:else if !draftLoaded}
      <div class="mt-8">
        <AsyncState kind="loading" title="Loading draft…" message="Picking up your picks." />
      </div>
    {:else if !draft}
      ...
      <div class="mt-10 flex flex-col gap-6 pb-32">
        ...
        {:else if draft.status === 'complete'}
          ...
          <div>
            ...
            >
              {starting ? 'Starting…' : launchError ? 'Try again' : 'Start season'}
            </button>
          </div>
        {/if}
