<script lang="ts">
  ...
  function handleSubmit(payload: { toFranchiseId: string; outgoing: string[]; incoming: string[]; influenceAmount: number; influenceFromSender: string | null }): void {
    if (!mounted || windowState === null || busy) return;
  ...
  // helpers for PackageBuilder: playable and available
