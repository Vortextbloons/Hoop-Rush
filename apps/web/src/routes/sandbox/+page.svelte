<script lang="ts">
  ...
  const availabilityMatrix = $derived(
    manifest
      ? new Map(
          manifest.availability.map((entry) => [`${entry.franchiseId}/${entry.eraId}`, entry]),
        )
  ...
  $effect(() => {
    ...
    const entry = manifest.pools.find((p) => p.franchiseId === franchiseId && p.eraId === eraId);
    if (!entry) {
      pool = null;
      ...
      {#if selectedAvailability && selectedAvailability.status === 'unavailable'}
        ...
        >
          ...
          <p class="mt-2 text-sm text-muted-foreground">
            {unavailableReasonText(selectedAvailability)}
          </p>
        ...
        <div class="mt-10 flex flex-col gap-6 pb-32">
          <div class="rounded-xl border border-border bg-card">
            <div class="flex items-start justify-between gap-3 border-b border-border px-4 py-3">
              ...
              <div
                class="shrink-0 text-right font-mono text-[10px] tracking-[0.14em] text-muted-foreground uppercase"
              >
