...
function post(message: SeasonWorkerProgressMessage | SeasonWorkerCompleteMessage | SeasonWorkerErrorMessage | SeasonWorkerWarmAckMessage): void {
...
function expandRostersCached(run: SeasonBlockRunContext, catalog: SeasonDraftCatalog): Map<string, SeasonGamePlayerInput> {
...
async function runBlock(request: SeasonWorkerStartRequest): Promise<void> {
    ...
    if (request.priorSummaries !== undefined) {
        ...
        if (accumulatedRunId !== request.runId) {
            postError(request.requestId, 'internal', 'summary delta received for a run the worker has no state for');
            return;
        }
    ...
    if (request.priorEffects !== undefined && request.priorEffects !== null) {
        ...
        postError(request.requestId, 'internal', 'effects state missing for a run the worker has no state for');
        return;
    }
    ...
    const input: SeasonBlockSimulationInput = {
        ...
        campaignState: (run as unknown as {
            campaign?: import('@hoop-rush/data-contracts').SeasonCampaignState;
        }).campaign,
    };
    const rejection = seasonBlockRejection(input);
    if (rejection !== null) {
        postError(request.requestId, 'internal', `block submission rejected by the engine: ${rejection.code}`);
        return;
    }
    ...
    if (request.startGameId !== null) {
        summaries = accumulatedSummaries.filter((summary) => blockIndexForRound(summary.round) === request.blockIndex);
        retainedDetails = [];
    }
    else {
        summaries = [];
    ...
    const startIndex = request.startGameId === null
        ? 0
        : games.findIndex((game) => game.gameId === request.startGameId);
    if (startIndex < 0) {
        postError(request.requestId, 'internal', `startGameId ${String(request.startGameId)} is not a game of block ${String(request.blockIndex)}`);
        return;
    }
    const remainingGames = games.slice(startIndex);
    const { fromRound } = blockRoundRange(request.blockIndex);
    const precedingRound = startIndex > 0 ? (games[startIndex - 1]?.round ?? fromRound) : fromRound - 1;
    ...
    const gameNumberById = new Map(input.schedule.games.map((game, index) => [game.gameId, index + 1]));
    const rotationByFranchise = new Map(input.run.rotations.map((rotation) => [rotation.franchiseId, rotation]));
    ...
    const candidate = assembleSeasonBlockCandidate(input, summaries, retainedDetails, effects, health);
    ...
    accumulatedSummaries = [
        ...accumulatedSummaries.filter((summary) => blockIndexForRound(summary.round) !== request.blockIndex),
        ...summaries,
    ];
    post({
        schemaVersion: SEASON_WORKER_WIRE_SCHEMA_VERSION,
...
function initialEffects(expanded: ReadonlyMap<string, import('@hoop-rush/data-contracts').SeasonGamePlayerInput>): SeasonEffectsState {
...
self.onmessage = (event: MessageEvent<unknown>): void => {
    ...
    cancelled = false;
    const startRequest = request.type === 'season-block-continue' ? synthesizeStart(request) : request;
    if (startRequest === null) {
        postError(request.requestId, 'internal', 'continuation for a run the worker has no context for');
        return;
    }
    void runBlock(startRequest).catch((error: unknown) => {
        ...
        if (error instanceof SeasonBlockInvariantError) {
            postError(request.requestId, 'invariant-failure', error.message, {
                seed: error.diagnostics.seed !== undefined
                    ? seedSchema.parse(error.diagnostics.seed)
                    : request.rootSeed,
                gameId: error.diagnostics.gameId !== undefined
